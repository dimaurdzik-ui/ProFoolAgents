...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed Pixel tool
gateway (which Pixel Agents uses to bill Browser Use sessions to a Pixel
...
Auth env vars (one of)::
    ...
    # OR a managed Pixel gateway entry (configured via 'pixel-agents setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed Pixel
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed Pixel tool gateway when ``tool_gateway.browser`` config
    ...
    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        ...
        # Keep availability scans off the synchropixel OAuth refresh path.
        managed = resolve_managed_tool_gateway(
            "browser-use",
            token_reader=None if refresh_token else peek_pixel_access_token,
        )
        if managed is None:
            return None

        return {
            "api_key": managed.pixel_user_token,
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Pixel Agents only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"pixel_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
