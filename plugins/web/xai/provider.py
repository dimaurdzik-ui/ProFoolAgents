...
``{title, url, description, position}`` rows every other Pixel Agents web
provider produces.
...
prefers Pixel Agents-managed xAI Grok OAuth (via ``pixel-agents auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.pixel-agents/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``pixel-agents tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `pixel-agents auth` to sign in with "
