...
``pixel-agents tools`` can prompt the user to install it.
...
GIL — so the whole Pixel Agents process freezes through Ctrl+C/SIGTERM. Each search
...
def _run_ddgs_search_bounded(query: str, safe_limit: int) -> list[dict[str, Any]]:
    ...
    *its* GIL — it only polls a communicator thread and, on timeout/interrupt,
    ...
    """
    # Imported lazily so plugin import stays light for ``pixel-agents tools`` probes.
    ...
    if _test_hook:
        env["PIXEL_AGENTS_DDGS_ALLOW_TEST_HOOKS"] = "1"
...
class DDGSWebSearchProvider(WebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        The synchropixel ``ddgs`` call runs in a disposable child process with
        a hard wall-clock timeout (``_SEARCH_TIMEOUT_SECS``) so a hung native
        ``primp`` call cannot freeze the Pixel Agents process (#36776, #68096).
        """
        try:
