/**
 * Pixel Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.pixel-agents/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__PIXEL_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__PIXEL_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "pixel-agents-kanban-dot-triage",
    todo: "pixel-agents-kanban-dot-todo",
    ready: "pixel-agents-kanban-dot-ready",
    running: "pixel-agents-kanban-dot-running",
    blocked: "pixel-agents-kanban-dot-blocked",
    done: "pixel-agents-kanban-dot-done",
    archived: "pixel-agents-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-pixel-agents-task";
  ...
  const DOCS_URL = "https://api.pixelagents.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://api.pixelagents.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "pixel-agents.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="pixel-agents-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])pixel-agents-kanban-md-code\1/i.test(attrs)) {
      return ' class="pixel-agents-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "pixel-agents-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "pixel-agents-kanban-md",
  ...
  // standard `drop` event and our `pixel-agents-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("pixel-agents-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("pixel-agents-kanban-column--drop");
          if (target) target.classList.add("pixel-agents-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("pixel-agents-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("pixel-agents-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("pixel-agents-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__PIXEL_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "pixel-agents-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "pixel-agents-kanban-attention",
        "pixel-agents-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "pixel-agents-kanban-attention-bar" },
        h("span", { className: "pixel-agents-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "pixel-agents-kanban-attention-text" },
          diagTasks.length === 1
        ...
        h("button", {
          className: "pixel-agents-kanban-attention-toggle",
          onClick: function () { setExpanded(function (x) { return !x; }); },
          type: "button",
        }, expanded ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
        h("button", {
          className: "pixel-agents-kanban-attention-dismiss",
      ...
      expanded
        ? h("div", { className: "pixel-agents-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "pixel-agents-kanban-attention-row",
                  "pixel-agents-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "pixel-agents-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "pixel-agents-kanban-attention-row-id" }, task.id),
                h("span", { className: "pixel-agents-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "pixel-agents-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "pixel-agents-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "pixel-agents-kanban-diag-action-btn",
      action.suggested ? "pixel-agents-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " pixel-agents-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".pixel-agents-kanban-drawer-comment-row input, .pixel-agents-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "pixel-agents-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("pixel-agents-kanban-diag", sevClass) },
      h("div", { className: "pixel-agents-kanban-diag-header" },
        h("span", { className: "pixel-agents-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "pixel-agents-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "pixel-agents-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "pixel-agents-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "pixel-agents-kanban-diag-data-row" },
                  h("span", { className: "pixel-agents-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "pixel-agents-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "pixel-agents-kanban-diag-data-row" },
                h("span", { className: "pixel-agents-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "pixel-agents-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "pixel-agents-kanban-diag-reassign-row" },
            h("span", { className: "pixel-agents-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "pixel-agents-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "pixel-agents-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "pixel-agents-kanban-diag-msg",
              msg.ok ? "pixel-agents-kanban-diag-msg--ok" : "pixel-agents-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "pixel-agents-kanban-section" },
      h("div", { className: "pixel-agents-kanban-section-head-row" },
        h("span", { className: "pixel-agents-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "pixel-agents-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "pixel-agents-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "pixel-agents-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "pixel-agents-kanban-docs-link",
      title: "Open Pixel Kanban docs in a new tab",
      "aria-label": "Pixel Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `Pixel Kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "pixel-agents-kanban-msg-ok" : "pixel-agents-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "pixel-agents-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "pixel-agents-kanban-boardswitcher" },
      h("div", { className: "pixel-agents-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "pixel-agents-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "pixel-agents-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "pixel-agents-kanban-dialog-title" },
        ...
        h("div", { className: "pixel-agents-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned pixel-agents profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "pixel-agents-kanban-bulk" },
      h("span", { className: "pixel-agents-kanban-bulk-count" },
      ...
      h("div", { className: "pixel-agents-kanban-bulk-priority",
      ...
      h("div", { className: "pixel-agents-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different pixel-agents profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "pixel-agents-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("pixel-agents-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("pixel-agents-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "pixel-agents-kanban-trash",
        dragOver ? "pixel-agents-kanban-trash--drop" : "",
        props.draggingTaskId ? "pixel-agents-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "pixel-agents-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "pixel-agents-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".pixel-agents-kanban-card")) return true;
      if (target.closest && target.closest(".pixel-agents-kanban-column-add")) return true;
      if (target.closest && target.closest(".pixel-agents-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("pixel-agents-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("pixel-agents-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".pixel-agents-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "pixel-agents-kanban-columns",
        isScrollable ? "pixel-agents-kanban-columns--scrollable" : "",
        isPanning ? "pixel-agents-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("pixel-agents-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("pixel-agents-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "pixel-agents-kanban-column",
        dragOver ? "pixel-agents-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "pixel-agents-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "pixel-agents-kanban-col-check",
        ...
        h("span", { className: cn("pixel-agents-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "pixel-agents-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "pixel-agents-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "pixel-agents-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "pixel-agents-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "pixel-agents-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "pixel-agents-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "pixel-agents-kanban-lane" },
                  h("div", { className: "pixel-agents-kanban-lane-head" },
                    h("span", { className: "pixel-agents-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "pixel-agents-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "pixel-agents-kanban-card--stale-red";
    if (age >= tier.amber) return "pixel-agents-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".pixel-agents-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "pixel-agents-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "pixel-agents-kanban-card",
        props.selected ? "pixel-agents-kanban-card--selected" : "",
        props.failed ? "pixel-agents-kanban-card--failed" : "",
        props.draggingSource ? "pixel-agents-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "pixel-agents-kanban-card-content" },
          h("div", { className: "pixel-agents-kanban-card-row" },
            h("label", {
              className: "pixel-agents-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "pixel-agents-kanban-card-check",
            ...
            h("span", { className: "pixel-agents-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or Pixel Kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "pixel-agents-kanban-warning-badge",
                    "pixel-agents-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
              ...
              : null,
            t.priority > 0
              ? h(Badge, { className: "pixel-agents-kanban-priority",
                           title: `Priority ${t.priority}. Higher-priority tasks are claimed first by the dispatcher.` }, `P${t.priority}`)
              : null,
            t.tenant
              ? h(Badge, { variant: "outline", className: "pixel-agents-kanban-tag",
                           title: `Tenant: ${t.tenant}. Free-form tag for grouping tasks (customer, project, team).` }, t.tenant)
              : null,
            progress
              ? h("span", {
                  className: cn(
                    "pixel-agents-kanban-progress",
                    progress.done === progress.total ? "pixel-agents-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "pixel-agents-kanban-needs-assignee",
          ...
          h("div", { className: "pixel-agents-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "pixel-agents-kanban-card-row pixel-agents-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "pixel-agents-kanban-assignee",
                            title: `Assigned to pixel-agents profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "pixel-agents-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "pixel-agents-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "pixel-agents-kanban-ago",
  ...
  // centered modal reusing the pixel-agents-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "pixel-agents-kanban-dialog pixel-agents-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "pixel-agents profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "pixel-agents profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "pixel-agents-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "pixel-agents-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "pixel-agents-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "pixel-agents-kanban-drawer-close",
        ...
        data ? h("div", { className: "pixel-agents-kanban-drawer-comment-foot" },
          h("div", {
            className: "pixel-agents-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "pixel-agents-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "pixel-agents-kanban-section" },
      h("div", { className: "pixel-agents-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "pixel-agents-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "pixel-agents-kanban-drawer-body" },
      h("div", { className: "pixel-agents-kanban-drawer-title" },
        h("span", { className: cn("pixel-agents-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "pixel-agents-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "pixel-agents-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "pixel-agents-kanban-section" },
            h("div", { className: "pixel-agents-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "pixel-agents-kanban-section" },
            h("div", { className: "pixel-agents-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "pixel-agents-kanban-done-no-result pixel-agents-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "pixel-agents-kanban-section" },
            h("div", { className: "pixel-agents-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "pixel-agents-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "pixel-agents-kanban-section" },
        h("div", { className: "pixel-agents-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "pixel-agents-kanban-comment" },
            h("div", { className: "pixel-agents-kanban-comment-head" },
              h("span", { className: "pixel-agents-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "pixel-agents-kanban-diag-action-btn",
      ...
      h("div", { className: "pixel-agents-kanban-section" },
        h("div", { className: "pixel-agents-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "pixel-agents-kanban-comment" },
                h("div", { className: "pixel-agents-kanban-comment-head" },
                  h("span", { className: "pixel-agents-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "pixel-agents-kanban-comment-ago" },
      ...
      h("div", { className: "pixel-agents-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "pixel-agents-kanban-event",
              isDiag ? "pixel-agents-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "pixel-agents-kanban-event-header" },
                  h("span", { className: "pixel-agents-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "pixel-agents-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "pixel-agents-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "pixel-agents-kanban-event-header-plain" },
                  h("span", { className: "pixel-agents-kanban-event-kind" }, e.kind),
                  h("span", { className: "pixel-agents-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "pixel-agents-kanban-event-phantom-row" },
                  h("span", { className: "pixel-agents-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "pixel-agents-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "pixel-agents-kanban-event-payload" },
    ...
    return h("div", { className: "pixel-agents-kanban-section" },
      h("div", { className: "pixel-agents-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "pixel-agents-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `pixel-agents-kanban-run--${r.outcome || r.status || "ended"}`
          : "pixel-agents-kanban-run--active";
        return h("div", { key: r.id, className: cn("pixel-agents-kanban-run", outcomeClass) },
          h("div", { className: "pixel-agents-kanban-run-head" },
            h("span", { className: "pixel-agents-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "pixel-agents-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "pixel-agents-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "pixel-agents-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "pixel-agents-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "pixel-agents-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "pixel-agents-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "pixel-agents-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "pixel-agents-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "pixel-agents-kanban-pre pixel-agents-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "pixel-agents-kanban-meta-row" },
      h("span", { className: "pixel-agents-kanban-meta-label" }, props.label),
      h("span", { className: "pixel-agents-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "pixel-agents-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "pixel-agents-kanban-meta-row" },
        h("span", { className: "pixel-agents-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "pixel-agents-kanban-meta-value pixel-agents-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "pixel-agents-kanban-meta-row" },
        h("span", { className: "pixel-agents-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "pixel-agents-kanban-meta-value pixel-agents-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "pixel-agents-kanban-meta-row" },
        h("span", { className: "pixel-agents-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "pixel-agents-kanban-meta-value pixel-agents-kanban-editable",
    ...
    return h("div", { className: "pixel-agents-kanban-meta-row" },
      h("span", { className: "pixel-agents-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "pixel-agents-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "pixel-agents-kanban-section" },
      h("div", { className: "pixel-agents-kanban-section-head-row" },
        h("span", { className: "pixel-agents-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "pixel-agents-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "pixel-agents-kanban-section" },
      h("div", { className: "pixel-agents-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "pixel-agents-kanban-deps-row" },
        h("span", { className: "pixel-agents-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "pixel-agents-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "pixel-agents-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "pixel-agents-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "pixel-agents-kanban-dep-chip-x",
      ...
      h("div", { className: "pixel-agents-kanban-deps-row" },
        h("span", { className: "pixel-agents-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "pixel-agents-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "pixel-agents-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "pixel-agents-kanban-msg-ok"
          : "pixel-agents-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "pixel-agents-kanban-section" },
      ...
      h("div", { className: "pixel-agents-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "pixel-agents-kanban-home-sub pixel-agents-kanban-home-sub--on"
              : "pixel-agents-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__PIXEL_PLUGINS__ && typeof window.__PIXEL_PLUGINS__.register === "function") {
    window.__PIXEL_PLUGINS__.register("kanban", KanbanPage);
  }
})();
