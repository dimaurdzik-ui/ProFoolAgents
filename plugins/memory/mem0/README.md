...
```bash
pixel-agents memory setup    # select "mem0"
```
...
Behavioral settings live in `$PIXEL_AGENTS_HOME/mem0.json` (set them via `pixel-agents memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.pixel-agents/.env`.
...
| `user_id` | `pixel-agents-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
   or in `$PIXEL_AGENTS_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh Pixel Agents session and call `mem0_search` — it connects to your server.
...
```bash
pixel-agents memory setup mem0 --mode oss \
...
```bash
pixel-agents memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$PIXEL_AGENTS_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.pixel-agents/mem0_qdrant"}}
  }
}
```
...
```bash
pixel-agents memory setup mem0 --mode platform --api-key sk-...
```
...
ls -la ~/.pixel-agents/mem0_qdrant
...
- Check `user_id` matches between sessions (`$PIXEL_AGENTS_HOME/mem0.json`).
