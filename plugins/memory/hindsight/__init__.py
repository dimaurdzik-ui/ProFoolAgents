...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: pixel-agents)
...
Or via $PIXEL_AGENTS_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Pixel Agents can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``pixel-agents memory status`` would stay green while the daemon
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $PIXEL_AGENTS_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_pixel_agents_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "pixel-agents": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "pixel-agents"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Pixel Agents config."""
    profile = config.get("profile", "pixel-agents")
    return str(profile or "pixel-agents")
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Pixel Agents .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Pixel Agents profile name (from agent_identity)
      {workspace} — Pixel Agents workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``pixel-agents-{user}`` with no user becomes ``pixel-agents``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Pixel Agents and the User"
    ...
    def save_config(self, values, pixel_home):
        """Write config to $PIXEL_AGENTS_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(pixel_home) / "hindsight"
    ...
    def post_setup(self, pixel_home: str, config: dict) -> None:
        ...
        from pixel_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        provider_config.setdefault("bank_id", "pixel-agents")
        ...
        self.save_config(provider_config, pixel_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(pixel_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(pixel_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "pixel-agents"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: pixel-agents-{profile}", "default": ""},
            ...
            {"key": "retain_async","description": "Process retain asynchropixelly on the Hindsight server", "default": True},
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Pixel Agents and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "pixel-agents"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "pixel-agents"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            if Version(installed) < Version(_MIN_CLIENT_VERSION):
                ...
                # durable data-volume target instead of /opt/pixel-agents (NS-605).
                from tools.lazy_deps import install_specs
        ...
        banks = cfg_get(self._config, "banks", "pixel-agents", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "pixel-agents")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Pixel Agents and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Pixel Agents as a non-root user, or switch "
                    "to cloud / local_external mode via 'pixel-agents memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Pixel Agents get sluggish. (issue #13125)
                try:
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See pixel-agents#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Pixel Agents created/used that client on the shared async loop,
