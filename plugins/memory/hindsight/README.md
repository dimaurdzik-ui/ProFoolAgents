...
Pixel Agents spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.pixel-agents/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p pixel-agents ui start
```
...
Config file: `~/.pixel-agents/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `pixel-agents` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `pixel-agents-{profile}` isolates memory per active Pixel Agents profile. Empty placeholders collapse cleanly (e.g. `pixel-agents-{user}` with no user becomes `pixel-agents`). |
...
| `retain_async` | `true` | Process retain asynchropixelly on the Hindsight server |
...
The LLM API key is stored in `~/.pixel-agents/.env` as `HINDSIGHT_LLM_API_KEY`.
