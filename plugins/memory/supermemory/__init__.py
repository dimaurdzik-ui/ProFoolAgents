...
_DEFAULT_CONTAINER_TAG = "pixel-agents"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=pixel-agents"
...
def _load_supermemory_config(pixel_home: str) -> dict:
    config = _default_config()
    config_path = Path(pixel_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, pixel_home: str) -> None:
    config_path = Path(pixel_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "pixel-agents"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Pixel Agents writes into the "Pixel Agents" Space in the Supermemory
        ...
        merged = {"sm_source": "pixel-agents", **(metadata or {})}
...
def _resolve_container_tag_for_setup(pixel_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(pixel_home)
...
def _probe_supermemory_connection(api_key: str, pixel_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(pixel_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(pixel_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `pixel-agents memory setup`.
        # All other options are documented for $PIXEL_AGENTS_HOME/supermemory.json
    ...
    def save_config(self, values, pixel_home):
        ...
        _save_supermemory_config(sanitized, pixel_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, pixel_home)
    ...
    def post_setup(self, pixel_home: str, config: dict) -> None:
        ...
        from pixel_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(pixel_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from pixel_constants import get_pixel_agents_home
        self._pixel_home = kwargs.get("pixel_home") or str(get_pixel_agents_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._pixel_home)
