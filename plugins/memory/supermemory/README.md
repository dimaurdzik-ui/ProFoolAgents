...
- Hosted: API key from [app.supermemory.ai/integrations?connect=pixel-agents](http://app.supermemory.ai/integrations?connect=pixel-agents)
...
Before running `pixel-agents memory setup`, add the local endpoint to
...
Then run `pixel-agents memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$PIXEL_AGENTS_HOME/supermemory.json`
...
| `container_tag` | `pixel-agents` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `pixel-agents-{identity}` → `pixel-agents-coder`). |
...
`https://api.supermemory.ai`. Pixel Agents resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: pixel-agents`, and document writes stamp
`metadata.sm_source: pixel-agents`. This is a **functional routing key, not telemetry**:
it groups Pixel Agents-written memories into a dedicated "Pixel Agents" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Pixel Agents profile:

```json
{
  "container_tag": "pixel-agents-{identity}"
}
```

For a profile named `coder`, this resolves to `pixel-agents-coder`. The default profile resolves to `pixel-agents-default`. Without `{identity}`, all profiles share the same container.
