"""pixel-agents-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $PIXEL_AGENTS_HOME/config.yaml (profile-scoped):
  plugins:
    pixel-agents-memory-store:
      db_path: $PIXEL_AGENTS_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "pixel-agents-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, pixel_home):
        """Write config to config.yaml under plugins.pixel-agents-memory-store."""
        from pathlib import Path
        config_path = Path(pixel_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["pixel-agents-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_pixel_agents_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _pixel_home = str(get_pixel_agents_home())
        _default_db = _pixel_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $PIXEL_AGENTS_HOME in user-supplied paths so config values like
        # "$PIXEL_AGENTS_HOME/memory_store.db" or "~/.pixel-agents/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$PIXEL_AGENTS_HOME", _pixel_home)
            db_path = db_path.replace("${PIXEL_AGENTS_HOME}", _pixel_home)
