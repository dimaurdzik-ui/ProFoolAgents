...
``pixelagents://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Pixel Agents OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "pixel-agents"
...
def begin_authorization(
    ...
    *,
    ...
    (``pixel-agents-desktop`` / ``pixel-agents-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Pixel Agents.</div>"
)
...
def start_loopback_flow_background(
    *,
    config_path: Path | None = None,
    host: str | None = None,
    source: str = "pixel-agents-desktop",
    timeout: float = 300.0,
) -> dict[str, str]:
    ...
    # the request, where a context-local PIXEL_AGENTS_HOME override can't reach.
