...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/pixel-agents>
...
```bash
pixel-agents memory setup honcho   # configure Honcho directly (works on a fresh install)
pixel-agents memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
pixel-agents config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.pixel-agents/.env
```

> `pixel-agents honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `pixel-agents memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `pixel-agents model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$PIXEL_AGENTS_HOME/honcho.json` | Profile-local (isolated Pixel Agents instances) |
| 2 | `~/.pixel-agents/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Pixel Agents profile: `pixel-agents` (default) or `pixel_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `pixel-agents honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Pixel Agents session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`pixel-agents`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`pixel-agents`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-pixel-agents`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening pixel-agents in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Pixel Agents session ID (timestamp + hex). Every `pixel-agents` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Pixel Agents profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`pixel-agents`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Pixel Agents profile: `pixel-agents` (default) or `pixel_<profile>` (e.g. `pixel-agents -p coder` -> host key `pixel_coder`). Older `pixel-agents.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `PIXEL_AGENTS_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `pixel-agents`) |
...
|---------|-------------|
| `pixel-agents memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `pixel-agents honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `pixel-agents memory setup`) |
| `pixel-agents honcho status` | Show resolved config for active profile |
| `pixel-agents honcho enable` / `disable` | Toggle Honcho for active profile |
| `pixel-agents honcho mode <mode>` | Change recall or observation mode |
| `pixel-agents honcho peer --user <name>` | Update user peer name |
| `pixel-agents honcho peer --ai <name>` | Update AI peer name |
| `pixel-agents honcho tokens --context <N>` | Set context token budget |
| `pixel-agents honcho tokens --dialectic <N>` | Set dialectic max chars |
| `pixel-agents honcho map <name>` | Map current directory to a session name |
| `pixel-agents honcho sync` | Create host blocks for all Pixel Agents profiles |
