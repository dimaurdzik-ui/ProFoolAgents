...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Pixel Agents peer ID in OpenViking (default: pixel-agents)
...
_DEFAULT_AGENT = "pixel-agents"
_AGENT_PROMPT_LABEL = "Pixel Agents peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "PIXEL_AGENTS_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Pixel Agents slash-skill scaffolding before sending content to OpenViking.
...
def _load_pixel_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("PIXEL_AGENTS_HOME", "")).expanduser() if os.environ.get("PIXEL_AGENTS_HOME") else Path.home() / ".pixel-agents"
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory disabled for this Pixel Agents run."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Pixel Agents session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Pixel Agents only", "write values only to Pixel Agents .env"),
    ...
    _save_pixel_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Pixel Agents .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See pixel-agents#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_pixel_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Pixel Agents peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, pixel_home: str, config: dict) -> None:
        ...
        from pixel_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        pixel_home_path = Path(pixel_home)
        env_path = pixel_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_pixel_openviking_config())
        ...
        self._turn_count = 0
        pixel_home = str(kwargs.get("pixel_home") or "").strip()
        if not pixel_home:
            try:
                from pixel_constants import get_pixel_agents_home
                pixel_home = str(get_pixel_agents_home())
            except Exception:
                pixel_home = str(Path.home() / ".pixel-agents")
        self._pixel_home = pixel_home
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Pixel Agents run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.pixel-agents/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts pixel-agents (#21130).
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._pixel_home:
            return None
        return Path(self._pixel_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._pixel_home:
            return None
        return Path(self._pixel_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Pixel Agents' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Pixel Agents canonical messages into OpenViking batch payloads."""
    ...
    def on_session_end(self, messages: List[Dict[str, Any]]) -> None:
        ...
        # synchropixel here (we want it to land before the process exits), but
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See pixel-agents#28296.
        ...
        # Rotate cached session state synchropixelly (cheap, in-memory) and
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/pixel-agents/.overview.md).
