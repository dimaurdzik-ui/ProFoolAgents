"""SimpleX Chat platform adapter (Pixel Agents plugin).
...
This adapter ships as a Pixel Agents platform plugin under
``plugins/platforms/simplex/``. The Pixel Agents plugin loader scans the
...
Optional environment variables:
    ...
    PIXEL_AGENTS_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``pixel-agents setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Pixel Agents — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("PIXEL_AGENTS_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``pixel-agents setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.pixel-agents/.env`` via
    ``pixel_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "pixel_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.pixel-agents/.env"
        )
        return
