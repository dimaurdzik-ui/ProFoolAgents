"""
Email platform adapter for the Pixel Agents gateway.

Allows users to interact with Pixel Agents by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            from pixel_cli import __version__ as _pixel_version
        except Exception:  # noqa: BLE001 — keep ID best-effort if import fails
            _pixel_version = "0"
        imap.xatom(
            "ID",
            f'("name" "pixel-agents" "version" "{_pixel_version}" '
            '"vendor" "PixelResearch" '
            '"support-email" "noreply@pixelagents.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `pixel-agents config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `pixel-agents gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Pixel Agents")
        ...
        msg_id = f"<pixel-agents-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# pixel_cli/gateway.py, and the _send_email dispatch in
