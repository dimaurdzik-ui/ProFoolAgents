...
googleapiclient is synchropixel. This keeps the event loop responsive.
...
# ``model_tools`` import time, so ``pixel-agents status``, ``pixel-agents chat``, etc.
...
# ``pixel_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Pixel Agents is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a pixel-agents MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "pixel-agents-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${PIXEL_AGENTS_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _pixel_home = _get_pixel_agents_home()
        except (ModuleNotFoundError, ImportError):
            _pixel_home = _Path.home() / ".pixel-agents"
        self._thread_count_store = _ThreadCountStore(
            _pixel_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Pixel Agents is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("PIXEL_AGENTS_HOME", str(_Path.home() / ".pixel-agents"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${PIXEL_AGENTS_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a pixel-agents MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Pixel Agents is thinking…' marker message.
        ...
        "Pixel Agents is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Pixel Agents is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Pixel Agents is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Pixel Agents is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``pixel-agents setup``.

    The setup wizard at ``pixel_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.pixel-agents/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. pixel-agents-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.pixel-agents/.env")
    print_info("Restart the gateway: pixel-agents gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``pixel-agents cron`` running as a
    separate process from ``pixel-agents gateway``).  Without this hook,
    ...
    # Bound the synchropixel urllib3-backed token refresh so a hung Google
...
def register(ctx) -> None:
    """Plugin entry point — called by the Pixel Agents plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `pixel-agents setup` to install Google Chat support.",
        ...
        # the "Pixel Agents is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Pixel Agents is thinking…' marker message "
