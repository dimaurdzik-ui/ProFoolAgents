"""
IRC Platform Adapter for Pixel Agents.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Pixel Agents agent.  Zero external dependencies — uses
      ...
      platforms:
        irc:
          ...
          extra:
            ...
            port: 6697
            nickname: pixel-agents-bot
            channel: "#pixel-agents"
            use_tls: true
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "pixel-agents-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Pixel Agents")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Pixel Agents shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: pixel_, pixel_1, pixel_2...
...
def check_requirements() -> bool:
    ...
    # pixel-agents setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `pixel-agents gateway setup` flow for the IRC platform.

    Lazy-imports ``pixel_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Pixel Agents to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    nickname = prompt(
        "Bot nickname (e.g. pixel-agents-bot)",
    ...
    channel = prompt(
        "Channel to join (e.g. #pixel-agents — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.pixel-agents/.env")
    print_info("Restart the gateway for changes to take effect: pixel-agents gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "pixel-agents-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "pixel-agents-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Pixel Agents (cron)")
