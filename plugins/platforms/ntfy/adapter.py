"""ntfy platform adapter (Pixel Agents plugin).
...
a Pixel Agents dependency.

This adapter ships as a Pixel Agents platform plugin under
``plugins/platforms/ntfy/``. The Pixel Agents plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "pixel-agents-in"              # subscribe topic (incoming)
          publish_topic: "pixel-agents-out"     # optional — defaults to topic
...
_ECHO_TAG = "pixel-agents"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``pixel-agents cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Pixel Agents dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `pixel-agents gateway status` without
