"""
LINE Messaging API platform adapter for Pixel Agents.
...
to Pixel Agents. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``PIXEL_AGENTS_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            pixel_home = Path(get_pixel_agents_home()).resolve()
        except Exception:
            pixel_home = Path.home().joinpath(".pixel-agents").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``pixel-agents status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``pixel-agents status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``pixel-agents setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.pixel-agents/.env`` via ``pixel_cli.config``.
    """
    print()
    ...
    try:
        from pixel_cli.config import get_env_var, set_env_var
    except ImportError:
        print("pixel_cli.config not available; set LINE_* vars manually in ~/.pixel-agents/.env")
        return
