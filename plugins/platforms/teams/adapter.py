"""
Microsoft Teams platform adapter for Pixel Agents.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Pixel Agents"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        async with create_ssrf_safe_async_client(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; PixelAgentsAgent/1.0)"},
            )
            response.raise_for_status()
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        pixel_action = data.get("pixel_action", "")
        session_key = data.get("session_key", "")

        if not pixel_action or not session_key:
        ...
        choice = choice_map.get(pixel_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="pixel_approve",
            data={**btn_data_base, "pixel_action": "approve_once"}, style="positive",
        )]
        if not smart_denied and allow_session:
            actions.append(ExecuteAction(
                title="Allow Session", verb="pixel_approve",
                data={**btn_data_base, "pixel_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="pixel_approve",
                    data={**btn_data_base, "pixel_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="pixel_approve",
            data={**btn_data_base, "pixel_action": "deny"}, style="destructive",
        ))
        body = [
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Pixel Agents\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.pixel-agents/.env")
