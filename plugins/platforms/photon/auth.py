...
Credential storage mirrors every other Pixel Agents channel:

    * runtime SDK creds  -> ``~/.pixel-agents/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.pixel-agents/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a pixel-agents dependency
...
# registers Pixel Agents as its own client_id.
...
# Default name of the project Pixel Agents provisions for the operator.
DEFAULT_PROJECT_NAME = "Pixel Agents"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of pixel-agents.

def _auth_json_path() -> Path:
    """Resolve ``~/.pixel-agents/auth.json`` honouring the active Pixel Agents profile."""
    try:
        from pixel_constants import get_pixel_agents_home
        return Path(get_pixel_agents_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.pixel-agents")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # pixel_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def store_photon_token(token: str) -> None:
    ...
    from pixel_cli.auth import _auth_store_lock
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.pixel-agents/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.pixel-agents/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.pixel-agents/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: pixel_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `pixel-agents photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `pixel-agents photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `pixel-agents photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `pixel-agents photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `pixel-agents photon setup`)"
