...
This plugin connects Pixel Agents to iMessage (and other Spectrum
...
TypeScript-only, Pixel Agents runs it inside a small supervised Node sidecar and
...
`pixel-agents photon setup` does, in order:

1. **Device login** (RFC 8628, `client_id=photon-cli`) — opens
   `https://app.photon.codes/` for approval and stores the bearer token.
2. **Find or create** the `Pixel Agents` project on the Photon dashboard.
3. **Provision the project secret** — mint a fresh project secret (the
   dashboard reveals it only once) and persist it to `~/.pixel-agents/.env` so the
...
There is no separate `login` command; like every other Pixel Agents channel,
...
Run `pixel-agents photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.pixel-agents/.env` (the same place every other
...
Management metadata lives in `~/.pixel-agents/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `pixel-agents photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Pixel Agents
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Pixel Agents coalesces
...
- **Native polls are supported.** Pixel Agents posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `pixel-agents send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<pixel-agents-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `pixel-agents photon status`, a DM and a group roundtrip,
