"""
``pixel-agents photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Pixel Agents
...
# Writable sidecar runtime dir (mirrors to PIXEL_AGENTS_HOME on immutable
...
# wiring imports this module for `pixel-agents --help`).
...
# when called later (gateway start, pixel-agents status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `pixel-agents photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Pixel Agents')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``pixel-agents photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Pixel Agents" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.pixel-agents/.env,
    ...
    try:
        from pixel_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  pixel-agents gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `pixel-agents photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`pixel-agents photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.pixel-agents/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `pixel-agents photon telemetry on` / `pixel-agents photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.pixel-agents/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  pixel-agents gateway restart")
    return 0
...
#
# `pixel-agents gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``pixel-agents photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `pixel-agents gateway setup` wizard."""
