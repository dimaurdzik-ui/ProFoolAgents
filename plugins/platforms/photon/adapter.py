"""
Photon Spectrum (iMessage) platform adapter for Pixel Agents.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Pixel Agents dep
...
#
# Out-of-process senders (cron subprocesses, `pixel-agents send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_pixel_agents_home() honors profile overrides — never hardcode ~/.pixel-agents.
    from pixel_constants import get_pixel_agents_home

    return get_pixel_agents_home() / "runtime" / _RUNTIME_RECORD_NAME
...
# this module (pixel-agents status, test collection, plugin discovery).
...
# artifacts so Pixel Agents sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `pixel-agents photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `pixel-agents photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `pixel-agents setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, pixel-agents status, GET /api/status polling) —
        ...
        if npm_error:
            logger.debug(
                ...
                "(last npm error: %s) — run: pixel-agents photon setup",
                _sidecar_dir(),
                npm_error,
            )
        else:
            logger.debug(
                "photon: spectrum-ts not installed at %s — run: pixel-agents photon setup",
                _sidecar_dir(),
            )
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `pixel-agents update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``pixel-agents photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Pixel Agents does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Pixel Agents defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Pixel-Agents-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Pixel Agents checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `pixel-agents photon setup`, so the connect path must be able to
            ...
            if not sidecar_deps_installed():
                raise RuntimeError(
                    ...
                    f"Run: cd {_sidecar_dir()} && npm ci   (or `pixel-agents photon setup`)"
                )
        # A `pixel-agents update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `pixel-agents send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`pixel-agents send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Pixel Agents gateway (which spawns the sidecar and "
                    "records its address under <pixel-agents-home>/runtime/"
    ...
    headers = {"X-Pixel-Agents-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Pixel Agents plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `pixel-agents photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: pixel-agents photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `pixel-agents gateway setup` alongside every other
    ...
    # Register CLI subcommands — `pixel-agents photon ...`
