...
Small Node helper that bridges Pixel Agents to Photon's Spectrum SDK
(`spectrum-ts`).  Pixel Agents is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Pixel-Agents-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Pixel Agents can receive inbound messages
...
The Pixel Agents plugin's `pixel-agents photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Pixel Agents is Python, so replies go through this sidecar
