// Pixel Agents — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Pixel Agents): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Pixel Agents -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Pixel-Agents-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `pixel-agents photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Pixel Agents can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "pixel-agents-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Pixel Agents'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Pixel Agents allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Pixel Agents, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-pixel-agents-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Pixel Agents supplied them so a known-good
