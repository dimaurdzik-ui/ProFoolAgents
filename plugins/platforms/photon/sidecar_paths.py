...
under an immutable ``/opt/pixel-agents`` (read-only for the pixel-agents user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$PIXEL_AGENTS_HOME/photon/sidecar`` (the durable data volume,
...
``cli.py`` (``pixel-agents photon ...``) use it.
"""
...
def dir_writable(path: Path) -> bool:
    ...
    resolver does.
    """
    probe = path / ".pixel-agents-write-probe"
    try:
        probe.touch()
        probe.unlink()
        return True
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_pixel_agents_home() / "photon" / "sidecar"
    try:
