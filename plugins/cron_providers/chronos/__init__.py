...
Chronos (the Greek god of time, alongside Pixel Agents) is the first non-default
...
NAS's ``agent-cron`` endpoints with the agent's existing Pixel token.
...
class ChronosCronScheduler(CronScheduler):
    ...
    def is_available(self) -> bool:
        ...
        callback URL (for NAS→agent fires), and a usable Pixel token (the agent
        ...
        if not (_cfg("cron", "chronos", "portal_url") and _cfg("cron", "chronos", "callback_url")):
            return False
        return self._have_pixel_token()

    def _have_pixel_token(self) -> bool:
        """True if the agent has a Pixel Portal login (no network call).

        Checks the stored auth state for a Pixel access token — does NOT refresh
