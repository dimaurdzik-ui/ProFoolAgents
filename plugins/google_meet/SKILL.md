...
pixel-agents meet install                 # pip deps + Chromium (transcribe only)
pixel-agents meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
pixel-agents meet auth                    # optional; skips guest-lobby wait
pixel-agents meet setup                   # preflight checks
```

`pixel-agents meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or PIXEL_AGENTS_MEET_REALTIME_KEY in ~/.pixel-agents/.env
```
...
pip install playwright websockets && python -m playwright install chromium
pixel-agents plugins enable google_meet
pixel-agents meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
pixel-agents meet node approve my-mac ws://<mac-ip>:18789 <token>
pixel-agents meet node ping my-mac                   # confirm reachable
```

Run `pixel-agents meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Pixel Agents agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `pixel-agents meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `PIXEL_AGENTS_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `pixel_meet_src`). |
...
Local:
```
$PIXEL_AGENTS_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$PIXEL_AGENTS_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `pixel-agents meet node approve`.
