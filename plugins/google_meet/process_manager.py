...
session-scoped state file under ``$PIXEL_AGENTS_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $PIXEL_AGENTS_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_pixel_agents_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this pixel-agents install, leave it first —
    ...
    env = os.environ.copy()
    env["PIXEL_AGENTS_MEET_URL"] = url
    env["PIXEL_AGENTS_MEET_OUT_DIR"] = str(out)
    env["PIXEL_AGENTS_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["PIXEL_AGENTS_MEET_HEADED"] = "1"
    if auth_state:
        env["PIXEL_AGENTS_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["PIXEL_AGENTS_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if PIXEL_AGENTS_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["PIXEL_AGENTS_MEET_MODE"] = mode
    if realtime_model:
        env["PIXEL_AGENTS_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["PIXEL_AGENTS_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["PIXEL_AGENTS_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["PIXEL_AGENTS_MEET_REALTIME_KEY"] = realtime_api_key
