...
Let the pixel-agents agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where pixel-agents runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `pixel-agents meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `pixel-agents meet` CLI |
...
| `cli.py` | `pixel-agents meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$PIXEL_AGENTS_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `pixel-agents meet node {run,list,approve,remove,status,ping}` |
...
pixel-agents meet install                                      # pip + Chromium
pixel-agents meet setup                                        # preflight
pixel-agents meet auth                                         # optional
pixel-agents meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
pixel-agents meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.pixel-agents/.env
pixel-agents meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
pixel-agents meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
pixel-agents meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.pixel-agents/.env
pixel-agents meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, pixel-agents will **not** switch your system audio input automatically — the
...
pixel-agents plugins enable google_meet
pixel-agents meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
pixel-agents meet node ping my-mac
