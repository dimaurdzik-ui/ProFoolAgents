"""`pixel-agents meet node ...` subcommand tree.

Wired into the existing ``pixel-agents meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``pixel-agents meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="pixel-agents-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``pixel-agents meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             pixel-agents meet node approve <name> ws://<host>:{args.port} {token}")
        try:
