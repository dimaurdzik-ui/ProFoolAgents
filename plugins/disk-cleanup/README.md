...
Auto-tracks and cleans up ephemeral files created during Pixel Agents
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$PIXEL_AGENTS_HOME` and `/tmp/pixel-agents-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `PIXEL_AGENTS_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `PIXEL_AGENTS_HOME` or `/tmp/pixel-agents-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$PIXEL_AGENTS_HOME/disk-cleanup/` is itself excluded
- `$PIXEL_AGENTS_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
