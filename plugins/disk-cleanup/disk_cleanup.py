"""disk_cleanup — ephemeral file cleanup for Pixel Agents.
...
Rules:
  ...
  - empty dirs    → always delete (under PIXEL_AGENTS_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly PIXEL_AGENTS_HOME and /tmp/pixel-agents-*
Never touches: ~/.pixel-agents/logs/ or any system directory.
"""
...
try:
    ...
    def get_pixel_agents_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("PIXEL_AGENTS_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".pixel-agents").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$PIXEL_AGENTS_HOME/logs/``."""
    return get_pixel_agents_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$PIXEL_AGENTS_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under PIXEL_AGENTS_HOME or ``/tmp/pixel-agents-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    pixel_home = get_pixel_agents_home()
    try:
        path.resolve().relative_to(pixel_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/pixel-agents-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("pixel-agents-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "pixel-agents", "backups", "profiles", ".worktrees",
})
...
# Paths under $PIXEL_AGENTS_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so PIXEL_AGENTS_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        pixel_home = get_pixel_agents_home()
        for parent in ("cron", "cronjobs"):
            base = pixel_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside PIXEL_AGENTS_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under PIXEL_AGENTS_HOME, but never recurse into known
    # durable state trees.  Some installs place the Pixel Agents checkout, venv,
    # and desktop build under PIXEL_AGENTS_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    pixel_home = get_pixel_agents_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in pixel_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    pixel_home = get_pixel_agents_home()
    try:
        rel = path.resolve().relative_to(pixel_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "pixel-agents",
        }:
            return None
        ...
        # Path isn't under PIXEL_AGENTS_HOME (e.g. /tmp/pixel-agents-*) — fall through.
        pass
