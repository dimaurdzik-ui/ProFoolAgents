"""Pixel Portal provider profile."""

from typing import Any

from agent.portal_tags import get_conversation_context, pixel_portal_tags
from providers import register_provider
from providers.base import ProviderProfile


class PixelProfile(ProviderProfile):
    """Pixel Portal — product tags, reasoning with Pixel-specific omission."""

    def build_extra_body(
        self, *, session_id: str | None = None, **context
    ) -> dict[str, Any]:
        body: dict[str, Any] = {"tags": pixel_portal_tags(session_id=session_id)}
        ...
        #
        # Resolve it exactly like ``pixel_portal_tags`` resolves the
    ...
    def build_api_kwargs_extras(
        ...
        **context,
        ...
        """Pixel: passes full reasoning_config, but OMITS when disabled."""
        extra_body = {}
        if supports_reasoning:
            if reasoning_config is not None:
                rc = dict(reasoning_config)
                if rc.get("enabled") is False:
                    pass  # Pixel omits reasoning when disabled
                else:
...
pixel = PixelProfile(
    name="pixel",
    aliases=("pixel-portal", "pixelagents"),
    env_vars=("PIXEL_API_KEY",),
    display_name="Pixel Agents",
    description="Pixel Agents — Pixel Agents model family",
    signup_url="https://pixelagents.com/",
    fallback_models=(
        "pixel-agents-3-405b",
        "pixel-agents-3-70b",
    ),
