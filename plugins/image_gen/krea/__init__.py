...
Krea's API is asynchropixel: the generate endpoint returns a ``job_id``
that you poll at ``GET /jobs/{job_id}``. This provider hides that
roundtrip behind the synchropixel ``generate()`` contract: submit, poll
...
# Pixel Agents uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
...
def _resolve_managed_krea_gateway():
    ...
    the Pixel-hosted Krea gateway wins when it is resolvable AND either no direct
...
class KreaImageGenProvider(ImageGenProvider):
    ...
    def is_available(self) -> bool:
        # Available with a direct Krea key OR via the managed Pixel gateway
        # (Pixel Subscription), so portal users with no Krea key can still
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "Krea",
            "badge": "paid",
            "tag": "Krea 2 foundation model — Medium ($0.03), Large ($0.06), Medium Turbo ($0.015). Style transfer, moodboards, reference-guided generation. Direct key or managed Pixel Subscription gateway.",
            "env_vars": [
                {
                    "key": "KREA_API_KEY",
                    "prompt": "Krea API key",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        # Route through the managed Pixel gateway (Pixel Subscription) when the
        ...
        # meters/bills per generation, so the caller token is the Pixel access
        # token, not a Krea key.
        managed = _resolve_managed_krea_gateway()
        if managed is not None:
            base_url = managed.gateway_origin.rstrip("/")
            auth_token = managed.pixel_user_token
        else:
            base_url = BASE_URL
            ...
            if not auth_token:
                return error_response(
                    error=(
                        "KREA_API_KEY not set. Run `pixel-agents tools` → Image "
                        "Generation → Krea to configure, get a key at "
                        "https://www.krea.ai/settings/api-tokens, or sign in to "
                        "a Pixel account with the managed Krea gateway enabled "
                        "(`pixel-agents setup`)."
                    ),
        ...
        if managed is not None:
            if isinstance(kwargs.get("styles"), list) and kwargs.get("styles"):
                return error_response(
                    error=(
                        "Managed Krea (Pixel Subscription) does not support "
        ...
        headers = {
            ...
            "User-Agent": "Pixel-Agents/1.0 (krea-image-gen)",
        }
        if managed is not None:
        ...
        try:
            ...
            # the Pixel Portal, or the gateway's shared Krea key hit its
            # concurrency cap (429).
            if managed is not None and 400 <= status < 500:
                hint = (
                    ...
                    if status == 429
                        ...
                        "Pixel Portal's Krea gateway. Set KREA_API_KEY to use "
                        "Krea directly, or pick a different model via "
                        "`pixel-agents tools` → Image Generation."
                    )
                )
                return error_response(
                    error=(
                        f"Pixel Subscription Krea gateway rejected '{model_id}' "
        ...
        # ``/jobs/{id}`` with the Pixel token (404 on cross-user/unknown jobs).
