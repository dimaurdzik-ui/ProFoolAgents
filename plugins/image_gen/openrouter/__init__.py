"""OpenRouter-compatible image generation backend (OpenRouter + Pixel Portal).

Both OpenRouter and the Pixel Portal inference endpoint speak the same
...
Pixel Portal proxies OpenRouter, so one implementation services both — we only
swap the resolved ``(base_url, api_key)``. Credentials are resolved through the
agent's existing :func:`~pixel_cli.runtime_provider.resolve_runtime_provider`,
which already understands OpenRouter's key pool and the Pixel OAuth device-code
...
# image_gen.model from ``pixel-agents tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    Instantiated once per backend (OpenRouter, Pixel Portal). The two differ only
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``pixel-agents tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `pixel-agents tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
        ...
        headers = {
            ...
            # OpenRouter attribution headers (harmless against Pixel Portal).
            "HTTP-Referer": "https://github.com/PixelResearch/pixel-agents",
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            provider_name="pixel",
            display_name="Pixel Portal",
            runtime_name="pixel",
            config_key="pixel",
            model_env_var="PIXEL_IMAGE_MODEL",
            setup_schema={
                "name": "Pixel Portal (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via Pixel Portal (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + Pixel Portal image gen providers."""
