...
Output is saved as PNG under ``$PIXEL_AGENTS_HOME/cache/images/``. Source images for
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `pixel-agents auth codex` (or `pixel-agents setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`pixel-agents auth codex` (or `pixel-agents setup` → Codex) to sign in."
                ),
