...
three virtual model IDs so the ``pixel-agents tools`` model picker and the
...
``$PIXEL_AGENTS_HOME/cache/images/``.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `pixel-agents tools` → Image "
                    "Generation → OpenAI to configure, or `pixel-agents setup` "
                    "to add the key."
                ),
