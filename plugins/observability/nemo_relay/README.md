...
Optional Pixel Agents observability plugin that configures exporters and maps
Pixel Agents-specific observer hooks to NeMo Relay marks and ATIF state. Pixel Agents core
...
not replace Pixel Agents's planner, tools, memory, model provider routing, or
CLI UX. Pixel Agents core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, Pixel Agents can:

- Export the Relay scopes and LLM/tool lifecycles emitted by Pixel Agents core.
- Add Pixel Agents session, turn, approval, and subagent mark events.
...
```bash
pixel-agents plugins enable observability/nemo_relay
```

The `PIXEL_AGENTS_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `PIXEL_AGENTS_HOME` that the
agent run will use:

```bash
env PIXEL_AGENTS_HOME=/tmp/pixel-agents-nemo-relay-test \
  pixel-agents plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`PIXEL_AGENTS_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`PIXEL_AGENTS_HOME` is the Pixel Agents profile/config home used by both
`pixel-agents plugins enable ...` and the later `pixel-agents chat ...` run. If unset,
Pixel Agents uses the user's default home, usually `~/.pixel-agents`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export PIXEL_AGENTS_HOME=/tmp/pixel-agents-nemo-relay-test
pixel-agents plugins enable observability/nemo_relay
pixel-agents chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `pixel-agents` command you run is built from the
checkout that contains this plugin. A globally installed older CLI will not see
new bundled plugins from your working tree.

```bash
uv sync
uv run pixel-agents plugins enable observability/nemo_relay
uv run pixel-agents chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, Pixel Agents installs its supported NeMo Relay runtime as a normal dependency:

```bash
uv build --wheel
python -m pip install --force-reinstall dist/pixel_agent-*.whl
pixel-agents plugins enable observability/nemo_relay
```
...
behavior; it does not control Pixel Agents shared client metrics.

## Export Configuration

The plugin can configure exporters directly from `PIXEL_AGENTS_NEMO_RELAY_*`
...
```bash
export PIXEL_AGENTS_NEMO_RELAY_ATOF_ENABLED=1
export PIXEL_AGENTS_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export PIXEL_AGENTS_NEMO_RELAY_ATIF_ENABLED=1
export PIXEL_AGENTS_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `PIXEL_AGENTS_NEMO_RELAY_ATOF_FILENAME`
- `PIXEL_AGENTS_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `PIXEL_AGENTS_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `PIXEL_AGENTS_NEMO_RELAY_ATIF_AGENT_NAME`
- `PIXEL_AGENTS_NEMO_RELAY_ATIF_AGENT_VERSION`
- `PIXEL_AGENTS_NEMO_RELAY_ATIF_MODEL_NAME`
- `PIXEL_AGENTS_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Pixel Agents at it:

```bash
export PIXEL_AGENTS_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Pixel Agents"
agent_version = "local"
```

When `PIXEL_AGENTS_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`PIXEL_AGENTS_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`PIXEL_AGENTS_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Pixel Agents does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Pixel Agents keeps the direct env-var fallbacks active for that run.

Pixel Agents core routes provider and tool execution through NeMo Relay managed APIs
...
Pixel Agents uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with Pixel Agents-owned
...
with this Pixel Agents-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Pixel Agents rejects
...
Pixel Agents activates these plugins before registering its managed LLM and tool
...
For the full generic Pixel Agents middleware contract, see
...
with Pixel Agents and a local Ollama model served through the OpenAI-compatible API.

```bash
export PIXEL_AGENTS_HOME=/tmp/pixel-agents-nemo-relay-docs/pixel-agents-home
mkdir -p "$PIXEL_AGENTS_HOME"

cat > "$PIXEL_AGENTS_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Pixel Agents session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export PIXEL_AGENTS_NEMO_RELAY_ATOF_ENABLED=1
export PIXEL_AGENTS_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/pixel-agents-nemo-relay-docs/subagent/atof
export PIXEL_AGENTS_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export PIXEL_AGENTS_NEMO_RELAY_ATOF_MODE=overwrite
export PIXEL_AGENTS_NEMO_RELAY_ATIF_ENABLED=1
export PIXEL_AGENTS_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/pixel-agents-nemo-relay-docs/subagent/atif
export PIXEL_AGENTS_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export PIXEL_AGENTS_NEMO_RELAY_ATIF_AGENT_NAME='Pixel Agents E2E'
export PIXEL_AGENTS_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export PIXEL_AGENTS_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"pixel-agents.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Pixel Agents E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Pixel Agents dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/pixel-agents-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/pixel-agents-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/pixel-agents-nemo-relay-docs/workdir/beta.txt
cd /tmp/pixel-agents-nemo-relay-docs/workdir

export PIXEL_AGENTS_NEMO_RELAY_ATOF_ENABLED=1
export PIXEL_AGENTS_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/pixel-agents-nemo-relay-docs/parallel/atof
export PIXEL_AGENTS_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export PIXEL_AGENTS_NEMO_RELAY_ATOF_MODE=overwrite
export PIXEL_AGENTS_NEMO_RELAY_ATIF_ENABLED=1
export PIXEL_AGENTS_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/pixel-agents-nemo-relay-docs/parallel/atif
export PIXEL_AGENTS_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export PIXEL_AGENTS_NEMO_RELAY_ATIF_AGENT_NAME='Pixel Agents E2E'
export PIXEL_AGENTS_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Pixel Agents sessions map to `agent` scopes.
- Pixel Agents core managed provider calls map to `llm` scope start/end events.
- Pixel Agents core managed tool calls map to `tool` scope start/end events.
...
Pixel Agents core owns the LLM and tool boundaries and enters NeMo Relay managed
execution while a Pixel Agents-managed Relay consumer is active. With no shared
metrics subscriber or explicitly configured Relay plugin, Pixel Agents calls the
...
Enable it for Pixel Agents:

```bash
export PIXEL_AGENTS_NEMO_RELAY_PLUGINS_TOML=/tmp/pixel-agents-middleware-test/plugins.toml
```

Execution follows these boundaries with or without an adaptive component:

```text
Pixel Agents provider call
  -> nemo_relay.llm.execute(...)
    -> Pixel Agents provider adapter callback(...)

Pixel Agents tool call
  -> nemo_relay.tools.execute(...)
    -> Pixel Agents authorization and dispatch callback(...)
```
...
middleware for a local Pixel Agents run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by NeMo Relay 0.6
and later.

```bash
export PIXEL_AGENTS_HOME=/tmp/pixel-agents-middleware-test/pixel-agents-home
mkdir -p "$PIXEL_AGENTS_HOME" /tmp/pixel-agents-middleware-test/nemo-relay
...
cat > /tmp/pixel-agents-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/pixel-agents-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/pixel-agents-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Pixel Agents Middleware E2E"
agent_version = "local"
...
export PIXEL_AGENTS_NEMO_RELAY_PLUGINS_TOML=/tmp/pixel-agents-middleware-test/nemo-relay/plugins.toml
...
```json
{
  ...
  "session_id": "middleware-demo-session",
  "agent": {
    "name": "Pixel Agents Middleware E2E",
    "version": "local",
