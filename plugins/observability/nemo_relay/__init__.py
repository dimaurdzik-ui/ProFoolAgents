"""nemo_relay — optional Pixel Agents plugin for NeMo Relay observability."""
...
_SESSION_INITIALIZER_NAME = "pixel-agents.nemo_relay.rich_observability"
...
class _Settings:
    ...
    atof_filename: str = "pixel-agents-atof.jsonl"
    atof_mode: str = "append"
    atif_enabled: bool = False
    atif_output_directory: str = ""
    atif_filename_template: str = "pixel-agents-atif-{session_id}.json"
    atif_subagent_export_mode: str = "embedded"
    atif_agent_name: str = "Pixel Agents"
        ...
        with self._lock:
            ...
            if self._owners:
                ...
                logger.warning(
                    ...
                    "Pixel Agents profile; keeping the existing process-global configuration "
...
class _Runtime:
    def __init__(
        ...
        self._atof_subscriber_name = f"pixel-agents.nemo_relay.atof.{self.host.runtime_id}"
        self._execution_consumer_name = (
            f"pixel-agents.nemo_relay.rich_observability.{self.host.runtime_id}"
        )
        ...
        with self._sessions_lock:
            ...
            if self.settings.atif_enabled and not self._plugins_toml_owns_exporter("atif"):
                ...
                state.atif_subscriber_name = (
                    f"pixel-agents.nemo_relay.atif.{self.host.runtime_id}.{session_id}"
                )
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if relay_session is None:
            raise RuntimeError("Pixel Agents core Relay session is unavailable")
    ...
    def mark_subagent_stop(self, kwargs: dict[str, Any]) -> None:
        ...
        self.mark("pixel-agents.subagent.stop", kwargs)
...
def register(ctx) -> None:
    ...
    # Activate dynamic plugins before Pixel Agents installs the managed execution
...
def on_session_end(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: (runtime.mark("pixel-agents.session.end", kwargs), runtime.export_atif(runtime.ensure_session(kwargs))))
...
def on_pre_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("pixel-agents.turn.start", kwargs))


def on_post_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("pixel-agents.turn.end", kwargs))


def on_pre_approval_request(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("pixel-agents.approval.request", kwargs))


def on_post_approval_response(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("pixel-agents.approval.response", kwargs))
    ...
    with _LOCK:
        ...
        try:
            ...
            if resolved_host is None:
                raise RuntimeError("Pixel Agents core Relay runtime is unavailable")
            runtime = _Runtime(
...
def _load_settings() -> _Settings:
    plugins_toml_path = _env("PIXEL_AGENTS_NEMO_RELAY_PLUGINS_TOML")
    ...
    return _Settings(
        ...
        atof_enabled=_env_bool("PIXEL_AGENTS_NEMO_RELAY_ATOF_ENABLED"),
        atof_output_directory=_env("PIXEL_AGENTS_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY"),
        atof_filename=_env("PIXEL_AGENTS_NEMO_RELAY_ATOF_FILENAME") or "pixel-agents-atof.jsonl",
        atof_mode=_env("PIXEL_AGENTS_NEMO_RELAY_ATOF_MODE") or "append",
        atif_enabled=_env_bool("PIXEL_AGENTS_NEMO_RELAY_ATIF_ENABLED"),
        atif_output_directory=_env("PIXEL_AGENTS_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"),
        atif_filename_template=_env("PIXEL_AGENTS_NEMO_RELAY_ATIF_FILENAME_TEMPLATE") or "pixel-agents-atif-{session_id}.json",
        atif_subagent_export_mode=_atif_subagent_export_mode(),
        atif_agent_name=_env("PIXEL_AGENTS_NEMO_RELAY_ATIF_AGENT_NAME") or "Pixel Agents",
        atif_agent_version=_env("PIXEL_AGENTS_NEMO_RELAY_ATIF_AGENT_VERSION") or "unknown",
        atif_model_name=_env("PIXEL_AGENTS_NEMO_RELAY_ATIF_MODEL_NAME") or "unknown",
    )
...
def _dynamic_plugin_specs(
    ...
    if plugins_section is not None:
        ...
        if plugins_section:
            logger.error(
                "Pixel Agents cannot activate Relay gateway [[plugins.dynamic]] records because "
                "the Python binding does not expose the CLI lifecycle resolver for "
                "enablement, trust policy, and worker environments. Use Pixel Agents-owned "
...
def _atif_subagent_export_mode() -> str:
    mode = _env("PIXEL_AGENTS_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()
...
def _resolve_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="pixel-agents-nemo-relay-awaitable",
        daemon=True,
    )
    thread.start()
    thread.join()
