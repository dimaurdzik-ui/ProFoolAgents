...
A standards-compliant OpenID Connect Relying Party for the ``pixel-agents dashboard``
OAuth gate. Unlike the bundled ``pixel`` provider (which encodes Pixel Portal's
bespoke contract — ``agent:{instance_id}`` client ids, a custom access-token
JWT, the ``x-pixel-refresh-token`` header, an ``oauth_contract_version`` claim),
...
:class:`~pixel_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``pixel_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~pixel_cli.dashboard_auth.Session`.
...
only choice that is universally correct across self-hosted IDPs. (The ``pixel``
provider verifies its *access* token because Pixel Portal mints a custom JWT
...
same precedence convention as the ``pixel`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.pixel-agents/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    PIXEL_AGENTS_DASHBOARD_OIDC_ISSUER
    PIXEL_AGENTS_DASHBOARD_OIDC_CLIENT_ID
    PIXEL_AGENTS_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    PIXEL_AGENTS_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
# JWKS cache (PyJWKClient handles its own caching; this mirrors the pixel
# provider's 5-minute lifespan so key rotation is picked up promptly).
_JWKS_CACHE_SECONDS = 300


# ---------------------------------------------------------------------------
# Skip-reason channel (mirrors the pixel plugin)
# ---------------------------------------------------------------------------
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Pixel Agents does not call
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        redirect_uris are permitted. Mirrors the pixel provider.
        """
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``PIXEL_AGENTS_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "PIXEL_AGENTS_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "PIXEL_AGENTS_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("PIXEL_AGENTS_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.pixel-agents/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "PIXEL_AGENTS_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(PIXEL_AGENTS_DASHBOARD_OIDC_ISSUER + PIXEL_AGENTS_DASHBOARD_OIDC_CLIENT_ID) "
