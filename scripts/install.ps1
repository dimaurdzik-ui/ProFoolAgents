# ============================================================================
# Pixel Agents Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$PixelAgentsHome = $(if ($env:PIXEL_AGENTS_HOME) { $env:PIXEL_AGENTS_HOME } else { "$env:LOCALAPPDATA\pixel-agents" }),
    [string]$InstallDir = $(if ($env:PIXEL_AGENTS_HOME) { "$env:PIXEL_AGENTS_HOME\pixel-agents" } else { "$env:LOCALAPPDATA\pixel-agents\pixel-agents" }),
    ...
    # builds apps/desktop into a launchable Pixel Agents.exe.
    #
    # Why opt-in:
    #   * Pixel Agents-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Pixel Agents.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Pixel Agents.exe
    ...
    #     `pixel-agents desktop` already builds on demand.
...
# 5.1's progress UI repaints synchropixelly on every received byte, which
...
$RepoUrlSsh = "git@github.com:PixelResearch/pixel-agents.git"
$RepoUrlHttps = "https://github.com/PixelResearch/pixel-agents.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Pixel Agents Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Pixel Agents.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $PixelAgentsHome)) {
        New-Item -ItemType Directory -Force -Path $PixelAgentsHome | Out-Null
    }
    $envFile = Join-Path $PixelAgentsHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $PixelAgentsHome "node"
...
function Install-Uv {
    # Pixel Agents owns its own uv at $PixelAgentsHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (pixel_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `pixel-agents update` stay in sync.
    $managedUv = Join-Path $PixelAgentsHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $PixelAgentsHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $PixelAgentsHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $PixelAgentsHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $PixelAgentsHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and PIXEL_AGENTS_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Pixel Agents-Setup.exe
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Pixel Agents setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Pixel Agents uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\pixel-agents\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Pixel Agents needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Pixel Agents copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\pixel-agents\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``PIXEL_AGENTS_GIT_BASH_PATH`` (User scope) so Pixel Agents can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Pixel Agents-managed PortableGit install instead..."
    }

    # Download PortableGit into $PixelAgentsHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $PixelAgentsHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Pixel Agents features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Pixel Agents needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``PIXEL_AGENTS_GIT_BASH_PATH`` (User env scope) so Pixel Agents can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $PixelAgentsHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $PixelAgentsHome\git\usr\bin\bash.exe
    $candidates += "$PixelAgentsHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$PixelAgentsHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("PIXEL_AGENTS_GIT_BASH_PATH", $candidate, "User")
            $env:PIXEL_AGENTS_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set PIXEL_AGENTS_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Pixel Agents may not find Git Bash."
    Write-Info "If needed, set PIXEL_AGENTS_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$PixelAgentsHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$PixelAgentsHome\node;$env:Path"
        Write-Success "Node.js $version found (Pixel Agents-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Pixel Agents-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $PixelAgentsHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $PixelAgentsHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\pixel-agents-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$PixelAgentsHome\node") { Remove-Item -Recurse -Force "$PixelAgentsHome\node" }
                Move-Item $extractedDir.FullName "$PixelAgentsHome\node"
                ...
                $version = & "$PixelAgentsHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $PixelAgentsHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\pixel-agents-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `pixel-agents
                    # update` path (#4735).
                    ...
                    $stashName = "pixel-agents-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. pixel-agents-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``pixel-agents update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Pixel Agents behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running pixel-agents processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/PixelResearch/pixel-agents/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/PixelResearch/pixel-agents/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/PixelResearch/pixel-agents/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\pixel-agents-$zipLabel.zip"
                $extractPath = "$env:TEMP\pixel-agents-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `pixel-agents update` (see the notes at the shared clone-path
    ...
    # next `pixel-agents update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Pixel Agents-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running pixel-agents process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running pixel-agents processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*PixelAgents_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (pixel-agents.exe) plus its child tree.
            & taskkill /F /T /IM pixel-agents.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM pixel-agents.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m pixel_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not pixel-agents.exe.
            ...
            # user (see pixel_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `pixel-agents update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `pixel-agents.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install pixel-agents package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and pixel-agents.exe broken with
    ...
    # here, not 30 seconds later when the user runs `pixel-agents`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register pixel-agents.exe in dist-info/RECORD but fail to
        ...
        # `pixel-agents` command while pixel-agents.exe / pixel-agents-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m pixel_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `pixel-agents dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\pixel_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `pixel-agents dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `pixel-agents dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: pixel_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up pixel-agents command..."
    
    if ($NoVenv) {
        $pixelAgentsBin = "$InstallDir"
    } else {
        $pixelAgentsBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so pixel-agents is globally available
    # On Windows, the pixel-agents.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$pixelAgentsBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$pixelAgentsBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $pixelAgentsBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set PIXEL_AGENTS_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\pixel-agents instead
    # of the Unix default ~/.pixel-agents
    $currentPixelAgentsHome = [Environment]::GetEnvironmentVariable("PIXEL_AGENTS_HOME", "User")
    if (-not $currentPixelAgentsHome -or $currentPixelAgentsHome -ne $PixelAgentsHome) {
        [Environment]::SetEnvironmentVariable("PIXEL_AGENTS_HOME", $PixelAgentsHome, "User")
        Write-Success "Set PIXEL_AGENTS_HOME=$PixelAgentsHome"
    }
    $env:PIXEL_AGENTS_HOME = $PixelAgentsHome
    
    # Update current session
    $env:Path = "$pixelAgentsBin;$env:Path"
    
    Write-Success "pixel-agents command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.pixel-agents-bootstrap-complete which tells the Pixel Agents
    ...
    # Pixel Agents-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `pixel-agents update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".pixel-agents-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the PIXEL_AGENTS_HOME directory structure ($PixelAgentsHome, default %LOCALAPPDATA%\pixel-agents)
    New-Item -ItemType Directory -Force -Path "$PixelAgentsHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PixelAgentsHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PixelAgentsHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PixelAgentsHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PixelAgentsHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PixelAgentsHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PixelAgentsHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PixelAgentsHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PixelAgentsHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$PixelAgentsHome\.env"
    ...
    # Create config.yaml
    $configPath = "$PixelAgentsHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Pixel Agents's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$PixelAgentsHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in pixel_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $PixelAgentsHome"
    
    # Seed bundled skills into $PixelAgentsHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $PixelAgentsHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $PixelAgentsHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$PixelAgentsHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $PixelAgentsHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Pixel Agents-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'pixel-agents setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\pixel-agents-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\pixel-agents-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\pixel-agents-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Pixel Agents` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Pixel Agents.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_pixel_desktop command
    # resolves apps/desktop/release/win-unpacked/Pixel Agents.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Pixel Agents-managed
    ...
    # The --dir mode produces an unpacked Pixel Agents.exe in
    ...
    # are NOT trying to work around). The Pixel Agents icon + product name are
    # stamped onto Pixel Agents.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\pixel-agents-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Pixel Agents.exe",
        "$desktopDir\release\win-arm64-unpacked\Pixel Agents.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Pixel Agents.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Pixel Agents icon + identity are stamped onto Pixel Agents.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, pixel-agents#38216).
    ...
    #    Pixel Agents.exe. We deliberately do NOT point them at `pixel-agents desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Pixel Agents.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Pixel Agents.lnk')
        )
        ...
        # with the Pixel Agents icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.pixel-agents/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'pixel-agents setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m pixel_cli.main setup
    } else {
        python -m pixel_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $pixelAgentsCmd = "$InstallDir\venv\Scripts\pixel-agents.exe"
    if (-not (Test-Path $pixelAgentsCmd)) {
        $pixelAgentsCmd = "pixel-agents"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$PixelAgentsHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'pixel-agents whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: pixel-agents gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$PixelAgentsHome\logs\gateway.log"
            Start-Process -FilePath $pixelAgentsCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$PixelAgentsHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: pixel-agents gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: pixel-agents gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$PixelAgentsHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$PixelAgentsHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$PixelAgentsHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$PixelAgentsHome\pixel-agents\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   pixel-agents              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   pixel-agents setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   pixel-agents config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   pixel-agents config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   pixel-agents gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   pixel-agents update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Pixel Agents repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Pixel Agents-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Pixel Agents to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://api.pixelagents.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
