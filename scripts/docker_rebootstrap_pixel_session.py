#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead Pixel bootstrap session.

Background
----------
A Pixel bootstrap session (client_id ``pixel-agents-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.pixel.last_auth_error.relogin_required = true``. From then on every
...
``stage2-hook.sh`` seeds ``auth.json`` from ``PIXEL_AGENTS_AUTH_JSON_BOOTSTRAP`` only
...
``PIXEL_AGENTS_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Pixel
...
Design constraints
------------------
- Pure stdlib, no pixel_cli imports: runs early in the boot hook, before the
  app venv/modules are guaranteed importable, as its own subprocess.
- Surgical: replaces ONLY ``providers.pixel`` in the existing auth.json, leaving
...
# from PIXEL_AGENTS_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
# paths can never be confused: BOOTSTRAP seeds a fresh volume; REBOOTSTRAP
# overwrites a terminally-dead Pixel entry on an existing volume.
REBOOTSTRAP_ENV = "PIXEL_AGENTS_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "pixel-agents-cli-vps"


def _pixel_entry_is_terminal(pixel_state: Any) -> bool:
    """True iff the on-disk Pixel provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``pixel_cli.auth.get_pixel_session_validity``:
    ...
    """
    if not isinstance(pixel_state, dict):
        return False
    last_err = pixel_state.get("last_auth_error")
    ...
    if pixel_state.get("access_token") or pixel_state.get("refresh_token"):
        return False
    return True


def _extract_pixel_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.pixel`` block out of a PIXEL_AGENTS_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    PIXEL_AGENTS_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
    ...
    if not isinstance(pixel, dict) or not pixel:
        return None
    if pixel.get("client_id") != BOOTSTRAP_CLIENT_ID:
        return None
    if not (
        isinstance(pixel.get("access_token"), str)
        and pixel["access_token"].strip()
        and isinstance(pixel.get("refresh_token"), str)
        and pixel["refresh_token"].strip()
    ):
        return None
    return pixel
...
def _seed_is_newer(local_pixel: Any, seed_pixel: dict) -> bool:
    ...
    """
    if not isinstance(local_pixel, dict):
        return False
    local_obtained = _parse_timestamp(local_pixel.get("obtained_at"))
    seed_obtained = _parse_timestamp(seed_pixel.get("obtained_at"))
    return bool(
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "bad_seed"         — seed present but unparseable / no pixel entry
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             PIXEL_AGENTS_AUTH_JSON_BOOTSTRAP path handle it)
    ...
    seed_pixel = _extract_pixel_from_seed(seed_raw)
    ...
    local_pixel = providers.get("pixel")
    terminal = _pixel_entry_is_terminal(local_pixel)
    newer_seed = _seed_is_newer(local_pixel, seed_pixel)
    ...
    # Surgical replacement: swap ONLY providers.pixel, preserve everything else.
    providers["pixel"] = seed_pixel
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] Pixel bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    elif result == "reseeded_newer":
        print("[rebootstrap] Applied newer orchestrator-issued Pixel bootstrap session from "
              f"{REBOOTSTRAP_ENV}")
    else:
