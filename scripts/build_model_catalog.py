#!/usr/bin/env python3
"""Build the Pixel Agents Model Catalog — a centralized JSON manifest of curated models.

This script reads the in-repo hardcoded curated lists (``OPENROUTER_MODELS``,
``_PROVIDER_MODELS["pixel"]``) and writes them to a JSON manifest that the
Pixel Agents CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Pixel Agents release.
...
# Ensure PIXEL_AGENTS_HOME is set for imports that touch it at module level.
os.environ.setdefault("PIXEL_AGENTS_HOME", os.path.join(os.path.expanduser("~"), ".pixel-agents"))

from pixel_cli.models import (  # noqa: E402
    OPENROUTER_MODELS,
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "pixel-agents repo",
            "docs": "https://api.pixelagents.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        'The entry labeled "default": true is the model Pixel Agents '
            ...
            "pixel": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        "(partition_pixel_models_by_tier), not this manifest. "
                        'The entry labeled "default": true is the model Pixel Agents '
                        "silently lands on when the user never picked one."
                    ),
                },
                "models": [
                    _pixel_entry(mid)
                    for mid in _PROVIDER_MODELS.get("pixel", [])
                ],
            },
        },
    }
