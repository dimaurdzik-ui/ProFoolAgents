...
# Python-driven tool session (e.g. Pixel Agents terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:PixelResearch/pixel-agents.git"
REPO_URL_HTTPS="https://github.com/PixelResearch/pixel-agents.git"
PIXEL_AGENTS_HOME="${PIXEL_AGENTS_HOME:-$HOME/.pixel-agents}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${PIXEL_AGENTS_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$PIXEL_AGENTS_INSTALL_DIR"
...
#   code at /usr/local/lib/pixel-agents, command at /usr/local/bin/pixel-agents,
#   data still at /root/.pixel-agents (PIXEL_AGENTS_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$PIXEL_AGENTS_HOME/.no-bundled-skills so future"
            echo "                   'pixel-agents update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Pixel Agents.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.pixel-agents/pixel-agents"
            echo "                   default (root, Linux): /usr/local/lib/pixel-agents"
            echo "  --pixel-agents-home PATH  Data directory (default: ~/.pixel-agents, or \$PIXEL_AGENTS_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Pixel Agents installs the code under"
            echo "  /usr/local/lib/pixel-agents and links the command into"
            echo "  /usr/local/bin/pixel-agents (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$PIXEL_AGENTS_HOME"
            echo "  (default /root/.pixel-agents).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$PIXEL_AGENTS_HOME/pixel-agents are preserved in-place."
...
print_banner() {
    ...
    echo "│             ⚕ Pixel Agents Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by Pixel Agents.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `pixel-agents update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Pixel Agents-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Pixel Agents","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install pixel-agents command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `pixel-agents` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $PIXEL_AGENTS_HOME/pixel-agents
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $PIXEL_AGENTS_HOME/pixel-agents
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/pixel-agents
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $PIXEL_AGENTS_HOME/pixel-agents — then preserve it)
#
# Always no-op when the user set --dir or $PIXEL_AGENTS_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in PIXEL_AGENTS_HOME.
    if is_termux; then
        INSTALL_DIR="$PIXEL_AGENTS_HOME/pixel-agents"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$PIXEL_AGENTS_HOME/pixel-agents/.git" ]; then
            INSTALL_DIR="$PIXEL_AGENTS_HOME/pixel-agents"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/pixel-agents)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/pixel-agents"
        ...
        # /usr/local/bin/pixel-agents wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/pixel-agents"
        log_info "  Data:    $PIXEL_AGENTS_HOME (unchanged)"
...
# Point a Pixel Agents-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $PIXEL_AGENTS_HOME/node/bin — which is NOT on
...
# Pixel Agents's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Pixel Agents-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$PIXEL_AGENTS_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$PIXEL_AGENTS_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$PIXEL_AGENTS_HOME/node/etc/npmrc"
}

get_pixel_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/pixel-agents" ]; then
        echo "$link_dir/pixel-agents"
    else
        echo "pixel-agents"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://api.pixelagents.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Pixel Agents owns its own uv at $PIXEL_AGENTS_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (pixel_cli/managed_uv.py) looks in the same
    # place, so install.sh and `pixel-agents update` stay in sync.
    local _managed_uv="$PIXEL_AGENTS_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $PIXEL_AGENTS_HOME/bin ..."
    mkdir -p "$PIXEL_AGENTS_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/pixel-agents-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/pixel-agents-uv-installer.$$.sh")"
    ...
    # directly into $PIXEL_AGENTS_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$PIXEL_AGENTS_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Pixel Agents-
...
check_node() {
    ...
    # Repair pre-existing Pixel Agents-managed installs where `npm install -g` lands
    ...
    # Prefer a Pixel Agents-managed Node from a previous run over a too-old system one.
    if [ -x "$PIXEL_AGENTS_HOME/node/bin/node" ] && node_satisfies_build "$("$PIXEL_AGENTS_HOME/node/bin/node" --version)"; then
        export PATH="$PIXEL_AGENTS_HOME/node/bin:$PATH"
        log_success "Node.js $("$PIXEL_AGENTS_HOME/node/bin/node" --version) found (Pixel Agents-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Pixel Agents-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.pixel-agents/node/..."
    ...
    # Place into ~/.pixel-agents/node/ and symlink binaries into the same bin dir
    # the pixel-agents command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$PIXEL_AGENTS_HOME/node"
    mkdir -p "$PIXEL_AGENTS_HOME"
    mv "$extracted_dir" "$PIXEL_AGENTS_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$PIXEL_AGENTS_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$PIXEL_AGENTS_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$PIXEL_AGENTS_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$PIXEL_AGENTS_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.pixel-agents/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Pixel Agents install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Pixel Agents itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `pixel-agents update` path
                # (#4735).
                ...
                local stash_name
                stash_name="pixel-agents-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``pixel-agents update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Pixel Agents behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"pixel-agents\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up pixel-agents command..."

    if [ "$USE_VENV" = true ]; then
        PIXEL_AGENTS_BIN="$INSTALL_DIR/venv/bin/python"
        PIXEL_AGENTS_ENTRYPOINT="$INSTALL_DIR/pixel-agents"
    else
        PIXEL_AGENTS_BIN="$(which pixel-agents 2>/dev/null || echo "")"
        if [ -z "$PIXEL_AGENTS_BIN" ]; then
            log_warn "pixel-agents not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$PIXEL_AGENTS_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$PIXEL_AGENTS_ENTRYPOINT" ]; }; then
        log_warn "Pixel Agents launcher prerequisites not found"
    ...
    # Create a user-facing shim for the pixel-agents command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $PIXEL_AGENTS_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$PIXEL_AGENTS_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/pixel-agents"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/pixel-agents" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$PIXEL_AGENTS_BIN" "$PIXEL_AGENTS_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/pixel-agents" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$PIXEL_AGENTS_BIN" "\$@"
EOF
    fi
    chmod +x "$command_link_dir/pixel-agents"
    log_success "Installed pixel-agents launcher → $command_link_display_dir/pixel-agents"

    # Also expose `pixel-agents-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `pixel-agents-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Pixel Agents as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/pixel-agents-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/pixel-agents-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$PIXEL_AGENTS_BIN" "$PIXEL_AGENTS_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/pixel-agents-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$PIXEL_AGENTS_BIN" acp "\$@"
EOF
    fi
    chmod +x "$command_link_dir/pixel-agents-acp"
    log_success "Installed pixel-agents-acp launcher → $command_link_display_dir/pixel-agents-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "pixel-agents command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v pixel-agents' \
                >/dev/null 2>&1; then
        ...
        log_info "pixel-agents not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Pixel Agents — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Pixel Agents — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Pixel Agents — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so pixel-agents works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.pixel-agents directory structure (config at top level, code in subdir)
    mkdir -p "$PIXEL_AGENTS_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.pixel-agents/.env (top level, easy to find)
    if [ ! -f "$PIXEL_AGENTS_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$PIXEL_AGENTS_HOME/.env"
            log_success "Created ~/.pixel-agents/.env from template"
        else
            touch "$PIXEL_AGENTS_HOME/.env"
            log_success "Created ~/.pixel-agents/.env"
        fi
    else
        log_info "~/.pixel-agents/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$PIXEL_AGENTS_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.pixel-agents/config.yaml (top level, easy to find)
    if [ ! -f "$PIXEL_AGENTS_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$PIXEL_AGENTS_HOME/config.yaml"
            log_success "Created ~/.pixel-agents/config.yaml from template"
        fi
    else
        log_info "~/.pixel-agents/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in pixel_cli/default_soul.py — the
    ...
    if [ ! -f "$PIXEL_AGENTS_HOME/SOUL.md" ]; then
        cat > "$PIXEL_AGENTS_HOME/SOUL.md" << 'SOUL_EOF'
You are Pixel Agents, an intelligent AI assistant created by Pixel Agents. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.pixel-agents/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.pixel-agents/"

    # Seed bundled skills into ~/.pixel-agents/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `pixel-agents update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'pixel-agents update'." \
            > "$PIXEL_AGENTS_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $PIXEL_AGENTS_HOME/.no-bundled-skills"
        log_info "  Future 'pixel-agents update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.pixel-agents/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.pixel-agents/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$PIXEL_AGENTS_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$PIXEL_AGENTS_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.pixel-agents/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$PIXEL_AGENTS_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Pixel Agents browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Pixel Agents will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Pixel Agents browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (pixel-agents --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `pixel-agents update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'pixel-agents setup' after install."
        return 0
    fi
    ...
    # Run pixel-agents setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m pixel_cli.main setup < /dev/tty
    else
        python -m pixel_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$PIXEL_AGENTS_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Pixel Agents to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$PIXEL_AGENTS_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'pixel-agents whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                PIXEL_AGENTS_CMD="$(get_pixel_command_path)"
                $PIXEL_AGENTS_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'pixel-agents whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'pixel-agents gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $PIXEL_AGENTS_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $PIXEL_AGENTS_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: pixel-agents gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: pixel-agents gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $PIXEL_AGENTS_CMD gateway > "$PIXEL_AGENTS_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.pixel-agents/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: pixel-agents gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: pixel-agents gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.pixel-agents-bootstrap-complete, which tells the Pixel Agents
    ...
    local marker_path="$INSTALL_DIR/.pixel-agents-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $PIXEL_AGENTS_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $PIXEL_AGENTS_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $PIXEL_AGENTS_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}pixel-agents${NC}              Start chatting"
    echo -e "   ${GREEN}pixel-agents setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}pixel-agents config${NC}       View/edit configuration"
    echo -e "   ${GREEN}pixel-agents config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}pixel-agents gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}pixel-agents update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'pixel-agents' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'pixel-agents' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'pixel-agents' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$PIXEL_AGENTS_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$PIXEL_AGENTS_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$PIXEL_AGENTS_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$PIXEL_AGENTS_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$PIXEL_AGENTS_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
# extract a tree MISSING the electron binary, so the `electron`->`Pixel Agents` rename
...
# _purge_electron_build_cache() used by `pixel-agents desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Pixel Agents-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `pixel-agents desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Pixel Agents" ]; then
            app="$desktop_dir/release/linux-unpacked/Pixel Agents"
        elif [ -x "$desktop_dir/release/linux-unpacked/pixel-agents" ]; then
            app="$desktop_dir/release/linux-unpacked/pixel-agents"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Pixel Agents.app" \
            "$desktop_dir/release/mac/Pixel Agents.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `pixel-agents desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading PIXEL_AGENTS_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if PIXEL_AGENTS_HOME="$PIXEL_AGENTS_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from pixel_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `pixel-agents update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Pixel Agents-managed Node
            # provisioned during prerequisites/node-deps (at $PIXEL_AGENTS_HOME/node/bin)
        ...
        complete)
            ...
            # $PIXEL_AGENTS_HOME. $PIXEL_AGENTS_HOME is a shared data dir (it can be
            # bind-mounted into a Docker gateway too), so a stamp there gets
            # clobbered by the container's 'docker' stamp and wrongly blocks
            # 'pixel-agents update' on this host install. See detect_install_method().
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $PIXEL_AGENTS_HOME.
    # $PIXEL_AGENTS_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'pixel-agents update' on this host install.
