...
When Pixel Agents runs in TUI mode, the gateway child process communicates with
...
# ``get_pixel_agents_home() / "plugins"`` (user) and ``./.pixel-agents/plugins/`` (project,
# gated behind ``PIXEL_AGENTS_ENABLE_PROJECT_PLUGINS``) — see
# ``pixel_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``pixel_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import pixel_constants (this script
    ...
    # plugins in get_pixel_agents_home()/plugins/ and project plugins in
    # ./.pixel-agents/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_pixel_agents_home() / "plugins"]
    if os.environ.get("PIXEL_AGENTS_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".pixel-agents" / "plugins")
