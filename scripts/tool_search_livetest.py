#!/usr/bin/env python3
"""Live test harness for Pixel Agents's Tool Search feature.
...
# Force-isolate the test environment BEFORE any pixel-agents imports.
ORIGINAL_HOME = os.environ.get("PIXEL_AGENTS_HOME")
ORIGINAL_AUTH = Path.home() / ".pixel-agents" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.pixel-agents/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.pixel-agents/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="pixel_ts_live_"))
    pixel_home = home_dir / ".pixel-agents"
    pixel_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, pixel_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".pixel-agents" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, pixel_home / ".env")
        ...
        load_pixel_dotenv(pixel_home=str(Path.home() / ".pixel-agents"))
    ...
    (pixel_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return pixel_home
...
def reset_module_state():
    """Drop cached modules so the new PIXEL_AGENTS_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "pixel_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["PIXEL_AGENTS_HOME"] = str(home)
...
def main():
    ...
    # Restore original PIXEL_AGENTS_HOME
    if ORIGINAL_HOME is not None:
        os.environ["PIXEL_AGENTS_HOME"] = ORIGINAL_HOME
    else:
