"""Run a real Pixel Agents CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_pixel_executable(pixel_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "pixel-agents",
        Path(".venv") / "Scripts" / "pixel-agents.exe",
    ):
        candidate = pixel_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("pixel-agents")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Pixel Agents executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Pixel Agents source checkout containing .venv/bin/pixel-agents",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated PIXEL_AGENTS_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if set(by_name) != {
        "pixel-agents.model_call.count",
        "pixel-agents.task_run.finished",
        "pixel-agents.task_run.started",
    }:
        raise AssertionError(
            f"Unexpected SQLite counters:\n{json.dumps(counters, indent=2)}"
        )
    expected_model = {
        "name": "pixel-agents.model_call.count",
        "dimensions": {
            "call_role": "primary",
            "locality": "local",
            "model_family": "gpt",
    ...
    if by_name["pixel-agents.model_call.count"] != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['pixel-agents.model_call.count']}"
        )
    expected_start = {
        "name": "pixel-agents.task_run.started",
        "dimensions": {
    ...
    if by_name["pixel-agents.task_run.started"] != expected_start:
        raise AssertionError(
            f"Unexpected task start: {by_name['pixel-agents.task_run.started']}"
        )
    terminal = by_name["pixel-agents.task_run.finished"]
...
def _validate_package(outbox: Path, schema_path: Path) -> tuple[Path, dict[str, Any]]:
    ...
    try:
        ...
        raise RuntimeError(
            "The Pixel Agents development environment requires jsonschema"
        ) from exc
    ...
    if metrics["pixel-agents.model_call.count"]["dimensions"] != {
        "call_role": "primary",
        "locality": "local",
        "model_family": "gpt",
        "outcome": "success",
        ...
        raise AssertionError(
            f"Unexpected model metric: {metrics['pixel-agents.model_call.count']}"
        )
    terminal = metrics["pixel-agents.task_run.finished"]
...
def main() -> int:
    args = _arguments()
    pixel_repo = args.pixel_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    pixel-agents = _resolve_pixel_executable(pixel_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="pixel-agents-relay-shared-metrics-"))
    home = root / "pixel-agents-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["PIXEL_AGENTS_HOME"] = str(home)
    ...
    (root / "pixel-agents.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "pixel-agents.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Pixel Agents exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    if not _ModelHandler.requests:
        raise AssertionError("Pixel Agents did not call the local model endpoint")
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Pixel Agents model request did not contain the prompt canary")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Pixel Agents did not print the mock model response")
    ...
    package_path, package = _validate_package(
        ...
        / "observability"
        / "schemas"
        / "pixel-agents.shared_metrics.v1.schema.json",
    )

    print("Pixel Agents -> NeMo Relay shared-metrics smoke test passed")
