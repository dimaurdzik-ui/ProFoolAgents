"""Windows UTF-8 bootstrap for Pixel Agents entry points.
...
should be imported at the very top of every Pixel Agents entry point
(``pixel-agents``, ``pixel-agents``, ``pixel-agents-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``pixel_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``pixel_bootstrap``, never ``pixel_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Pixel Agents modules.

    Pixel Agents ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Pixel Agents source root
    ...
    """
    root = src_root or os.environ.get("PIXEL_AGENTS_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``PIXEL_AGENTS_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("PIXEL_AGENTS_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import pixel_bootstrap``
# (or ``from pixel_bootstrap import apply_windows_utf8_bootstrap``) at
