# Pixel Agents Has Had "Routines" Since March
...
Pixel Agents equivalent — works today:
```bash
...
Pixel Agents equivalent — works today:
```bash
pixel-agents webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Pixel Agents implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Pixel Agents |
|---|---|---|
...
## Things Pixel Agents Does That Routines Can't
...
```bash
pixel-agents cron create "every 1h" \
  "If CHANGE DETECTED, summarize what changed. If NO_CHANGE, respond with [SILENT]." \
  --script ~/.pixel-agents/scripts/watch-site.py \
...
Pixel Agents has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Pixel Agents is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
pixel-agents gateway setup    # enable webhooks
pixel-agents webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [api.pixelagents.com/docs/reference/automation-blueprints-catalog](https://api.pixelagents.com/docs/reference/automation-blueprints-catalog)

Documentation: [api.pixelagents.com](https://api.pixelagents.com)

GitHub: [github.com/PixelResearch/pixel-agents](https://github.com/PixelResearch/pixel-agents)

---

*Pixel Agents is built by [Pixel Agents](https://pixelagents.com). Open source, model-agnostic, runs on your infrastructure.*
