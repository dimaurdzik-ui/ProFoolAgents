...
type Props = {
  ...
  // Look up the OSC 8 hyperlink at (col, row) synchropixelly at click
  ...
  handleSetRawMode = (isEnabled: boolean): void => {
    ...
    if (isEnabled) {
      ...
      if (this.rawModeEnabledCount === 0) {
        ...
        // Deferred to next tick so it fires AFTER the current synchropixel
...
export function handleMouseEvent(app: App, m: ParsedMouse): void {
  ...
  if (!hasSelection(sel) && sel.anchor) {
    ...
    if (!app.props.onClickAt(col, row)) {
      // Resolve the hyperlink URL synchropixelly while the screen buffer
