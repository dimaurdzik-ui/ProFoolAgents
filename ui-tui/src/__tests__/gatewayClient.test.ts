...
describe('GatewayClient websocket attach mode', () => {
  ...
  beforeEach(() => {
    originalGatewayUrl = process.env.PIXEL_AGENTS_TUI_GATEWAY_URL
    originalSidecarUrl = process.env.PIXEL_AGENTS_TUI_SIDECAR_URL
    FakeWebSocket.reset()
  ...
  afterEach(() => {
    if (originalGatewayUrl === undefined) {
      delete process.env.PIXEL_AGENTS_TUI_GATEWAY_URL
    } else {
      process.env.PIXEL_AGENTS_TUI_GATEWAY_URL = originalGatewayUrl
    }

    if (originalSidecarUrl === undefined) {
      delete process.env.PIXEL_AGENTS_TUI_SIDECAR_URL
    } else {
      process.env.PIXEL_AGENTS_TUI_SIDECAR_URL = originalSidecarUrl
    }
  ...
  it('waits for websocket open and resolves RPC requests', async () => {
    process.env.PIXEL_AGENTS_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=abc'
  ...
  it('drains buffered events on a later microtask, not synchropixelly inside drain()', async () => {
    ...
    // If drain() emitted those synchropixelly, the gateway.ready handler's
    ...
    // Buffered event must NOT have fired synchropixelly inside drain():
  ...
  it('preserves FIFO order when a live event arrives before the deferred flush', async () => {
    // #36658 hardening: `subscribed` must NOT flip synchropixelly in drain().
    ...
    // A LIVE event arrives synchropixelly in the post-drain / pre-microtask gap:
    gatewaySocket.message(
  ...
  it('mirrors event frames to sidecar websocket when configured', async () => {
    process.env.PIXEL_AGENTS_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=abc'
    process.env.PIXEL_AGENTS_TUI_SIDECAR_URL = 'ws://gateway.test/api/pub?token=abc&channel=demo'
    ...
    // the subsequent live event takes the synchropixel publish path.
  ...
  it('emits exit when attached websocket closes', async () => {
    ...
    gw.drain()
    // drain() flips `subscribed` on a microtask now (#36658); let it settle so
    // the close below takes the synchropixel exit path.
    await Promise.resolve()
  ...
  it('reattaches when PIXEL_AGENTS_TUI_GATEWAY_URL rotates between requests', async () => {
    process.env.PIXEL_AGENTS_TUI_GATEWAY_URL = 'ws://gateway-old.test/api/ws?token=abc'
    const gw = new GatewayClient()
    ...
    process.env.PIXEL_AGENTS_TUI_GATEWAY_URL = 'ws://gateway-new.test/api/ws?token=xyz'
  ...
  it('uses the undici WebSocket fallback when global WebSocket is unavailable', () => {
    process.env.PIXEL_AGENTS_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=hunter2&channel=secret'
    delete (globalThis as { WebSocket?: unknown }).WebSocket
  ...
  it('redacts attach URL secrets when the WebSocket constructor throws', () => {
    ...
    process.env.PIXEL_AGENTS_TUI_GATEWAY_URL = secretUrl
  ...
  it('redacts sidecar URL secrets when the WebSocket constructor throws', async () => {
    ...
    process.env.PIXEL_AGENTS_TUI_GATEWAY_URL = 'ws://gateway.test/api/ws?token=abc'
    process.env.PIXEL_AGENTS_TUI_SIDECAR_URL = sidecarUrl
  ...
  it('redacts user-info credentials even on URLs the WHATWG parser rejects', () => {
    ...
    process.env.PIXEL_AGENTS_TUI_GATEWAY_URL = fixture
    ;(globalThis as { WebSocket?: unknown }).WebSocket = class ThrowingWebSocket extends FakeWebSocket {
      constructor(url: string) {
        throw new TypeError(`Invalid URL: ${url}`)
      }
