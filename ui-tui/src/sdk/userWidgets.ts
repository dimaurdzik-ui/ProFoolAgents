...
/**
 * User widget apps — Pixel Agents authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$PIXEL_AGENTS_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.pixel-agents/plugins/`: files under PIXEL_AGENTS_HOME execute
...
const widgetsDir = () => join(process.env.PIXEL_AGENTS_HOME?.trim() || join(homedir(), '.pixel-agents'), 'tui-widgets')
 ...
 *  every change, so a widget Pixel Agents writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Pixel Agents) ever writes must hot-load too — a 10s poll here read
