...
// Deferred @pixel-agents/ink import: loading `@pixel-agents/ink` at module top-level
...
async function _ensureEvictInkCaches(): Promise<(level: 'all' | 'half') => unknown> {
  ...
  _evictInkCachesPromise ??= import('@pixel-agents/ink')
    .then(mod => {
...
export function startMemoryMonitor({
  ...
  const cooldownRaw = process.env.PIXEL_AGENTS_AUTO_HEAPDUMP_COOLDOWN_MS?.trim()
  ...
  const tick = async () => {
    ...
    // Deferred import keeps `@pixel-agents/ink` off the cold-start critical path;
