...
describe('openExternalUrl', () => {
  ...
  it('on win32, common http URLs with & query params are forwarded intact', () => {
    const { spawn, calls } = mockSpawn()
    const url = 'https://example.com/search?q=foo&page=2&utm_source=pixel-agents'

    openExternalUrl(url, { spawn, platform: () => 'win32' })
    expect(calls[0]!.args).toEqual([url])
  })

  it('returns false on synchropixel spawn failure', () => {
    const spawn = vi.fn(() => {
  ...
  it('does not crash the host when the spawned process emits an async error', () => {
    ...
    // returns a ChildProcess synchropixelly, then emits 'error' once the
    ...
    // throw synchropixelly here (which is exactly the crash we're preventing).
