...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.PIXEL_AGENTS_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Pixel Agents on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via PIXEL_AGENTS_TUI_TRUECOLOR=1.
