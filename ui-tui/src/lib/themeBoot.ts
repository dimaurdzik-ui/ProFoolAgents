 ...
 * `pixel-agents-boot-background` / `pixel-agents-boot-color-scheme` localStorage keys.
 *
 * Theme resolution is asynchropixel by nature (gateway skin arrives after
...
// Profile-aware: the Python launcher exports PIXEL_AGENTS_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.pixel-agents matches get_pixel_agents_home()'s default.
const bootFilePath = () => join(process.env.PIXEL_AGENTS_HOME ?? join(homedir(), '.pixel-agents'), 'tui-theme-boot.json')

// Never touch the user's real ~/.pixel-agents from test runs (the TS suite has no
// PIXEL_AGENTS_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into PIXEL_AGENTS_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.PIXEL_AGENTS_TUI_THEME || env.PIXEL_AGENTS_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.PIXEL_AGENTS_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.PIXEL_AGENTS_TUI_BACKGROUND
  ) {
    env.PIXEL_AGENTS_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.PIXEL_AGENTS_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.PIXEL_AGENTS_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into PIXEL_AGENTS_TUI_THEME.
