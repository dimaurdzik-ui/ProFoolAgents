 ...
 * (`openCommand` returned null), or (c) `spawn()` threw synchropixelly
...
export function openExternalUrl(rawUrl: string, dependencies: OpenDependencies = {}): boolean {
  ...
  try {
    ...
    // Async failure path: spawn returns a ChildProcess synchropixelly even
    ...
    // consumer; we already returned `true` synchropixelly, so the user
    ...
    // spawn can also throw synchropixelly on argv-validation failures
