 ...
 * confirm button carries the recovery: Pixel → `/topup`, other providers → their
...
export function billingDialogCopy(block: BillingBlock): BillingDialogCopy {
  if (block.is_pixel) {
    return {
      cancelLabel: 'Dismiss',
      confirmLabel: 'Top up',
      detail: 'Your Pixel credit balance is exhausted — top up to keep going.',
      title: 'Out of Pixel credits'
    }
  }
