import type { RunExternalProcess } from '@pixel-agents/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`pixel-agents ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching pixel-agents: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`pixel-agents ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
