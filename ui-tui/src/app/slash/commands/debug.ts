...
import { terminalBackgroundHex } from '@pixel-agents/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $PIXEL_AGENTS_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see PIXEL_AGENTS_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['PIXEL_AGENTS_TUI_BACKGROUND', process.env.PIXEL_AGENTS_TUI_BACKGROUND ?? '(unset)'],
            ['PIXEL_AGENTS_TUI_THEME', process.env.PIXEL_AGENTS_TUI_THEME ?? '(unset)'],
