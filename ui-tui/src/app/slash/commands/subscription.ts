...
function buildManageUrl(s: SubscriptionStateResponse, tierId?: string): string | null {
  ...
  // on the Python side (e.g. https://portal.pixelagents.com/billing). Strip any
...
export const subscriptionCommands: SlashCommand[] = [
  {
    help: 'View or change your Pixel subscription plan',
    name: 'subscription',
    aliases: ['upgrade'],
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<SubscriptionStateResponse>(s => {
            if (!s.logged_in) {
              sys('Not logged into Pixel Portal — run /portal to log in, then /subscription.')
