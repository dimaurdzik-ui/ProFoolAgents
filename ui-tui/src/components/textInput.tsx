import type { InputEvent, Key } from '@pixel-agents/ink'
import * as Ink from '@pixel-agents/ink'
 ...
 * but that accepts `key.meta` on macOS — and pixel-agents-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.PIXEL_AGENTS_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
