import { Box, Text, useStdout } from '@pixel-agents/ink'
...
const TAG_FULL = 'Pixel Agents · Messenger of the Digital Gods'
const TAG_MID = 'Messenger of the Digital Gods'
const TAG_TINY = 'Pixel Agents'
const HIDE_BELOW = 34
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  // pixel_cli/banner.py) and the "connected" label on the collapse toggle.
  ...
  const heroColumn = wide ? (
    <Box flexDirection="column" width="100%">
      ...
      <Text color={t.color.accent}>
        ...
        <Text color={t.color.muted}> · Pixel Agents</Text>
      </Text>
    ...
    <Box flexDirection="column" width="100%">
      ...
      {typeof info.update_behind === 'number' && info.update_behind > 0 && (
        <Text bold color={t.color.warn}>
          ...
          <Text bold color={t.color.warn}>
            {info.update_command || 'pixel-agents update'}
          </Text>
          <Text bold={false} color={t.color.warn} dimColor>
            {' '}
            to update
