...
React + Ink terminal UI for Pixel Agents. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `PIXEL_AGENTS_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `pixel-agents setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage Pixel remote spending — buy credits, auto-reload, limits
...
- Input history is stored in `~/.pixel-agents/.pixel_history` or under `PIXEL_AGENTS_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage Pixel remote spending — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — Pixel credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `pixel-agents setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/pixel-agents-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external pixel-agents setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage Pixel remote spending
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.pixel-agents/tui-parent.log
    ...
    types/
      pixel-agents-ink.d.ts            type declarations for @pixel-agents/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent PixelAgentsCLI subprocess for slash commands
```
