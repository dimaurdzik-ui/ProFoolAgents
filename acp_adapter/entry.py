"""CLI entry point for the pixel-agents ACP adapter.

Loads environment variables from ``~/.pixel-agents/.env``, configures logging
...
try:
    ...
    # shadowing Pixel Agents's own modules — ``pixel-agents acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from PIXEL_AGENTS_HOME (default ``~/.pixel-agents``)."""
    from pixel_cli.env_loader import load_pixel_dotenv

    pixel_home = get_pixel_agents_home()
    loaded = load_pixel_dotenv(pixel_home=pixel_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", pixel_home / ".env"
        )


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="pixel-agents-acp",
        description="Run Pixel Agents as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Pixel Agents version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Pixel Agents provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.pixel-agents/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import PixelAgentsACPAgent  # noqa: F401

    print("Pixel Agents ACP check OK")


def _run_setup() -> None:
    from pixel_cli.main import main as pixel_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "pixel-agents", "model"]
        pixel_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting pixel-agents ACP adapter")
    ...
    import acp
    from .server import PixelAgentsACPAgent
    ...
    if os.environ.get("PIXEL_AGENTS_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
