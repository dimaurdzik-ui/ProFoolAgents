"""ACP session manager — maps ACP sessions to Pixel Agents AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.pixel-agents/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Pixel Agents itself is running in WSL.

    Windows ACP clients can launch ``pixel-agents acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from pixel_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Pixel Agents from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["pixel-agents-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Pixel Agents agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Pixel Agents AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Pixel Agents runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.pixel-agents/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``PIXEL_AGENTS_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_pixel_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=pixel_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _restore(self, session_id: str) -> Optional[SessionState]:
        ...
        # for the rest of the session (see pixel_state.get_messages_as_conversation).
        try:
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Pixel Agents daemon's process cwd.
