"""ACP agent server — exposes Pixel Agents via the Agent Client Protocol."""
...
from tools.approval import (
    reset_pixel_interactive_context,
    set_pixel_interactive_context,
)
...
try:
    from pixel_cli import __version__ as PIXEL_AGENTS_VERSION
except Exception:
    PIXEL_AGENTS_VERSION = "0.0.0"

# Thread pool for running AIAgent (synchropixel) in parallel.
...
# already uses (`pixel_cli/moa_cmd.py`). This bounds each provider's row, not
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Pixel Agents running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Pixel Agents cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Pixel Agents/OpenAI-compatible user content payload."""
...
class PixelAgentsACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Pixel Agents AIAgent."""
    ...
    _SLASH_COMMANDS = {
        ...
        "version": "Show Pixel Agents version",
    }
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Pixel Agents version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Pixel Agents maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Pixel Agents inventory is also used by ``pixel-agents model``, the TUI,
        ...
        try:
            ...
            from pixel_cli.models import normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from pixel_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Pixel Agents estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_pixel_session_id: str,
        previous_pixel_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.pixel-agents.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_pixel_session_id,
                previous_pixel_session_id=previous_pixel_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_pixel_session_id: Optional[str] = None,
        previous_pixel_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Pixel Agents changes it.

        When the internal Pixel Agents head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_pixel_session_id`` so the
        attached ``_meta.pixel-agents.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # pixel_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_pixel_session_id or session_id,
            previous_pixel_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["pixel-agents-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="pixel-agents", version=PIXEL_AGENTS_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Pixel Agents' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Pixel Agents setup/model selection out-of-band.
    ...
    def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
        ...
        Two distinct keys under ``_meta.pixel-agents`` (ACP's extensibility
        ...
        * ``containsCompactionSummary: true`` — a merged-tail message: real
        ...
        if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
            ...
            kind = "standalone"
        if kind == "standalone":
            return {"pixel-agents": {"compactionSummary": True}}
        if kind == "merged":
            return {"pixel-agents": {"containsCompactionSummary": True}}
        return None
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Pixel Agents
    ...
    async def load_session(
        ...
        **kwargs: Any,
        ...
        # spec-compliant ACP client that measures notifications synchropixelly
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Pixel Agents on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Pixel Agents' local kawaii waiting/status
        ...
        # tools.approval (set_pixel_interactive_context) rather than
        # os.environ["PIXEL_AGENTS_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue PIXEL_AGENTS_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind PIXEL_AGENTS_SESSION_KEY for this session so per-session caches
            ...
            try:
                ...
                # Without it the prompt advertises the global Pixel Agents workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.pixel-agents/workspace and the
            ...
            interactive_token = set_pixel_interactive_context(True)
            ...
            previous_session_id = os.environ.get("PIXEL_AGENTS_SESSION_ID")
            os.environ["PIXEL_AGENTS_SESSION_ID"] = session_id
            try:
                ...
                if interactive_token is not None:
                    reset_pixel_interactive_context(interactive_token)
                # Restore PIXEL_AGENTS_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("PIXEL_AGENTS_SESSION_ID", None)
                else:
                    os.environ["PIXEL_AGENTS_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Pixel Agents DB session id before the turn so we
            ...
            pre_turn_pixel_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (PIXEL_AGENTS_SESSION_KEY in
        ...
        # _meta.pixel-agents.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_pixel_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_pixel_id
            and pre_turn_pixel_id
            and post_turn_pixel_id != pre_turn_pixel_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_pixel_session_id=post_turn_pixel_id,
                    previous_pixel_session_id=pre_turn_pixel_id,
                )
            except Exception:
                logger.debug(
        ...
        # Pixel Agents' local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Pixel Agents install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Pixel Agents v{PIXEL_AGENTS_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Pixel Agents has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
