"""ACP auth helpers — detect and advertise Pixel Agents authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "pixel-agents-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Pixel Agents runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Pixel Agents can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Pixel Agents.
    ...
    not have Pixel Agents provider credentials configured yet, so Pixel Agents always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Pixel Agents using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Pixel Agents provider",
            description=(
                "Open Pixel Agents' interactive model/provider setup in a terminal. "
                "Use this when Pixel Agents has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
