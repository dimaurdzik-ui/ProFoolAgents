...
``pixel_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import pixel_state (cycle) — shared
module-level constants live in pixel_state_common.
"""
...
from pixel_state_common import (
    DEFERRED_INDEX_SQL,
    FTS_SQL,
    FTS_STORAGE_VERSION,
    FTS_TRIGRAM_SQL,
...
# Moved methods logged under the "pixel_state" logger before the split;
...
class SessionSchemaMixin:
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._pixel_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._pixel_fts5_probe")
            return True
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `pixel-agents sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `pixel-agents db optimize`) must keep its EXISTING
