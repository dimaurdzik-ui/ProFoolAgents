"""
Pixel Agents MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Pixel Agents-specific extra)

Usage:
    pixel-agents mcp serve
    pixel-agents mcp serve --verbose
...
logger = logging.getLogger("pixel-agents.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using PIXEL_AGENTS_HOME."""
    from pixel_constants import get_pixel_agents_home

    return get_pixel_agents_home() / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    directory_file = get_pixel_agents_home() / "channel_directory.json"
...
class EventBridge:
    ...
    This is the Pixel Agents equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        db_file = get_pixel_agents_home() / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Pixel Agents MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "pixel-agents",
        instructions=(
            "Pixel Agents messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Pixel Agents MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
