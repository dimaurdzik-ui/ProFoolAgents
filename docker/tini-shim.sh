...
# Pixel Agents WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: PIXEL_AGENTS_TINI_SHIM_TARGET overrides the `/init` path (and
# PIXEL_AGENTS_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${PIXEL_AGENTS_TINI_SHIM_TARGET:-/init}"
WRAPPER="${PIXEL_AGENTS_TINI_SHIM_WRAPPER:-/opt/pixel-agents/docker/main-wrapper.sh}"
