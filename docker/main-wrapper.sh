...
# shellcheck shell=sh
# /opt/pixel-agents/docker/main-wrapper.sh — wraps the container's CMD with
# the same argument-routing logic the pre-s6 entrypoint.sh used. Runs
# as /init's "main program" (Docker CMD) so it inherits stdin/stdout/
# stderr from the container.
#
# Shebang note: /init scrubs env before invoking CMD, so a plain
# `#!/bin/sh` wrapper sees an empty environ and `ENV PIXEL_AGENTS_HOME=/opt/data`
# from the Dockerfile never reaches `pixel-agents`. with-contenv repopulates
# the env from /run/s6/container_environment before exec'ing, which is
# what s6-supervised services use too (see main-pixel-agents/run).
#
# Routing:
#   no args                       → exec `pixel-agents` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `pixel-agents <args>` (subcommand passthrough)
#
# Drop to pixel-agents via s6-setuidgid, but skip it when already non-root.
set -e

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid pixel-agents "$@"; exec "$@"; }
...
# an arbitrary non-root, non-pixel-agents UID, the bootstrap was skipped and the
# baked image dirs (owned by the pixel-agents build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u pixel-agents)" ]; then
    cat >&2 <<EOF
[pixel-agents] ERROR: container started with --user $cur_uid (an arbitrary, non-pixel-agents UID) — not supported.
...
# to the pixel-agents user's home before dropping privileges so libraries that
...
_pixel_orig_cwd="${PIXEL_AGENTS_ORIG_CWD:-$PWD}"
...
# the user's command so `pixel-agents chat` starts in the Docker -w
...
# Pixel Agents subcommand pass-through.
drop pixel-agents "$@"
