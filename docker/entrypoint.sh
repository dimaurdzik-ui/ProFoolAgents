...
# by /etc/cont-init.d/01-pixel-agents-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs pixel-agents"
...
echo "[pixel-agents] WARNING: docker/entrypoint.sh is a deprecated shim under " \
