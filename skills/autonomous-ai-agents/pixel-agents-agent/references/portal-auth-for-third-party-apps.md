# Pixel Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my Pixel Portal subscription without me
...
## Layer 1 — Is this thing a Pixel Agents plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Pixel Agents process**. Its LLM calls go through Pixel Agents's already-configured provider. | Already uses Portal if user's Pixel Agents is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `inference-api.pixelagents.com`. | Layer 2/3 below. |

**Pitfall to avoid**: do not pitch "OAuth into Portal" as the solution for a
plugin that already runs inside Pixel Agents. That LLM call is already authenticated
via Pixel Agents's provider config. The plugin's own server auth (e.g.
...
Portal at `https://inference-api.pixelagents.com/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.pixelagents.com → API Keys`, or
...
"Sign in with Pixel Portal" SSO that third-party apps can register as clients
against. There is no shared cookie or session that browser-Portal-login
extends to other apps on the same machine.

What Pixel Agents has that *feels* like OAuth — `pixel-agents login --provider pixel`
opening a browser, user signs in, token lands in `~/.pixel-agents/auth.json` — is a
**Pixel Agents-specific browser flow**. Under the hood it produces a credential
Pixel Agents uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Pixel Agents's existing Portal credential out of `~/.pixel-agents/auth.json`.
2. Expose a local OpenAI-compatible endpoint at `http://localhost:NNNN/v1`.
3. Forward incoming requests to `inference-api.pixelagents.com/v1` with that
   bearer attached.
...
rides on the credential Pixel Agents already holds.

Where this could live in Pixel Agents:
...
Only when the consumer is *another first-party Pixel thing* (a future SDK, a
Pixel-branded extension, a Discord-bot integration that needs per-user
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Pixel Agents itself.
...
1. First decide: Pixel Agents plugin (runs inside Pixel Agents) or separate app? If plugin,
   it already uses Portal via Pixel Agents's provider config — done.
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://inference-api.pixelagents.com/v1`. Rate limits are
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Pixel Agents agent itself") is fully supported. Pixel Agents treats agent
...
pitch in the Tool Gateway announcement is specifically about Pixel Agents's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Pixel Agents is configured to use Portal as its provider. It is
