...
Author widget apps for the Pixel Agents TUI (`pixel-agents --tui`): glanceable ambient
...
- The TUI must be in use (`pixel-agents --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.pixel-agents/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.pixel-agents/logs/tui_gateway_crash.log` if a widget never appears.
