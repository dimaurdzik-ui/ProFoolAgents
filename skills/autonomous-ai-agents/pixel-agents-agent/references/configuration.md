...
Edit with `pixel-agents config edit` or `pixel-agents config set section.key value`.
Full reference: https://api.pixelagents.com/docs/user-guide/configuration
...
`pixel-agents config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `pixel-agents tools` (interactive) or `pixel-agents tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_PIXEL_AGENTS_CORE_TOOLS` is the default bundle most platforms inherit).
