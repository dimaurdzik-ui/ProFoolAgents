...
Registry of record: `pixel_cli/commands.py` (`COMMAND_REGISTRY`) — every
...
/snapshot [sub]          Create/restore Pixel Agents config+state snapshots (CLI)
...
/subscription (/upgrade) Pixel plan info (CLI)
/topup                   Pixel balance / billing
/copy [N]                Copy last response to clipboard (CLI)
/paste                   Attach clipboard image (CLI)
/image <path>            Attach a local image file (CLI)
/update                  Update Pixel Agents to latest
/version (/v)            Show version
/debug [pixel|local]      Upload debug report, get shareable links
```
