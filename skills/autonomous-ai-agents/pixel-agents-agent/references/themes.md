# Themes / Skins — Author a Pixel Agents Color Theme

Author a Pixel Agents **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`pixel_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.pixel-agents/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Pixel Agents itself.
...
- Write access to the Pixel Agents home dir — `~/.pixel-agents` by default, or `$PIXEL_AGENTS_HOME`
  / the active profile's dir. Skins live in `<pixel-agents-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `pixel-agents config set`).
...
3. `write_file` it to `<pixel-agents-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `pixel-agents config set display.skin default` (or they can `/skin default`).
...
```
pixel-agents skin set <key> <hex>      # e.g. pixel-agents skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `pixel-agents skin set background "#08201f"` changes
only the background; `pixel-agents skin use <name>` / `pixel-agents skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.pixel-agents`** when a profile is active — resolve the real home
  from `$PIXEL_AGENTS_HOME` first, falling back to `~/.pixel-agents`.
...
- **Name collisions**: a skin named like a desktop built-in (`mono`, `slate`,
  `cyberpunk`, `pixel`, `midnight`, `ember`) won't override that built-in on the
  GUI. Pick a fresh name.
- **Never hand-edit `config.yaml` to activate.** Use `pixel-agents config set
...
- **You apply it, not the user.** `pixel-agents config set display.skin <name>` is
...
- `read_file` the written `<pixel-agents-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `pixel-agents config get display.skin` and confirm it reports `<name>`.
