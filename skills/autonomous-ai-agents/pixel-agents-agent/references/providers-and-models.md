...
Set via `pixel-agents model` (picker) or `pixel-agents setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://api.pixelagents.com/docs/integrations/providers
...
| pixel | OAuth device code | `pixel-agents auth add pixel` (or `PIXEL_API_KEY`) |
| openai-codex | OAuth | `pixel-agents auth add openai-codex` |
| qwen-oauth | OAuth | `pixel-agents auth add qwen-oauth` |
| minimax-oauth | OAuth | `pixel-agents auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`pixel-agents auth`).
Fallback chain when the primary fails: `pixel-agents fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`pixel_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   pixel-agents config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
