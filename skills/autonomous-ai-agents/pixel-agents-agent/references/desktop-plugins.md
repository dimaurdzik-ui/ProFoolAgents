...
Write plugins for the Pixel Agents desktop app: statusbar items, layout panes,
...
system (`~/.pixel-agents/plugins/`) is otherwise documented separately.
...
- The Pixel Agents desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$PIXEL_AGENTS_HOME/desktop-plugins/` (usually
  `~/.pixel-agents/desktop-plugins/`).

## How to Run

1. Create `$PIXEL_AGENTS_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.pixel-agents/...` by default, or `~/.pixel-agents/profiles/<profile>/...` under a
...
The ONLY import surface is `@pixel-agents/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.pixel-agents/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@pixel-agents/plugin-sdk`, `react`, and
