...
For occasional contributors and PR authors. Full developer docs: https://api.pixelagents.com/docs/developer-guide/
...
Config: `~/.pixel-agents/config.yaml` (settings), `~/.pixel-agents/.env` (API keys) — both under `$PIXEL_AGENTS_HOME` when it is set.
...
`_PIXEL_AGENTS_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_pixel_agents_home()` for paths,
never hardcode `~/.pixel-agents`. For custom/local-only tools, write a plugin in
`~/.pixel-agents/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `pixel_cli/commands.py`
...
- Tests auto-redirect `PIXEL_AGENTS_HOME` to temp dirs — never touch real `~/.pixel-agents/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/pixel_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_pixel_agents_home()` from `pixel_constants` for all paths (profile-safe)
