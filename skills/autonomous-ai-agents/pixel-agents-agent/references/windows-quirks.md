...
Pixel Agents runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`pixel-agents config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Pixel Agents's env scrubber dropping
...
Pixel Agents-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Pixel Agents tool and most Windows APIs. Prefer forward slashes in code
