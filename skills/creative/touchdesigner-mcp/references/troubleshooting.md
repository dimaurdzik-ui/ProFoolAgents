...
## 5. Pixel Agents Configuration

### Config location

`$PIXEL_AGENTS_HOME/config.yaml` (defaults to `~/.pixel-agents/config.yaml` when `PIXEL_AGENTS_HOME` is unset)
...
Restart the Pixel Agents session for changes to take effect. The MCP connection is
