#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Pixel Agents.
...
# Ensure sibling modules (_pixel_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _pixel_home import display_pixel_agents_home, get_pixel_agents_home

PIXEL_AGENTS_HOME = get_pixel_agents_home()
TOKEN_PATH = PIXEL_AGENTS_HOME / "google_token.json"
CLIENT_SECRET_PATH = PIXEL_AGENTS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = PIXEL_AGENTS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Pixel Agents profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Pixel Agents Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Pixel Agents Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pixel-agents setup")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Pixel Agents home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_pixel_agents_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Pixel Agents")
