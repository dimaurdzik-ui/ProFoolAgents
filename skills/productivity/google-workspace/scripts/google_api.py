#!/usr/bin/env python3
"""Google Workspace API CLI for Pixel Agents.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Pixel Agents-facing JSON contract and falls back to the Python client
...
from _pixel_home import get_pixel_agents_home
...
def _gws_binary() -> str | None:
    override = os.getenv("PIXEL_AGENTS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Pixel Agents")
