#!/usr/bin/env bash
# GitHub environment detection helper for Pixel Agents skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _pixel_env="${PIXEL_AGENTS_HOME:-$HOME/.pixel-agents}/.env"; [ -f "$_pixel_env" ] && grep -q "^GITHUB_TOKEN=" "$_pixel_env" 2>/dev/null; then
