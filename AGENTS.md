# Pixel Agents - Development Guide

Instructions for AI coding assistants and developers working on the pixel-agents codebase.

**Never give up on the right solution.**

## What Pixel Agents Is

Pixel Agents is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Pixel Agents ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`pixel-agents tools`, `pixel-agents setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `PIXEL_AGENTS_HOME`. Mocks hide integration bugs.
...
- **New `PIXEL_AGENTS_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `pixel-agents tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.pixel-agents/plugins/` (or via a
  pip entry point), and promote them in the Pixel Agents Discord
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `pixel-agents <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `pixel-agents webhook`, `pixel-agents cron`, `pixel-agents tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.pixel-agents/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.pixel-agents/pixel-agents/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _PIXEL_AGENTS_CORE_TOOLS list
├── cli.py                # PixelAgentsCLI class — interactive CLI orchestrator (~11k LOC)
├── pixel_state.py       # SessionDB — SQLite session store (FTS5 search)
├── pixel_constants.py   # get_pixel_agents_home(), display_pixel_agents_home() — profile-aware paths
├── pixel_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── pixel_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── pixel-agents-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `pixel-agents --tui`
...
**User config:** `~/.pixel-agents/config.yaml` (settings), `~/.pixel-agents/.env` (API keys only).
**Logs:** `~/.pixel-agents/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_pixel_agents_home()`.
Browse with `pixel-agents logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Pixel Agents: desktop, TUI, website, and future TS packages.
...
The core loop is inside `run_conversation()` — entirely synchropixel, with
...
- **Skin engine** (`pixel_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `PixelAgentsCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.pixel-agents/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`pixel_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/pixel-agents` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `pixel_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `PixelAgentsCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `pixel-agents --tui` or `PIXEL_AGENTS_TUI=1`.
...
npm run dev       # watch mode (rebuilds pixel-agents-ink + tsx --watch)
npm start         # production
npm run build     # full build (pixel-agents-ink + tsc)
...
### TUI in the Dashboard (`pixel-agents dashboard` → `/chat`)

The dashboard embeds the real `pixel-agents --tui` — **not** a rewrite.  See `pixel_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `pixel_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `pixel-agents --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `pixel-agents --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@pixel-agents/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `pixel-agents serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `PIXEL_AGENTS_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `pixel-agents` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `pixel-agents --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Pixel Agents built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Pixel Agents core.
Use the plugin route instead: create `~/.pixel-agents/plugins/<name>/plugin.yaml`
and `~/.pixel-agents/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Pixel Agents tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_PIXEL_AGENTS_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_PIXEL_AGENTS_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_pixel_agents_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `PIXEL_AGENTS_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_pixel_agents_home()` for the base directory — never `Path.home() / ".pixel-agents"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `pixel_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `pixel_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `pixel-agents tools`, `pixel-agents setup`, most CLI subcommands | `pixel_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`pixel_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
pixel_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.pixel-agents/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Pixel Agents gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `pixel_cli/skin_engine.py`:
...
Users create `~/.pixel-agents/skins/<name>.yaml`:
...
Pixel Agents has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.pixel-agents/plugins/` and pip-installed entry points.

### General plugins (`pixel_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.pixel-agents/plugins/`, `./.pixel-agents/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `pixel-agents` at startup so
  `pixel-agents <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(pixel_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `pixel-agents <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `pixel-agents --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `pixel_cli/main.py`, etc.).
...
into `~/.pixel-agents/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `pixel-agents memory setup` / `post_setup()` without
...
`~/.pixel-agents/plugins/` (or via pip entry points). They register through
...
don't own. Promote standalone plugins in the Pixel Agents Discord
...
2. User: `$PIXEL_AGENTS_HOME/plugins/model-providers/<name>/`
...
[`pixel-agents-example-plugins`](https://github.com/PixelResearch/pixel-agents-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `pixel-agents skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.pixel-agents.tags`, `metadata.pixel-agents.category`,
`metadata.pixel-agents.related_skills`, `metadata.pixel-agents.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.pixel-agents.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Pixel Agents tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Pixel Agents" is the secondary collaborator. If the
   contributor's commit shows "Pixel Agents" as author (because they
   used Pixel Agents to draft the skill), replace it with their actual name
...
lives in the `pixel-agents-dev` skill at
...
`"messaging"`); `_PIXEL_AGENTS_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `pixel-agents tools` (the curses UI) or the
...
Pixel Agents returns a delegation id immediately and the result re-enters the
...
go to `~/.pixel-agents/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `pixel_cli/curator.py` wires `pixel-agents curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.pixel-agents/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `pixel-agents cron <verb>`
...
- File lock at `~/.pixel-agents/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `pixel-agents kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `pixel_cli/kanban.py` wires `pixel-agents kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`pixel-agents-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `PIXEL_AGENTS_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Pixel-Agents ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `PIXEL_AGENTS_BACKGROUND_NOTIFICATIONS` env var):
...
Pixel Agents supports **profiles** — multiple fully isolated instances, each with its own
`PIXEL_AGENTS_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `pixel_cli/main.py` sets
`PIXEL_AGENTS_HOME` before any module imports. All `get_pixel_agents_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_pixel_agents_home()` for all PIXEL_AGENTS_HOME paths.** Import from `pixel_constants`.
   NEVER hardcode `~/.pixel-agents` or `Path.home() / ".pixel-agents"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".pixel-agents" / "config.yaml"
   ```

2. **Use `display_pixel_agents_home()` for user-facing messages.** Import from `pixel_constants`.
   This returns `~/.pixel-agents` for default or `~/.pixel-agents/profiles/<name>` for profiles.
   ```python
   # GOOD
   from pixel_constants import display_pixel_agents_home
   print(f"Config saved to {display_pixel_agents_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.pixel-agents/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_pixel_agents_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_pixel_agents_home()`,
   not `Path.home() / ".pixel-agents"`.

4. **Tests that mock `Path.home()` must also set `PIXEL_AGENTS_HOME`** — since code now uses
   `get_pixel_agents_home()` (reads env var), not `Path.home() / ".pixel-agents"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"PIXEL_AGENTS_HOME": str(tmp_path / ".pixel-agents")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not PIXEL_AGENTS_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".pixel-agents" / "profiles"`, NOT `get_pixel_agents_home() / "profiles"`.
   This is intentional — it lets `pixel-agents -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.pixel-agents` paths
Use `get_pixel_agents_home()` from `pixel_constants` for code paths. Use `display_pixel_agents_home()`
for user-facing print/log messages. Hardcoding `~/.pixel-agents` breaks profiles — each profile
has its own `PIXEL_AGENTS_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `pixel_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `pixel_cli/curses_ui.py` — see
`pixel_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `PIXEL_AGENTS_HOME`.

### Tests must not write to `~/.pixel-agents/`
The `_isolate_pixel_home` autouse fixture in `tests/conftest.py` redirects `PIXEL_AGENTS_HOME` to a temp dir. Never hardcode `~/.pixel-agents/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_pixel_home()` resolve within the temp dir.
Use the pattern from `tests/pixel_cli/test_profiles.py`:
```python
@pytest.fixture
...
subprocess (`--file-retries`, default 1; `PIXEL_AGENTS_TEST_FILE_RETRIES=0` to
...
| HOME / `~/.pixel-agents/` | Your real config+auth.json                  | Temp dir per test                         |
