"""
Pixel Agents — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m pixel_cli.main web          # Start on http://127.0.0.1:9119
    python -m pixel_cli.main web --port 8080
"""
...
from pixel_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
try:
    ...
    # running `pixel-agents dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["PIXEL_AGENTS_WEB_DIST"]) if "PIXEL_AGENTS_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``pixel-agents gateway run`` — but the
    desktop app spawns a ``pixel-agents dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same PIXEL_AGENTS_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import pixel_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from pixel_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    if os.getenv("PIXEL_AGENTS_DESKTOP") == "1":
...
app = FastAPI(title="Pixel Agents", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from pixel_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# PIXEL_AGENTS_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
def _resolve_session_token() -> str:
    return os.environ.get("PIXEL_AGENTS_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Pixel-Agents-Session-Token"
...
#
# This list is defined in ``pixel_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from pixel_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Pixel-Agents-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``pixel-agents-0day``
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from pixel_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from pixel_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from pixel_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `pixel-agents egress setup` "
            "and `pixel-agents egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Pixel Agents (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during pixel-agents update. "
...
from pixel_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Pixel Agents provider slug — fine.
    ...
    1. Vendor-name → Pixel Agents-provider mapping: when the provider string is
       not a known Pixel Agents provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    from pixel_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from pixel_cli.model_normalize import normalize_model_for_provider
    from pixel_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Pixel Agents
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from pixel_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    while another Pixel Agents process is writing to it.
    """
    from pixel_state import DEFAULT_DB_PATH, SessionDB
...
_MANAGED_FILES_ROOT_ENV = "PIXEL_AGENTS_DASHBOARD_FILES_ROOT"
...
# PIXEL_AGENTS_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to PIXEL_AGENTS_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Pixel Agents credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no PIXEL_AGENTS_HOME resolution, so it blocks these trees wherever
...
def _default_pixel_root_is_opt_data() -> bool:
    raw = os.environ.get("PIXEL_AGENTS_HOME", "").strip()
    if not raw:
        return False
    try:
        from pixel_constants import get_default_pixel_agents_root

        root = get_default_pixel_agents_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``pixel-agents update``.
    ...
    a container — e.g. the pixel-agents-webui image sharing the Pixel Agents source tree),
    the dashboard's ``pixel-agents update`` button is the correct update path and
    ...
    """
    if _default_pixel_root_is_opt_data():
        return True
    try:
        from pixel_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Pixel Agents root is actually /opt/data
    # (the container/hosted layout) or when PIXEL_AGENTS_DASHBOARD_FILES_ROOT is set.
    if _default_pixel_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Pixel Agents inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``pixel-agents:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.pixel-agents-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``pixel_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from pixel_cli import web_git as _web_git  # noqa: E402
...
from pixel_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)
from pixel_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from pixel_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_pixel_agents_home() at call time
    ...
    try:
        ...
        # PIXEL_AGENTS_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of pixel_cli.gateway blocks the
        ...
        # and which providers are registered so ``pixel-agents status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Pixel Agents" or
        ...
        auth_flows: list[str] = []
        try:
            from pixel_cli.dashboard_auth import (
        ...
        # Pixel bootstrap-session validity for the NAS health sweep. A hosted
        # agent whose Pixel auth dies terminally (invalid_grant / quarantine)
        ...
        pixel_session_valid = "unknown"
        try:
            from pixel_cli.auth import get_pixel_session_validity
            pixel_session_valid = get_pixel_session_validity()
        except Exception:
            pixel_session_valid = "unknown"
        ...
        status = {
            ...
            "can_update_pixel": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
            ...
            "pixel_session_valid": pixel_session_valid,
        }
        ...
        try:
            ...
            storage_check = await asyncio.get_running_loop().run_in_executor(
                None, functools.partial(_probe_state_db, get_pixel_agents_home())
            )
        ...
        try:
            from pixel_state import SessionDB as _SDB
            from pixel_constants import get_pixel_agents_home as _ghh
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Pixel Agents Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the pixel-agents home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_pixel_agents_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_pixel_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Portal endpoint — Pixel Portal auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------
...
async def get_portal_status():
    ...
    try:
        ...
        feats = get_pixel_subscription_features(cfg)
        if feats is not None:
            for feat in feats.items():
                if getattr(feat, "managed_by_pixel", False):
                    state = "via Pixel Portal"
...
async def run_prompt_size():
    try:
        proc = _spawn_pixel_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_pixel_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_pixel_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchropixel*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from pixel_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.pixel-agents/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_pixel_agents_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "pixel-agents-update": "pixel-agents-update.log",
...
def _spawn_pixel_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``pixel-agents <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``pixel_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "pixel_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _PIXEL_AGENTS_GATEWAY=1. Inheriting it makes a spawned `pixel-agents gateway restart`
    ...
    action_env = {**os.environ, "PIXEL_AGENTS_NONINTERACTIVE": "1"}
    action_env.pop("_PIXEL_AGENTS_GATEWAY", None)
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["pixel-agents", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``pixel-agents gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``pixel-agents gateway restart`` children race each other on the
    ...
    return _spawn_pixel_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``pixel-agents gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``PIXEL_AGENTS_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/pixel-agents/update")
async def update_pixel():
    """Kick off ``pixel-agents update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Pixel Agents updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("pixel-agents-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "pixel-agents-update",
    ...
    try:
        proc = _spawn_pixel_action(["update"], "pixel-agents-update")
    except Exception as exc:
        _log.exception("Failed to spawn pixel-agents update")
...
@app.get("/api/pixel-agents/update/check")
async def check_pixel_update(force: bool = False):
    """Report whether a Pixel Agents update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/pixel-agents/update`` actually runs ``pixel-agents update``.

    Returns:
        install_method: 'git' | 'docker' | 'nix' | 'nixos' | 'unknown'
        current_version: installed Pixel Agents version string
    ...
    try:
        ...
        if force:
            try:
                (get_pixel_agents_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def transcribe_audio_upload(
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            prefix="pixel-agents-desktop-voice-",
            suffix=suffix,
            delete=False,
        ) as tmp:
...
async def speak_text(payload: TTSSpeakRequest, profile: Optional[str] = None):
    ...
    configured in ``~/.pixel-agents/config.yaml`` under ``tts.``.
    """
...
from pixel_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from pixel_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from pixel_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from pixel_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Pixel Agents supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_pixel_agents_home() / provider.name / "config.json"
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/pixel-agents is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (PIXEL_AGENTS_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        pixel_home / f"{name}.json",
        pixel_home / name / "config.json",
    ):
    ...
    # Holographic stores under plugins.pixel-agents-memory-store.
    plugins_cfg = cfg.get("plugins") if isinstance(cfg, dict) else {}
    if name == "holographic" and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get("pixel-agents-memory-store")
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_pixel_agents_home()))
            return
...
# in pixel_cli/config.py — listed here for deterministic ordering in the UI.
...
async def get_model_options(
    ...
    """
    try:
        from pixel_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `pixel-agents model` does so GUI onboarding lands on a
    sensible default instead of blindly taking the first curated entry. For
    Pixel this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for Pixel and None otherwise. `model` may be
    ...
    if slug == "pixel":
        try:
            from pixel_cli.models import (
                get_curated_pixel_model_ids,
                get_pricing_for_provider,
                check_pixel_free_tier,
                partition_pixel_models_by_tier,
            ...
            model_ids = get_curated_pixel_model_ids()
            pricing = get_pricing_for_provider("pixel") or {}
            free_tier = check_pixel_free_tier(force_fresh=True)
            ...
            if free_tier:
                ...
                model_ids, _unavailable = partition_pixel_models_by_tier(
            ...
            model = pick_silent_default_model(model_ids, provider="pixel")
            return {"provider": "pixel", "model": model, "free_tier": bool(free_tier)}
        except Exception:
            _log.exception("GET /api/model/recommended-default (pixel) failed")
            return {"provider": "pixel", "model": "", "free_tier": None}

    # Non-Pixel: preferred silent default when the provider's curated list
    ...
    try:
        from pixel_cli.inventory import build_models_payload, load_picker_context
        from pixel_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from pixel_cli.moa_config import normalize_moa_config
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from pixel_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.pixel-agents/config.yaml`` — applies to **new** sessions only.
...
def _apply_model_assignment_sync(
    ...
    """Synchropixel body of POST /api/model/set.
    ...
    if scope == "main":
        ...
        # When switching the main provider to Pixel, mirror the CLI's
        # post-model-selection behaviour (pixel_cli/main.py
        # prompt_enable_tool_gateway / tools_config apply_pixel_managed_defaults):
        # auto-route any *unconfigured* tools through the Pixel Tool Gateway.
        # This is purely additive — apply_pixel_managed_defaults skips every
        ...
        if provider.strip().lower() == "pixel":
            try:
                from pixel_cli.pixel_subscription import apply_pixel_managed_defaults
                from pixel_cli.tools_config import _get_platform_tools

                enabled = _get_platform_tools(
                    cfg, "cli", include_default_mcp_servers=False
                )
                changed = apply_pixel_managed_defaults(
                    cfg,
                ...
                # Portal lookup hiccups / non-subscriber / non-pixel gating
                # must never block saving the model assignment.
                _log.debug("apply_pixel_managed_defaults skipped", exc_info=True)

        save_config(cfg)

        # Register a named ``custom_providers`` entry for a custom/local
        # endpoint, mirroring the ``pixel-agents model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from pixel_cli.main import _auto_provider_name, _save_custom_provider
        ...
        # a now-unpaid provider (e.g. pixel with $0 balance) keeps paying 402s
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``pixel-agents model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from pixel_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from pixel_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `pixel-agents model` provider — would be
        ...
        # Vertex — a `pixel-agents model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `pixel-agents model` picker uses (not desktop-only prefix guesses).
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from pixel_cli.credential_lifecycle import save_provider_env_credential
...
async def remove_env_var(body: EnvVarDelete, profile: Optional[str] = None):
    try:
        with _profile_scope(body.profile or profile):
            ...
            from pixel_cli.credential_lifecycle import remove_provider_env_credential
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Pixel Agents from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Pixel Agents to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Pixel Agents from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Pixel Agents to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Pixel Agents in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Pixel Agents through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Pixel Agents via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Pixel Agents through an IMAP/SMTP mailbox.",
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Pixel Agents to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Pixel Agents inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Pixel Agents to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Pixel Agents through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Pixel Agents to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "description": "Connect Pixel Agents to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Pixel Agents.",
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Pixel Agents from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Pixel Agents over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Pixel Agents through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Pixel Agents over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Pixel Agents to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Pixel Agents as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Pixel Agents via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://api.pixelagents.com/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Pixel Agents Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in pixel-agents gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in pixel-agents gateway setup",
...
# pixel_cli/setup_hidden_env.py. Shared with the `pixel-agents setup gateway`
# wizard so the surfaces ask for the same things.
from pixel_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # PIXEL_AGENTS_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    ...
    return get_pixel_agents_dir("platforms/whatsapp/session", "whatsapp/session")
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.api.pixelagents.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"PixelAgentsDashboard/{__version__}"
@dataclass
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Pixel Agents").strip() or "Pixel Agents"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Pixel Agents appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the PIXEL_AGENTS_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized PIXEL_AGENTS_HOME)
...
# Anthropic, device-code for Pixel/Codex) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``pixel-agents auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.pixel-agents/.anthropic_oauth.json`` — Pixel Agents-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_pixel_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_pixel_oauth_credentials,
            _get_pixel_oauth_file,
        )
    except ImportError:
        read_pixel_oauth_credentials = None  # type: ignore
        _get_pixel_oauth_file = None  # type: ignore

    pixel_creds = None
    if read_pixel_oauth_credentials:
        try:
            pixel_creds = read_pixel_oauth_credentials()
        except Exception:
            pixel_creds = None
    if pixel_creds and pixel_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "pixel_pkce",
            "source_label": f"Pixel Agents PKCE ({_get_pixel_oauth_file() if _get_pixel_oauth_file else None})",
            "token_preview": _truncate_token(pixel_creds.get("accessToken")),
            "expires_at": pixel_creds.get("expiresAt"),
            "has_refresh_token": bool(pixel_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from pixel_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Pixel Agents even
    when they also have a separate Pixel Agents-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Pixel Agents never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Pixel Portal",
        "flow": "device_code",
        "cli_command": "pixel-agents auth add pixel",
        "docs_url": "https://portal.pixelagents.com",
        "status_fn": None,  # dispatched via auth.get_pixel_auth_status
    },
    {
        "id": "openai-codex",
        "name": "OpenAI OAuth (ChatGPT)",
        "flow": "device_code",
        "cli_command": "pixel-agents auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "pixel-agents auth add qwen-oauth",
    ...
    },
    {
        ...
        # as Pixel's device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "pixel-agents auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "pixel-agents auth add xai-oauth",
        "docs_url": "https://api.pixelagents.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "pixel-agents auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from pixel_cli import auth as hauth
        if provider_id == "pixel":
            # Read-only accounts-tab card: refresh-free snapshot so listing
            # providers never performs an OAuth refresh.
            raw = hauth.get_pixel_auth_status_local()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "pixel_portal",
                "source_label": raw.get("portal_base_url") or "Pixel Portal",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Pixel Agents, so the disconnect
    ...
    user sees exactly what executes, and Pixel Agents then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Pixel Agents — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``pixel-agents model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `pixel-agents model` universe (zero-edit for new plugins).
    try:
        from pixel_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"pixel-agents auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("pixel_pkce", "claude_code", ...)
    ...
    sync with the `pixel-agents model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Pixel Agents-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_pixel_oauth_file
                oauth_file = _get_pixel_oauth_file()
            ...
            try:
                from pixel_cli.auth import clear_provider_auth
        ...
        try:
            from pixel_cli.auth import clear_provider_auth, invalidate_pixel_auth_status_cache
            cleared = clear_provider_auth(provider_id)
            if provider_id == "pixel":
                invalidate_pixel_auth_status_cache()
...
#          → persists to ~/.pixel-agents/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (Pixel, OpenAI Codex):
#     1. POST /api/providers/oauth/{pixel|openai-codex}/start
...
# Guarded so pixel-agents web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Pixel Agents file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``pixel-agents auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_pixel_oauth_file
    oauth_file = _get_pixel_oauth_file()
    payload = {
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "pixel-agents-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    """Initiate a device-code flow (Pixel, OpenAI Codex, MiniMax, or xAI).
    ...
    if provider_id == "pixel":
        ...
        import httpx
        pconfig = PROVIDER_REGISTRY["pixel"]
        portal_base_url = (
            os.getenv("PIXEL_AGENTS_PORTAL_BASE_URL")
            or os.getenv("PIXEL_PORTAL_BASE_URL")
            or pconfig.portal_base_url
        ).rstrip("/")
        client_id = pconfig.client_id
        scope = pconfig.scope

        def _do_pixel_device_request():
            with httpx.Client(
        ...
        device_data, effective_scope = await asyncio.get_running_loop().run_in_executor(
            None, _do_pixel_device_request
        )
        sid, sess = _new_oauth_session("pixel", "device_code", profile=profile)
        ...
        threading.Thread(
            target=_pixel_poller, args=(sid,), daemon=True, name=f"oauth-poll-{sid[:6]}"
        ).start()
        return {
            "session_id": sid,
            "flow": "device_code",
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to Pixel's device-code
    ...
    if provider_id == "xai-oauth":
        from pixel_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _pixel_poller(session_id: str) -> None:
    """Background poller that drives a Pixel device-code flow to completion."""
    from pixel_cli.auth import (
        _poll_for_token,
        refresh_pixel_oauth_from_state,
    )
    from datetime import datetime, timezone
    import httpx
    with _oauth_sessions_lock:
    ...
    try:
        ...
        # Same post-processing as _pixel_device_code_login (validate/refresh JWT)
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            full_state = refresh_pixel_oauth_from_state(
                auth_state,
                timeout_seconds=15.0,
                force_refresh=False,
            )
            from pixel_cli.auth import persist_pixel_credentials
            persist_pixel_credentials(full_state)
        with _oauth_sessions_lock:
            sess["status"] = "approved"
        _log.info("oauth/device: pixel login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("pixel device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors `_pixel_poller` but calls the MiniMax-specific token endpoint,
    which uses a PKCE-style ``code_verifier`` + ``user_code`` rather than
    the ``device_code`` field used by Pixel. On success, builds the same
    auth_state dict that ``_minimax_oauth_login`` (the CLI flow) builds
    and persists via ``_minimax_save_auth_state`` — so the dashboard
    path leaves the system in the same state as
    ``pixel-agents auth add minimax-oauth``.
    """
    from pixel_cli.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `pixel-agents auth add xai-oauth`: first credential may become
            ...
            # prior ``pixel-agents auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``pixel-agents model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Pixel Agents "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from pixel_cli.auth import _save_codex_tokens
...
async def poll_oauth_session(
    ...
    Shared by the device-code flows (Pixel, OpenAI Codex, MiniMax, xAI).
...
def _maybe_auto_archive_for_profile(db, profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``pixel-agents serve`` — it runs neither the
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `pixel-agents sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_pixel_agents_home()
...
async def get_logs(
    ...
    from pixel_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_pixel_agents_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from pixel_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (PIXEL_AGENTS_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.pixel-agents``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    PIXEL_AGENTS_HOME outside the profiles tree) has no profile-dir equivalent, so
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, PIXEL_AGENTS_HOME)."""
    from pixel_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    token = set_pixel_agents_home_override(str(home))
    try:
...
from pixel_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from pixel_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Pixel Agents home so the job's store,
...
#
# Wraps the same config data layer the CLI uses (pixel_cli.mcp_config), so
# servers managed here show up under `pixel-agents mcp list` and vice versa.  Secrets
...
def _normalize_mcp_server_create(
    ...
    """
    from pixel_cli.mcp_config import (
        _bearer_auth_headers,
        _strip_bearer_prefix,
    )
    from pixel_cli.mcp_security import validate_mcp_server_entry
...
from pixel_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from pixel_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from pixel_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.pixel_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from pixel_constants import reset_pixel_agents_home_override, set_pixel_agents_home_override
        from tools.mcp_dashboard_oauth import dashboard_oauth_flow
        from tools.mcp_oauth import PixelAgentsTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_pixel_agents_home_override(flow.pixel_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.pixel_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = PixelAgentsTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        pixel_home=flow.pixel_home,
                    )
...
#
# Wraps the same JSON store the CLI uses (pixel_cli.webhook); the webhook
...
async def list_webhooks():
    import pixel_cli.webhook as wh
...
# spawn the real `pixel-agents gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_pixel_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_pixel_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from pixel_cli.auth import read_credential_pool
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    try:
        ...
        # (via DELETE below or `pixel-agents auth remove`) can seed again.
        # Mirrors the `pixel-agents auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``pixel-agents auth remove`` uses: each source cleans up its
    ...
    from pixel_cli.auth import suppress_credential_source
...
async def run_doctor():
    try:
        proc = _spawn_pixel_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_pixel_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_pixel_agents_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"pixel-agents-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_pixel_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_pixel_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    try:
        from pixel_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <pixel_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_pixel_agents_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_pixel_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``pixel-agents`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_pixel_action`` tracks one process/log per name, so a shared
...
from pixel_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from pixel_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
# Human-readable labels for each hub source id (matches `pixel-agents skills search`
# provenance).  Keep in sync with create_source_router()'s source list.
_SKILL_HUB_SOURCE_LABELS = {
    "official": "Official (Pixel)",
    "pixel-agents-index": "Pixel Agents Index",
    "skills-sh": "skills.sh",
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_pixel_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "pixel-agents setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local PIXEL_AGENTS_HOME override so the write lands in the target
    ...
    token = set_pixel_agents_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local PIXEL_AGENTS_HOME override (same mechanism as
    ...
    """
    from pixel_constants import set_pixel_agents_home_override, reset_pixel_agents_home_override
    from pixel_cli.mcp_config import _save_bearer_auth_token
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the PIXEL_AGENTS_HOME override. Returns the
    number of skills newly disabled.
    """
    from pixel_constants import set_pixel_agents_home_override, reset_pixel_agents_home_override
    from pixel_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_pixel_agents_home()`` at call
       time — the context-local override from ``set_pixel_agents_home_override``
    ...
    retargeted to the *current* ``get_pixel_agents_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a PIXEL_AGENTS_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
    ...
    token = None
    if not requested or requested.lower() == "current":
        profile_dir = get_pixel_agents_home()
    else:
        profile_dir = _resolve_profile_dir(requested)
        token = set_pixel_agents_home_override(str(profile_dir))

    with _SKILLS_PROFILE_LOCK:
        old_home = _skills_tool.PIXEL_AGENTS_HOME
        old_skills_dir = _skills_tool.SKILLS_DIR
        old_mgr_home = _skill_mgr.PIXEL_AGENTS_HOME
        old_mgr_skills_dir = _skill_mgr.SKILLS_DIR
        _skills_tool.PIXEL_AGENTS_HOME = profile_dir
        _skills_tool.SKILLS_DIR = profile_dir / "skills"
        _skill_mgr.PIXEL_AGENTS_HOME = profile_dir
        _skill_mgr.SKILLS_DIR = profile_dir / "skills"
        try:
            yield profile_dir if token is not None else None
        finally:
            _skills_tool.PIXEL_AGENTS_HOME = old_home
            _skills_tool.SKILLS_DIR = old_skills_dir
            _skill_mgr.PIXEL_AGENTS_HOME = old_mgr_home
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_pixel_agents_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_pixel_agents_home()`` at call time
...
from pixel_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from pixel_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    the managed "Pixel Subscription" image row instead carries the legacy
...
def _toolset_model_catalog(ts_key: str, plugin_name: str):
    ...
    from pixel_cli.tools_config import (
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Pixel Agents,
...
#
# The endpoint spawns the same ``pixel-agents --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from pixel_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from pixel_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from pixel_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from pixel_cli.dashboard_auth.audit import AuditEvent, audit_log
        from pixel_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``pixel-agents --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``PIXEL_AGENTS_TUI_RESUME`` env var —
    matching what ``pixel_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``PIXEL_AGENTS_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``PIXEL_AGENTS_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``PIXEL_AGENTS_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``PIXEL_AGENTS_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_pixel_agents_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``pixel-agents -p <name>`` performs. The in-process
    ``PIXEL_AGENTS_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from pixel_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # Pixel Agents TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope below still overrides PIXEL_AGENTS_HOME afterwards.
    from tools.environments.local import build_subprocess_env
    ...
    env.setdefault("PIXEL_AGENTS_TUI_DISABLE_MOUSE", "1")
    env.setdefault("PIXEL_AGENTS_TUI_INLINE", "1")
    ...
    env["PIXEL_AGENTS_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["PIXEL_AGENTS_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["PIXEL_AGENTS_TUI_RESUME"] = resume

    if sidecar_url:
        env["PIXEL_AGENTS_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile PIXEL_AGENTS_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["PIXEL_AGENTS_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``PIXEL_AGENTS_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("PIXEL_AGENTS_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from pixel_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def _resolve_chat_argv_async(
    ...
    ``_make_tui_argv``.  Keep that synchropixel work off the WebSocket event
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="pixel-agents-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Pixel Agents Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Pixel Agents CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="pixel-agents-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from pixel_cli.console_engine import PixelAgentsConsoleEngine

        engine = PixelAgentsConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Pixel Agents Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Pixel Agents inside WSL2 to use the dashboard's /chat "
    ...
    if raw_resume and env:
        registry_resume = env.get("PIXEL_AGENTS_TUI_RESUME") or raw_resume
...
# PIXEL_AGENTS_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`pixel_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from pixel_cli.dashboard_auth.prefix import normalise_prefix
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Pixel Agents Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="pixel-agents-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/pixel-agents/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /pixel-agents`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__PIXEL_AGENTS_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `pixel-agents serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("PIXEL_AGENTS_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (pixel-agents serve): web UI disabled — use "
            "`pixel-agents dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/pixel-agents``)
        ...
        ``__PIXEL_AGENTS_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__PIXEL_AGENTS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__PIXEL_AGENTS_BASE_PATH__="{prefix}";'
                f"window.__PIXEL_AGENTS_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__PIXEL_AGENTS_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__PIXEL_AGENTS_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__PIXEL_AGENTS_BASE_PATH__="{prefix}";'
                f"window.__PIXEL_AGENTS_AUTH_REQUIRED__={gated_js};"
                f"</script>"
        ...
        # (``PIXEL_AGENTS_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Pixel Agents Teal canvas, then
    ...
    # under ``/pixel-agents`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Pixel Agents backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Pixel Agents Teal",         "description": "Classic dark teal — the canonical Pixel Agents look"},
    {"name": "default-large", "label": "Pixel Agents Teal (Large)", "description": "Pixel Agents Teal with bigger fonts and roomier spacing"},
    {"name": "pixel-blue",     "label": "Pixel Blue",           "description": "Light mode — vivid Pixel-blue accents on cream canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.pixel-agents/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.pixel-agents/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.

    Uses the dashboard process launch home, not ``get_pixel_agents_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``PIXEL_AGENTS_HOME``.
    """
    themes_dir = get_process_pixel_agents_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.pixel-agents/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as pixel_cli.plugins):
    1. User plugins:    ~/.pixel-agents/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.pixel-agents/plugins/  (only if PIXEL_AGENTS_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from pixel_cli.plugins import get_bundled_plugins_dir
    ...
    # PIXEL_AGENTS_HOME override (e.g. embedded /chat under --open-profile).
    search_dirs = [
        (get_process_pixel_agents_home() / "plugins", "user"),
    ...
    # ``pixel_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("PIXEL_AGENTS_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".pixel-agents" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from pixel_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_pixel_agents_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"pixel-agents auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from pixel_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from pixel_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from pixel_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from pixel_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.pixel-agents/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.pixel-agents/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"pixel_dashboard_plugin_{plugin['name']}"
...
from pixel_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``PIXEL_AGENTS_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("PIXEL_AGENTS_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``PIXEL_AGENTS_SERVE_HEADLESS``), so
    ...
    try:
        ...
        _log.debug("Pixel auth keepalive did not start: %s", exc)
    ...
    # the pixel-agents-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing PIXEL_AGENTS_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Pixel Agents exposes a
            # module-level ``LAST_SKIP_REASON`` string for this purpose;
            # without it the operator would only see "no providers" which
            # is misleading when the provider IS installed but unconfigured.
            skip_reasons: list[str] = []
            try:
                from plugins.dashboard_auth import pixel as _pixel_plugin

                if _pixel_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • pixel: {_pixel_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `pixel-agents dashboard register` (Pixel Portal) or "
            ...
            # (#54489).
            try:
                from pixel_cli.config import load_config as _load_cfg
                from pixel_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
                ...
                if _has_creds and (set(_disabled) & _BASIC_AUTH_PLUGIN_KEYS):
                    _fix_hint = (
                        ...
                        "`pixel-agents plugins enable basic`), then restart the "
                        "dashboard.\n\n"
                    ) + _fix_hint
            except Exception:
                pass
    ...
    # PIXEL_AGENTS_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    # as agent turns, and a single synchropixel GIL-holding call on a worker
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "PIXEL_AGENTS_BACKEND_READY" if headless else "PIXEL_AGENTS_DASHBOARD_READY"
            print(f"{ready_token} port={actual_port}", flush=True)
            if headless:
                # No SPA, and the JSON-RPC/WS endpoints are auth-gated — don't
                # advertise a paste-and-connect URL, just announce the bind.
                print(f"  Pixel Agents backend listening on {host}:{actual_port}")
            else:
                print(f"  Pixel Agents Web UI → http://{host}:{actual_port}")
