...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Pixel Agents'
...
The actual runtime resolution happens in pixel_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "pixel-agents"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, pixel-agents-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Pixel Agents' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Pixel Agents for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from pixel_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the pixel-agents-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "pixel-agents-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Pixel Agents tool callback are always-on production
            ...
            if "pixel-agents-tools" in mig_report.migrated:
                msg_lines.append(
                    "Pixel Agents tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Pixel Agents runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Pixel Agents tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Pixel Agents runtime.")
