...
The Pixel Agents docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Pixel Portal). This module fetches, validates, and caches that manifest,
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/PixelResearch/pixel-agents/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
...
def _cache_path() -> Path:
    ...
    return get_pixel_agents_home() / "cache" / "model_catalog.json"
...
def get_curated_pixel_models() -> list[str] | None:
    """Return Pixel Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("pixel")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Pixel Agents silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`pixel-agents update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``pixel-agents update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.pixel-agents/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``pixel-agents model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
