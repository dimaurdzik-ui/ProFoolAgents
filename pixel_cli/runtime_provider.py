...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from pixel_cli.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(
    ...
    *,
    ...
    try:
        from pixel_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of Pixel Agents' tool dispatch. Gated behind config key
...
def _pixel_inference_base_url_override() -> str:
    """Return the trusted Pixel runtime base URL override, if configured.

    Delegates to ``auth._pixel_inference_env_override`` so every
    ``PIXEL_INFERENCE_BASE_URL`` read shares one normalization path
    ...
    """
    return _pixel_inference_env_override() or ""
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    if provider == "openai-codex":
        ...
        api_mode = pixel_api_mode(effective_model)
        base_url = _pixel_inference_base_url_override() or base_url
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from pixel_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from pixel_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("PIXEL_AGENTS_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the canonical provider itself (``pixel``, ``openrouter``, …) so
            # accidentally shadowing a canonical provider still resolves to
            # the built-in. See tests/pixel_cli/test_runtime_provider_resolution.py
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'pixel-agents doctor' for details."
        )
        return None
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``PIXEL_AGENTS_INFERENCE_PROVIDER`` equivalent). When neither
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("PIXEL_AGENTS_INFERENCE_PROVIDER", "").strip()
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'pixel-agents model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.pixel-agents/.env or run 'pixel-agents model' to configure. To use "
            "keyless Microsoft Entra ID auth instead, set "
            "model.auth_mode: entra_id in config.yaml (or pick "
            "'Microsoft Entra ID' in 'pixel-agents model')."
        )
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "pixel":
        ...
        state = auth_mod.get_provider_auth_state("pixel") or {}
        base_url = (
            explicit_base_url
            or _pixel_inference_base_url_override()
            or str(state.get("inference_base_url") or auth_mod.DEFAULT_PIXEL_INFERENCE_URL).strip().rstrip("/")
        )
        ...
        # by resolve_pixel_runtime_credentials().
        api_key = explicit_api_key or (
            str(state.get("agent_key") or "").strip()
            if _agent_key_is_usable(
                state,
                max(60, env_int("PIXEL_AGENTS_PIXEL_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_pixel_runtime_credentials(
                timeout_seconds=float(_getenv("PIXEL_AGENTS_PIXEL_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
        ...
        return {
            "provider": "pixel",
            "api_mode": pixel_api_mode(target_model or model_cfg.get("default") or ""),
            "base_url": base_url,
            "api_key": api_key,
            "source": "explicit",
...
def resolve_runtime_provider(
    *,
    ...
    from pixel_cli.config import is_provider_enabled, load_config
    ...
    if pool and pool.has_credentials():
        ...
        # For Pixel, the pool entry's runtime_api_key is the agent_key
        # compatibility field. It must be an invoke JWT. The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `pixel-agents auth list`). If the key is
        # expired/missing, refresh the selected pool entry before falling back
        # to singleton auth resolution.
        if provider == "pixel" and entry is not None:
            min_ttl = max(60, env_int("PIXEL_AGENTS_PIXEL_MIN_KEY_TTL_SECONDS", 1800))
            pixel_state = {
                "agent_key": getattr(entry, "agent_key", None),
                "agent_key_expires_at": getattr(entry, "agent_key_expires_at", None),
                "scope": getattr(entry, "scope", None),
            }
            if not _agent_key_is_usable(pixel_state, min_ttl):
                logger.debug("Pixel pool entry agent_key expired/missing, refreshing selected pool entry")
                try:
                    refreshed = pool.try_refresh_current()
                except Exception as exc:
                    logger.debug("Pixel pool entry refresh failed: %s", exc)
                    refreshed = None
                ...
                if not pool_api_key or not _agent_key_is_usable(pixel_state, min_ttl):
                    logger.debug("Pixel pool entry agent_key still unavailable, falling through to runtime resolution")
                    pool_api_key = ""
        if (
            entry is not None
            and pool_api_key
    ...
    if provider == "pixel":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected Pixel but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected Pixel provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "pixel-agents-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from pixel_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Pixel Agents canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Pixel Agents-compatible importers).
