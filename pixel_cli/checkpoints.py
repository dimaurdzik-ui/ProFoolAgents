"""`pixel-agents checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.pixel-agents/checkpoints/``.  Actions:

    pixel-agents checkpoints               # same as `status`
    pixel-agents checkpoints status        # total size, project count, breakdown
    pixel-agents checkpoints list          # per-project checkpoint counts + workdir
    pixel-agents checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    pixel-agents checkpoints clear [-f]    # nuke the entire base (asks first)
    pixel-agents checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    pixel-agents checkpoints
    pixel-agents checkpoints prune --retention-days 3 --max-size-mb 200
    pixel-agents checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: pixel-agents checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``pixel-agents checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `pixel-agents checkpoints` → status
