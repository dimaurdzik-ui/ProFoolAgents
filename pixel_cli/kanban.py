"""CLI for the Pixel Agents Kanban board — ``pixel-agents kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/pixel-agents-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from pixel_cli import kanban_swarm as ks
...
def _check_dispatcher_presence(
    pixel_home: Optional[Path] = None,
) -> tuple[bool, str]:
    """Return ``(running, message)``.

    - ``running=True``: a gateway is alive for this PIXEL_AGENTS_HOME and its
    ...
    Used by ``pixel-agents kanban create`` (and callers) to warn when a task
    ...
    ``pixel_home`` scopes the probe to a named profile's directory. The
    dashboard plugin API passes it because the dashboard backend process can
    be running under a different PIXEL_AGENTS_HOME than the profile the request
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(
            profile_dir=pixel_home, use_cache=False
        )
    except Exception:
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`pixel-agents kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Pixel Agents profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://api.pixelagents.com/docs/user-guide/features/kanban "
            "or docs/pixel-agents-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # PIXEL_AGENTS_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `pixel-agents kanban boards switch <slug>` or the "
            "PIXEL_AGENTS_KANBAN_BOARD env var). Use `pixel-agents kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_create.add_argument("--project", default=None,
                          help="Link to a project (id or slug). Anchors the task's "
                               ...
                               "deterministic branch. See `pixel-agents project list`.")
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $PIXEL_AGENTS_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $PIXEL_AGENTS_PROFILE or 'user')")
    ...
    p_attach.add_argument("--author", default=None,
                          help="uploaded_by label (default: $PIXEL_AGENTS_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `pixel-agents gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.pixel-agents/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $PIXEL_AGENTS_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
    ...
    p_decompose.add_argument(
        ...
        default=None,
        help="Author name recorded on the audit comment "
             "(default: $PIXEL_AGENTS_PROFILE or 'decomposer')",
    )
    p_decompose.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``pixel-agents kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: pixel-agents kanban <action> [options]\n"
                "Run 'pixel-agents kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0

    # Fast-fail for clearer CLI UX only. The durable trust boundary is lower in
    # pixel_cli.kanban_db, because children can import DB mutators directly.
    ...
    # env-var pin for the duration of this call. Using PIXEL_AGENTS_KANBAN_BOARD
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `pixel-agents kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # PIXEL_AGENTS_HOME. Previously only `init` and `daemon` triggered
    ...
    with board_scope:
        ...
        # every `pixel-agents kanban repair` into "could not initialize database"
...
def _profile_author() -> str:
    ...
    for env in ("PIXEL_AGENTS_PROFILE_NAME", "PIXEL_AGENTS_PROFILE"):
...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return bool(os.environ.get("PIXEL_AGENTS_DELEGATED_CHILD_CONTEXT"))


# ---------------------------------------------------------------------------
# Boards management (pixel-agents kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``pixel-agents kanban boards <action>``.
    ...
    DB, so a fresh PIXEL_AGENTS_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `pixel-agents kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `pixel-agents kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `pixel-agents kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `pixel-agents kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # through to `pixel-agents -p <name>`.
    try:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.pixel-agents/profiles/.")
        print("Create one with `pixel-agents -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  pixel-agents gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `pixel-agents -p <name> setup`)")
        return 0
    # Header
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if len(all_boards) > 1:
        ...
        print(
            ...
            f"`pixel-agents kanban boards list`)\n"
        )
    if not tasks:
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from pixel_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``pixel-agents kanban show <id>``
...
def _cmd_complete(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        for tid in ids:
            ...
            # `pixel-agents kanban complete <id>` from the terminal tool and
            ...
            if task and task.goal_mode:
                ...
                if judge_available:
                    ...
                    try:
                        ...
                        # pixel_cli/goals.py. Unpacking fewer raises
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "pixel-agents kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    pixel-agents gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`pixel-agents kanban list --status ready` / "
                    f"`pixel-agents kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Pixel Agents profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from pixel_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from pixel_cli import kanban_decompose as decomp
