"""
Unified self-relaunch for Pixel Agents CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``pixel-agents sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``pixel-agents`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched pixel-agents."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_pixel_bin() -> Optional[str]:
    """Find the pixel-agents entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("pixel-agents")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m pixel_cli.main``.
    ...
    ``pixel-agents.exe`` on PATH or the ``sys.executable -m pixel_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    # PATH lookup
    path_bin = shutil.which("pixel-agents")
    if path_bin:
        return path_bin
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with pixel-agents.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_pixel_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "pixel_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh pixel-agents invocation.
    ...
    relaunch contract wants: "run pixel-agents again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``pixel-agents.exe`` (a Python console-script shim that wraps
    ``python -m pixel_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "pixel-agents exited, then
    new pixel-agents started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``pixel-agents`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nPixelAgents relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run pixel-agents.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
