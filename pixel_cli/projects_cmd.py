"""``pixel-agents project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$PIXEL_AGENTS_HOME/projects.db`` store
(see :mod:`pixel_cli.projects_db`).
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Pixel Agents'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``pixel-agents project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: pixel-agents project <action> [options]\n"
                "Run 'pixel-agents project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `pixel-agents project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `pixel-agents project add-folder`.",
            file=sys.stderr,
        )
        return 1
