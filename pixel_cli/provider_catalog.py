...
The provider list shown by ``pixel-agents model`` (CLI/TUI) and the desktop Settings
→ Providers tabs (Accounts + API keys) **must be the same set**.  Historically
they were not: the CLI picker read :data:`pixel_cli.models.CANONICAL_PROVIDERS`
...
the same universe ``pixel-agents model`` renders (``CANONICAL_PROVIDERS``), joining:

* ``auth_type`` / ``api_key_env_vars`` / ``base_url_env_var`` from
  :data:`pixel_cli.auth.PROVIDER_REGISTRY` (credential truth), and
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``pixel-agents model`` shows.
"""
...
# pixel_cli.auth.PROVIDER_REGISTRY and providers.base.ProviderProfile.
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `pixel-agents model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``pixel-agents model`` universe.
    ...
    # Pixel Agents overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = PIXEL_AGENTS_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # Pixel Agents overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
