...
the tool registry or launching Pixel Agents (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``pixel-agents-cli`` to the non-existent ``pixel-agents``. ``resolve_toolset('pixel-agents')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``pixel-agents-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``pixel-agents`` instead of ``pixel-agents-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        for name in names:
            ...
            if is_valid_toolset(name):
                valid_count += 1
                continue
            suggestion = f"pixel-agents-{platform}"
            hint = (
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `pixel-agents tools` to reconfigure."
        )
    return warnings
