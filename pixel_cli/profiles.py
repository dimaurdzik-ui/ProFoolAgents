"""
Profile management for multiple isolated Pixel Agents instances.

Each profile is a fully independent PIXEL_AGENTS_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.pixel-agents/profiles/<name>/`` by default.

The "default" profile is ``~/.pixel-agents`` itself — backward compatible,
zero migration needed.

Usage::

    pixel-agents profile create coder          # fresh profile + bundled skills
    pixel-agents profile create coder --clone  # also copy config, .env, SOUL.md, skills
    pixel-agents profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    pixel-agents -p coder chat                 # or via flag
    pixel-agents profile use coder             # set as sticky default
    pixel-agents profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See pixel_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.pixel-agents``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   pixel-agents  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `pixel-agents backup` archives
...
# Marker file written by `pixel-agents profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `pixel-agents
...
# `pixel-agents skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Pixel Agents infrastructure directories that only the default profile
         (``~/.pixel-agents``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_pixel_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.pixel-agents) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "pixel-agents",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when PIXEL_AGENTS_HOME equals the
...
# into the export. The set below identifies the *known Pixel Agents profile
# artifacts* at the root of PIXEL_AGENTS_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``pixel_constants``.
...
_RESERVED_NAMES = frozenset({
    "pixel-agents", "default", "test", "tmp", "root", "sudo",
})

# Pixel Agents subcommands that cannot be used as profile names/aliases
_PIXEL_AGENTS_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the pixel-agents root, NOT to the current PIXEL_AGENTS_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where PIXEL_AGENTS_HOME points outside
    ``~/.pixel-agents``, profiles live under ``PIXEL_AGENTS_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_pixel_home() / "profiles"


def _get_default_pixel_home() -> Path:
    """Return the default (pre-profile) PIXEL_AGENTS_HOME path.

    In standard deployments this is ``~/.pixel-agents``.
    In Docker/custom deployments where PIXEL_AGENTS_HOME is outside ``~/.pixel-agents``
    (e.g. ``/opt/data``), returns PIXEL_AGENTS_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_pixel_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``pixel-agents``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``pixel-agents`` profile inside ``~/.pixel-agents/``) or get refused
    at alias-creation time anyway. ``default`` is a special pass-through —
    it's a valid alias for the built-in root profile.
    """
    if name == "default":
        return  # special alias for ~/.pixel-agents
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Pixel Agents installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its PIXEL_AGENTS_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_pixel_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, pixel-agents subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _PIXEL_AGENTS_SUBCOMMANDS:
        return f"'{canon}' conflicts with a pixel-agents subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            pixel_exe = shutil.which("pixel-agents") or "pixel-agents"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(pixel_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Pixel Agents. If we
    ...
    try:
        ...
        # not be migrated. The next `pixel-agents doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``pixel-agents -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``pixel-agents profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``pixel-agents -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``pixel-agents profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``PIXEL_AGENTS_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Pixel Agents config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_pixel_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, pixel_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own PIXEL_AGENTS_HOME.
    ...
    The returned ``pixel_home`` is the path to pass to
    ``set_pixel_agents_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_pixel_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``pixel-agents update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.pixel-agents)."
        )
    ...
    if clone_from is not None or clone_all or clone_config:
        if clone_from is None:
            ...
            source_dir = get_pixel_agents_home()
        else:
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.pixel-agents/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `pixel-agents -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Pixel Agents profile.\n"
    ...
    if not soul_path.exists():
        try:
            from pixel_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `pixel-agents update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`pixel-agents profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Pixel Agents (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `pixel-agents profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `pixel-agents -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches PIXEL_AGENTS_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``pixel-agents profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "PIXEL_AGENTS_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_pixel_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Pixel Agents *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Pixel Agents backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``PIXEL_AGENTS_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `pixel-agents -p <canon> profile
    ...
    pixel_markers = ("pixel_cli.main", "pixel-agents-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Pixel Agents process: either an entrypoint marker in argv, or
            # a resolved executable named `pixel-agents`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_pixel = (
                any(marker in joined for marker in pixel_markers)
                or exe_name == "pixel-agents"
                or exe_name.startswith("pixel-agents")
            )
            if not is_pixel:
                continue
            ...
            # ...or by PIXEL_AGENTS_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("PIXEL_AGENTS_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.pixel-agents).\n"
            "To remove everything, use: pixel-agents uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_pixel_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``PIXEL_AGENTS_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from pixel_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set PIXEL_AGENTS_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("PIXEL_AGENTS_HOME")
    try:
        os.environ["PIXEL_AGENTS_HOME"] = str(profile_dir)
        from pixel_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["PIXEL_AGENTS_HOME"] = old_home
        elif "PIXEL_AGENTS_HOME" in os.environ:
            del os.environ["PIXEL_AGENTS_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.pixel-agents/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: pixel-agents profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from PIXEL_AGENTS_HOME.

    Returns ``"default"`` if PIXEL_AGENTS_HOME is not set or points to ``~/.pixel-agents``.
    Returns the profile name if PIXEL_AGENTS_HOME points into ``~/.pixel-agents/profiles/<name>``.
    Returns ``"custom"`` if PIXEL_AGENTS_HOME is set to an unrecognized path.
    """
    from pixel_constants import get_pixel_agents_home
    pixel_home = get_pixel_agents_home()
    resolved = pixel_home.resolve()

    default_resolved = _get_default_pixel_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      PIXEL_AGENTS_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Pixel Agents file the
      user may have lying alongside PIXEL_AGENTS_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.pixel-agents itself — its parent is ~/ and its
        # directory name is ".pixel-agents", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: pixel-agents profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.pixel-agents itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.pixel-agents). "
            "Specify a different name: pixel-agents profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="pixel_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"pixel_{old_name}"
    legacy_old_host = f"pixel-agents.{old_name}"
    new_host = f"pixel_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_pixel_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("pixel_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a PIXEL_AGENTS_HOME path string.

    Called early in the CLI entry point, before any pixel-agents modules
    are imported, to set the PIXEL_AGENTS_HOME environment variable.
    """
