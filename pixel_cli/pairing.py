...
Usage:
    pixel-agents pairing list              # Show all pending + approved users
    pixel-agents pairing approve <platform> <request-id|code>  # Approve a pairing request
    pixel-agents pairing revoke <platform> <user_id> # Revoke user access
    pixel-agents pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle pixel-agents pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: pixel-agents pairing {list|approve|revoke|clear-pending}")
        print("Run 'pixel-agents pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: pixel-agents pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.pixel-agents/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'pixel-agents pairing list' to see pending requests.\n")
