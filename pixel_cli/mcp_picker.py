"""MCP picker — interactive `pixel-agents mcp picker` (also the default `pixel-agents mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``pixel-agents mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `pixel-agents plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from pixel_cli.cli_output import prompt_yes_no
from pixel_cli.curses_ui import curses_single_select
from pixel_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Pixel Agents session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from pixel_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `pixel-agents mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: pixel-agents mcp install <name>    Picker: pixel-agents mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Pixel Agents is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Pixel Agents — run `pixel-agents update` "
...
def show_catalog() -> None:
    """`pixel-agents mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`pixel-agents mcp picker` (and default `pixel-agents mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`pixel-agents mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `pixel-agents mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
