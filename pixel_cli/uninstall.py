"""
Pixel Agents Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.pixel-agents/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Pixel Agents PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing pixel-agents or pixel-agents PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Pixel Agents" comment and following line
                if '# Pixel Agents' in line or '# pixel-agents' in line:
                    skip_next = True
                    continue
                if skip_next and ('pixel-agents' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing pixel-agents
                if 'pixel-agents' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the pixel-agents wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "pixel-agents",
        Path.home() / ".local" / "bin" / "pixel-agents-acp",
        Path("/usr/local/bin/pixel-agents"),
        Path("/usr/local/bin/pixel-agents-acp"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains pixel_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'pixel_cli' in content or 'pixel-agents' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(pixel_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``pixel-agents`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Pixel Agents home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (pixel_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``pixel-agents gateway run`` processes
    ...
    try:
        from pixel_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from pixel_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``PIXEL_AGENTS_HOME`` and ``PIXEL_AGENTS_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\pixel-agents\git\cmd``, ``%LOCALAPPDATA%\pixel-agents\git\bin``,
#      ``%LOCALAPPDATA%\pixel-agents\git\usr\bin``, ``%LOCALAPPDATA%\pixel-agents\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\pixel-agents\git\`` and Node to
#      ``%LOCALAPPDATA%\pixel-agents\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``pixel-agents dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\pixel-agents\gateway-service\`` and sometimes
...
def _pixel_path_markers(pixel_home: Path) -> list[str]:
    """Path-entry substrings that identify Pixel Agents-owned User-PATH entries."""
    root = str(pixel_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare pixel-agents install dir.
    markers = [root + "\\pixel-agents", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if PIXEL_AGENTS_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "pixel-agents".  We don't want to catch
    # unrelated entries like "cPixelAgents-foo" or "ephermeral", so we look for
    # backslash-pixel-agents as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(pixel_home: Path) -> list[str]:
    """Strip Pixel Agents-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _pixel_path_markers(pixel_home)
            kept: list[str] = []
...
def remove_pixel_env_vars_windows() -> list[str]:
    """Delete PIXEL_AGENTS_HOME and PIXEL_AGENTS_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("PIXEL_AGENTS_HOME", "PIXEL_AGENTS_GIT_BASH_PATH"):
                try:
...
def _is_default_pixel_home(pixel_home: Path) -> bool:
    """Return True when ``pixel_home`` points at the default (non-profile) root."""
    try:
        from pixel_constants import get_default_pixel_agents_root
        return pixel_home.resolve() == get_default_pixel_agents_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its PIXEL_AGENTS_HOME directory.

    We shell out to ``pixel-agents -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current PIXEL_AGENTS_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m pixel_cli.main` so we don't depend on a `pixel-agents`
    #    wrapper that may be half-removed mid-uninstall.
    pixel_invocation = [_sys.executable, "-m", "pixel_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                pixel_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's PIXEL_AGENTS_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``pixel-agents uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$PIXEL_AGENTS_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from pixel_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(pixel_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Pixel Agents Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Pixel Agents Chat GUI installation was found.")
        print(f"  Checked: {pixel_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Pixel Agents agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(pixel_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Pixel Agents agent at {pixel_home / 'pixel-agents'}")
        print(f"  • Your config, sessions, and secrets under {pixel_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Pixel Agents agent is still installed. Run 'pixel-agents' to use the CLI,")
    print("or 'pixel-agents uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.pixel-agents/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.pixel-agents/ for future reinstall
    """
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            ...
            pixel_home=pixel_home,
            full_uninstall=bool(getattr(args, "full", False)),
        )
        return

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie PIXEL_AGENTS_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_pixel_home(pixel_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.pixel-agents data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Pixel Agents Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {pixel_home / 'config.yaml'}")
    print(f"  Secrets: {pixel_home / '.env'}")
    print(f"  Data:    {pixel_home / 'cron/'}, {pixel_home / 'sessions/'}, {pixel_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their PIXEL_AGENTS_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Pixel Agents data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Pixel Agents code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, pixel_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Pixel Agents PATH entries from shell configs / Windows User PATH")
    print("  • Pixel Agents wrapper scripts and Pixel Agents-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Pixel Agents config/data: {pixel_home}")
        if _is_default_pixel_home(pixel_home):
        ...
        print(f"  • Keep Pixel Agents config/data: {pixel_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``pixel-agents`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$PIXEL_AGENTS_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in pixel_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\pixel-agents\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(pixel_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Pixel Agents-owned PATH entries in User environment")

        log_info("Removing PIXEL_AGENTS_HOME / PIXEL_AGENTS_GIT_BASH_PATH User env vars...")
        removed_env = remove_pixel_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Pixel Agents-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing pixel-agents command...")
    ...
    #     (only when they still point into this Pixel Agents home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Pixel Agents-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(pixel_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Pixel Agents-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(pixel_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE PIXEL_AGENTS_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from pixel_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(pixel_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.pixel-agents/, just remove the pixel-agents subdir
            if pixel_home in project_root.parents or project_root.parent == pixel_home:
    ...
    #     under PIXEL_AGENTS_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(pixel_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(pixel_home)
    ...
    # 5. Optionally remove ~/.pixel-agents/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile PIXEL_AGENTS_HOME dirs live under
        ...
        try:
            if pixel_home.exists():
                shutil.rmtree(pixel_home)
                log_success(f"Removed {pixel_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {pixel_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {pixel_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://api.pixelagents.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://api.pixelagents.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Pixel Agents! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m pixel_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import pixel_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``pixel_constants`` + ``pixel_cli.colors``
    (and lazily ``pixel_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m pixel_cli.uninstall")
    parser.add_argument(
        "--mode",
