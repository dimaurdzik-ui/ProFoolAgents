...
on Japanese locales, etc.).  Pixel Agents's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``pixel_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Pixel Agents is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``PIXEL_AGENTS_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("PIXEL_AGENTS_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Pixel Agents-managed Git install directories so
    ...
    # already-running shells don't see the change — which means pixel-agents
    ...
    # install without Pixel Agents-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Pixel Agents's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Pixel Agents's
    ``pixel-agents config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Pixel Agents (User env var in Windows Settings, or
    export in a PowerShell profile) and Pixel Agents picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Pixel Agents-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\pixel-agents\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``pixel-agents`` from right after install) retain their old PATH.

    Any subprocess Pixel Agents spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Pixel Agents-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "pixel-agents", "git", "cmd"),
        os.path.join(local_appdata, "pixel-agents", "git", "bin"),
        os.path.join(local_appdata, "pixel-agents", "git", "usr", "bin"),
        # Pixel Agents venv Scripts directory — host of the pixel-agents.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes pixel-agents, but harmless to include.
        os.path.join(local_appdata, "pixel-agents", "pixel-agents", "venv", "Scripts"),
