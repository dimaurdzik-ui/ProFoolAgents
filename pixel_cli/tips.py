...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or pixel-agents -c.",
    ...
    "Ctrl+Z suspends Pixel Agents to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "pixel-agents -c resumes your most recent CLI session. pixel-agents -c \"project name\" resumes by title.",
    "pixel-agents -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "pixel-agents -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "pixel-agents chat -t web,terminal enables only specific toolsets for a focused session.",
    "pixel-agents chat -s github-pr-workflow preloads a skill at launch.",
    "pixel-agents chat -q \"query\" runs a single non-interactive query and exits.",
    "pixel-agents chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "pixel-agents chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "pixel-agents --yolo bypasses all dangerous command approval prompts for the entire session.",
    "pixel-agents chat --source telegram tags the session for filtering in pixel-agents sessions list.",
    "pixel-agents -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "pixel-agents doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "pixel-agents dump outputs a compact setup summary — great for bug reports.",
    "pixel-agents config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "pixel-agents config edit opens config.yaml in your default editor.",
    "pixel-agents config check scans for missing or stale configuration options.",
    "pixel-agents sessions browse opens an interactive session picker with search.",
    "pixel-agents sessions stats shows session counts by platform and database size.",
    "pixel-agents sessions prune --older-than 30 cleans up old sessions.",
    "pixel-agents skills search react --source skills-sh searches the skills.sh public directory.",
    "pixel-agents skills check scans installed hub skills for upstream updates.",
    "pixel-agents skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "pixel-agents skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "pixel-agents mcp add github --command npx adds MCP servers from the command line.",
    "pixel-agents mcp serve runs Pixel Agents itself as an MCP server for other agents.",
    "pixel-agents auth add lets you add multiple API keys for credential pool rotation.",
    "pixel-agents completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "pixel-agents logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "pixel-agents backup creates a zip backup of your entire Pixel Agents home directory.",
    "pixel-agents profile create coder creates an isolated profile that becomes its own command.",
    "pixel-agents profile create work --clone copies your current config and keys to a new profile.",
    "pixel-agents update syncs new bundled skills to ALL profiles automatically.",
    "pixel-agents gateway install sets up Pixel Agents as a system service (systemd/launchd).",
    "pixel-agents memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "pixel-agents webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: pixel-agents tools disables unused tools, pixel-agents skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "pixel-agents models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as pixel-agents -w).",
    ...
    "PIXEL_AGENTS_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Pixel Agents tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'pixel-agents profile create coder' creates the 'coder' command.",
    "pixel-agents profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Pixel Agents prints a resume command with session ID and stats.",
    "pixel-agents sessions export backup.jsonl exports all sessions for backup or analysis.",
    "pixel-agents -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "pixel-agents skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .pixel-agents/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: pixel-agents cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Pixel Agents runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "pixel-agents gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Pixel Agents via a local macOS server.",
    ...
    "Context files (.pixel-agents.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Pixel Agents loads project context from .pixel-agents.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "pixel-agents mcp opens an interactive picker of Pixel-approved MCPs you can install in one keystroke.",
    "pixel-agents mcp catalog lists Pixel-approved MCP servers shipped with the repo.",
    "pixel-agents mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — pixel-agents tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.pixel-agents/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "pixel-agents chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "pixel-agents plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "pixel-agents acp runs Pixel Agents as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "PIXEL_AGENTS_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "pixel-agents auth add pixel or pixel-agents auth add openai-codex sets up OAuth-based providers.",
    ...
    "pixel-agents status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.pixel-agents/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Pixel Agents your own.",
    ...
    "Before a gateway session reset, Pixel Agents auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.pixel-agents/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Pixel Agents state via PIXEL_AGENTS_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "PIXEL_AGENTS_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in PIXEL_AGENTS_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.pixel-agents/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'pixel-agents profile create ls' would be rejected.",
    "pixel-agents profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "pixel-agents claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "PIXEL_AGENTS_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via pixel-agents tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.pixel-agents/.env.",
    "Skills from trusted repos (PixelResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.pixel-agents/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Pixel Agents auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Pixel Agents config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Pixel Agents is working.',
    ...
    '/reload reloads ~/.pixel-agents/.env into the running session — pick up new API keys without restarting.',
    ...
    "PIXEL_AGENTS_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.pixel-agents/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.pixel-agents/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'pixel-agents curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "pixel-agents curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'pixel-agents curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'pixel-agents auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Pixel subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'PIXEL_AGENTS_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "PIXEL_AGENTS_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'pixel-agents dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'pixel-agents dashboard embeds the full Pixel Agents TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.pixel-agents/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.pixel-agents/dashboard-plugins/ — no npm build required.',
    ...
    'PIXEL_AGENTS_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'PIXEL_AGENTS_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'PIXEL_AGENTS_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'PIXEL_AGENTS_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.pixel-agents/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.pixel-agents/skills/ so drop-in skills appear without restarting the session.',
    ...
    'pixel-agents -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'pixel-agents chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'pixel-agents chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'pixel-agents chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "pixel-agents chat --source tool tags programmatic chats so they don't clutter pixel-agents sessions list.",
    'pixel-agents dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'pixel-agents sessions rename <ID> "new title" renames any past session; pixel-agents sessions delete <ID> removes one.',
    'pixel-agents import restores a full Pixel Agents backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'pixel-agents fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'pixel-agents pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'pixel-agents setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'pixel-agents status --deep runs the full health sweep across every component; plain pixel-agents status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'PIXEL_AGENTS_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'PIXEL_AGENTS_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.pixel-agents/plugins/ — trust-gated by design.',
    "PIXEL_AGENTS_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'PIXEL_AGENTS_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'PIXEL_AGENTS_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'PIXEL_AGENTS_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'PIXEL_AGENTS_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'PIXEL_AGENTS_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'PIXEL_AGENTS_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'PIXEL_AGENTS_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'PIXEL_AGENTS_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'PIXEL_AGENTS_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'PIXEL_AGENTS_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Pixel Agents block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from pixel-agents sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.pixel-agents/dashboard-plugins/.',
]
