"""Windows ConPTY bridge for the `pixel-agents dashboard` chat tab.

Drop-in counterpart to ``pixel_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``pixel_cli.web_server`` consumes: ``spawn``, ``read``,
