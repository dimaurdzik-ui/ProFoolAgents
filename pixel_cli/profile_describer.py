...
Used by ``pixel-agents profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``pixel_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Pixel Agents kanban board.
...
Rules:
  ...
  - Never write "Pixel Agents profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_pixel_agents_home())
        else:
