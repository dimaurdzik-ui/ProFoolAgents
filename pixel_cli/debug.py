"""``pixel-agents debug`` debug tools for Pixel Agents.

Currently supports:
    pixel-agents debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.pixel-agents/logs/*.log`` are not leaked into
                          the public paste service. Pass ``--no-redact``
                          to disable.
                          Pass ``--pixel`` to upload instead to Pixel-internal
                          storage (AWS S3) via a signed URL minted by the
                          Pixel account service: the bundle is private
                          (viewable only by Pixel staff / allowlisted mods via
...
_REDACTION_BANNER = (
    "[pixel-agents debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.pixel-agents/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``pixel-agents debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    """Synchropixelly DELETE any pending pastes whose ``expire_at`` has passed.
    ...
    intended to be called from every ``pixel-agents debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `pixel-agents debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "pixel-agents/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``pixel-agents debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.pixel-agents/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``pixel-agents debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "pixel-agents/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----PixelAgentsDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from pixel_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_pixel_agents_home() / "logs" / filename) if filename else None
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``pixel-agents debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``pixel-agents dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``pixel-agents dump`` is run
        internally.
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by both the paste.rs and Pixel-S3 paths)
# ---------------------------------------------------------------------------

# Bundle format identifier embedded in the Pixel-S3 JSON envelope. The
# discord-support viewer keys off this string to parse the bundle.
_PIXEL_BUNDLE_FORMAT = "pixel-agents-debug-share/1"
...
def collect_share_bundle(
    ...
    Pixel-S3 path (``--pixel``).  Centralising it guarantees the Pixel bundle is
    built from the *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so the Pixel path must never see raw
    logs.
...
def build_pixel_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip-compress a :func:`collect_share_bundle` mapping into the Pixel envelope.

    The JSON shape is what the discord-support viewer (Repo 3) parses::

        {"format": "pixel-agents-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``pixel-agents debug share`` (CLI) and the
    ...
    # shared collector so the paste.rs and Pixel-S3 paths build identical,
    ...
    if redact:
        logger.info(
            "pixel-agents debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    if pixel:
        _run_debug_share_pixel(args, log_lines=log_lines, redact=redact)
        return
    ...
    try:
        ...
        print("\nRun `pixel-agents debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  pixel-agents debug delete <url>")

    print("\nShare these links with the Pixel Agents team for support.")


_PIXEL_PRIVACY_NOTICE = """\
⚠️  --pixel: This uploads your debug bundle to Pixel-INTERNAL storage (AWS S3),
    NOT a public paste service. The following is included:
  • System info (OS, Python/Pixel Agents version, provider, which API keys are
  ...
  • The bundle is viewable only by Pixel staff (and allowlisted Discord mods)
...
def _run_debug_share_pixel(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``pixel-agents debug share --pixel``: upload the bundle to Pixel-S3.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the Pixel envelope, requests a signed URL from NAS, uploads, and prints the
    private viewer link. On any failure falls back to a clear error that
    suggests ``--local``.
    """
    from pixel_cli.diagnostics_upload import share_to_pixel

    print(_PIXEL_PRIVACY_NOTICE)
    if not _confirm_upload(args):
        return
    if not redact:
        print(
    ...
    if redact:
        logger.info(
            "pixel-agents debug share --pixel: applied force-mode redaction before upload"
        )
    blob = build_pixel_bundle(bundle, redact=redact)

    print("Uploading to Pixel diagnostics storage...")
    try:
        res = share_to_pixel(blob)
    except Exception as exc:
        print(
            f"\nPixel upload failed: {exc}\n"
            "\nThe Pixel diagnostics service may be unavailable or not yet "
            "provisioned.\n"
            "Run `pixel-agents debug share --local` to print the report instead, "
            "or `pixel-agents debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)

    view_url = res.get("viewUrl") or res.get("view_url")
    print("\nDebug bundle uploaded to Pixel (private):")
    if view_url:
    ...
    print(
        "\nShare this private link with the Pixel team — only Pixel staff "
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: pixel-agents debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'pixel-agents debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``pixel-agents debug`` call.
    ...
    # best-effort — any failure is swallowed so ``pixel-agents debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: pixel-agents debug <command>")
        print()
        print("Commands:")
        ...
        print("  --pixel       Upload to Pixel-internal storage (private, staff-only,")
