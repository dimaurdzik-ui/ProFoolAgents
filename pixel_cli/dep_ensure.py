...
def _has_pixel_agent_browser() -> bool:
    ...
    # install.sh installs globally into $PIXEL_AGENTS_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-PixelAgentsHome", str(get_pixel_agents_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = pixel_subprocess_env(inherit_credentials=False)
