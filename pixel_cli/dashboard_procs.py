"""Dashboard process-hygiene helpers — extracted from ``pixel_cli/main.py``.
...
``_detect_concurrent_pixel_instances``) are lifted verbatim. References to
helpers that STAY in ``pixel_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``pixel_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``pixel_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``pixel_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``pixel-agents dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``pixel-agents update`` replaces files on disk, the running
    ...
    ``pixel-agents-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Pixel Agents Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``pixel-agents serve`` as
    ...
    environment variable ``PIXEL_AGENTS_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`pixel-agents serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "pixel-agents.*dashboard"`) keeps us consistent
            # with `pixel_cli.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``pixel-agents dashboard`` / ``pixel-agents serve`` processes.

    Called at the end of ``pixel-agents update`` (default ``reason``) and also
    from ``pixel-agents dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``pixel-agents update`` path), a detected
    ``pixel-agents-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``pixel-agents-serve.service``) has its owning unit
    ...
    # When the Pixel Agents Desktop Electron app spawns this dashboard as a
    # backend child, it sets PIXEL_AGENTS_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("PIXEL_AGENTS_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `pixel-agents update` path) — `--stop` must
    ...
    #    remote backend (pixel-agents serve) under Restart=on-failure never comes
...
def _detect_concurrent_pixel_instances(
    ...
    The Pixel Agents Desktop Electron app spawns ``pixel-agents.EXE`` as a backend child,
    so during ``pixel-agents update`` the user-invoked process and the desktop's
    ...
    shims (``pixel-agents.exe`` / ``pixel-agents-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``pixel-agents update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``pixel-agents.exe``) is a separate process from the Python
    ...
    for shim in _m()._pixel_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated pixel-agents.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``pixel-agents update``
    ...
    #      pixel-agents.exe sitting *under* a non-Pixel Agents parent (e.g. a Pixel Agents
