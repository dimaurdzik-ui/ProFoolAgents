"""Pixel Agents update pipeline — extracted from ``pixel_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``pixel_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``pixel_cli.main`` reference — so existing call sites and test monkeypatches
that target ``pixel_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``pixel_cli.main``.
...
Imports are one-way: ``pixel_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``pixel_cli.main`` reference.

    Lets callers keep patching ``pixel_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``pixel_cli.main`` -> ``pixel_cli.update_cmd`` stays one-way
    at import time.
    """
    from pixel_cli import main
...
def _reload_updated_runtime_modules() -> None:
    ...
    ``pixel-agents update`` keeps running in the pre-pull Python process. After a
...
# Critical files that Pixel Agents must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "pixel_cli/main.py",
    "pixel_cli/config.py",
    "pixel_cli/__init__.py",
    "pixel_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``pixel-agents`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="pixel-agents-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``pixel-agents update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `pixel-agents update`.
    ...
    print("  Preview now:  pixel-agents curator run --dry-run")
    print("  Pause it:     pixel-agents curator pause")
    print(
        "  Docs:         https://api.pixelagents.com/docs/user-guide/features/curator"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `pixel-agents update`.
    ...
    try:
        ...
        return
    db_path = get_pixel_agents_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `pixel-agents sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    pixel-agents sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  pixel-agents sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``pixel-agents update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``pixel-agents update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: pixel-agents curator status)"
    )
...
def _finish_dashboard_update_cleanup(node_failures: list[str]) -> None:
    ...
    print("    pixel-agents dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.pixel-agents-update-staging"
    backup = f"{dst}.pixel-agents-update-old"
...
def _update_via_zip(args):
    """Update Pixel Agents by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `pixel-agents update "
            f"--branch {branch}`, or update against main with `pixel-agents update`."
        )
        _m().sys.exit(1)
    zip_url = (
        f"https://github.com/PixelResearch/pixel-agents/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="pixel-agents-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"pixel-agents-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for pixel-agents itself
        ...
        # GitHub ZIPs extract to pixel-agents-<branch>/
        extracted = os.path.join(tmp_dir, f"pixel-agents-{branch}")
    ...
    from pixel_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: pixel-agents skills list-modified  "
    ...
    try:
        from pixel_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from pixel_cli.backup import _quick_snapshot_root, verify_sqlite_integrity

        _state_path = get_pixel_agents_home() / "state.db"
        if _state_path.exists():
            ...
            if not _state_ok.get("valid"):
                ...
                _snap_root = _quick_snapshot_root(get_pixel_agents_home())
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "pixel-agents-update-autostash-%Y%m%d-%H%M%S"
    )
    ...
    if push.returncode != 0:
        if stash_created:
            ...
            print(
                ...
                "then re-run `pixel-agents update`."
            )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Pixel Agents behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes pixel-agents completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Pixel Agents couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Pixel Agents couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Pixel Agents behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Pixel Agents couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Pixel Agents couldn't drop "
...
OFFICIAL_REPO_URLS = {
    "https://github.com/PixelResearch/pixel-agents.git",
    "git@github.com:PixelResearch/pixel-agents.git",
    "https://github.com/PixelResearch/pixel-agents",
    "git@github.com:PixelResearch/pixel-agents",
}

OFFICIAL_REPO_URL = "https://github.com/PixelResearch/pixel-agents.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_pixel_agents_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_pixel_agents_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Pixel Agents repository.")
        print("  This means you may miss updates from PixelResearch/pixel-agents.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/PixelResearch/pixel-agents.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/PixelResearch/pixel-agents.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``pixel-agents update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from pixel_constants import get_default_pixel_agents_root

    default_home = get_default_pixel_agents_root()
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "pixel-agents.exe"
    lines = ["✗ Another pixel-agents.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Pixel Agents Desktop, exit any open `pixel-agents` REPLs, and")
    lines.append("  stop the gateway (`pixel-agents gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `pixel-agents update --force` if you've already")
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``pixel-agents update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `pixel-agents update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `pixel-agents update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Pixel Agents' editable-install extras or
    ...
    try:
        from pixel_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from pixel_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$PIXEL_AGENTS_HOME/bin/uv``, but on Termux the official
    ...
    """
    from pixel_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `pixel-agents update` is exactly the step
...
def _npm_lockfile_changed(pixel_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `pixel-agents
    ...
    try:
        ...
        cache_file = pixel_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(pixel_root: Path) -> None:
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from pixel_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `pixel-agents update`.")
            failed = ["repo root"]
            if any(
    ...
    # Pixel Agents profile using this checkout. Keep one per-checkout cache under the
    # shared Pixel Agents root rather than rerunning npm once per named profile.
    shared_pixel_root = get_default_pixel_agents_root()
    if not _m()._npm_lockfile_changed(shared_pixel_root):
    ...
    # need desktop during `pixel-agents update`, so we install root-only first
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `pixel-agents update`.")
        return list(labels)
    ...
    nixos_env = with_pixel_node_path(_m()._nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `pixel-agents update` comes from
    ...
    if ws_result.returncode == 0:
        _record_npm_lockfile_hash(shared_pixel_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.pixel-agents/logs/update.log`` only, never the terminal.

    During ``pixel-agents update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``pixel-agents update --check``: fetch and report without installing.
    ...
    """
    from pixel_cli.config import detect_install_method, recommended_update_command_for_method
    method = detect_install_method(_m().PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``pixel-agents update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from pixel_cli.config import format_docker_update_message
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``pixel-agents update`` without requiring a reinstall.  The
    ...
    ``pixel-agents`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/pixel-agents.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v pixel-agents'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/pixel-agents, code at /usr/local/lib/pixel-agents).
    fhs_link = Path("/usr/local/bin/pixel-agents")
    ...
    path_comment = (
        "# Pixel Agents — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    """Self-heal: install a ``pixel-agents-acp`` launcher next to the ``pixel-agents`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``pixel-agents update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``pixel-agents-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Pixel Agents as not installed even when it is.

    The shim simply delegates to the sibling ``pixel-agents`` launcher with the
    ...
    ``pixel-agents-acp.exe`` already resolves) and wherever a ``pixel-agents-acp`` is
    already present next to the ``pixel-agents`` command.  Unwritable directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        return
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        pixel_cmd = bin_dir / "pixel-agents"
        acp_cmd = bin_dir / "pixel-agents-acp"
        try:
            if not (pixel_cmd.is_file() or pixel_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of PIXEL_AGENTS_HOME under
      ``backups/`` (restorable via ``pixel-agents import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from pixel_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from pixel_constants import
        # get_pixel_agents_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from pixel_cli.config import get_pixel_agents_home as _get_home
    ...
    try:
        from pixel_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_pixel_agents_home so the user sees ~/.pixel-agents/...
    try:
        from pixel_constants import get_pixel_agents_home, display_pixel_agents_home

        home = get_pixel_agents_home()
        try:
            display_path = f"{display_pixel_agents_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  pixel-agents import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``pixel-agents update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``pixel-agents update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run pixel-agents from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.pixel-agents-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".pixel-agents-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The pixel-agents.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m pixel_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``pixel-agents
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Pixel Agents processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Pixel Agents Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Pixel Agents desktop app / other Pixel Agents terminals, then re-run:"
    )
    lines.append("    pixel-agents update")
    lines.append("  (or use `pixel-agents update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    pixel-agents.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Pixel Agents...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m pixel_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: pixel-agents gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``pixel-agents gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from pixel_cli import gateway_windows
        from pixel_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``pixel-agents-gateway*.service`` from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not unit.startswith("pixel-agents-gateway"):
            continue
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    print("    pixel-agents gateway status")
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``pixel-agents update`` never touched them, so installs created before the
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``pixel-agents update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``pixel-agents update``, which ships with the checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Pixel Agents...")
    print()

    # On Windows, abort early if another pixel-agents.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_pixel_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `pixel-agents serve` backend) will keep .pyd
    ...
    # the pixel-agents.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Pixel Agents windows/gateways and re-run: pixel-agents update")
            else:
                print("✓ Already up to date!")
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Pixel Agents gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in pixel_cli/config.py bricked
        # every user who ran ``pixel-agents update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``pixel-agents
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``pixel-agents update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_pixel_agents_home added to pixel_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``pixel-agents`` launch finishes the install
        ...
        # symbols in pixel_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `pixel-agents` again "
        ...
        # build.  ``pixel-agents desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``pixel-agents desktop`` shouldn't be forced into a full
        # Electron build by ``pixel-agents update``.
        ...
        if (desktop_dir / "package.json").exists() and _m()._resolve_node_runtime_npm() and has_desktop_app:
            ...
            # `pixel-agents desktop --build-only` subprocess re-imports the whole
            ...
            if _skip_desktop_build:
                ...
                _desktop_build_cmd = [sys.executable, "-m", "pixel_cli.main", "desktop", "--build-only"]
                ...
                # debuggable.
                #
                # Start the build subprocess with the Pixel Agents-managed Node on PATH:
                # when `pixel-agents update` runs inside the desktop updater chain
                # (Desktop → pixel-agents-setup → pixel-agents update), the shell PATH
                # customizations are lost, so a bare-PATH child would fail with
                # `node: not found` before cmd_gui can self-heal.
                from pixel_constants import with_pixel_node_path

                _build_env = with_pixel_node_path()
                build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    build_result = _m()._run_logged_subprocess(_desktop_build_cmd, cwd=_m().PROJECT_ROOT, env=_build_env)
                if build_result.returncode != 0:
                    print("  ⚠ Desktop build failed (non-fatal; run `pixel-agents desktop` to retry)")
        ...
        try:
            ...
            if _state_path.exists():
                ...
                if _state_ok.get("valid"):
                    ...
                    if _pre_snap_id:
                        _snap_state = (
                            _quick_snapshot_root(get_pixel_agents_home())
                            / _pre_snap_id
                            / "state.db"
                        )
                        if _snap_state.exists():
        ...
        # current — so copy it straight over ~/.pixel-agents/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit PIXEL_AGENTS_HOME so
        # it is not affected by sync_skills()'s module-level PIXEL_AGENTS_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's PIXEL_AGENTS_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from pixel_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'pixel-agents config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: pixel-agents config migrate")
            else:
                print()
                print("Skipped. Run 'pixel-agents config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from pixel_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `pixel-agents curator status`. Self-stamps after printing so it
        ...
        # Self-heal the pixel-agents-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Pixel Agents on PATH without
        # a reinstall.  No-op on Windows and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("pixel-agents-acp launcher self-heal failed: %s", e)
        ...
        # don't have it. Tying the refresh to ``pixel-agents update`` gives users a
        ...
        try:
            ...
            if (
                ...
                # driver) keeps the installed version — `pixel-agents update`
                # must stay fast; `pixel-agents computer-use install --upgrade`
        ...
        # When running as ``pixel-agents update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_pixel_agents_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... pixel-agents-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... pixel-agents-gateway*)
            ...
            # Discover all pixel-agents-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: pixel-agents gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `pixel-agents update` restart it automatically, allow\n"
                        ...
                        # path in `pixel-agents gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: pixel-agents gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: pixel-agents -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Pixel Agents gateway unit files are still installed.
        # When both pixel-agents.service (from a pre-rename install) and the
        # current pixel-agents-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `pixel-agents update` surfaces the issue until the user migrates.
        try:
            from pixel_cli.gateway import (
                has_legacy_pixel_units,
                _find_legacy_pixel_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_pixel_units():
                print()
                print("⚠ Legacy Pixel Agents gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_pixel_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (pixel-agents.service) fight the current")
                print("  pixel-agents-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    pixel-agents gateway migrate-legacy")
                print()
        ...
        print("  pixel-agents model              # Select provider and model")
