...
from ``pixel_constants`` or other pixel_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$PIXEL_AGENTS_HOME/logs/dashboard-auth.log``.

    Uses ``pixel_constants.get_pixel_agents_home()`` (a leaf module — no import
    ...
    return get_pixel_agents_home() / "logs" / "dashboard-auth.log"
