...
def active_session_limit_message(
    ...
    return (
        f"Pixel Agents is at the active session limit ({active_count}/{max_sessions})."
        f"{detail} Try again when another session finishes."
    )


def _state_dir() -> Path:
    return Path(get_pixel_agents_home()) / "runtime"
...
def release_orphaned_leases(live_lease_ids: set[str]) -> int:
    ...
    that runs for days (``pixel-agents dashboard`` / ``serve``) never trips that
