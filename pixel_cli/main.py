#!/usr/bin/env python3
"""
Pixel Agents CLI - Main entry point.

Usage:
    pixel-agents                     # Interactive chat (default)
    pixel-agents chat                # Interactive chat
    pixel-agents gateway             # Run gateway in foreground
    pixel-agents gateway start       # Start gateway as service
    pixel-agents gateway stop        # Stop gateway service
    pixel-agents gateway status      # Show gateway status
    pixel-agents gateway install     # Install gateway service
    pixel-agents gateway uninstall   # Uninstall gateway service
    pixel-agents setup               # Interactive setup wizard
    pixel-agents logout              # Clear stored authentication
    pixel-agents status              # Show status of all components
    pixel-agents cron                # Manage cron jobs
    pixel-agents cron list           # List cron jobs
    pixel-agents cron status         # Check if cron scheduler is running
    pixel-agents doctor              # Check configuration and dependencies
    pixel-agents honcho setup                    # Configure Honcho AI memory integration
    pixel-agents honcho status                   # Show Honcho config and connection status
    pixel-agents honcho sessions                 # List directory → session name mappings
    pixel-agents honcho map <name>               # Map current directory to a session name
    pixel-agents honcho peer                     # Show peer names and dialectic settings
    pixel-agents honcho peer --user NAME         # Set user peer name
    pixel-agents honcho peer --ai NAME           # Set AI peer name
    pixel-agents honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    pixel-agents honcho mode                     # Show current memory mode
    pixel-agents honcho mode [hybrid|honcho|local]  # Set memory mode
    pixel-agents honcho tokens                   # Show token budget settings
    pixel-agents honcho tokens --context N       # Set session.context() token cap
    pixel-agents honcho tokens --dialectic N     # Set dialectic result char cap
    pixel-agents honcho identity                 # Show AI peer identity representation
    pixel-agents honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    pixel-agents honcho migrate                  # Step-by-step migration guide: OpenClaw native → Pixel Agents + Honcho
    pixel-agents version             Show version
    pixel-agents update              Update to latest version
    pixel-agents uninstall           Uninstall Pixel Agents
    pixel-agents acp                 Run as an ACP server for editor integration
    pixel-agents sessions browse     Interactive session picker with search

    pixel-agents claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: pixel_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``pixel_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``pixel-agents update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``pixel_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, pixel-agents crashes on import and the user can't run
# ``pixel-agents update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
from pixel_cli._subprocess_compat import suppress_platform_ver_console
...
# a prior ``pixel-agents update`` left a recovery marker and a core package's import
# files were wiped (#57828 — failed lazy backend refresh), the module-level
# ``from pixel_cli.env_loader import ...`` / ``from pixel_cli.config import
...
# package directory, so if IT can't import, nothing else in pixel_cli can
# either. It is also the canonical home of the probe/repair tables reused by
# the full recovery path below.
from pixel_cli import _early_recovery as _early_recovery_mod
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from pixel_cli.oneshot import run_oneshot
...
def _set_process_title() -> None:
    """Set the process title to 'pixel-agents' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``pixel-agents tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``pixel-agents.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("pixel-agents")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"pixel-agents", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"pixel-agents")
        # Windows: the .exe name is already ``pixel-agents.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* pixel_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("PIXEL_AGENTS_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".pixel-agents", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``PIXEL_AGENTS_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``pixel-agents … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("PIXEL_AGENTS_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``PIXEL_AGENTS_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("PIXEL_AGENTS_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`pixel-agents --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from pixel_cli import __release_date__, __version__

    print(f"Pixel Agents v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``pixel-agents --version`` before config/logging imports on Termux."""
    if os.environ.get("PIXEL_AGENTS_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from pixel_cli.sessions_cmd import cmd_sessions  # noqa: F401
from pixel_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from pixel_cli.subcommands.cron import build_cron_parser
from pixel_cli.subcommands.sync import build_sync_parser
from pixel_cli.subcommands.gateway import build_gateway_parser
from pixel_cli.subcommands.profile import build_profile_parser
from pixel_cli.subcommands.model import build_model_parser
from pixel_cli.subcommands.setup import build_setup_parser

from pixel_cli.subcommands.whatsapp import build_whatsapp_parser
from pixel_cli.subcommands.slack import build_slack_parser
from pixel_cli.subcommands.login import build_login_parser
from pixel_cli.subcommands.logout import build_logout_parser
from pixel_cli.subcommands.auth import build_auth_parser
from pixel_cli.subcommands.status import build_status_parser
from pixel_cli.subcommands.webhook import build_webhook_parser
from pixel_cli.subcommands.hooks import build_hooks_parser
from pixel_cli.subcommands.doctor import build_doctor_parser
from pixel_cli.subcommands.security import build_security_parser
from pixel_cli.subcommands.approvals import build_approvals_parser
from pixel_cli.subcommands.dump import build_dump_parser
from pixel_cli.subcommands.debug import build_debug_parser
from pixel_cli.subcommands.backup import build_backup_parser
from pixel_cli.subcommands.import_cmd import build_import_cmd_parser
from pixel_cli.subcommands.import_agent import build_import_agent_parser
from pixel_cli.subcommands.config import build_config_parser
from pixel_cli.subcommands.skin import build_skin_parser
from pixel_cli.subcommands.console import build_console_parser
from pixel_cli.subcommands.version import build_version_parser
from pixel_cli.subcommands.update import build_update_parser
from pixel_cli.subcommands.uninstall import build_uninstall_parser
from pixel_cli.subcommands.dashboard import build_dashboard_parser
from pixel_cli.subcommands.gui import build_gui_parser
from pixel_cli.subcommands.logs import build_logs_parser
from pixel_cli.subcommands.prompt_size import build_prompt_size_parser
from pixel_cli.subcommands.memory import build_memory_parser
from pixel_cli.subcommands.acp import build_acp_parser
from pixel_cli.subcommands.tools import build_tools_parser
from pixel_cli.subcommands.insights import build_insights_parser
from pixel_cli.subcommands.monitoring import build_monitoring_parser
from pixel_cli.subcommands.skills import build_skills_parser
from pixel_cli.subcommands.pairing import build_pairing_parser
from pixel_cli.subcommands.plugins import build_plugins_parser
from pixel_cli.subcommands.mcp import build_mcp_parser
from pixel_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (pixel-agents tools, pixel-agents setup, pixel-agents model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'pixel-agents {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any pixel-agents module import.
#
# Many modules cache PIXEL_AGENTS_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("PIXEL_AGENTS_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.pixel-agents/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set PIXEL_AGENTS_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `pixel-agents mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Pixel Agents' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo pixel-agents -p <name>` against the invoking user's home.
        ...
        candidate = home / ".pixel-agents" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`pixel-agents chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors pixel_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If PIXEL_AGENTS_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.pixel-agents/profiles/coder or
    # /opt/data/profiles/coder).  If PIXEL_AGENTS_HOME points to the pixel-agents root
    # instead (e.g. systemd hardcodes PIXEL_AGENTS_HOME=/root/.pixel-agents), we must
    # still read active_profile — the user may have switched profiles via
    # `pixel-agents profile use` and the gateway should honour that choice.
    # See issue #22502.
    pixel_home_env = os.environ.get("PIXEL_AGENTS_HOME", "")
    if profile_name is None and pixel_home_env:
        if Path(pixel_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the pixel-agents root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as PIXEL_AGENTS_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``pixel-agents gateway run`` to mean
    # "the root PIXEL_AGENTS_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("PIXEL_AGENTS_S6_SUPERVISED_CHILD"):
        try:
            from pixel_constants import get_default_pixel_agents_root

            active_path = get_default_pixel_agents_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set PIXEL_AGENTS_HOME
    if profile_name is not None:
        try:
            from pixel_cli.profiles import resolve_profile_env

            pixel_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            pixel_home = _resolve_sudo_user_profile_env(profile_name)
            if not pixel_home:
            ...
            # A bug in profiles.py must NEVER prevent pixel-agents from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["PIXEL_AGENTS_HOME"] = pixel_home
...
# Load .env from ~/.pixel-agents/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from pixel_cli.config import get_pixel_agents_home
from pixel_cli.env_loader import load_pixel_dotenv

load_pixel_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → PIXEL_AGENTS_REDACT_SECRETS env
# var BEFORE pixel_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    # yaml.load — the SAME parse then serves pixel_logging's
    # _read_logging_config and any later raw reads in this process, collapsing
    # 3-4 config.yaml parses per invocation into one.
    from pixel_cli.config import read_raw_config as _read_raw_early

    _cfg_path = get_pixel_agents_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            ...
            pass
        if "PIXEL_AGENTS_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["PIXEL_AGENTS_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `pixel-agents` subcommands
...
try:
    from pixel_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from pixel_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if pixel_constants not importable yet
...
# Provider model-selection wizard flows extracted to pixel_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (pixel_cli.main._model_flow_*) keep resolving unchanged.
from pixel_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_pixel_agents_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("PIXEL_AGENTS_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("PIXEL_AGENTS_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from pixel_cli.config import get_env_path, get_pixel_agents_home, load_config
    from pixel_cli.auth import get_auth_status

    # Determine whether Pixel Agents itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_pixel_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check for Pixel Portal OAuth credentials
    auth_file = get_pixel_agents_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Pixel Agents has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Pixel Agents to use their tokens.
    if _has_pixel_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    ``pixel-agents -c`` from repo A continues repo A's last session rather than the
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, pixel_bin
        cli_args: the original CLI arguments (everything after 'pixel-agents')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    pixel_bin = container_info["pixel_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo pixel-agents {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo pixel-agents {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  pixel-agents --tui --resume {target}")
    if title:
        print(f'  pixel-agents --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @pixel-agents/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@pixel-agents" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/pixel-agents-ink/package.json",
    "packages/pixel-agents-ink/index.js",
    "packages/pixel-agents-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``PIXEL_AGENTS_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("PIXEL_AGENTS_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``PIXEL_AGENTS_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("PIXEL_AGENTS_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "PIXEL_AGENTS_HOME": pixel_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(pixel_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(pixel_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if pixel_cli_dir is None:
        pixel_cli_dir = Path(__file__).parent
    bundled = pixel_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``pixel-agents update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("PIXEL_AGENTS_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Pixel Agents checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `pixel-agents update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Pixel Agents checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `pixel-agents --tui`\n"
        "If the checkout is still inconsistent, run `pixel-agents update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (PIXEL_AGENTS_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("PIXEL_AGENTS_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from pixel_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("PIXEL_AGENTS_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with PIXEL_AGENTS_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset PIXEL_AGENTS_TUI_DIR (e.g. `unset PIXEL_AGENTS_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # pixel_cli/tui_dist/entry.js but never ships ui-tui/ at all (that
    ...
    if tui_dev:
        # Keep the local @pixel-agents/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @pixel-agents/ink resolves through
        # packages/pixel-agents-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "pixel-agents-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from pixel_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    src_root = str(env.get("PIXEL_AGENTS_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["PIXEL_AGENTS_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("PIXEL_AGENTS_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["PIXEL_AGENTS_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("PIXEL_AGENTS_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["PIXEL_AGENTS_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["PIXEL_AGENTS_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    # TUI child is a pixel-agents process: propagate the profile-home contract via
    # the single factory; keep secrets (the TUI/agent needs provider creds).
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    try:
        from pixel_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="pixel-agents-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["PIXEL_AGENTS_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["PIXEL_AGENTS_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    _apply_tui_python_env(env)

    if model:
        env["PIXEL_AGENTS_MODEL"] = model
        env["PIXEL_AGENTS_INFERENCE_MODEL"] = model
    if provider:
        env["PIXEL_AGENTS_TUI_PROVIDER"] = provider
        env["PIXEL_AGENTS_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["PIXEL_AGENTS_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["PIXEL_AGENTS_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["PIXEL_AGENTS_TUI_SKILLS"] = value
    if query:
        env["PIXEL_AGENTS_TUI_QUERY"] = query
    if image:
        env["PIXEL_AGENTS_TUI_IMAGE"] = image
    if checkpoints:
        env["PIXEL_AGENTS_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["PIXEL_AGENTS_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["PIXEL_AGENTS_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["PIXEL_AGENTS_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["PIXEL_AGENTS_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["PIXEL_AGENTS_ACCEPT_HOOKS"] = "1"
    ...
    # PIXEL_AGENTS_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from a full os.environ snapshot (via
    # build_subprocess_env), an exported/stale value
    # in the user's shell would otherwise make a plain `pixel-agents --tui` try to
    ...
    # still set PIXEL_AGENTS_TUI_RESUME themselves.
    env.pop("PIXEL_AGENTS_TUI_RESUME", None)
    if resume_session_id:
        env["PIXEL_AGENTS_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `pixel-agents update` so
    ...
    if code == 42:
        from pixel_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``PIXEL_AGENTS_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``pixel-agents kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``pixel-agents kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("PIXEL_AGENTS_KANBAN_BOARD"):
        return
    try:
        from pixel_cli.kanban_db import get_current_board

        os.environ["PIXEL_AGENTS_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.pixel-agents/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Pixel Agents — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Pixel Agents still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``pixel-agents … chat -q`` with stdout
    ...
    try:
        ...
        return False
    if os.environ.get("PIXEL_AGENTS_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'pixel-agents sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from pixel_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from pixel_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'pixel-agents doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Pixel Agents isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  pixel-agents setup")
        print()

        from pixel_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'pixel-agents setup' at any time to configure.")
        sys.exit(1)
    ...
    # user's ~/.pixel-agents/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["PIXEL_AGENTS_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["PIXEL_AGENTS_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["PIXEL_AGENTS_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from pixel_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from pixel_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from pixel_cli.config import get_env_value, save_env_value
    from pixel_constants import find_node_executable, with_pixel_node_path
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Pixel Agents?")
        print()
    ...
    # has no creds.json.  Every subsequent `pixel-agents gateway` then paid a 30s
    ...
    session_dir = get_pixel_agents_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: pixel-agents gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `pixel-agents gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  pixel-agents gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Pixel Agents'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  pixel-agents gateway")
        ...
        print()
        print("  Or install as a service: pixel-agents gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'pixel-agents whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``pixel-agents whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``pixel_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from pixel_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_setup(args):
    ...
    from pixel_cli.setup import run_setup_wizard
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from pixel_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``pixel-agents model``) and the setup wizard
    ...
    from pixel_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("PIXEL_AGENTS_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'pixel-agents model' for "
                "available providers, or run 'pixel-agents doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in pixel_cli/models.py). A multi-member group shows one
    ...
    # Honor ``model_catalog.excluded_providers`` so the CLI ``pixel-agents model``
    ...
    # matching against pixel_id / alias / canonical slug.
    _cli_excluded = {
        str(p).strip().lower()
    ...
    elif selected_provider == "pixel":
        _model_flow_pixel(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.pixel-agents/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.pixel-agents/.env if the active provider is not 'custom'.
    ...
    """
    from pixel_cli.config import get_env_value, save_env_value, load_config
...
#
# Pixel Agents uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `pixel-agents model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `pixel-agents model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`pixel_cli.plugins.PluginContext.register_auxiliary_task`) appear
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from pixel_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Pixel Agents only falls back to a lightweight backend (OpenRouter,")
        print("  Pixel Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    up new ones through the normal ``pixel-agents model`` flow, then route aux tasks
    to them here.
    """
    from pixel_cli.config import load_config
    from pixel_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _aux_flow_provider_model(
    ...
    from pixel_cli.auth import _prompt_model_selection
    from pixel_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from pixel_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from pixel_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Pixel Agents URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from pixel_cli.curses_ui import curses_radiolist
...
# downstream call sites read `pixel_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `pixel-agents` invocation — including
# fast paths like `pixel-agents --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from pixel_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(
    ...
    """Shared API-key entry point for ``pixel-agents setup`` / ``pixel-agents model``.
    ...
    recover from a malformed paste without editing ``~/.pixel-agents/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from pixel_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from pixel_cli.config import save_env_value
    from pixel_cli.secret_prompt import masked_secret_prompt
    ...
    from pixel_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(
            f"  API key cleared.  Re-run `pixel-agents setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from pixel_constants import display_pixel_agents_home as _dhh_fn

            print(
                f"    Pixel Agents will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
    ...
    try:
        ...
        print("    4. Re-run:               pixel-agents model")
        print()
...
def cmd_login(args):
    """Authenticate Pixel Agents CLI with a provider."""
    from pixel_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from pixel_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from pixel_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from pixel_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from pixel_cli.cron import cron_command
...
def cmd_sync(args):
    ...
    if sub in {"enable", "disable"}:
        ...
        if not is_curation_eligible(skill):
            print(
                ...
                f"skills under ~/.pixel-agents/skills/ can sync.",
                file=sys.stderr,
            )
            return 1
    ...
    if sub == "status":
        ...
        if status.get("org_available"):
            ...
            if modified:
                print(
                    ...
                    f"{', '.join(modified)}\n"
                    f"  Share them back with `pixel-agents sync propose <skill>`. "
        ...
        if not status.get("logged_in"):
            print("\nNot logged into Pixel Portal — sync is inert.", file=sys.stderr)
        elif not status.get("pixel_admin"):
            print(
            ...
            print(
                "\nSync feature is off for this instance (set PIXEL_AGENTS_SYNC_ENABLED=1 "
            ...
            print(
                ...
                "PIXEL_AGENTS_SYNC_BASE_URL). Sync is inert.",
                file=sys.stderr,
            )
        return 0
    ...
    try:
        ...
        return 1
    if not identity.get("pixel_admin"):
        print(
    ...
    try:
        if sub == "pull":
            ...
            result = ssc.push_skills(identity=identity, message="pixel-agents sync push")
        elif sub == "now":
            pull_res = ssc.pull_skills(identity=identity)
            push_res = ssc.push_skills(identity=identity, message="pixel-agents sync now")
...
def cmd_webhook(args):
    ...
    from pixel_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``pixel-agents slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: pixel-agents slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `pixel-agents slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from pixel_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from pixel_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from pixel_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from pixel_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from pixel_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `pixel-agents security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from pixel_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    """Dispatch `pixel-agents approvals <subcmd>`."""
    from pixel_cli.approvals_suggest import approvals_command
...
def cmd_dump(args):
    ...
    from pixel_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from pixel_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from pixel_cli.config import config_command
...
def cmd_skin(args):
    ...
    from pixel_cli.skin_cmd import skin_command

    skin_command(args)


def cmd_backup(args):
    """Back up Pixel Agents home directory to a zip file."""
    if getattr(args, "quick", False):
        from pixel_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from pixel_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Pixel Agents backup from a zip file."""
    from pixel_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from pixel_cli.config import detect_install_method
    from pixel_cli.slash_exec import CommandContext, execute_command
    ...
    # Show update status (synchropixel — acceptable since user asked for version info)
    try:
        from pixel_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Pixel Agents (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from pixel_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from pixel_cli.uninstall import run_gui_uninstall
    ...
    from pixel_cli.uninstall import run_uninstall
...
# Update pipeline extracted to pixel_cli/update_cmd.py (main.py decomposition,
# mechanical move). Every moved name is re-exported here so the argparse wiring
# and existing test monkeypatches (pixel_cli.main._cmd_update_impl,
# pixel_cli.main._run_pre_update_backup, ...) keep resolving unchanged.
from pixel_cli.update_cmd import (  # noqa: F401
...
# validated against. Lives next to the checkout (NOT in PIXEL_AGENTS_HOME) because
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    ``pixel-agents update``, a manual ``git pull``, a ZIP update, a file-sync
    ...
    Update-time clears alone can never close this class: ``pixel-agents update``
    ...
    ``pixel-agents`` entry point compares the checkout fingerprint (cheap file
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    mtime comparison. ``git checkout`` / ``git pull`` / ``pixel-agents update``
    ...
    ``pixel_cli/web_dist/`` (per vite.config.ts outDir), NOT ``web/dist/``,
    ...
    dist_dir = project_root / "pixel_cli" / "web_dist"
...
def _web_ui_stamp_path() -> Path:
    """Return the path to the web UI build stamp file under $PIXEL_AGENTS_HOME."""
    from pixel_constants import get_pixel_agents_home
    return get_pixel_agents_home() / "web-ui-build-stamp.json"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``pixel-agents`` launcher present but ``pixel_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the pixel-agents venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — pixel-agents venv python3, no nix-shell overhead
    ...
    # a pixel-agents venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``pixel-agents update`` to stash the
    lockfile — repeatedly.
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    project_root = web_dir.parent.parent if web_dir.parent.name == "apps" else web_dir.parent
    dist_index = project_root / "pixel_cli" / "web_dist" / "index.html"
    try:
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``pixel-agents web``).
    ...
    # build path usable on a stock `py -m pixel_cli.main web` invocation.
    ...
    from pixel_constants import with_pixel_node_path
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_pixel_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (pixel-agents web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``pixel-agents update`` can unconditionally call ``pixel-agents desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``pixel-agents desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $PIXEL_AGENTS_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $PIXEL_AGENTS_HOME."""
    from pixel_constants import get_pixel_agents_home
    return get_pixel_agents_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``pixel-agents update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Pixel Agents.app/Contents/MacOS/Pixel Agents"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Pixel Agents.exe",
            release_dir / "win-ia32-unpacked" / "Pixel Agents.exe",
            release_dir / "win-arm64-unpacked" / "Pixel Agents.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "pixel-agents",
            release_dir / "linux-unpacked" / "Pixel Agents",
            release_dir / "linux-arm64-unpacked" / "pixel-agents",
            release_dir / "linux-arm64-unpacked" / "Pixel Agents",
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # Picking purely by mtime can then hand a wrong-architecture Pixel Agents.exe
...
#
# The desktop self-update chain (Desktop → pixel-agents-setup --update →
# `pixel-agents update` → `pixel-agents desktop --build-only` → relaunch) rebuilds
# Pixel Agents.exe on the end user's machine and used to verify only that the file
...
# wrong-architecture Pixel Agents.exe is caught BEFORE the updater replaces the
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    process arch) and rejected a correctly-built ARM64 ``Pixel Agents.exe``.
...
def _windows_native_machine() -> str:
    ...
    emulation: the desktop update chain runs an x64 pixel-agents-setup.exe (and thus
...
def _ensure_desktop_exe_launchable(
    ...
    print(f"✗ The built Pixel Agents.exe failed its integrity check: {error}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Pixel Agents.exe from backup.")
        print("    Your existing version was kept and still works. Run `pixel-agents desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `pixel-agents desktop --force-build` to rebuild, or re-run the Pixel Agents")
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Pixel Agents'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_pixel_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Pixel Agents.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Pixel Agents.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Pixel Agents.exe: Access is
    ...
    an unrelated "Pixel Agents" process is never touched. Best-effort: never raises.
...
def _desktop_macos_local_codesign(
    ...
    #    contains the outer Pixel Agents.app component, so an absolute-parts check
...
def _desktop_macos_relaunchable_fixup(
    ...
    *,
    ...
    "Pixel Agents is damaged and can't be opened" — and macOS TCC forgets every
    ...
    if exe is None:
        return True
    # exe = .../Pixel Agents.app/Contents/MacOS/Pixel Agents  ->  app bundle = .../Pixel Agents.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``pixel-agents desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Pixel Agents Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Pixel Agents Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the PIXEL_AGENTS_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from pixel_logging import setup_logging as _setup_logging_gui
    ...
    # with_pixel_node_path() copies os.environ when called with no arg.
    env = with_pixel_node_path()
    if getattr(args, "fake_boot", False):
        env["PIXEL_AGENTS_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["PIXEL_AGENTS_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "pixel_root", None):
        env["PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS_ROOT"] = str(Path(args.pixel_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["PIXEL_AGENTS_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["PIXEL_AGENTS_DESKTOP_CWD"] = os.getcwd()
    ...
    # `PIXEL_AGENTS_DESKTOP_DISABLE_GPU=... pixel-agents desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "PIXEL_AGENTS_DESKTOP_DISABLE_GPU" not in os.environ:
        env["PIXEL_AGENTS_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  pixel-agents gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            # Put the Pixel Agents-managed Node on PATH so npm's child scripts (which
            # shell out to bare `node`, e.g. electron-winstaller's
            # select-7z-arch.js) resolve it even when the parent PATH is
            # stripped — the desktop updater chain (Desktop → pixel-agents-setup →
            # pixel-agents update) loses shell PATH customizations. Wrapping the
            # NixOS build env keeps its PYTHON hint while restoring managed Node
            # ahead of a bare PATH (same idiom as the `pixel-agents update` path).
            nixos_env = with_pixel_node_path(_nixos_build_env())
            ...
            if not source_mode:
                ...
                # holds Pixel Agents.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Pixel Agents.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Pixel Agents.exe, close any")
                    print("  running Pixel Agents desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> pixel-agents desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Pixel Agents is
                ...
                # success on a Pixel Agents.exe Windows cannot load (truncated PE from
    ...
    if source_mode:
        print("→ Launching Pixel Agents Desktop from source build...")
    ...
    print(f"→ Launching packaged Pixel Agents Desktop: {' '.join(launch_command)}")
    launch_result = subprocess.run(launch_command, cwd=desktop_dir, env=env, check=False)
    sys.exit(launch_result.returncode)


# Dashboard process-hygiene helpers extracted to pixel_cli/dashboard_procs.py
# (main.py decomposition, mechanical move). Re-exported so callers and test
# monkeypatches on pixel_cli.main.<name> keep resolving unchanged.
from pixel_cli.dashboard_procs import (  # noqa: F401
    _detect_concurrent_pixel_instances,
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    if any(
        ...
        for pattern in (
            ...
            "pixel_cli.main dashboard",
            "pixel_cli/main.py dashboard",
        )
    ):
        mode = "dashboard"
    elif any(
        ...
        for pattern in (
            "pixel-agents serve",
            "pixel_cli.main serve",
            "pixel_cli/main.py serve",
        )
    ):
        mode = "serve"
    if mode is None:
...
_DASHBOARD_SYSTEMD_UNIT = "pixel-agents-dashboard.service"
...
def _restart_managed_dashboard_service(
    ...
    # Probe the user manager first: Pixel Agents installs Linux services in the
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    ...
    ``pixel-agents-serve.service``).  Returns ``None`` when the PID is not
    ...
    try:
        ...
        for line in text.splitlines():
            line = line.strip()
            # Format: 0::/system.slice/pixel-agents-serve.service
...
def _dashboard_cmdline_for_pid(pid: int) -> list[str] | None:
    ...
    is reconstructed, but pixel-agents launch commands don't embed exotic args).
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Best-effort respawn of manually-started dashboards after ``pixel-agents update``.
    ...
    log_path = get_pixel_agents_home() / "logs" / "dashboard-restart.log"
    try:
...
# =========================================================================
# Fork detection and upstream management for `pixel-agents update`
# =========================================================================
...
# Install-scoped breadcrumbs live next to the venv (not under $PIXEL_AGENTS_HOME)
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``pixel-agents update``.
    ...
    protocol stream (``pixel-agents acp`` speaks JSON-RPC on stdout) must never get
...
def _recover_core_update_marker_locked() -> None:
    ...
    """
    print(
        "⚠ A previous `pixel-agents update` was interrupted mid-install — "
        "finishing dependency installation now..."
    )

    # Windows: a normal ``pixel-agents.exe`` launch always has the launcher as an
    ...
    self_locked = _windows_running_pixel_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from pixel-agents.exe; applying package-only first aid, "
    ...
    try:
        from pixel_cli.managed_uv import ensure_uv
        ...
        if self_locked:
            print(
                "  Pixel Agents is still running from the launcher that needs "
                "replacing. Close other Pixel Agents windows, restart from a "
...
def _windows_running_pixel_launcher_locked() -> bool:
    """True when a venv ``pixel-agents*.exe`` shim is this process or an ancestor.
    ...
    if scripts_dir is None:
        return False
    shims = _pixel_exe_shims(scripts_dir)
    if not shims:
        return False
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``pixel-agents update`` is still progressing even if pip/uv itself is silent.
    """
...
def _pixel_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_pixel_exe(
    ...
    when ``pixel-agents update`` runs, ``pixel-agents.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``pixel-agents.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next pixel-agents invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Pixel Agents Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Pixel Agents Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _pixel_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Pixel Agents Desktop, exit other `pixel-agents` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `pixel-agents update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_pixel_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``pixel-agents.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``pixel-agents.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``pixel-agents`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_pixel_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``pixel-agents.exe.old.*`` left by prior updates.

    Called early on every pixel-agents invocation. The .old files are unlocked once
...
def _run_package_only_install(
    ...
    *,
    ...
    rewrite ``pixel-agents.exe``. The editable-install quarantine path would rename
...
def _repair_venv_via_import_probes(
    ...
    *,
    ...
    detected (#57828). Package-only reinstall — never rewrites ``pixel-agents.exe``.
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `pixel-agents update`:")
    print(
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``pixel-agents.exe`` / ``pixel-agents-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_pixel_exe`` for the rationale.
    """
    ...
    # hours later inside ``pixel-agents update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``pixel-agents.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``pixel-agents.exe`` shim is locked during ``pixel-agents update``, or when uv/distlib
    skips a launcher write. The symptom is ``pixel-agents.exe`` and
    ``pixel-agents-acp.exe`` present but ``pixel-agents.exe`` missing, so ``pixel-agents`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `pixel-agents update --force` after "
            "closing other pixel-agents processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m pixel_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``pixel-agents update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``pixel-agents.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``pixel-agents`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `pixel-agents update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `pixel-agents update --force` after closing other pixel-agents processes."
        )
        return
...
def _resolve_node_runtime_npm() -> str | None:
    ...
    """
    from pixel_constants import find_node_executable
...
class _UpdateOutputStream:
    """Stream wrapper used during ``pixel-agents update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.pixel-agents/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``pixel-agents update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``pixel-agents update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.pixel-agents/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``pixel-agents update --gateway``) the update is already
    ...
    try:
        ...
        # pixel_cli.config.get_pixel_agents_home to simulate setup failure.
        from pixel_cli.config import get_pixel_agents_home as _get_pixel_agents_home

        logs_dir = _get_pixel_agents_home() / "logs"
...
def cmd_update(args):
    """Update Pixel Agents to the latest version.
    ...
    if is_managed():
        managed_error("update Pixel Agents")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `pixel-agents update --branch=<x>` would actually pull.
    ...
    from pixel_cli.update_lock import (
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``pixel-agents -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `pixel-agents profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_pixel_agents_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → pixel-agents -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.pixel-agents)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  pixel-agents profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     pixel-agents -p {name} chat")
                else:
        ...
        from pixel_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from pixel_constants import get_pixel_agents_home as _hh
        ...
        from pixel_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `pixel-agents profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → pixel-agents -p {name}  ({wrapper})")
        print()
        ...
        from pixel_cli.profiles import profile_exists, validate_alias_name
        ...
        from pixel_cli.profiles import rename_profile
        ...
        from pixel_cli.profiles import export_profile
        ...
        from pixel_cli.profiles import import_profile
        ...
        import tempfile
        from pixel_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="pixel_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  pixel-agents -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      pixel-agents -p {plan.manifest.name} chat")
        ...
        from pixel_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `pixel-agents profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"pixel-agents -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from pixel_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("pixel_requires"):
            print(f"Requires:     Pixel Agents {data['pixel_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from pixel_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.pixel_requires:
        print(f"  Requires: Pixel Agents {mf.pixel_requires}")
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No pixel-agents dashboard processes running.")
        return 0

    print(f"{len(live)} pixel-agents dashboard process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``pixel-agents dashboard register`` for OAuth.
    ...
    try:
        from pixel_cli.web_server import should_require_auth
    ...
    try:
        from pixel_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Pixel Portal (run `pixel-agents dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    if choice == "2":
        ...
        print(
            ...
            "  It provisions a Pixel Portal OAuth client and writes "
            "PIXEL_AGENTS_DASHBOARD_OAUTH_CLIENT_ID into ~/.pixel-agents/.env for you.\n"
            "  Docs: https://api.pixelagents.com/docs/"
    ...
    try:
        from pixel_cli.config import load_config, save_config
        from pixel_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from pixel_cli.windows_ssh_runtime import read_token
        return read_token(path)

    import stat as _stat
    from pathlib import Path as _Path
    from pixel_constants import get_pixel_agents_home as _get_pixel_agents_home

    if not os.path.isabs(path):
        raise SystemExit("--ssh-session-token-file must be absolute")

    token_path = _Path(path)
    token_root = _get_pixel_agents_home() / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    Packaged Desktop sets ``PIXEL_AGENTS_WEB_DIST`` to ``.../app.asar/dist`` or
    ``.../app.asar.unpacked/dist``. A standalone ``pixel-agents dashboard`` that
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No pixel-agents dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `pixel-agents update`.
    ...
    if _token_file and not _headless_backend:
        raise SystemExit("--ssh-session-token-file is only valid with pixel-agents serve")

    # ── Sanitize Desktop-inherited env that hijacks a standalone launch ─
    # Desktop Electron spawns its backend with PIXEL_AGENTS_DESKTOP=1 plus
    # PIXEL_AGENTS_WEB_DIST=<packaged app.asar[/unpacked]/dist> (and often
    # PIXEL_AGENTS_SERVE_HEADLESS=1 on the serve path). A shell that inherits
    # those vars then runs `pixel-agents dashboard` would otherwise:
    #   - serve the desktop renderer → "Desktop IPC bridge is unavailable"
    #     (issue #52945), or
    #   - disable the SPA via inherited PIXEL_AGENTS_SERVE_HEADLESS.
    # Only strip Electron-packaged WEB_DIST contamination — caller-managed
    # PIXEL_AGENTS_WEB_DIST overrides (dev / custom builds) must still work.
    # The desktop-spawned backend itself (PIXEL_AGENTS_DESKTOP=1) keeps its dist.
    # Intentionally headless `serve` re-sets PIXEL_AGENTS_SERVE_HEADLESS below.
    if os.environ.get("PIXEL_AGENTS_DESKTOP") != "1":
        _inherited_web_dist = os.environ.get("PIXEL_AGENTS_WEB_DIST", "")
        if _is_electron_packaged_web_dist(_inherited_web_dist):
            os.environ.pop("PIXEL_AGENTS_WEB_DIST", None)
    if not _headless_backend:
        os.environ.pop("PIXEL_AGENTS_SERVE_HEADLESS", None)
    ...
    # launches the dashboard (`worker dashboard` → PIXEL_AGENTS_HOME points into
    ...
    if (
        ...
        and os.environ.get("PIXEL_AGENTS_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "pixel_cli.main",
            "-p", "default",
        ...
        from tools.environments.local import build_subprocess_env
        # Exact env preservation: PIXEL_AGENTS_HOME is explicitly pinned to the
        ...
        # PIXEL_AGENTS_HOME.  We must resolve the root explicitly instead of just
        # dropping PIXEL_AGENTS_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV PIXEL_AGENTS_HOME=/opt/data`), so an unset
        # PIXEL_AGENTS_HOME falls back to $HOME/.pixel-agents = /opt/data/.pixel-agents — an
        ...
        # update-button guard also misfires).  get_default_pixel_agents_root()
        # returns the root for both layouts: ~/.pixel-agents for a standard install
        ...
        try:
            from pixel_constants import get_default_pixel_agents_root
            env["PIXEL_AGENTS_HOME"] = str(get_default_pixel_agents_root())
        except Exception:
            # Best-effort: if root resolution fails, fall back to the prior
            # behaviour (drop PIXEL_AGENTS_HOME) rather than block the reroute.
            env.pop("PIXEL_AGENTS_HOME", None)
    ...
    # the same logs directory as every other Pixel Agents surface.
    try:
        from pixel_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if _headless_backend:
        ...
        os.environ["PIXEL_AGENTS_SERVE_HEADLESS"] = "1"
    elif "PIXEL_AGENTS_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["PIXEL_AGENTS_WEB_DIST"])
            if "PIXEL_AGENTS_WEB_DIST" in os.environ
            else PROJECT_ROOT / "pixel_cli" / "web_dist"
        )
        if not (_dist_root / "index.html").exists():
            ...
            # default dist location is recoverable: a custom PIXEL_AGENTS_WEB_DIST
            # points at a caller-managed directory the build cannot populate.
            _recoverable = "PIXEL_AGENTS_WEB_DIST" not in os.environ
            if _recoverable:
        ...
        # PIXEL_AGENTS_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["PIXEL_AGENTS_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ PIXEL_AGENTS_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset PIXEL_AGENTS_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads PIXEL_AGENTS_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["PIXEL_AGENTS_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from PIXEL_AGENTS_WEB_DIST: {_dist_root}")

    # Discover and load plugins so any DashboardAuthProvider plugin
    # (e.g. plugins/dashboard_auth/pixel) registers BEFORE start_server's
    ...
    try:
        from pixel_cli.mcp_startup import start_background_mcp_discovery
    ...
    from pixel_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    """Register a self-hosted dashboard OAuth client with Pixel Portal."""
    from pixel_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from pixel_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from pixel_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from pixel_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Pixel Agents log files."""
    from pixel_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Pixel Agents command console."""
    from pixel_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from pixel_cli.models import CANONICAL_PROVIDERS as _cp
        return ["auto"] + [p.slug for p in _cp]
    except Exception:
        # Fallback: static list guarantees the CLI always works
        return [
            "auto", "openrouter", "pixel", "openai-codex", "xai-oauth", "copilot-acp", "copilot",
...
# so that in ``pixel-agents -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``pixel_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``pixel-agents -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``pixel-agents`` or only flags → defaults to ``chat``.
        return False
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    registers a top-level ``pixel-agents <name>`` command (e.g. Photon ->
    ...
    import it, so the CLI registration never happens and ``pixel-agents photon``
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("PIXEL_AGENTS_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that PIXEL_AGENTS_YOLO_MODE is set before ANY
    ...
    if _run_inline_mcp_discovery:
        try:
            # MCP tool discovery remains synchropixel for entrypoints that do
...
def _apply_safe_mode(args) -> None:
    if not getattr(args, "safe_mode", False):
        return
    os.environ["PIXEL_AGENTS_SAFE_MODE"] = "1"
    os.environ["PIXEL_AGENTS_IGNORE_USER_CONFIG"] = "1"
    os.environ["PIXEL_AGENTS_IGNORE_RULES"] = "1"
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from pixel_cli._parser import build_top_level_parser
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["PIXEL_AGENTS_DEFER_AGENT_STARTUP"] = "1"
            os.environ["PIXEL_AGENTS_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["PIXEL_AGENTS_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `pixel-agents --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        from pixel_constants import get_pixel_agents_home, display_pixel_agents_home
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_pixel_agents_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_pixel_agents_home()}/memories/\n")
    else:
        from pixel_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Pixel Agents as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from pixel_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from pixel_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from pixel_cli.tools_config import tools_command
...
def cmd_monitoring(args):
    ...
    if action == "status":
        ...
        print(f"  OTel SDK:       {'installed' if otlp_exporter.is_available() else 'not installed'} "
              f"(optional extra: pixel-agents[otlp])")
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from pixel_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from pixel_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from pixel_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from pixel_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from pixel_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from pixel_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for pixel-agents CLI."""
    # Cosmetic: make the process show up as 'pixel-agents' instead of 'python3.11'
    ...
    # Sweep stale ``pixel-agents.exe.old.*`` quarantine files left by previous
    # ``pixel-agents update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_pixel_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # If the checkout changed since the last launch (pixel-agents update, manual
    ...
    # Self-heal a venv left half-built by an interrupted ``pixel-agents update``
    ...
    # ``pixel-agents skills install update``) merely defers recovery one launch;
    # under-matching (missing ``pixel-agents -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in pixel_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from pixel_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from pixel_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://api.pixelagents.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `pixel-agents model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.pixel-agents/.env.  Supports Bitwarden "
            "Secrets Manager and 1Password.  See: "
            "https://api.pixelagents.com/docs/user-guide/secrets/"
        ),
    )
    ...
    from pixel_cli import secrets_cli as _secrets_cli
    from pixel_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # `pixel-agents proxy` (defined elsewhere in this file) is a separate INBOUND
    ...
    egress_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://api.pixelagents.com/docs/user-guide/egress/iron-proxy"
        ),
    )

    from pixel_cli import proxy_cli as _proxy_cli
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        ...
        # from the inbound OAuth ``pixel-agents proxy`` subparser (dest='proxy_command').
    ...
    # migrate command
    # =========================================================================
    from pixel_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in pixel_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in pixel_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)


    # =========================================================================
    # whatsapp command  (parser built in pixel_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `pixel-agents whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in pixel_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from pixel_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in pixel_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in pixel_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in pixel_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in pixel_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in pixel_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)
    build_sync_parser(subparsers, cmd_sync=cmd_sync)

    # =========================================================================
    # webhook command  (parser built in pixel_cli/subcommands/webhook.py)
    # =========================================================================
    build_webhook_parser(subparsers, cmd_webhook=cmd_webhook)

    # =========================================================================
    # portal command — Pixel Portal status + Tool Gateway routing
    # =========================================================================
    from pixel_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from pixel_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from pixel_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in pixel_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in pixel_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in pixel_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # approvals command  (parser built in pixel_cli/subcommands/approvals.py)
    # =========================================================================
    build_approvals_parser(subparsers, cmd_approvals=cmd_approvals)

    # =========================================================================
    # dump command  (parser built in pixel_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in pixel_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in pixel_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.pixel-agents/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo pixel-agents uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from pixel_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in pixel_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # import-agent command  (parser: pixel_cli/subcommands/import_agent.py)
    # =========================================================================
    def cmd_import_agent(args):
        from pixel_cli.agent_import import import_agent_command
        import_agent_command(args)

    build_import_agent_parser(subparsers, cmd_import_agent=cmd_import_agent)

    # =========================================================================
    # config command  (parser built in pixel_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # skin command  (parser built in pixel_cli/subcommands/skin.py)
    # =========================================================================
    build_skin_parser(subparsers, cmd_skin=cmd_skin)

    # =========================================================================
    # console command  (parser built in pixel_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in pixel_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in pixel_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from pixel_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in pixel_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``pixel-agents --help``, ``pixel-agents version``, ``pixel-agents logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from pixel_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from pixel_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Pixel Agents shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from pixel_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from pixel_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in pixel_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in pixel_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `pixel-agents computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `pixel-agents tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `pixel-agents computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Pixel Agents app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "install":
            from pixel_cli.tools_config import install_cua_driver
        ...
        if action == "status":
            ...
            if path:
                version = ""
                try:
                    from pixel_cli.tools_config import _cua_driver_env
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: pixel-agents computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: pixel-agents computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: pixel-agents computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: pixel-agents computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: pixel-agents computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: pixel-agents computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in pixel_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def _add_session_filter_args(p, default_older_help):
        ...
        p.add_argument(
            ...
            "(e.g. 'sonnet', 'gpt-5', 'pixel-agents')",
        )
        p.add_argument(
            "--provider",
            help="Only match sessions billed through this provider "
            "(e.g. openrouter, anthropic, pixel)",
        )
        p.add_argument(
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <pixel-agents home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    # cmd_sessions lives in pixel_cli/sessions_cmd.py (main.py decomposition).
    ...
    # =========================================================================
    # insights command  (parser built in pixel_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)
    build_monitoring_parser(subparsers, cmd_monitoring=cmd_monitoring)

    # =========================================================================
    # claw command  (parser built in pixel_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in pixel_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in pixel_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in pixel_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in pixel_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in pixel_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in pixel_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Pixel Agents-Setup.exe success screen tells users to
    # run `pixel-agents desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in pixel_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in pixel_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in pixel_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``pixel-agents -c Pokemon Agent Dev`` → ``pixel-agents -c 'Pokemon Agent Dev'``
    ...
    from pixel_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'pixel-agents -c model' where 'model' is consumed as the
    ...
    # --yolo: set PIXEL_AGENTS_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (pixel-agents hooks list, cron
    ...
    # ``pixel-agents egress start`` refusing when credential_source=bitwarden
