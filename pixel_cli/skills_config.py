"""
Skills configuration for Pixel Agents.
`pixel-agents skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.pixel-agents/config.yaml under:
...
from pixel_cli.config import cfg_get, load_config, save_config
from pixel_cli.colors import Colors, color
from pixel_cli.platforms import PLATFORMS as _PLATFORMS
...
def skills_command(args=None):
    """Entry point for `pixel-agents skills`."""
    from pixel_cli.curses_ui import curses_checklist
