"""Pydantic request/response models for the Pixel Agents dashboard web server.

Extracted verbatim from ``pixel_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from pixel_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``pixel-agents model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `pixel-agents import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`pixel-agents -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the PIXEL_AGENTS_HOME
