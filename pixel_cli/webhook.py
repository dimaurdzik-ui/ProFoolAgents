"""pixel-agents webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.pixel-agents/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _pixel_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
    _dhh = display_pixel_agents_home()
    return f"""
  ...
  Then start the gateway: pixel-agents gateway run
"""
...
def webhook_command(args):
    """Entry point for 'pixel-agents webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: pixel-agents webhook {subscribe|list|remove|test}")
        print("Run 'pixel-agents webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (pixel-agents gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: pixel-agents webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from pixel-agents webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (pixel-agents gateway run)")
