"""Client for uploading ``pixel-agents debug share`` bundles to Pixel-internal S3.

This is the opt-in (``--pixel``) destination for ``pixel-agents debug share``.
Unlike the public paste.rs path, bundles uploaded here go to a Pixel-owned
S3 bucket via a short-lived signed URL minted by the Pixel account service
(NAS).  The bucket auto-expires objects after 14 days, and the contents are
only viewable by Pixel staff (and allowlisted Discord mods) through a
...
# Base URL of the Pixel account service that mints the signed upload URL.
...
NAS_BASE = os.environ.get(
    "PIXEL_AGENTS_DIAGNOSTICS_BASE_URL", "https://portal.pixelagents.com"
)
...
_USER_AGENT = "pixel-agents/debug-share"
...
def share_to_pixel(report_bundle: bytes) -> dict:
    """Orchestrate the full Pixel-S3 upload of a gzipped *report_bundle*.
