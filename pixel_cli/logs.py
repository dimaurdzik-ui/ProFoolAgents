"""``pixel-agents logs`` — view and filter Pixel Agents log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.pixel-agents/logs/``.

Usage examples::

    pixel-agents logs                    # last 50 lines of agent.log
    pixel-agents logs -f                 # follow agent.log in real time
    pixel-agents logs errors             # last 50 lines of errors.log
    pixel-agents logs gateway -n 100    # last 100 lines of gateway.log
    pixel-agents logs gui -f            # follow gui.log (dashboard/pty/ws)
    pixel-agents logs desktop -f        # follow desktop.log (Electron app boot/backend)
    pixel-agents logs --level WARNING    # only WARNING+ lines
    pixel-agents logs --session abc123   # filter by session ID substring
    pixel-agents logs --component tools  # only tool-related lines
    pixel-agents logs --since 1h         # lines from the last hour
    pixel-agents logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_pixel_agents_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Pixel Agents runs — try 'pixel-agents chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_pixel_agents_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_pixel_agents_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_pixel_agents_home()}/logs/")
        return

    print(f"Log files in {display_pixel_agents_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'pixel-agents chat' to generate logs)")
