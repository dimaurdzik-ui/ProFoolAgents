...
Pure display functions with no PixelAgentsCLI state dependency.
"""
import json
import logging
import os
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `pixel-agents ... > file`) raises
...
from pixel_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built pixel-agents — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/PixelResearch/pixel-agents.git"
_OFFICIAL_REPO_CANONICAL = "github.com/pixelagents/pixel-agents"
...
def check_for_updates() -> Optional[int]:
    """Check whether a Pixel Agents update is available.

    Two paths: if ``PIXEL_AGENTS_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = pixel_home / ".update_check"
    embedded_rev = os.environ.get("PIXEL_AGENTS_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # PIXEL_AGENTS_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/pixel-agents/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from pixel_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $PIXEL_AGENTS_HOME/pixel-agents/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = pixel_home / "pixel-agents"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Pixel Agents git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$PIXEL_AGENTS_HOME/pixel-agents/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``pixel_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from pixel_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/PixelResearch/pixel-agents/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Pixel Agents checkout. Cached per-process. Release URL always points at the
    canonical PixelResearch/pixel-agents repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Pixel Agents v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else PIXEL_AGENTS_CADUCEUS
    except Exception:
        _bskin = None
        _hero = PIXEL_AGENTS_CADUCEUS
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Pixel Agents[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Pixel Agents[/]")

    if os.getenv("PIXEL_AGENTS_YOLO_MODE"):
    ...
    try:
        from pixel_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from pixel_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built pixel-agents; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else PIXEL_AGENTS_AGENT_LOGO
        console.print(_logo)
        console.print()
