"""``pixel-agents slack ...`` CLI subcommands.

Today only ``pixel-agents slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ pixel-agents slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Pixel Agents. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Pixel Agents deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Pixel Agents on Slack's older Assistant messaging
    ...
    """
    from pixel_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Pixel Agents in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Pixel Agents in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Pixel Agents agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``pixel_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$PIXEL_AGENTS_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Pixel Agents")
    ...
    """
    name = getattr(args, "name", None) or "Pixel Agents"
    description = getattr(args, "description", None) or "Your Pixel Agents agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "pixel-agents slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"pixel-agents slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "pixel-agents slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "pixel-agents slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Pixel Agents app\n"
