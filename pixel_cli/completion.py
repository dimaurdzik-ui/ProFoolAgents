"""Shell completion script generation for pixel-agents CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_pixel_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Pixel Agents bash completion
# Add to ~/.bashrc:
#   eval "$(pixel-agents completion bash)"

_pixel_profiles() {{
    local profiles_dir="$HOME/.pixel-agents/profiles"
...
_pixel_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_pixel_profiles)" -- "$cur"))
        return
    fi
...
complete -F _pixel_completion pixel-agents
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef pixel-agents
# Pixel Agents zsh completion
# Add to ~/.zshrc:
...
_pixel_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.pixel-agents/profiles" ]]; then
        profiles+=($HOME/.pixel-agents/profiles/*(N/:t))
    fi
...
_pixel() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_pixel_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'pixel-agents command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Pixel Agents fish completion",
        "# Add to your config:",
        "#   pixel-agents completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __pixel_profiles",
        "    echo default",
        "    if test -d $HOME/.pixel-agents/profiles",
        "        for d in $HOME/.pixel-agents/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c pixel-agents -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c pixel-agents -f -s p -l profile"
        " -d 'Profile name' -xa '(__pixel_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__pixel_profiles)' -d 'Profile name'"
                )
