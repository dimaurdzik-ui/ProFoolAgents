"""MCP catalog — curated, Pixel-approved MCP servers shipped with the repo.
...
entries via ``pixel-agents mcp catalog`` or the interactive ``pixel-agents mcp picker``,
and install them with ``pixel-agents mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into pixel-agents. Presence in the
  ``optional-mcps/`` directory = Pixel approval. No community tier, no trust
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``pixel-agents mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.pixel-agents/.env`` (the
...
from pixel_constants import get_pixel_agents_home, get_optional_mcps_dir
...
from pixel_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.pixel-agents/.env.
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Pixel Agents install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Pixel Agents understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Pixel Agents is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Pixel Agents
        understands. Update Pixel Agents to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_pixel_agents_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.pixel-agents/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.pixel-agents/.env via save_env_value()."""
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from pixel_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``pixel-agents mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `pixel-agents mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `pixel-agents mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `pixel-agents mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `pixel-agents mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `pixel-agents auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`pixel-agents auth {entry.auth.provider}` if you have not "
    ...
    from pixel_cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new Pixel Agents session to load its tools.",
        Colors.GREEN,
    ))
