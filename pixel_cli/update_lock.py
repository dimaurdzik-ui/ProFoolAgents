"""Cross-process mutual exclusion for in-flight Pixel Agents updates.

Three different surfaces can start an update of the same install tree:

* ``pixel-agents update`` from a terminal,
* the dashboard's Update button (``POST /api/pixel-agents/update`` →
  ``_spawn_pixel_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``pixel-agents-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``pixel-agents update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <PIXEL_AGENTS_HOME>/.pixel-agents-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``pixel-agents update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Pixel Agents is still running", retry forever).
...
MARKER_NAME = ".pixel-agents-update-in-progress"

# Set by an orchestrating updater (the Tauri `pixel-agents-setup --update` flow) to
# its own pid before spawning `pixel-agents update` as a child stage. The parent
...
HANDOFF_PID_ENV = "PIXEL_AGENTS_UPDATE_HANDOFF_PID"
...
# to show "Pixel Agents is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Pixel Agents home (never the context-local profile override):
    the Rust updater resolves ``$PIXEL_AGENTS_HOME`` or the platform default, and the
    ...
    """
    from pixel_constants import get_process_pixel_agents_home

    return get_process_pixel_agents_home() / MARKER_NAME
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Pixel Agents update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        orchestrating parent (the Tauri updater spawning `pixel-agents update` as a
