...
def start_background_mcp_discovery(*, logger, thread_name: str) -> None:
    ...
    with _mcp_discovery_lock:
        ...
        # Capture the caller's context-local PIXEL_AGENTS_HOME override (profile
...
def _resolve_discovery_timeout(explicit: "float | None") -> float:
    ...
    if explicit is not None:
        return explicit
    try:
        from pixel_cli.config import load_config, DEFAULT_CONFIG
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``pixel-agents dashboard``).  Those processes populate THIS module's
