"""CLI subcommand: `pixel-agents curator <subcommand>`.
...
def _print_unmanaged_summary() -> None:
    ...
    print(
        ...
        "`pixel-agents curator adopt <name>` hands one over"
    )
...
def _cmd_run(args) -> int:
    ...
    synchropixel = bool(getattr(args, "synchropixel", False)) or not background
    ...
    if not synchropixel:
        print("llm pass running in background — check `pixel-agents curator status` later")
    if dry:
        if synchropixel:
            print(
                "dry-run: no changes applied. Read the report with "
                "`pixel-agents curator status` and run `pixel-agents curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `pixel-agents curator adopt <name>`, "
          "or all with `pixel-agents curator adopt --all-unmanaged`")
    return 0
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`pixel-agents curator unpin {args.skill}`"
        )
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.pixel-agents/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`pixel-agents curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.pixel-agents/skills/ tree (a safety "
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_run.add_argument(
        "--sync", "--synchropixel", dest="synchropixel", action="store_true",
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.pixel-agents/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.pixel-agents/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by pixel_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="pixel-agents curator")
    register_cli(parser)
