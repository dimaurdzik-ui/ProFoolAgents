"""Normalized Pixel Portal account entitlement helpers."""
...
PixelAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]

# Free tool-pool coverage categories. Kept byte-for-byte aligned with the
# Portal's TOOL_COVERAGE_CATEGORIES (pixel-account-service
...
_account_info_cache: tuple[str, float, "PixelPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class PixelPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class PixelPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class PixelPortalAccountInfo:
    logged_in: bool
    source: PixelAccountInfoSource
    fresh: bool
    ...
    pixel_client: Optional[str] = None
    ...
    subscription: Optional[PixelPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[PixelPaidServiceAccessInfo] = None
    tool_access: Optional[PixelToolAccessInfo] = None
...
def pixel_portal_billing_url(account_info: Optional[PixelPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized Pixel account snapshot."""
    try:
    ...
    if not isinstance(base, str) or not base.strip():
        base = DEFAULT_PIXEL_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def pixel_portal_topup_url(account_info: Optional[PixelPortalAccountInfo] = None) -> str:
    ...
    """
    base_billing = pixel_portal_billing_url(account_info)  # {base}/billing
...
def format_pixel_portal_entitlement_message(
    account_info: Optional[PixelPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing Pixel tool-gateway entitlement.
    ...
    """
    billing_url = pixel_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Pixel Portal "
    ...
    if account_info is None:
        return (
            f"Pixel Agents could not verify your Pixel Portal entitlement, so {capability} "
            f"is unavailable. Run `pixel-agents model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Pixel inference credentials are configured, but Pixel Agents cannot verify "
                f"your Pixel Portal paid access for {capability}. Log in with "
                f"`pixel-agents model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Pixel Portal to use {capability}: run `pixel-agents model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Pixel Agents could not verify your Pixel Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `pixel-agents model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Pixel Agents could not find a Pixel Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `pixel-agents model` to "
            f"authenticate again; if the problem persists, contact Pixel support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `pixel-agents model` to refresh Pixel Agents."
        return message

    return (
        f"Your Pixel Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def _no_paid_access_message(
    account_info: PixelPortalAccountInfo,
    capability: str,
    billing_url: str,
) -> str:
    access = account_info.paid_service_access_info
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your Pixel Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Pixel Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Pixel Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Pixel Portal account has no usable paid credits{credit_detail}, so "
...
def _fresh_account_info(
    *,
    ...
    try:
        from pixel_cli.auth import get_provider_auth_state, resolve_pixel_access_token

        access_token = resolve_pixel_access_token()
        ...
        payload = _fetch_pixel_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[PixelPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque Pixel keys."""
    try:
        entry = _select_pixel_pool_entry()
        if entry is None:
            return None
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_pixel_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _select_pixel_pool_entry() -> Optional[Any]:
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from pixel_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = PixelPaidServiceAccessInfo(
        allowed=paid_access,
    ...
    return PixelPortalAccountInfo(
        ...
        pixel_client=_coerce_str(claims.get("pixel_client")),
        portal_base_url=portal_base_url,
...
def _tool_access_from_value(value: Any) -> Optional[PixelToolAccessInfo]:
    """Parse a Portal ``tool_access`` object (from the JWT claim or the account
    API) into :class:`PixelToolAccessInfo`. Fails closed: a non-object value
    ...
    return PixelToolAccessInfo(enabled=enabled, coverage=coverage)


def _subscription_from_payload(value: Any) -> Optional[PixelPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return PixelPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[PixelPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return PixelPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
