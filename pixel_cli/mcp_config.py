"""
MCP Server Management CLI — ``pixel-agents mcp`` subcommand.

Implements ``pixel-agents mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.pixel-agents/config.yaml under the ``mcp_servers`` key.
"""
...
from pixel_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_pixel_agents_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from pixel_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.pixel-agents/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``pixel-agents mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import PixelAgentsTokenStorage
        return PixelAgentsTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # pixel_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  pixel-agents mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  pixel-agents mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  pixel-agents mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_pixel_agents_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: pixel-agents mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_pixel_agents_home()}/config.yaml ({tool_count}/{total} tools enabled)")
        _info("Start a new session to use these tools.")


# ─── pixel-agents mcp remove ───────────────────────────────────────────────────────
...
def cmd_mcp_remove(args):
    ...
    # earlier `pixel-agents mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  pixel-agents mcp add <name> --url <endpoint>')
        _info('  pixel-agents mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``pixel-agents mcp login`` and
    ``pixel-agents mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `pixel-agents mcp remove` + `pixel-agents mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `pixel-agents mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Pixel Agents desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `pixel-agents mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``pixel-agents mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``pixel-agents mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: pixel-agents mcp reauth <name>   |   pixel-agents mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
# ─── pixel-agents mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'pixel-agents mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``pixel-agents mcp`` subcommands."""
    ...
    if action == "picker":
        from pixel_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from pixel_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from pixel_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `pixel-agents plugin`.
        from pixel_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("pixel-agents mcp                                    Open the catalog picker (default)")
        _info("pixel-agents mcp catalog                            List Pixel-approved MCPs")
        _info("pixel-agents mcp install <name>                     Install a catalog MCP")
        _info("pixel-agents mcp serve                              Run as MCP server")
        _info("pixel-agents mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("pixel-agents mcp add <name> --command <cmd>         Add a stdio server")
        _info("pixel-agents mcp add <name> --preset <preset>       Add from a known preset")
        _info("pixel-agents mcp remove <name>                      Remove a server")
        _info("pixel-agents mcp list                               List configured servers")
        _info("pixel-agents mcp test <name>                        Test connection")
        _info("pixel-agents mcp configure <name>                   Toggle tools")
        _info("pixel-agents mcp login <name>                       Re-authenticate OAuth")
        _info("pixel-agents mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
