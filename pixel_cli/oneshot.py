...
configured for "cli" in `pixel-agents tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (PIXEL_AGENTS_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `pixel-agents chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"pixel-agents -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "pixel-agents -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"pixel-agents -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "pixel-agents -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "pixel-agents -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to PIXEL_AGENTS_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("PIXEL_AGENTS_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "pixel-agents -z: --provider requires --model (or PIXEL_AGENTS_INFERENCE_MODEL). "
    ...
    # to its inline/synchropixel path. See declare_stateless_channel().
    ...
    if failure is not None:
        ...
        real_stderr.write(f"pixel-agents -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("pixel-agents -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``pixel-agents -z`` / oneshot mode.

    Oneshot bypasses ``PixelAgentsCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when pixel-agents is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from pixel_cli.config import load_config
    from pixel_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("PIXEL_AGENTS_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from pixel_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("PIXEL_AGENTS_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                PIXEL_AGENTS_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via PIXEL_AGENTS_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via PIXEL_AGENTS_YOLO_MODE=1
