"""
Interactive setup wizard for Pixel Agents.
...
Config files are stored in ~/.pixel-agents/ for easy access.
"""
...
from tools.tool_backend_helpers import managed_pixel_tools_enabled
...
_DOCS_BASE = "https://api.pixelagents.com/docs"
...
# display_pixel_agents_home imported lazily at call sites (stale-module safety during pixel-agents update)
...
from pixel_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from pixel_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Pixel Agents Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Pixel Agents using environment variables or config commands:")
    print_info("  pixel-agents config set model.provider custom")
    print_info("  pixel-agents config set model.base_url http://localhost:8080/v1")
    print_info("  pixel-agents config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'pixel-agents setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``PIXEL_AGENTS_NONINTERACTIVE=1`` (see ``pixel_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("PIXEL_AGENTS_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``PIXEL_AGENTS_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from pixel_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'pixel-agents setup')")


def _print_setup_summary(config: dict, pixel_home):
    ...
    tool_status = []
    subscription_features = get_pixel_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'pixel-agents setup' to configure"))


    # Web tools (Exa, Parallel, Firecrawl, or Tavily)
    if subscription_features.web.managed_by_pixel:
        tool_status.append(("Web Search & Extract (Pixel subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_pixel:
        tool_status.append(("Browser Automation (Pixel Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via Pixel), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_pixel:
        tool_status.append(("Image Generation (Pixel subscription)", True, None))
        ...
        try:
            ...
            from pixel_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `pixel-agents tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_pixel:
        tool_status.append(("Video Generation (FAL via Pixel subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from pixel_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_pixel:
        tool_status.append(("Text-to-Speech (OpenAI via Pixel subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'pixel-agents setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'pixel-agents setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_pixel:
        tool_status.append(("Speech-to-Text (OpenAI via Pixel subscription)", True, None))
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'pixel-agents tools' → Speech-to-Text")
            )

    if subscription_features.modal.managed_by_pixel:
        tool_status.append(("Modal Execution (Pixel subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'pixel-agents setup terminal'"))
    elif managed_pixel_tools_enabled() and subscription_features.pixel_auth_present:
        tool_status.append(("Modal Execution (optional via Pixel subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via pixel-agents auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'pixel-agents setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {pixel_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('pixel-agents setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('pixel-agents setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('pixel-agents setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('pixel-agents setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('pixel-agents setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('pixel-agents config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('pixel-agents config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('pixel-agents config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('pixel-agents', Colors.GREEN)}              Start chatting")
    print(f"   {color('pixel-agents gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('pixel-agents doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `pixel-agents tools` and `pixel-agents setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``pixel-agents model``)
    ...
    provider added to ``pixel-agents model`` is automatically available here.
    ...
    # Delegate to the shared pixel-agents model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from pixel_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: pixel-agents model")
    ...
    # on demand via `pixel-agents auth add`, `pixel-agents setup` vision, and
    # `pixel-agents setup tts`. This keeps both quick and full setup thin.


    # Tool Gateway prompt is already shown by _model_flow_pixel() above.
    save_config(config)
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``pixel-agents model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from pixel_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_pixel_tools_enabled() and subscription_features.pixel_auth_present:
        choices.append("Pixel Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("pixel-openai")
    choices.extend(
        [
    ...
    selected_via_pixel = selected == "pixel-openai"
    if selected == "pixel-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Pixel gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.pixel-agents/.env."
            )
    ...
    if selected == "neutts":
    ...
    elif selected == "openai" and not selected_via_pixel:
        ...
        # via the Pixel Agents auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via pixel-agents setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'pixel-agents setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Pixel Agents runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `pixel-agents setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `pixel-agents setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `pixel-agents egress setup` then `pixel-agents egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `pixel-agents egress setup`."
            )
        ...
        managed_modal_available = bool(
            managed_pixel_tools_enabled()
            and
            get_pixel_subscription_features(config).pixel_auth_present
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Pixel gateway and bill to your subscription.")
        ...
        print_info("Requires the optional SDK: pip install 'pixel-agents[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'pixel-agents[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into PIXEL_AGENTS_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("PIXEL_AGENTS_MAX_ITERATIONS")
    ...
    print_info("  Run `pixel-agents setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # PIXEL_AGENTS_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.pixel-agents/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from pixel_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_pixel_home(Path(get_pixel_agents_home()))
    except Exception:
        pass
...
def _profile_name_from_pixel_home(pixel_home) -> str | None:
    """Return the active profile name when PIXEL_AGENTS_HOME is a profile dir."""
    if pixel_home.parent.name == "profiles":
        return pixel_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Pixel Agents delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Pixel Agents to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from pixel_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://api.pixelagents.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://api.pixelagents.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  pixel-agents config edit")
    print_info("   Open config in your editor:  pixel-agents config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from pixel_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Pixel Agents from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'pixel-agents setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     pixel-agents config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        from pixel_cli.gateway import (
            _is_service_installed,
            _is_service_running,
            supports_systemd_services,
            has_conflicting_systemd_units,
            has_legacy_pixel_units,
        ...
        if supports_systemd and has_legacy_pixel_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: pixel-agents gateway install --system")
                print_info("  Or run in foreground:  pixel-agents gateway")
        else:
            from pixel_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   pixel-agents gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   pixel-agents gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `pixel-agents setup tools` and `pixel-agents tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Pixel Agents profile and are not uploaded.")
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from pixel_cli.config
    ...
    if section_key == "model":
        ...
        from pixel_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_pixel.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_pixel migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_pixel", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Pixel Agents.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Pixel Agents to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Pixel Agents at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Pixel Agents at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Pixel Agents at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Pixel Agents at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Pixel Agents equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Pixel Agents config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Pixel Agents.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Pixel Agents's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(pixel_home: Path) -> bool:
    ...
    print_info("Pixel Agents can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: pixel-agents claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=pixel_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: pixel-agents claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Pixel Agents configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Pixel Agents config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Pixel Agents (use pixel-agents claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Pixel Portal setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``pixel-agents setup --portal`` and ``pixel-agents portal``. This is the
    Pixel-Portal slice of the first-time quick setup, collapsed into a single
    shareable command so a brand-new user goes from zero to a fully working
    Pixel Agents session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``pixel-agents setup`` and hunt for the quick-setup option.

    The login + model selection + provider switch + Tool Gateway opt-in are all
    delegated to ``_model_flow_pixel`` — the exact same flow quick setup uses
    (``_run_first_time_quick_setup``) and the same one ``pixel-agents model`` runs
    when you pick Pixel. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``pixel-agents portal`` always offers a model picker,
    and there is a single source of truth for the Pixel onboarding steps.
    """
    ...
    print(color("│     ⚕ Pixel Agents Setup — Pixel Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your Pixel Portal sub.")
    print()
    print_info("  Sign up: https://portal.pixelagents.com/manage-subscription")
    print()

    # _model_flow_pixel handles BOTH the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Pixel model picker), then offers the Tool Gateway opt-in and sets
    # provider=pixel via the login/model save. This is the same routine quick
    # setup calls, so `pixel-agents portal` == quick setup's Pixel step.
    try:
        from pixel_cli.main import _model_flow_pixel

        _model_flow_pixel(config)
    except (KeyboardInterrupt, EOFError, SystemExit):
        # _login_pixel raises SystemExit(130)/(1) on cancel/failure; the
        # logged-out path inside _model_flow_pixel catches it, but the
        ...
        print_info("  You can retry later with `pixel-agents portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_pixel error during `pixel-agents portal`: %s", exc)
        print()
        print_error(f"  Pixel Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `pixel-agents portal`.")
        return

    # Re-sync the in-memory config from disk — _model_flow_pixel (and the
    ...
    print_info("  Run `pixel-agents portal info` to inspect routing.")
    print_info("  Run `pixel-agents` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      pixel-agents setup           — full or quick (auto-detected)
      pixel-agents setup model     — just model/provider
      pixel-agents setup tts       — just text-to-speech
      pixel-agents setup terminal  — just terminal backend
      pixel-agents setup gateway   — just messaging platforms
      pixel-agents setup tools     — just tool configuration
      pixel-agents setup telemetry — just local shared metrics
      pixel-agents setup agent     — just agent settings
    """
    from pixel_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot Pixel Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Pixel Agents Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Pixel Agents Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Pixel Agents installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, pixel_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Pixel Agents configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'pixel-agents setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(pixel_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Pixel Agents?",
            [
                "Quick Setup (Pixel Portal) — free OAuth login, no API keys, model + tools (recommended)",
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, pixel_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, pixel_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {pixel_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'pixel-agents config edit'")
    ...
    # Tune later with `pixel-agents setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, pixel_home)


def _run_first_time_quick_setup(config: dict, pixel_home, is_existing: bool):
    """Streamlined first-time setup via Pixel Portal: OAuth, model, terminal & messaging.

    Routes straight to the Pixel Portal provider — runs the device-code OAuth
    login, picks a Pixel model, then configures the terminal backend and (optionally)
    a messaging platform. Applies sensible defaults for everything else (agent
    settings, tools); the user can customize later via ``pixel-agents setup <section>``
    or switch providers with ``pixel-agents model``.
    """
    from pixel_cli.config import load_config

    # Step 1: Pixel Portal — OAuth login + model selection.
    # _model_flow_pixel() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Pixel model picker). Provider is set to "pixel" by the login/model save.
    print()
    print_header("Pixel Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.pixelagents.com/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("Pixel Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_pixel error during quick setup: %s", exc)
        print_warning(f"Pixel Portal setup encountered an error: {exc}")
        print_info("You can try again later with: pixel-agents model")

    # Re-sync the wizard's config dict from disk — _model_flow_pixel (and the
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'pixel-agents setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    pixel-agents setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  pixel-agents setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``pixel-agents tools`` (which
    ...
    try:
        ...
        from pixel_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``pixel-agents setup agent`` /
    ``pixel-agents config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, pixel_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `pixel-agents update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        pixel-agents tools")
        print_info("  Seed skills:         pixel-agents skills opt-in --sync")
        print_info("  Add MCP servers:     pixel-agents mcp add")
        print_info("  Enable plugins:      pixel-agents plugins")
        print_info("  Tune agent settings: pixel-agents setup agent")
        print()
        _print_setup_summary(config, pixel_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, pixel_home)


def _blank_slate_walkthrough(config: dict, pixel_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`pixel-agents update` runs from re-injecting them. Opt back in any")
            print_info("time with `pixel-agents skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `pixel-agents tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `pixel-agents plugins list` / `pixel-agents plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `pixel-agents plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `pixel-agents mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `pixel-agents mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   pixel-agents tools")
    print_info("  Seed skills:         pixel-agents skills opt-in --sync")
    print_info("  Add MCP servers:     pixel-agents mcp add")
    print_info("  Tune agent settings: pixel-agents setup agent")
    print()
...
def _run_quick_setup(config: dict, pixel_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'pixel-agents setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Pixel Agents to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'pixel-agents setup gateway'.")
