"""Profile distributions — shareable, packaged Pixel Agents profiles via git.

A distribution is a Pixel Agents profile published as a git repository (or
...
* ``pixel-agents profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``pixel-agents skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``pixel-agents profile``, not a parallel tree):

    pixel-agents profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "pixel-agents", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            pixel_requires=str(data.get("pixel_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.pixel_requires:
            out["pixel_requires"] = self.pixel_requires
        if self.author:
...
def check_pixel_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Pixel Agents {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``pixel-agents profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Pixel Agents profile distribution."
            )
...
def plan_install(
    ...
    from pixel_cli import __version__ as pixel_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Pixel Agents distribution."
        )

    # Version check up-front so we fail fast
    check_pixel_requires(manifest.pixel_requires, pixel_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.pixel-agents).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="pixel_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `pixel-agents profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`pixel-agents profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="pixel_dist_update_") as tmp:
        plan = plan_install(
