...
``pixel_cli.config.migrate_config``. Each step is a function
...
``pixel_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``pixel_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``pixel_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``pixel_cli.config`` (e.g.
``patch("pixel_cli.config.read_raw_config", ...)``) keep working, because
...
#: PIXEL_AGENTS_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_pixel_agents_home()}/config.yaml and run `pixel-agents setup` to "
...
def _cfg():
    """Return the live ``pixel_cli.config`` module (lazy, cycle-free)."""
    from pixel_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_pixel_agents_home = _c.get_pixel_agents_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$PIXEL_AGENTS_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `pixel-agents plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `pixel-agents curator status` has no
    ...
    #   3. Creates `~/.pixel-agents/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_pixel_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_pixel_agents_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `pixel-agents config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `pixel-agents config set`"
                )
...
# behavior set it to true explicitly via `pixel-agents config set`.
