"""
Security advisory checker for Pixel Agents.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``pixel-agents doctor --ack <id>``; the ack is persisted to
...
1. ``pixel-agents doctor`` (and ``pixel-agents doctor --ack <id>``)
...
run in environments where the rest of Pixel Agents failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including pixel-agents when configured "
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.pixel-agents/.env (OpenRouter, Anthropic, OpenAI, "
            "Pixel, GitHub, AWS, Google, Mistral, etc.).",
            ...
            "After cleanup: pixel-agents doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Pixel Agents requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'pixel-agents doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.pixel-agents/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_pixel_agents_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``pixel-agents doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `pixel-agents doctor` on the gateway host for details.")
