"""
Status command for pixel-agents CLI.

Shows the status of all Pixel Agents components.
"""
...
from pixel_cli.auth import AuthError, resolve_provider
from pixel_cli.colors import Colors, color
from pixel_cli.config import get_env_path, get_env_value, get_pixel_agents_home, load_config
from pixel_cli.models import provider_label
...
from pixel_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``pixel-agents config``'s
...
def show_status(args):
    """Show status of all Pixel Agents components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Pixel Agents Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `pixel-agents status`
    ...
    pixel_account_info = None
    if (
        pixel_status.get("logged_in")
        or pixel_status.get("access_token")
        or pixel_status.get("portal_base_url")
        or pixel_status.get("inference_credential_present")
        or pixel_status.get("error_code")
    ):
        try:
            pixel_account_info = get_pixel_portal_account_info()
        except Exception:
            pixel_account_info = None

    pixel_logged_in = bool(
        pixel_status.get("logged_in")
        or (pixel_account_info and pixel_account_info.logged_in)
    )
    pixel_inference_present = bool(
        pixel_status.get("inference_credential_present")
        or (pixel_account_info and pixel_account_info.inference_credential_present)
    )
    pixel_error = pixel_status.get("error")
    if pixel_logged_in:
        pixel_label = "logged in"
    elif pixel_inference_present:
        pixel_label = "not logged in (Pixel inference key configured)"
    else:
        pixel_label = "not logged in (run: pixel-agents portal)"
    print(
        f"  {'Pixel Portal':<12}  {check_mark(pixel_logged_in)} "
        f"{pixel_label}"
    )
    portal_url = pixel_status.get("portal_base_url") or "(unknown)"
    inference_url = (
        pixel_status.get("inference_base_url")
        or (pixel_account_info.inference_base_url if pixel_account_info else None)
    )
    access_exp = _format_iso_timestamp(pixel_status.get("access_expires_at"))
    key_exp = _format_iso_timestamp(pixel_status.get("agent_key_expires_at"))
    refresh_label = "yes" if pixel_status.get("has_refresh_token") else "no"
    if pixel_logged_in or portal_url != "(unknown)" or pixel_error:
        print(f"    Portal URL: {portal_url}")
    if pixel_inference_present and inference_url:
        print(f"    Inference:  {inference_url}")
    if pixel_logged_in or pixel_status.get("access_expires_at"):
        print(f"    Access exp: {access_exp}")
    if pixel_logged_in or pixel_inference_present or pixel_status.get("agent_key_expires_at"):
        print(f"    Key exp:    {key_exp}")
    if pixel_logged_in or pixel_status.get("has_refresh_token"):
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: pixel-agents model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: pixel-agents auth add minimax-oauth)'}"
    )
    ...
    # disrupt the already-printed Pixel/Codex/Qwen/MiniMax rows above.
    try:
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: pixel-agents auth add xai-oauth)'}"
    )
    ...
    # =========================================================================
    # Pixel Subscription Features
    # =========================================================================
    if managed_pixel_tools_enabled():
        features = get_pixel_subscription_features(config)
        print()
        print(color("◆ Pixel Tool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.pixel_auth_present:
            print("  Pixel Portal   ✗ not logged in")
        else:
            print("  Pixel Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_pixel:
                state = "active via Pixel subscription"
            elif feature.active:
                current = feature.current_provider or "configured provider"
                state = f"active via {current}"
            elif feature.included_by_default and features.pixel_auth_present:
                state = "included by subscription, not currently selected"
            elif feature.key == "modal" and features.pixel_auth_present:
                state = "available via subscription (optional)"
            else:
                state = "not configured"
            print(f"  {feature.label:<15} {check_mark(feature.available or feature.active or feature.managed_by_pixel)} {state}")
    elif pixel_logged_in or pixel_inference_present:
        # Pixel OAuth without entitlement, or an opaque inference key without
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: pixel-agents model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from pixel_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'pixel-agents[vercel]')"
    ...
    try:
        from pixel_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   pixel-agents gateway")
    ...
    jobs_file = get_pixel_agents_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    if _session_count is not None and _session_count > 0:
        ...
        sessions_file = get_pixel_agents_home() / "sessions" / "sessions.json"
    ...
    try:
        from pixel_cli.active_sessions import (
    ...
    print(color("  Run 'pixel-agents doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'pixel-agents setup' to configure", Colors.DIM))
    print()
