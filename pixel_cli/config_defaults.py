"""Default configuration data for Pixel Agents.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from pixel_cli/config.py. Must not import from pixel_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["pixel-agents-cli"],
    # SQLite journal mode used by every Pixel Agents database opener. WAL is the
    ...
    "agent": {
        ...
        # the Pixel Agents-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Pixel Agents
        ...
        # identity slot (SOUL.md). Empty by default. The PIXEL_AGENTS_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Pixel Agents adds a coding operating brief
        ...
        # blocks indefinitely (input() is synchropixel) and ignores this.
        ...
        # synchropixelly before the gate runs.  Set to 0 to disable the bound
        ...
        # ``PIXEL_AGENTS_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             PIXEL_AGENTS_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force PIXEL_AGENTS_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Pixel Agents auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Pixel Agents sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Pixel Agents
        ...
        # bundled Pixel Agents image, which drops to the `pixel-agents` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Pixel Agents sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Pixel Agents should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``pixel-agents chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.pixel-agents/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: pixel-agents sweeps the checkpoint base at startup
        ...
        # ``pixel-agents checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .pixel-agents.md, or .cursorrules before Pixel Agents applies
    ...
    # single read_file page exceeds these limits, Pixel Agents truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Pixel Agents' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); pixel-agents =
                                      # Pixel Agents' compression threshold triggers
    ...
    "auxiliary": {
        ...
        "vision": {
            "provider": "auto",    # auto | openrouter | pixel | codex | custom
        ...
        # Invoked by ``pixel-agents kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``pixel-agents kanban decompose`` and the kanban
        ...
        # ``pixel-agents profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `pixel-agents model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `pixel-agents` (and `pixel-agents chat`) launches by default:
        ...
        # REPL and `--tui` (or PIXEL_AGENTS_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `pixel-agents --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `pixel-agents -c` muscle memory.  Default off so existing
        # users aren't surprised.  PIXEL_AGENTS_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `pixel-agents --tui` drops a one-time hint
        ...
        # Pixel credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # model — see pixel_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to PIXEL_AGENTS_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/pixel-agents`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `pixel-agents pets`. Disabled until
        ...
        "pet": {
            ...
            # get_pixel_agents_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled Pixel Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``PIXEL_AGENTS_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``PIXEL_AGENTS_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.pixelagents.com).
        "oauth": {
        ...
        # (``PIXEL_AGENTS_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # pixel-account-service at deploy time via the
        # ``PIXEL_AGENTS_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``PIXEL_AGENTS_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/pixel-agents``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Pixel Agents appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.pixel-agents/cache/piper-voices/
    ...
    # "Hey Pixel Agents" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey pixel-agents",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_pixel" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.pixel-agents/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, Pixel, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.pixel-agents/cache/delegation/
        ...
        "max_concurrent_children": 3,  # unified concurrency cap: max parallel children per batch
                                       ...
                                       # fall back to synchropixel execution. Floor of 1, no ceiling.
    ...
    # response. If not, Pixel Agents feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Pixel Agents auto-pauses the goal and
    ...
    "moa": {
        ...
        # <pixel_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.pixel-agents/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${PIXEL_AGENTS_SKILL_DIR} and ${PIXEL_AGENTS_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `pixel-agents curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `pixel-agents curator run --consolidate` overrides
        ...
        # normally restored on every `pixel-agents update`, so pruning them only
        ...
        # skipped), snapshot ~/.pixel-agents/skills/ into
        # ~/.pixel-agents/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `pixel-agents curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for pixel-agents-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Pixel Agents*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (PIXEL_AGENTS_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Pixel Agents' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.pixel-agents/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or PIXEL_AGENTS_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `pixel-agents doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``pixel_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Pixel Agents to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $PIXEL_AGENTS_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing Pixel Portal token — there is no token key here.
        "chronos": {
            ...
            "portal_url": "https://portal.pixelagents.com",
        ...
        # Also overridable via PIXEL_AGENTS_CRON_MAX_PARALLEL env var.
        ...
        # SessionDB opens/migrates state.db synchropixelly and has no timeout
        ...
        # PIXEL_AGENTS_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `pixel-agents -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `pixel-agents` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `pixel-agents kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     pixel-agents's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Pixel Agents tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.pixel-agents/logs/.
    ...
    # curated model lists for OpenRouter and Pixel Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a pixel-agents release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://api.pixelagents.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `pixel-agents model` invocation; network failures
    ...
    "monitoring": {
        ...
        "gateway_health_export": {
            ...
            "resource_attributes": {
                "service.name": "pixel-agents-gateway",
    ...
    "gateway": {
        ...
        # internal PIXEL_AGENTS_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.pixel-agents/sessions/sessions.json entirely.
        ...
        # the PIXEL_AGENTS_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.pixel-agents.gateway` that defenses 1-2 don't
        ...
        # ``PIXEL_AGENTS_GATEWAY_MAX_STARTS`` / ``PIXEL_AGENTS_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.pixel-agents/.env,
        ...
        # behavior: files must live under the Pixel Agents cache, under
        ...
        # user. Bridged to PIXEL_AGENTS_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Pixel Agents
        # cache (~/.pixel-agents/cache/{documents,images,audio,video,screenshots})
        ...
        # to PIXEL_AGENTS_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # PIXEL_AGENTS_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to PIXEL_AGENTS_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.pixel-agents/state.db.
    ...
    "sessions": {
        ...
        # the default of ``pixel-agents sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.pixel-agents/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `pixel-agents sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `pixel-agents update` prints a one-line notice with
        ...
        # ~/.pixel-agents/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to PIXEL_AGENTS_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # PIXEL_AGENTS_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <PIXEL_AGENTS_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``pixel-agents backup``-style zip
        #     of PIXEL_AGENTS_HOME into <PIXEL_AGENTS_HOME>/backups/, restorable with
        #     ``pixel-agents import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `pixel-agents update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<PIXEL_AGENTS_HOME>/lsp/bin/`` on first use.
    ...
    # `pixel-agents tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.pixel-agents/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.pixel-agents/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.pixel-agents/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Pixel Agents may use this encrypted
            ...
            # ~/.pixel-agents/bin/ on first use.  When False you must install
            ...
            # `pixel-agents secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.pixel-agents/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Pixel Agents disables it for our users unless they
        # explicitly opt in here. When false (default), Pixel Agents sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Pixel Agents also calls
    ...
    #
    # Configure with `pixel-agents egress setup`.  Disabled by default — the rest of
    # Pixel Agents works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.pixel-agents/bin/ on
        ...
        # Together, DeepSeek, Pixel).  Wildcards (`*.foo.com`) are supported.
        "extra_allowed_hosts": [],
    },

    # Pixel Agents Desktop (Electron app) launch options. These only affect
    # `pixel-agents desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the PIXEL_AGENTS_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    "PIXEL_BASE_URL": {
        "description": "Pixel Portal base URL override",
        "prompt": "Pixel Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Pixel Agents mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'pixel-agents model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Pixel Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Pixel Subscribers only, used to derive vendor hosts, e.g. pixelagents.com -> firecrawl-gateway.pixelagents.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Pixel Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Pixel Subscriber access token for tool-gateway requests (optional; otherwise read from the Pixel Agents auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Pixel Agents' wake word engine (optional; openWakeWord is the free default)",
    ...
    "PIXEL_AGENTS_LANGFUSE_PUBLIC_KEY": {
    ...
    "PIXEL_AGENTS_LANGFUSE_SECRET_KEY": {
    ...
    "PIXEL_AGENTS_LANGFUSE_BASE_URL": {
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Pixel Agents, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @pixel-agents:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. PIXEL_AGENTS_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #pixel-agents)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: pixel-agents-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'pixel-agents' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Pixel Agents API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Pixel Agents API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Pixel Agents API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # PIXEL_AGENTS_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to PIXEL_AGENTS_TOOL_PROGRESS_MODE for backward
    ...
    # PIXEL_AGENTS_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "PIXEL_AGENTS_PREFILL_MESSAGES_FILE": {
    ...
    "PIXEL_AGENTS_EPHEMERAL_SYSTEM_PROMPT": {
