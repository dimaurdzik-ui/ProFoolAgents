...
Pure logic: walks a Pixel Agents config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `pixel-agents doctor` and a future `pixel-agents migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Pixel Agents config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Pixel Agents config and return retirement issues.
...
def apply_migration(
    ...
    from pixel_cli.config import require_readable_config_before_write
