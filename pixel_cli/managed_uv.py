"""Pixel Agents-managed uv and Python runtime repair.

Pixel Agents owns its own uv binary at ``$PIXEL_AGENTS_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$PIXEL_AGENTS_HOME/bin`` so the installer writes directly there — no PATH
probing, no conda guards, no multi-location resolution chains.

The Python backing the install is different: it is shared by every Pixel Agents
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.pixel-agents-runtime/python``. A
...
synchropixel rollback and is parked for cleanup after the updating process
releases it.
"""
...
from pixel_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".pixel-agents-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
...
def managed_uv_path() -> Path:
    """Return the path where Pixel Agents keeps *its* uv binary.

    ``$PIXEL_AGENTS_HOME/bin/uv`` on POSIX, ``$PIXEL_AGENTS_HOME\\bin\\uv.exe`` on
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Pixel Agents-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Pixel Agents keeps databases "
            "out of WAL mode on this SQLite build. The next `pixel-agents update` "
            "will retry."
        )
        return
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``pixel-agents update`` runs
    the call site from the *old*, already-imported ``pixel_cli.main`` against
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``pixel-agents update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under PIXEL_AGENTS_HOME
    caches the last successful self-update time.
    """
    try:
        from pixel_constants import get_pixel_agents_home

        stamp = get_pixel_agents_home() / "cache" / ".uv_self_update_stamp"
...
def update_managed_uv(
    *,
    ...
    Call this during ``pixel-agents update`` so the managed copy stays current.
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Pixel Agents generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("pixel_cli.main")
    ...
    if holders:
        ...
        return True, f"other Pixel Agents processes still hold the venv (PID {pids})"
    return False, ""
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Pixel Agents-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``pixel-agents update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``pixel-agents update`` repairs the runtime (issue class:
...
def repair_vulnerable_runtime(
    ...
    *,
    ...
    post-cutover smoke failures restore the parked venv synchropixelly.
    """
    root = Path(project_root) if project_root is not None else _PROJECT_ROOT
    ...
    try:
        ...
        print(
            "  ⚠ Pixel Agents venv links SQLite "
...
def _install_uv(target: Path) -> None:
    ...
    ``$PIXEL_AGENTS_HOME/bin/`` instead of ``~/.local/bin/``.
    """
