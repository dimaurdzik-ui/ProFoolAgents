"""
Gateway subcommand for pixel-agents CLI.

Handles: pixel-agents gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_pixel_agents_home is imported lazily at call sites to avoid ImportError
# when pixel_constants is cached from a pre-update version during `pixel-agents update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "pixel-agents CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _request_gateway_self_restart(pid: int) -> bool:
    """Ask a running gateway ancestor to restart itself asynchropixelly."""
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``pixel-agents gateway status`` from falsely counting the ``pixel-agents`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``pixel-agents gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_pixel_agents_home().resolve())
    # Forward slashes on both sides of the PIXEL_AGENTS_HOME= match — see
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"pixel_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. PIXEL_AGENTS_HOME
        ...
        # PIXEL_AGENTS_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "pixel_home=" in command_lc
            and f"pixel_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``pixel-agents update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Pixel Agents profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Pixel Agents profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from pixel_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "pixel_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m pixel_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``pixel-agents update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Pixel Agents's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose PIXEL_AGENTS_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``pixel-agents update`` users
    # with no running gateway until they re-invoke ``pixel-agents gateway``
    ...
    # interpreter and captures the stable cwd + env overlay (PIXEL_AGENTS_HOME,
    ...
    if sys.platform == "win32":
        try:
            from pixel_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / PIXEL_AGENTS_HOME) the windowless
        # base interpreter needs to import pixel_cli.  Empty on POSIX, where
...
def _pixel_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``PIXEL_AGENTS_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("PIXEL_AGENTS_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_pixel_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``PIXEL_AGENTS_HOME``.

    Under ``sudo``, ``PIXEL_AGENTS_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_pixel_agents_home` falls back to ``/root/.pixel-agents`` — the wrong
    profile. The unit file pins ``PIXEL_AGENTS_HOME`` for the actual gateway
    ...
    unit_home = (_pixel_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("PIXEL_AGENTS_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("PIXEL_AGENTS_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["PIXEL_AGENTS_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}pixel-agents gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}pixel-agents gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from pixel_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: pixel-agents gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `pixel-agents gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``pixel-agents gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from pixel_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (pixel_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (PIXEL_AGENTS_HOME-scoped).
...
from pixel_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Pixel Agents Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``pixel-agents gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``PIXEL_AGENTS_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("PIXEL_AGENTS_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "pixel-agents-gateway"
SERVICE_DESCRIPTION = "Pixel Agents Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current PIXEL_AGENTS_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (PIXEL_AGENTS_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from pixel_constants import get_default_pixel_agents_root

    home = get_pixel_agents_home().resolve()
    default = get_default_pixel_agents_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary PIXEL_AGENTS_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(pixel_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when PIXEL_AGENTS_HOME is a named profile.

    For ``~/.pixel-agents/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        pixel_home: Optional explicit PIXEL_AGENTS_HOME path. Defaults to the current
            ``get_pixel_agents_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Pixel Agents root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_pixel_agents_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from pixel_constants import get_default_pixel_agents_root

    home = Path(pixel_home or str(get_pixel_agents_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_pixel_agents_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(pixel_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".pixel-agents"
    try:
        Path(pixel_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(pixel_home, default_root=target_root)
    except ValueError:
        return _profile_arg(pixel_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this PIXEL_AGENTS_HOME.

    Default ``~/.pixel-agents`` returns ``pixel-agents-gateway`` (backward compatible).
    Profile ``~/.pixel-agents/profiles/coder`` returns ``pixel-agents-gateway-coder``.
    Any other PIXEL_AGENTS_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Pixel Agents installs that predate the
# pixel-agents-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (pixel-agents-gateway-*.service) and unrelated third-party
# "pixel-agents" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("pixel-agents.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "pixel_cli.main gateway",
    "pixel_cli/main.py gateway",
    "gateway/run.py",
    " pixel-agents gateway ",
    "/pixel-agents gateway ",
)
...
def _find_legacy_pixel_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Pixel Agents gateway units.

    Detects unit files installed by older Pixel Agents versions that used a
    different service name (e.g. ``pixel-agents.service`` before the rename to
    ``pixel-agents-gateway.service``). When both a legacy unit and the current
    ``pixel-agents-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``pixel-agents-gateway-coder.service`` and unrelated third-party
      ``pixel-agents-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``pixel-agents.service`` running an unrelated
...
def has_legacy_pixel_units() -> bool:
    """Return True when any legacy Pixel Agents gateway unit files exist."""
    return bool(_find_legacy_pixel_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Pixel Agents gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_pixel_units()
    if not legacy:
        return
    print_warning("Legacy Pixel Agents gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current pixel-agents-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    pixel-agents gateway migrate-legacy")


def remove_legacy_pixel_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Pixel Agents gateway unit files.

    Iterates over whatever ``_find_legacy_pixel_units()`` returns — which is
    ...
    legacy = _find_legacy_pixel_units()
    if not legacy:
        print("No legacy Pixel Agents gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Pixel Agents gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: pixel-agents gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo pixel-agents gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    pixel-agents gateway uninstall")
    print_info("    sudo pixel-agents gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: pixel-agents gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Pixel Agents often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.pixel-agents`` → ``ai.pixel-agents.gateway.plist`` (backward compatible).
    Profile ``~/.pixel-agents/profiles/coder`` → ``ai.pixel-agents.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.pixel-agents.gateway-{suffix}" if suffix else "ai.pixel-agents.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.pixel-agents/pixel-agents  -> /home/alice/.pixel-agents/pixel-agents
      /opt/pixel-agents                 -> /opt/pixel-agents  (kept as-is)
...
def _pixel_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current PIXEL_AGENTS_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_pixel_agents_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.pixel-agents                    → /home/alice/.pixel-agents
      /root/.pixel-agents/profiles/coder     → /home/alice/.pixel-agents/profiles/coder
      /opt/custom-pixel-agents               → /opt/custom-pixel-agents  (kept as-is)
    """
    current_pixel_raw = os.environ.get("PIXEL_AGENTS_HOME", "").strip()
    current_pixel = (
        Path(current_pixel_raw).expanduser()
        if current_pixel_raw
        else get_pixel_agents_home()
    )
    ...
    # different PIXEL_AGENTS_HOME into the generated service unit.
    current_default = Path.home() / ".pixel-agents"
    target_default = Path(target_home_dir) / ".pixel-agents"

    # Default ~/.pixel-agents → remap to target user's default
    if current_pixel == current_default:
        return str(target_default)

    # Profile or subdir of ~/.pixel-agents → preserve the relative structure
    try:
        relative = current_pixel.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.pixel-agents) — keep as-is
        return str(current_pixel)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    pixel_node = pixel_home / "node" / "bin"
    if _is_dir(pixel_node):
        candidates.append(str(pixel_node))
    pixel_nm = pixel_home / "node_modules" / ".bin"
    if _is_dir(pixel_nm):
        candidates.append(str(pixel_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m pixel_cli.main``, so module
    ...
    dir, or a clone that ``pixel-agents update`` later relocates/removes — the path
    ...
    ``PIXEL_AGENTS_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if PIXEL_AGENTS_HOME
...
def _systemd_watchdog_seconds(pixel_home: str | Path | None = None) -> int:
    ...
    if pixel_home is not None:
        ...
        override_token = set_pixel_agents_home_override(pixel_home)
        reset_home_override = reset_pixel_agents_home_override
    try:
...
def _systemd_watchdog_service_fields(
    pixel_home: str | Path | None = None,
) -> tuple[str, str]:
    """Return systemd service fields for the effective gateway config."""
    seconds = _systemd_watchdog_seconds(pixel_home)
    if seconds <= 0:
        return "simple", ""
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        pixel_home = _pixel_home_for_target_user(home_dir)
        systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
            pixel_home
        )
        profile_arg = _profile_arg_for_target_user(pixel_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's PIXEL_AGENTS_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(pixel_home) if pixel_home else _remap_path_for_user(working_dir, home_dir)
...
Group={group_name}
ExecStart={python_path} -m pixel_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="PIXEL_AGENTS_HOME={pixel_home}"
Restart=always
RestartSec=5
...
"""
    ...
    pixel_home = str(get_pixel_agents_home().resolve())
    systemd_type, systemd_watchdog_directives = _systemd_watchdog_service_fields(
        pixel_home
    )
    profile_arg = _profile_arg(pixel_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m pixel_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="PIXEL_AGENTS_HOME={pixel_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    # ── PIXEL_AGENTS_HOME sync chokepoint ──────────────────────────────────────
    ...
    # pinned PIXEL_AGENTS_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo pixel-agents gateway … --system``, PIXEL_AGENTS_HOME is often stripped
    # and falls back to ``/root/.pixel-agents``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / PIXEL_AGENTS_HOME comparisons
    ...
    _sync_pixel_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir PIXEL_AGENTS_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved PIXEL_AGENTS_HOME
    ...
    exported a throwaway ``PIXEL_AGENTS_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``pixel-agents gateway restart``
    with ``PIXEL_AGENTS_HOME=/tmp/pixel-agents-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="PIXEL_AGENTS_HOME=..."`` lines and launchd
    ``<key>PIXEL_AGENTS_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'PIXEL_AGENTS_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>PIXEL_AGENTS_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp PIXEL_AGENTS_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: PIXEL_AGENTS_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported PIXEL_AGENTS_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single PIXEL_AGENTS_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only PIXEL_AGENTS_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp PIXEL_AGENTS_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../pixel_test`` path into
    # ``Environment="PIXEL_AGENTS_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/pixel_test"' in new_unit
        or "/pixel_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # PIXEL_AGENTS_HOME into the unit (manual E2E homes like /tmp/pixel-agents-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Pixel Agents install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo pixel-agents gateway uninstall --system")
    print_info("         pixel-agents gateway install")
    print_info("         pixel-agents gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (pixel-agents.service from pre-rename installs)
    # before installing the new pixel-agents-gateway.service. If both remain, they
    ...
    if has_legacy_pixel_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_pixel_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin PIXEL_AGENTS_HOME; adopt it before any
    ...
    # ``sudo pixel-agents gateway install --system --force`` would bake /root/.pixel-agents
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}pixel-agents gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}pixel-agents gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}pixel-agents gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # PIXEL_AGENTS_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `pixel-agents gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `pixel-agents gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}pixel-agents gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}pixel-agents gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}pixel-agents gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}pixel-agents gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.pixel-agents.gateway-{suffix}" if suffix else "ai.pixel-agents.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``pixel-agents gateway start/stop/status`` call).
...
#      background process (the `nohup pixel-agents gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_pixel_agents_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_pixel_agents_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m pixel_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active PIXEL_AGENTS_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "pixel_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup pixel-agents gateway run --replace`
    ...
    """
    from pixel_cli._subprocess_compat import windows_detach_popen_kwargs

    log_dir = get_pixel_agents_home() / "logs"
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup pixel-agents gateway run --replace "
...
<dict>
    ...
    <dict>
        ...
        <key>PIXEL_AGENTS_HOME</key>
        <string>{pixel_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.pixel-agents/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See pixel-agents-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_pixel_agents_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Pixel Agents install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  pixel-agents gateway status             # Check status")
    from pixel_constants import display_pixel_agents_home as _dhh
...
def launchd_stop():
    ...
    # `pixel-agents gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # Pixel Agents PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Pixel Agents PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Pixel Agents install")
    else:
        print("⚠ Service definition is stale relative to the current Pixel Agents install")
        print("  Run: pixel-agents gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: pixel-agents gateway stop")
            else:
    ...
    if deep:
        log_file = get_pixel_agents_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _is_official_docker_checkout() -> bool:
    return (
        str(PROJECT_ROOT) == "/opt/pixel-agents"
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``PIXEL_AGENTS_S6_SUPERVISED_CHILD``.
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``pixel-agents gateway run`` is always a misconfiguration, so we
    ...
    try:
        from pixel_constants import get_default_pixel_agents_root
        default_root = get_default_pixel_agents_root()
    ...
    print()
    print("    pixel-agents gateway restart")
    print()
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``pixel-agents gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``pixel-agents gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other PIXEL_AGENTS_HOME roots.
    """
    ...
    print("  Use 'pixel-agents gateway restart' to replace it,")
    print("  or 'pixel-agents gateway stop' first.")
    print("  Or use 'pixel-agents gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("PIXEL_AGENTS_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Pixel Agents gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'pixel-agents' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/pixel-agents/docker/entrypoint.sh before the Pixel Agents command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$PIXEL_AGENTS_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set PIXEL_AGENTS_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `pixel-agents gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set PIXEL_AGENTS_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `pixel-agents gateway restart` which already
    ...
    # `pixel-agents gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Pixel Agents Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # PIXEL_AGENTS_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("PIXEL_AGENTS_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
    ...
    # the env vars ``PIXEL_AGENTS_GATEWAY_MAX_STARTS`` /
    # ``PIXEL_AGENTS_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("PIXEL_AGENTS_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("PIXEL_AGENTS_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``pixel-agents gateway run`` enters through this CLI wrapper, not through
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. pixel-agents) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Pixel Agents delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Pixel Agents connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: pixel-agents pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Pixel Agents will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``pixel-agents setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run pixel-agents under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.pixel-agents/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_pixel_agents_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    from pixel_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from pixel_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'pixel-agents pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: pixel-agents pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'pixel-agents gateway setup'"
                    )
            continue
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from pixel_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Pixel Agents will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Pixel Agents will store the returned account_id/token in ~/.pixel-agents/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `pixel-agents gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_pixel_agents_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `pixel-agents pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `pixel-agents pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "PixelAgentsAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``pixel_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from pixel_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.pixel-agents/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.pixel-agents/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if supports_systemd_services() and has_legacy_pixel_units():
        print_legacy_unit_warning()
        print()
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: pixel-agents gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: pixel-agents gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: pixel-agents gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo pixel-agents gateway install --system"
                        )
                    print_info("  Or run in foreground:  pixel-agents gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: pixel-agents gateway run")
                print_info(
                    "  For persistence:   tmux new -s pixel-agents 'pixel-agents gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: pixel-agents gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup pixel-agents gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'pixel-agents gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`pixel_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from pixel_cli.service_manager import (
    ...
    if profile is None:
        ...
        # PIXEL_AGENTS_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``pixel-agents gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``pixel-agents gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``pixel-agents gateway run`` are unaffected.
      2. ``PIXEL_AGENTS_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``pixel-agents gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``PIXEL_AGENTS_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("PIXEL_AGENTS_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("PIXEL_AGENTS_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${PIXEL_AGENTS_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if PIXEL_AGENTS_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or PIXEL_AGENTS_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-pixel-agents service's
    # pattern in docker/s6-rc.d/main-pixel-agents/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: pixel-agents gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: pixel-agents gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s pixel-agents 'pixel-agents gateway run'"
                )
                print()
            ...
            print()
            print(
                "  pixel-agents gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s pixel-agents 'pixel-agents gateway run'         # persistent via tmux"
            )
            print(
                "  nohup pixel-agents gateway run > ~/.pixel-agents/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from pixel_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  pixel-agents profile create <name>     # creates the s6 service slot")
                print("  pixel-agents -p <name> gateway start   # bring it up via s6")
                print("  pixel-agents status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: pixel-agents gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: pixel-agents gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: pixel-agents gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  pixel-agents profile delete <name>     # tears down the s6 service slot")
                print("  pixel-agents -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `pixel-agents -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: pixel-agents gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_PIXEL_AGENTS_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `pixel-agents gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_PIXEL_AGENTS_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `pixel-agents gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: pixel-agents gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  pixel-agents gateway install")
                else:
                    print("To install as a service:")
                    print("  pixel-agents gateway install")
                    print("  sudo pixel-agents gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  pixel-agents gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup pixel-agents gateway run > ~/.pixel-agents/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s pixel-agents 'pixel-agents gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  pixel-agents gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  pixel-agents gateway install  # Install as user service")
                    print(
                        "  sudo pixel-agents gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Pixel Agents gateway unit files from
        # pre-rename installs (e.g. pixel-agents.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_pixel_units(interactive=not yes, dry_run=dry_run)
