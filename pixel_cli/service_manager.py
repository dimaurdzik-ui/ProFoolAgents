...
the existing module-level functions in pixel_cli.gateway and
pixel_cli.gateway_windows directly. This protocol is a thin facade
...
``pixel-agents gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``pixel-agents gateway start/stop/restart``).
    """
    ...
    # `pixel-agents gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged pixel-agents
    ...
    detection always fails. Since every Pixel Agents runtime call inside the
    container drops to pixel-agents via ``s6-setuidgid``, that silent failure
...
# in ``pixel_cli.gateway`` (systemd/launchd) and ``pixel_cli.gateway_windows``
...
# active (set via the ``pixel-agents -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in pixel_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from pixel_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from pixel_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from pixel_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from pixel_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in pixel_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from pixel_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from pixel_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from pixel_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from pixel_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``pixel_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `pixel-agents profile create`
# runs inside the container (Phase 4). Static services (main-pixel-agents, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's PIXEL_AGENTS_HOME blindly;
    derive the shared profile root from the current PIXEL_AGENTS_HOME and map the
    ...
    pixel_home = Path(os.environ.get("PIXEL_AGENTS_HOME", "/opt/data"))
    if pixel_home.parent.name == "profiles":
        root = pixel_home.parent.parent
    else:
        root = pixel_home
...
# UID/GID of the in-image ``pixel-agents`` user. Hardcoded to match what
...
_PIXEL_AGENTS_UID = 10000
_PIXEL_AGENTS_GID = 10000


def _seed_supervise_skeleton(svc_dir: Path) -> None:
    """Pre-create the ``supervise/`` and top-level ``event/`` skeleton
    inside a service directory, owned by the pixel-agents user.
    ...
    (the ``pixel-agents`` user — UID 10000 — running every Pixel Agents runtime
    ...
    is dispatched as the pixel-agents user.
    ...
    with pixel-agents ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           pixel-agents:pixel-agents, 0755 (parent must already exist)
    ``svc_dir/event/``                     pixel-agents:pixel-agents, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 pixel-agents:pixel-agents, 0755
    ``svc_dir/supervise/event/``           pixel-agents:pixel-agents, 03730
    ``svc_dir/supervise/control``          pixel-agents:pixel-agents, 0660    (FIFO)
    ...
    pixel-agents user reads them fine.
    ...
    slot's supervise/ was pixel-agents-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _PIXEL_AGENTS_UID, _PIXEL_AGENTS_GID)
        except PermissionError:
            # Running as the pixel-agents user already — directory is pixel-agents-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _PIXEL_AGENTS_UID, _PIXEL_AGENTS_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is pixel-agents-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _PIXEL_AGENTS_UID, _PIXEL_AGENTS_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``pixel-agents -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`pixel-agents profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-pixel-agents, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources PIXEL_AGENTS_HOME (and any extra env) via with-contenv —
             so e.g. ``-e PIXEL_AGENTS_HOME=/data/pixel-agents`` is honored at run
          ...
          4. Drops to the pixel-agents user and exec's
             ``pixel-agents -p <profile> gateway run`` (or just ``pixel-agents
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``pixel-agents gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root PIXEL_AGENTS_HOME profile" (the implicit profile that exists at
        the top of $PIXEL_AGENTS_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``pixel_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$PIXEL_AGENTS_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``PIXEL_AGENTS_HOME``,
        ...
        allocator in ``pixel_cli.profiles._allocate_gateway_port``).
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/pixel-agents/.venv/bin/activate",
        ]
        ...
        # guard.
        lines.append("export PIXEL_AGENTS_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's PIXEL_AGENTS_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``pixel-agents gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-PIXEL_AGENTS_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The PIXEL_AGENTS_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own PIXEL_AGENTS_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "pixel-agents gateway run --replace"
        else:
            gateway_cmd = f"pixel-agents -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid pixel-agents {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${PIXEL_AGENTS_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the PIXEL_AGENTS_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e PIXEL_AGENTS_HOME=/data/pixel-agents``
        gets its logs under /data/pixel-agents/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``pixel-agents logs`` reads and what
        ...
        return (
            ...
            f': "${{PIXEL_AGENTS_HOME:=/opt/data}}"\n'
            f'log_dir="$PIXEL_AGENTS_HOME/logs/gateways/{prof}"\n'
            # Create the leaf and clear a stale s6-log lock as pixel-agents when
            # this script starts as root. Never chown or unlink pixel-agents-writable
            # volume paths from this restartable root-context script:
            # log/supervise/control is pixel-agents-owned, so an unprivileged user
            # can race a pathname op through a symlink swap (CWE-59 /
            # CWE-367). Parent logs/gateways is seeded pixel-agents-owned at stage2
            # boot (#45258; tests/docker/test_log_dir_seed.py).
            f'if [ "$(id -u)" = 0 ]; then\n'
            f'  s6-setuidgid pixel-agents mkdir -p "$log_dir"\n'
            f'  s6-setuidgid pixel-agents rm -f "$log_dir/lock"\n'
            f'else\n'
            f'  mkdir -p "$log_dir"\n'
            f'  rm -f "$log_dir/lock"\n'
            f'fi\n'
            ...
            f'exec s6-setuidgid pixel-agents s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``pixel-agents gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``pixel-agents -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with pixel-agents ownership
            ...
            # the pixel-agents user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # Give s6-svscan a moment to reap. There's no synchropixel
        ...
        # is pixel-agents-owned (see ``_seed_supervise_skeleton``), and on
