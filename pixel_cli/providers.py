"""
Single source of truth for provider identity in Pixel Agents.
...
2. **Pixel Agents overlays** — transport type, auth patterns, aggregator flags,
...
# -- Pixel Agents overlay ----------------------------------------------------------
# Pixel Agents-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class PixelAgentsOverlay:
    """Pixel Agents-specific provider metadata layered on top of models.dev."""
...
PIXEL_AGENTS_OVERLAYS: Dict[str, PixelAgentsOverlay] = {
    "moa": PixelAgentsOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": PixelAgentsOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "pixel": PixelAgentsOverlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_override="https://inference-api.pixelagents.com/v1",
    ),
    "openai-codex": PixelAgentsOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": PixelAgentsOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": PixelAgentsOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": PixelAgentsOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="PIXEL_AGENTS_QWEN_BASE_URL",
    ),
    "lmstudio": PixelAgentsOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": PixelAgentsOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": PixelAgentsOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": PixelAgentsOverlay(
        transport="anthropic_messages",
        extra_env_vars=("ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN"),
    ),
    "zai": PixelAgentsOverlay(
        transport="openai_chat",
        extra_env_vars=("GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY"),
        base_url_env_var="GLM_BASE_URL",
    ),
    "kimi-for-coding": PixelAgentsOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": PixelAgentsOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": PixelAgentsOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": PixelAgentsOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": PixelAgentsOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": PixelAgentsOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": PixelAgentsOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": PixelAgentsOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": PixelAgentsOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": PixelAgentsOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": PixelAgentsOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "kilo": PixelAgentsOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="KILOCODE_BASE_URL",
    ),
    "huggingface": PixelAgentsOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": PixelAgentsOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="NOVITA_BASE_URL",
    ),
    "xai": PixelAgentsOverlay(
        transport="codex_responses",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "nvidia": PixelAgentsOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": PixelAgentsOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": PixelAgentsOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": PixelAgentsOverlay(
        transport="openai_chat",
        base_url_override="https://api.arcee.ai/api/v1",
        base_url_env_var="ARCEE_BASE_URL",
    ),
    "gmi": PixelAgentsOverlay(
        transport="openai_chat",
        extra_env_vars=("GMI_API_KEY",),
        base_url_override="https://api.gmi-serving.com/v1",
        base_url_env_var="GMI_BASE_URL",
    ),
    "fireworks": PixelAgentsOverlay(
        transport="openai_chat",
        extra_env_vars=("FIREWORKS_API_KEY",),
        base_url_override="https://api.fireworks.ai/inference/v1",
    ),
    "upstage": PixelAgentsOverlay(
        transport="openai_chat",
        extra_env_vars=("UPSTAGE_API_KEY",),
        base_url_override="https://api.upstage.ai/v1",
        base_url_env_var="UPSTAGE_BASE_URL",
    ),
    "ollama-cloud": PixelAgentsOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": PixelAgentsOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": PixelAgentsOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "pixel-agents", "user-config"
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Pixel Agents overlays (for providers not in models.dev: pixel, openai-codex, etc.)
      2. models.dev catalog + Pixel Agents overlay
    ...
    overlay = PIXEL_AGENTS_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + pixel-agents extra
    ...
    if overlay is not None:
        # Pixel Agents-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def pixel_api_mode(model: str = "") -> str:
    """Resolve the wire protocol for a Pixel Portal model.
    ...
    historical Pixel transport — so callers that don't yet know the model
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    Resolution order:
      ...
      2. Pixel Portal dual-wire (model-derived; overlay alone is openai_chat).
      3. Known provider → transport → TRANSPORT_TO_API_MODE.
      4. Direct provider checks (bedrock).
      5. Default: 'chat_completions'.

    *model* is optional but required for dual-wire providers (Pixel) whose
    ...
    # Pixel is dual-wire: anthropic/* → Messages, everything else →
    # chat_completions. The Pixel Agents overlay still advertises openai_chat
    ...
    if provider_norm in {"pixel", "pixel-portal", "pixelagents"}:
        return pixel_api_mode(model)

    pdef = get_provider(provider)
    if pdef is not None:
        return TRANSPORT_TO_API_MODE.get(pdef.transport, "chat_completions")

    # Direct provider checks for providers not in PIXEL_AGENTS_OVERLAYS
    if provider == "bedrock":
        return "bedrock_converse"
...
def resolve_provider_full(
    ...
    # 0.5 Exact Pixel Agents provider IDs must win over LOSSY alias collapsing.
    ...
    if canonical != raw:
        try:
            from pixel_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
            ...
            if _pcfg is not None:
                ...
                if len(_collapsed_siblings) > 1:
                    return ProviderDef(
                        ...
                        source="pixel-agents-auth-registry",
                    )
        except Exception:
            pass
