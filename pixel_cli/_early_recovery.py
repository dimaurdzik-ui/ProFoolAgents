"""Dependency-light venv recovery that runs BEFORE pixel_cli.main's imports.

The ``pixel-agents`` console entry point is ``pixel_cli.main:main``.  Importing
``pixel_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``pixel_cli.env_loader``, ``yaml`` via ``pixel_cli.config``, ...).  In
...
a corrupted venv.  ``pixel_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``pixel_cli.main`` to become
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``pixel-agents acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``pixel_cli.main``
...
def recover_if_needed(
    ...
    """Repair wiped core packages so ``pixel_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls.  Only acts when a
    recovery marker from a prior ``pixel-agents update`` exists AND an import probe
