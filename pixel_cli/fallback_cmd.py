"""
pixel-agents fallback — manage the fallback provider chain.
...
Subcommands:
  pixel-agents fallback [list]   Show the current fallback chain (default when no subcommand)
  pixel-agents fallback add      Pick provider + model via the same picker as `pixel-agents model`,
                           then append the selection to the chain
  pixel-agents fallback remove   Pick an entry to delete from the chain
  pixel-agents fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.pixel-agents/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from pixel_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `pixel-agents model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  pixel-agents fallback add")
        print()
        return
    ...
    print("  Docs: https://api.pixelagents.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `pixel-agents model`, then append the selection to the chain."""
    from pixel_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `pixel-agents model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `pixel-agents fallback list` to view, or `pixel-agents fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``pixel-agents fallback [subcommand]``."""
