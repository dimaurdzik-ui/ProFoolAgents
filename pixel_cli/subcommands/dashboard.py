"""``pixel-agents dashboard`` / ``pixel-agents serve`` subcommand parsers.
...
``pixel_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `pixel-agents dashboard` / `pixel-agents serve` cmdlines and SIGTERM them directly —
    # the same path `pixel-agents update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Pixel Agents web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Pixel Agents web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Pixel Agents web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Pixel Agents desktop app shells (<= 0.15.x) spawn the
    # backend as `pixel-agents dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Pixel Agents couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Pixel Agents Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Pixel Agents backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Pixel Agents backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports PIXEL_AGENTS_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `pixel-agents dashboard register` — register a self-hosted dashboard OAuth
    # client with Pixel Portal and write the client_id into ~/.pixel-agents/.env.
    # Nested subparser so bare `pixel-agents dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Register a self-hosted dashboard with Pixel Portal (writes the OAuth client ID to .env)",
        description=(
            "Register this install as a self-hosted dashboard with your Pixel "
            "Portal account. Creates an OAuth client, writes "
            "PIXEL_AGENTS_DASHBOARD_OAUTH_CLIENT_ID into ~/.pixel-agents/.env, and prints "
            "how to engage the login gate. Requires being logged in (pixel-agents setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://pixel-agents.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        default=None,
        help=(
            "Override the Pixel Portal base URL for registration (default: the "
            "portal you logged into). The access token must be valid at this "
            "portal. Also settable via PIXEL_AGENTS_DASHBOARD_PORTAL_URL. Mainly for "
