"""``pixel-agents login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``pixel-agents login`` was removed in favor of ``pixel-agents auth`` / ``pixel-agents model``
    (the runtime handler in ``pixel_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``pixel-agents login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``pixel-agents --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``pixel-agents login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``pixel-agents model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `pixel-agents auth` to manage credentials, "
            "`pixel-agents model` to select a provider, or `pixel-agents setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `pixel-agents model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: pixel-agents-cli)"
    )
