"""``pixel-agents gui`` subcommand parser.
...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Pixel Agents Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any pixel-agents CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--pixel-agents-root",
        help="Override the Pixel Agents source root used by Desktop (sets PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets PIXEL_AGENTS_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",
