"""``pixel-agents cron`` subcommand parser.

Extracted verbatim from ``pixel_cli/main.py:main()`` — same arguments, same
...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.pixel-agents/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--provider",
        dest="model_provider",
        help="Inference provider paired with --model (e.g. 'openrouter', 'pixel').",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.pixel-agents/scripts/. Pass empty string to clear. "
