"""``pixel-agents debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Pixel Agents. Use 'pixel-agents debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    pixel-agents debug share              Upload debug report (asks for confirmation)
    pixel-agents debug share --yes        Skip confirmation (for scripts/CI)
    pixel-agents debug share --lines 500  Include more log lines
    pixel-agents debug share --expire 30  Keep paste for 30 days
    pixel-agents debug share --local      Print report locally (no upload)
    pixel-agents debug share --no-redact  Disable upload-time secret redaction
    pixel-agents debug share --pixel       Upload to Pixel-internal storage (private)
    pixel-agents debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    share_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload the debug bundle to Pixel-internal storage (AWS S3) instead "
            "of a public paste service. The bundle is private — viewable only "
            "by Pixel staff (and allowlisted Discord mods) via a Google-login-"
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'pixel-agents debug share'",
    )
