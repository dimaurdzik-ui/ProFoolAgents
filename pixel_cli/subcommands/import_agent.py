"""``pixel-agents import-agent`` subcommand parser.

Follows the ``pixel-agents claw`` pattern (see ``pixel_cli/subcommands/claw.py``):
parser building lives here, the handler is injected to avoid importing
``main``, and the import logic itself lives in ``pixel_cli/agent_import.py``.
"""
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent",
        help="Import a Claude Code or Codex CLI setup into Pixel Agents",
        description=(
            "One-command import of another coding agent's setup into Pixel Agents. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their Pixel Agents equivalents. "
            "Always shows a preview before making changes. API keys and "
            "credentials are never imported — run 'pixel-agents setup' for those."
        ),
    )
    parser.add_argument(
        "agent",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing Pixel Agents items on name conflicts (default: skip)",
    )
    parser.add_argument(
