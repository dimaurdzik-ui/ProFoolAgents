"""``pixel-agents sync`` subcommand parser — Skill Sync.

Cloned from ``pixel_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    pixel-agents sync status                 show gate/opt-in/head state
    pixel-agents sync pull                   pull and materialize opted-in skills
    pixel-agents sync push                   push opted-in skills
    pixel-agents sync now                    reconcile: pull then push
    pixel-agents sync enable <skill>         opt a skill into sync
    pixel-agents sync disable <skill>        opt a skill out of sync
    pixel-agents sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    pixel-agents sync propose <skill>        share a skill with your organisation

Sync is INERT unless the resolved Pixel token carries the access-gate claim
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  pixel-agents sync status            what is synced, and from where\n"
            "  pixel-agents sync enable my-skill   include a skill in your sync\n"
            "  pixel-agents sync now               pull, then push\n"
            "  pixel-agents sync propose my-skill  share a skill with your team\n"
        ),
