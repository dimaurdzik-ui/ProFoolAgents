"""``pixel-agents gateway`` and ``pixel-agents proxy`` subcommand parsers.
...
from pixel_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if PIXEL_AGENTS_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy pixel-agents.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Pixel Agents gateway unit files "
            "(e.g. pixel-agents.service) left over from older installs. Profile "
            "units (pixel-agents-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Authenticates as your Pixel Portal account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.pixel-agents/.env. "
            "Requires being logged in (pixel-agents setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.pixel-agents/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. Pixel Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="pixel",
        help="Upstream provider: pixel or xai (default: pixel). See `pixel-agents proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
