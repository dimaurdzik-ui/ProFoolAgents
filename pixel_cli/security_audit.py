"""On-demand supply-chain audit for Pixel Agents installs.

Scans three surfaces a Pixel Agents user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Pixel Agents venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.pixel-agents/plugins``
...
def _discover_plugins(pixel_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.pixel-agents/plugins``.
...
def run_audit(
    *,
    ...
    home = pixel_home or Path(get_pixel_agents_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, pixel_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(pixel_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `pixel-agents security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, pixel_home=home
    )
    if total == 0:
