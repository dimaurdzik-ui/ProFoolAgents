"""Safe Pixel Agents Console command engine.

This module backs ``pixel-agents console`` and is intentionally narrower than the
full Pixel Agents CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["PixelAgentsConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'pixel-agents doctor'")
        and last.startswith("Run 'pixel-agents setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="pixel-agents", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `pixel-agents "):
        return ""
    return summary
...
# dashboard opens a fresh PixelAgentsConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("pixel_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace], None] | None = None,
) -> Callable[["PixelAgentsConsoleEngine", list[str]], str]:
    def handler(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    engine: "PixelAgentsConsoleEngine",
    *,
    root: str,
    paths: Iterable[Sequence[str]],
    handler_factory: Callable[[Sequence[str]], Callable[["PixelAgentsConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `pixel-agents {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `pixel-agents {usage}`?",
        )


class PixelAgentsConsoleEngine:
    """Curated line-command executor for Pixel Agents Console."""
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "pixel-agents":
                tokens = tokens[1:]
            if not tokens:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Pixel Agents Console does not run shell syntax. Use one supported "
                    "Pixel Agents command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Pixel Agents component status.", _status)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Pixel Agents logs.", _logs)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Pixel Agents configuration?",
        )
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Pixel Agents Console."""

        extracted = {
            "version": (
                "pixel_cli.subcommands.version",
            ...
            "prompt-size": (
                "pixel_cli.subcommands.prompt_size",
            ...
            "insights": (
                "pixel_cli.subcommands.insights",
            ...
            "security": (
                "pixel_cli.subcommands.security",
            ...
            "import": (
                "pixel_cli.subcommands.import_cmd",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Pixel Agents configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "pixel_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("pixel_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "pixel_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("pixel_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("pixel_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        raise ConsoleCommandError(f"Unsupported Pixel Agents Console command: {probe}.{suffix}")

    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Pixel Agents Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`pixel-agents {first}` is not available in Pixel Agents Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Pixel Agents Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Pixel Agents Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Pixel Agents Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Pixel Agents Console.",
            ("skills", "publish"): "`skills publish` is not available in Pixel Agents Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Pixel Agents Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Pixel Agents Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Pixel Agents Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Pixel Agents Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Pixel Agents Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Pixel Agents Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Pixel Agents Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Pixel Agents Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Pixel Agents Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Pixel Agents Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Pixel Agents Console."
        return ""
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Pixel Agents Console.")
...
def _status(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Pixel Agents Console.")
    ...
    from pixel_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
...
def _sessions_stats(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from pixel_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from pixel_cli.config import get_config_path

    return str(get_config_path())


def _config_set(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    key = args[0]
    value = " ".join(args[1:])
    from pixel_cli.config import set_config_value

    return _capture_output(lambda: set_config_value(key, value))


def _config_migrate(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config migrate")

    def _run() -> None:
        from pixel_cli.config import migrate_config
...
def _sessions_export(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from pixel_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _profile_status(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="pixel_cli.subcommands.profile",
        builder_name="build_profile_parser",
        main_handler_name="cmd_profile",
    )


def _cron_list(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from pixel_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from pixel_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from pixel-agents console")
...
def _cron_resume(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def _cron_run(_engine: PixelAgentsConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``pixel-agents console`` REPL."""
    ...
    engine = PixelAgentsConsoleEngine()
    if interactive:
        print("Pixel Agents Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("pixel-agents> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
