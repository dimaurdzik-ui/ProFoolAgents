"""Implementation of the ``pixel-agents bundles`` CLI subcommand.

Mirrors the structure of ``pixel_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `pixel-agents bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  pixel-agents bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``pixel-agents bundles`` argparse tree.

    Called from ``pixel_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `pixel-agents bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``pixel-agents bundles <subcommand>`` to the right handler."""
