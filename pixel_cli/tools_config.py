"""
Unified tool configuration for Pixel Agents.

`pixel-agents tools` and `pixel-agents setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.pixel-agents/config.yaml under
...
from pixel_cli.pixel_subscription import (
    MANAGED_FEATURE_COVERAGE_CATEGORY,
    apply_pixel_managed_defaults,
    get_pixel_subscription_features,
)
from pixel_cli.pixel_account import format_pixel_portal_entitlement_message
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``pixel-agents tools post-setup <key>`` child. On Windows, every console child
    ...
    :func:`pixel_cli._subprocess_compat.windows_hide_flags`) suppresses it
...
from pixel_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _PIXEL_AGENTS_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `pixel-agents tools` → Video Generation, which walks
...
# `pixel-agents tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search"}


# Config-only capabilities: they appear in `pixel-agents tools` for provider/API-key
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from pixel_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `pixel-agents tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``pixel-agents tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from pixel_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``pixel-agents tools`` checklist actually offers
    for ``platform``.
    ...
    so ``pixel-agents tools`` never claims to add or remove a toolset the user was
...
from pixel_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            {
                "name": "Pixel Subscription",
                "badge": "subscription",
                "tag": "Managed OpenAI TTS billed to your subscription",
                "env_vars": [],
                "tts_provider": "openai",
                "requires_pixel_auth": True,
                "managed_pixel_feature": "tts",
    ...
    "web": {
        ...
        #   - "Pixel Subscription" — managed Firecrawl billed via Pixel
        #     subscription (requires_pixel_auth + override_env_vars).
    ...
    "image_gen": {
        ...
        #   - "Pixel Subscription" — managed FAL billed via the Pixel
        #     subscription (requires_pixel_auth + override_env_vars).
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_pixel_auth": True,
                "managed_pixel_feature": "image_gen",
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "Pixel Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the Pixel Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_pixel_auth": True,
                "managed_pixel_feature": "video_gen",
                "override_env_vars": ["FAL_KEY"],
                # The underlying plugin backend — when the user picks
                # "Pixel Subscription" we set video_gen.provider = "fal"
    ...
    "x_search": {
        ...
        "setup_note": (
            "Pixel Agents routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # backend, never on the paid Pixel Subscription gateway row:
        #   - "Local Browser" — non-cloud option, no CloudBrowserProvider.
        #   - "Pixel Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Pixel subscription (requires_pixel_auth +
        ...
        "providers": [
            ...
            {
                "name": "Pixel Subscription (Browser Use cloud)",
                "badge": "subscription",
                "tag": "Managed Browser Use billed to your subscription",
                "env_vars": [],
                "browser_provider": "browser-use",
                "requires_pixel_auth": True,
                "managed_pixel_feature": "browser",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set PIXEL_AGENTS_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "PIXEL_AGENTS_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "PIXEL_AGENTS_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "PIXEL_AGENTS_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "PIXEL_AGENTS_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "PIXEL_AGENTS_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `pixel-agents model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("PIXEL_AGENTS_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Pixel Agents telemetry policy applied.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``pixel-agents update`` and
      by ``pixel-agents computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``pixel-agents update`` sets
    ...
    a further ~600s wait on Windows). ``pixel-agents computer-use install
    ...
    ``pixel-agents update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `pixel-agents update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `pixel-agents computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `pixel-agents update` (require_confirmed_update=True) keeps the
    ...
    # `pixel-agents computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: pixel-agents computer-use install --upgrade"
            )
            return True
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `pixel-agents update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `pixel-agents update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.pixel-agents/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Pixel Agents process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_pixel_agents_home()}/pixel-agents && npm install --workspaces=false")
                if result.stderr:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in pixel-agents directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/pixelagents/pixel-agents:latest"
            )
            return
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.pixel-agents/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.pixel-agents/config.yaml")
        ...
        # Run the full `pixel-agents auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.pixel-agents/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from pixel_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: pixel-agents auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `pixel-agents auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: pixel-agents auth spotify")
        except Exception as exc:
        ...
        try:
            from pixel_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: pixel-agents plugins enable observability/langfuse")
        _print_info("    Restart Pixel Agents for tracing to take effect.")
        _print_info("    Verify: pixel-agents plugins list")
        ...
        try:
            ...
            _print_info("    Run later: pixel-agents auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `pixel-agents auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: pixel-agents auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: pixel-agents auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``pixel-agents tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``pixel-agents tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: pixel-agents tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``pixel-agents-discord``, which contains those
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``pixel-agents-discord``) is an opt-in to the platform's native default-off
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"pixel-agents-{platform}"
    ...
    # "pixel-agents-cli" (which include all _PIXEL_AGENTS_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[pixel-agents-cli, spotify]`` after enabling Spotify via ``pixel-agents
        ...
        # (e.g. "pixel-agents-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``pixel-agents tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `pixel-agents tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"pixel-agents-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("pixel-agents-")}
    ...
    # shipped as a bundled plugin but user must opt in via `pixel-agents tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `pixel-agents tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by pixel-agents tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `pixel-agents` instead of
    # `pixel-agents-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `pixel-agents update`/`pixel-agents doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `pixel-agents tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (pixel-agents-cli, pixel-agents-telegram, etc.)
    ...
    # Opening `pixel-agents tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "stt", "browser"}:
        features = get_pixel_subscription_features(config, force_fresh=force_fresh)
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_pixel):
            return True
...
# ("Pixel Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Pixel Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    Pixel-managed Tool Gateway rows (``managed_pixel_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline Pixel Portal
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    for provider in cat.get("providers", []):
        # Pixel-managed Tool Gateway rows stay visible regardless of auth —
        # selecting one drives an inline Portal login. A `requires_pixel_auth`
        # row that is NOT a managed gateway feature (pure pre-auth UX) is
        # still hidden until the user is logged in.
        if (
            provider.get("requires_pixel_auth")
            and not provider.get("managed_pixel_feature")
            and not features.pixel_auth_present
        ):
            continue
        ...
        if (
            pool_only
            and provider.get("managed_pixel_feature") == "video_gen"
    ...
    # later) so the picker lists them alongside FAL / Pixel Subscription.
    ...
    # remaining hardcoded rows ("Pixel Subscription", "Firecrawl
    ...
    # the hardcoded "Pixel Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_pixel_gateway_message(
    cat: dict,
    config: dict,
    capability: str,
    *,
    ...
    """Deprecated: Pixel Tool Gateway rows are no longer hidden.

    Previously this returned a "log in / upgrade" banner shown above a
    category when its Pixel-managed rows were filtered out for unentitled
    ...
    (``ensure_pixel_portal_access``). Kept as a no-op so call sites stay simple;
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `pixel-agents tools` gets a silent no-op
...
def _agent_browser_installed() -> bool:
    ...
    from pixel_cli.pixel_subscription import _local_browser_runnable

    # The install hook runs in a spawned ``pixel-agents tools post-setup`` process,
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from pixel_cli.pixel_subscription import _has_agent_browser
...
def provider_readiness_status(
    ...
    *,
    ...
    - ``"needs_auth"``  — needs a sign-in: Pixel Portal login/entitlement for
    ...
    zero-env-var row, including logged-out Pixel Subscription rows).

    ``features`` (a ``PixelSubscriptionFeatures``) can be passed to avoid
    ...
    managed_feature = provider.get("managed_pixel_feature")
    if provider.get("requires_pixel_auth") or managed_feature:
        if features is None:
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_pixel_message = _hidden_pixel_gateway_message(
        cat,
        config,
        f"the Pixel Subscription provider for {name}",
    ...
    if len(providers) == 1:
        ...
        if hidden_pixel_message:
            for line in hidden_pixel_message.splitlines():
        ...
        # When the user is logged into Pixel, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of Pixel.
        try:
            _pixel_logged_in = bool(
                get_pixel_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).pixel_auth_present
        ...
        for p in providers:
            ...
            # Mark Pixel-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via Pixel Portal" hint so
            ...
            sub_marker = ""
            if p.get("managed_pixel_feature"):
                if _pixel_logged_in:
                    sub_marker = "  ★ Included with your Pixel subscription"
                else:
                    sub_marker = "  ★ via Pixel Portal (login on select)"
...
def _is_provider_active(
    ...
    *,
    ...
    if video_plugin_name and not provider.get("managed_pixel_feature"):
    ...
    if managed_feature:
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if image_cfg.get("use_gateway") is not None and not is_truthy_value(image_cfg.get("use_gateway"), default=False):
                    return False
            return feature.managed_by_pixel
        ...
        if "browser_provider" in provider:
            ...
            return feature.managed_by_pixel and provider["browser_provider"] == current
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return feature.managed_by_pixel and current == provider["web_backend"]
        return feature.managed_by_pixel
...
# Kept in sync with the dashboard selects (pixel_cli/web_server.py
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    NOT prompt for env vars, run post-setup hooks, gate on Pixel auth, or run
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Pixel Portal auth, or run interactive model
...
def _configure_provider(
    ...
    *,
    ...
    # Pixel-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid Pixel
    ...
    # "enable all tools" prompt (that lives in `pixel-agents model`).
    if managed_feature:
        from pixel_cli.pixel_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
            ensure_pixel_portal_access,
        )

        if not ensure_pixel_portal_access(
            capability=f"{provider.get('name', 'the Pixel Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Pixel Portal access is required for this backend."
            )
            return

    # Pure pre-auth UX rows (requires_pixel_auth without a managed gateway
    # feature) keep the old gate. Managed rows are handled by the inline
    # login above, so don't double-check them here.
    if provider.get("requires_pixel_auth") and not managed_feature:
        features = get_pixel_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(
            features.account_info and features.account_info.paid_service_access is True
        )
        if not features.pixel_auth_present or not entitled:
            message = format_pixel_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'Pixel Subscription')}",
            )
            _print_warning(
                f"  {message or 'Pixel Subscription is only available after logging into Pixel Portal.'}"
            )
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Pixel subscription.")
    ...
    # Pixel-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to Pixel.
    _show_portal_hint = False
    if env_vars and not managed_feature and not provider.get("requires_pixel_auth"):
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_pixel_feature") for sib in _providers
                ):
                    _has_managed_sibling = True
                    break
            if _has_managed_sibling:
                _features = get_pixel_subscription_features(
                    config,
                    force_fresh=force_fresh,
                )
                _show_portal_hint = not _features.pixel_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through Pixel Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``pixel-agents model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``pixel-agents model`` aux-task
    ...
    """
    try:
        from pixel_cli.inventory import (
    ...
    if not providers:
        _print_warning(
            ...
            "with `pixel-agents model`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Pixel Portal login + entitlement gate as _configure_provider:
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `pixel-agents model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `pixel-agents tools` and `pixel-agents setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Pixel Agents Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://api.pixelagents.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_pixel_managed_defaults(
                config,
            ...
            for ts_key in sorted(auto_configured):
                ...
                print(color(f"  ✓ {label}: using your Pixel subscription defaults", Colors.GREEN))
    ...
    print()
    from pixel_constants import display_pixel_agents_home
    print(color(f"  Tool configuration saved to {display_pixel_agents_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'pixel-agents' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, pixel-agents mcp
