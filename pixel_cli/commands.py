"""Slash command definitions and autocomplete for the Pixel Agents CLI.
...
class CommandDef:
    ...
    # Names a key in ``pixel_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``pixel_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("team", "Manage AI office staff (hire, list, archive)", "Tools & Skills",
               args_hint="<hire|list|archive> [profession-or-worker-id]"),
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Pixel Agents config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Pixel Agents works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Pixel Agents' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Pixel Agents is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.pixel-agents/skills/ for newly installed or removed skills",
    ...
    CommandDef("subscription", "View your Pixel plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "Show your Pixel balance and manage billing on the portal", "Info"),
    ...
    CommandDef("update", "Update Pixel Agents to the latest version", "Info",
               busy_policy="dispatch"),
    CommandDef("version", "Show Pixel Agents version", "Info", aliases=("v",),
               busy_policy="dispatch", execute="version"),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[pixel|local]"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`pixel_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/pixel-agents`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Pixel Agents ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Pixel Agents
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``pixel-agents skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``pixel-agents skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``pixel-agents skills list`` and
...
# unrelated command landed. These claim their slots right after /pixel-agents,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /pixel-agents <command>).
...
# native slot, the alias spelling stays reachable via /pixel-agents reset).
...
# ``/pixel-agents <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/pixel-agents" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /pixel-agents topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /pixel-agents moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /pixel-agents debug on Slack.
#   - egress: Docker-only proxy status; reachable as /pixel-agents egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /pixel-agents init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /pixel-agents version on
...
#   - diff: git working-tree diff; reached via /pixel-agents diff on Slack so it
...
#     /pixel-agents update on Slack. Demoted to free the native slot /approvals now
...
_SLACK_VIA_PIXEL_AGENTS_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/pixel-agents <verb>`` subcommand.
    ...
    can still reach them via ``/pixel-agents <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/pixel-agents`` is always reserved as the first entry so the
    legacy ``/pixel-agents <subcommand>`` form keeps working for anything that
    ...
    # Reserve /pixel-agents as the catch-all top-level command.
    entries.append(("pixel-agents", "Talk to Pixel Agents or run a subcommand", "[subcommand] [args]"))
    seen.add("pixel-agents")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_PIXEL_AGENTS_ONLY:
            # Intentionally Slack-via-/pixel-agents only (see _SLACK_VIA_PIXEL_AGENTS_ONLY).
            return
    ...
    # everything except /pixel-agents, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://pixel-agents.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /pixel-agents handler.

    Maps both canonical names and aliases so /pixel-agents bg do stuff works
    the same as /pixel-agents background do stuff.

    Plugin-registered slash commands are included so ``/pixel-agents <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from pixel_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
