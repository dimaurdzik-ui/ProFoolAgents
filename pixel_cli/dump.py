"""
Dump command for pixel-agents CLI.

Outputs a compact, plain-text summary of the user's Pixel Agents setup
...
from pixel_cli.config import get_pixel_agents_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.pixel-agents/.env.
    ...
    exports. ``pixel-agents debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.pixel_build_sha`` by the Dockerfile's
    ``PIXEL_AGENTS_GIT_SHA`` build-arg (see ``pixel_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``pixel-agents dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from pixel_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(pixel_home: Path) -> int:
...
def _cron_summary(pixel_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = pixel_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines = []
    lines.append("--- pixel-agents dump ---")
    ...
    lines.append(f"pixel_home:      {display_pixel_agents_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.pixel-agents/.env: a managed
        ...
        # A credential added via `pixel-agents auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `pixel-agents auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["pixel-agents-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(pixel_home)}")
    lines.append(f"  skills:             {_count_skills(pixel_home)}")
