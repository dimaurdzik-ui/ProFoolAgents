"""
Configuration management for Pixel Agents.

Config files are stored in ~/.pixel-agents/ for easy access:
- ~/.pixel-agents/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.pixel-agents/.env         - API keys and secrets

This module provides:
- pixel-agents config          - Show current configuration
- pixel-agents config edit     - Open config in editor
- pixel-agents config get      - Print a resolved configuration value
- pixel-agents config set      - Set a specific value
- pixel-agents config unset    - Remove a user configuration value
- pixel-agents config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``pixel-agents config set`` (which rewrites
    ...
    ``config.yaml`` in place: pixel-agents never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.pixel-agents/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``pixel-agents logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  pixel-agents config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Pixel Agents makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Pixel Agents itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``pixel-agents update``, the TUI build.
...
#   on every plugin install / ``pixel-agents update``.
...
# * ``PIXEL_AGENTS_HOME`` / ``PIXEL_AGENTS_PROFILE`` / ``PIXEL_AGENTS_CONFIG`` /
#   ``PIXEL_AGENTS_ENV`` — Pixel Agents runtime location flags. Writing these into
...
#
# IMPORTANT: ``PIXEL_AGENTS_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (PIXEL_AGENTS_LANGFUSE_PUBLIC_KEY,
# PIXEL_AGENTS_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "PYTHONEXECUTABLE", "PYTHONPIXELERSITE",
    # Node
    ...
    # Pixel Agents runtime location — never via dashboard env writer.
    # NOT a PIXEL_AGENTS_* blanket: integration credentials (PIXEL_AGENTS_GEMINI_*,
    # PIXEL_AGENTS_LANGFUSE_*, PIXEL_AGENTS_SPOTIFY_*, ...) ARE allowed.
    "PIXEL_AGENTS_HOME", "PIXEL_AGENTS_PROFILE", "PIXEL_AGENTS_CONFIG", "PIXEL_AGENTS_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Pixel Agents runtime location "
            "(PIXEL_AGENTS_HOME, PIXEL_AGENTS_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.pixel-agents/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # PIXEL_AGENTS_TOOL_PROGRESS_MODE is deprecated (replaced by display.tool_progress
    ...
    # Activation is via plugins.enabled (opt-in through `pixel-agents plugins enable
    # observability/langfuse` or `pixel-agents tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "PIXEL_AGENTS_LANGFUSE_ENV",
    "PIXEL_AGENTS_LANGFUSE_RELEASE",
    "PIXEL_AGENTS_LANGFUSE_SAMPLE_RATE",
    "PIXEL_AGENTS_LANGFUSE_MAX_CHARS",
    "PIXEL_AGENTS_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from pixel_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
# from `nix run` / `nix profile install` (which don't set PIXEL_AGENTS_MANAGED).
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("PIXEL_AGENTS_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_pixel_agents_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Pixel Agents is running in package-manager-managed mode.

    Two signals: the PIXEL_AGENTS_MANAGED env var (set by the systemd service),
    or a .managed marker file in PIXEL_AGENTS_HOME (set by the NixOS activation
...
_NIX_UPDATE_MSG = (
    "Update Pixel Agents through the Nix source that installed it "
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``pixel_cli/`` — i.e. the git checkout for source
    installs, ``/opt/pixel-agents`` inside the published image. It is a property of
    the running interpreter, NOT of ``$PIXEL_AGENTS_HOME``, which is why a
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Pixel Agents was installed: 'docker', 'nix', 'nixos', 'git', or 'unknown'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$PIXEL_AGENTS_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. PIXEL_AGENTS_MANAGED env / .managed marker (NixOS managed mode)
    4. /nix/store/ path detection -> 'nix' (nix run / nix profile install)
    5. .git directory presence -> 'git'
    6. Fallback -> 'unknown'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.pixel-agents``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$PIXEL_AGENTS_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.pixel-agents:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$PIXEL_AGENTS_HOME``, a
    ...
    ``docker`` and ``pixel-agents update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``pixelagents/pixel-agents`` image bakes a ``docker``
        stamp into ``/opt/pixel-agents`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $PIXEL_AGENTS_HOME.
    try:
    ...
    #    host install whose shared $PIXEL_AGENTS_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``pixel-agents update``.
    try:
        method = (
            (get_pixel_agents_home() / ".install_method")
    ...
    # detect Nix installs that don't set PIXEL_AGENTS_MANAGED (e.g. ``nix run``,
...
def _running_in_container() -> bool:
    """Thin wrapper around ``pixel_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$PIXEL_AGENTS_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/pixel-agents`` in the published image, which instead bakes the stamp at
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method in {"nix", "nixos"}:
        return _NIX_UPDATE_MSG
    if method == "docker":
        return "docker pull pixelagents/pixel-agents:latest"
    return "pixel-agents update"
...
# Long-form text for ``pixel-agents update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# pixel_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Pixel Agents, it doesn't update
...
✗ ``pixel-agents update`` doesn't apply inside the Docker container.

Pixel Agents runs as a published image (pixelagents/pixel-agents), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull pixelagents/pixel-agents:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate pixel-agents
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm pixelagents/pixel-agents:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/pixelagents/pixel-agents/tags
  • Your config and session history live under ``$PIXEL_AGENTS_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``pixel-agents update`` inside Docker.
...
def format_managed_message(action: str = "modify this Pixel Agents installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("PIXEL_AGENTS_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Pixel Agents installation is managed by NixOS "
            f"(PIXEL_AGENTS_MANAGED={env_hint}).\n"
            "Edit services.pixel-agents.settings in your configuration.nix and run:\n"
            "  sudo nixos-rebuild switch"
        )

    return (
        f"Cannot {action}: this Pixel Agents installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Pixel Agents."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from PIXEL_AGENTS_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, pixel_bin
    or None if container mode is not active, we're already inside the
    container, or PIXEL_AGENTS_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("PIXEL_AGENTS_DEV") == "1":
        return None
    ...
    container_mode_file = get_pixel_agents_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "pixel-agents")
    exec_user = info.get("exec_user", "pixel-agents")
    pixel_bin = info.get("pixel_bin", "/data/current-package/bin/pixel-agents")
...
# Re-export from pixel_constants — canonical definition lives there.
from pixel_constants import get_pixel_agents_home, get_process_pixel_agents_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_pixel_agents_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_pixel_agents_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_pixel_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the PIXEL_AGENTS_UID / PIXEL_AGENTS_GID env vars set by Docker deployments.

    Docker containers running Pixel Agents commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level PIXEL_AGENTS_HOME once, but
    subdirectories created at runtime by ``ensure_pixel_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("PIXEL_AGENTS_UID", "").strip()
    gid_str = os.environ.get("PIXEL_AGENTS_GID", "").strip()
    try:
...
def _chown_to_pixel_uid(path) -> None:
    """Chown ``path`` to ``PIXEL_AGENTS_UID:PIXEL_AGENTS_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_pixel_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_pixel_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the pixel-agents group can
    share state with the gateway service.

    The mode can be overridden via the PIXEL_AGENTS_HOME_MODE environment variable
    (e.g. PIXEL_AGENTS_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse PIXEL_AGENTS_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``PIXEL_AGENTS_UID``/``PIXEL_AGENTS_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("PIXEL_AGENTS_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_pixel_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Pixel Agents runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("PIXEL_AGENTS_CONTAINER") or os.environ.get("PIXEL_AGENTS_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set PIXEL_AGENTS_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into PIXEL_AGENTS_HOME, upgrading legacy empty templates.
...
# ensure_pixel_home(). Only successful passes are recorded, so a raised
# managed-mode/missing-profile error keeps re-checking on later loads.
_PIXEL_AGENTS_HOME_ENSURED: set = set()


def ensure_pixel_home():
    """Ensure ~/.pixel-agents directory structure exists with secure permissions.
    ...
    After the first successful pass for a given ``PIXEL_AGENTS_HOME`` we only re-run
    ...
    ``get_pixel_agents_home()`` and therefore re-run for the new path.
    """
    home = get_pixel_agents_home()
    key = str(home)

    if key in _PIXEL_AGENTS_HOME_ENSURED and home.is_dir():
        return
    # Named profiles must be created explicitly (e.g. ``pixel-agents profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old PIXEL_AGENTS_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_pixel_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
    ...
    _PIXEL_AGENTS_HOME_ENSURED.add(key)


def _ensure_pixel_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"PIXEL_AGENTS_HOME {home} does not exist. "
...
from pixel_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: F401
...
# selection step (Pixel Portal / OpenRouter / Custom endpoint), so this
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `pixel-agents config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    api_keys = [
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.pixel-agents.config`` entries, then checks
    ...
    try:
        ...
        # should never break `pixel-agents update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Pixel Agents' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Pixel Agents versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``pixel_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (pixel_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard (pixel_cli/setup.py)
    "platform_toolsets",     # written by the setup wizard (pixel_cli/setup.py)
    "known_plugin_toolsets", # written/read by pixel_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'pixel-agents setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Pixel Agents won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # scalars are bridged into os.environ (gateway/run.py, pixel-agents send) so
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'pixel-agents doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        ...
        hint_path = display_pixel_agents_home()
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "pixel-agents update / pixel-agents -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from pixel_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ pixel-agents config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
        # ── Versioned migration ladder (table-driven) ──
        # The per-version steps live in pixel_cli.config_migrations as a
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from pixel_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from pixel_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: pixel-agents config set <key> <value>")
    ...
    # `pixel-agents update`, but only the version bump is persisted.
    ...
    # metadata.pixel-agents.config in their SKILL.md frontmatter.
    ...
    * ``${env:VAR}`` — Cursor-style SecretRef, same resolution after the
    ...
    if inner.startswith("env:"):
        ...
        if val is not None:
            return val
        logger.warning(
            "Config ref %r: %s is not set (check ~/.pixel-agents/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_pixel_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``pixel-agents config set``
    ...
    backends) — while display paths (``pixel-agents status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _pixel_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.pixel-agents/config.yaml as-is, without merging defaults or migrating.
...
def read_user_config_raw(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    ``_pixel_home``, tui profile override, multi-profile probes).
    """
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.pixel-agents/config.yaml.
    ...
    (which change ``PIXEL_AGENTS_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/pixel-agents/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_pixel_dotenv()
...
#   openai-codex (OAuth — pixel-agents auth) — OpenAI Codex
#   pixel         (OAuth — pixel-agents auth) — Pixel Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.pixel-agents/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Pixel Agents writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.pixel-agents/.env.
    ...
    interactive menu render (`pixel-agents tools`, `pixel-agents setup`, status
    panels). Sanitisation is O(lines), so re-parsing the
    same file on every call was burning ~300ms of CPU per `pixel-agents tools`
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.pixel-agents/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.pixel-agents/.env."""
    if is_managed():
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    - The fixed ``PIXEL_AGENTS_CUSTOM_`` prefix keeps the result a valid POSIX name
    ...
    return f"PIXEL_AGENTS_CUSTOM_{slug}_API_KEY" if slug else "PIXEL_AGENTS_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.pixel-agents/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.pixel-agents/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Pixel Agents — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Pixel Agents vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.pixel-agents/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.pixel-agents/.env`` over ``os.environ``.

    Used for Pixel Agents-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    print(color("│              ⚕ Pixel Agents Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from pixel_cli.auth import get_anthropic_key
    ...
    # Warn on stale PIXEL_AGENTS_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("PIXEL_AGENTS_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale PIXEL_AGENTS_MAX_ITERATIONS={_env_ghost} "
                f"(run 'pixel-agents doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  pixel-agents config edit     # Edit config file", Colors.DIM))
    print(color("  pixel-agents config set <key> <value>", Colors.DIM))
    print(color("  pixel-agents setup           # Run setup wizard", Colors.DIM))
    print()
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "changed model/provider. Inspect with `pixel-agents cron list`, then pin the "
...
def _known_top_level_keys() -> set[str]:
    ...
    to decide whether a ``pixel-agents config set`` invocation is targeting a
    known shape.
    """
...
def _suggest_closest_key(key: str, candidates: set[str], cutoff: float = 0.6) -> Optional[str]:
    ...
    similar enough to ``key``, else None.  Used by ``pixel-agents config set``
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    Args:
        ...
        force: When True, skip the unknown-key warning — useful for scripted
            ...
            exposes this via ``pixel-agents config set --force``.
    """
    if is_managed():
    ...
    # so a fresh `pixel-agents config set model.api_base ...` lands on the canonical
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_pixel_agents_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
    ...
    # — e.g. `pixel-agents config set model.api_key cfut_...` routes to config.yaml
    ...
    if not is_known and not force:
        print(color(
            ...
            "but Pixel Agents may not read it.",
            Colors.YELLOW,
        ))
        if suggestion:
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # model-cache rows too, so `pixel-agents config unset <KEY>` fully removes
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key:
            print("Usage: pixel-agents config get <key> [--json]")
            print()
            print("Examples:")
            print("  pixel-agents config get model")
            print("  pixel-agents config get terminal.backend")
            print("  pixel-agents config get skills.config --json")
            sys.exit(1)
        ...
        if not key or value is None:
            print("Usage: pixel-agents config set [--force] <key> <value>")
            print()
            print("Examples:")
            print("  pixel-agents config set model anthropic/claude-sonnet-4")
            print("  pixel-agents config set terminal.backend docker")
            print("  pixel-agents config set OPENROUTER_API_KEY sk-or-...")
            print()
        ...
        key = getattr(args, 'key', None)
        if not key:
            print("Usage: pixel-agents config unset <key>")
            print()
            print("Examples:")
            print("  pixel-agents config unset model")
            print("  pixel-agents config unset terminal.backend")
            print("  pixel-agents config unset OPENROUTER_API_KEY")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'pixel-agents config migrate' to add them")
        ...
        print("  pixel-agents config           Show current configuration")
        print("  pixel-agents config edit      Open config in editor")
        print("  pixel-agents config get <key>          Print a resolved config value")
        print("  pixel-agents config set <key> <value>   Set a config value")
        print("  pixel-agents config unset <key>        Remove a config value")
        print("  pixel-agents config check     Check for missing/outdated config")
        print("  pixel-agents config migrate   Update config with new options")
        print("  pixel-agents config path      Show config file path")
        print("  pixel-agents config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``pixel-agents config`` UI so users
