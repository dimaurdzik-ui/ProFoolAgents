...
late-binding seam in :mod:`pixel_cli.web_deps` (``late`` for callables,
...
from pixel_cli.web_deps import late, LateState
from pixel_cli.web_models import (
...
_log = logging.getLogger("pixel_cli.web_server")
...
_spawn_pixel_action = late("_spawn_pixel_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from pixel_cli.platforms import platform_label
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``pixel-agents tools`` picker shows
    ...
    with _profile_scope(profile):
        ...
        if cat:
            ...
            # re-probing per row.
            features = get_pixel_subscription_features(config, force_fresh=True)
            for prov in _visible_providers(cat, config, force_fresh=True):
                ...
                row = {
                    ...
                    "requires_pixel_auth": bool(prov.get("requires_pixel_auth")),
                    "is_active": is_active,
                    # Honest server-side readiness. The GUI's old client-side
                    # heuristic showed "Ready" for every zero-env-var row —
                    # including logged-out Pixel Subscription rows and never-run
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `pixel-agents tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``pixel-agents tools``
    ...
    Managed Pixel rows (``managed_pixel_feature``) additionally report the
    Portal entitlement state: the CLI flow gates these selections on
    ``ensure_pixel_portal_access`` (inline login), but the GUI has no inline
    ...
    ``managed_by_pixel``). The response now carries an additive
    ``needs_pixel_auth: true`` + ``feature`` so the client can drive the
    existing Pixel Portal OAuth flow (``POST /api/providers/oauth/pixel/start``)
    and refetch.
    """
    from pixel_cli.tools_config import (
        TOOL_CATEGORIES,
    ...
    with _profile_scope(body.profile or profile):
        ...
        # Entitlement check for managed Pixel rows — mirrors the gate the CLI
        # applies via ensure_pixel_portal_access at selection time.
        cat = TOOL_CATEGORIES.get(name)
        row = None
        if cat:
            row = next(
        ...
        managed_feature = (row or {}).get("managed_pixel_feature")
        if managed_feature:
            features = get_pixel_subscription_features(config, force_fresh=True)
            ...
            if not entitled:
                response["needs_pixel_auth"] = True
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.pixel-agents/.env`` via ``save_env_value`` —
    the same store ``pixel-agents tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``pixel-agents tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``pixel-agents -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same PIXEL_AGENTS_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``pixel-agents computer-use permissions grant`` as a background action.
