...
def get_sessions(
    ...
    try:
        ...
        try:
            ...
            # `pixel-agents serve` backend. No-op when disabled or run recently.
...
async def search_sessions(
    ...
    try:
        ...
        try:
            ...
            # logs, or another Pixel Agents surface. FTS can't find those unless the
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Pixel Agents backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `pixel-agents sessions stats`).
