...
late-binding seam in :mod:`pixel_cli.web_deps` so tests that
...
from pixel_cli.web_deps import late
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's PIXEL_AGENTS_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``pixel-agents profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from PIXEL_AGENTS_HOME).
    """
    from pixel_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``pixel-agents profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the PIXEL_AGENTS_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``pixel-agents profile describe
    <name> --auto``.
    ...
    try:
        from pixel_cli import profile_describer
