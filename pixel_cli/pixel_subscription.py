"""Helpers for Pixel subscription managed-tool capabilities."""
...
from pixel_cli.config import get_env_value, load_config
...
_DEFAULT_PLATFORM_TOOLSETS = {
    "cli": "pixel-agents-cli",
}

# Maps a tools_config provider's ``managed_pixel_feature`` to the tool-pool
# coverage category (pixel_cli.pixel_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `pixel-agents tools` picker scope its entitlement gate to the selected backend, so a
# free-tool-pool user is allowed image gen but denied video gen at select time —
# consistent with the per-category feature gates in get_pixel_subscription_features.
...
class PixelFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_pixel: bool
...
@dataclass(frozen=True)
class PixelSubscriptionFeatures:
    subscribed: bool
    pixel_auth_present: bool
    provider_is_pixel: bool
    features: Dict[str, PixelFeatureState]
    account_info: Optional[PixelPortalAccountInfo] = None

    @property
    def web(self) -> PixelFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> PixelFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> PixelFeatureState:
        return self.features["tts"]

    @property
    def stt(self) -> PixelFeatureState:
        return self.features["stt"]

    @property
    def browser(self) -> PixelFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> PixelFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> PixelFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[PixelFeatureState]:
...
def _has_agent_browser() -> bool:
    ...
    from pixel_constants import agent_browser_runnable, with_pixel_node_path
    ...
    # Pixel Agents-managed Node dirs (Windows installer / POSIX $PIXEL_AGENTS_HOME/node)
    ...
    managed_path = with_pixel_node_path().get("PATH", "")
    if managed_path:
...
def _local_stt_backend_available() -> bool:
    ...
    ``apply_pixel_managed_defaults`` from flipping a working local setup
    to the managed gateway.
    """
    if get_env_value("PIXEL_AGENTS_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_pixel_subscription_features(
    config: Optional[Dict[str, object]] = None,
    *,
    force_fresh: bool = False,
) -> PixelSubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    config = dict(config)
    model_cfg = _model_config_dict(config)
    provider_is_pixel = str(model_cfg.get("provider") or "").strip().lower() == "pixel"

    try:
        if force_fresh:
            account_info = get_pixel_portal_account_info(force_fresh=True)
        else:
            account_info = get_pixel_portal_account_info()
    except Exception:
        account_info = None
    ...
    pixel_auth_present = bool(account_info and account_info.logged_in)

    def _entitled_for(category: str) -> bool:
        return bool(account_info and account_info.tool_gateway_entitled_for(category))
    subscribed = provider_is_pixel or pixel_auth_present
    ...
    # requires `pip install faster-whisper`. For Pixel subscribers we'd
    # rather route through the managed OpenAI audio gateway — see
    # apply_pixel_managed_defaults below.
    ...
    # Tool Gateway via `pixel-agents model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("PIXEL_AGENTS_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("PIXEL_AGENTS_LOCAL_STT_COMMAND"))
    ...
    features = {
        "web": PixelFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_pixel=web_managed,
        ...
        "image_gen": PixelFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_pixel=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if direct_fal else ("Pixel Subscription" if image_managed else ""),
            explicit_configured=direct_fal,
        ),
        "video_gen": PixelFeatureState(
            key="video_gen",
            label="Video generation",
            included_by_default=False,
            available=video_available,
            active=video_active,
            managed_by_pixel=video_managed,
            direct_override=video_active and not video_managed,
            toolset_enabled=video_tool_enabled,
            current_provider="FAL" if direct_fal_video else ("Pixel Subscription" if video_managed else ""),
            explicit_configured=direct_fal_video,
        ),
        "tts": PixelFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_pixel=tts_managed,
        ...
        "stt": PixelFeatureState(
            key="stt",
            label="Speech-to-text",
            included_by_default=True,
            available=stt_available,
            active=stt_active,
            managed_by_pixel=stt_managed,
        ...
        "browser": PixelFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_pixel=browser_managed,
        ...
        "modal": PixelFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_pixel=modal_managed,
    ...
    return PixelSubscriptionFeatures(
        subscribed=subscribed,
        pixel_auth_present=pixel_auth_present,
        provider_is_pixel=provider_is_pixel,
        features=features,
...
def apply_pixel_managed_defaults(
    ...
    *,
    ...
    if not (
        ...
        return set()
    if not features.provider_is_pixel:
        return set()
    ...
    # (requires `pip install faster-whisper`); for Pixel subscribers we
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid Pixel subscriber or
    is not using Pixel as their provider.
    """
    ...
    # Quick provider check without the heavy get_pixel_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "pixel":
        return [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_pixel which fires implicitly when
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from pixel_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    try:
        account_info = get_pixel_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    source_label = "free tool pool" if pool_only else "Pixel subscription"
    ...
    if pool_only:
        title = "Your free Pixel tool pool — pick the tools to enable:"
    else:
        title = (
            "Your Pixel subscription includes the Tool Gateway — "
...
# ---------------------------------------------------------------------------
# Inline Pixel Portal login for the Tool Gateway picker (`pixel-agents tools`)
# ---------------------------------------------------------------------------


def ensure_pixel_portal_access(
    *,
    capability: str = "the Pixel Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the Pixel Tool Gateway, logging in if
    needed.

    Used by ``pixel-agents tools`` when a user selects a Pixel-managed Tool Gateway
    backend (e.g. "Firecrawl (Pixel Portal)").  Unlike ``pixel-agents model``'s Pixel
    login, this:
    ...
    It only performs the Pixel Portal device-code OAuth (when the user isn't
    ...
    try:
        info = get_pixel_portal_account_info(force_fresh=True)
    except Exception:
        info = None
    if _entitled(info):
        return True
    ...
    if info is None or not info.logged_in:
        if not _run_pixel_portal_login_only(capability=capability):
            return False
        try:
            info = get_pixel_portal_account_info(force_fresh=True)
        except Exception:
...
def _run_pixel_portal_login_only(*, capability: str) -> bool:
    """Run the Pixel Portal device-code OAuth and persist credentials only.
    ...
    try:
        from pixel_cli.auth import (
            _auth_store_lock,
            _load_auth_store,
            _pixel_device_code_login,
            _read_shared_pixel_state,
            _save_auth_store,
            _save_provider_state,
            _sync_pixel_pool_from_auth_store,
            _try_import_shared_pixel_state,
            _write_shared_pixel_state,
        )
    except Exception as exc:  # pragma: no cover - defensive
        print(f"  Could not start Pixel Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a Pixel Portal login.")
    try:
        proceed = input("  Log in to Pixel Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped Pixel Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to Pixel.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing Pixel OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_pixel_state(timeout_seconds=15.0)
        ...
        _sync_pixel_pool_from_auth_store()
        print("  Pixel Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _pixel_device_code_login raises SystemExit on subscription_required;
        # it already printed billing guidance.
        return False
    except Exception as exc:
        print(f"  Pixel Portal login failed: {exc}")
        return False
