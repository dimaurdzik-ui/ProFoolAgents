...
Add, remove, or reorder entries here — both `pixel-agents setup` and
`pixel-agents` provider-selection will pick up the change automatically.
"""
...
from pixel_cli.urllib_security import open_credentialed_url

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_PIXEL_AGENTS_USER_AGENT = f"pixel-agents-cli/{_PIXEL_AGENTS_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `pixel-agents model`
    flow without maintaining a separate static list.
    """
    from pixel_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $PIXEL_AGENTS_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Pixel Agents-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $PIXEL_AGENTS_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Pixel Agents refreshes the cache from
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see pixel_cli/model_setup_flows.py).
...
# ---------------------------------------------------------------------------
# Pixel Portal free-model helper
# ---------------------------------------------------------------------------
# The Pixel Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Pixel Portal account tier detection
# ---------------------------------------------------------------------------
def is_pixel_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_pixel_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split Pixel models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/pixel/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["pixel"]`` list happens to contain or whether the
    ...
    older Pixel Agents that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/pixel/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["pixel"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Pixel Agents that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_pixel_models_by_tier` keeps free models selectable;
...
def check_pixel_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Pixel Portal user is on a free (unpaid) tier.
    ...
    try:
        ...
        account_info = get_pixel_portal_account_info(force_fresh=force_fresh)
...
# ---------------------------------------------------------------------------
# Pixel Portal recommended models
#
...
PIXEL_RECOMMENDED_MODELS_PATH = "/api/pixel/recommended-models"
_PIXEL_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_pixel_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def _pixel_recommended_disk_path() -> "Path":
    """Disk path for the persisted recommended-models cache."""
    from pixel_constants import get_pixel_agents_home
    return get_pixel_agents_home() / "cache" / "pixel_recommended_cache.json"


def _read_pixel_recommended_disk(base: str) -> dict[str, Any] | None:
    ...
    """
    try:
        with open(_pixel_recommended_disk_path(), encoding="utf-8") as fh:
            blob = json.load(fh)
...
def _write_pixel_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    if not data:
        return
    path = _pixel_recommended_disk_path()
    try:
        try:
        ...
        logging.getLogger(__name__).debug(
            "pixel recommended-models disk cache write failed: %s", exc
        )


def fetch_pixel_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the Pixel Portal's curated recommended-models payload.

    Hits ``<portal>/api/pixel/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_PIXEL_RECOMMENDED_CACHE_TTL`` seconds in process; pass
    ``force_refresh=True`` to bypass the in-process cache.

    A successful live fetch is also persisted to a per-base disk cache
    (``$PIXEL_AGENTS_HOME/cache/pixel_recommended_cache.json``) as last-known-good.
    ...
    """
    base = (portal_base_url or "https://portal.pixelagents.com").rstrip("/")
    now = time.monotonic()
    cached = _pixel_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _PIXEL_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{PIXEL_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    if data:
        ...
        _pixel_recommended_cache[base] = (data, now)
        _write_pixel_recommended_disk(base, data)
        return data
    ...
    disk = _read_pixel_recommended_disk(base)
    if disk:
        _pixel_recommended_cache[base] = (disk, now)
        return disk
...
def _resolve_pixel_portal_url() -> str:
    ...
    try:
        ...
        return str(DEFAULT_PIXEL_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://portal.pixelagents.com"
...
def get_pixel_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_pixel_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_pixel_portal_url()
    payload = fetch_pixel_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None

    if free_tier is None:
        try:
            free_tier = check_pixel_free_tier()
        except Exception:
...
# this list:  pixel-agents model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `pixel-agents model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `pixel-agents model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("pixel",           "Pixel Portal",              "Pixel Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
#
# Some vendors expose several Pixel Agents provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `pixel-agents model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``pixel-agents model``, the
...
# In-repo fallback for the model Pixel Agents silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from pixel_cli.model_catalog import get_default_model_from_cache
...
# curated-list entry [0]. Metered aggregators (Pixel Portal, OpenRouter) order
...
# ``pixel-agents model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in pixel_cli/web_server.py and
# ``partition_pixel_models_by_tier`` — which can hit the Portal.
_SILENT_DEFAULT_PROVIDERS: frozenset[str] = frozenset({"pixel", "openrouter"})
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``pixel-agents auth add openai-codex`` without
    ``pixel-agents model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``pixel-agents model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    pixel-agents is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Pixel Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from pixel_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — pixel-agents
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Pixel Agents lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_pixel_model_ids() -> list[str]:
    """Return the curated Pixel Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["pixel"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from pixel_cli.model_catalog import get_curated_pixel_models
        remote = get_curated_pixel_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("pixel", []))
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from pixel_constants import AI_GATEWAY_BASE_URL
...
def compute_sale_discount(
    ...
    Pixel Portal-only feature: callers gate on the provider; this helper only
    sees ``original`` because the Pixel fetch path opted in via
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Pixel Portal).

    When *include_sale_original* is true (Pixel Portal only) and the gateway
    ...
    headers: dict[str, str] = {
        ...
        "User-Agent": _PIXEL_AGENTS_USER_AGENT,
    }
    if api_key:
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome is Pixel Portal-only. Never copy pricing.original for
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return pixel-agents-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; pixel-agents's picker expects
...
_DEFAULT_PIXEL_INFERENCE_BASE = "https://inference-api.pixelagents.com"


def _resolve_pixel_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Pixel Portal pricing.

    The Pixel inference ``/v1/models`` endpoint exposes pricing without
    ...
    1. ``PIXEL_INFERENCE_BASE_URL`` env override (staging / preview)
    ...
    try:
        ...
        env_base = _pixel_inference_env_override()
    except Exception:
        env_base = None
    ...
    base_url = (env_base or creds_base or _DEFAULT_PIXEL_INFERENCE_BASE).rstrip("/")
    return (api_key, base_url)


def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, pixel, ai-gateway, novita)."""
    ...
    if normalized == "pixel":
        api_key, base_url = _resolve_pixel_pricing_credentials()
        if base_url:
            # Pixel base_url typically looks like https://inference-api.pixelagents.com/v1
            ...
            return fetch_models_with_pricing(
                ...
                base_url=stripped,
                force_refresh=force_refresh,
                # Sale chrome (pricing.original) is Pixel Portal-only.
                include_sale_original=True,
            )
    return {}
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``pixel-agents model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from pixel_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        pixel:pixel-agents-3                           →  ("pixel", "pixel-agents-3")
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Pixel, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"pixel", "openrouter", "ai-gateway", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from pixel_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model pixel` or `/model anthropic`, treat it as a
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if (
            ...
            # curated list is ordered most-capable-first (e.g. Pixel Portal),
            # entry [0] is the priciest flagship, and typing ``/model pixel``
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Pixel Agents' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``pixel_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Pixel Agents should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``pixel-agents auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.pixel-agents/.env``.
    ...
    """
    try:
        from pixel_cli.auth import resolve_api_key_provider_credentials
...
#   - "pixel": curated list and Portal /models endpoint are the source of
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from pixel_cli.model_search import model_alias_canonical
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Pixel),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Pixel Agents release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Pixel Agents release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "pixel":
        # Try live Pixel Portal /models endpoint
        try:
            from pixel_cli.auth import fetch_pixel_models, resolve_pixel_runtime_credentials
            creds = resolve_pixel_runtime_credentials()
            if creds:
                live = fetch_pixel_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
        # Live failed (or no creds). Fall back to the docs-hosted manifest
        # — NOT the in-repo _PROVIDER_MODELS["pixel"] snapshot — so newly
        # added Portal models still surface without a Pixel Agents release.
        manifest_ids = get_curated_pixel_model_ids()
        if manifest_ids:
            return manifest_ids
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``pixel-agents model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $PIXEL_AGENTS_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from pixel_constants import get_pixel_agents_home
    return get_pixel_agents_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, pixel portal), the
    relevant tokens live in ``$PIXEL_AGENTS_HOME/auth.json`` and external
...
def copilot_default_headers(*, is_agent_turn: bool = True) -> dict[str, str]:
    ...
    try:
        ...
        return {
            ...
            "User-Agent": "PixelAgentsAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _PIXEL_AGENTS_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Pixel Agents' default Claude IDs elsewhere use
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Pixel Agents' native Anthropic adapter. The live catalog may
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _PIXEL_AGENTS_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"pixel-agents/{_PIXEL_AGENTS_VERSION}"
    ...
    if isinstance(request_headers, dict):
        ...
        from pixel_cli.config import normalize_extra_headers
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=pixel-agents"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from pixel_constants import get_pixel_agents_home
    return get_pixel_agents_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `pixel-agents moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Pixel Agents will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Pixel Agents cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Pixel Agents actually ships.  (#46850)
