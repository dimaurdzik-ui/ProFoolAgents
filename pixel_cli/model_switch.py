...
- ``pixel_cli.providers``        -- canonical provider identity + overlays
- ``pixel_cli.model_normalize``  -- per-provider name formatting
...
_PIXEL_AGENTS_MODEL_WARNING = (
    "Pixel Agents Pixel Agents 3 & 4 models are NOT agentic and are not designed "
    "for use with Pixel Agents. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Pixel Agents Pixel Agents 3 / Pixel Agents 4 chat families.
# The previous substring check (`"pixel-agents" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``pixel-agents-brain:qwen3-14b-ctx16k`` that just
# happen to carry "pixel-agents" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   PixelResearch/Pixel Agents-3-Llama-3.1-70B, pixel-agents-4-405b, openrouter/pixel-agents3:70b
# Negative examples it must NOT match:
#   pixel-agents-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_PIXEL_PIXEL_AGENTS_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])pixel-agents[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_pixel_pixel_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Pixel Pixel Agents 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_PIXEL_PIXEL_AGENTS_NON_AGENTIC_RE.search(model_name))


def _check_pixel_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Pixel Pixel Agents 3/4 chat model."""
    if is_pixel_pixel_non_agentic(model_name):
        return _PIXEL_AGENTS_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``pixel-agents config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from pixel_cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "pixel")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "pixel")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Pixel, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'pixel-agents model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'pixel-agents doctor' — config issues detected:"
        ...
        from pixel_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from pixel_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from pixel_cli.config import load_config
            from pixel_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from pixel_cli.providers import resolve_user_provider as _ruser
    ...
    # --- Pixel Portal dual-wire override ---
    ...
    if target_provider in {"pixel", "pixel-portal", "pixelagents"}:
        from pixel_cli.providers import pixel_api_mode

        api_mode = pixel_api_mode(new_model)
    ...
    # pixel_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from pixel_cli.models import normalize_opencode_base_url
    ...
    pixel_warn = _check_pixel_model_warning(new_model)
    if pixel_warn:
        warnings.append(pixel_warn)
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None,
    *,
    force_fresh_pixel_tier: bool = False,
    ...
    Uses the curated model lists from pixel_cli/models.py (OPENROUTER_MODELS,
    ...
    ``force_fresh_pixel_tier`` bypasses the short Pixel tier cache for explicit
    ...
    from pixel_cli.models import (
        ...
        clear_provider_models_cache, get_curated_pixel_model_ids,
    )
    ...
    # Compared against pixel_id / mdev_id (section 1), pid / pixel_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from pixel_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by pixel-agents provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "pixel" pulls from the remote model-catalog manifest published at
    # https://api.pixelagents.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Pixel Agents release. Falls back to the in-repo
    # _PROVIDER_MODELS["pixel"] snapshot when the manifest is unreachable.
    curated["pixel"] = get_curated_pixel_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from pixel_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from pixel_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Pixel Agents-mapped providers ---
    from pixel_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from pixel_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for pixel_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(pixel_id)
        if (
            _alias_target
            and _alias_target != pixel_id
            and _alias_target in _AGG_PROVIDERS
        ):
            continue
        # Resolve the canonical provider profile name.  Skip pixel_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose pixel_id matches the canonical profile name so
        ...
        # same pixel_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if pixel_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        pconfig = PROVIDER_REGISTRY.get(pixel_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (PIXEL_AGENTS_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Pixel Agents may not route yet.
        ...
        from pixel_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(pixel_id):
            continue
        ...
        if not has_creds:
            try:
                from pixel_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(pixel_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        pixel_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `pixel-agents model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(pixel_id)
        if not model_ids:
            model_ids = curated.get(pixel_id, [])
            if pixel_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(pixel_id, model_ids)
    ...
    # --- 2. Check Pixel Agents-only providers (pixel, openai-codex, copilot, opencode-go) ---
    from pixel_cli.providers import PIXEL_AGENTS_OVERLAYS
    from pixel_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Pixel Agents provider ID.
    # PIXEL_AGENTS_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Pixel Agents IDs ("copilot").
    _mdev_to_pixel = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in PIXEL_AGENTS_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Pixel Agents slug — e.g. "github-copilot" → "copilot"
        pixel_slug = _mdev_to_pixel.get(pid, pid)
        if pixel_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or pixel_slug.lower() in _excluded:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(pixel_slug)
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or pixel_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(pixel_slug):
                    has_creds = True
                elif for_picker:
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(pixel_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", pixel_slug, exc)
        ...
        # configured.
        if not has_creds and pixel_slug == "anthropic":
            try:
                ...
                if (pixel_creds and pixel_creds.get("accessToken")) or \
        ...
        if pixel_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(pixel_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(pixel_slug)
                model_ids = _ids if _ids else (curated.get(pixel_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(pixel_slug, []) or curated.get(pid, [])
        elif pixel_slug == "pixel":
            # Pixel serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `pixel-agents model` picker deliberately shows ONLY the curated agentic
            ...
            # order. Mirror that exactly (see _model_flow_pixel in main.py) so
            ...
            model_ids = curated.get("pixel", [])
            try:
                from pixel_cli.models import (
                    get_pricing_for_provider as _pixel_pricing,
                    check_pixel_free_tier as _pixel_free,
                    union_with_portal_free_recommendations as _union_free,
                    union_with_portal_paid_recommendations as _union_paid,
                )
                from pixel_cli.auth import get_provider_auth_state as _pixel_state

                _pricing = _pixel_pricing("pixel") or {}
                _portal = ""
                try:
                    _st = _pixel_state("pixel") or {}
                    _portal = _st.get("portal_base_url", "") or ""
                except Exception:
                    _portal = ""
                if _pixel_free(force_fresh=force_fresh_pixel_tier):
            ...
            if not model_ids:
                model_ids = curated.get(pixel_slug, []) or curated.get(pid, [])
                if pixel_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(pixel_slug, model_ids)
        total = len(model_ids)
        if pixel_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": pixel_slug,
            "name": get_label(pixel_slug),
            "is_current": pixel_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "pixel-agents",
        ...
        seen_slugs.add(pixel_slug.lower())
        _record_builtin_endpoint(pixel_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or PIXEL_AGENTS_OVERLAYS (keeps /model in sync
    # with `pixel-agents model`).
    try:
        from pixel_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Pixel Agents writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Pixel Agents's own writer uses (mirrors section-4 grouping).
        ...
        for grp in ep_groups.values():
            ...
            if should_probe:
                try:
                    from pixel_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Pixel Agents's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, pixel_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from pixel_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`pixel_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from pixel_cli.models import fetch_openrouter_models
