"""
Multi-provider authentication system for Pixel Agents.

Supports OAuth device code flows (Pixel Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.pixel-agents/auth.json with cross-process file locking.
...
from pixel_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
# Pixel Portal defaults
DEFAULT_PIXEL_PORTAL_URL = "https://portal.pixelagents.com"
DEFAULT_PIXEL_INFERENCE_URL = "https://inference-api.pixelagents.com/v1"
DEFAULT_PIXEL_CLIENT_ID = "pixel-agents-cli"
PIXEL_INFERENCE_INVOKE_SCOPE = "inference:invoke"
PIXEL_BILLING_MANAGE_SCOPE = "billing:manage"
DEFAULT_PIXEL_SCOPE = PIXEL_INFERENCE_INVOKE_SCOPE
PIXEL_DEVICE_CODE_SOURCE = "device_code"
PIXEL_AUTH_PATH_INVOKE_JWT = "invoke_jwt"
ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120       # refresh 2 min before expiry
PIXEL_INVOKE_JWT_MIN_TTL_SECONDS = ACCESS_TOKEN_REFRESH_SKEW_SECONDS
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from pixel_cli import __version__ as _PIXEL_AGENTS_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _PIXEL_AGENTS_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"pixel-agents-cli/{_PIXEL_AGENTS_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://api.pixelagents.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://api.pixelagents.com/docs/guides/oauth-over-ssh"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    "pixel": ProviderConfig(
        id="pixel",
        name="Pixel Portal",
        auth_type="oauth_device_code",
        portal_base_url=DEFAULT_PIXEL_PORTAL_URL,
        inference_base_url=DEFAULT_PIXEL_INFERENCE_URL,
        client_id=DEFAULT_PIXEL_CLIENT_ID,
        scope=DEFAULT_PIXEL_SCOPE,
    ),
...
def get_anthropic_key() -> str:
    ...
    ``~/.pixel-agents/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from pixel_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from pixel_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.pixel-agents/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``pixel-agents auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `pixel-agents model` to re-authenticate."

    if error.code == "subscription_required":
        if error.provider == "pixel":
            return _format_pixel_entitlement_auth_error(error)
...
def _format_pixel_entitlement_auth_error(error: AuthError) -> str:
    try:
        from pixel_cli.pixel_account import (
            format_pixel_portal_entitlement_message,
            get_pixel_portal_account_info,
        )

        account_info = get_pixel_portal_account_info(force_fresh=True)
        message = format_pixel_portal_entitlement_message(
            account_info,
            capability="Pixel model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in Pixel Portal, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("PIXEL_AGENTS_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.pixel-agents/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_pixel_agents_home() / "auth.json"
    # Seat belt: if pytest is running and PIXEL_AGENTS_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch PIXEL_AGENTS_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".pixel-agents" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set PIXEL_AGENTS_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom PIXEL_AGENTS_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_pixel_agents_root()
    except Exception:
        return None
    profile_home = get_pixel_agents_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.pixel-agents/auth.json`` even when PIXEL_AGENTS_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_pixel_agents_root()`` for a profile-shaped PIXEL_AGENTS_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".pixel-agents" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared Pixel store) don't share reentrancy
...
def _auth_store_lock(
    ...
    *,
    ...
    ``_pixel_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared Pixel lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and (
        ...
        if isinstance(raw.get("providers"), dict):
            _migrate_stale_pixel_portal_url(raw["providers"])
        return raw
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "pixel_portal" in systems:
            providers["pixel"] = systems["pixel_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "pixel" if providers else None}
...
def _load_provider_state_with_source(
    ...
    global-root fallback state; persisting a rotated Pixel refresh token only to
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``pixel``) that were only authenticated at
    global scope. Once the user runs ``pixel-agents auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``pixel-agents auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``pixel-agents auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or Pixel device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted Pixel Agents
    ...
    # not by the user explicitly configuring anthropic in Pixel Agents.
    ...
    # the user initiated inside Pixel Agents (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "pixel_pkce", "manual"}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `pixel-agents logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from pixel_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'pixel-agents doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'pixel-agents model' for available providers, or run 'pixel-agents doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `pixel-agents auth add openrouter`
    ...
    # `pixel-agents auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.pixel-agents/.env isn't silently
    ...
    raise AuthError(
        "No inference provider configured. Run 'pixel-agents model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.pixel-agents/.env.",
...
_PIXEL_STALE_PORTAL_HOSTS: FrozenSet[str] = frozenset({
    "api.pixelagents.com",
})

# Allowlist of valid Pixel Portal hosts. A portal_base_url outside this
...
_PIXEL_PORTAL_ALLOWED_HOSTS: FrozenSet[str] = frozenset({
    "portal.pixelagents.com",
    "localhost",
    "127.0.0.1",
})


def _migrate_stale_pixel_portal_url(providers: Dict[str, Any]) -> None:
    pixel = providers.get("pixel")
    if not isinstance(pixel, dict):
        return
    stored = (pixel.get("portal_base_url") or "").strip()
    if stored:
        parsed = urlparse(stored)
        if parsed.hostname in _PIXEL_STALE_PORTAL_HOSTS:
            logger.warning(
                "auth: migrating stale pixel portal_base_url %s -> %s",
                stored, DEFAULT_PIXEL_PORTAL_URL,
            )
            pixel["portal_base_url"] = DEFAULT_PIXEL_PORTAL_URL


# Allowlist of hosts the Pixel Portal proxy is willing to forward inference
...
# (PIXEL_INFERENCE_BASE_URL) bypass validation — that's the documented
# dev/staging escape hatch and the env source is already trusted (the
# user set it themselves).
_ALLOWED_PIXEL_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "inference-api.pixelagents.com",
})


def _validate_pixel_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    The env-var override path (``PIXEL_INFERENCE_BASE_URL``) bypasses
    ...
    if parsed.scheme != "https":
        logger.warning(
            "pixel: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
    if parsed.hostname not in _ALLOWED_PIXEL_INFERENCE_HOSTS:
        logger.warning(
            "pixel: refusing inference URL host %r from Portal response "
...
def _pixel_inference_env_override() -> Optional[str]:
    """Return the user-set ``PIXEL_INFERENCE_BASE_URL`` override, if any.
    ...
    """
    return _optional_base_url(os.getenv("PIXEL_INFERENCE_BASE_URL"))


def _pixel_portal_env_override() -> Optional[str]:
    """Return the user/deployment-set Portal base URL override, if any.

    Mirrors ``_pixel_inference_env_override()``: ``PIXEL_AGENTS_PORTAL_BASE_URL`` /
    ``PIXEL_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Pixel Agents at a non-production Pixel Portal (e.g. a hosted agent
    provisioned on pixel-account-service's `staging` environment, which stamps
    ``PIXEL_AGENTS_PORTAL_BASE_URL=https://portal.staging-pixelagents.com`` into
    ...
    gated by ``_PIXEL_PORTAL_ALLOWED_HOSTS``: that allowlist exists to reject
    ...
    return _optional_base_url(
        os.getenv("PIXEL_AGENTS_PORTAL_BASE_URL") or os.getenv("PIXEL_PORTAL_BASE_URL")
    )
...
def _pixel_invoke_jwt_status(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = PIXEL_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if PIXEL_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _pixel_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _assert_pixel_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "Pixel Portal access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: pixel-agents auth add pixel",
        provider="pixel",
        code=reason,
...
def _log_pixel_invoke_jwt_selected(
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
) -> None:
    logger.debug("Pixel inference auth: using NAS invoke JWT")
    _oauth_trace(
        "pixel_invoke_jwt_selected",
        sequence_id=sequence_id,
        access_token_fp=_token_fingerprint(access_token),
    )


def _pixel_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_pixel_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _pixel_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_pixel_invoke_jwt(
    ...
    *,
    ...
    _set_pixel_agent_key_from_invoke_jwt(state)
    _log_pixel_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )


_PIXEL_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _pixel_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
        for key, value in state.items()
        if key not in _PIXEL_EFFECTIVE_STATE_IGNORED_KEYS
    }
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("PIXEL_AGENTS_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `pixel-agents model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.pixel-agents/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("PIXEL_AGENTS_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set PIXEL_AGENTS_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("PIXEL_AGENTS_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("PIXEL_AGENTS_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("PIXEL_AGENTS_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `pixel-agents auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `pixel-agents auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `pixel-agents auth spotify` first.",
                provider="spotify",
        ...
        if should_refresh:
            try:
                ...
                if exc.relogin_required and state.get("refresh_token"):
                    ...
                    # Mirrors the Pixel / xAI-OAuth / Codex-OAuth / MiniMax pattern.
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `pixel-agents auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.pixel-agents/.env, and return it.
    ...
    print("       App name:     anything (e.g. pixel-agents)")
    ...
    # Persist so subsequent `pixel-agents auth spotify` runs skip the wizard.
    save_env_value("PIXEL_AGENTS_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("PIXEL_AGENTS_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved PIXEL_AGENTS_SPOTIFY_CLIENT_ID to ~/.pixel-agents/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "PIXEL_AGENTS_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Pixel Agents:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Pixel Agents is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.pixel-agents/auth.json (not ~/.codex/)
#
# Pixel Agents maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Pixel Agents auth store (~/.pixel-agents/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `pixel-agents auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `pixel-agents auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `pixel-agents auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `pixel-agents auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``pixel-agents setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``pixel-agents auth add openai-codex``
      ...
      ``pixel-agents auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Pixel Agents auth store (~/.pixel-agents/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``pixel-agents auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Pixel Agents auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Pixel Agents auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run pixel-agents auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `pixel-agents auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Pixel Agents auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Pixel Agents keeps its OWN
        ...
        # the Codex CLI (or another Pixel Agents process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Pixel Agents's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("PIXEL_AGENTS_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("PIXEL_AGENTS_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Pixel Agents processes
    ...
    return {
        ...
        "base_url": base_url,
        "api_key": access_token,
        "source": "pixel-agents-auth-store",
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    Pixel Agents persists a Codex 429's ``reset_at`` locally and freezes the
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.pixel-agents/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `pixel-agents model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `pixel-agents model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `pixel-agents model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `pixel-agents model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.pixel-agents/auth.json even when PIXEL_AGENTS_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``pixel-agents auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.pixel-agents/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `pixel-agents model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``PIXEL_AGENTS_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `pixel-agents model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Pixel Agents (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `pixel-agents model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``pixel-agents model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("PIXEL_AGENTS_XAI_REFRESH_TIMEOUT_SECONDS", 20)
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("PIXEL_AGENTS_CA_BUNDLE")
...
# =============================================================================
# Pixel Portal — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared Pixel token store — lets OAuth credentials persist across profiles
# so a new `pixel-agents --profile <name> auth add pixel --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${PIXEL_AGENTS_SHARED_AUTH_DIR}/pixel_auth.json, defaulting to
# ``<pixel-agents-root>/shared/pixel_auth.json`` where ``<pixel-agents-root>`` is what
# ``get_default_pixel_agents_root()`` returns — ``~/.pixel-agents`` on Linux/macOS,
# ``%LOCALAPPDATA%\pixel-agents`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's PIXEL_AGENTS_HOME so named profiles (which
# typically live under ``<pixel-agents-root>/profiles/<name>/``) all see the
# same file.
#
...
PIXEL_SHARED_STORE_FILENAME = "pixel_auth.json"
_pixel_shared_lock_holder = threading.local()


def _pixel_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared Pixel token store.

    Honors ``PIXEL_AGENTS_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<pixel-agents-root>/shared/``, where ``<pixel-agents-root>`` is what
    :func:`pixel_constants.get_default_pixel_agents_root` returns — so
    Linux/macOS classic installs land at ``~/.pixel-agents/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\pixel-agents\\shared\\``, and
    Docker / custom ``PIXEL_AGENTS_HOME`` deployments at
    ``<PIXEL_AGENTS_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("PIXEL_AGENTS_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from pixel_constants import get_default_pixel_agents_root
    return get_default_pixel_agents_root() / "shared"


def _pixel_shared_store_path() -> Path:
    path = _pixel_shared_auth_dir() / PIXEL_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Pixel Agents root, refuse rather than silently corrupt cross-profile
    # state. Tests must set PIXEL_AGENTS_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from pixel_constants import get_default_pixel_agents_root
        real_home_shared = (
            get_default_pixel_agents_root() / "shared" / PIXEL_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared Pixel auth store during test run: "
                f"{path}. Set PIXEL_AGENTS_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path


@contextmanager
def _pixel_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared Pixel OAuth store.
    ...
    ``_try_import_shared_pixel_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _pixel_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        # No PIXEL_AGENTS_HOME yet (pre-setup): fall through without locking.
        yield
        return

    with _file_lock(
        lock_path,
        _pixel_shared_lock_holder,
        timeout_seconds,
        "Timed out waiting for shared Pixel auth lock",
    ):
        yield


def _merge_shared_pixel_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local Pixel state."""
    shared = _read_shared_pixel_state()
    if not shared:
        return False
...
def _write_shared_pixel_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the Pixel OAuth state to the shared store.
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_PIXEL_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_PIXEL_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_PIXEL_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_PIXEL_INFERENCE_URL,
        "obtained_at": state.get("obtained_at"),
        "expires_at": state.get("expires_at"),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    try:
        with _pixel_shared_store_lock():
            path = _pixel_shared_store_path()
            path.parent.mkdir(parents=True, exist_ok=True)
            # secure_parent_dir refuses to chmod / or top-level dirs (#25821).
            secure_parent_dir(path)
            tmp = path.with_name(f"{path.name}.tmp.{os.getpid()}.{uuid.uuid4().hex}")
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed Pixel
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace(
            "pixel_shared_store_written",
            path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token),
        )
    except Exception as exc:
        logger.debug("Failed to write shared Pixel auth store: %s", exc)


def _read_shared_pixel_state() -> Optional[Dict[str, Any]]:
    """Return the shared Pixel OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared Pixel auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_pixel_state(reason: str) -> None:
    """Remove the shared Pixel OAuth store after a terminal token failure."""
    try:
        with _pixel_shared_store_lock():
            path = _pixel_shared_store_path()
            try:
                ...
                pass
        _oauth_trace("pixel_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared Pixel auth store: %s", exc)


def _is_terminal_pixel_refresh_error(exc: Exception) -> bool:
    """True when retrying the same Pixel refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "pixel"
...
def _quarantine_pixel_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Pixel Agents
    # has a known bug class where credential-shaped literals get corrupted in logs.
    forensic: Dict[str, Any] = {
        "reason": reason,
        "error_code": error.code,
        # No session_id field exists on Pixel state; provenance is client_id +
    ...
    logger.warning(
        "Pixel OAuth state quarantined (terminal auth death): %s",
    ...
    _clear_shared_pixel_state(reason)
    invalidate_pixel_auth_status_cache()


def _quarantine_pixel_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
) -> bool:
    """Remove singleton-seeded Pixel pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("pixel")
    if not isinstance(entries, list):
        return False

    retained = []
    removed = False
    singleton_sources = {PIXEL_DEVICE_CODE_SOURCE, f"manual:{PIXEL_DEVICE_CODE_SOURCE}"}
    for entry in entries:
    ...
    if removed:
        pool["pixel"] = retained
        _oauth_trace(
            "pixel_pool_device_code_quarantined",
            reason=reason,
            error_code=error.code,
        )
    return removed
...
def _try_import_shared_pixel_state(
    *,
    timeout_seconds: float = 15.0,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate Pixel OAuth state from the shared store.
    ...
    """
    try:
        with _pixel_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            shared = _read_shared_pixel_state()
            if not shared:
                return None

            # Build a full state dict so refresh_pixel_oauth_from_state has every
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_PIXEL_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_PIXEL_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_PIXEL_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_PIXEL_SCOPE,
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_pixel_state(updated_state)

            refreshed = refresh_pixel_oauth_from_state(
                state,
                timeout_seconds=timeout_seconds,
                force_refresh=True,
                on_state_update=_persist_shared_refresh,
            )
            _write_shared_pixel_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
            "pixel_shared_import_failed",
            error_type=type(exc).__name__,
            error_code=getattr(exc, "code", None),
        )
        if _is_terminal_pixel_refresh_error(exc):
            _clear_shared_pixel_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared Pixel import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "pixel_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-pixel-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="pixel", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="pixel", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the Pixel portal
    ...
    # called POST /api/oauth/token with Pixel Agents's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Pixel Agents's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "Pixel Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Pixel Agents install sharing "
            "~/.pixel-agents/auth.json) called POST /api/oauth/token with Pixel Agents's "
            "refresh token without persisting the rotated token back.\n"
            "Pixel refresh tokens are single-use — only Pixel Agents may call the "
            "refresh endpoint. For health checks, use `pixel-agents auth status` "
            "instead.\n"
            "Re-authenticate with: pixel-agents auth add pixel"
        )
        relogin = True

    raise AuthError(description, provider="pixel", code=code, relogin_required=relogin)


def fetch_pixel_models(
    *,
    inference_base_url: str,
    api_key: str,
    timeout_seconds: float = 15.0,
    ...
    """Fetch available model IDs from the Pixel inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="pixel", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Pixel Agents models — they're not reliable for agentic tool-calling
            if "pixel-agents" in mid.lower():
                continue
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if not isinstance(key, str) or not key.strip():
        return False
    return _pixel_invoke_jwt_is_usable(
        key,
        scope=state.get("scope"),
...
def resolve_pixel_access_token(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """Resolve a refresh-aware Pixel Portal access token for managed tool gateways."""
    with _provider_state_transaction("pixel") as (
        auth_store,
        state,
        state_source_path,
    ):
        ...
        if not state:
            raise AuthError(
                "Pixel Agents is not logged into Pixel Portal.",
                provider="pixel",
                relogin_required=True,
            )

        # PIXEL_AGENTS_PORTAL_BASE_URL / PIXEL_PORTAL_BASE_URL is the trusted
        # operator/deployment override (mirrors PIXEL_INFERENCE_BASE_URL) and
        ...
        # no override is set.
        env_portal_override = _pixel_portal_env_override()
        if env_portal_override:
            portal_base_url = env_portal_override.rstrip("/")
        else:
            portal_base_url = (
                _optional_base_url(state.get("portal_base_url"))
                or DEFAULT_PIXEL_PORTAL_URL
            ).rstrip("/")

            parsed_portal_url = urlparse(portal_base_url)
            if parsed_portal_url.hostname and parsed_portal_url.hostname not in _PIXEL_PORTAL_ALLOWED_HOSTS:
                logger.warning(
        ...
        client_id = str(state.get("client_id") or DEFAULT_PIXEL_CLIENT_ID)
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)

        with _pixel_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            merged_shared = _merge_shared_pixel_oauth_state(state)
            access_token = state.get("access_token")
            refresh_token = state.get("refresh_token")
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for Pixel Portal login.",
                    provider="pixel",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state_to_source(auth_store, "pixel", state, state_source_path)
                return access_token
            ...
            with httpx.Client(
                ...
                try:
                    ...
                    if _is_terminal_pixel_refresh_error(exc):
                        _quarantine_pixel_oauth_state(
                            state,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
                        _quarantine_pixel_pool_entries(
                            auth_store,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
            ...
            _save_provider_state_to_source(auth_store, "pixel", state, state_source_path)
            _write_shared_pixel_state(state)
            return state["access_token"]


def refresh_pixel_oauth_pure(
    access_token: str,
    refresh_token: str,
    client_id: str,
    portal_base_url: str,
    ...
    *,
    ...
    """Refresh Pixel OAuth state without mutating auth.json directly.
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_PIXEL_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_PIXEL_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_PIXEL_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_PIXEL_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        current_invoke_jwt_status = _pixel_invoke_jwt_status(
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for Pixel Portal.",
                    provider="pixel",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
            ...
            refreshed_url = _validate_pixel_inference_url_from_network(refreshed.get("inference_base_url"))
            state["inference_base_url"] = refreshed_url or DEFAULT_PIXEL_INFERENCE_URL
            state["obtained_at"] = now.isoformat()
            state["expires_in"] = access_ttl
            state["expires_at"] = datetime.fromtimestamp(
        ...
        _assert_pixel_inference_jwt_usable(state)
        _select_pixel_invoke_jwt(state)

    return state


def refresh_pixel_oauth_from_state(
    state: Dict[str, Any],
    *,
    timeout_seconds: float = 15.0,
    force_refresh: bool = False,
    ...
    """Refresh Pixel OAuth from a state dict. Thin wrapper around refresh_pixel_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_pixel_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "pixel-agents-cli"),
        state.get("portal_base_url", DEFAULT_PIXEL_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_PIXEL_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_PIXEL_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def persist_pixel_credentials(
    creds: Dict[str, Any],
    *,
    label: Optional[str] = None,
):
    """Persist Pixel OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    Pixel credentials are read at runtime from two independent locations:

    - ``providers.pixel``: singleton state read by
      ``resolve_pixel_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.pixel``: used by the runtime ``pool.select()`` path.

    Historically ``pixel-agents auth add pixel`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.pixel``. When the runtime credential
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.pixel`` then calls ``load_pool("pixel")`` so
    ...
    ``pixel-agents auth add pixel --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("pixel")`` instead of the
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "pixel", state)
        _save_auth_store(auth_store)

    # Mirror to the shared store so a new profile can one-tap import
    # these credentials via `pixel-agents auth add pixel --type oauth`. Best-
    ...
    return next(
        (e for e in pool.entries() if e.source == PIXEL_DEVICE_CODE_SOURCE),
        None,
    )


def _sync_pixel_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.pixel changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("pixel")
    except Exception as exc:
        logger.debug("Failed to sync Pixel credential pool from auth store: %s", exc)


def resolve_pixel_runtime_credentials(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    ...
    """
    Resolve Pixel inference credentials for runtime use.
    ...
    with _provider_state_transaction("pixel") as (
        ...
        if not state:
            raise AuthError("Pixel Agents is not logged into Pixel Portal.",
                            provider="pixel", relogin_required=True)
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("PIXEL_AGENTS_PORTAL_BASE_URL")
            ...
            if env_portal_override:
                ...
                if (
                    not portal_host
                    or portal_host not in _PIXEL_PORTAL_ALLOWED_HOSTS
                    ...
                    portal_url = DEFAULT_PIXEL_PORTAL_URL
            ...
            stored_inference_url = (
                _validate_pixel_inference_url_from_network(
                    _optional_base_url(state.get("inference_base_url"))
                )
                or DEFAULT_PIXEL_INFERENCE_URL
            )
            effective_inference_url = (
                _pixel_inference_env_override() or stored_inference_url
            )
            effective_client_id = str(
                state.get("client_id") or DEFAULT_PIXEL_CLIENT_ID
            )
            return (
                portal_url,
                stored_inference_url,
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed Pixel auth-status cache warm during read paths.
            if (
                _pixel_effective_provider_state(state)
                == _pixel_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
                    "pixel_state_persist_skipped",
                    sequence_id=sequence_id,
                    reason=reason,
                )
                return
            ...
            # _write_shared_pixel_state.
            _write_shared_pixel_state(state)

        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "pixel_runtime_credentials_start",
            sequence_id=sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
        )
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                with _pixel_shared_store_lock(
                    ...
                    if _merge_shared_pixel_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                        (
                            portal_base_url,
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Pixel Portal login.",
                                provider="pixel", relogin_required=True)

            invoke_jwt_status = _pixel_invoke_jwt_status(
                access_token,
                scope=state.get("scope"),
                expires_at=state.get("expires_at"),
            )
            if force_refresh or invoke_jwt_status is not None:
                with _pixel_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    ...
                    if force_refresh or invoke_jwt_status is not None:
                        ...
                        state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
                        state["scope"] = refreshed.get("scope") or state.get("scope")
                        # Heal a poisoned stored value (see refresh_pixel_oauth_pure):
                        ...
                        # persisted to auth.json below. The PIXEL_INFERENCE_BASE_URL
                        # env override is layered on for the client/return value
                        # only (see below) — it is never persisted.
                        refreshed_url = _validate_pixel_inference_url_from_network(refreshed.get("inference_base_url"))
                        stored_inference_base_url = refreshed_url or DEFAULT_PIXEL_INFERENCE_URL
                        inference_base_url = (
                            _pixel_inference_env_override() or stored_inference_base_url
                        )
            ...
            _assert_pixel_inference_jwt_usable(
                state,
                access_token=access_token,
            )
            _select_pixel_invoke_jwt(
                state,
                access_token=access_token,
                sequence_id=sequence_id,
            )
        ...
        _persist_state("resolve_pixel_runtime_credentials_final")

    if state_persisted:
        _sync_pixel_pool_from_auth_store()

    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a Pixel inference API key",
                        provider="pixel", code="server_error")
    ...
    return {
        ...
        "expires_at": expires_at,
        "expires_in": expires_in,
        "source": PIXEL_AUTH_PATH_INVOKE_JWT,
        ...
        "auth_path": PIXEL_AUTH_PATH_INVOKE_JWT,
...
def _empty_pixel_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_pixel_pool_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if not pool or not pool.has_credentials():
            return _empty_pixel_auth_status()
...
# ── Process-level memo for get_pixel_auth_status() ──
# get_pixel_auth_status() validates state by calling resolve_pixel_runtime_credentials(),
# which does a synchropixel OAuth refresh POST to portal.pixelagents.com. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`pixel-agents tools`, status panels,
# subscription-feature checks) call it many times per render — `pixel-agents tools` → "All Platforms"
...
# `pixel-agents auth login/logout/add/remove` invalidate naturally on the next call.
_PIXEL_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_pixel_auth_status_cache: Optional[Tuple[float, str, Optional[float], Dict[str, Any]]] = None
...
def invalidate_pixel_auth_status_cache() -> None:
    """Clear the get_pixel_auth_status() process-level memo.

    Call this from any code path that mutates Pixel auth state without going
    through resolve_pixel_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _pixel_auth_status_cache
    _pixel_auth_status_cache = None


def get_pixel_auth_status() -> Dict[str, Any]:
    """Status snapshot for Pixel auth.
    ...
    ``invalidate_pixel_auth_status_cache()`` explicitly.
    """
    global _pixel_auth_status_cache
    now = time.monotonic()
    auth_file_key, mtime = _auth_file_cache_key()
    cached = _pixel_auth_status_cache
    if cached is not None:
        ...
        if (
            ...
            and (now - cached_at) < _PIXEL_AUTH_STATUS_CACHE_TTL
        ):
            return dict(cached_status)

    status = _compute_pixel_auth_status()
    _pixel_auth_status_cache = (now, auth_file_key, mtime, dict(status))
    return status


def _compute_pixel_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_pixel_auth_status(). See that function."""
    state = get_provider_auth_state("pixel")
    if state:
        base_status = {
        ...
        try:
            ...
            refreshed_state = get_provider_auth_state("pixel") or state
            base_status.update(
                {
                    "logged_in": True,
    ...
    return _snapshot_pixel_pool_status()


def get_pixel_auth_status_local() -> Dict[str, Any]:
    """Refresh-free Pixel auth snapshot for read-only display surfaces.

    Unlike :func:`get_pixel_auth_status`, this NEVER calls
    ``resolve_pixel_runtime_credentials()`` and therefore never performs an
    ...
    need a live credential) should keep using ``get_pixel_auth_status()``.
    ...
    access_token = state.get("access_token")
    jwt_reason = _pixel_invoke_jwt_status(
        access_token,
        scope=state.get("scope"),
        expires_at=state.get("expires_at"),
    )
...
# Enum values reported on the dashboard /api/status as ``pixel_session_valid``.
...
PIXEL_SESSION_VALID = "valid"
PIXEL_SESSION_TERMINAL = "terminal"
PIXEL_SESSION_UNKNOWN = "unknown"


def get_pixel_session_validity() -> str:
    """Classify the Pixel bootstrap session for the dashboard /api/status probe.

    Returns one of:
      - ``"valid"``    — a usable Pixel credential is present (login healthy).
      - ``"terminal"`` — the Pixel session has taken a terminal auth failure
      ...
      - ``"unknown"``  — indeterminate (no Pixel provider state, or a transient/
    ...
    # into the Pixel provider state when it clears dead tokens (the exact path
    ...
    if isinstance(last_err, dict) and last_err.get("relogin_required"):
        ...
        if not (state.get("access_token") or state.get("refresh_token")):
            return PIXEL_SESSION_TERMINAL

    if _pixel_invoke_jwt_status(
        state.get("access_token"),
        scope=state.get("scope"),
        expires_at=state.get("expires_at"),
    ) is None:
        return PIXEL_SESSION_VALID
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `pixel-agents auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `pixel-agents auth` and
    # `pixel-agents model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("PIXEL_AGENTS_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("PIXEL_AGENTS_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``pixel-agents doctor`` runs the live
    ...
    try:
        from pixel_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Pixel Agents' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `pixel-agents doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set PIXEL_AGENTS_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from pixel_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `pixel-agents logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"pixel", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _prompt_model_selection(
    ...
    # Sale chrome (★ / -N% / was) is Pixel Portal-only — never for OpenRouter
    # or other providers even if pricing.original is somehow present.
    sale_chrome = (confirm_provider or "").strip().lower() == "pixel"
    ...
    # Leave room for a leading "★ " on sale rows (Pixel only).
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_PIXEL_PORTAL_URL).rstrip("/")
        ...
        from pixel_cli.model_search import model_search_text
    ...
    from pixel_cli.curses_ui import format_radio_item_ansi
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from pixel_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'pixel-agents model' or 'pixel-agents setup' instead."""
    print("The 'pixel-agents login' command has been removed.")
    print("Use 'pixel-agents auth' to manage credentials,")
    print("'pixel-agents model' to select a provider, or 'pixel-agents setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.pixel-agents/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Pixel Agents-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Pixel Agents auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Pixel Agents will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("PIXEL_AGENTS_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Pixel Agents will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Pixel Agents gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Pixel Agents creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Pixel Agents auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider("openai-codex", creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    print()
    print("Login successful!")
    from pixel_constants import display_pixel_agents_home as _dhh
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Pixel Agents auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Pixel Agents creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``pixel-agents auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``pixel-agents auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Pixel Agents auth store (~/.pixel-agents/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Pixel Agents login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the Pixel / xAI-OAuth / Codex-OAuth quarantine pattern
    so subsequent calls fail fast without a network retry.
    """
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `pixel-agents model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``pixel-agents status`` that just want to know
...
def _pixel_device_code_login(
    *,
    ...
    """Run the Pixel device-code flow and return full OAuth state without persisting."""
    ...
    requested_inference_url = (
        inference_base_url
        or os.getenv("PIXEL_INFERENCE_BASE_URL")
    ...
    print(f"Starting Pixel Agents login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        return refresh_pixel_oauth_from_state(
            auth_state,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        )
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `pixel-agents model` again to finish setup.")
            raise SystemExit(1)
        raise


def pixel_token_has_billing_scope() -> bool:
    """Return True if the currently-held Pixel token carries ``billing:manage``.
    ...
    if not isinstance(scope, str):
        return False
    return PIXEL_BILLING_MANAGE_SCOPE in scope.split()


def step_up_pixel_billing_scope(
    *,
    open_browser: bool = True,
    timeout_seconds: float = 15.0,
    on_verification: Optional[Callable[[str, str], None]] = None,
    ...
    targets the same deployment (incl. a preview via ``PIXEL_AGENTS_PORTAL_BASE_URL`` set
    at the original login). Persists to the auth store + shared store + pool, exactly
    like ``_login_pixel`` — but WITHOUT the model picker (this is a scope upgrade, not
    a fresh login).

    Returns True iff the new token carries ``billing:manage``.
    """
    prior = get_provider_auth_state("pixel") or {}
    pconfig = PROVIDER_REGISTRY["pixel"]
    ...
    requested: list[str] = []
    for tok in (prior_scope.split() or [PIXEL_INFERENCE_INVOKE_SCOPE, "tool:invoke"]):
        if tok and tok not in requested:
            requested.append(tok)
    if PIXEL_BILLING_MANAGE_SCOPE not in requested:
        requested.append(PIXEL_BILLING_MANAGE_SCOPE)
    scope = " ".join(requested)

    auth_state = _pixel_device_code_login(
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "pixel", auth_state)
        _save_auth_store(auth_store)

    # Mirror to shared store + reseed the pool (best-effort), same as _login_pixel.
    try:
        _write_shared_pixel_state(auth_state)
    except Exception:
        pass
    try:
        _sync_pixel_pool_from_auth_store()
    ...
    return isinstance(granted, str) and PIXEL_BILLING_MANAGE_SCOPE in granted.split()


def _login_pixel(args, pconfig: ProviderConfig) -> None:
    """Pixel Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing Pixel credential
        # from any other profile. If present, offer to rehydrate it.
        shared = _read_shared_pixel_state()
        if shared:
            try:
                shared_path = _pixel_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
                print(f"Found existing Pixel OAuth credentials at {shared_path}")
            else:
                print("Found existing shared Pixel OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating Pixel session from shared credentials...")
                auth_state = _try_import_shared_pixel_state(
                    timeout_seconds=timeout_seconds,
                )
        ...
        # overwrites it to "pixel".  If the user picks "Skip (keep current)"
        ...
        # leave the config in a half-updated state (provider=pixel but model
        ...
        try:
            ...
            from pixel_cli.models import (
                get_curated_pixel_model_ids, get_pricing_for_provider,
                check_pixel_free_tier, partition_pixel_models_by_tier,
            ...
            unavailable_message = ""
            if model_ids:
                pricing = get_pricing_for_provider("pixel")
                ...
                if free_tier:
                    try:
                        ...
                        _account_info = get_pixel_portal_account_info(force_fresh=True)
                        unavailable_message = (
                            format_pixel_portal_entitlement_message(
                                _account_info,
                                capability="paid Pixel models",
                            )
                            or ""
                        )
                    except Exception:
                    ...
                    # as free so users on older Pixel Agents builds still see
                    # newly-launched free models without a CLI release.
                    model_ids, pricing = union_with_portal_free_recommendations(
                        model_ids, pricing, _portal_for_recs,
                    )
                    model_ids, unavailable_models = partition_pixel_models_by_tier(
            ...
            if model_ids:
                ...
                selected_model = _prompt_model_selection(
                    ...
                    confirm_provider="pixel",
                    confirm_base_url=inference_base_url,
                    confirm_api_key=runtime_key,
                )
            elif unavailable_models:
                _url = (_portal or DEFAULT_PIXEL_PORTAL_URL).rstrip("/")
                print("No free models currently available.")
                print(unavailable_message or f"Upgrade at {_url} to access paid models.")
            else:
                print("No curated models available for Pixel Portal.")
        except Exception as exc:
        ...
        # them to Pixel with a mismatched model.  The Pixel OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "pixel".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. Pixel credentials saved for future use.")
            print("  Run `pixel-agents model` again to switch to Pixel Portal.")
            return

        config_path = _update_config_for_provider(
            "pixel", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=pixel)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Pixel Agents will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `pixel-agents model` or configure an API key to use Pixel Agents.")
        else:
