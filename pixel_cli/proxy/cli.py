"""CLI handlers for the ``pixel-agents proxy`` subcommand."""
...
from pixel_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "pixel-agents proxy requires aiohttp. Run `pixel-agents setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "pixel"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"pixel-agents auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Pixel Agents proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Pixel Agents proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: pixel-agents proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``pixel-agents proxy <subcommand>``."""
    ...
    print(
        "pixel-agents proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  pixel-agents proxy start [--provider pixel|xai] [--host 127.0.0.1] [--port 8645]\n"
