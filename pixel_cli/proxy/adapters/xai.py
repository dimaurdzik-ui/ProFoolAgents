...
# xAI's public API is OpenAI-compatible for the endpoints Pixel Agents commonly
# uses. The Responses endpoint is included because Pixel Agents' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Pixel Agents-managed OAuth credentials."""

    auth_hint = "pixel-agents auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`pixel-agents auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`pixel-agents auth reset xai-oauth` or re-authenticate with "
                    "`pixel-agents auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `pixel-agents auth add xai-oauth --type oauth`."
            )
