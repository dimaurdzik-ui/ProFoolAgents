"""CLI handlers for ``pixel-agents migrate ...``.

Currently exposes only ``pixel-agents migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``pixel-agents migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: pixel-agents migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `pixel-agents migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `pixel-agents doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
