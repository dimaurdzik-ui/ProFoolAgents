"""``pixel-agents gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Pixel Portal access token from the existing login
     (``~/.pixel-agents/auth.json``) — the same path ``pixel-agents dashboard register``
     uses (``resolve_pixel_access_token``). This proves *which Pixel org (tenant)*
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.pixel-agents/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _default_gateway_id() -> str:
    ...
    try:
        ...
        host = ""
    return f"gw-{host or 'pixel-agents'}"
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or Pixel Portal).
    ...
    self-hosted-IdP path; otherwise Pixel Portal). Raises RuntimeError on failure.
    """
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your Pixel Portal "
                "token could not be verified — try `pixel-agents auth add pixel` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    from pixel_cli.config import is_managed, save_env_value
    ...
    if is_managed():
        print(
            "✗ `pixel-agents gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    gapped / self-hosted-IdP, NO Pixel Portal); otherwise the Pixel Portal token.
    try:
    ...
    #    save_env_value writes them to ~/.pixel-agents/.env (0600 dir) and never logs.
    to_write = {
