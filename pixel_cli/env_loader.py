"""Helpers for loading Pixel Agents .env files consistently across entrypoints."""
...
# load_pixel_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Paths we've already emitted a UTF-32 refuse-to-mangle warning for.
# load_pixel_dotenv can call _sanitize_env_file_if_needed multiple times
# for the same file (user env + project env + hot-reload); once per path
# is enough.
_WARNED_UTF32_PATHS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_pixel_dotenv().
# Used by setup / `pixel-agents model` flows to label detected credentials so
...
# PIXEL_AGENTS_HOME paths we've already pulled external secrets for during this
# process.  ``load_pixel_dotenv()`` is called at module-import time from
# several hot modules (cli.py, pixel_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_pixel_dotenv()`` call.  Returns
...
def get_secret_source_values(
    pixel_home: str | os.PathLike,
) -> dict[str, str]:
    """Return the external-secret value snapshot for ``pixel_home``."""
    home_key = str(Path(pixel_home).resolve())
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(home_key, {}))


def reset_secret_source_cache() -> None:
    """Forget which PIXEL_AGENTS_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `pixel-agents setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``pixel_cli.config._sanitize_env_lines`` normalizes line endings while
    treating content after the first ``=`` as opaque for boundary discovery.
    """
    if not path.exists():
        return
    try:
        from pixel_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_pixel_dotenv(
    *,
    pixel_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Pixel Agents environment files with user config taking precedence.

    Behavior:
    - `~/.pixel-agents/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(pixel_home or os.getenv("PIXEL_AGENTS_HOME", Path.home() / ".pixel-agents"))
    ...
    #   EnvironmentFile=-/path/to/.pixel-agents/.op.env
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of PIXEL_AGENTS_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Pixel Agents reads
    ...
    owns the once-per-PIXEL_AGENTS_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_pixel_dotenv()`` runs at import
    time from several hot modules (cli.py, pixel_cli/main.py,
    ...
    if not cfg:
        ...
        # on its next load_pixel_dotenv() call instead of never.
        return
    ...
    # so the 3-5 import-time load_pixel_dotenv() calls per startup don't
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `pixel-agents model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `pixel-agents` startup, so populating the
    # shared cache here lets main.py's early bridge and pixel_logging reuse
    ...
    if home_path == _process_pixel_home():
        try:
...
def _process_pixel_home() -> Path:
    """The PIXEL_AGENTS_HOME the shared config cache is keyed to."""
    try:
