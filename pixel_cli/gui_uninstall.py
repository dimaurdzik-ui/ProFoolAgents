"""
Pixel Agents Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``pixel-agents desktop`` / ``pixel-agents gui``)
     Built inside the agent checkout under ``$PIXEL_AGENTS_HOME/pixel-agents/``:
       ...
       - ``$PIXEL_AGENTS_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Pixel Agents.app`` or ``~/Applications/Pixel Agents.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Pixel Agents`` (NSIS per-user)
...
the app name ("Pixel Agents"), separate from ``$PIXEL_AGENTS_HOME``:
  - macOS:   ``~/Library/Application Support/Pixel Agents``
  - Windows: ``%APPDATA%\\Pixel Agents``
  - Linux:   ``$XDG_CONFIG_HOME/Pixel Agents`` (default ``~/.config/Pixel Agents``)
...
``pixel-agents uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(pixel_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return pixel_home / "pixel-agents"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Pixel Agents"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Pixel Agents"
...
def source_built_gui_artifacts(pixel_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``pixel-agents desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``pixel-agents/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(pixel_home)
    ...
    return [
        ...
        pixel_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Pixel Agents" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Pixel Agents.app"),
            home / "Applications" / "Pixel Agents.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Pixel Agents).
            local_base / "Programs" / "Pixel Agents",
            # Older / alternate layout some builds used.
            local_base / "pixel-agents-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Pixel Agents")
    else:
        ...
        paths += [
            data_base / "applications" / "pixel-agents.desktop",
            data_base / "applications" / "Pixel Agents.desktop",
        ]
    return paths


def agent_is_installed(pixel_home: Path) -> bool:
    """Return True when a usable Python agent install exists under PIXEL_AGENTS_HOME.
    ...
    if (agent_root / "pixel_cli").is_dir():
        return True
...
def gui_is_installed(pixel_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(pixel_home):
        if p.exists():
            return True
...
def gui_install_summary(pixel_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = pixel_home if pixel_home is not None else get_pixel_agents_home()
...
def uninstall_gui(pixel_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``pixel-agents/pixel_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$PIXEL_AGENTS_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove pixel-agents' or "
            "'sudo dnf remove pixel-agents'). AppImage builds are a single file you "
