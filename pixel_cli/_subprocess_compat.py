...
Pixel Agents is developed on Linux / macOS and tested natively on Windows too.
...
# Electron exits, so the gateway never gets respawned after a `pixel-agents
...
def windows_hide_flags() -> int:
    ...
    flash but also want to collect stdout/exit code synchropixelly.
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Pixel Agents' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Pixel Agents shells out to git from many non-interactive contexts — MCP catalog
