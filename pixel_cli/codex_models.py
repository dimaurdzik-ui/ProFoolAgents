...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # in pixel_cli/models.py — only the openai-codex (OAuth) provider
    ...
    # Pixel Agents uses. Removed here. If OpenAI re-enables them on Codex backend,
...
_FORWARD_COMPAT_TEMPLATE_MODELS: List[tuple[str, tuple[str, ...]]] = [
    ...
    # entitlement; Pixel Agents does not.
...
def _read_cache_models(codex_home: Path) -> List[str]:
    ...
    if isinstance(entries, list):
        for item in entries:
            ...
            # public OpenAI API, while Pixel Agents openai-codex talks to the same
