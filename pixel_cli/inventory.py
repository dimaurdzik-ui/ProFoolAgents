...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Pixel /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for Pixel, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``pixel-agents model`` CLI picker. Adds network calls
      (pricing fetch + Pixel tier check); only set for interactive pickers.
    ...
    - ``force_fresh_pixel_tier``: bypass the short Pixel free-tier cache when
      selecting Portal-recommended Pixel models and applying tier gating. Keep
    ...
    """
    from pixel_cli.model_switch import list_authenticated_providers
    ...
    rows = list_authenticated_providers(
        ...
        force_fresh_pixel_tier=force_fresh_pixel_tier,
    ...
    try:
        from pixel_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
    ...
    if pricing:
        _apply_pricing(rows, force_fresh_pixel_tier=force_fresh_pixel_tier)
    if capabilities:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``pixel_cli/moa_cmd.py``.
...
def _apply_featured(rows: list[dict]) -> None:
    ...
    Aggregator providers (pixel, openrouter) serve dozens of models across many
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from pixel_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `pixel-agents model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Pixel Agents.
    """
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `pixel-agents model` to configure ({auth_type})"
        )
...
def _apply_pricing(
    ...
    *,
    ...
    # Resolve Pixel free-tier once (cached in models.py for the TTL window).
    pixel_free_tier: Optional[bool] = None
    ...
    for row in rows:
        ...
        for mid in models:
            ...
            # Sale chrome is Pixel Portal-only. Other providers (OpenRouter,
        ...
        if slug == "pixel":
            try:
                if pixel_free_tier is None:
                    pixel_free_tier = check_pixel_free_tier(
                        force_fresh=force_fresh_pixel_tier
                    )
                row["free_tier"] = bool(pixel_free_tier)
                if pixel_free_tier:
                    _selectable, unavailable = partition_pixel_models_by_tier(
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`pixel_cli.model_switch.list_picker_providers`) so the
