"""pixel-agents import-agent — import Claude Code / Codex CLI setups into Pixel Agents.

Usage:
    pixel-agents import-agent                       # auto-detect ~/.claude or ~/.codex
    pixel-agents import-agent claude-code           # import from ~/.claude
    pixel-agents import-agent codex                 # import from ~/.codex
    pixel-agents import-agent claude-code --dry-run # preview only, no changes
    pixel-agents import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``pixel-agents claw migrate`` /
``optional-skills/migration/openclaw-migration/scripts/openclaw_to_pixel.py``):
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in PIXEL_AGENTS_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → PIXEL_AGENTS_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in PIXEL_AGENTS_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in PIXEL_AGENTS_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → PIXEL_AGENTS_HOME/skills/codex-imports/<name>/
...
deliberately via ``pixel-agents setup`` or config.yaml.
"""
...
# Same entry delimiter as the Pixel Agents memory store and the openclaw migration
...
# ---------------------------------------------------------------------------
# Memory-entry primitives (ported from openclaw_to_pixel.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Pixel Agents command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into a Pixel Agents glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Pixel Agents equivalent — "
        ...
        for name, srv in servers.items():
            ...
            if srv.get("url"):
                ...
                if isinstance(headers, dict):
                    ...
                    if kept_headers:
                        pixel_srv["headers"] = kept_headers
                    for k in headers:
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → PIXEL_AGENTS_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``pixel-agents import-agent`` (invoked from pixel_cli.main)."""
    from pixel_cli.config import get_config_path, load_config, save_config
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: pixel-agents import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: pixel-agents import-agent claude-code   or   pixel-agents import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          ⚕ Pixel Agents — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: pixel-agents import-agent "
    ...
    print_info("Secrets:     never imported — run 'pixel-agents setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: pixel-agents import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'pixel-agents setup' "
               "to configure providers, or add them to ~/.pixel-agents/.env.")


def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    """Print a formatted per-item import report (claw-migrate style)."""
    from pixel_cli.setup import Colors, color, print_header, print_info
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'pixel-agents setup' or ~/.pixel-agents/.env.")
        print()
