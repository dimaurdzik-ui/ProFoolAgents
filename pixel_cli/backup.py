"""
Backup and import commands for pixel-agents CLI.

`pixel-agents backup` creates a zip archive of the entire ~/.pixel-agents/ directory
(excluding the pixel-agents repo and transient files).

`pixel-agents import` restores from a backup zip, overlaying onto the current
PIXEL_AGENTS_HOME root.
"""
...
from pixel_constants import get_default_pixel_agents_root, get_pixel_agents_home, display_pixel_agents_home
...
# ``pixel-agents`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/pixel-agents/``
...
# PIXEL_AGENTS_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "pixel-agents",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under PIXEL_AGENTS_HOME) —
...
# File names that ``pixel-agents import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the Pixel portal
...
# Reserved archive subtree for provider state that lives OUTSIDE PIXEL_AGENTS_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside PIXEL_AGENTS_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to pixel-agents root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``pixel-agents`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/pixel-agents/`` — must be preserved.
        if part == "pixel-agents" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside PIXEL_AGENTS_HOME.
...
# to the user as a hung `pixel-agents update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # pixel_cli.sqlite_safe_read). Verification targets snapshots and
...
def run_backup(args) -> None:
    """Create a zip backup of the Pixel Agents home directory."""
    pixel_root = get_default_pixel_agents_root()

    if not pixel_root.is_dir():
        print(f"Error: Pixel Agents home directory not found at {pixel_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"pixel-agents-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"pixel-agents-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_pixel_agents_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(pixel_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(pixel_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``pixel-agents`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "pixel-agents" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(pixel_root)
    ...
    # outside PIXEL_AGENTS_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_pixel_agents_home()}."
        )
    ...
    if not errors:
        print(f"\nRestore with: pixel-agents import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Pixel Agents backup.
    ...
    # Look for telltale files that a pixel-agents home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Pixel Agents backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.pixel-agents/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a pixel-agents dir name
        if prefix in {".pixel-agents", "pixel-agents"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_pixel_agents_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (pixel_root / "config.yaml").exists()
        has_env = (pixel_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Pixel Agents configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under PIXEL_AGENTS_HOME.
            ...
            # Pixel portal. Matched by basename so both the root profile and
            # named profiles (profiles/<name>/gateway_state.json) are covered.
            if Path(rel).name in _IMPORT_SKIP_NAMES:
                skipped_runtime.append(rel)
                continue

            target = pixel_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(pixel_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_pixel_agents_home()}."
            )
        ...
        profiles_dir = pixel_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # pixel_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: pixel-agents profile list  (after installing pixel-agents)")

        # Guidance
        print()
        if not (pixel_root / "pixel-agents").is_dir():
            print("Note: The pixel-agents codebase was not included in the backup.")
            print("  If this is a fresh install, run: pixel-agents update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  pixel-agents -p {pname} gateway install")

        print("Done. Your Pixel Agents configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and pixel-agents backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to PIXEL_AGENTS_HOME).
...
# — `pixel-agents update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $PIXEL_AGENTS_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside PIXEL_AGENTS_HOME and the entry is silently
...
def _quick_snapshot_root(pixel_home: Optional[Path] = None) -> Path:
...
def create_quick_snapshot(
    ...
    Args:
        max_file_size: When set, individual files larger than this many bytes
            ...
            never stall ``pixel-agents update`` or silently eat disk — the small
            pairing/cron/config files the snapshot exists to protect are
            always captured. ``None`` (default) copies everything, which
            preserves manual ``/snapshot`` and ``pixel-agents backup --quick``
            behavior.
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: pixel-agents snapshot list"
        )
        logger.error(
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(pixel_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside PIXEL_AGENTS_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``pixel-agents update``.
    ...
    Args:
        ...
        pixel_home: Override for the Pixel Agents home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(pixel_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for pixel-agents backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_pixel_agents_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, pixel_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``pixel_root`` to ``out_path``.
...
def _pre_update_backup_dir(pixel_home: Optional[Path] = None) -> Path:
    home = pixel_home or get_pixel_agents_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of PIXEL_AGENTS_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<PIXEL_AGENTS_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``pixel-agents update``) should continue even if the backup fails.
    """
    pixel_root = pixel_home or get_default_pixel_agents_root()
    if not pixel_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(pixel_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, pixel_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `pixel-agents claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of PIXEL_AGENTS_HOME under ``backups/`` before a
    ``pixel-agents claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``pixel-agents import <archive>``.  Writes to
    ``<PIXEL_AGENTS_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `pixel-agents import` and the
