...
manual BotFather token copy-paste. Pixel Agents talks only to the Pixel onboarding
...
# Default pairing API base URL (Pixel-hosted Cloudflare Worker).
# Override for PoC/staging with TELEGRAM_ONBOARDING_URL.
DEFAULT_API_URL = "https://setup.api.pixelagents.com"
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"

# The Pixel-hosted manager bot username (without @). The backend returns the
# actual deep link, so this is only used by local helpers/tests.
DEFAULT_MANAGER_BOT = "PixelAgentsSetupBot"

DEFAULT_BOT_NAME = "Pixel Agents"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``pixel_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``pixel_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"pixel_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Pixel Agents Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Pixel Agents Telegram onboarding service.")
