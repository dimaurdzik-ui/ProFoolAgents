"""Migrate Pixel Agents' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Pixel Agents' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Pixel Agents mcp_servers.<n>.command/args/env  → codex stdio transport
  Pixel Agents mcp_servers.<n>.url/headers       → codex streamable_http transport
  Pixel Agents mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Pixel Agents mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Pixel Agents-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Pixel Agents' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Pixel Agents config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Pixel Agents' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, pixel_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Pixel Agents MCP server config to the codex inline-table dict
    ...
    if not isinstance(pixel_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(pixel_cfg.get("command"))
    has_url = bool(pixel_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (PIXEL_AGENTS_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Pixel Agents)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Pixel Agents' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Pixel Agents' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Pixel Agents
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="pixel-agents-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a PIXEL_AGENTS_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed pixel-agents-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.pixel-agents`` symlink that happens to live under ``/private/var/folders``
...
def _build_pixel_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Pixel Agents' own
    ...
    so a pixel-agents installed under /opt/, /usr/local/, or a venv all work.
    PIXEL_AGENTS_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # PIXEL_AGENTS_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_pixel_agents_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever PIXEL_AGENTS_HOME its launcher sets
    ...
    # — that would override the launcher's PIXEL_AGENTS_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("PIXEL_AGENTS_HOME", tmp_path) would
    ...
    pixel_home = os.environ.get("PIXEL_AGENTS_HOME") or ""
    if pixel_home and _looks_like_test_tempdir(pixel_home):
        pixel_home = ""
    if pixel_home:
        env["PIXEL_AGENTS_HOME"] = pixel_home
    # PYTHONPATH passes through so a worktree-launched pixel-agents finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["PIXEL_AGENTS_QUIET"] = "1"
    env["PIXEL_AGENTS_REDACT_SECRETS"] = env.get("PIXEL_AGENTS_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_pixel_tools: bool = True,
) -> MigrationReport:
    """Translate Pixel Agents mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        pixel_config: full ~/.pixel-agents/config.yaml dict
        ...
        expose_pixel_tools: when True (default), register Pixel Agents' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Pixel Agents for tools
    ...
    pixel_servers = (pixel_config or {}).get("mcp_servers") or {}
    if not isinstance(pixel_servers, dict):
        report.errors.append(
            "mcp_servers in Pixel Agents config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in pixel_servers.items():
    ...
    # Inject Pixel Agents' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Pixel Agents for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/pixel_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_pixel_tools:
        translated["pixel-agents-tools"] = _build_pixel_tools_mcp_entry()
        if "pixel-agents-tools" not in report.migrated:
            report.migrated.append("pixel-agents-tools")
