"""pixel-agents hooks — inspect and manage shell-script hooks.

Usage::

    pixel-agents hooks list
    pixel-agents hooks test <event> [--for-tool X] [--payload-file F]
    pixel-agents hooks revoke <command>
    pixel-agents hooks doctor

Consent records live under ``~/.pixel-agents/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.pixel-agents/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``pixel-agents hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: pixel-agents hooks {list|test|revoke|doctor}")
        print("Run 'pixel-agents hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.pixel-agents/config.yaml.")
        print("See `pixel-agents hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `pixel-agents hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `pixel-agents hooks test` and `pixel-agents hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Pixel Agents wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `pixel-agents hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `pixel-agents hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `pixel-agents hooks doctor`.")
