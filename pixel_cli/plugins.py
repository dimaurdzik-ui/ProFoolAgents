"""
Pixel Agents Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with pixel-agents;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.pixel-agents/plugins/<name>/``
3. **Project plugins** – ``./.pixel-agents/plugins/<name>/`` (opt-in via
   ``PIXEL_AGENTS_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``pixel_agent.plugins``
   entry-point group.
...
from pixel_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``PIXEL_AGENTS_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("PIXEL_AGENTS_BUNDLED_PLUGINS")
    if env_override:
...
# ---------------------------------------------------------------------------
#
# Set ``PIXEL_AGENTS_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.pixel-agents/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("PIXEL_AGENTS_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When PIXEL_AGENTS_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Pixel Agents loggers.
    """
    ...
    logger.debug(
        "PIXEL_AGENTS_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Pixel Agents' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by pixel_cli.kanban_db when a task
    ...
    # separate `pixel-agents -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `pixel-agents kanban dispatch`),
...
ENTRY_POINTS_GROUP = "pixel_agent.plugins"

_NS_PARENT = "pixel_plugins"
...
class PluginManifest:
    ...
    #              in ~/.pixel-agents/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``pixel-agents plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Pixel Agents profile name (e.g. ``"default"``).

        Derived from ``PIXEL_AGENTS_HOME`` via
        :func:`pixel_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.pixel-agents/profiles/<name>``, or ``"custom"`` when
        ``PIXEL_AGENTS_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Pixel Agents core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``pixel-agents honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``pixel-agents <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`pixel_cli.dashboard_auth.DashboardAuthProvider`. Used by
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_pixel_dotenv()`` startup — after
        ``~/.pixel-agents/.env`` loads, before Pixel Agents reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Pixel Agents' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``pixel-agents model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: pixel_cli.main imports plugins indirectly
        from pixel_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.pixel-agents/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        if env_var_enabled("PIXEL_AGENTS_SAFE_MODE"):
            logger.info("PIXEL_AGENTS_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to pixel_cli/. Two layouts are
        ...
        # 2. User plugins (~/.pixel-agents/plugins/)
        user_dir = get_pixel_agents_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.pixel-agents/plugins/)
        if _env_enabled("PIXEL_AGENTS_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".pixel-agents" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set PIXEL_AGENTS_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with pixel-agents and must
            ...
            # all ~20 of them added several seconds to every `pixel-agents`
            # invocation — including plain `pixel-agents chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Pixel Agents ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `pixel-agents plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``pixel-agents plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`pixel-agents plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `pixel-agents plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``pixel_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``pixel_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    own loop so the caller still gets a concrete result synchropixelly. The
    ...
    thread = threading.Thread(
        target=_runner,
        name="pixel-agents-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``pixel-agents tools`` TUI so plugin-provided toolsets appear
