"""Bounded product contract for the first Pixel Agents shared-metrics slice."""
...
SCHEMA_KEY = "pixel-agents.metrics.schema_version"
SCHEMA_VERSION = "pixel-agents.metrics.event.v1"
MODEL_CALL_SCOPE = "pixel-agents.model_call"
TASK_SCOPE = "pixel-agents.task_run"
SUBSCRIBER_NAME = "pixel-agents.nemo_relay.shared_metrics"
PRIMARY_MODEL_CALL_ROLE = "primary"
MODEL_CALL_METRIC = "pixel-agents.model_call.count"
TASK_STARTED_METRIC = "pixel-agents.task_run.started"
TASK_FINISHED_METRIC = "pixel-agents.task_run.finished"
...
# in Pixel Agents's execution metadata because that flag has narrower routing/catalog
# semantics there.
...
# Pixel Agents intentionally resolves these local runtimes through the generic custom
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from pixel_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Pixel Agents terminal state to bounded task outcome dimensions."""
...
def provider_family(kwargs: dict[str, Any]) -> str:
    """Map a Pixel Agents provider to a bounded product category."""
...
def _provider_metadata(provider: str) -> tuple[str, bool, bool]:
    ...
    try:
        ...
        from pixel_cli.providers import PIXEL_AGENTS_OVERLAYS, normalize_provider
...
def _known_provider_ids() -> frozenset[str]:
    """Cache Pixel Agents's static provider catalog for the process lifetime."""
    try:
        from pixel_cli.provider_catalog import provider_catalog_by_slug
