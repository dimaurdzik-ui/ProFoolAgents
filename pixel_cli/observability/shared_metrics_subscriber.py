"""Relay subscriber for the persisted Pixel Agents shared-metrics slice."""
...
class SharedMetricsSubscriber:
    """Persist validated Pixel Agents counters from Relay lifecycle events."""
    ...
    def __init__(
        ...
        *,
        ...
        self.store = store
        self._pixel_version = pixel_version or "unknown"
    ...
    def __call__(self, event: Any) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "Unable to persist the Pixel Agents shared metric: %s",
                    metric_name,
                    exc_info=True,
                )
