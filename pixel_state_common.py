...
Extracted verbatim from pixel_state.py so the SessionDB mixin modules
(pixel_state_search / pixel_state_schema / pixel_state_portability) can
reference them without importing pixel_state (which would be a cycle).
pixel_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``pixel-agents sessions optimize-storage``. A legacy DB sits at
...
# `pixel-agents sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `pixel-agents db optimize`. This is
