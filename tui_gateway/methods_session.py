...
def _(rid, params: dict) -> dict:
    ...
    if agent_id:
        ...
        if template is None or not template.enabled:
            return _err(rid, 4008, "Unknown or disabled agent template")
        agent_prompt_version = template.prompt_version
    ...
    # and each turn re-bind PIXEL_AGENTS_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    with _profile_db(params) as db:
        ...
        try:
            ...
            # custom `PIXEL_AGENTS_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    # synchropixelly (e.g. tests of the build race) passes ``eager_build: true``
    ...
    _enable_gateway_prompts()
    home_token = (
        set_pixel_agents_home_override(str(profile_home)) if profile_home is not None else None
    )
    secret_token = (
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_pixel_agents_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            init_secret_token = (
            ...
            try:
                ...
                if init_home_token is not None:
                    reset_pixel_agents_home_override(init_home_token)
            ...
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds PIXEL_AGENTS_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``pixel-agents --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    with _profile_db(params) as db:
        ...
        if profile_home is not None:
            ...
            sessions_dir = get_pixel_agents_home() / "sessions"
        try:
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``pixel-agents gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    # Pixel credits block — agent-independent (a portal fetch), so it shows even
    # with zero API calls or on a resumed session. The TUI /usage panel renders
    # these lines regardless of `calls`. Fail-open: [] when not logged into Pixel
    # or on any portal hiccup.
    try:
        from agent.account_usage import pixel_credits_lines

        credits = pixel_credits_lines()
        if credits:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from pixel_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from pixel_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from pixel_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from pixel_cli.pets import set_pet_scale
...
def _(rid, params: dict) -> dict:
    ...
    True only when a reference-capable image backend (Pixel Portal / OpenRouter /
...
def _(rid, params: dict) -> dict:
    ...
    from pixel_cli.pixel_billing import BillingError, post_subscription_preview
...
def _(rid, params: dict) -> dict:
    ...
    billing:manage.
    """
    from pixel_cli.pixel_billing import BillingError, put_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    """
    from pixel_cli.pixel_billing import BillingError, delete_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    from pixel_cli.pixel_billing import BillingError, post_subscription_upgrade
...
def _(rid, params: dict) -> dict:
    ...
    """
    from pixel_cli.pixel_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from pixel_cli.pixel_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from pixel_cli.pixel_billing import BillingError
        ...
        granted = step_up_pixel_billing_scope(
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Pixel Agents profile home
    # (~/.pixel-agents/sessions/saved/) rather than the project/workspace CWD, and
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        home_token = (
            set_pixel_agents_home_override(parent_home) if parent_home else None
        )
        try:
