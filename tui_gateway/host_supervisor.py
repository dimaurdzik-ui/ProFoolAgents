...
def _default_registry_path() -> Path:
    return get_pixel_agents_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_pixel_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_pixel_home = expected_pixel_home if expected_pixel_home is not None else str(get_pixel_agents_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["PIXEL_AGENTS_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("pixel_home") or "")
        if got_home and got_home != self.expected_pixel_home:
            raise RuntimeError(f"compute host PIXEL_AGENTS_HOME mismatch: {got_home} != {self.expected_pixel_home}")
