...
Jobs are stored in ~/.pixel-agents/cron/jobs.json
Output is saved to ~/.pixel-agents/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own PIXEL_AGENTS_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same PIXEL_AGENTS_HOME — so a job authored in profile
# `coder` lives in `~/.pixel-agents/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_pixel_agents_home()` (the active profile home), NOT `get_default_pixel_agents_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever PIXEL_AGENTS_HOME the
...
# `_get_pixel_agents_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
PIXEL_AGENTS_DIR = get_pixel_agents_home().resolve()
...
CRON_DIR = PIXEL_AGENTS_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``pixel-agents cron status`` process share it
...
# threshold in `pixel-agents cron status` (pixel_cli/cron.py), so the two never
# drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_pixel_agents_home()
       (context-local override, then the PIXEL_AGENTS_HOME env var) — so a test
       or embedder that re-points PIXEL_AGENTS_HOME after this module was
...
# the cron inactivity timeout is disabled (PIXEL_AGENTS_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. PIXEL_AGENTS_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``PIXEL_AGENTS_CRON_TIMEOUT`` (the cron inactivity timeout the
    ...
    """
    raw = os.getenv("PIXEL_AGENTS_CRON_TIMEOUT", "").strip()
...
def _jobs_lock():
    ...
    and standalone ``pixel-agents`` CLI invocations, which previously shared no lock
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec pixel-agents pixel-agents cron create ...`` — ``docker exec``
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Pixel Agents timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `pixel_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                pixel_tz = _pixel_now().tzinfo
                dt = dt.replace(tzinfo=pixel_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _pixel_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Pixel Agents configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Pixel Agents timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _pixel_now().tzinfo
    if dt.tzinfo is None:
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Pixel Agents/system local time
    ...
    if kind == "once":
        ...
        if not _ensure_croniter():
            logger.warning(
                ...
                "reinstall pixel-agents or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `pixel-agents cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``pixel-agents cron status``) never sees a torn/truncated
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `pixel-agents cron status` can tell a thread that is merely *alive and looping*
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``pixel-agents cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``pixel-agents cron
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _pixel_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_pixel_agents_home() / "config.yaml"
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.pixel-agents/scripts/; ``.sh`` / ``.bash`` files run via bash,
    ...
    # covered, not just `pixel-agents cron create`.
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _pixel_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _pixel_now().isoformat(),
        },
    )
...
def _write_wedged_oneshot_diagnostic(job: Dict[str, Any]) -> None:
    ...
    try:
        ...
        text = (
            ...
            f"- removed at: {_pixel_now().isoformat()}\n\n"
...
def _machine_id() -> str:
    ...
    Uses ``PIXEL_AGENTS_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("PIXEL_AGENTS_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from PIXEL_AGENTS_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one PIXEL_AGENTS_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from PIXEL_AGENTS_CRON_TIMEOUT),
...
# execution. Unlike the quick-snapshot store (`pixel_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``pixel_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _pixel_now().strftime("%Y-%m-%d_%H-%M-%S")
