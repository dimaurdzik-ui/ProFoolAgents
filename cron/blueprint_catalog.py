...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``pixelagents://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``pixelagents://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"pixelagents://blueprint/{quote(blueprint.key)}{qs}"
