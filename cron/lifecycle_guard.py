...
``pixel-agents gateway restart`` (or ``launchctl kickstart ai.pixel-agents.gateway``
or ``systemctl restart pixel-agents-gateway``).  When the cron fires, the
...
``pixel-agents cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``pixel-agents gateway``, ``launchctl ... pixel-agents-gateway``,
``systemctl ... pixel-agents-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_PIXEL_AGENTS_GATEWAY=1``, and
``pixel-agents gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `pixel-agents gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:pixel-agents\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a pixel-agents-gateway label. macOS launchd
    # labels look like `ai.pixel-agents.gateway` / `pixel-agents-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated pixel-agents services (e.g.
    # `launchctl unload ai.pixel-agents.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bPixelAgents[.\-]?gateway)"
    # Branch C: systemctl ops on a pixel-agents-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bPixelAgents[.\-]?gateway)"
    # Branch D: pkill / kill targeting the pixel-agents gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<PIXEL_AGENTS_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.pixel-agents/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_pixel_agents_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `pixel-agents gateway restart` from a shell outside "
