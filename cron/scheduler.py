...
Uses a file-based lock (~/.pixel-agents/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `pixel-agents update` reloads
# the module) fail with ModuleNotFoundError for pixel_time et al.
...
def _set_cron_session_title(session_db, session_id, base_title):
    ...
    - #50536: this runs synchropixelly in the cron finally block ahead of the
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy pixel_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from pixel_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``pixel-agents tools`` config for the ``cron`` platform.
    ...
    try:
        from pixel_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``pixel-agents update`` / ``pixel-agents gateway stop`` / systemd restart, or an
...
_pixel_home: Path | None = None


def _get_pixel_agents_home() -> Path:
    """Resolve Pixel Agents home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active PIXEL_AGENTS_HOME at call time
    ...
    """
    return _pixel_home or get_pixel_agents_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    pixel_home = _get_pixel_agents_home()
    lock_dir = pixel_home / "cron"
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Pixel Agents — {task_name}"
    try:
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("PIXEL_AGENTS_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid PIXEL_AGENTS_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(
    ...
    Scripts must reside within PIXEL_AGENTS_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Pixel Agents-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against PIXEL_AGENTS_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_pixel_agents_home() / "scripts"
    ...
    # escape — scripts MUST reside within PIXEL_AGENTS_HOME/scripts/.
    try:
...
def _run_job_script_with_claim_heartbeat(
    ...
    Script execution is synchropixel and may legitimately outlive the stale
...
def run_job(
    ...
    #
    # Bounded with its own timeout (separate from PIXEL_AGENTS_CRON_TIMEOUT, which
    # only watches the agent's run_conversation below): SessionDB.__init__
    # opens/migrates state.db synchropixelly and has no timeout of its own
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("PIXEL_AGENTS_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid PIXEL_AGENTS_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_pixel_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    _cron_session_id = f"cron_{job_id}_{_pixel_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["PIXEL_AGENTS_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed PIXEL_AGENTS_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads PIXEL_AGENTS_SESSION_PLATFORM
    #     and PIXEL_AGENTS_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via PIXEL_AGENTS_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     PIXEL_AGENTS_SESSION_PLATFORM == "telegram".
    ...
    #     send_message gate read PIXEL_AGENTS_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the PIXEL_AGENTS_CRON_AUTO_DELIVER_* vars set
    # below, so clearing PIXEL_AGENTS_SESSION_* here does not affect delivery.
    ...
    _ctx_tokens = set_session_vars(
        ...
        # PIXEL_AGENTS_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ PIXEL_AGENTS_SESSION_KEY, which risks routing a cron subagent's output
        # into an unrelated user chat.)
        #
        # Declaring the channel stateless routes delegate_task to its existing
        # inline/synchropixel path, so results return within the job's own turn.
        # See declare_stateless_channel(). Upstream: #53027, #63142.
        async_delivery=False,
        cwd=_job_workdir or "",
    )
    _cron_delivery_vars = (
        "PIXEL_AGENTS_CRON_AUTO_DELIVER_PLATFORM",
        "PIXEL_AGENTS_CRON_AUTO_DELIVER_CHAT_ID",
        "PIXEL_AGENTS_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        # load_pixel_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this PIXEL_AGENTS_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_pixel_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from pixel_cli.env_loader import (
            load_pixel_dotenv,
            reset_secret_source_cache,
        )
        reset_secret_source_cache()
        load_pixel_dotenv(pixel_home=_get_pixel_agents_home())

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["PIXEL_AGENTS_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["PIXEL_AGENTS_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["PIXEL_AGENTS_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        # cron-fleet default) > PIXEL_AGENTS_MODEL env > config.yaml ``model:``
        ...
        model = job.get("model") or os.getenv("PIXEL_AGENTS_MODEL") or ""
        ...
        try:
            ...
            _cfg_path = str(_get_pixel_agents_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"PIXEL_AGENTS_MODEL={os.getenv('PIXEL_AGENTS_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`cronjob action=update job_id={job_id} model=<name>` or set a "
                "default with `pixel-agents model <name>`."
            )
        ...
        prefill_file = (
            os.getenv("PIXEL_AGENTS_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_pixel_agents_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject PIXEL_AGENTS_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        # to a paid PROVIDER (e.g. pixel) OR a paid MODEL on the same provider
        ...
        agent = AIAgent(
            ...
            # PIXEL_AGENTS_HOME. When a workdir is configured, also inject project
        ...
        # override via PIXEL_AGENTS_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("PIXEL_AGENTS_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid PIXEL_AGENTS_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_pixel_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_pixel_now().strftime('%b %d %H:%M')}"
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_pixel_agents_home())
        )
...
def tick(
    ...
    *,
    ...
    Args:
        ...
        can_dispatch: Optional synchropixel gate; false leaves due jobs untouched
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _pixel_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _pixel_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set PIXEL_AGENTS_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("PIXEL_AGENTS_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid PIXEL_AGENTS_CRON_MAX_PARALLEL value; defaulting to unbounded")
