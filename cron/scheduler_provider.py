...
class InProcessCronScheduler(CronScheduler):
    ...
    a daemon thread. ``can_dispatch`` is an optional synchropixel gate supplied
    ...
    def start(
        ...
        *,
        ...
        # only the process-global PIXEL_AGENTS_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `pixel-agents cron
        ...
        # Heartbeat once before the first sleep so `pixel-agents cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `pixel-agents cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_pixel_agents_home_override()`` + ``use_cron_store()``
        ...
        for entry in profile_homes:
            ...
            home_token = set_pixel_agents_home_override(str(home))
            try:
            ...
            for entry in profile_homes:
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `pixel-agents cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:
