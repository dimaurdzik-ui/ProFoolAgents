# Contribuir a Pixel Agents

¡Gracias por contribuir a Pixel Agents! Esta guía cubre todo lo que necesitas: configurar tu entorno de desarrollo, entender la arquitectura, decidir qué construir y conseguir que tu PR sea aceptado.
...
2. **Compatibilidad entre plataformas** — macOS, diferentes distribuciones de Linux y WSL2 en Windows. Queremos que Pixel Agents funcione en todas partes.
...
Las habilidades incluidas (en `skills/`) se envían con cada instalación de Pixel Agents. Deben ser **ampliamente útiles para la mayoría de los usuarios**:
...
Si tu habilidad es oficial y útil pero no universalmente necesaria (ej., una integración de servicio de pago, una dependencia pesada), ponla en **`optional-skills/`** — se envía con el repositorio pero no está activada por defecto. Los usuarios pueden descubrirla a través de `pixel-agents skills browse` (etiquetada como "oficial") e instalarla con `pixel-agents skills install` (sin advertencia de terceros, confianza integrada).

Si tu habilidad es especializada, contribuida por la comunidad o de nicho, es mejor para un **Skills Hub** — súbela a un registro de habilidades y compártela en el [Discord de Pixel Agents](https://discord.gg/PixelResearch). Los usuarios pueden instalarla con `pixel-agents skills install`.

---

## Proveedores de Memoria: Publicar como Plugin Independiente

**Ya no aceptamos nuevos proveedores de memoria en este repositorio.** El conjunto de proveedores integrados en `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) está cerrado. Si quieres añadir un nuevo backend de memoria, publícalo como un **repositorio de plugin independiente** que los usuarios instalen en `~/.pixel-agents/plugins/` (o a través de un entry point de pip).

Los plugins de memoria independientes:

- Implementan el mismo ABC `MemoryProvider` (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown` y opcionalmente `post_setup(pixel_home, config)` para integración con el asistente de configuración
...
- Se integran con `pixel-agents memory setup` a través de `post_setup()` — sin necesidad de tocar el código base
...
├── cli.py                    # Clase PixelAgentsCLI — TUI interactiva, integración prompt_toolkit
├── model_tools.py            # Orquestación de herramientas (capa delgada sobre tools/registry.py)
├── toolsets.py               # Agrupaciones y presets de herramientas (pixel-agents-cli, pixel-agents-telegram, etc.)
├── pixel_state.py           # Base de datos de sesiones SQLite con búsqueda de texto completo FTS5, títulos de sesión
...
│
├── pixel_cli/               # Implementaciones de comandos CLI
...
│   ├── auth.py                   # Resolución de proveedor, OAuth, Pixel Portal
...
│
├── skills/                   # Habilidades incluidas (copiadas a ~/.pixel-agents/skills/ en la instalación)
├── optional-skills/          # Habilidades opcionales oficiales (descubribles vía hub, no activadas por defecto)
├── tests/                    # Suite de tests
├── website/                  # Sitio de documentación (api.pixelagents.com)
│
├── cli-config.yaml.example   # Configuración de ejemplo (copiada a ~/.pixel-agents/config.yaml)
└── AGENTS.md                 # Guía de desarrollo para asistentes de codificación IA
```

### Configuración del usuario (almacenada en `~/.pixel-agents/`)

| Ruta | Propósito |
|------|-----------|
| `~/.pixel-agents/config.yaml` | Configuración (modelo, terminal, toolsets, compresión, etc.) |
| `~/.pixel-agents/.env` | Claves API y secretos |
| `~/.pixel-agents/auth.json` | Credenciales OAuth (Pixel Portal) |
| `~/.pixel-agents/skills/` | Todas las habilidades activas (incluidas + instaladas desde hub + creadas por el agente) |
| `~/.pixel-agents/memories/` | Memoria persistente (MEMORY.md, USER.md) |
| `~/.pixel-agents/state.db` | Base de datos de sesiones SQLite |
| `~/.pixel-agents/sessions/` | Índice de enrutamiento del gateway (`sessions.json`), migas de pan de solicitudes, transcripciones `*.jsonl` del gateway y (opcionalmente) snapshots JSON por sesión cuando `sessions.write_json_snapshots: true` está configurado. Los snapshots por sesión están desactivados por defecto; state.db es canónica. |
| `~/.pixel-agents/cron/` | Datos de trabajos programados |
| `~/.pixel-agents/whatsapp/session/` | Credenciales del puente WhatsApp |
...
- **Persistencia de sesión**: Todas las conversaciones se almacenan en SQLite (`pixel_state.py`) con búsqueda de texto completo y títulos de sesión únicos.
...
(por ejemplo `_PIXEL_AGENTS_CORE_TOOLS` o un toolset dedicado); de lo contrario la herramienta
...
2. **Las herramientas referenciadas en el cuerpo de SKILL.md deben ser herramientas nativas de Pixel Agents o servidores MCP que la habilidad espere explícitamente.** Usa los nombres de herramientas en comillas invertidas: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, etc.
...
Pixel Agents usa un sistema de skins basado en datos — no se necesitan cambios de código para añadir una nueva skin.

**Opción A: Skin de usuario (archivo YAML)**

Crea `~/.pixel-agents/skins/<nombre>.yaml`:
...
Añade al dict `_BUILTIN_SKINS` en `pixel_cli/skin_engine.py`. Usa el mismo esquema que arriba pero como dict de Python.
...
Pixel Agents se ejecuta en Linux, macOS y Windows nativo (además de WSL2). Al escribir código
...
Pixel Agents tiene acceso al terminal. La seguridad importa.
...
2. **Probar manualmente**: Ejecuta `pixel-agents` y ejercita la ruta de código que cambiaste
...
- Usa [GitHub Issues](https://github.com/PixelResearch/pixel-agents/issues)
- Incluye: SO, versión de Python, versión de Pixel Agents (`pixel-agents version`), traza de error completa
...
- **Discord**: [discord.gg/PixelResearch](https://discord.gg/PixelResearch) — para preguntas, mostrar proyectos y compartir habilidades
