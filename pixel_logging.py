"""Centralized logging setup for Pixel Agents.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.pixel-agents/logs/`` (profile-aware via ``get_pixel_agents_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``pixel_cli.web_server``,
    ``pixel_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Pixel Agents (TUI, gateway, ``hy_memory`` server, MCP
...
from pixel_constants import get_config_path, get_pixel_agents_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_pixel_session_injector", False):
    ...
    _session_record_factory._pixel_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``pixel-agents logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``pixel-agents logs --component gateway``.
    "gateway": ("gateway", "pixel_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("pixel_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "pixel_cli.web_server",
        "pixel_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Pixel Agents logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    pixel_home
        Override for the Pixel Agents home directory.  Falls back to
        ``get_pixel_agents_home()`` (profile-aware).
    log_level
    ...
    home = pixel_home or get_pixel_agents_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_pixel_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._pixel_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the pixel-agents group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from pixel_cli.config import is_managed
...
# ---------------------------------------------------------------------------
# Asynchropixel file logging — keep the cross-process rotation lock off the loop
#
# The rotating file handlers serialize rollover with a cross-process lock (see
# the module header): when several Pixel Agents processes log to the same file, an
...
class _NonFormattingQueueHandler(QueueHandler):
    ...
    record is still owned by the emitting thread (and any synchropixel handler
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._pixel_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="pixel-agents-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_pixel_queue", False):
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse pixel_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when pixel_cli.config isn't importable (bare
        # pixel_logging consumers).
        try:
            from pixel_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
