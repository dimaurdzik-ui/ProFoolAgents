...
A **profile distribution** packages a complete Pixel Agents agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a Pixel Agents agent meant sending someone:
...
```bash
pixel-agents profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `pixel-agents profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Pixel Agents in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`pixel-agents profile export` / `import`](../reference/profile-commands.md#pixel-agents-profile-export) — that's what those are for.
...
:::caution
**Pixel Agents does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `pixel-agents profile install` or `pixel-agents profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
pixel-agents profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.pixel-agents/profiles/research-bot/SOUL.md
...
Create `~/.pixel-agents/profiles/research-bot/distribution.yaml`:

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
pixel_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
Create `~/.pixel-agents/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.pixel-agents/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `pixel-agents profile update research-bot` will pull the latest.
...
5. Copies distribution-owned files into `~/.pixel-agents/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
pixel-agents profile install github.com/you/research-bot

# Full HTTPS
pixel-agents profile install https://github.com/you/research-bot.git

# SSH
pixel-agents profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
pixel-agents profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
pixel-agents profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
pixel-agents profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
pixel-agents profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
pixel-agents profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Pixel Agents distribution.
...
```bash
cp ~/.pixel-agents/profiles/research-bot/.env.EXAMPLE ~/.pixel-agents/profiles/research-bot/.env
...
```bash
pixel-agents profile info research-bot
```
...
`pixel-agents profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
pixel-agents profile delete research-bot
```
...
Path:    ~/.pixel-agents/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `pixel-agents profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.pixel-agents/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
# Each engineer
pixel-agents profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `pixel-agents profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.pixel-agents/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/pixel-agents-polymarket-trader.git
git push -u origin main --tags

# Anyone
pixel-agents profile install github.com/you/pixel-agents-polymarket-trader --alias
```
...
You built Pixel Agents-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
pixel-agents profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
pixel-agents profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `pixel-agents profile info <name>` and hold off on updates until you're ready.
:::
...
pixel-agents profile info research-bot | grep Version
...
```yaml
# ~/.pixel-agents/profiles/research-bot/local/my-overrides.yaml
...
pixel-agents profile delete research-bot --yes
pixel-agents profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
pixel-agents profile update research-bot --force-config --yes
```
...
pixel-agents profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.pixel-agents/profiles/forked-research-bot/
...
pixel-agents profile install ~/.pixel-agents/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
pixel-agents profile delete research-bot-test --yes
pixel-agents profile install ~/.pixel-agents/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `pixel-agents -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `pixel-agents profile` — not a parallel command tree.
...
- Reserved profile names (`pixel-agents`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`pixel-agents profile export` / `import`](../reference/profile-commands.md#pixel-agents-profile-export) — local backup / restore (not distribution)
- [Using SOUL with Pixel Agents](../guides/use-soul-with-pixel-agents.md) — authoring personalities
