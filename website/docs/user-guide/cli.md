...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Pixel Agents terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Pixel Agents's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `pixel-agents setup --portal` — and you're ready to `pixel-agents chat`. See [Pixel Portal](/integrations/pixel-portal).
:::

:::tip
Pixel Agents also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `pixel-agents --tui` — see the [TUI](tui.md) guide.
:::
...
pixel-agents chat --model "anthropic/claude-sonnet-4"

# With a specific provider
pixel-agents chat --provider pixel        # Use Pixel Portal
pixel-agents chat --provider openrouter  # Force OpenRouter

# With specific toolsets
pixel-agents chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
pixel-agents -s pixel-agents-dev,github-auth
pixel-agents chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
pixel-agents --continue             # Resume the most recent CLI session (-c)
pixel-agents --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
pixel-agents chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
pixel-agents -w                         # Interactive mode in worktree
pixel-agents -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Pixel Agents CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Pixel Agents CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `PIXEL_AGENTS_YOLO_MODE` is on (either `pixel-agents --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Pixel Agents to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Pixel Agents supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Pixel Agents replies |
...
```bash
pixel-agents -s pixel-agents-dev,github-auth
pixel-agents chat -s github-pr-workflow -s github-auth
```

Pixel Agents loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.pixel-agents/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.pixel-agents/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Pixel Agents..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Pixel Agents recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Pixel Agents — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Pixel Agents is working, Pixel Agents prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Pixel Agents to the background — just like any terminal process. The shell prints a confirmation:

```
Pixel Agents has been suspended. Run `fg` to bring Pixel Agents back.
```
...
Resume this session with:
  pixel-agents --resume 20260225_143052_a1b2c3
...
```bash
pixel-agents --continue                          # Resume the most recent CLI session
pixel-agents -c                                  # Short form
pixel-agents -c "my project"                     # Resume a named session (latest in lineage)
pixel-agents --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
pixel-agents --resume "refactoring auth"         # Resume by title
pixel-agents -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `pixel-agents sessions rename <id> <title>` from the command line. Use `pixel-agents sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Pixel Agents's SQLite state database under `~/.pixel-agents/state.db`. The database keeps:
...
Pixel Agents immediately confirms the task and gives you back the prompt:
