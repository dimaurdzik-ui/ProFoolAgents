...
description: Control your smart home with Pixel Agents via Home Assistant integration.
...
Pixel Agents integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Pixel Agents"
5. Copy the token
...
Pixel Agents registers four tools for smart home control:
...
Configure which events the agent sees in `~/.pixel-agents/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Pixel Agents".
...
The adapter reads credentials from `~/.pixel-agents/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Pixel Agents profile
...
running Pixel Agents — `curl -H "Authorization: Bearer <token>" <url>/api/` should
