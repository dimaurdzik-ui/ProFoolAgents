---
sidebar_position: 30
title: "Pixel Agents Relay"
description: "Connect Pixel Agents to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Pixel Agents Relay (Connector)
...
Pixel Agents Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Pixel Portal access token from your existing login
   (`~/.pixel-agents/auth.json`) — this proves which Pixel org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.pixel-agents/.env`:
...
`pixel-agents gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.pixel-agents/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
| `gateway.idp.token_url` | `config.yaml` | When set, enrollment/provisioning authenticates via generic OAuth2 client-credentials against your own IdP instead of Pixel Portal. |
...
token. Re-login with `pixel-agents auth add pixel` (or `pixel-agents setup`) and retry.
...
startup. Restart it (`pixel-agents gateway restart`).
