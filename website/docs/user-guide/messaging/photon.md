...
Connect Pixel Agents to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Pixel Agents — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Pixel Agents runs it in a
...
pixel-agents photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Pixel Agents` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.pixel-agents/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.pixel-agents/.env`
...
(device token, dashboard project id) lives in `~/.pixel-agents/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Pixel Agents
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Pixel Agents replies with a pairing code. Approve it with:

```bash
pixel-agents pairing approve photon <CODE>
```

Use `pixel-agents pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.pixel-agents/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.pixel-agents/.env`):
...
By default Pixel Agents responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Pixel Agents` and
`@Pixel Agents agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Pixel Agents will reply.

## Status & troubleshooting

```bash
pixel-agents photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Pixel Agents uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run pixel-agents photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `pixel-agents photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `pixel-agents photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `pixel-agents photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Pixel Agents sends images, voice
  ...
  media.
- **Native polls are supported.** Pixel Agents sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Pixel Agents sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `pixel-agents send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<pixel-agents-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Pixel Agents is set
...
| `PHOTON_MENTION_PATTERNS` | Pixel Agents wake words  | JSON list / comma / newline regex patterns for group mentions |
