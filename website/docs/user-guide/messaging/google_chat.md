...
sidebar_position: 12
title: "Google Chat"
description: "Set up Pixel Agents as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Pixel Agents to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Pixel Agents
...
> Run `pixel-agents gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Pixel Agents can read (e.g.,
`~/.pixel-agents/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a pixel-agents restart)
...
- Principal: `pixel-agents-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Pixel Agents calls
...
- **App name**: whatever you want users to see ("Pixel Agents" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/pixel-agents-chat-events`.
...
`ADDED_TO_SPACE` event that Pixel Agents uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Pixel Agents

Add the Google Chat section to `~/.pixel-agents/.env`:
...
Install the dependencies the Google Chat adapter needs (no Pixel Agents extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Pixel Agents is thinking…" marker, then
...
`~/.pixel-agents/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Pixel Agents is thinking…").
...
Thread support: when a user replies inside a thread, Pixel Agents detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Pixel Agents session.
...
3. Download the JSON. Move it onto the host that runs Pixel Agents.
4. Register the client with Pixel Agents (run under the profile you want it scoped to):
...
pixel-agents -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Pixel Agents home (e.g.
`~/.pixel-agents/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.pixel-agents/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.pixel-agents/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Pixel Agents isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `pixel-agents gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Pixel Agents will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.pixel-agents/google_chat_user_tokens/<sanitized_email>.json` (filesystem
