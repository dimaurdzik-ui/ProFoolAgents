...
description: "Chat with Pixel Agents from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Pixel Agents from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Pixel Agents](/guides/use-voice-mode-with-pixel-agents).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [Pixel Portal](/integrations/pixel-portal) subscription bundles all of them.
:::
...
```mermaid
flowchart TB
    subgraph Gateway["Pixel Agents Gateway"]
...
For group chats, hooks, and automation flows, Pixel Agents supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Pixel Agents keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Pixel Agents does not hide failures just because the text resembles a silence token.
...
```bash
pixel-agents gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
pixel-agents gateway              # Run in foreground
pixel-agents gateway setup        # Configure messaging platforms interactively
pixel-agents gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo pixel-agents gateway install --system   # Linux only: install a boot-time system service
pixel-agents gateway start        # Start the default service
pixel-agents gateway stop         # Stop the default service
pixel-agents gateway status       # Check default service status
pixel-agents gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.pixel-agents/config.yaml"
gateway:
...
```bash
pixel-agents gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Pixel Agents sends heartbeats
...
| `/update` | Update Pixel Agents to the latest version |
...
with the `session_reset` section in `~/.pixel-agents/config.yaml`:
...
Configure per-platform overrides in `~/.pixel-agents/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.pixel-agents/gateway-config.yaml`:
...
pixel-agents pairing approve telegram XKGH5N7P

# Other pairing commands:
pixel-agents pairing list          # View pending + approved users
pixel-agents pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Pixel Agents appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.pixel-agents/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.pixel-agents/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `PIXEL_AGENTS_HOME`:

- `~/.pixel-agents/status_phrases.yaml` or any `*.yaml` in `~/.pixel-agents/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to PIXEL_AGENTS_HOME
...
Off by default. When enabled, Pixel Agents prepends a human-readable timestamp
...
Pixel Agents confirms immediately:
...
Each `/background` prompt spawns a **separate agent instance** that runs asynchropixelly:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.pixel-agents/config.yaml`:
...
```bash
PIXEL_AGENTS_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
pixel-agents gateway install               # Install as user service
pixel-agents gateway start                 # Start the service
pixel-agents gateway stop                  # Stop the service
pixel-agents gateway status                # Check status
journalctl --user -u pixel-agents-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo pixel-agents gateway install --system
sudo pixel-agents gateway start --system
sudo pixel-agents gateway status --system
journalctl -u pixel-agents-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Pixel Agents installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit pixel-agents-gateway` (or `sudo systemctl edit pixel-agents-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `pixel-agents update`. When `pixel-agents update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart pixel-agents-gateway` command (it never blocks on an interactive password prompt).
...
```bash
pixel-agents gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `pixel-agents update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo pixel-agents update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/pixel-agents-gateway`:

```
pixel-agents ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed pixel-agents-gateway*, /usr/bin/systemctl --no-ask-password start pixel-agents-gateway*, /usr/bin/systemctl --no-ask-password restart pixel-agents-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Pixel Agents will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Pixel Agents installations on the same machine (with different `PIXEL_AGENTS_HOME` directories), each gets its own systemd service name. The default `~/.pixel-agents` uses `pixel-agents-gateway`; other installations use `pixel-agents-gateway-<hash>`. The `pixel-agents gateway` commands automatically target the correct service for your current `PIXEL_AGENTS_HOME`.
:::

### macOS (launchd)

```bash
pixel-agents gateway install               # Install as launchd agent
pixel-agents gateway start                 # Start the service
pixel-agents gateway stop                  # Stop the service
pixel-agents gateway status                # Check status
tail -f ~/.pixel-agents/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.pixel-agents.gateway.plist`. It includes three environment variables:
...
- **PIXEL_AGENTS_HOME** — scopes the gateway to your Pixel Agents installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `pixel-agents gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `PIXEL_AGENTS_HOME` directory gets its own launchd label. The default `~/.pixel-agents` uses `ai.pixel-agents.gateway`; other installations use `ai.pixel-agents.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `pixel-agents-cli` | Full access |
| Telegram | `pixel-agents-telegram` | Full tools including terminal |
| Discord | `pixel-agents-discord` | Full tools including terminal |
| WhatsApp | `pixel-agents-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `pixel-agents-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `pixel-agents-slack` | Full tools including terminal |
| Google Chat | `pixel-agents-google_chat` | Full tools including terminal |
| Signal | `pixel-agents-signal` | Full tools including terminal |
| SMS | `pixel-agents-sms` | Full tools including terminal |
| Email | `pixel-agents-email` | Full tools including terminal |
| Home Assistant | `pixel-agents-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `pixel-agents-mattermost` | Full tools including terminal |
| Matrix | `pixel-agents-matrix` | Full tools including terminal |
| DingTalk | `pixel-agents-dingtalk` | Full tools including terminal |
| Feishu/Lark | `pixel-agents-feishu` | Full tools including terminal |
| WeCom | `pixel-agents-wecom` | Full tools including terminal |
| WeCom Callback | `pixel-agents-wecom-callback` | Full tools including terminal |
| Weixin | `pixel-agents-weixin` | Full tools including terminal |
| BlueBubbles | `pixel-agents-bluebubbles` | Full tools including terminal |
| QQBot | `pixel-agents-qqbot` | Full tools including terminal |
| Yuanbao | `pixel-agents-yuanbao` | Full tools including terminal |
| Microsoft Teams | `pixel-agents-teams` | Full tools including terminal |
| API Server | `pixel-agents-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `pixel-agents-webhook` | Full tools including terminal |
| Raft | `pixel-agents-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.pixel-agents/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
