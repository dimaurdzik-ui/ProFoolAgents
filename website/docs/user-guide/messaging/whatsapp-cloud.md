...
description: "Set up Pixel Agents as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

Pixel Agents can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The Pixel Agents gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a Pixel Agents limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. Pixel Agents degrades gracefully if absent.
...
2. **Add** → name (e.g. `pixel-agents-bot`) → role: **Admin**.
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.pixel-agents/.env` → restart the gateway.

System User tokens don't expire unless you explicitly revoke them.

---

## Exposing Pixel Agents to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Pixel Agents gateway has to be reachable from Meta's servers.  Three common ways:
...
2. Generate a **Verify Token** — the wizard does this for you with `secrets.token_urlsafe(32)`; if you're configuring manually, run:
   ...
   ```
   Save it as `WHATSAPP_CLOUD_VERIFY_TOKEN` in `~/.pixel-agents/.env`.
3. Start the Pixel Agents gateway: `pixel-agents gateway`.
...
## Allowlist (Pixel Agents-side)

In addition to Meta's recipient whitelist, Pixel Agents has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.pixel-agents/.env`:
...
The `pixel-agents whatsapp-cloud` wizard prints these links at the end of setup. None of this is required for the bot to work — it's pure polish for how your bot appears to users.

---

## Configuration reference

All settings live in `~/.pixel-agents/.env`.  Required values are in **bold**.
...
- **Voice notes** — auto-downloaded as `.ogg`, transcribed via your configured STT provider (local faster-whisper, OpenAI/Pixel, Groq, etc.), then handed to the agent as text.
...
When the agent invokes any of these flows, Pixel Agents uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
Pixel Agents acknowledges inbound messages immediately:
...
Pixel Agents TTS produces MP3. Two paths:
...
Pixel Agents warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Pixel Agents. If you need it, please [open an issue](https://github.com/PixelResearch/pixel-agents/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  Pixel Agents's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  Pixel Agents doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Verify token mismatch** — the token in `~/.pixel-agents/.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's dashboard.  Run the curl probe above to confirm the gateway's verify handshake works locally first.
- **Gateway not running** — check `pixel-agents gateway` is up.
- **App Secret not set** — without it, Pixel Agents refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in Pixel Agents.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `pixel-agents tools list` and verify the platform is using `pixel-agents-whatsapp` (the default Cloud adapter toolset, same as Baileys).

If the model emits tool-call-shaped text instead of a structured call, it usually means the toolset was effectively empty.  See `pixel_cli/platforms.py` for the platform → default toolset mapping.

### STT (voice note transcription) returns empty / "could not transcribe"

The default `stt.provider: local` requires `pip install faster-whisper`.  If you're a Pixel subscriber, you can route STT through Meta's managed audio gateway instead:

```bash
pixel-agents config set stt.provider openai
pixel-agents config set stt.use_gateway true
pixel-agents gateway restart
```

This uses your Pixel Portal access token instead of needing a separate OpenAI key.

---

## Security notes

- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that Pixel Agents will accept as authentic.
...
| | Baileys (`pixel-agents whatsapp`) | Cloud API (`pixel-agents whatsapp-cloud`) |
|---|---|---|
...
Most users running Pixel Agents for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
