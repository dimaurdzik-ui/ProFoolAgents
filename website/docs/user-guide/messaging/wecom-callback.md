...
Connect Pixel Agents to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Pixel Agents supports two WeCom integration modes:
...
> Run `pixel-agents gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Pixel Agents decrypts the message, queues it for the agent
...
(Use `pixel-agents gateway start` only after `pixel-agents gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Pixel Agents and the token the
...
in `~/.pixel-agents/.env` values around `=` will also break signature checks. After
fixing, restart `pixel-agents gateway run`.
...
Check `pixel-agents gateway run` logs for the bound host/port. If the adapter bound to
