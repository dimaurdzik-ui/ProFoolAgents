...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Pixel Agents' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Pixel Agents")

The wake word turns Pixel Agents into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Pixel Agents listens in the background for a spoken
trigger phrase. Say it, and Pixel Agents starts a fresh session, opens the microphone,
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey pixel-agents"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey pixel-agents"** — a model for it ships with Pixel Agents, so
...
```bash
cd ~/.pixel-agents/pixel-agents && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `pixel-agents` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.pixel-agents/config.yaml`,
...
wake_word:
  ...
  phrase: "hey pixel-agents"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_pixel         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey pixel-agents" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey pixel-agents" utterances are missed.
...
default) to let Pixel Agents choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Pixel Agents installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Pixel Agents surfaces, and `surface` picks which one
...
| `cli` | Only the classic `pixel-agents` CLI. |
| `tui` | Only `pixel-agents --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Pixel Agents surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Pixel Agents does not silently fail over to another open surface.
...
"Hey Pixel Agents" works out of the box — the bundled openWakeWord model
(`model: hey_pixel`) is the default. To wake on something else, the easiest
...
- "hey pixel-agents" → default profile
...
command (`pixel-agents -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.pixel-agents/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("pixel-agents" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Pixel Agents" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  porcupine:
    keyword: ~/.pixel-agents/wakewords/hey_pixel.ppn
```

Set your access key in `~/.pixel-agents/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `pixel-agents tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `pixel-agents[wake]`).
...
but the phrase never fires. Pixel Agents detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Pixel Agents
backend (it may appear as your terminal, `python`, or Pixel Agents), then toggle the
...
```bash
pixel-agents config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
pixel-agents config set wake_word.input_device null
```
