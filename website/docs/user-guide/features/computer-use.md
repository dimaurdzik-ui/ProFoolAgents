...
Pixel Agents can drive your desktop — clicking, typing, scrolling,
...
```
pixel-agents computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `pixel-agents computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `pixel-agents tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Pixel Agents app). `pixel-agents computer-use doctor` will tell you which permission is missing. |
...
```
pixel-agents -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.pixel-agents/config.yaml`.

## Permission modes and logged-in browser profiles

Pixel Agents maps its existing approval UX onto cua-driver 0.10's immutable daemon
modes. There is no second permission toggle to keep in sync:

| Pixel Agents session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Pixel Agents approvals; Cua stops at its protected boundary | Refuses unless a certified protected host is available; Pixel Agents does not claim one today |
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Pixel Agents risk acceptance; no runtime Cua prompts | Allowed within Cua's built-in, managed, and user policy ceilings |

The unrestricted daemon is private to that Pixel Agents session. Turning `/yolo`
...
daemon's mode or grants another Pixel Agents conversation the same authority.
...
## `pixel-agents computer-use doctor` — your first triage stop

`pixel-agents computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ pixel-agents computer-use doctor
...
agent is acting here." Each Pixel Agents run declares its own cua-driver
**session id** (something like `pixel-agents-3a7b9c14d2e8`); the cursor's
...
Pixel Agents intentionally keeps its skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Pixel Agents-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Pixel Agents skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Pixel Agents
...
Pixel Agents applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.pixel-agents/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Pixel Agents applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Pixel Agents agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Pixel Agents agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Pixel Agents warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Pixel Agents from inside the RDP / console
...
```
PIXEL_AGENTS_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
PIXEL_AGENTS_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Pixel Agents disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Pixel Agents sets
...
When it's on, `pixel-agents computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Pixel Agents at a binary you built from source instead
of the published release. Pixel Agents resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`PIXEL_AGENTS_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Pixel Agents testing (see notes).
...
### Option B — point Pixel Agents straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`PIXEL_AGENTS_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
PIXEL_AGENTS_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
PIXEL_AGENTS_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Pixel Agents is using your build

- `pixel-agents computer-use status` prints the resolved binary path and
  version.
- `pixel-agents computer-use doctor` confirms the binary is reachable and
...
- **Pixel Agents spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Pixel Agents YOLO, Pixel Agents instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Pixel Agents
  change needed either way.
- **Local builds skip the version check.** Pixel Agents warns when the
...
**First action when anything's off: run `pixel-agents computer-use doctor`.**
...
Run `pixel-agents computer-use install` to fetch the cua-driver binary, or
run `pixel-agents tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `pixel-agents computer-use doctor` will
...
- **Pixel Agents-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Pixel Agents `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Pixel Agents (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Pixel Agents among them)
