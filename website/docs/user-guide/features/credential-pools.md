...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Pixel Agents automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [Pixel Portal](/integrations/pixel-portal) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
If you already have an API key set in `.env`, Pixel Agents auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
pixel-agents auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
pixel-agents auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
pixel-agents auth add anthropic --type oauth
...
Run `pixel-agents auth` with no subcommand for an interactive wizard:
...
For providers that support both API keys and OAuth (Anthropic, Pixel, Codex), the add flow asks which type:
...
| Command | Description |
|---------|-------------|
| `pixel-agents auth` | Interactive pool management wizard |
| `pixel-agents auth list` | Show all pools and credentials |
| `pixel-agents auth list <provider>` | Show a specific provider's pool |
| `pixel-agents auth add <provider>` | Add a credential (prompts for type and key) |
| `pixel-agents auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `pixel-agents auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `pixel-agents auth remove <provider> <index>` | Remove credential by 1-based index |
| `pixel-agents auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `pixel-agents auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `pixel-agents model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via pixel-agents model:
pixel-agents auth list
# Shows:
...
pixel-agents auth add Together.ai --api-key sk-together-second-key
```
...
Pixel Agents automatically discovers credentials from multiple sources and seeds the pool on startup:

| Source | Example | Auto-seeded? |
|--------|---------|-------------|
| Environment variables | `OPENROUTER_API_KEY`, `ANTHROPIC_API_KEY` | Yes |
| OAuth tokens (auth.json) | Codex device code, Pixel device code | Yes |
| Claude Code credentials | `~/.claude/.credentials.json` | Yes (Anthropic) |
| Pixel Agents PKCE OAuth | `~/.pixel-agents/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `pixel-agents auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `pixel-agents auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Pixel Agents can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Pixel Agents-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`pixel_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`pixel_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.pixel-agents/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Pixel Agents' credential store, so its token remains persistable.
