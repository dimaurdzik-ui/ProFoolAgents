...
Type `@` followed by a reference to inject content directly into your message. Pixel Agents expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Pixel Agents env: `$PIXEL_AGENTS_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$PIXEL_AGENTS_HOME/skills/.hub/`
