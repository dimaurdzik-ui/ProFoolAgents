...
**Tool Search** is Pixel Agents' opt-in progressive-disclosure layer for that
...
:::info Built-in Pixel Agents tools never defer
The tools that make up Pixel Agents' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_PIXEL_AGENTS_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Pixel Agents **unwraps the bridge** and
...
- **No JS sandbox.** Pixel Agents uses the simpler "structured tools" mode
