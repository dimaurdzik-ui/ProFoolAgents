...
It exists so that skills created via the [self-improvement loop](/user-guide/features/skills#agent-managed-skills-skill_manage-tool) don't pile up forever. Every time the agent solves a novel problem and saves a skill, that skill lands in `~/.pixel-agents/skills/`. Without maintenance, you end up with dozens of narrow near-duplicates that pollute the catalog and waste tokens.

By default (`prune_builtins: true`) the curator can archive **unused bundled built-in skills** (shipped with the repo) after `archive_after_days` of non-use, alongside the agent-created skills it primarily manages. Hub-installed skills (from [agentskills.io](https://agentskills.io)) are always off-limits. Set `curator.prune_builtins: false` to restore the old agent-created-only behavior, where bundled skills are never touched. The curator also **never auto-deletes** — the worst outcome is archival into `~/.pixel-agents/skills/.archive/`, which is recoverable.

Tracks [issue #7816](https://github.com/PixelResearch/pixel-agents/issues/7816).

## How it runs

The curator is triggered by an inactivity check, not a cron daemon. On CLI session start, and on a recurring tick inside the gateway's cron-ticker thread, Pixel Agents checks whether:
...
On a brand-new install (or the first time a pre-curator install ticks after `pixel-agents update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `pixel-agents curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.pixel-agents/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
**Easiest — `pixel-agents model`:**

```bash
pixel-agents model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`pixel-agents model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
pixel-agents curator status         # last run, counts, pinned list, LRU top 5
pixel-agents curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
pixel-agents curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
pixel-agents curator run --background  # fire-and-forget: start the run in a background thread
pixel-agents curator run --dry-run  # preview only — report without any mutations
pixel-agents curator backup         # take a manual snapshot of ~/.pixel-agents/skills/
pixel-agents curator rollback       # restore from the newest snapshot
pixel-agents curator rollback --list     # list available snapshots
pixel-agents curator rollback --id <ts>  # restore a specific snapshot
pixel-agents curator rollback -y         # skip the confirmation prompt
pixel-agents curator pause          # stop runs until resumed
pixel-agents curator resume
pixel-agents curator pin <skill>    # never auto-transition this skill
pixel-agents curator unpin <skill>
pixel-agents curator adopt <skill>    # hand an unmanaged skill to the curator
pixel-agents curator adopt --all-unmanaged   # hand over every unmanaged skill
pixel-agents curator list-unmanaged   # itemize skills with no provenance marker
pixel-agents curator restore <skill>  # move an archived skill back to active
pixel-agents curator list-archived    # list skills currently in ~/.pixel-agents/skills/.archive/
pixel-agents curator archive <skill>  # manually archive a single skill now
pixel-agents curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
```

## Backups and rollback

Before every real curator pass, Pixel Agents takes a tar.gz snapshot of `~/.pixel-agents/skills/` at `~/.pixel-agents/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
pixel-agents curator rollback        # restore newest snapshot (with confirmation)
pixel-agents curator rollback -y     # skip the prompt
pixel-agents curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, Pixel Agents takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `pixel-agents curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `pixel-agents curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`pixel-agents curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
`~/.pixel-agents/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.pixel-agents/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.pixel-agents/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed Pixel Agents at an external skill
...
**To see which skills the curator actually manages**, run `pixel-agents curator status`.
...
`pixel-agents curator status` reports an **unmanaged** count alongside the managed
one:
...
unmanaged (no provenance marker): 112 total
  ...
  never auto-staled or archived — `pixel-agents curator adopt <name>` hands one over
```
...
```bash
pixel-agents curator list-unmanaged                    # itemize them, with reasons
pixel-agents curator adopt <name> [<name> ...]         # hand specific skills over
pixel-agents curator adopt --all-unmanaged --dry-run   # preview the full list
pixel-agents curator adopt --all-unmanaged             # hand over everything (prompts)
pixel-agents curator adopt --all-unmanaged --yes       # skip the prompt
```
...
`.usage.json`; read it as policy. `pixel-agents curator adopt` changes the policy, and
...
**wrote** it — Pixel Agents edits user-authored skills on your behalf constantly. An
...
- Archives are recoverable via `pixel-agents curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `pixel-agents curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `pixel-agents curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
pixel-agents curator pin <skill>
pixel-agents curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.pixel-agents/skills/.usage.json`, so it survives across sessions.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.pixel-agents/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.pixel-agents/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.pixel-agents/logs/curator/`:

```
~/.pixel-agents/logs/curator/
└── 20260429-111512/
...
```bash
pixel-agents curator restore <skill-name>
```

This moves the skill back from `~/.pixel-agents/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.pixel-agents/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `pixel-agents curator pause` — the pause persists across sessions; use `resume` to re-enable.
...
- [Issue #7816](https://github.com/PixelResearch/pixel-agents/issues/7816) — original proposal and design discussion
