...
description: "Customize Pixel Agents's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Pixel Agents's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `PIXEL_AGENTS_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Pixel Agents is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Pixel Agents now seeds a default `SOUL.md` automatically in:

```text
~/.pixel-agents/SOUL.md
```

More precisely, it uses the current instance's `PIXEL_AGENTS_HOME`, so if you run Pixel Agents with a custom home directory, it will use:

```text
$PIXEL_AGENTS_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Pixel Agents creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Pixel Agents loads `SOUL.md` only from `PIXEL_AGENTS_HOME`
- Pixel Agents does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Pixel Agents falls back to a built-in default identity
...
If Pixel Agents loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `PIXEL_AGENTS_HOME`, the personality belongs to the Pixel Agents instance itself.

That also makes it easier to teach users:
- "Edit `~/.pixel-agents/SOUL.md` to change Pixel Agents' default personality."
...
- how Pixel Agents should handle uncertainty, disagreement, or ambiguity
...
## What Pixel Agents injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Pixel Agents falls back to a built-in default identity ("You are Pixel Agents, an intelligent AI assistant created by Pixel Agents..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Pixel Agents ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Pixel Agents, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Pixel Agents its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.pixel-agents/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.pixel-agents/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-pixel-agents)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Pixel Agents speaks
- `display.skin` and `/skin` affect how Pixel Agents looks in the terminal
