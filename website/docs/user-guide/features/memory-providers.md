...
Pixel Agents ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
pixel-agents memory setup      # interactive picker + configuration
pixel-agents memory status     # check what's active
pixel-agents memory off        # disable external provider
```

You can also select the active memory provider via `pixel-agents plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.pixel-agents/config.yaml`:
...
When a memory provider is active, Pixel Agents automatically:
...
**Setup Wizard:**
```bash
pixel-agents memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `pixel-agents honcho setup` command still works (it now redirects to `pixel-agents memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$PIXEL_AGENTS_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$PIXEL_AGENTS_HOME/honcho.json` > `~/.pixel-agents/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/PixelResearch/pixel-agents/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/pixel-agents).
...
:::tip Migrating from `pixel-agents honcho`
If you previously used `pixel-agents honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Pixel Agents profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Pixel Agents profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Pixel Agents profile. Host key `pixel-agents` → default; `pixel-agents.<profile>` for others. |
...
`--clone` creates a `pixel-agents.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
pixel-agents honcho sync
```

Scans every Pixel Agents profile, creates host blocks for any profile without one, inherits settings from the default `pixel-agents` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `pixel-agents memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/PixelResearch/pixel-agents/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/pixel-agents).
...
`pixel-agents memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.pixel-agents/.env`, and for named profiles use
...
`OPENVIKING_AGENT` is Pixel Agents' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
pixel-agents memory setup    # select "mem0" → "Platform"
# Or manually:
pixel-agents config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.pixel-agents/.env
```

**Setup (OSS):**
```bash
pixel-agents memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
pixel-agents memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
pixel-agents memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
pixel-agents memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
pixel-agents memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.pixel-agents/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.pixel-agents/.env
```
...
**Config:** `$PIXEL_AGENTS_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.pixel-agents/.env`.
...
| `user_id` | `pixel-agents-user` | User identifier |
| `agent_id` | `pixel-agents` | Agent identifier |
...
**Switching modes:** Re-run `pixel-agents memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
pixel-agents memory setup    # select "hindsight"
# Or manually:
pixel-agents config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.pixel-agents/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p pixel-agents ui start`

**Config:** `$PIXEL_AGENTS_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `pixel-agents` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchropixelly on the server |
| `retain_context` | `conversation between Pixel Agents and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/PixelResearch/pixel-agents/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
pixel-agents memory setup    # select "holographic"
# Or manually:
pixel-agents config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.pixel-agents-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$PIXEL_AGENTS_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
pixel-agents memory setup    # select "retaindb"
# Or manually:
pixel-agents config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.pixel-agents/.env
```
...
# Then configure Pixel Agents
pixel-agents memory setup    # select "byterover"
# Or manually:
pixel-agents config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$PIXEL_AGENTS_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=pixel-agents), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
pixel-agents memory setup    # select "supermemory"
# Or manually:
pixel-agents config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.pixel-agents/.env
```
...
Before running `pixel-agents memory setup`, set `base_url` in
`$PIXEL_AGENTS_HOME/supermemory.json`:
...
Then run `pixel-agents memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$PIXEL_AGENTS_HOME/supermemory.json`
...
| `container_tag` | `pixel-agents` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `pixel-agents-{identity}` → `pixel-agents-coder`) to isolate memories per Pixel Agents profile
...
| **Requires** | `pip install pixel-agents-memori` + `pixel-agents-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install pixel-agents-memori
pixel-agents-memori install
pixel-agents config set memory.provider memori
pixel-agents memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `pixel-agents-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$PIXEL_AGENTS_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$PIXEL_AGENTS_HOME/` so each profile has its own credentials
