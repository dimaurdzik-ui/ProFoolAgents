...
When Pixel Agents runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.pixel-agents/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.pixel-agents/config.yaml` (switchable per session
via `/personality`).
...
If you use Pixel Agents' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.pixel-agents/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Pixel Agents's deliverable
