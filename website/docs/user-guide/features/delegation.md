...
Top-level model calls run in the background automatically. Pixel Agents returns a handle immediately so the conversation can continue, then posts the result back as a new message. An orchestrator subagent waits for its own workers so it can synthesize their results before returning.
...
When a top-level agent provides a `tasks` array, Pixel Agents returns one background handle, runs the subagents in parallel, and posts one consolidated result after every child finishes. An orchestrator subagent waits for its batch in the current turn so it can synthesize the results.
...
- **Cancellation:** Follow-up messages do not cancel a top-level background batch. `/stop` or closing/resetting the owning session cancels its active children. Synchropixel orchestrator children still follow their parent's interrupt state

Synchropixel single-task delegation from an orchestrator runs directly without thread pool overhead.

### Durable background completions

When a background delegation finishes, Pixel Agents stores its completion event in
the active profile's `state.db` before publishing it to the normal fresh-turn
queue. If Pixel Agents restarts after completion but before delivery, the pending
...
Pixel Agents cannot prove whether its external side effects happened. Pending and
...
```
<pixel_home>/cache/delegation/live/<delegation_id>/task-<n>.log
```
...
```bash
tail -f ~/.pixel-agents/cache/delegation/live/deleg_ab12cd34/task-0.log
```
...
Top-level model-facing `delegate_task` calls run in the background automatically where the session supports later delivery. Pixel Agents returns a handle immediately, and the result re-enters the conversation after the child or batch finishes. Orchestrator subagents wait for their workers in the current turn because they must synthesize those results before returning. Stateless request/response endpoints fall back to synchropixel execution when they cannot deliver a detached result later.
...
- A Pixel Agents process restart does **not** resume a running child. Its attempt becomes `unknown` because Pixel Agents cannot prove which side effects happened.
...
- **Cancellation follows ownership** — `/stop` or closing/resetting the owning session cancels its background children; synchropixel descendants under orchestrators follow their parent's interrupt state
