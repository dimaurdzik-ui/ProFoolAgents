...
description: "Plugins shipped with Pixel Agents that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Pixel Agents ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.pixel-agents/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Pixel Agents Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.pixel-agents/plugins/<name>/`
3. **Project** — `./.pixel-agents/plugins/<name>/` (requires `PIXEL_AGENTS_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `pixel_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `pixel-agents memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `pixel-agents plugins list` and the interactive `pixel-agents plugins` UI), but none load until you explicitly enable them:

```bash
pixel-agents plugins enable disk-cleanup
```

Or via `~/.pixel-agents/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Pixel Agents. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
pixel-agents plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `pixel-agents plugins enable <name>`.
...
| `pixel-agents-achievements` | dashboard tab | Steam-style collectible badges generated from your real Pixel Agents session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `pixel-agents memory` and `pixel-agents plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `PIXEL_AGENTS_HOME` or `/tmp/pixel-agents-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under PIXEL_AGENTS_HOME | always | Never |
...
**State** — everything lives at `$PIXEL_AGENTS_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `PIXEL_AGENTS_HOME` or `/tmp/pixel-agents-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `pixel-agents plugins enable disk-cleanup` (or check the box in `pixel-agents plugins`).

**Disabling again:** `pixel-agents plugins disable disk-cleanup`.
...
**Enabling:** `pixel-agents plugins enable security-guidance` (or check the box in `pixel-agents plugins`).

**Disabling again:** `pixel-agents plugins disable security-guidance`.
...
Traces Pixel Agents turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Pixel Agents' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `pixel-agents logs`.
...
```bash
pixel-agents tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Pixel Agents and the next turn ships a trace.
...
Then put the credentials in `~/.pixel-agents/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Pixel Agents turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `PIXEL_AGENTS_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Pixel Agents session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `pixel-agents chat` session lives under one Langfuse session.

**Verify:**

```bash
pixel-agents plugins list                 # observability/langfuse should show "enabled"
pixel-agents chat -q "hello"              # check the Langfuse UI for a "Pixel Agents turn" trace
```
...
| `PIXEL_AGENTS_LANGFUSE_RELEASE` | — | Release/version tag |
| `PIXEL_AGENTS_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `PIXEL_AGENTS_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `PIXEL_AGENTS_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Pixel Agents-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Pixel Agents-prefixed wins when both are set.
...
**Disabling:** `pixel-agents plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, status) saved under `~/.pixel-agents/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
pixel-agents plugins enable google_meet
pixel-agents meet setup   # preflight: playwright, chromium, auth file
pixel-agents meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `pixel-agents plugins disable google_meet`. Any saved transcripts stay in `~/.pixel-agents/workspace/meetings/` until you remove them.

### pixel-agents-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Pixel Agents session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Pixel Agents feature changes.

**How it works:**

- Scans your entire `~/.pixel-agents/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$PIXEL_AGENTS_HOME/plugins/pixel-agents-achievements/state.json`
...
| Secret | Hidden until Pixel Agents detects the first related signal in your history |

**API** — routes mount under `/api/plugins/pixel-agents-achievements/`:
...
| `POST /rescan` | Manual synchropixel rescan (blocks; use when the user clicks the rescan button) |
| `POST /reset-state` | Clear unlock history and cached snapshot |

**State files** — live under `$PIXEL_AGENTS_HOME/plugins/pixel-agents-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Pixel Agents updates. |
...
**Enabling:** Nothing to enable — `pixel-agents-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `pixel-agents dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/pixel-agents-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.pixel-agents/plugins/pixel-agents-achievements/` that ships no dashboard. The plugin's state files under `$PIXEL_AGENTS_HOME/plugins/pixel-agents-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Pixel Agents plugin — see [Build a Pixel Agents Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.pixel-agents/plugins/<name>/`
- Manifest source is reported as `bundled` in `pixel-agents plugins list`
