...
title: "Plugins"
description: "Extend Pixel Agents with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Pixel Agents has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Pixel Agents
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Pixel Agents Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.pixel-agents/plugins/` with a `plugin.yaml` and Python code:
...
Start Pixel Agents — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.pixel-agents/plugins/hello-world/plugin.yaml`**
...
**`~/.pixel-agents/plugins/hello-world/__init__.py`**

```python
"""Minimal Pixel Agents plugin — registers a tool and a hook."""
...
Drop both files into `~/.pixel-agents/plugins/hello-world/`, restart Pixel Agents, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.pixel-agents/plugins/` are disabled by default. Enable them only for trusted repositories by setting `PIXEL_AGENTS_ENABLE_PROJECT_PLUGINS=true` before starting Pixel Agents.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `pixel-agents <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `pixel-agents plugins install` |
| Distribute via pip | `[project.entry-points."pixel_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Pixel Agents — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.pixel-agents/plugins/` | Personal plugins |
| Project | `.pixel-agents/plugins/` | Project-specific plugins (requires `PIXEL_AGENTS_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `pixel_agent.plugins` entry_points | Distributed packages |
| Nix | `services.pixel-agents.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Pixel Agents also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.pixel-agents/plugins/model-providers/<name>/` and `~/.pixel-agents/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `pixel-agents plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.pixel-agents/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
pixel-agents plugins                    # interactive toggle (space to check/uncheck)
pixel-agents plugins enable <name>      # add to allow-list
pixel-agents plugins disable <name>     # remove from allow-list + add to disabled
```

After `pixel-agents plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Pixel Agents' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.pixel-agents/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.pixel-agents/plugins/`.

### Migration for existing users

When you upgrade to a version of Pixel Agents that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.pixel-agents/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
Pixel Agents has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.pixel-agents/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Pixel Agents also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Pixel Agents Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Pixel Agents Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Pixel Agents Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `pixel-agents <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `PIXEL_AGENTS_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Pixel Agents auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `pixel-agents skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.pixel-agents/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `pixel-agents plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
pixel-agents plugins                               # unified interactive UI
pixel-agents plugins list                          # table: enabled / disabled / not enabled
pixel-agents plugins install user/repo             # install from Git, then prompt Enable? [y/N]
pixel-agents plugins install user/repo --enable    # install AND enable (no prompt)
pixel-agents plugins install user/repo --no-enable # install but leave disabled (no prompt)
pixel-agents plugins update my-plugin              # pull latest
pixel-agents plugins remove my-plugin              # uninstall
pixel-agents plugins enable my-plugin              # add to allow-list
pixel-agents plugins disable my-plugin             # remove from allow-list + add to disabled
```

### Interactive UI

Running `pixel-agents plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `pixel-agents plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
