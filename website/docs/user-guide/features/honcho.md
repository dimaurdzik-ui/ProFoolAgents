...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Pixel Agents's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Pixel Agents instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
pixel-agents memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `pixel-agents` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Pixel Agents syncs them back at session init.
...
The `pixel-agents honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `pixel-agents memory setup honcho` (or run `pixel-agents memory setup` and pick it from the list); the `pixel-agents honcho` subcommand then appears on the next invocation.

```bash
pixel-agents memory setup honcho    # Configure Honcho directly (works before activation)
pixel-agents honcho status          # Connection status, config, and key settings
pixel-agents honcho setup           # Redirects to `pixel-agents memory setup` (post-activation alias)
pixel-agents honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
pixel-agents honcho peer            # Show or update peer names + dialectic reasoning level
pixel-agents honcho mode            # Show or set recall mode (hybrid/context/tools)
pixel-agents honcho tokens          # Show or set token budget for context and dialectic
pixel-agents honcho identity        # Seed or show the AI peer's Honcho identity
pixel-agents honcho sync            # Sync Honcho config to all existing profiles
pixel-agents honcho peers           # Show peer identities across all profiles
pixel-agents honcho sessions        # List known Honcho session mappings
pixel-agents honcho map             # Map current directory to a Honcho session name
pixel-agents honcho enable          # Enable Honcho for the active profile
pixel-agents honcho disable         # Disable Honcho for the active profile
pixel-agents honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `pixel-agents honcho`

If you previously used the standalone `pixel-agents honcho setup`:
...
No re-login or re-setup needed. Run `pixel-agents memory setup` and select "honcho" — the wizard detects your existing config.
