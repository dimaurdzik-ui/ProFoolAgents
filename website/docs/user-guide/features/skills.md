...
All skills live in **`~/.pixel-agents/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Pixel Agents at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `pixel-agents update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.pixel-agents` profile):

```bash
curl -fsSL https://api.pixelagents.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
pixel-agents profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
pixel-agents skills opt-out            # stop future seeding — nothing on disk is touched
pixel-agents skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
pixel-agents skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `pixel-agents update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `pixel-agents skills opt-in`) to re-enable.

:::note Safe by default
`pixel-agents skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Pixel Agents installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Pixel Agents to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.pixel-agents/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
pixel-agents chat --toolsets skills -q "What skills do you have?"
pixel-agents chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Pixel Agents-tool framing, no invented
commands).
...
/home/user/.pixel-agents/cache/chart-q4-2025.png
...
When a missing value is encountered, Pixel Agents asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `pixel-agents setup` or `~/.pixel-agents/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `pixel-agents config migrate` prompts for unconfigured settings, and `pixel-agents config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.pixel-agents/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Pixel Agents scans the
...
If you maintain skills outside of Pixel Agents — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Pixel Agents to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.pixel-agents/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.pixel-agents/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Pixel Agents process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Pixel Agents ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.pixel-agents/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.pixel-agents/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `pixel-agents bundles list`.
...
pixel-agents bundles create research

# Overwrite an existing bundle
pixel-agents bundles create backend-dev --skill ... --force

# Delete a bundle
pixel-agents bundles delete backend-dev

# Re-scan ~/.pixel-agents/skill-bundles/ and report changes
pixel-agents bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.pixel-agents/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.pixel-agents/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.pixel-agents/pending/skills/` and are
...
```bash
pixel-agents skills browse                              # Browse all hub skills (official first)
pixel-agents skills browse --source official            # Browse only official optional skills
pixel-agents skills search kubernetes                   # Search all sources
pixel-agents skills search react --source skills-sh     # Search the skills.sh directory
pixel-agents skills search https://mintlify.com/docs --source well-known
pixel-agents skills inspect openai/skills/k8s           # Preview before installing
pixel-agents skills install openai/skills/k8s           # Install with security scan
pixel-agents skills install official/security/1password
pixel-agents skills install skills-sh/vercel-labs/json-render/json-render-react --force
pixel-agents skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
pixel-agents skills list --source hub                   # List hub-installed skills
pixel-agents skills check                               # Check installed hub skills for upstream updates
pixel-agents skills update                              # Reinstall hub skills with upstream changes when needed
pixel-agents skills audit                               # Re-scan all hub skills for security
pixel-agents skills uninstall k8s                       # Remove a hub skill
pixel-agents skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
pixel-agents skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
pixel-agents skills publish skills/my-skill --to github --repo owner/repo
pixel-agents skills snapshot export setup.json          # Export skill config
pixel-agents skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Pixel Agents. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `pixel-agents skills search <query> --source skills-sh`. Pixel Agents resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Pixel Agents currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Pixel Agents repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Pixel Agents can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
pixel-agents skills search https://mintlify.com/docs --source well-known
pixel-agents skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
pixel-agents skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Pixel Agents can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
pixel-agents skills install openai/skills/k8s
pixel-agents skills tap add myorg/skills-repo
```
...
[Skills Hub](https://api.pixelagents.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Pixel Agents-side changes required.
...
- Pixel Agents source id: `clawhub`

#### 6. LobeHub (`lobehub`)

Pixel Agents can search and convert agent entries from LobeHub's public catalog into installable Pixel Agents skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Pixel Agents source id: `lobehub`

#### 7. browse.sh (`browse-sh`)

Pixel Agents integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Pixel Agents' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Pixel Agents source id: `browse-sh`
- Trust level: `community`

```bash
pixel-agents skills search airbnb --source browse-sh
pixel-agents skills inspect browse-sh/airbnb.com/search-listings-ddgioa
pixel-agents skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Pixel Agents also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Pixel Agents source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Pixel Agents does not enumerate or copy unrelated files from the host.

```bash
pixel-agents skills install https://sharethis.chat/SKILL.md
pixel-agents skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
pixel-agents skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `pixel-agents skills update` re-fetches from the same URL automatically when you want to refresh.
...
`pixel-agents skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Pixel Agents | Always trusted |
...
```bash
pixel-agents skills check          # Report which installed hub skills changed upstream
pixel-agents skills update         # Reinstall only the skills with updates available
pixel-agents skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Pixel Agents users add with `pixel-agents skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.pixel-agents.tags`, `version`, `author`, `platforms`, `metadata.pixel-agents.config`).
...
Pixel Agents discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/pixel-agents-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any Pixel Agents user can subscribe and install:

```bash
pixel-agents skills tap add my-org/pixel-agents-skills
pixel-agents skills search deploy
pixel-agents skills install my-org/pixel-agents-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.pixel-agents/skills/.hub/taps.json`:
...
The `pixel-agents skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `pixel-agents skills tap list` shows the effective path per tap.
...
```bash
pixel-agents skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.pixel-agents/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`pixel-agents skills reset`)

Pixel Agents ships with a set of bundled skills in `skills/` inside the repo. On install and on every `pixel-agents update`, a sync pass copies those into `~/.pixel-agents/skills/` and records a manifest at `~/.pixel-agents/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Pixel Agents recomputes the hash of your local copy and compares it to the origin hash:
...
pixel-agents skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `PIXEL_AGENTS_HOME`, so `pixel-agents -p coder skills reset <name>` only affects that profile.
:::
