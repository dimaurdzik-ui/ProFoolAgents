...
Pixel Agents runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Pixel Agents
...
1. Pixel Agents captures a baseline of current diagnostics for the file.
...
…). Pixel Agents auto-detects the binary on PATH or in
`<PIXEL_AGENTS_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Pixel Agents at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<PIXEL_AGENTS_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`pixel-agents lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Pixel Agents installs both packages together when you
run `pixel-agents lsp install typescript` or auto-install fires on first
use.

## CLI

```
pixel-agents lsp status          # service state + per-server install status
pixel-agents lsp list            # registry, optionally --installed-only
pixel-agents lsp install <id>    # eagerly install one server
pixel-agents lsp install-all     # try every server with a known recipe
pixel-agents lsp restart         # tear down running clients
pixel-agents lsp which <id>      # print resolved binary path
```

`pixel-agents lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <PIXEL_AGENTS_HOME>/lsp/bin
...
When `install_strategy: auto`, Pixel Agents installs binaries into
`<PIXEL_AGENTS_HOME>/lsp/bin/`. NPM packages land in
`<PIXEL_AGENTS_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Pixel Agents-owned and is
...
**`pixel-agents lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<PIXEL_AGENTS_HOME>/lsp/bin/`. Run
`pixel-agents lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `pixel-agents lsp status`**
...
When `pixel-agents lsp status` shows a `Backend warnings` section, install
...
`~/.pixel-agents/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.pixel-agents/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `pixel-agents lsp restart` to clear the set;
