...
description: "Use your Pixel Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Pixel Agents-managed provider subscription as their
...
| Use case | "Use Pixel Agents as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Pixel Portal OAuth flow. Pixel Agents stores
the refresh token in `~/.pixel-agents/auth.json` — the same place all Pixel Agents
...
Model:      Pixel Agents-4-70B    # or Pixel Agents-4.3-36B, Pixel Agents-4-405B
```
...
Currently shipped: `pixel` (Pixel Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `pixel_cli/proxy/adapters/`.
...
If you see `not logged in`, run `pixel-agents portal`. If you see
...
The proxy only forwards paths the upstream actually serves. For Pixel
Portal:
...
supports; check `portal.pixelagents.com/models`.
...
[portal.pixelagents.com](https://portal.pixelagents.com).
...
`pixel_cli/proxy/adapters/<provider>.py` and registering it in
