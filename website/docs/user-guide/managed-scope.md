...
`~/.pixel-agents/config.yaml`, `~/.pixel-agents/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/pixel-agents`:

```text
/etc/pixel-agents/
├── config.yaml     # managed config layer (wins over ~/.pixel-agents/config.yaml)
└── .env            # managed env layer (wins over ~/.pixel-agents/.env + shell)
```
...
The location can be relocated with the `PIXEL_AGENTS_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `PIXEL_AGENTS_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Pixel Agents.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export PIXEL_AGENTS_MANAGED_DIR=/opt/org/pixel-agents-policy
```

:::warning
A user who can set `PIXEL_AGENTS_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `pixel-agents doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/pixel-agents/config.yaml` (managed) | `/etc/pixel-agents/.env` (managed) |
| 2 | `~/.pixel-agents/config.yaml` (user) | `~/.pixel-agents/.env` (user) |
...
```bash
pixel-agents config        # shows a header naming the managed source + the pinned keys
pixel-agents doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Pixel Agents refuses and names the source:

```bash
$ pixel-agents config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/pixel-agents/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `pixel-agents config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/pixel-agents

# Pin some config values for every user on this machine
sudo tee /etc/pixel-agents/config.yaml >/dev/null <<'YAML'
model:
  provider: pixel
security:
  redact_secrets: true
...
sudo tee /etc/pixel-agents/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/pixel-agents
sudo chmod 0644 /etc/pixel-agents/config.yaml /etc/pixel-agents/.env
```

Changes take effect on the next Pixel Agents start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`pixel-agents doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Pixel Agents as `root`), managed scope is advisory.
