...
Pixel Agents automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.pixel-agents/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Pixel Agents stores session history so it can resume conversations, but it does not
...
Pixel Agents explicitly injects for that turn.
...
For example, if a user sends an image and asks Pixel Agents to make a meme from it,
Pixel Agents may inspect that image once with vision and run an image-processing
...
`pixel-agents sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`pixel-agents` or `pixel-agents chat`) |
...
pixel-agents -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
pixel-agents -c "my project"   # → resumes "my project #3"
```
...
pixel-agents --resume 20250305_091523_a1b2c3d4
pixel-agents -r 20250305_091523_a1b2c3d4

# Resume by title
pixel-agents --resume "refactoring auth"

# Or with the chat subcommand
pixel-agents chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `pixel-agents sessions list`.
...
```bash
pixel-agents --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`pixel-agents sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
pixel-agents sessions list --workspace my-project
pixel-agents sessions list --workspace ~/code/pixel-agents
```

### Conversation Recap on Resume

When you resume a session, Pixel Agents displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Pixel Agents session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.pixel-agents/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `pixel-agents -r "<title>"` from the shell) and pick up where the platform left off.
...
Pixel Agents automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `pixel-agents sessions list` or `pixel-agents sessions browse`.
...
```bash
pixel-agents sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Pixel Agents creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`pixel-agents -c "my project"`), it automatically picks the most recent session in the lineage.
...
Pixel Agents provides a full set of session management commands via `pixel-agents sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
pixel-agents sessions list

# Filter by platform
pixel-agents sessions list --source telegram

# Show more sessions
pixel-agents sessions list --limit 50
```
...
`pixel-agents sessions export` is one surface for every export format, selected with `--format`:
...
pixel-agents sessions export backup.jsonl

# Export sessions from a specific platform
pixel-agents sessions export telegram-history.jsonl --source telegram

# Export a single session
pixel-agents sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
pixel-agents sessions export backup.jsonl --redact
```
...
pixel-agents sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
pixel-agents sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
pixel-agents sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
pixel-agents sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `pixel-agents-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
pixel-agents sessions export --format trace

# One session to a local trace file
pixel-agents sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
pixel-agents sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.pixel-agents/session-exports`).

```bash
# Export one session to Markdown
pixel-agents sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
pixel-agents sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
pixel-agents sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
pixel-agents sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
pixel-agents sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
pixel-agents sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
pixel-agents sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
pixel-agents sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
pixel-agents sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
pixel-agents sessions prune

# Custom age threshold — bare numbers are days
pixel-agents sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
pixel-agents sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
pixel-agents sessions prune --newer-than 5h

# Explicit window with absolute timestamps
pixel-agents sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
pixel-agents sessions prune --source telegram
pixel-agents sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
pixel-agents sessions prune --newer-than 5h --title "smoke test"   # title substring
pixel-agents sessions prune --older-than 30 --max-messages 3        # tiny sessions
pixel-agents sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
pixel-agents sessions prune --model gpt-5 --older-than 1w           # by model (substring)
pixel-agents sessions prune --provider openrouter --older-than 60   # by billing provider
pixel-agents sessions prune --branch feature/old-experiment         # by git branch
pixel-agents sessions prune --user 12345678 --chat-type group       # by messaging origin
pixel-agents sessions prune --max-tokens 500 --older-than 7         # by token usage
pixel-agents sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
pixel-agents sessions prune --newer-than 5h --dry-run

# Skip confirmation
pixel-agents sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `pixel-agents sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `pixel-agents sessions prune` keeps the 90-day cutoff. Every
...
`pixel-agents sessions archive` takes the same filters as `prune` but soft-hides
...
pixel-agents sessions archive --newer-than 5h

# Archive by title substring, preview first
pixel-agents sessions archive --title "dry run" --dry-run
pixel-agents sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `pixel-agents sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`pixel-agents sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
pixel-agents sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`pixel-agents insights`](/reference/cli-commands#pixel-agents-insights).
...
When Pixel Agents cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Pixel Agents uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Pixel Agents in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.pixel-agents/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.pixel-agents/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.pixel-agents/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.pixel-agents/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.pixel-agents/sessions/sessions.json` is a **legacy mirror** of it,
...
`pixel-agents sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.pixel-agents/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `pixel-agents sessions list`, the cause is
`state.db` not receiving them — run `pixel-agents sessions repair` and watch for a
...
`*.jsonl` files in `~/.pixel-agents/sessions/`. They are no longer written or
read by Pixel Agents. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Pixel Agents process in the same `PIXEL_AGENTS_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.pixel-agents/config.yaml`:
...
pixel-agents sessions delete <session_id>
