...
sidebar_position: 3
title: "Desktop App"
description: "The native Pixel Agents desktop app — a polished experience for chatting with Pixel Agents, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Pixel Agents desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Pixel Agents core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `pixel-agents` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Pixel Agents has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`pixel-agents`) and **[TUI](./tui.md)** (`pixel-agents --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`pixel-agents dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Pixel Agents Desktop](../getting-started/installation.md).

If you already have Pixel Agents installed, simply run
...
- **The same conversation history** as every other Pixel Agents surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a Pixel Agents instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Pixel Agents Desktop to a remote backend](./features/web-dashboard.md#connecting-pixel-agents-desktop-to-a-remote-backend).

#### Repository discovery

Pixel Agents Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional Pixel Agents sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `pixel-agents desktop --cwd <path>` (or the `PIXEL_AGENTS_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask Pixel Agents to open PR**. You can also create and switch branches from here.
...
Talk to Pixel Agents and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `pixel-agents model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Pixel Agents management surface so you don't have to drop to a terminal:
...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#pixel-agents-cron).
- **Profiles** — switch between [Pixel Agents profiles](./profiles.md) (isolated config/skills/sessions).
- **Messaging** — set up gateway channels.
- **Agents** and **Command Center** — orchestration surfaces for multi-agent work.

### Keyboard & navigation

- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update Pixel Agents, and more.
...
The [manual update process](https://api.pixelagents.com/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Pixel Agents agent, your config, and your chats stay. (Same as `pixel-agents uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `pixel-agents uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `pixel-agents uninstall --full`.)
...
You can do the same from the terminal — `pixel-agents uninstall --gui` for the GUI alone, or `pixel-agents uninstall` / `pixel-agents uninstall --full` for the agent too.

:::note
Running `pixel-agents uninstall --gui` from a **source checkout** (a `pixel-agents desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `pixel-agents desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `pixel-agents desktop`

To launch via the CLI, simply run `pixel-agents desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `pixel-agents update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `PIXEL_AGENTS_DESKTOP_CWD`)           |
| `--pixel-agents-root PATH` | Override the Pixel Agents source root the app uses (sets `PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `pixel-agents` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Pixel Agents runtime into `PIXEL_AGENTS_HOME` (`~/.pixel-agents`, or `%LOCALAPPDATA%\pixel-agents` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS_ROOT`, then a completed managed install, then a probed `pixel-agents` on `PATH` (unless `--ignore-existing` / `PIXEL_AGENTS_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `pixel-agents serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `pixel-agents --tui`. The desktop app is **self-contained**: it runs its own `pixel-agents serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Pixel Agents backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

**Settings → Gateway → Connection mode** offers the alternatives to the local gateway:

- **Remote gateway** — enter the URL of a `pixel-agents serve` backend you run yourself and sign in. This is the mode the rest of this section walks through.
- **Pixel Agents Cloud** — sign in once to Pixel Agents Cloud and pick from the agents on your account; no URL to paste. The app discovers your agents (with an organization picker if your account spans several orgs), and connecting to one switches the session over automatically. The status bar shows the cloud connection while it's active.
...
:::info The remote backend is a running `pixel-agents serve` process
"Remote backend" means a **`pixel-agents serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `pixel-agents serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
- **OAuth (Pixel Portal) — preferred for anything reachable beyond your own machine.** Logins are verified against your Pixel account, so this is the option suitable for a VPS, a public host, or any remote backend. Register the dashboard with `pixel-agents dashboard register` (or the Portal [`/local-dashboards`](https://portal.pixelagents.com/local-dashboards) page) to provision its OAuth client, then sign in from the app with **Sign in with Pixel Agents**. A self-hosted OIDC provider works the same way if you run your own identity provider.
...
The rest of this section shows the username/password path because it's the quickest to stand up on a trusted network; for the OAuth path see [Web Dashboard → Default provider: Pixel Agents](./features/web-dashboard.md#default-provider-pixel-agents).

### On the backend (the remote machine)

Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.pixel-agents/.env` (the secrets file, mode 0600):
...
pixel-agents serve --host 0.0.0.0 --port 9119
```

Keep that `pixel-agents serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `pixel-agents serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `PIXEL_AGENTS_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.pixel-agents/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/pixel-agents` work if you front it with a reverse proxy)
2. **Sign in** — the app detects which provider the backend advertises and adapts the button. For a username/password backend it shows a **Sign in** button that opens a credential form (enter the credentials from step 1). For an OAuth backend it shows **Sign in with `<provider>`** (e.g. *Sign in with Pixel Agents*), which runs the provider's browser sign-in. Either way the app ends up with an authenticated session against the backend.
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `PIXEL_AGENTS_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `PIXEL_AGENTS_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateway settings panel.
...
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `PIXEL_AGENTS_DASHBOARD_BASIC_AUTH_USERNAME` / `PIXEL_AGENTS_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for an unknown user and a wrong password (no enumeration oracle), so double-check both. Confirm the gate is on with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.pixel-agents/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `PIXEL_AGENTS_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Pixel Agents Desktop to a remote backend](./features/web-dashboard.md#connecting-pixel-agents-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Pixel Agents Desktop](../reference/environment-variables.md#web-dashboard--pixel-agents-desktop).
...
`$PIXEL_AGENTS_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
Boot logs land in `PIXEL_AGENTS_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:

```bash
pixel-agents logs gui -f
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.pixel-agents/pixel-agents/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
certificate once and tell Pixel Agents to use it:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `Pixel Agents Local Signing`, Identity Type: *Self-Signed Root*,
   Certificate Type: **Code Signing**.
3. `pixel-agents config set desktop.macos_signing_identity "Pixel Agents Local Signing"`
...
`tccutil reset All com.pixelagents.pixel-agents` and re-grant.

## See also

- [CLI Guide](./cli.md) — the terminal interface
- [TUI](./tui.md) — the modern terminal UI used by `pixel-agents --tui` and the dashboard chat tab
