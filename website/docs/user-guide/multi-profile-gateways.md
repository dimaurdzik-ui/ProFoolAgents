...
If you only run one Pixel Agents agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Pixel Agents agents that should all
...
(`ai.pixel-agents.gateway-<name>.plist`) or systemd user service
(`pixel-agents-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
pixel-agents profile create coder
pixel-agents profile create personal-bot
pixel-agents profile create research
...
```bash
pixel-agents config set gateway.multiplex_profiles true
pixel-agents gateway restart
```

Equivalently, in the default profile's `~/.pixel-agents/config.yaml`:
...
You do **not** run `pixel-agents gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `pixel-agents gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.pixel-agents/profiles/coder/.env`; the default listener key is rejected.
...
home). `pixel-agents status` reports the multiplexer and the profiles it serves;
`pixel-agents status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/pixel-agents-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: pixel-agents-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    pixel-agents gateway "$action"
  else
    pixel-agents -p "$profile" gateway "$action"
  fi
}
...
```bash
pixel-agents-gateways start      # start every configured profile
pixel-agents-gateways stop       # stop every configured profile
pixel-agents-gateways restart    # restart all
pixel-agents-gateways status     # status across all
pixel-agents-gateways list       # delegates to `pixel-agents gateway list`
```

:::tip
The `default` profile is targeted with `pixel-agents gateway <action>` (no `-p`),
not `pixel-agents -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `pixel-agents -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.pixel-agents.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/pixel-agents-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.pixel-agents.gateway.plist` /
`pixel-agents-gateway.service`.
...
```bash
# Default profile
tail -f ~/.pixel-agents/logs/gateway.log
tail -f ~/.pixel-agents/logs/gateway.error.log

# Named profile
tail -f ~/.pixel-agents/profiles/<name>/logs/gateway.log
tail -f ~/.pixel-agents/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.pixel-agents/logs/gateway.log ~/.pixel-agents/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
pixel-agents logs -f                  # follow default profile
pixel-agents -p coder logs -f         # follow one profile
pixel-agents logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
pixel-agents profile list             # profiles + model + gateway state
pixel-agents-gateways status          # full status across every profile
launchctl list | grep pixel-agents    # macOS — PIDs and labels
systemctl --user list-units 'pixel-agents-gateway-*'   # Linux — units
```
...
```
~/.pixel-agents/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.pixel-agents/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
pixel-agents config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.pixel-agents/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=pixel-agents --why="gateways running" \
  sleep infinity &
...
`pixel-agents-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.pixel-agents/.env ~/.pixel-agents/profiles/*/.env
```

## Updating the code

`pixel-agents update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `pixel-agents gateway start` after a previous `pixel-agents gateway stop`. The
...
```bash
ps -ef | grep "pixel_cli.*-p <profile>"
cat ~/.pixel-agents/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.pixel-agents.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.pixel-agents.gateway-<profile>.plist

# Linux
systemctl --user restart pixel-agents-gateway-<profile>.service
```

### Health check

```bash
pixel-agents doctor                  # default profile
pixel-agents -p <profile> doctor     # one profile
```
