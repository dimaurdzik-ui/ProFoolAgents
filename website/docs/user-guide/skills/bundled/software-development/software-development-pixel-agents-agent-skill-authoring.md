---
title: "Pixel Agents Skill Authoring — Author in-repo SKILL.md files: frontmatter and structure"
sidebar_label: "Pixel Agents Skill Authoring"
description: "Author in-repo SKILL.md files: frontmatter and structure"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Pixel Agents Skill Authoring
...
| Path | `skills/software-development/pixel-agents-skill-authoring` |
| Version | `1.1.0` |
| Author | Pixel Agents |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `skills`, `authoring`, `pixel-agents`, `conventions`, `skill-md` |
...
# Authoring Pixel-Agents Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.pixel-agents/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
2. **In-repo (this skill is about this case):** `/home/bb/pixel-agents/skills/<category>/<name>/SKILL.md` — committed, shipped with the package. Use `write_file` + `git add`. `skill_manage(action='create')` does NOT target this tree.

## When to Use

- User asks you to add a skill "in this branch / repo / commit"
- You're committing a reusable workflow that should ship with pixel-agents
- You're editing an existing skill under `/home/bb/pixel-agents/skills/` (use `patch` for small edits, `write_file` for rewrites; `skill_manage` still works for patch on in-repo skills, but not for `create`)
...
2. **Choose the right context load.** A model-invoked Pixel Agents skill pays for its description every turn. Keep descriptions focused on trigger classes and the skill's distinctive behavior. Put details in the body or linked references.
...
- Pixel Agents-specific recipes (tests via scripts/run_tests.sh, ui-tui paths, etc.)
...
`metadata.pixel-agents.related_skills` unions both trees (`skills/` in-repo and `~/.pixel-agents/skills/`) at load time. You CAN reference a user-local skill from an in-repo skill, but it won't resolve for other users who clone the repo fresh. Prefer referencing only in-repo skills from in-repo skills. If a frequently-referenced skill lives only in `~/.pixel-agents/skills/`, consider promoting it to the repo.
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.pixel-agents/skills/`, not the repo tree. Use `write_file` for in-repo creation.
...
3. **Description too generic or trigger buried past char 57.** The system prompt
   ...
   - Good: `Use when debugging Pixel Agents skill discovery failures.`
   - Bad: `This skill contains detailed guidance for agents working on Pixel Agents skill discovery failures.`
...
- [ ] File is at `skills/<category>/<name>/SKILL.md` (not in `~/.pixel-agents/skills/`)
- [ ] Frontmatter starts at byte 0 with `---`, closes with `\n---\n`
- [ ] `name`, `description`, `version`, `author`, `license`, `metadata.pixel-agents.{tags, related_skills}` all present
