...
Everything operator-facing is a `pixel-agents teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env`:
...
```bash
pixel-agents teams-pipeline validate              # config snapshot — run first after any change
pixel-agents teams-pipeline token-health          # Graph token status
pixel-agents teams-pipeline token-health --force-refresh   # force a fresh token acquisition
pixel-agents teams-pipeline list                  # recent meeting jobs
pixel-agents teams-pipeline list --status failed  # only failed jobs
pixel-agents teams-pipeline show <job-id>         # full detail of one job
pixel-agents teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
pixel-agents teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
pixel-agents teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
pixel-agents teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
pixel-agents teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

pixel-agents teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
pixel-agents teams-pipeline delete-subscription <sub-id>
pixel-agents teams-pipeline maintain-subscriptions            # renew near-expiry ones
pixel-agents teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `pixel-agents teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `pixel-agents cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
