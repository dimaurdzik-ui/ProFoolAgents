...
4. Store the token in `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env` (or via `pixel-agents setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Pixel Agents. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Important Notes for Pixel Agents
...
- **`AIRTABLE_API_KEY` flows from `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
