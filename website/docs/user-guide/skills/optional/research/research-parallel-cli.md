...
| | |
|---|---|
| Source | Optional — install with `pixel-agents skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Pixel Agents core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Pixel Agents native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Pixel Agents native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Pixel Agents native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended Pixel Agents workflow:
...
## Recommended Pixel Agents usage patterns
...
- Do not silently choose Parallel when Pixel Agents native tools are already sufficient.
