...
| | |
|---|---|
| Source | Optional — install with `pixel-agents skills install official/productivity/telephony` |
...
This optional skill gives Pixel Agents practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env`
...
It does **not** turn Pixel Agents into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Pixel Agents memory
...
### 1) "I want Pixel Agents to own a real phone number"
Use **Twilio**.
...
- pairs well with Pixel Agents `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env`
...
### `~/.pixel-agents/telephony_state.json`
...
```bash
SCRIPT="$(find ~/.pixel-agents/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
pixel-agents skills search telephony
pixel-agents skills install official/productivity/telephony
```
...
Then save credentials into Pixel Agents:
...
3. Buy it and save it into `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Pixel Agents calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Pixel Agents's existing `text_to_speech` support.

Use this when:
- you want the call to use Pixel Agents's configured TTS voice rather than Twilio `<Say>`
...
Recommended Pixel Agents TTS -> Twilio Play workflow:

1. Generate the audio with Pixel Agents `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Pixel Agents to create the message audio with `text_to_speech`
...
Important note:
- Pixel Agents TTS is great for prerecorded outbound messages
...
- Pixel Agents STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Pixel Agents memory.
...
- Do not store arbitrary third-party phone numbers in Pixel Agents memory.
...
3. persist that number to `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env`
