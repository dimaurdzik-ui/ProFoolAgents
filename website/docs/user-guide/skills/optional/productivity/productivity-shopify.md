...
| | |
|---|---|
| Source | Optional — install with `pixel-agents skills install official/productivity/shopify` |
...
4. Save to `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Pixel Agents","productType":"Apparel","tags":["test"]}}'
```
