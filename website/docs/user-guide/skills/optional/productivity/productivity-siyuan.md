...
| | |
|---|---|
| Source | Optional — install with `pixel-agents skills install official/productivity/siyuan` |
...
3. Store it in `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env`:
   ```
...
```yaml
# In ~/.pixel-agents/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
