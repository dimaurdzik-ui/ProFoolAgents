...
| | |
|---|---|
| Source | Optional — install with `pixel-agents skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${PIXEL_AGENTS_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${PIXEL_AGENTS_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${PIXEL_AGENTS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client pixel-agents
```
...
```bash
PUBLISH="${PIXEL_AGENTS_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client pixel-agents
```
...
```bash
DRIVE="${PIXEL_AGENTS_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `pixel-agents`)    |
