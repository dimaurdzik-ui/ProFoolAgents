...
| | |
|---|---|
| Source | Optional — install with `pixel-agents skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Pixel Agents"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env`:
...
```bash
CANVAS="python $PIXEL_AGENTS_HOME/skills/productivity/canvas/scripts/canvas_api.py"
