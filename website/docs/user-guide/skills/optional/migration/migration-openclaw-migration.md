---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Pixel Agents"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Pixel Agents"
---
...
Import an OpenClaw setup (memories, skills) into Pixel Agents.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `pixel-agents skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Pixel Agents (Pixel Agents) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Pixel Agents`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Pixel Agents Migration

Use this skill when a user wants to move their OpenClaw setup into Pixel Agents with minimal manual cleanup.
...
```bash
pixel-agents claw migrate              # Full interactive migration
...
pixel-agents claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `pixel-agents setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_pixel.py` to:

- import `SOUL.md` into the Pixel Agents home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Pixel Agents memory entries
- merge OpenClaw command approval patterns into Pixel Agents `command_allowlist`
- migrate Pixel Agents-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Pixel Agents working-directory configuration
- copy OpenClaw skills into `~/.pixel-agents/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Pixel Agents workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.pixel-agents/tts/`
- archive non-secret docs that do not have a direct Pixel Agents destination
...
- `scripts/openclaw_to_pixel.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.pixel-agents/skills/migration/openclaw-migration/scripts/openclaw_to_pixel.py`

Do not guess a shorter path like `~/.pixel-agents/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.pixel-agents/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Pixel Agents-compatible secrets, currently:
...
Pixel Agents CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Pixel Agents. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Pixel Agents workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Pixel Agents skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.pixel-agents/skills/migration/openclaw-migration/scripts/openclaw_to_pixel.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.pixel-agents/skills/migration/openclaw-migration/scripts/openclaw_to_pixel.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.pixel-agents/skills/migration/openclaw-migration/scripts/openclaw_to_pixel.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.pixel-agents/skills/migration/openclaw-migration/scripts/openclaw_to_pixel.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.pixel-agents/skills/migration/openclaw-migration/scripts/openclaw_to_pixel.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.pixel-agents/skills/migration/openclaw-migration/scripts/openclaw_to_pixel.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Pixel Agents unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Pixel Agents targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Pixel Agents destination. Unsupported auth blobs must still be reported as skipped.
...
- Pixel Agents persona state imported
- Pixel Agents memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.pixel-agents/skills/openclaw-imports/`
