...
| | |
|---|---|
| Source | Optional — install with `pixel-agents skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Pixel Agents |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Pixel Agents terminal calls (desktop app flow only)
...
### Service Account (recommended for Pixel Agents)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env` (the skill will prompt for this on first load).
...
## Pixel Agents Execution Pattern (desktop app flow)

Pixel Agents terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/pixel-agents-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/pixel-agents-op.sock"
