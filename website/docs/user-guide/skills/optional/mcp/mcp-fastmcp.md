...
| | |
|---|---|
| Source | Optional — install with `pixel-agents skills install official/mcp/fastmcp` |
...
| Related skills | [`pixel-agents`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-pixel-agents), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Pixel Agents or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Pixel Agents. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.pixel-agents/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.pixel-agents/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Pixel Agents integration, either:

- configure the server in `~/.pixel-agents/config.yaml` using the `native-mcp` skill, or
...
### Pixel Agents cannot see the deployed server

The server-building part may be correct while the Pixel Agents config is not. Load the `native-mcp` skill and configure the server in `~/.pixel-agents/config.yaml`, then restart Pixel Agents.
