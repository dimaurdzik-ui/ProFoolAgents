...
| | |
|---|---|
| Source | Optional — install with `pixel-agents skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Pixel Agents MCP catalog. The
...
Two halves, in this order: the editor side must be up before Pixel Agents connects.
...
### One-time, Pixel Agents side

    pixel-agents mcp install unreal-engine
...
`~/.pixel-agents/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Pixel Agents — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Pixel Agents connects
...
2. Start the Pixel Agents session. Tools register as `mcp_unreal_engine_*`. If
   they're missing: editor wasn't up first — start it, then open a new
   Pixel Agents session.
...
three meta-tools, and every real tool is reached through them. Through Pixel Agents
they appear as:

| Pixel Agents tool | Purpose |
|---|---|
...
then happens at `pixel-agents mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Pixel Agents runs batched calls
...
- **Start order matters.** Editor + server up first, then the Pixel Agents
...
- **Timeouts on long operations.** Pixel Agents' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.pixel-agents/config.yaml` for
