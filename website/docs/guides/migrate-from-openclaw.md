...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Pixel Agents — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`pixel-agents claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Pixel Agents. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::note
Coming from **Claude Code** or **OpenAI Codex CLI** instead? Use [`pixel-agents import-agent`](../user-guide/import-from-other-agents.md).
:::

:::tip
If your OpenClaw setup was multi-provider, `pixel-agents setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Pixel Portal](/integrations/pixel-portal).
:::
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.pixel-agents/` (by default a single restore-point archive is written before apply, under `~/.pixel-agents/backups/pre-migration-*.zip`; restorable with `pixel-agents import`). |
...
| What | OpenClaw source | Pixel Agents destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.pixel-agents/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.pixel-agents/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.pixel-agents/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.pixel-agents/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Pixel Agents destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.pixel-agents/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.pixel-agents/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.pixel-agents/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.pixel-agents/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Pixel Agents skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Pixel Agents destination | Notes |
|------|---------------------|-------------------|-------|
| Default model | `agents.defaults.model` | `config.yaml` → `model` | Can be a string or `{primary, fallbacks}` object |
| Custom providers | `models.providers.*` | `config.yaml` → `custom_providers` (auto-migrated to the canonical `providers:` dict on the next `pixel-agents update` config migration) | Maps `baseUrl`, `apiType`/`api` — handles both short ("openai", "anthropic") and hyphenated ("openai-completions", "anthropic-messages", "google-generative-ai") values |
| Provider API keys | `models.providers.*.apiKey` | `~/.pixel-agents/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Pixel Agents config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Pixel Agents config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Pixel Agents field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.pixel-agents/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Pixel Agents `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Pixel Agents path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `PIXEL_AGENTS_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct Pixel Agents equivalent)

These are saved to `~/.pixel-agents/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Pixel Agents |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Pixel Agents has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `pixel-agents cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `pixel-agents webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `pixel-agents honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `pixel-agents skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Pixel Agents profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to Pixel Agents manually via `pixel-agents config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.pixel-agents/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `pixel-agents status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart pixel-agents-gateway`

6. **Check session policies** — run `pixel-agents config show` and verify the `session_reset` value matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `pixel-agents whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `pixel-agents claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `pixel-agents config set`.

### Skills not appearing after migration

Imported skills land in `~/.pixel-agents/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `pixel-agents config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
