...
```bash
pixel-agents teams-pipeline validate
```
...
```bash
pixel-agents teams-pipeline token-health
pixel-agents teams-pipeline token-health --force-refresh
```
...
```bash
pixel-agents teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
pixel-agents teams-pipeline maintain-subscriptions
pixel-agents teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Pixel Agents cron (recommended if you already run the Pixel Agents gateway)

Pixel Agents ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.pixel-agents/scripts/`. First create the script:

```bash
mkdir -p ~/.pixel-agents/scripts
cat > ~/.pixel-agents/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec pixel-agents teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.pixel-agents/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
pixel-agents cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/pixel-agents-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Pixel Agents Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=pixel-agents
EnvironmentFile=/etc/pixel-agents/env
ExecStart=/usr/local/bin/pixel-agents teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/pixel-agents-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Pixel Agents Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now pixel-agents-teams-pipeline-maintain.timer
systemctl list-timers pixel-agents-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/pixel-agents teams-pipeline maintain-subscriptions >> /var/log/pixel-agents/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.pixel-agents/.env` at the top of a wrapper script that crontab calls.
...
```bash
pixel-agents teams-pipeline subscriptions   # should show expirationDateTime advanced
pixel-agents teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
pixel-agents teams-pipeline list
pixel-agents teams-pipeline list --status failed
pixel-agents teams-pipeline show <job-id>
```

### Replay a stored job

```bash
pixel-agents teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
pixel-agents teams-pipeline fetch --meeting-id <meeting-id>
pixel-agents teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `pixel-agents teams-pipeline maintain-subscriptions --dry-run`
- inspect `pixel-agents teams-pipeline list --status failed`
...
- run `pixel-agents teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `pixel-agents teams-pipeline run`
...
- [ ] `pixel-agents teams-pipeline validate` returns an OK snapshot
- [ ] `pixel-agents teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Pixel Agents cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
