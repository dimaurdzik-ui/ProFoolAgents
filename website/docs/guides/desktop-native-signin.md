...
description: "How the Pixel Agents Desktop app signs in to a gated gateway using your system browser and PKCE — no embedded webview, no session cookies"
---

# Desktop Native Sign-In (RFC 8252)

When the Pixel Agents Desktop app connects to a **gated gateway** (a hosted or
...
For Pixel Agents specifically, native sign-in means:
...
```
Desktop app                Gateway (/auth/native/*)          Pixel Portal (IDP)
...
desktop app* and an OAuth client *to the upstream identity provider* (Pixel
...
brokerable OAuth provider registered (e.g. the bundled **Pixel** provider). No
...
- [Run Pixel Agents with Pixel Portal](./run-pixel-agents-with-pixel-portal.md)
