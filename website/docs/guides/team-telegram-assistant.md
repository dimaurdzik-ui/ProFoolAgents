...
This tutorial walks you through setting up a Telegram bot powered by Pixel Agents that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Pixel Agents installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.pixel-agents/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Pixel Agents itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Pixel Agents Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_pixel_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Pixel Agents. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.pixel-agents/.env`:
...
```
[Gateway] Starting Pixel Agents Gateway...
...
# View live logs
journalctl --user -u pixel-agents-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `pixel-agents gateway install` to update the plist.
:::
...
```bash
pixel-agents gateway stop && pixel-agents gateway start
```
...
pixel-agents pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.pixel-agents/.env`:
...
Control how much detail the bot shows when using tools. In `~/.pixel-agents/config.yaml`:
...
Customize how the bot communicates by editing `~/.pixel-agents/SOUL.md`:

For a full guide, see [Use SOUL.md with Pixel Agents](/guides/use-soul-with-pixel-agents).
...
```bash
# From the CLI
pixel-agents cron list          # View all scheduled jobs
pixel-agents cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u pixel-agents-gateway` (Linux) or `~/.pixel-agents/logs/gateway.log` (macOS) |
| Cron job output | `~/.pixel-agents/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.pixel-agents/cron/jobs.json` |
| Pairing data | `~/.pixel-agents/pairing/` |
| Session history | `~/.pixel-agents/sessions/` |
