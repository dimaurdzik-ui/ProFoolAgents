...
Copy-paste blueprints for common automation patterns. Each blueprint uses Pixel Agents's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`pixel-agents webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `pixel-agents webhook subscribe`) |
...
```bash
pixel-agents cron create "0 2 * * *" \
  "You are a project manager triaging the PixelResearch/pixel-agents GitHub repo.

1. Run: gh issue list --repo PixelResearch/pixel-agents --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
pixel-agents webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
pixel-agents cron create "0 9 * * 1" \
  "Scan the PixelResearch/pixel-agents repo for documentation drift.

1. Run: gh pr list --repo PixelResearch/pixel-agents --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (pixel_cli/commands.py, pixel_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (pixel_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
pixel-agents cron create "0 6 * * *" \
  "Run a dependency security audit on the pixel-agents project.

1. cd ~/.pixel-agents/pixel-agents && source .venv/bin/activate
...
```bash
pixel-agents webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
pixel-agents webhook subscribe alert-triage \
...
```python title="~/.pixel-agents/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Pixel Agents-Monitor/1.0"})
...
```bash
pixel-agents cron create "every 30m" \
  ...
  --script ~/.pixel-agents/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
pixel-agents cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Pixel Agents development." \
...
```bash
pixel-agents webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
pixel-agents webhook subscribe stripe-payments \
...
```bash
pixel-agents cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the pixel-agents codebase.
...
```bash
pixel-agents cron create "0 10 * * 3" \
