...
description: "Diagnose and fix common Pixel Agents cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`pixel-agents gateway` for foreground, or `pixel-agents gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `pixel-agents cron tick`.
...
```bash
date
pixel-agents cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.pixel-agents/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.pixel-agents/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.pixel-agents/.env` |
...
| `local` | Write access to `~/.pixel-agents/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `pixel-agents cron list` for updated `last_error` field (if available).
...
```bash
pixel-agents skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `pixel-agents skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `pixel-agents skills list`.
...
2. `~/.pixel-agents/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `pixel-agents cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Pixel Agents config directory). Verify:
```bash
ls ~/.pixel-agents/scripts/your-script.py   # Must exist
pixel-agents cron edit <job_id> --script ~/.pixel-agents/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `pixel-agents skills install <skill-name>`.
...
```bash
ps aux | grep pixel-agents
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.pixel-agents/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.pixel-agents/cron/jobs.json
chmod 600 ~/.pixel-agents/cron/jobs.json   # Your user should own it
```
...
```bash
pixel-agents cron list                    # Show all jobs, states, next_run times
pixel-agents cron run <job_id>            # Schedule for next tick (for testing)
pixel-agents cron edit <job_id>           # Fix configuration issues
pixel-agents logs                         # View recent Pixel Agents logs
pixel-agents skills list                  # Verify installed skills
```
...
1. Run the job with `pixel-agents cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.pixel-agents/logs/agent.log` for scheduler messages and `~/.pixel-agents/logs/errors.log` for warnings
3. Open an issue at [github.com/PixelResearch/pixel-agents](https://github.com/PixelResearch/pixel-agents) with:
