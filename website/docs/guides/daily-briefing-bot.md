...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `pixel-agents setup --portal`. See [Pixel Portal](/integrations/pixel-portal).
:::
...
2. **Pixel Agents spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.pixel-agents/cron/output/` and you can read them anytime.
:::
...
Pixel Agents will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Pixel Agents
```
...
Before creating cron jobs, ensure Pixel Agents has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Pixel Agents what you want:
...
Pixel Agents will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Pixel Agents to delegate each topic to a sub-agent:
...
Pixel Agents will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
