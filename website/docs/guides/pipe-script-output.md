...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `pixel-agents send`."
---

# Pipe Script Output to Messaging Platforms

`pixel-agents send` is a small, scriptable CLI that pushes a message to any
messaging platform Pixel Agents is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | pixel-agents send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `pixel-agents
...
pixel-agents send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
pixel-agents send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
pixel-agents send --list

# Filter by platform
pixel-agents send --list telegram
```
...
Any platform Pixel Agents ships adapters for works as a target:
...
`pixel-agents send` resolves the message body in this order:

1. **Positional argument** — `pixel-agents send --to telegram "hi"`
2. **`--file PATH`** — `pixel-agents send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | pixel-agents send --to telegram`

When stdin is a TTY (no pipe), Pixel Agents does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  pixel-agents send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `pixel-agents send` reuses your Pixel Agents config, the same script works on
any host where Pixel Agents is installed — no need to export bot tokens into
...
`pixel-agents send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  pixel-agents send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | pixel-agents send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.pixel-agents/bin/pixel-agents send \
...
```bash
./train.py --epochs 200 && \
  pixel-agents send --to telegram "training done" || \
  pixel-agents send --to telegram "training failed (exit $?)"
```
...
pixel-agents send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(pixel-agents send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `pixel-agents send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `pixel-agents send` calls
the platform's REST endpoint directly using credentials from
`~/.pixel-agents/.env` and `~/.pixel-agents/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `pixel-agents gateway start` and retry.
...
# Machine-readable
pixel-agents send --list --json
```

The listing is built from `~/.pixel-agents/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`pixel-agents gateway
...
| Approach | Multi-platform | Reuses Pixel Agents creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `pixel-agents send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`pixel-agents send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `pixel-agents send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `pixel-agents send` shares with cron delivery.
