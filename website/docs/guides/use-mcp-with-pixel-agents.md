---
sidebar_position: 6
title: "Use MCP with Pixel Agents"
description: "A practical guide to connecting MCP servers to Pixel Agents, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Pixel Agents

This guide shows how to actually use MCP with Pixel Agents in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Pixel Agents tool
- you want Pixel Agents to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Pixel Agents to internal APIs, databases, or company systems without modifying Pixel Agents core

Do not use MCP when:
- a built-in Pixel Agents tool already solves the job well
...
- Pixel Agents remains the agent
- MCP servers contribute tools
- Pixel Agents discovers those tools at startup or reload time
...
If you installed Pixel Agents with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Pixel Agents banner/status should show MCP integration when configured
- ask Pixel Agents what tools it has available
...
## WSL2: bridge Pixel Agents in WSL to Windows Chrome

This is the practical setup when:

- Pixel Agents runs inside WSL2
...
In this setup, Pixel Agents does **not** connect to Chrome directly. Instead:

- Pixel Agents runs in WSL
- Pixel Agents starts a local stdio MCP server
...
```text
Pixel Agents (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Pixel Agents stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Pixel Agents core browser transport
...
```bash
pixel-agents mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
pixel-agents mcp test chrome-devtools-win
```

Then start a fresh Pixel Agents session or run:
...
Once loaded, Pixel Agents can use the MCP-prefixed browser tools directly. For example:
...
If Pixel Agents runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Pixel Agents from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Pixel Agents from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Pixel Agents:
...
2. Pixel Agents-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Pixel Agents will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Pixel Agents to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Pixel Agents to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Pixel Agents remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
pixel-agents mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
pixel-agents mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `pixel-agents mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Pixel Agents to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Pixel Agents to merge, publish, deploy, or spawn runtimes.
...
Now Pixel Agents can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Pixel Agents core.
...
Because Pixel Agents now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
