...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Pixel Agents  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Pixel Agents instantly when PRs are opened or updated.
:::
...
- **Pixel Agents installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  pixel-agents gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.pixel-agents/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Pixel Agents can access GitHub. Start a chat:
...
```
Run: gh pr list --repo PixelResearch/pixel-agents --state open --limit 3
```
...
Still in the chat, ask Pixel Agents to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo PixelResearch/pixel-agents
```
...
A skill gives Pixel Agents consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.pixel-agents/skills/code-review
```

Create `~/.pixel-agents/skills/code-review/SKILL.md`:
...
Verify it loaded — start `pixel-agents` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Pixel Agents your team's standards:
...
```bash
pixel-agents cron run pr-review
```
...
2. Teach Pixel Agents your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
pixel-agents gateway status    # Is the gateway running?
pixel-agents cron list         # Is the job enabled?
```
