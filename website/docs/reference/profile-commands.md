...
This page covers all commands related to [Pixel Agents profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `pixel-agents profile` without a subcommand shows help.
...
## `pixel-agents profile use`

```bash
pixel-agents profile use <name>
```

Sets `<name>` as the active profile. All subsequent `pixel-agents` commands (without `-p`) will use this profile.
...
pixel-agents profile use default
```

## `pixel-agents profile create`

```bash
pixel-agents profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `pixel-agents profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `pixel-agents update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.pixel-agents`), use `pixel-agents skills opt-out` / `pixel-agents skills opt-in`. |
...
pixel-agents profile create work2 --clone-from work

# Clone everything from a specific profile
pixel-agents profile create work2-backup --clone-from work --clone-all
```

## `pixel-agents profile describe`

```bash
pixel-agents profile describe [<name>] [options]
```
...
pixel-agents profile describe researcher

# Set it explicitly
pixel-agents profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
pixel-agents profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
pixel-agents profile describe --all --auto
```

## `pixel-agents profile delete`

```bash
pixel-agents profile delete <name> [options]
```
...
```bash
pixel-agents profile delete mybot
pixel-agents profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.pixel-agents`) cannot be deleted — use `pixel-agents uninstall` to remove everything.
:::

## `pixel-agents profile show`

```bash
pixel-agents profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Pixel Agents home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ pixel-agents profile show work
Profile: work
Path:    ~/.pixel-agents/profiles/work
...
## `pixel-agents profile alias`

```bash
pixel-agents profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Pixel Agents installation.
...
```bash
pixel-agents profile alias work
# Creates/updates ~/.local/bin/work

pixel-agents profile alias work --name mywork
# Creates ~/.local/bin/mywork

pixel-agents profile alias work --remove
# Removes the wrapper script
```

## `pixel-agents profile rename`

```bash
pixel-agents profile rename <old-name> <new-name>
```
...
```bash
pixel-agents profile rename mybot assistant
# ~/.pixel-agents/profiles/mybot → ~/.pixel-agents/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `pixel-agents profile export`

```bash
pixel-agents profile export <name> [options]
```
...
```bash
pixel-agents profile export work
# Creates work.tar.gz in the current directory

pixel-agents profile export work -o ./work-2026-03-29.tar.gz
```

## `pixel-agents profile import`

```bash
pixel-agents profile import <archive> [options]
```
...
```bash
pixel-agents profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

pixel-agents profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`pixel-agents profile export` / `import` are still the right commands for
...
### `pixel-agents profile install`

```bash
pixel-agents profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `pixel-agents -p telemetry`). |
...
pixel-agents profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
pixel-agents profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
pixel-agents profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
pixel-agents profile install ./telemetry/
```

### `pixel-agents profile update`

```bash
pixel-agents profile update <name> [--force-config] [--yes]
```
...
### `pixel-agents profile info`

```bash
pixel-agents profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Pixel Agents version, author, env var requirements, the source URL/path, and
...
`pixel-agents profile list` also shows the distribution name and version in a
`Distribution` column, and `pixel-agents profile show <name>` / `delete <name>`
...
pixel-agents profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
pixel-agents profile install https://github.com/your-org/internal-assistant.git
```
...
`pixel_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Pixel Agents version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Pixel Agents can clone from.
3. Tell recipients to run `pixel-agents profile install <your-repo-url>`.
...
## `pixel-agents -p` / `pixel-agents --profile`

```bash
pixel-agents -p <name> <command> [options]
pixel-agents --profile <name> <command> [options]
```

Global flag to run any Pixel Agents command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
pixel-agents -p work chat -q "Check the server status"
pixel-agents --profile dev gateway start
pixel-agents -p personal skills list
pixel-agents -p work config edit
```

## `pixel-agents completion`

```bash
pixel-agents completion <shell>
```
...
- `pixel-agents profile <TAB>` — subcommands (list, use, create, etc.)
- `pixel-agents profile use <TAB>` — profile names
- `pixel-agents -p <TAB>` — profile names
