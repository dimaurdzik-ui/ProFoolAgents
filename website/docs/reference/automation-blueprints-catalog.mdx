...
of fields, and Pixel Agents schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Pixel Agents asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.pixel-agents.blueprint` block in its
