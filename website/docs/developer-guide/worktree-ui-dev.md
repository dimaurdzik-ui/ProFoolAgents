...
| Variable | Meaning |
|----------|---------|
| `PIXEL_AGENTS_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `PIXEL_AGENTS_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `PIXEL_AGENTS_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Pixel Agents itself — they're private to these helpers. The variables Pixel Agents *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `pixel-agents --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_pixel_root)" || { echo "htui: not in a Pixel Agents checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$PIXEL_AGENTS_MAIN_CHECKOUT/.venv/bin/python" -m pixel_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `PIXEL_AGENTS_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_pixel_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `PIXEL_AGENTS_TUI_DIR` are mutually exclusive
`PIXEL_AGENTS_TUI_DIR` points Pixel Agents at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `pixel-agents --tui --dev` exits with an error. Run `unset PIXEL_AGENTS_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_pixel_root)" || { echo "hgui: not in a Pixel Agents checkout" >&2; return 1; }
  deps="${PIXEL_AGENTS_GUI_DEPS_CHECKOUT:-$PIXEL_AGENTS_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _pixel_link_deps "$desktop" "$deps/apps/desktop"
    _pixel_link_deps "$root" "$deps"
  else
  ...
  trap '_pixel_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS_ROOT="$root" \
    PIXEL_AGENTS_DESKTOP_PYTHON="$PIXEL_AGENTS_MAIN_CHECKOUT/.venv/bin/python" \
    PIXEL_AGENTS_DESKTOP_IGNORE_EXISTING=1 \
    PIXEL_AGENTS_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `pixel-agents`. |
| `PIXEL_AGENTS_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `PIXEL_AGENTS_DESKTOP_IGNORE_EXISTING` | Ignores any `pixel-agents` on `PATH` so it can't shadow the worktree. |
| `PIXEL_AGENTS_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Pixel Agents checkout.
_pixel_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/pixel_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_pixel_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'pixel_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `pixel-agents --tui --dev` and the `PIXEL_AGENTS_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/PixelResearch/pixel-agents/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `PIXEL_AGENTS_*` variable Pixel Agents reads
