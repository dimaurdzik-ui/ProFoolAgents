...
title: "Desktop Plugin SDK (@pixel-agents/plugin-sdk)"
description: "Extend the native Pixel Agents Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Pixel Agents Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `PixelAgentsPlugin`.
It imports one module — `@pixel-agents/plugin-sdk` — and gets everything: the app's
...
`$PIXEL_AGENTS_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Pixel Agents. This page is the **native
desktop app** (`pixel-agents desktop`) SDK — the `@pixel-agents/plugin-sdk` module and
`$PIXEL_AGENTS_HOME/desktop-plugins/`. The **web dashboard** (`pixel-agents dashboard`) has
its own, unrelated plugin system on `window.__PIXEL_AGENTS_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Pixel Agents Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$PIXEL_AGENTS_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

Both take the same `PixelAgentsPlugin` contract, appear in **Settings → Plugins**, and
...
Create `$PIXEL_AGENTS_HOME/desktop-plugins/hello/plugin.js` (that's `~/.pixel-agents/...`
by default, or `~/.pixel-agents/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.pixel-agents/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@pixel-agents/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Pixel Agents' }),
      jsx('div', {
...
The only importable specifiers are `@pixel-agents/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `PixelAgentsPlugin`:

```ts
interface PixelAgentsPlugin {
...
interface PluginContext {
  ...
  /** Plugin-scoped JSON persistence (keys live under `pixel-agents.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@pixel-agents/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@pixel-agents/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@pixel-agents/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@pixel-agents/plugin-sdk'
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@pixel-agents/plugin-sdk'
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@pixel-agents/plugin-sdk'
...
```javascript
import { queryClient } from '@pixel-agents/plugin-sdk'
...
`dashboard/` subfolder of a regular Pixel Agents plugin and declare it in a
`manifest.json`:

```
~/.pixel-agents/plugins/<id>/
└── dashboard/
...
pixel-agents codebase directly (`pixel_state`, `pixel_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.pixel-agents/`)
...
(`pixel-agents.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `PixelAgentsPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@pixel-agents/plugin-sdk` alias.
2. It's still lint-fenced to `@pixel-agents/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@pixel-agents/plugin-sdk`, `react`,
...
| Plugin contract | `PixelAgentsPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginRestOptions`, `Contribution` |
...
### Agents: the `pixel-agents-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`pixel-agents-desktop-plugins`** skill — it carries the same contract as this page in
...
`$PIXEL_AGENTS_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `pixel-agents logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@pixel-agents/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.pixel-agents/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.pixel-agents/logs/errors.log` for
