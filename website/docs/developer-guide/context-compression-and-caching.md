...
Pixel Agents uses a dual compression system and Anthropic prompt caching to
...
Configure via `pixel-agents plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Pixel Agents has two separate compression layers that operate independently:
...
compression:
  ...
  codex_app_server_auto: native  # native|pixel-agents|off for Codex app-server thread compaction
...
auxiliary:
  compression:
    ...
    provider: auto           # Provider: "auto", "openrouter", "pixel", "main", etc.
...
| `codex_app_server_auto` | `native` | `native`, `pixel-agents`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Pixel Agents raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$PIXEL_AGENTS_HOME`
...
```bash
pixel-agents config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
pixel-agents config set compression.codex_gpt55_autoraise_notice false
```
...
thread context, so Pixel Agents' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Pixel Agents
  records the resulting compaction events (compression counters, session
  events). Set `pixel-agents` to let Pixel Agents' compression threshold initiate
  app-server compaction, or `off` to disable Pixel Agents-initiated automatic
  compaction entirely (codex may still compact natively).

Pixel Agents' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Pixel Agents' summary compressor.
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Pixel Agents
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Pixel Agents can avoid; user-facing docs for `/model`, fallback providers, and
