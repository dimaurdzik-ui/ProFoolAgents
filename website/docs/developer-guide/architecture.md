...
sidebar_position: 1
title: "Architecture"
description: "Pixel Agents internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Pixel Agents internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ pixel_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # PixelAgentsCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── pixel_state.py           # SQLite session/state database with FTS5
├── pixel_constants.py       # PIXEL_AGENTS_HOME, profile-aware paths
...
│
├── pixel_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `pixel-agents` subcommands (large file)
...
│   ├── skills_config.py      # pixel-agents skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # pixel-agents tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # pixel-agents gateway start/stop
│
...
```text
User input → PixelAgentsCLI.process_input()
...
The synchropixel orchestration engine (`AIAgent` in `run_agent.py`). Handles provider selection, prompt construction, tool execution, retries, fallback, callbacks, compression, and persistence. Supports three API modes for different provider backends.
...
Three discovery sources: `~/.pixel-agents/plugins/` (user), `.pixel-agents/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `pixel-agents plugins` or `config.yaml`.
...
Exposes Pixel Agents as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`pixel-agents -p <name>`) gets its own PIXEL_AGENTS_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
