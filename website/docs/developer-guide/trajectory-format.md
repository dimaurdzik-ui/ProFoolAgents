...
Pixel Agents saves conversation trajectories in ShareGPT-compatible JSONL format
...
It follows the Pixel Agents function-calling prompt template with:
...
Trajectory saving is a `run_agent.py` / library-level switch — the `pixel-agents` CLI
