...
title: "Build a Pixel Agents Plugin"
description: "Step-by-step guide to building a complete Pixel Agents plugin with tools, hooks, data files, and skills"
---

# Build a Pixel Agents Plugin

This guide walks through building a complete Pixel Agents plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Pixel Agents has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `PIXEL_AGENTS_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.pixel-agents/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `pixel-agents skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `PixelResearch/pixel-agents`. Users install them into `~/.pixel-agents/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Pixel Agents Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/PixelResearch/pixel-agents/blob/main/CONTRIBUTING.md) for the policy.
:::
...
```bash
mkdir -p ~/.pixel-agents/plugins/calculator
cd ~/.pixel-agents/plugins/calculator
```
...
This tells Pixel Agents: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Pixel Agents may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `pixel-agents my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Pixel Agents continues fine
...
If your plugin doesn't show up — or shows up but isn't loading — set `PIXEL_AGENTS_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
PIXEL_AGENTS_PLUGINS_DEBUG=1 pixel-agents plugins list
```
...
The same logs are always written to `~/.pixel-agents/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
pixel-agents logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `pixel-agents plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.pixel-agents/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.pixel-agents/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
```
~/.pixel-agents/plugins/calculator/
...
```
~/.pixel-agents/plugins/my-plugin/
├── __init__.py
├── plugin.yaml
└── skills/
    ├── my-workflow/
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.pixel-agents/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.pixel-agents/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `pixel-agents plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Pixel Agents into installing arbitrary packages — only specs Pixel Agents itself ships are eligible |
...
auditable in `~/.pixel-agents/logs/agent.log`. Plugins load after built-in
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$PIXEL_AGENTS_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $PIXEL_AGENTS_HOME/hook_outputs
```
...
- **The system prompt is Pixel Agents's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `pixel-agents <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for pixel-agents my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: pixel-agents my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for pixel-agents my-plugin."""
...
After registration, users can run `pixel-agents my-plugin status`, `pixel-agents my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `pixel-agents name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `pixel-agents chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `PIXEL_AGENTS_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `pixel-agents <subcommand>` (e.g. `pixel-agents kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "pixel-agents kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Pixel Agents from a hook.
...
Pixel Agents has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.pixel-agents/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `pixel-agents gateway status` without SDK instantiation.
...
Pixel Agents also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Pixel Agents without any Python plugin. Declare them in `~/.pixel-agents/config.yaml`:
...
Pixel Agents connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.pixel-agents/hooks/<name>/`:

```yaml
# ~/.pixel-agents/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.pixel-agents/hooks/long-task-alert/handler.py
...
pixel-agents skills search my-workflow --source myorg/skills-repo
pixel-agents skills install myorg/skills-repo/my-workflow
```
...
```toml
# pyproject.toml
[project.entry-points."pixel_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install pixel-agents-plugin-calculator
# Plugin auto-discovered on next pixel-agents startup
```
...
```python
# Wrong — will break if Pixel Agents passes extra context
def handler(args):
    ...
