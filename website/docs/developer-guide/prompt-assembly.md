...
description: "How Pixel Agents builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Pixel Agents deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.pixel-agents.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
```
# Layer 1: Agent Identity (from ~/.pixel-agents/SOUL.md)
You are Pixel Agents, an AI assistant created by Pixel Agents.
...
The platform hint (Layer 10 above) is the per-surface guidance Pixel Agents
...
`SOUL.md` lives at `~/.pixel-agents/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_pixel_md(cwd_path)       # 1. .pixel-agents.md / PIXEL_AGENTS.md (walks to git root)
    ...
    # SOUL.md from PIXEL_AGENTS_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.pixel-agents.md`, `PIXEL_AGENTS.md` | CWD up to git root | Pixel Agents-native project config |
...
- **YAML frontmatter stripped** — `.pixel-agents.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, Pixel Agents concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.pixel-agents.md` / `PIXEL_AGENTS.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Pixel Agents already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.pixel-agents/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.pixel-agents/MEMORY.md` and `~/.pixel-agents/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.pixel-agents.md`, `PIXEL_AGENTS.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Pixel Agents.
- Ephemeral overlays such as `PIXEL_AGENTS_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Pixel Agents assembles prompts for everyone, change Python and treat it as a code contribution
