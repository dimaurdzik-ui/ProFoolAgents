...
The ACP adapter wraps Pixel Agents' synchropixel `AIAgent` in an async JSON-RPC stdio server.
...
```text
pixel-agents acp / pixel-agents-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.pixel-agents/.env
  -> configure stderr logging
  -> construct PixelAgentsACPAgent
...
- `allow_once` -> Pixel Agents `once`
- `allow_always` -> Pixel Agents `always`
- reject options -> Pixel Agents `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Pixel Agents tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["pixel-agents-acp"])
...
Instead it reuses Pixel Agents' runtime resolver:

- `acp_adapter/auth.py`
- `pixel_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Pixel Agents provider/credentials. It also always advertises a terminal setup auth method (`pixel-agents-setup`, args `--setup`) so first-run ACP clients can open Pixel Agents' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.pixel-agents/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `pixel-agents-acp` toolset definition
- `pixel_cli/main.py` — `pixel-agents acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `pixel-agents-acp` script
