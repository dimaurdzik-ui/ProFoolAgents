...
description: "How Pixel Agents stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `pixel_cli/cron.py` | CLI `pixel-agents cron` subcommands |
...
Jobs are stored in `~/.pixel-agents/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
- **a named provider** (e.g. `chronos`, a managed-cron provider for
  ...
  `$PIXEL_AGENTS_HOME/plugins/<name>/`.
...
In CLI mode, cron jobs only fire when `pixel-agents cron` commands are run or during active CLI sessions.
...
keep the process awake), it asks Pixel infrastructure to arm exactly **one
managed one-shot per job at that job's real next-fire time**. At fire time Pixel
...
The flow (the managed scheduler is provided by Pixel; the agent holds no
scheduler credentials):

```
create/update a cron job
  → Chronos asks Pixel to arm a one-shot at the job's next_run_at
      (authenticated with the agent's existing Pixel token)
  → at fire time Pixel calls the gateway: POST {callback_url}/api/cron/fire
      (authenticated with a short-lived, purpose-scoped Pixel-minted JWT)
...
Config (all non-secret; on hosted agents Pixel sets these at provision time):

| key | meaning |
|---|---|
| `cron.provider` | `chronos` to activate (empty = built-in ticker) |
| `cron.chronos.portal_url` | Pixel base URL (arming + the fire-token issuer) |
...
If Chronos is misconfigured or the agent isn't logged into Pixel,
...
agent↔Pixel wire contract lives in `docs/chronos-managed-cron-contract.md`.
...
```python
# ~/.pixel-agents/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.pixel-agents/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `pixel-agents cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `pixel-agents cron` CLI provides direct job management:

```bash
pixel-agents cron list                    # Show all jobs
pixel-agents cron create                  # Interactive job creation (alias: add)
pixel-agents cron edit <job_id>           # Edit job configuration
pixel-agents cron pause <job_id>          # Pause a running job
pixel-agents cron resume <job_id>         # Resume a paused job
pixel-agents cron run <job_id>            # Trigger immediate execution
pixel-agents cron remove <job_id>         # Delete a job
```
