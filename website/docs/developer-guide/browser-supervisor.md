...
description: "How Pixel Agents detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Pixel Agents' browser tooling:
...
`window.alert`/`confirm`/`prompt` with a synchropixel XHR to a magic host
(`pixel-agents-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Pixel Agents `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `pixel-agents-acp`, `pixel-agents-api-server`, and core toolsets (gated on CDP reachability)
- `pixel_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
