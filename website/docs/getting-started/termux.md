...
description: "Run Pixel Agents directly on an Android phone with Termux"
---

# Pixel Agents on Android with Termux
...
Pixel Agents can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Pixel Agents from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Pixel Agents now ships a Termux-aware installer path:
...
- links `pixel-agents` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `pixel-agents` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/pixel-agents" "$PREFIX/bin/pixel-agents"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `pixel-agents` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.pixel-agents/.env`.
...
### `pixel-agents doctor` says ripgrep or Node is missing
