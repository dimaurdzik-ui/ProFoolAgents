...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Pixel Agents — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Pixel Agents setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Pixel Agents Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Pixel Agents Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Pixel Agents for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Pixel Agents working on my machine | `pixel-agents setup` | Run a real chat and verify it responds |
| I already know my provider | `pixel-agents model` | Save the config, then start chatting |
| I want a bot or always-on setup | `pixel-agents gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `pixel-agents model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `pixel-agents model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Pixel Agents cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Pixel Agents
### With the Pixel Agents Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Pixel Agents Desktop installer](https://api.pixelagents.com/) from our website and run it.

### Without Pixel Agents Desktop:
For a command-line only install without Pixel Agents Desktop, run:
...
The single most important setup step. Use `pixel-agents model` to walk through the choice interactively:

```bash
pixel-agents model
```

:::tip Easiest path: Pixel Portal
...
That logs you in, sets Pixel as your provider, and turns on the Tool Gateway in one command.
:::

:::info Setup modes
On a fresh install, `pixel-agents setup` offers three modes:

- **Quick Setup (Pixel Portal)** — free OAuth login, no API keys; sets up a model plus the Tool Gateway tools. The recommended fast path.
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `pixel-agents update`. Re-enable anything later with `pixel-agents tools`, seed skills with `pixel-agents skills opt-in --sync`, or tune settings with `pixel-agents setup agent`.
:::
...
|----------|-----------|---------------|
| **Pixel Portal** | Subscription-based, zero-config | OAuth login via `pixel-agents model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `pixel-agents model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `pixel-agents model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `pixel_cli/models.py` may change between releases) | `pixel-agents model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `pixel-agents model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `pixel-agents model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `pixel-agents model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `pixel-agents model` (requires `copilot` CLI + `copilot login`) |
...
Pixel Agents requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `pixel-agents model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Pixel Agents separates secrets from normal config:

- **Secrets and tokens** → `~/.pixel-agents/.env`
- **Non-secret settings** → `~/.pixel-agents/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
pixel-agents config set model anthropic/claude-opus-4.6
pixel-agents config set terminal.backend docker
pixel-agents config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
pixel-agents            # classic CLI
pixel-agents --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Pixel Agents ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `pixel-agents` vs `pixel-agents --tui`.
:::
...
- Pixel Agents replies without error
...
```bash
pixel-agents --continue    # Resume the most recent session
pixel-agents -c            # Short form
```
...
```bash
pixel-agents gateway setup    # Interactive platform configuration
```
...
- `pixel-agents tools` — tune tool access per platform
- `pixel-agents skills` — browse and install reusable workflows
...
```bash
pixel-agents config set terminal.backend docker    # Docker isolation
pixel-agents config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `pixel-agents egress setup && pixel-agents egress start`; `pixel-agents setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Pixel Agents install directory (the curl installer placed it at
# ~/.pixel-agents/pixel-agents on Linux/macOS or %LOCALAPPDATA%\pixel-agents\pixel-agents on Windows):
cd ~/.pixel-agents/pixel-agents
...
Skills are on-demand instruction documents that teach Pixel Agents how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Pixel Agents ships with a catalog of bundled skills already installed in `~/.pixel-agents/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
pixel-agents skills browse                      # list everything available
pixel-agents skills search kubernetes           # find skills by keyword
pixel-agents skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `pixel-agents skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Pixel Agents ask what you need
```
...
```yaml
# Add to ~/.pixel-agents/config.yaml
mcp_servers:
  github:
    command: npx
...
(If you installed without `[all]`, run `cd ~/.pixel-agents/pixel-agents && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Pixel Agents opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `pixel-agents model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `pixel-agents gateway setup` and check `pixel-agents gateway status` |
| `pixel-agents --continue` can't find old session | Switched profiles or session never saved | Check `pixel-agents sessions list` and confirm you're in the right profile |
...
| `pixel-agents doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |

## Recovery Toolkit

When something feels off, use this order:

1. `pixel-agents doctor`
2. `pixel-agents model`
3. `pixel-agents setup`
4. `pixel-agents sessions list`
5. `pixel-agents --continue`
6. `pixel-agents gateway status`
...
| `pixel-agents tools` | Configure which tools are enabled per platform |
| `pixel-agents setup` | Full setup wizard (configures everything at once) |
| `pixel-agents doctor` | Diagnose issues |
| `pixel-agents update` | Update to latest version |
| `pixel-agents gateway` | Start the messaging gateway |
| `pixel-agents --continue` | Resume last session |
