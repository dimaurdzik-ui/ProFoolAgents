...
- 点击"New SSH key"
- 粘贴公钥内容
- 填写标题，如"pixel-agents-&lt;machine-name>"
...
elif [ -f ~/.pixel-agents/.env ] && grep -q "^GITHUB_TOKEN=" ~/.pixel-agents/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.pixel-agents/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
