#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Pixel Agents docs site.
...
Both publish at:
  https://api.pixelagents.com/docs/llms.txt
  https://api.pixelagents.com/docs/llms-full.txt
...
SITE_BASE = "https://api.pixelagents.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Pixel Agents as a Python Library", None),
        ("guides/use-mcp-with-pixel-agents", "Use MCP with Pixel Agents", None),
        ("guides/use-voice-mode-with-pixel-agents", "Use Voice Mode with Pixel Agents", None),
        ("guides/use-soul-with-pixel-agents", "Use SOUL.md with Pixel Agents", None),
        ("guides/build-a-pixel-agents-plugin", "Build a Pixel Agents Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Pixel Agents"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Pixel Agents")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Pixel Agents. A terminal-native "
        ...
        "backends. Works with Pixel Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/PixelResearch/"
        "pixel-agents/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/PixelResearch/pixel-agents")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Pixel Agents — Full Documentation\n",
        (
            "This file is the entire Pixel Agents documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Pixel Agents, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://api.pixelagents.com/docs\n",
        "Short index: https://api.pixelagents.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
