...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/PixelResearch/pixel-agents/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    pixel_meta = (fm.get("metadata") or {}).get("pixel-agents") or {}
    tags = pixel_meta.get("tags") or []
    related = pixel_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `pixel-agents skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Pixel Agents"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Pixel Agents ships with a large built-in skill library copied into `~/.pixel-agents/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Pixel Agents also syncs bundled skills on `pixel-agents update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.pixel-agents/skills/` tree, it is still shipped with Pixel Agents; restore it with `pixel-agents skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with pixel-agents — install via pixel-agents skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with pixel-agents under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
