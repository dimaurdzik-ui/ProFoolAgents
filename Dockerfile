...
# Do not write .pyc files at runtime: /opt/pixel-agents is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/pixel-agents/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when pixel-agents
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# pixel-agents process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
db.execute(\"INSERT INTO docs VALUES ('pixel-agents')\"); \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main pixel-agents process, the dashboard,
...
# 'Pixel Agents WebUI' catalog, NAS compose projects that preserve an old
...
# Non-root user for runtime; UID can be overridden via PIXEL_AGENTS_UID at runtime
RUN useradd -u 10000 -m -d /opt/data pixel-agents
...
WORKDIR /opt/pixel-agents

# ---------- Layer-cached dependency install ----------
# Copy only package manifests first so npm install + Playwright are cached
# unless the lockfiles themselves change.
#
# ui-tui/packages/pixel-agents-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/pixel-agents-ink/ ui-tui/packages/pixel-agents-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as pixel-agents-ink above).
...
# lock on the @pixel-agents/ink entry, tripped the TUI launcher's
...
#
# The [otlp] extra contains the SDK/exporter imported by Pixel Agents when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Pixel Agents production image.
#
# The hindsight memory provider's client (hindsight-client) is baked in
# for the same reason: it lazy-installs into /opt/pixel-agents/.venv at first
...
# gives the non-root pixel-agents user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link pixel-agents itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/pixel-agents are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the pixel-agents user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/pixel-agents/bin && \
    cp /opt/pixel-agents/docker/pixel-agents-exec-shim.sh /opt/pixel-agents/bin/pixel-agents && \
    chmod 0755 /opt/pixel-agents/bin/pixel-agents && \
    printf 'docker\n' > /opt/pixel-agents/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $PIXEL_AGENTS_HOME. $PIXEL_AGENTS_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``pixel-agents update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the pixel-agents user via
# `s6-setuidgid pixel-agents` in its run script. If PIXEL_AGENTS_UID is unset, services
# run as the default pixel-agents user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `pixel-agents dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the PIXEL_AGENTS_GIT_SHA build-arg to
# /opt/pixel-agents/.pixel_build_sha at build time, and have
# pixel_cli/build_info.py read it at runtime.  Both `pixel-agents dump` and
...
ARG PIXEL_AGENTS_GIT_SHA=
RUN if [ -n "${PIXEL_AGENTS_GIT_SHA}" ]; then \
        printf '%s\n' "${PIXEL_AGENTS_GIT_SHA}" > /opt/pixel-agents/.pixel_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-pixel-agents + dashboard.
...
# `exec pixel-agents`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $PIXEL_AGENTS_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/pixel-agents/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-pixel-agents-setup && \
    chmod +x /etc/cont-init.d/01-pixel-agents-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV PIXEL_AGENTS_WEB_DIST=/opt/pixel-agents/pixel_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/pixel-agents/ui-tui/dist/entry.js`) and skip the
...
ENV PIXEL_AGENTS_TUI_DIR=/opt/pixel-agents/ui-tui
ENV PIXEL_AGENTS_HOME=/opt/data
ENV PIXEL_AGENTS_WRITE_SAFE_ROOT=/opt/data
ENV PIXEL_AGENTS_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/pixel-agents (root-owned, read-only) so a runtime
...
# is seeded + chowned to the pixel-agents user by docker/stage2-hook.sh and lives
...
ENV PIXEL_AGENTS_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> pixel-agents ...` they default to root, and any file the
# command writes under $PIXEL_AGENTS_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/pixel-agents/bin/pixel-agents, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid pixel-agents`
# when invoked as root. Non-root callers (supervised processes,
# `--user pixel-agents`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/pixel-agents/.venv/bin/pixel-agents). See the shim source for
# the opt-out env var (PIXEL_AGENTS_DOCKER_EXEC_AS_ROOT=1).

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> pixel-agents ...` and any
# subprocess that doesn't activate the venv first still find pixel-agents.
#
# /opt/pixel-agents/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/pixel-agents/bin:/opt/pixel-agents/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. pixel-agents
# subcommand vs. no-args), drops to the pixel-agents user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/pixel-agents/docker/main-wrapper.sh" ]
CMD [ ]
