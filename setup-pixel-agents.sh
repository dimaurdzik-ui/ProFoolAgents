...
# 5. Symlinks the 'pixel-agents' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Pixel Agents Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/pixel-agents-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink pixel-agents into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up pixel-agents command..."

PIXEL_AGENTS_BIN="$SCRIPT_DIR/venv/bin/pixel-agents"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$PIXEL_AGENTS_BIN" "$COMMAND_LINK_DIR/pixel-agents"
echo -e "${GREEN}✓${NC} Symlinked pixel-agents → $COMMAND_LINK_DISPLAY_DIR/pixel-agents"
...
# ============================================================================
# Seed bundled skills into ~/.pixel-agents/skills/
# ============================================================================

PIXEL_AGENTS_SKILLS_DIR="${PIXEL_AGENTS_HOME:-$HOME/.pixel-agents}/skills"
mkdir -p "$PIXEL_AGENTS_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.pixel-agents/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$PIXEL_AGENTS_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  pixel-agents status        # Check configuration"
if is_termux; then
    echo "  pixel-agents gateway       # Run gateway in foreground"
else
    echo "  pixel-agents gateway install # Install gateway service (messaging + cron)"
fi
echo "  pixel-agents cron list     # View scheduled jobs"
echo "  pixel-agents doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m pixel_cli.main setup
fi
