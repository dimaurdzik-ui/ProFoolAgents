...
def _state_file() -> Path:
    return get_pixel_agents_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.pixel-agents/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `pixel-agents update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``pixel-agents curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``pixel-agents update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``pixel-agents curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `pixel-agents curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_DRY_RUN_BANNER = (
    ...
    "`pixel-agents curator run` (no flag).\n"
    "\n"
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Pixel Agents' background skill CURATOR. This is an "
    ...
    "into ~/.pixel-agents/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: pixel-agents-config-*, pixel-agents-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.pixel-agents/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.pixel-agents/logs/curator/``)
    ...
    authored skill data in ``~/.pixel-agents/skills/``.

    ``ensure_pixel_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_pixel_agents_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every pixel-agents install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: pixel-agents curator status
        keep an umbrella stable: pixel-agents curator pin document-tools
    ...
    lines.append("full report: pixel-agents curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: pixel-agents curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.pixel-agents/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.pixel-agents/skills/.archive/` for "
            "safety and can be restored via `pixel-agents curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.pixel-agents/skills/.archive/`. "
            "Restore any via `pixel-agents curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `pixel-agents curator restore <name>`")
    lines.append("- All archives live under `~/.pixel-agents/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    If *synchropixel* is True, the LLM review runs in the calling thread; the
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``pixel-agents curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `pixel-agents curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `pixel-agents update` only if "
        ...
        # recorded in state so `pixel-agents curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``pixel-agents model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `pixel-agents model` → auxiliary
    ...
    try:
        ...
        # run_curator_review(synchropixel=True) from the CLI.
