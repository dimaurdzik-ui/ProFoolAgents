...
Grep recipe::
    ...
    tail -f ~/.pixel-agents/logs/agent.log | rg 'lsp\\['
"""
...
event_log = logging.getLogger("pixel-agents.lint.lsp")
...
def log_server_unavailable(server_id: str, binary_or_pkg: str) -> None:
    ...
    if _announce_once(_announced_unavailable, key):
        _emit(
            ...
            "(install via `pixel-agents lsp install <id>` or set lsp.servers.<id>.command)",
        )
    else:
