...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Pixel Agents also honours PIXEL_AGENTS_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("PIXEL_AGENTS_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
_PROVIDER_PREFIXES: frozenset[str] = frozenset({
    "openrouter", "pixel", "openai-codex", "copilot", "copilot-acp",
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_pixel_agents_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from pixel_constants import get_pixel_agents_home
    return get_pixel_agents_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Pixel Agents.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # (native MiniMax-M3, OpenRouter/Pixel minimax/minimax-m3).
    ...
    # so these hardcoded fallbacks prevent Pixel Agents from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.pixelagents.com": "pixel",
    ...
    # (often 8K) makes it unusable for Pixel Agents' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Pixel Agents minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from pixel_constants import get_pixel_agents_home
    return get_pixel_agents_home() / "context_length_cache.yaml"
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    is_output_cap_error = (
        ...
        # OpenRouter/Pixel phrasing of the same condition.
    ...
    # OpenRouter/Pixel format: "maximum context length is N … (A of text input,
...
def is_output_cap_error(error_msg: str) -> bool:
    ...
    output_cap_signal = (
        ...
        or ("in the output" in error_lower                  # OpenRouter / Pixel
            and "maximum context length" in error_lower)
...
def _model_name_suggests_minimax_m3(model: str) -> bool:
    ...
    across surfaces (native MiniMax-M3, OpenRouter/Pixel minimax/minimax-m3).
...
def _query_local_context_length(model: str, base_url: str, api_key: str = "") -> Optional[int]:
    ...
    issues synchropixel ``detect_local_server_type`` + query HTTP calls (bounded
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Pixel Agents grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    Pixel uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
def _resolve_pixel_context_length(
    model: str,
    base_url: str = "",
    api_key: str = "",
) -> Tuple[Optional[int], str]:
    """Resolve Pixel Portal model context length.

    Tries the live Pixel inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    Pixel model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the Pixel /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx <= 32768 and _model_name_suggests_kimi(or_id):
            logger.info(
                ...
                "(Kimi-family underreport, Pixel path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  Pixel URLs,
    ...
    5. Provider-aware lookups (before generic OpenRouter cache):
       ...
       b. Pixel: live /v1/models probe first (authoritative), then OR
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            ...
            # leftover — drop it and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Pixel Portal: the portal /v1/models endpoint is authoritative.
            ...
            elif _infer_provider_from_url(base_url) == "pixel":
                logger.debug(
                    "Bypassing persistent cache for %s@%s (Pixel portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from pixel_cli.models import get_copilot_model_context
    ...
    if effective_provider == "pixel":
        ctx, source = _resolve_pixel_context_length(
        ...
        if ctx:
            ...
            # want it leaking into the persistent cache for Pixel URLs.
    ...
    # the dedicated Pixel/Copilot/GMI branches above.
    ...
    # ``Pixel Agents-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
async def get_model_context_length_async(
    ...
    Offloads the entire synchropixel resolution chain (which contains
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Pixel Agents sends to providers:
