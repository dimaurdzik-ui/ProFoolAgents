...
``kind: backend``) or ``~/.pixel-agents/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`pixel_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.pixel-agents/.env``) so credentials set
    through Pixel Agents' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``pixel-agents tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``pixel-agents tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``pixel_cli/tools_config.py`` to inject this provider as a
