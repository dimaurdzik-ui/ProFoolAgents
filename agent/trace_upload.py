"""Upload a Pixel Agents session transcript to Hugging Face as an agent trace.

Pixel Agents stores sessions in its own SQLite store (``pixel_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``pixel-agents trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Pixel Agents' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "pixel-agents-traces"
_PIXEL_AGENTS_VERSION = "pixel-agents"
...
# ---------------------------------------------------------------------------
# Conversion: Pixel Agents OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Pixel Agents' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Pixel Agents conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.pixel-agents/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `pixel-agents trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/pixel-agents-traces`` dataset. Returns a
