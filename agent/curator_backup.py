...
A pre-run snapshot of ``~/.pixel-agents/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.pixel-agents/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.pixel-agents/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_pixel_agents_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.pixel-agents/cron/jobs.json``)."""
    return get_pixel_agents_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.pixel-agents/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.pixel-agents/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.pixel-agents/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.pixel-agents/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `pixel-agents curator rollback --list` to see available snapshots)",
            None,
        )
