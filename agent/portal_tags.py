"""Centralized Pixel Portal request tags.

Every Pixel Agents request that hits the Pixel Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Pixel can attribute usage to Pixel Agents and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=pixel-agents",
        "client=pixel-agents-client-v<__version__>",
    ]

The version is sourced live from ``pixel_cli.__version__`` so it auto-aligns
...
``pixel_cli.__version__`` is the canonical source of truth.
"""
...
# active conversation id here and ``pixel_portal_tags()`` picks it up as a
...
_conversation_id: ContextVar[Optional[str]] = ContextVar(
    "pixel_portal_conversation_id", default=None
)
...
def _pixel_version() -> str:
    """Return the current Pixel Agents release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``pixel_cli`` cannot be imported (should
...
def pixel_client_tag() -> str:
    """Return the ``client=...`` tag for Pixel Portal requests.

    Format: ``client=pixel-agents-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=pixel-agents-client-v{_pixel_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Pixel Agents session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Pixel Agents conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.pixel-agents/sessions/`` storage, session logs, and lineage.
...
def pixel_portal_tags(session_id: str | None = None) -> List[str]:
    """Return the canonical list of Pixel Portal product tags.

    Always returns a fresh list so callers can mutate it freely
    (e.g. ``merged_extra.setdefault("tags", []).extend(pixel_portal_tags())``).

    When ``session_id`` is provided, a ``conversation=<session_id>`` tag is
    appended so Portal usage can be attributed to a specific Pixel Agents
    ...
    """
    tags = ["product=pixel-agents", pixel_client_tag()]
