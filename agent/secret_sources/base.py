...
``~/.pixel-agents/.env`` has loaded and BEFORE the rest of Pixel Agents reads
``os.environ``.
...
* **Startup-time, synchropixel.**  ``fetch()`` is called once per process
  (per PIXEL_AGENTS_HOME) by the orchestrator in
...
class ErrorKind(str, Enum):
    ...
    A fixed vocabulary keeps startup warnings and ``pixel-agents secrets status``
...
class SecretSource(ABC):
    ...
    Attributes:
        ...
        label: Human-readable name used in startup messages and
            ``pixel-agents secrets status`` (e.g. ``"Bitwarden Secrets Manager"``).
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        may be malformed.  ``home_path`` is the resolved PIXEL_AGENTS_HOME.
        """
    ...
    def remediation(self, kind: Optional["ErrorKind"], cfg: dict) -> str:
        ...
        Called by the startup status printer (and ``pixel-agents secrets ...
        ...
        *what to run* next to fix it — not just what broke.  Sources
        ...
        ``pixel-agents secrets bitwarden token`` for AUTH_FAILED).  Return an
        ...
        generic = {
            ErrorKind.NOT_CONFIGURED: (
                f"Run `pixel-agents secrets {self.name} setup` to finish configuration."
            ),
            ErrorKind.BINARY_MISSING: (
                f"Run `pixel-agents secrets {self.name} setup` to install the helper CLI."
            ),
            ErrorKind.AUTH_FAILED: (
                f"Credentials rejected — run `pixel-agents secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.AUTH_EXPIRED: (
                f"Credentials expired — run `pixel-agents secrets {self.name} setup` "
                "to re-authenticate."
            ),
            ErrorKind.NETWORK: (
...
# codes that must not reach Pixel Agents' own startup output.
...
def run_secret_cli(
    ...
    * The child gets ``PATH``/``HOME``/locale basics plus only the env
      ...
      this point holds every credential Pixel Agents knows about.
    * ``NO_COLOR=1`` is set and stderr/stdout are ANSI-scrubbed so
      helper diagnostics can't smuggle escape sequences into Pixel Agents
      output.
