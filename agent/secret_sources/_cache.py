...
cache problem must never block Pixel Agents startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Pixel Agents home used for cache paths.

    ``home_path`` is whatever ``load_pixel_dotenv()`` already resolved;
    falling back to ``$PIXEL_AGENTS_HOME`` / ``~/.pixel-agents`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from pixel_constants import get_pixel_agents_home

        home_path = get_pixel_agents_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<pixel_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if PIXEL_AGENTS_HOME is ever made traversable.
            try:
