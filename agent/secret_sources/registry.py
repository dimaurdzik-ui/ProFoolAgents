...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``pixel-agents`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's PIXEL_AGENTS_HOME is ``~/.pixel-agents/profiles/<name>``; the
    default profile (``~/.pixel-agents``) returns "".
    """
    if home_path is not None:
        resolved = Path(home_path)
        if resolved.parent.name == "profiles" and resolved.name:
            return resolved.name
    for env_name in ("PIXEL_AGENTS_PROFILE_NAME", "PIXEL_AGENTS_PROFILE"):
