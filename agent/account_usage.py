...
from pixel_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def build_pixel_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    """Map a PixelPortalAccountInfo into an AccountUsageSnapshot for /usage.
    ...
    """
    try:
        from pixel_cli.pixel_account import pixel_portal_topup_url
        ...
        details.append(f"Top up: {pixel_portal_topup_url(account_info)}")
...
def pixel_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    """Return rendered Pixel-credits /usage lines, or [] when there's nothing to show.

    Account-independent of any live agent: gated on "a Pixel account is logged in"
    ...
    Dev override: when PIXEL_AGENTS_DEV_CREDITS_FIXTURE selects a fixture state, /usage
    ...
    try:
        ...
        tok = (get_provider_auth_state("pixel") or {}).get("access_token")
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(
                get_pixel_portal_account_info, force_fresh=True
            ).result(timeout=timeout)
        snapshot = build_pixel_credits_snapshot(account)
        return render_account_usage_lines(snapshot, markdown=markdown)
    except Exception:
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so PIXEL_AGENTS_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    ...
    try:
        ...
        details.append("(dev fixture — PIXEL_AGENTS_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="pixel",
            source="dev-fixture",
            fetched_at=_utc_now(),
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    try:
        ...
        from pixel_cli.pixel_account import (
            ...
            pixel_portal_topup_url,
        )

        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(get_pixel_portal_account_info, force_fresh=True).result(
                timeout=timeout
            )
    except Exception:
    ...
    # ("Top up: <url>" + "(or run /topup)") that build_pixel_credits_snapshot
    ...
    return CreditsView(
        ...
        topup_url=pixel_portal_topup_url(account),
...
def _resolve_codex_usage_credentials(
    ...
    state, then the credential pool.  Pixel Agents's native OAuth setup now stores
    ...
    *,
    ...
    try:
        ...
        return CodexResetRedeemResult(
            status="unavailable",
            message="No Codex credentials available. Run `pixel-agents auth` to sign in with your ChatGPT account.",
        )
        ...
        if code in (401, 403):
            return CodexResetRedeemResult(
                ...
                message=(
                    ...
                    "run `pixel-agents auth` and sign in with your ChatGPT account."
                ),
            )
        return CodexResetRedeemResult(
            status="unavailable",
    ...
    if code == "reset":
        ...
        # persisted pool cooldowns so Pixel Agents doesn't keep the credential
        # frozen behind the now-stale ``last_error_reset_at`` (issue #43747).
        try:
            from pixel_cli.auth import clear_codex_pool_quota_cooldowns
