...
projection into Pixel Agents' display, approval bridging, transcript projection into
...
"codex_app_server"`. Pixel Agents' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    Threading model:
      - Spawning thread (caller) drives request/response pairs synchropixelly.
    ...
    Intentionally NOT async. AIAgent.run_conversation() is synchropixel and
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Pixel Agents secret — gateway
        ...
        spawn_env = pixel_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("PIXEL_AGENTS_KANBAN_TASK"):
            kanban_db = spawn_env.get("PIXEL_AGENTS_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "PIXEL_AGENTS_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("PIXEL_AGENTS_HOME", os.path.expanduser("~/.pixel-agents")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "pixel-agents",
        client_title: str = "Pixel Agents",
