...
Owns one Codex thread per Pixel Agents session. Drives `turn/start`, consumes
...
call is synchropixel and behaves like AIAgent's existing chat_completions loop.
"""
...
# Pixel Agents' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_PIXEL_AGENTS_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Pixel Agents/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Pixel Agents session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _PIXEL_AGENTS_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("PIXEL_AGENTS_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            ...
            client_title="Pixel Agents",
            client_version=_get_pixel_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        into Pixel Agents' messages shape.
        ...
        # supports rich content but Pixel Agents' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Pixel Agents' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own pixel-agents-tools callback we
            # auto-accept — the user already approved Pixel Agents' tools
            ...
            if server_name == "pixel-agents-tools":
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Pixel Agents
        approval-mode/timeout logic. The Pixel Agents-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Pixel Agents approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Pixel Agents approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Pixel Agents returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Pixel Agents approval mode/timeout resolution
...
def _get_pixel_version() -> str:
    """Best-effort Pixel Agents version string for codex's userAgent line."""
    try:
