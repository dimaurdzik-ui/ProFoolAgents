"""Projects codex app-server events into Pixel Agents' messages list.

The translator that lets Pixel Agents' memory/skill review keep working under the
...
Pixel Agents' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Pixel Agents
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Pixel Agents' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Pixel Agents stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Pixel Agents already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Pixel Agents' messages store text only.
        text_parts: list[str] = []
