...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Pixel Agents with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.pixel-agents/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Pixel Agents
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``pixel-agents tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``pixel-agents setup`` for
