...
as ``kind: backend``) or ``~/.pixel-agents/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``pixel-agents tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``pixel-agents tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``pixel-agents tools`` picker.
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        model knows when ``image_url`` is honored. Used by ``pixel-agents tools``
...
def _images_cache_dir() -> Path:
    """Return ``$PIXEL_AGENTS_HOME/cache/images/``, creating parents as needed."""
    from pixel_constants import get_pixel_agents_home

    path = get_pixel_agents_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$PIXEL_AGENTS_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$PIXEL_AGENTS_HOME/cache/images/``.
