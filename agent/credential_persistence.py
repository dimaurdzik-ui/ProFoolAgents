...
``auth.json``.  They intentionally have no dependency on ``pixel_cli.auth`` so
...
# Sources Pixel Agents owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Pixel Agents-owned OAuth/device-code state)
