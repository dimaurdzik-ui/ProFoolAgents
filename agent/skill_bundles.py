...
Storage
-------
Bundles live in ``~/.pixel-agents/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``PIXEL_AGENTS_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``pixel-agents bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under PIXEL_AGENTS_HOME.

    Honors ``PIXEL_AGENTS_BUNDLES_DIR`` for tests; falls back to
    ``<PIXEL_AGENTS_HOME>/skill-bundles``.
    """
    override = os.environ.get("PIXEL_AGENTS_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_pixel_agents_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `pixel-agents bundles` CLI subcommand.
# ---------------------------------------------------------------------------
