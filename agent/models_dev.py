...
Data resolution order:
  ...
  2. Disk cache (~/.pixel-agents/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Pixel Agents ↔ models.dev
# ---------------------------------------------------------------------------

# Pixel Agents provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Pixel Agents (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_pixel_agents_home() / "models_dev_cache.json"
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``pixel-agents config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Pixel Agents provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Pixel Agents agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Pixel Agents provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Pixel Agents ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Pixel Agents or models.dev provider ID.  Tries exact match then
