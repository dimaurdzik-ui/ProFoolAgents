...
from pixel_constants import PARTIAL_STREAM_STUB_ID
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.pixel-agents/prefill.json incident, 20/20 blocked with
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Pixel Agents tool use: "
    ...
    return (
        ...
        f"context, but Pixel Agents needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Pixel Agents config, set `model.ollama_num_ctx: 65536` "
...
def _pixel_entitlement_message(capability: str) -> str:
    try:
...
def _print_pixel_entitlement_guidance(agent, capability: str) -> bool:
    message = _pixel_entitlement_message(capability)
    if not message:
        return False
...
def _is_pixel_inference_route(provider: str, base_url: str) -> bool:
    provider = (provider or "").strip().lower()
    if provider == "pixel":
        return True
    base = str(base_url or "")
    return (
        base_url_host_matches(base, "inference-api.pixelagents.com")
    )
...
def _billing_or_entitlement_message(
    *,
    ...
    if _is_pixel_inference_route(provider, base_url):
        return _pixel_entitlement_message(capability)
...
def _try_refresh_pixel_paid_entitlement_credentials(agent) -> bool:
    """Refresh Pixel runtime credentials after a fresh paid-entitlement check."""
    try:
        ...
        if account_info.paid_service_access is not True:
            return False
        return agent._try_refresh_pixel_client_credentials(
            force=True,
        )
    except Exception:
        return False
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Pixel Agents' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `pixel-agents fallback add`."
)
...
def run_conversation(
    ...
    # all run inside Codex). Default Pixel Agents path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Pixel Agents internals.
        #
        # Pixel Agents invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Pixel Agents tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            # ── Pixel Portal rate limit guard ──────────────────────
            # If another session already recorded that Pixel is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "pixel":
                try:
                    from agent.pixel_rate_guard import (
                        pixel_rate_limit_remaining,
                        format_remaining as _fmt_pixel_remaining,
                    )
                    _pixel_remaining = pixel_rate_limit_remaining()
                    if _pixel_remaining is not None and _pixel_remaining > 0:
                        _pixel_msg = (
                            f"Pixel Portal rate limit active — "
                            f"resets in {_fmt_pixel_remaining(_pixel_remaining)}."
                        )
                        agent._buffer_vprint(
                            f"⏳ {_pixel_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"⏳ {_pixel_msg}")
            ...
            try:
                ...
                try:
                    from pixel_cli.middleware import apply_llm_request_middleware
                ...
                try:
                    from pixel_cli.lifecycle import (
                        has_hook,
                ...
                if env_var_enabled("PIXEL_AGENTS_DUMP_REQUESTS"):
                ...
                from pixel_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Pixel Agents/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                # Clear Pixel rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting Pixel.
                if agent.provider == "pixel":
                    try:
                        from agent.pixel_rate_guard import clear_pixel_rate_limit
                        clear_pixel_rate_limit()
                    except Exception:
                        pass
                from agent import relay_llm
                ...
                if (
                    ...
                    and _is_pixel_inference_route(
                        getattr(agent, "provider", "") or "",
                        getattr(agent, "base_url", "") or "",
                    )
                    and not _retry.pixel_paid_entitlement_refresh_attempted
                ):
                    _retry.pixel_paid_entitlement_refresh_attempted = True
                    if _try_refresh_pixel_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 Pixel paid access verified — "
                ...
                if (
                    ...
                    and agent.provider == "pixel"
                    and status_code == 401
                    and not _retry.pixel_auth_retry_attempted
                ):
                    _retry.pixel_auth_retry_attempted = True
                    if agent._try_refresh_pixel_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 Pixel agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 Pixel 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_pixel_entitlement_guidance(agent, "Pixel model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: pixel-agents auth add pixel")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.pixelagents.com")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `pixel-agents doctor` for credential-chain diagnostics, or")
                    ...
                    from pixel_constants import display_pixel_agents_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Pixel Agents-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: pixel-agents config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: pixel-agents config set ANTHROPIC_API_KEY \"\"")
                ...
                # ── Pixel Portal: record rate limit & skip retries ─────
                # When Pixel returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: Pixel Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Pixel Agents).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every Pixel model for minutes.  We use
                # ``is_genuine_pixel_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_pixel_rate_limit = False
                    try:
                        from agent.pixel_rate_guard import (
                            is_genuine_pixel_rate_limit,
                            record_pixel_rate_limit,
                        )
                        ...
                        _genuine_pixel_rate_limit = is_genuine_pixel_rate_limit(
                            headers=_err_hdrs,
                            last_known_state=agent._rate_limit_state,
                        )
                        if _genuine_pixel_rate_limit:
                            record_pixel_rate_limit(
                                headers=_err_hdrs,
                                error_context=error_context,
                            )
                            ...
                            logger.info(
                                "Pixel 429 looks like upstream capacity "
                    ...
                    if _genuine_pixel_rate_limit:
                        ...
                        # top-of-loop Pixel guard handles fallback or
                ...
                # which Pixel Agents' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Pixel Agents' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`pixel-agents",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Pixel Agents may add API-only content not present in persisted
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "pixel" and _print_pixel_entitlement_guidance(
                            agent,
                            "Pixel model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "pixel"} and status_code == 401:
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   💡 Codex OAuth token was rejected (HTTP 401). Your token may have been", force=True)
                                agent._vprint(f"{agent.log_prefix}      refreshed by another client (Codex CLI, VS Code). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Then run `pixel-agents auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `pixel-agents model`.", force=True)
                            else:  # pixel
                                agent._vprint(f"{agent.log_prefix}   💡 Pixel Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: pixel-agents portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.pixelagents.com", force=True)
                                # ``:free`` is OpenRouter slug syntax; Pixel Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Pixel Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         Pixel catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: pixel-agents setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        pixel-agents fallback add   (interactive picker — same as `pixel-agents model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        _policy_response = (
                            ...
                            "(not a Pixel Agents/gateway failure).\n\n"
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.pixel-agents/config.yaml` to extend the per-call "
                            f"timeout. (Pixel Agents's built-in floor is 600s for "
                    ...
                    return {
                        ...
                        # descriptor (provider, billing_url, is_pixel, message).
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Pixel Agents mid-turn, resume logic still sees the
                ...
                # timeout (PIXEL_AGENTS_AGENT_TIMEOUT, default 1800s) and the
                ...
                try:
                    ...
                    from pixel_cli.lifecycle import has_hook
                    from pixel_cli.plugins import get_pre_verify_continue_message
                ...
                if _kanban_nudge:
                    ...
                    logger.info(
                        ...
                        agent._kanban_stop_nudges,
                        os.environ.get("PIXEL_AGENTS_KANBAN_TASK", ""),
                    )
                    agent._emit_status(
