...
row taxonomy is inferred from the atlas shape so Pixel Agents can render both legacy
...
# rows (1536x1872), while older Pixel Agents/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Pixel Agents' activity state names. They are not always identical to the
...
# Legacy Pixel Agents/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Pixel Agents activity names -> accepted row-name aliases in descending
