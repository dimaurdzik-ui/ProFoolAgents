"""Anthropic Messages API adapter for Pixel Agents.

Translates between Pixel Agents's internal OpenAI-style message format and
...
# Pixel Agents effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_pixel_portal_endpoint(base_url: str | None) -> bool:
    """Return True for Pixel Portal's Anthropic Messages route.

    Portal serves its ``anthropic/*`` catalog natively at
    ``https://inference-api.pixelagents.com/v1/messages``.  Portal-specific
    behaviours key off this: Bearer JWT auth, verbatim catalog model ids,
    and native thinking-signature replay.

    Trusted hosts only:

    1. Prod hostname ``inference-api.pixelagents.com``
    2. The operator-set ``PIXEL_INFERENCE_BASE_URL`` hostname (staging/preview)

    Lookalikes such as ``inference-api.pixelagents.com.attacker.test`` are
    rejected (hostname match, not substring).
    """
    if base_url_host_matches(base_url or "", "inference-api.pixelagents.com"):
        return True
    try:
        from pixel_cli.auth import _pixel_inference_env_override

        override = _pixel_inference_env_override()
    except Exception:
        return False
    if not override:
        return False
...
def _requires_bearer_auth(base_url: str | None) -> bool:
    ...
    Foundry's Anthropic-style endpoint, Palantir Foundry's LLM proxy, and Pixel
    Portal's Messages route follow this pattern.
    """
    if _is_pixel_portal_endpoint(base_url):
        return True
    normalized = _normalize_base_url_text(base_url)
    if not normalized:
        return False
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to pixel-agents's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to pixel-agents's outer conversation
    ...
    # from ``ANTHROPIC_API_KEY`` (Pixel Agents loads that into the process env from
    # ``~/.pixel-agents/.env``). The result is dual auth —
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Pixel Agents notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Pixel Agents historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``pixel-agents models``) never mutates ``~/.pixel-agents/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Pixel Agents)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json)
         — with automatic refresh if expired and a refresh token is available
      4. Anthropic credential_pool OAuth entry (~/.pixel-agents/auth.json)
    ...
    # 1. Pixel Agents-managed OAuth/setup token env var
    ...
    # 4. Pixel Agents credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration Pixel Agents configs.
...
# ── Pixel Agents-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.pixel-agents/.anthropic_oauth.json (our own file).
...
def _get_pixel_oauth_file() -> Path:
    return get_pixel_agents_home() / ".anthropic_oauth.json"
...
def run_pixel_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Pixel Agents-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Pixel Agents with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from pixel_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_pixel_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Pixel Agents-managed OAuth credentials from ~/.pixel-agents/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Pixel Agents OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See pixel-agents#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See pixel-agents: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See pixel-agents#13848 (Kimi) and
    pixel-agents#16748 (DeepSeek).

    Pixel Portal's ``/v1/messages`` route is the exception among third-party
    ...
    _is_third_party = (
        ...
        and not _is_pixel_portal_endpoint(base_url)
    )
...
def build_anthropic_kwargs(
    ...
    # Pixel Portal routes on its own catalog ids (``anthropic/claude-opus-4.8``);
    ...
    if not _is_pixel_portal_endpoint(base_url):
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Pixel Agents", "Claude Code")
                text = text.replace("Pixel Agents agent", "Claude Code")
                text = text.replace("pixel-agents", "claude-code")
                text = text.replace("Pixel Agents", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Pixel Agents-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Pixel Agents surfaces in its CLI. We
...
def create_anthropic_message(
    ...
    *,
    ...
    parsed ``Message`` drops — Pixel Portal's ``x-pixel-credits-*`` balance family
