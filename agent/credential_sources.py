"""Unified removal contract for every credential source Pixel Agents reads from.

Pixel Agents seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.pixel-agents/.env
    claude_code   — ~/.claude/.credentials.json
    pixel_pkce   — ~/.pixel-agents/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (pixel, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `pixel-agents auth add`
...
unify here is **removal**:
    ...
    ``pixel-agents auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, pixel device_code (partial), pixel_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"pixel"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.pixel-agents/.env  → clear it
    ...
    """
    from pixel_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.pixel-agents/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Pixel Agents.",
            f"  The pool entry is now suppressed — Pixel Agents will ignore "
            f"{env_var} until you run `pixel-agents auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Pixel Agents stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `pixel-agents auth add anthropic` to re-enable if needed.",
    ])


def _remove_pixel_pkce(provider: str, removed) -> RemovalResult:
    """~/.pixel-agents/.anthropic_oauth.json is ours — delete it outright."""
    from pixel_constants import get_pixel_agents_home

    result = RemovalResult()
    oauth_file = get_pixel_agents_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Pixel Agents Anthropic OAuth credentials")
...
def _remove_pixel_device_code(provider: str, removed) -> RemovalResult:
    """Pixel OAuth lives in auth.json providers.pixel — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `pixel-agents auth add pixel` (or any other path that writes providers.pixel)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `pixel-agents auth add pixel` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as Pixel: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``pixel-agents auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `pixel-agents model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Pixel Agents auth store is not enough — _seed_from_singletons() would
    ...
    via ``pixel-agents auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `pixel-agents auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `pixel-agents auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Pixel Agents stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `pixel-agents auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="pixel_pkce",
        remove_fn=_remove_pixel_pkce,
        description="~/.pixel-agents/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="pixel", source_id="device_code",
        remove_fn=_remove_pixel_device_code,
        description="auth.json providers.pixel",
    ))
    register(RemovalStep(
