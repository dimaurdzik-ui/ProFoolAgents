...
from pixel_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.pixel-agents/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``PIXEL_AGENTS_KANBAN_TASK`` / ``PIXEL_AGENTS_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("PIXEL_AGENTS_KANBAN_TASK") or os.getenv("PIXEL_AGENTS_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Pixel Agents Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``pixel_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``PIXEL_AGENTS_PLATFORM`` or
            ``PIXEL_AGENTS_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("PIXEL_AGENTS_PLATFORM")
        or get_session_env("PIXEL_AGENTS_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``pixel-agents`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.pixel-agents/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``pixel-agents`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against PIXEL_AGENTS_HOME, not cwd
        if not p.is_absolute():
            p = (pixel_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.pixel-agents/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.pixel-agents/skills/`` (including via symlinks) or configured
    ...
    # ~/.pixel-agents/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Pixel Agents can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    pixel_agents = metadata.get("pixel-agents") or {}
    if not isinstance(pixel_agents, dict):
        pixel_agents = {}
    return {
        "fallback_for_toolsets": pixel_agents.get("fallback_for_toolsets", []),
        "requires_toolsets": pixel_agents.get("requires_toolsets", []),
        "fallback_for_tools": pixel_agents.get("fallback_for_tools", []),
        "requires_tools": pixel_agents.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    pixel_agents = metadata.get("pixel-agents")
    if not isinstance(pixel_agents, dict):
        return []
    raw = pixel_agents.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Pixel Agents metadata, VCS, virtualenv/dependency, cache, and skill
