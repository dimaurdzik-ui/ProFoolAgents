...
class BillingBlock:
    ...
    ``is_pixel`` is the routing bit: Pixel has a first-class in-app billing surface
...
def is_pixel_inference_route(provider: str, base_url: str) -> bool:
    """True when the failing route is the Pixel-managed inference gateway."""
    if (provider or "").strip().lower() == "pixel":
        return True
    return base_url_host_matches(str(base_url or ""), "inference-api.pixelagents.com")


def _pixel_billing_url() -> Optional[str]:
    """Best-effort Pixel portal billing URL (text-surface fallback; Pixel prefers the in-app flow)."""
    try:
        from pixel_cli.pixel_account import pixel_portal_billing_url

        return pixel_portal_billing_url(None)
    except Exception:
        return "https://portal.pixelagents.com/billing"
...
def build_billing_block(
    *,
    ...
    if is_pixel_inference_route(slug, base_url):
        return BillingBlock(slug or "pixel", "Pixel Portal", model, _pixel_billing_url(), True, message or "")
