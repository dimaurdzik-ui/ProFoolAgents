...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into Pixel Agents accounting.

    Codex app-server reports usage via thread/tokenUsage/updated as:
    inputTokens, cachedInputTokens, outputTokens, reasoningOutputTokens,
    totalTokens.

    Pixel Agents' canonical prompt bucket includes uncached input + cached input.
    The Codex app-server protocol does not currently expose cache-write tokens,
    so that bucket remains zero on this runtime.

    Even when Codex omits usage for a turn, Pixel Agents should still count that turn
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in Pixel Agents state.

    The app-server owns the compacted thread context, so Pixel Agents should not
    ...
    if compressor is not None:
        ...
        if callable(record_boundary):
            # Codex owns this summary. A prior Pixel Agents deterministic-fallback
...
# ---------------------------------------------------------------------------
# Codex app-server → Pixel Agents UI bridge (#33200)
#
# The codex_app_server runtime hands the entire turn to a subprocess and
# bypasses the normal Pixel Agents tool loop. Without this bridge gateway
...
# Codex item types that map to a Pixel Agents tool_call in the projector (and
...
# Internal MCP server that wraps Pixel Agents' native tools for codex. When
# codex calls back through it, the inner dispatch runs in a SEPARATE
# pixel-agents-tools-mcp-server subprocess that has no access to the parent
...
# the display event for those calls; we strip the mcp.pixel-agents-tools.*
# namespacing and emit the bare tool name (web_search, browser_navigate,
# vision_analyze, ...) since the user thinks of these as Pixel Agents tools,
# not as MCP calls.
_INTERNAL_MCP_SERVER = "pixel-agents-tools"


def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Pixel Agents tool name for a codex item. Mirrors
...
def _codex_item_to_preview(item: dict) -> Any:
    ...
    None when no useful preview is available (Pixel Agents' UI tolerates None)."""
    item_type = item.get("type") or ""
    if item_type == "commandExecution":
...
def make_codex_app_server_event_bridge(agent) -> Callable[[dict], None]:
    ...
    notifications into Pixel Agents' gateway UI callbacks.
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into Pixel Agents'
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to Pixel Agents' standard prompt flow if a
        ...
        # explicitly opted out of Pixel Agents approvals — via `approvals.mode: off`
        # in config, the /yolo session toggle, or --yolo / PIXEL_AGENTS_YOLO_MODE —
        # honor that and let codex's own sandbox permission profile
        # (~/.codex/config.toml) be the policy gate instead of double-gating
        # with a missing Pixel Agents UI. Defaults (manual/smart/unset) preserve the
        ...
        # item/agentMessage/delta, ...) into Pixel Agents' gateway UI callbacks
    ...
    for attempt in range(max_stream_retries + 1):
        ...
        try:
            try:
                ...
                # and close the physical attempt scope before Pixel Agents returns.
