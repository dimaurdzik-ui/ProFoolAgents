...
Plugins built on Pixel Agents often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Pixel Agents
...
:class:`~pixel_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Pixel Agents' main config
...
handles every provider, fallback chain, and per-task override Pixel Agents
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`pixel_cli.plugins.PluginContext`
