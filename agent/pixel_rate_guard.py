"""Cross-session rate limit guard for Pixel Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Pixel Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Pixel triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Pixel Agents retries), and every one of those calls counts
...
_STATE_FILENAME = "pixel.json"


def _state_path() -> str:
    """Return the path to the Pixel rate limit state file."""
    try:
        from pixel_constants import get_pixel_agents_home
        base = get_pixel_agents_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".pixel-agents")
...
def record_pixel_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Pixel Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Pixel rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Pixel rate limit state: %s", exc)


def pixel_rate_limit_remaining() -> Optional[float]:
    """Check if Pixel Portal is currently rate-limited.
...
def clear_pixel_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Pixel request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Pixel rate limit state: %s", exc)
...
def is_genuine_pixel_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Pixel Portal is a real account rate limit.

    Pixel Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Pixel Agents, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Pixel is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Pixel.

    Tripping the cross-session breaker on (b) blocks ALL Pixel requests
    (and all models, since Pixel is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Pixel emits
