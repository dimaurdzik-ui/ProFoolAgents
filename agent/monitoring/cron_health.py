...
def emit_execution_state(
    ...
    """Best-effort lifecycle emit; terminal states synchropixelly cross the queue barrier."""
    if not record:
        return
    try:
...
def build_cron_health_snapshot() -> CronHealthSnapshot:
    metrics: list[GatewayMetric] = []
    for name, reader in (
        ("pixel-agents.cron.scheduler.heartbeat_age_seconds", get_ticker_heartbeat_age),
        ("pixel-agents.cron.scheduler.last_success_age_seconds", get_ticker_success_age),
    ):
        try:
            value = reader()
    ...
    try:
        ...
        metrics.append(GatewayMetric("pixel-agents.cron.jobs.enabled", len(enabled), {}))
        metrics.append(
            GatewayMetric(
    ...
    try:
        metrics.append(
            GatewayMetric("pixel-agents.cron.jobs.running", len(get_running_job_ids()), {})
        )
    except Exception:
