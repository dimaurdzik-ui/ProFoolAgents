"""Credits tracking for Pixel inference API responses.

Parses x-pixel-credits-* (and optional x-pixel-tool-pool-*) headers from
...
Header schema (x-pixel-credits-* family):
    x-pixel-credits-version                    contract/schema version
    x-pixel-credits-remaining-micros           total remaining balance (micros)
    x-pixel-credits-remaining-usd              same, formatted USD string
    x-pixel-credits-subscription-micros        subscription balance (SIGNED; may be negative/debt)
    x-pixel-credits-subscription-usd           same, formatted USD string
    x-pixel-credits-subscription-limit-micros  subscription cap (PAIRED/optional)
    x-pixel-credits-subscription-limit-usd     same, formatted USD string (PAIRED/optional)
    x-pixel-credits-rollover-micros            rolled-over balance (micros)
    x-pixel-credits-purchased-micros           purchased balance (micros)
    x-pixel-credits-purchased-usd              same, formatted USD string
    x-pixel-credits-denominator-kind           "subscription_cap" | "none"
    x-pixel-credits-paid-access                "true" | "false" (STRING!)
    x-pixel-credits-disabled-reason            reason string (header omitted when null)
    x-pixel-credits-as-of-ms                   server-side timestamp (ms epoch)

Tool-pool headers use a SEPARATE prefix:
    x-pixel-tool-pool-micros                   tool-pool balance (micros)
    x-pixel-tool-pool-gated-off                "true" | "false" (STRING!)
...
@dataclass
class CreditsState:
    """Full credits state parsed from x-pixel-credits-* response headers."""
...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    """Return True when *model* is a Pixel free-tier model, using ONLY local data.

    Two signals, both zero-network:

    1. The ``:free`` suffix — the canonical Pixel free SKU marker (e.g.
       ``nvidia/nemotron-3-ultra:free``). Free by construction on the API side
       (spend is forced to 0 for ``:free`` ids).
    2. A peek into the in-process pricing cache in ``pixel_cli.models``
       ...
       *base_url*). PEEK ONLY — a cache miss never triggers a fetch. This is
       ...
       suffix (which all Pixel free SKUs carry).
    ...
    if not base_url:
        return False
    try:
        from pixel_cli.models import _is_model_free, _pricing_cache

        # Mirror get_pricing_for_provider's key normalization: the agent's
        # Pixel base_url is /v1-suffixed (https://inference-api.pixelagents.com/v1)
...
def evaluate_credits_notices(
    ...
    *,
    ...
    ``model_is_free``: True when the session's active model is a Pixel free-tier
    ...
    if target_band != shown_band:
        ...
        if target_band is not None:
            ...
            # only meaningful against a Pixel subscription cap (no cap → never fires),
...
def parse_credits_headers(
    ...
    """Parse x-pixel-credits-* (and x-pixel-tool-pool-*) headers into a CreditsState.

    Returns None (miss) on ANY of:
    - No ``x-pixel-credits-version`` header present.
    ...
    try:
        ...
        # sentinel header is absent (the common case for non-Pixel providers, on
        ...
        if not any(k.lower() == "x-pixel-credits-version" for k in headers):
            return None
        ...
        version_raw = lowered.get("x-pixel-credits-version")
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update Pixel Agents",
                    version_val,
                )
            return None
        ...
        # ── Parse micros fields ──────────────────────────────────────────────
        remaining_micros = _req_nonneg("x-pixel-credits-remaining-micros")
        if remaining_micros is _SENTINEL:
            return None

        subscription_micros = _req_int("x-pixel-credits-subscription-micros")
        if subscription_micros is _SENTINEL:
            return None

        rollover_micros = _req_nonneg("x-pixel-credits-rollover-micros")
        if rollover_micros is _SENTINEL:
            return None

        purchased_micros = _req_nonneg("x-pixel-credits-purchased-micros")
        if purchased_micros is _SENTINEL:
            return None

        # tool_pool_micros is OPTIONAL: absent → 0 (default); present-but-invalid → None (miss).
        _tp_raw = lowered.get("x-pixel-tool-pool-micros")
        if _tp_raw is None:
            tool_pool_micros = 0
        else:
        ...
        as_of_ms = _req_nonneg("x-pixel-credits-as-of-ms")
        if as_of_ms is _SENTINEL:
            return None

        # ── Validate USD strings ─────────────────────────────────────────────
        remaining_usd = lowered.get("x-pixel-credits-remaining-usd", "")
        if not _validate_usd(remaining_usd):
            return None

        subscription_usd = lowered.get("x-pixel-credits-subscription-usd", "")
        if not _validate_usd(subscription_usd):
            return None

        purchased_usd = lowered.get("x-pixel-credits-purchased-usd", "")
        ...
        sub_limit_micros_raw = lowered.get("x-pixel-credits-subscription-limit-micros")
        sub_limit_usd_raw = lowered.get("x-pixel-credits-subscription-limit-usd")
        ...
        # ── denominator_kind ─────────────────────────────────────────────────
        denominator_kind = lowered.get("x-pixel-credits-denominator-kind", "none")
        ...
        if "x-pixel-credits-paid-access" in lowered:
            pa_raw = lowered["x-pixel-credits-paid-access"].strip().lower()
        ...
        if "x-pixel-tool-pool-gated-off" in lowered:
            tpgo_raw = lowered["x-pixel-tool-pool-gated-off"].strip().lower()
        ...
        disabled_reason = lowered.get("x-pixel-credits-disabled-reason")  # None if absent
...
# ── Dev test fixtures (PIXEL_AGENTS_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set PIXEL_AGENTS_DEV_CREDITS_FIXTURE to either a
...
# immediately), and (3) the /usage view (pixel_credits_lines renders the fixture).
# `clear` / `none` / unset → real behaviour. Delete with the rest of the
# PIXEL_AGENTS_DEV_CREDITS scaffolding.
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for PIXEL_AGENTS_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag PIXEL_AGENTS_DEV_CREDITS
    is also on, so a stray PIXEL_AGENTS_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("PIXEL_AGENTS_DEV_CREDITS")):
        return None
    raw = os.environ.get("PIXEL_AGENTS_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
...
def _credits_state_from_account(info) -> Optional[CreditsState]:
    """Map a PixelPortalAccountInfo into a header-shaped CreditsState for the seed.
...
def seed_credits_at_session_start(agent) -> bool:
    ...
    Returns True if it seeded this call, False otherwise (not pixel / already seeded /
    fail-open error). Never raises — credits must never block session startup.
    """
    try:
        if getattr(agent, "provider", "") != "pixel":
            return False
        ...
        if fixture is not None:
            # Synchropixel: a fixture is instant (no network), and tests rely on the
