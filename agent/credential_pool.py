...
# un-throttled log fires on *every* selection. On Windows several Pixel Agents
...
# readiness handshake ("Timed out connecting to Pixel Agents backend after
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "pixel":
            # Pixel stores the runtime inference credential in agent_key for
            ...
            for token, expires_at in (
                ...
                if (
                    ...
                    and token.strip()
                    and auth_mod._pixel_invoke_jwt_is_usable(
                        token,
...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): pixel, openai-codex, and xai-oauth rotate the
    ...
    ``pixel_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``pixel-agents model`` / ``pixel-agents auth`` which
        ...
        Mirrors the Pixel/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Pixel Agents process
    ...
    def _sync_pixel_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Pixel pool entry from auth.json if tokens differ.

        Pixel OAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_pixel_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the Pixel Portal.
        """
        if self.provider != "pixel" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
                state = _load_provider_state(auth_store, "pixel")
            if not state:
                return entry
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing Pixel state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync Pixel entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Pixel, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Pixel/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``pixel-agents`` invocation that defaults to the active provider
        (e.g. setup wizard, ``pixel-agents auth status``) would land on
        ...
        # device-code sources (pixel, openai-codex, xAI) use ``device_code``.
        ...
        try:
            with _auth_store_lock():
                ...
                # pixel_cli.auth._save_xai_oauth_tokens (#43589); the pool
                ...
                if self.provider == "pixel":
                    ...
                    _store_provider_state(auth_store, "pixel", state, set_active=False)
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Pixel Agents
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``PIXEL_AGENTS_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "PIXEL_AGENTS_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "PIXEL_AGENTS_XAI_REFRESH_TIMEOUT_SECONDS"
        )
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("pixel_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Pixel Agents
            ...
            elif self.provider == "pixel":
                synced = self._sync_pixel_entry_from_auth_store(entry)
                if synced is not entry:
                    entry = synced
                auth_mod.resolve_pixel_runtime_credentials(
                    force_refresh=force,
                )
                updated = self._sync_pixel_entry_from_auth_store(entry)
            else:
                return entry
        except Exception as exc:
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("pixel_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as pixel — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the Pixel quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/pixel — another Pixel Agents process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Pixel quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    self._persist(removed_ids=removed_ids)
                    return None
            # For pixel: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "pixel":
                synced = self._sync_pixel_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("Pixel refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                    ...
                    return updated
                if auth_mod._is_terminal_pixel_refresh_error(exc):
                    logger.debug("Pixel refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                            state = _load_provider_state(auth_store, "pixel") or {
                            ...
                            entry_refresh = str(entry.refresh_token or "").strip()
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_pixel_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_pixel_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                        ...
                        logger.debug("Failed to clear terminal Pixel OAuth state: %s", clear_exc)

                    singleton_sources = {
                        auth_mod.PIXEL_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.PIXEL_DEVICE_CODE_SOURCE}",
                    }
                    removed_ids = [
                        item.id for item in self._entries
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "pixel":
            # Pixel refresh can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Pixel Agents profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For pixel entries, sync from auth.json before status checks.
            # Another process may have successfully refreshed via
            # resolve_pixel_runtime_credentials(), making this entry's
            ...
            # re-authed via `pixel-agents model` / `pixel-agents auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `pixel-agents model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``pixel-agents auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `pixel-agents auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `pixel-agents auth remove <provider> <N>` is stable across all source types.
    try:
        from pixel_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Pixel Agents PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `pixel-agents setup` ("Claude
        ...
        # Claude Code CLI, pixel_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.pixel-agents/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `pixel-agents setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"pixel_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_pixel_oauth_credentials

        for source_name, creds in (
            ("pixel_pkce", read_pixel_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_pixel_credentials(label=...) when the user ran
            # `pixel-agents auth add pixel --label <name>`).  Fall back to the
        ...
        # Pixel Agents auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from pixel_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.pixel-agents/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `pixel-agents auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `pixel-agents auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Pixel Agents auth store.  Without this gate the removal is instantly
        ...
        # Pixel Agents owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `pixel-agents auth openai-codex`.
        ...
        # When the user logs in via ``pixel-agents model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``pixel-agents auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            # always surfaced as ``device_code`` (consistent with pixel/codex).
            source = "device_code"
            if _is_suppressed(provider, source):
                return changed, active_sources
            active_sources.add(source)
            from pixel_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.pixel-agents/.env over os.environ — the user's config file is the
    # authoritative source for Pixel Agents credentials. Stale env vars from parent
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_pixel_dotenv() -> apply_onepassword_secrets()).  The raw
    ...
    # Honour user suppression — `pixel-agents auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.pixel-agents/.env.
    ...
    try:
        from pixel_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from pixel_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.pixel-agents/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `pixel-agents auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Pixel Agents
