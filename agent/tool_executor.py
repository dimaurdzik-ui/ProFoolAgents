...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Pixel Agents process cwd (notably in Docker).  Resolve
...
def _resolve_concurrent_tool_timeout() -> float | None:
    raw = os.getenv("PIXEL_AGENTS_CONCURRENT_TOOL_TIMEOUT_S", "").strip()
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "invalid PIXEL_AGENTS_CONCURRENT_TOOL_TIMEOUT_S=%r; using %.0fs",
            raw,
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Pixel Agents process before the normal turn-end persistence path runs.
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before Pixel Agents policy and dispatch exactly once."""
    ...
    def _pixel_pipeline(relay_args: dict[str, Any]) -> Any:
