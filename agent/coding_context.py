"""Coding-context awareness — base Pixel Agents, every interactive surface.

When the user runs Pixel Agents inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Pixel Agents shifts into a **coding posture**. This module is the
...
from pixel_cli._subprocess_compat import bounded_git_probe

logger = logging.getLogger("pixel-agents.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "pixel-agents", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _PIXEL_AGENTS_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``PIXEL_AGENTS_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Pixel Agents should operate in its coding posture right now."""
