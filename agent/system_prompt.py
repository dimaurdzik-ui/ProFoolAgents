...
upstream prefix cache warm.  See ``pixel-agents-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, pixel subscription block, tool-use
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_pixel_subscription_prompt``,
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``PIXEL_AGENTS_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("PIXEL_AGENTS_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Pixel Agents never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want PIXEL_AGENTS_HOME persona while keeping
    ...
    # Pointer to the pixel-agents skill + docs for user questions about Pixel Agents itself.
    stable_parts.append(PIXEL_AGENTS_AGENT_HELP_GUIDANCE)
    ...
    # PIXEL_AGENTS_KANBAN_TASK env var). Normal chat sessions never see
    ...
    pixel_subscription_prompt = _r.build_pixel_subscription_prompt(agent.valid_tool_names)
    if pixel_subscription_prompt:
        stable_parts.append(pixel_subscription_prompt)
    ...
    # Active-profile hint — names the Pixel Agents profile the agent is running
    # under so it doesn't conflate ~/.pixel-agents/skills/ (default profile) with
    # ~/.pixel-agents/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        post_workspace_parts.append(
            ...
            "under " + str(get_pixel_agents_home()) + "/profiles/<name>/. Each profile has its own "
        ...
        post_workspace_parts.append(
            f"Active Pixel Agents profile: {active_profile}. This session reads "
            f"and writes {get_pixel_agents_home()}/profiles/{active_profile}/. The default "
            f"profile's data lives at {get_pixel_agents_home()}/skills/, {get_pixel_agents_home()}/plugins/, "
            f"{get_pixel_agents_home()}/cron/, {get_pixel_agents_home()}/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Pixel Agents). Every other surface (desktop chat panel,
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Pixel Agents never rebuilds or reinjects parts of it
