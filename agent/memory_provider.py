...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `pixel-agents backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - pixel_home (str): The active PIXEL_AGENTS_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.pixel-agents``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "pixel-agents").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'pixel-agents memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], pixel_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'pixel-agents memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``pixel_home`` is the active PIXEL_AGENTS_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE PIXEL_AGENTS_HOME.

        ``pixel-agents backup`` only walks PIXEL_AGENTS_HOME, so any provider state kept
        ...
        of the archive, and ``pixel-agents import`` restores them to their original
