...
from pixel_constants import get_pixel_agents_home, get_skills_dir, is_wsl
...
_PIXEL_AGENTS_MD_NAMES = (".pixel-agents.md", "PIXEL_AGENTS.md")


def _find_pixel_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.pixel-agents.md`` or ``PIXEL_AGENTS.md``.
    ...
    # could pick up a .pixel-agents.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _PIXEL_AGENTS_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Pixel Agents, an intelligent AI assistant created by Pixel Agents. "
...
PIXEL_AGENTS_AGENT_HELP_GUIDANCE = (
    "You run on Pixel Agents (by Pixel Agents). When the user needs help with "
    "Pixel Agents itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://api.pixelagents.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `pixel-agents` skill with skill_view(name='pixel-agents') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.pixel-agents/kanban.db`. Your task id is in "
    "`$PIXEL_AGENTS_KANBAN_TASK`; your workspace is `$PIXEL_AGENTS_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $PIXEL_AGENTS_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $PIXEL_AGENTS_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${PIXEL_AGENTS_KANBAN_BRANCH:-wt/$PIXEL_AGENTS_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$PIXEL_AGENTS_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`pixel-agents profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `pixel-agents kanban <verb>` for board operations. Use "
...
#
# The pixel-agents runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to pixel-agents's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "when Pixel Agents has none; explicit Pixel Agents YOLO uses a private unrestricted "
        ...
        "run `pixel-agents computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Pixel Agents "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Pixel Agents terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Pixel Agents desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Pixel Agents WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Pixel Agents itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Pixel Agents runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Pixel Agents itself is running. The host OS, home, and cwd "
                f"of the Pixel Agents process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Pixel Agents "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Pixel Agents
    ...
    extra = (os.getenv("PIXEL_AGENTS_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
# its own context, collects its own warnings, and drains them synchropixelly.
...
def _skills_prompt_snapshot_path() -> Path:
    return get_pixel_agents_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("PIXEL_AGENTS_PLATFORM") or os.environ.get("PIXEL_AGENTS_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.pixel-agents/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Pixel Agents itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `pixel-agents` skill "
            "first. It has the actual commands (e.g. `pixel-agents config set …`, `pixel-agents tools`, "
            "`pixel-agents setup`) so you don't have to guess or invent workarounds.\n"
...
def build_pixel_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact Pixel subscription capability block for the system prompt."""
    try:
        ...
        logger.debug("Failed to import Pixel subscription helper: %s", exc)
        return ""

    if not managed_pixel_tools_enabled():
        return ""
    ...
    features = get_pixel_subscription_features()

    def _status_line(feature) -> str:
        if feature.managed_by_pixel:
            return f"- {feature.label}: active via Pixel subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.pixel_auth_present:
            return f"- {feature.label}: included with Pixel subscription, not currently selected"
        if feature.key == "modal" and features.pixel_auth_present:
            return f"- {feature.label}: optional via Pixel subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# Pixel Subscription",
        "Pixel subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, OpenAI Whisper STT, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a Pixel-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, OpenAI Whisper, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that Pixel subscription would unlock or simplify, suggest Pixel subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: pixel-agents setup, pixel-agents setup tools, pixel-agents setup terminal, pixel-agents status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from PIXEL_AGENTS_HOME and return its content, or None.
    ...
    """
    try:
        from pixel_cli.config import ensure_pixel_home
        ensure_pixel_home()
    except Exception as e:
        logger.debug("Could not ensure PIXEL_AGENTS_HOME before loading SOUL.md: %s", e)

    soul_path = get_pixel_agents_home() / "SOUL.md"
...
def _load_pixel_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.pixel-agents.md / PIXEL_AGENTS.md — walk to git root."""
    pixel_md_path = _find_pixel_md(cwd_path)
    if not pixel_md_path:
        return ""
    try:
        content = pixel_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = pixel_md_path.name
        try:
            rel = str(pixel_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".pixel-agents.md", context_length=context_length,
            read_path=str(pixel_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", pixel_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .pixel-agents.md / PIXEL_AGENTS.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from PIXEL_AGENTS_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Pixel Agents install/source
    ...
    # explicitly configured cwd is honored verbatim — the Pixel Agents tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Pixel Agents in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Pixel Agents install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_pixel_md(cwd_path, context_length)
    ...
    # SOUL.md from PIXEL_AGENTS_HOME only — skip when already loaded as identity
    if not skip_soul:
