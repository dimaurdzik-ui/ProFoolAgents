...
def _pixel_home_path() -> Path:
    """Resolve the active PIXEL_AGENTS_HOME (profile-aware) without circular imports."""
    try:
        from pixel_constants import get_pixel_agents_home  # local import to avoid cycles
        return get_pixel_agents_home()
    except Exception:
        return Path(os.path.expanduser("~/.pixel-agents"))


def _pixel_root_path() -> Path:
    """Resolve the Pixel Agents root dir (always the parent of any profile, never per-profile)."""
    try:
        from pixel_constants import get_default_pixel_agents_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    pixel_home = _pixel_home_path()
    pixel_root = _pixel_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(pixel_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(pixel_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(pixel_home / "cache" / "bws_cache.enc.json"),
            str(pixel_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved PIXEL_AGENTS_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("PIXEL_AGENTS_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    pixel_dirs = []
    for base in (_pixel_home_path(), _pixel_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in pixel_dirs:
                pixel_dirs.append(real)
        except Exception:
            continue

    for base_real in pixel_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside PIXEL_AGENTS_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Pixel Agents path.

    Three categories are blocked:

      * Internal Pixel Agents cache files under ``PIXEL_AGENTS_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under PIXEL_AGENTS_HOME and the global Pixel Agents
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.pixel-agents/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active PIXEL_AGENTS_HOME (profile-aware) AND the global
    # Pixel Agents root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (PIXEL_AGENTS_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    pixel_dirs: list[Path] = []
    for base in (_pixel_home_path(), _pixel_root_path()):
        try:
            real = base.resolve()
            if real not in pixel_dirs:
    ...
    for hd in pixel_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Pixel Agents cache file "
                "and cannot be read directly to prevent prompt injection. "
                "Use the skills_list or skill_view tools instead."
            )

    # Credential / secret stores. Exact-file matches under either
    # PIXEL_AGENTS_HOME or <root>.
    ...
    for hd in pixel_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Pixel Agents credential store "
    ...
    for hd in pixel_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Pixel Agents MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Pixel Agents MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Pixel Agents read (see
...
#
# Pixel Agents profiles are separate PIXEL_AGENTS_HOME dirs under
...
#
# Reference: May 2026 incident where a pixel-agents-security profile session
# edited skills under both ``~/.pixel-agents/profiles/pixel-agents-security/skills/``
# AND ``~/.pixel-agents/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under PIXEL_AGENTS_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from PIXEL_AGENTS_HOME.

    ``~/.pixel-agents``              -> ``"default"``
    ``~/.pixel-agents/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _pixel_home_path().resolve()
        root_real = _pixel_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Pixel Agents scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Pixel Agents entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Pixel Agents profile {info['target_profile']!r}, but the "
...
#
#   <PIXEL_AGENTS_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.pixel-agents/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.pixel-agents/…`` segment and warns
# regardless of which Pixel Agents profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.pixel-agents/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.pixel-agents`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.pixel-agents/…`` and returns the
    index where the inner Pixel Agents-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .pixel-agents / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".pixel-agents":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Pixel Agents state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.pixel-agents``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.pixel-agents`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Pixel Agents resolver to
    succeed, so it works correctly even when called from contexts where
    PIXEL_AGENTS_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Pixel Agents process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real PIXEL_AGENTS_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.pixel-agents/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.pixel-agents/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host PIXEL_AGENTS_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Pixel Agents state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Pixel Agents "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real PIXEL_AGENTS_HOME. Use the "
