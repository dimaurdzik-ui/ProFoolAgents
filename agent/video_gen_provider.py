...
as ``kind: backend``) or ``~/.pixel-agents/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``pixel-agents tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$PIXEL_AGENTS_HOME/cache/videos/``, creating parents as needed."""
    from pixel_constants import get_pixel_agents_home

    path = get_pixel_agents_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$PIXEL_AGENTS_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$PIXEL_AGENTS_HOME/cache/videos/``.
