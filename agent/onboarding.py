...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Pixel Agents starts and finds ``~/.openclaw/``.

    Points users at ``pixel-agents claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``pixel-agents claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Pixel Agents, run "
        "`pixel-agents claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `pixel-agents claw cleanup` (renames it to ~/.openclaw.pre-migration — "
