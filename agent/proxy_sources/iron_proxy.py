...
* The ``iron-proxy`` binary is auto-installed into ``<pixel_home>/bin/iron-proxy``
  on first use.  Pixel Agents pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<pixel_home>/proxy/ca.{crt,key}`` is generated on
  first ``pixel-agents egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<pixel_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``pixel-agents egress start``), pidfile
  at ``<pixel_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<pixel_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``pixel-agents egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<pixel_home>/proxy/management.token`` (0600), and injected into the
...
_MGMT_API_KEY_ENV = "PIXEL_AGENTS_IRON_PROXY_MGMT_KEY"
...
_BEARER_PROVIDERS: Dict[str, Tuple[str, ...]] = {
    ...
    "PIXEL_API_KEY": ("inference.pixelagents.com",),
}
...
# credential (Pixel Agents' auth.py keys Google on both GEMINI_API_KEY and
...
def _pixel_bin_dir() -> Path:
    from pixel_constants import get_pixel_agents_home

    return get_pixel_agents_home() / "bin"
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.pixel-agents/proxy/`` just to
    check whether a pidfile exists.
    """
    from pixel_constants import get_pixel_agents_home

    return get_pixel_agents_home() / "proxy"
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<pixel_home>/bin/iron-proxy``  (our managed copy — preferred)
    ...
    managed = _pixel_bin_dir() / _platform_binary_name()
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``pixel-agents proxy install`` surface lets them
    propagate so the wizard can show a clear error.
    """

    bin_dir = _pixel_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="pixel-agents-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "pixel-agents"})
    try:
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``pixel-agents egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="pixel-agents-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
        ...
        subprocess.run(  # noqa: S603
            [
                ...
                "-key", str(tmp_key),
                "-sha256", "-days", "3650",
                "-subj", "/CN=pixel-agents iron-proxy CA",
...
def mint_proxy_token(prefix: str = "pixel-agents-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<pixel_home>/proxy/management.token`` with 0600 perms.
    The daemon receives it via the ``PIXEL_AGENTS_IRON_PROXY_MGMT_KEY`` env var
    (named in the generated config's ``management.api_key_env``);
    ``pixel-agents egress reload`` reads the same file to authenticate.
    """
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="pixel-agents-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `pixel-agents egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `pixel-agents egress setup` and use "
            "`pixel-agents egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `pixel-agents egress setup`, "
            "then `pixel-agents egress restart`."
        )
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "run `pixel-agents egress restart`."
            ) from exc
        raise RuntimeError(
        ...
        raise RuntimeError(
            ...
            "`pixel-agents egress restart` once."
        ) from exc
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``pixel-agents egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `pixel-agents egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<pixel_home>/proxy/proxy.yaml``.

    Uses ``yaml.safe_dump`` so we never emit Python tags.
    """

    try:
        import yaml  # PyYAML is already a Pixel Agents dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``pixel-agents egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``pixel-agents egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_PIXEL_AGENTS_IRON_PROXY_NONCE_ENV = "PIXEL_AGENTS_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_PIXEL_AGENTS_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `pixel-agents egress install`."
        )
    ...
    if not cfg.exists():
        raise RuntimeError(
            ...
            "Run `pixel-agents egress setup` first."
        )
    ...
    # Pixel Agents process.
    _proxy_nonce = hashlib.sha256(os.urandom(16)).hexdigest()
    env[_PIXEL_AGENTS_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # close-on-success path, every restart leaked one fd in the Pixel Agents
    # process.
    #
    ...
    # ``pixel-agents egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``pixel-agents egress start`` doesn't leak
    ...
    try:
        ...
        # grace window is 0 (test harness / synchropixel fast-path).
        while True:
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `pixel-agents egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`pixel-agents egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `pixel-agents secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
