...
supports multiple backends — **Browser Use** (cloud, default for Pixel
...
from pixel_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Pixel Agents secret
...
def _build_browser_env() -> dict:
    ...
    Strips Pixel Agents-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``pixel_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = pixel_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    pixel_node_bin = str(pixel_home / "node" / "bin")
    pixel_node_root = str(pixel_home / "node")
    pixel_nm_bin = str(pixel_home / "node_modules" / ".bin")
    return [pixel_node_bin, pixel_node_root, pixel_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``pixel-agents doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.pixel-agents/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed Pixel gateway or
    ...
    browser plugins (``~/.pixel-agents/plugins/browser/<vendor>/``) participate
    ...
    if resolved is None:
        # Auto-detect path: Browser Use first (managed Pixel gateway or
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Pixel Agents silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Pixel Agents CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-pixel_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed pixel-agents processes — this way every clean pixel-agents exit sweeps
    ...
    # Sweep orphans from other crashed pixel-agents processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live pixel-agents processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current pixel-agents PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live pixel-agents process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning pixel-agents process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced pixel-agents PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent pixel-agents instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-pixel_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live pixel-agents process.
            continue
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Pixel Agents-managed
    ...
    # node_modules binary, which disappears on the next ``pixel-agents update`` and
    ...
    # Build an extended search PATH including Pixel Agents-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_pixel_agents_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_pixel_agents_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_pixel_agents_home() / "node")),
            ]
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this pixel-agents PID as the session owner (cross-process safe
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_pixel_agents_dir("cache/web", "web_cache")
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = pixel_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Pixel Agents
    ...
    from pixel_constants import get_pixel_agents_dir
    screenshots_dir = get_pixel_agents_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/pixelagents/pixel-agents:latest")
                else:
