...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Pixel Agents exit
...
# display_pixel_agents_home imported lazily at call site (stale-module safety during pixel-agents update)
...
from tools.tool_backend_helpers import (
    ...
    pixel_tool_gateway_unavailable_message,
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of pixel-agents directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "pixel-agents-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("PIXEL_AGENTS_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("PIXEL_AGENTS_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("PIXEL_AGENTS_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (PIXEL_AGENTS_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (PIXEL_AGENTS_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive Pixel Agents password prompt or the sudo -S password-pipe path.
    ...
    # silently without Pixel Agents prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    has_sudo_prompt_callback = _get_sudo_password_callback() is not None
    should_prompt_for_sudo = (
        env_var_enabled("PIXEL_AGENTS_INTERACTIVE") or has_sudo_prompt_callback
    )
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Pixel Agents can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``pixel-agents=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Pixel Agents processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Pixel Agents processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Pixel Agents processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, PixelAgentsSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.pixel-agents/.env or environment variables."
        )
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    expand itself, so the Pixel Agents host/container must NOT ``expanduser`` it.

    SSH ``cwd`` is interpreted by the *remote* shell (``cd ~`` / ``cd ~/x``
    over ``ssh ... bash -c``). Expanding ``~`` locally would rewrite it to the
    Pixel Agents host HOME (often ``/opt/data`` under Docker) and inject a
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``pixel-agents
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for pixel-agents-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Pixel Agents processes that hit SIGKILL / OOM / a closed terminal
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Pixel Tool Gateway access is not currently available and no direct "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Pixel Agents can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/pixel-agents
        # restart|stop targeting pixel-agents-gateway) must never run inside the
        ...
        # never restart. This mirrors the `pixel-agents gateway restart` guard in
        # pixel_cli/gateway.py and the cron-path guard in pixel_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_PIXEL_AGENTS_GATEWAY") == "1":
            from pixel_cli.cron import _contains_gateway_lifecycle_command
            if _contains_gateway_lifecycle_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Run `pixel-agents gateway restart` from a separate shell outside "
                        "the running gateway."
                    ),
                    "status": "error",
                }, ensure_ascii=False)
        ...
        if background:
            ...
            try:
                ...
                # pixel-agents dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in pixel-agents dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `pixel-agents -z`, a "
                        ...
                        _gw_platform = _gse("PIXEL_AGENTS_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("PIXEL_AGENTS_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("PIXEL_AGENTS_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("PIXEL_AGENTS_SESSION_USER_ID", "")
                            _gw_user_name = _gse("PIXEL_AGENTS_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("PIXEL_AGENTS_SESSION_MESSAGE_ID", "")
                ...
                # asynchropixelly and can spam the user long after the process ends.
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("PIXEL_AGENTS_INTERACTIVE"):
                    output += (
