...
Lets the LLM write a Python script that calls Pixel Agents tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `pixel_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `pixel_tools.py` with file-based RPC stubs
...
# available on every platform Pixel Agents itself runs on.
...
# match a safe prefix, the operational PIXEL_AGENTS_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Pixel Agents
# code can lose the DB-layer Kanban mutation guard while still inheriting
# PIXEL_AGENTS_HOME.
#
# NB: the broad "PIXEL_AGENTS_" prefix was deliberately removed (#27303) — it leaked
# PIXEL_AGENTS_*-named config that lacks a secret substring (e.g. PIXEL_AGENTS_BASE_URL,
# PIXEL_AGENTS_KANBAN_DB, PIXEL_AGENTS_*_WEBHOOK).  The child only needs the few
# location/profile vars in _PIXEL_AGENTS_CHILD_ALLOWED below; PIXEL_AGENTS_RPC_SOCKET /
# PIXEL_AGENTS_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational PIXEL_AGENTS_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set pixel_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_PIXEL_AGENTS_CHILD_ALLOWED = frozenset({
    "PIXEL_AGENTS_HOME",
    "PIXEL_AGENTS_PROFILE",
    "PIXEL_AGENTS_CONFIG",
    "PIXEL_AGENTS_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational PIXEL_AGENTS_* vars (_PIXEL_AGENTS_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret PIXEL_AGENTS_* vars dropped by the tightened allowlist (#27303). The
    # broad "PIXEL_AGENTS_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like PIXEL_AGENTS_KANBAN_DB / PIXEL_AGENTS_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _PIXEL_AGENTS_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("PIXEL_AGENTS_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped PIXEL_AGENTS_* var.
            _dropped_pixel.append(k)
    if _dropped_pixel:
        logger.debug(
            "execute_code: dropped %d non-allowlisted PIXEL_AGENTS_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Pixel Agents-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Pixel Agents-profile soft guard."""',
...
def generate_pixel_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the pixel_tools.py stub module.
...
"""Auto-generated Pixel Agents tools RPC stubs."""
...
def _connect():
    ...
    PIXEL_AGENTS_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["PIXEL_AGENTS_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("PIXEL_AGENTS_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Pixel Agents tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("PIXEL_AGENTS_RPC_DIR") or os.path.join(tempfile.gettempdir(), "pixel_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated pixel_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/pixel_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_pixel_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/pixel_tools.py", tools_src)
        ...
        env_prefix = (
            f"PIXEL_AGENTS_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PIXEL_AGENTS_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("PIXEL_AGENTS_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    to a subset of Pixel Agents tools.
    ...
    # here before either dispatch path spawns it. Runs synchropixelly in the
    ...
    # --- Set up temp directory with pixel_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="pixel_sandbox_")
    ...
    # selector from PIXEL_AGENTS_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"pixel_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated pixel_tools module.
        ...
        tools_src = generate_pixel_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "pixel_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  PIXEL_AGENTS_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["PIXEL_AGENTS_RPC_SOCKET"] = rpc_endpoint
        child_env["PIXEL_AGENTS_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the pixel-agents root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from pixel_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _pixel_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _pixel_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # PIXEL_AGENTS_TIMEZONE is an internal Pixel Agents setting and must not leak
        # into child processes.
        _tz_name = os.getenv("PIXEL_AGENTS_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("PIXEL_AGENTS_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.pixel-agents/.env')).
...
def _load_config() -> dict:
    ...
    ``pixel-agents --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (pixel-agents's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``pixel-agents tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and pixel-agents's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.pixel-agents/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Pixel Agents tools programmatically. "
        ...
        f"Available via `from pixel_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into pixel_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from pixel_tools import {import_str}` "
