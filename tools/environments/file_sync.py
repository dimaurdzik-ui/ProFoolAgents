...
_FORCE_SYNC_ENV = "PIXEL_AGENTS_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.pixel-agents") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.pixel-agents to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.pixel-agents", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``PIXEL_AGENTS_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, pixel_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.pixel-agents/`` directory as a tar archive,
        ...
        # against an uninitialized remote .pixel-agents/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (pixel_home or get_pixel_agents_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="pixel-agents-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.pixel-agents/skills/a.md`` →
        ``~/.pixel-agents/skills/a.md``, a new remote file at
        ``/root/.pixel-agents/skills/b.md`` maps to ``~/.pixel-agents/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
