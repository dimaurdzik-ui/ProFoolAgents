...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.pixel-agents",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``PIXEL_AGENTS_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside PIXEL_AGENTS_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.

    Containment alone is not sufficient, because PIXEL_AGENTS_HOME is exactly where
    ...
    """
    pixel_home = _resolve_pixel_home()

    # Reject absolute paths — they bypass the PIXEL_AGENTS_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to PIXEL_AGENTS_HOME)",
            relative_path,
        )
        return False

    host_path = pixel_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape PIXEL_AGENTS_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, pixel_home)
    ...
    # inside PIXEL_AGENTS_HOME and therefore pass the containment check above.
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.pixel-agents/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="pixel-agents-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching pixel_constants.get_pixel_agents_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_pixel_agents_dir()`` for backward compatibility with old directory layouts.
    """
    from pixel_constants import get_pixel_agents_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_pixel_agents_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.pixel-agents``,
    SSH ``<remote_home>/.pixel-agents``, etc.) and whether translation is wanted.
