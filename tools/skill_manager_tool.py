...
~/.pixel-agents/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `pixel-agents config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.pixel-agents/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch PIXEL_AGENTS_HOME and
    ...
    live profile-scoped PIXEL_AGENTS_HOME on every call.
    """
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`pixel-agents curator unpin {name}` if they want to delete it. "
...
def _background_review_write_guard(
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`pixel-agents curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
    ...
    try:
        ...
        # shapes; `pixel-agents curator adopt <name>` is the supported way in.
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `pixel-agents curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.pixel-agents/skills/) first, then any
    ...
    try:
        ...
        if not ssc.sync_org_auto_propose():
            return (
                ...
                f"Run `pixel-agents sync propose {name}` to share it back."
            )
        ...
        return (
            ...
            f"right now — run `pixel-agents sync propose {name}` to retry."
        )
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `pixel-agents sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `pixel-agents sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Pixel Agents profiles.
    ...
    try:
        root = get_default_pixel_agents_root()
    except Exception:
        return matches
    ...
    # Default profile (~/.pixel-agents/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.pixel-agents/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`pixel-agents -p "
            ...
            base += (
                ...
                f"Switch profiles (`pixel-agents -p <name>`) to edit there, or "
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.pixel-agents/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `pixel-agents curator restore` promises the skill can be brought back. Route
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `pixel-agents curator
        ...
        # Inert unless the access gate is open (the user is a Pixel admin on the
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_pixel_agents_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `pixel-agents curator unpin <name>`. "
