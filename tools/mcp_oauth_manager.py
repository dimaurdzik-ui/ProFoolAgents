...
`mcp_tool.py`, and `pixel_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# PixelAgentsMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_pixel_provider_class() -> Optional[type]:
    ...
    class PixelAgentsMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._pixel_server_name = server_name
            self._pixel_home = ""
            ...
            self._pixel_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`PixelAgentsTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, PixelAgentsTokenStorage)
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            but runs synchropixelly before the first request instead of
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._pixel_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import PixelAgentsTokenStorage
                        if isinstance(storage, PixelAgentsTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._pixel_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import PixelAgentsTokenStorage
            if not isinstance(storage, PixelAgentsTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``pixel-agents mcp reauth``.
            ...
            back to ``pixel-agents mcp reauth``.
            """
            try:
                if self._pixel_preregistered:
                    return
    ...
    return PixelAgentsMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_PIXEL_AGENTS_PROVIDER_CLS: Optional[type] = _make_pixel_provider_class()
...
class MCPOAuthManager:
        ...
        with self._entries_lock:
            ...
            if entry.provider is None:
                ...
                if entry.provider is not None:
                    entry.provider._pixel_home = key[0]
    ...
    def _build_provider(
        ...
        Constructs :class:`PixelAgentsMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _PIXEL_AGENTS_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `pixel-agents mcp login "
        ...
        return _PIXEL_AGENTS_PROVIDER_CLS(
    ...
    def remove(
        ...
        *,
        ...
        Called by ``pixel-agents mcp remove <name>`` and (indirectly) by
        ``pixel-agents mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            entry = self._entries.pop(self._key(server_name, pixel_home), None)

        from tools.mcp_oauth import remove_oauth_tokens
        remove_oauth_tokens(server_name, pixel_home=pixel_home)
        logger.info(
    ...
    def restore_entry(
        ...
        *,
        ...
        if entry is None:
            return
        with self._entries_lock:
            self._entries.setdefault(self._key(server_name, pixel_home), entry)
    ...
    def evict(
        ...
        *,
        ...
        with self._entries_lock:
            self._entries.pop(self._key(server_name, pixel_home), None)
    ...
    async def invalidate_if_disk_changed(
        ...
        *,
        ...
        entry = self._entries.get(self._key(server_name, pixel_home))
        if entry is None or entry.provider is None:
            return False

        async with entry.lock:
            tokens_path = _get_token_dir(pixel_home) / f"{_safe_filename(server_name)}.json"
            try:
