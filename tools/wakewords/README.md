...
`hey_pixel.onnx` / `hey_pixel.tflite` — the on-device "Hey Pixel Agents" hotword
...
required to say "hey pixel-agents".
...
- **Label:** the model registers as `hey_pixel` (matches the filename).
