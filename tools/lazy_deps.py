"""
Lazy dependency installer for opt-in Pixel Agents backends.

Many Pixel Agents features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``pixel-agents[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``pixel-agents tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``PIXEL_AGENTS_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/pixel-agents`` read-only), setting
  ``PIXEL_AGENTS_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Pixel Agents, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # ─── Wake word ("Hey Pixel Agents") engines ──────────────────────────────────
    ...
    # (PIXEL_AGENTS_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # Dashboard (`pixel-agents dashboard`)
    "tool.dashboard": (
        "fastapi==0.133.1",
    ...
    # HF Agent Trace Viewer upload (pixel-agents trace upload / /upload-trace).
    #
    ...
    # presence, the `pixel-agents update` lazy-refresh pass re-asserts THIS pin on
...
_LAZY_TARGET_ENV = "PIXEL_AGENTS_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``PIXEL_AGENTS_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("PIXEL_AGENTS_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``pixel-agents update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Pixel Agents under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``pixel-agents update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="pixel-agents-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``pixel_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = pixel_subprocess_env(inherit_credentials=False)
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``PIXEL_AGENTS_DISABLE_LAZY_INSTALLS=1``) and sets
      ``PIXEL_AGENTS_LAZY_INSTALL_TARGET``, installs are redirected to the writable
    ...
    * **Gated** — honors ``security.allow_lazy_installs`` and refuses to run
    ...
    if not _allow_lazy_installs():
        target = _lazy_install_target()
        if os.environ.get("PIXEL_AGENTS_DISABLE_LAZY_INSTALLS") == "1" and target is None:
            reason = (
                "runtime installs are disabled on this deployment: the agent "
                "environment is immutable and no writable install target is "
                "configured (PIXEL_AGENTS_LAZY_INSTALL_TARGET)"
            )
        else:
...
def active_features() -> list[str]:
    ...
    Used by ``pixel-agents update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``pixel-agents update``. Never raises; lazy-install failures
