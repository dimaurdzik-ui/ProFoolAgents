...
Backend is selected during ``pixel-agents tools`` setup (web.backend in config.yaml).
When available, Pixel Agents can route Firecrawl calls through a Pixel-hosted tool-gateway
for Pixel Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for Pixel Subscribers)
...
from tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    ...
    peek_pixel_access_token as _peek_pixel_access_token,
    read_pixel_access_token as _read_pixel_access_token,
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Pixel Agents config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Pixel Agents' config/.env layer (``pixel-agents config set``, ``pixel-agents tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.pixel-agents/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``pixel-agents tools``).
    ...
    # pre-empted by a Pixel OAuth token whose subscription tier may not
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `pixel-agents tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_pixel_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchropixel HTTP
    refresh against the Pixel portal on every CLI startup (invoked at
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `pixel-agents plugins enable {disabled_key}` "
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `pixel-agents tools` to set one up."
                    ),
                }
        else:
            logger.info(
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`pixel-agents plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from pixel_cli.config import get_env_value as _gev
    ...
    print("")
    print("  # Search (synchropixel)")
    print("  results = web_search_tool('Python tutorials')")
    print("")
    print("  # Extract (asynchropixel, no LLM — truncate-and-store)")
