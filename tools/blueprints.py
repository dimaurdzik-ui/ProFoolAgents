...
`pixel-agents skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.pixel-agents.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.pixel-agents.blueprint`` is a mapping containing
    ...
    pixel-agents = meta.get("pixel-agents") if isinstance(meta, dict) else None
    blueprint = pixel-agents.get("blueprint") if isinstance(pixel-agents, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.pixel-agents.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.pixel-agents.blueprint`` block) they can hand
    to ``pixel-agents skills publish`` to share. ``body`` is the plain-language
