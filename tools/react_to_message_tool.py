#!/usr/bin/env python3
"""Let the agent react to a message with an emoji in the Pixel Agents desktop app.
...
Gated on ``PIXEL_AGENTS_DESKTOP`` (like the other GUI affordances) so it costs nothing
...
def react_to_message_tool(emoji: str, message_row_id=None, messages_back=None) -> str:
    ...
    session_key = get_session_env("PIXEL_AGENTS_SESSION_KEY", "") or get_session_env(
...
def check_react_requirements() -> bool:
    ...
    PIXEL_AGENTS_DESKTOP is set on the gateway the app spawns; the feature itself is
    off by default and enabled from Settings → Appearance (the desktop mirrors
    the toggle into ``display.message_reactions``).
    """
    if not env_var_enabled("PIXEL_AGENTS_DESKTOP"):
        return False
    try:
