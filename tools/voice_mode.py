...
def _voice_capture_install_hint() -> str:
    ...
    # If we're running inside a venv (e.g. the bundled Pixel Agents venv at
    # ~/.pixel-agents/profiles/<name>/pixel-agents/venv/), `pip install` on the
...
_TEMP_DIR = os.path.join(tempfile.gettempdir(), "pixel_voice")
...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.pixel-agents/config.yaml during a
...
class AudioRecorder:
    ...
    def _max_duration_reached(self, elapsed: float) -> bool:
        ...
        recording (see ``PixelAgentsCLI._voice_start_recording``). A value <= 0
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # A unique suffix prevents concurrent Pixel Agents TTS calls from colliding on
        ...
        if _is_wsl2_env():
            try:
                ...
                if _win_tmp_wsl:
                    ...
                    _wsl_wav = os.path.join(_win_tmp_wsl, f"pixel-agents-tts-{_unique}.wav")
...
def _voice_debug_enabled() -> bool:
    return os.environ.get("PIXEL_AGENTS_VOICE_DEBUG", "").strip() == "1"


def _vad_log(msg: str) -> None:
    """VAD decision-point diagnostic — always logger.debug, plus stderr when
    PIXEL_AGENTS_VOICE_DEBUG=1 so live hardware tuning doesn't need a log tail."""
    logger.debug(msg)
