...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/pixel-agents-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/pixel-agents-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/pixel-agents-results"
HEREDOC_MARKER = "PIXEL_AGENTS_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/pixel-agents-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"PIXEL_AGENTS_PERSIST_{uuid.uuid4().hex[:8]}"
