#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Pixel Agents Skills Hub.
...
Used by pixel_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _pixel_home() -> Path:
    return get_pixel_agents_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _pixel_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            pixel_meta = metadata.get("pixel-agents", {})
            if isinstance(pixel_meta, dict):
                tags = pixel_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Pixel Agents) but not activated
    ...
    OFFICIAL_REPO = "PixelResearch/pixel-agents"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                pixel_meta = meta_block.get("pixel-agents", {})
                if isinstance(pixel_meta, dict):
                    tags = pixel_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Pixel Agents centralized index source
# ---------------------------------------------------------------------------

PIXEL_AGENTS_INDEX_URL = "https://api.pixelagents.com/docs/api/skills-index.json"
PIXEL_AGENTS_INDEX_TTL = 6 * 3600  # 6 hours


def _pixel_index_cache_file() -> Path:
    return _index_cache_dir() / "pixel-agents-index.json"


def _load_pixel_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for PIXEL_AGENTS_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    pixel_index_cache_file = _pixel_index_cache_file()
    if pixel_index_cache_file.exists():
        try:
            age = time.time() - pixel_index_cache_file.stat().st_mtime
            if age < PIXEL_AGENTS_INDEX_TTL:
                return json.loads(pixel_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Pixel Agents index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Pixel Agents index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Pixel Agents index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        pixel_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        pixel_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class PixelAgentsIndexSource(SkillSource):
    """Skill source backed by the centralized Pixel Agents Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "pixel-agents-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "pixel-agents-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        PixelAgentsIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "pixel-agents-index"
