...
409), and is driven by:
  ...
  * the ``pixel-agents sync status|pull|push|now`` CLI.

It lives beside ``tools/skills_sync.py`` (NOT under ``pixel_cli/``) so the
...
**Pixel admin**. We read that off the access token, which rides on the same
bearer ``resolve_pixel_runtime_credentials()`` returns; we decode the JWT
...
status conflates "may administer Pixel" with "has Skill Sync enabled", and has
...
is toggled via ``pixel-agents sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.pixel-agents/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``pixel-agents sync enable`` — it is
...
# namespace from pixel-agents's local ``content_hash`` (skills_guard.py:846),
...
#
# We reuse resolve_pixel_runtime_credentials() for the bearer (it honors the
...
def _decode_jwt_payload_unverified(token: str) -> Dict[str, Any]:
    ...
    plugins/dashboard_auth/pixel/__init__.py:463.
    """
    try:
...
def resolve_identity() -> Dict[str, Any]:
    """Resolve the Pixel bearer + owner + dev-gate flag.

    Returns a dict: ``{api_key, base_url, owner, pixel_admin, claims}``.
    ...
    try:
        ...
        raise SyncInertError(f"no Pixel credentials: {e}") from e
    ...
    pixel_admin = claims.get(PIXEL_ADMIN_CLAIM) is True
    return {
        "api_key": api_key,
        "base_url": (creds or {}).get("base_url"),
        "owner": str(owner),
        "pixel_admin": pixel_admin,
        "claims": claims,
    }
...
def dev_gate_open() -> bool:
    ...
    try:
        return bool(resolve_identity().get("pixel_admin"))
...
# sync.base_url (or the PIXEL_AGENTS_SYNC_BASE_URL bridge env) overrides it to point
...
DEFAULT_SYNC_BASE_URL = "https://gateway-gateway.pixelagents.com"

def resolve_sync_base_url() -> Optional[str]:
    """Resolve the sync-plane base URL.

    Order: PIXEL_AGENTS_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("PIXEL_AGENTS_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Pixel Agents Cloud instance can be set
...
# precedence as base_url: the PIXEL_AGENTS_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   PIXEL_AGENTS_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   PIXEL_AGENTS_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   PIXEL_AGENTS_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``PIXEL_AGENTS_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a Pixel Agents Cloud deployment sets to opt its instances into sync by
    default. It is checked by the gate-and-swallow entrypoints IN ADDITION to
    the Pixel-admin token gate and a configured base URL — all three must hold for
    background sync to run.
    """
    return _sync_config_bool("PIXEL_AGENTS_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``PIXEL_AGENTS_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``pixel-agents sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "PIXEL_AGENTS_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``PIXEL_AGENTS_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``pixel-agents sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Pixel Agents Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("PIXEL_AGENTS_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.pixel-agents/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.pixel-agents/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Pixel Agents Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``PIXEL_AGENTS_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.pixel-agents/skills/ containing
...
def stable_device_id() -> str:
    ...
    ~/.pixel-agents/skills/.sync_device_id.
    ...
    Use ``set_device_name()`` / ``pixel-agents sync device --name`` to set an explicit
    label."""
    ...
    # Pixel Agents Cloud (and any templated deployment) can seed the label
    # declaratively via PIXEL_AGENTS_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `pixel-agents sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("PIXEL_AGENTS_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.pixel-agents/skills/.sync_device_id, overwriting
...
# the caller's three-way merge. Auth is the Pixel bearer resolved above.
# ---------------------------------------------------------------------------
...
# ~/.pixel-agents/skills/.sync_state as JSON.
#
...
# path under ~/.pixel-agents/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.pixel-agents/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Pixel Agents speaks "
            f"{WIRE_VERSION} — update Pixel Agents to sync with it"
        )
...
def push_skills(
    ...
    *,
    ...
    message: str = "pixel-agents skill sync",
) -> Dict[str, Any]:
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (pixel-agents sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.pixel-agents/skills/. Only paths the user has opted into (``sync: true``) are
...
# (no push, no pull, no-op) unless the signed-in user is a Pixel admin.
# ---------------------------------------------------------------------------

def maybe_push_skills(*, message: str = "pixel-agents skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not identity.get("pixel_admin"):
            return None  # access gate: inert unless the user is a Pixel admin
        if not sync_feature_enabled():
            return None  # feature off for this instance (PIXEL_AGENTS_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``pixel-agents sync status``. Never raises."""
    status: Dict[str, Any] = {
        "pixel_admin": False,
        "logged_in": False,
    ...
    try:
        ...
        status["pixel_admin"] = bool(identity.get("pixel_admin"))
    except SyncInertError:
        pass
    except Exception as e:
...
#
# Org skills live under a DISTINCT local namespace, ~/.pixel-agents/skills/_org/
...
#
# `pixel-agents sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.pixel-agents/skills/_org/<org_id>/``.
