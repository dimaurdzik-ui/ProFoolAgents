...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.pixel-agents/config.yaml``. Pixel Agents
...
Configuration is loaded from ~/.pixel-agents/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``pixel_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from pixel_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``pixel-agents auth add <provider_id>``).
...
from tools.xai_http import pixel_xai_user_agent
...
# The managed OpenAI audio gateway (Pixel portal proxy) only proxies these speech
...
# Base URL now resolved via pixel_cli.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_pixel_agents_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.pixel-agents/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.pixel-agents/config.yaml.
    ...
    try:
        ...
        logger.debug("pixel_cli.config not available, using default TTS config")
        return {}
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``pixel-agents tools``); otherwise the historical Edge backend remains active.
    """
...
# custom voice-cloning scripts, etc.) into Pixel Agents without any Python code
...
#           output_format: wav
#
# Pixel Agents writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__PIXEL_AGENTS_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Pixel Agents secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Pixel Agents secrets (salvage of #56332) while still
    ...
    scrubbed = pixel_subprocess_env(inherit_credentials=False)
...
# file, so retired models disappear from pixel-agents the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``pixel_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `pixel-agents setup` to configure, "
    ...
    from pixel_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `pixel-agents model` or set XAI_API_KEY.")
    ...
    # output_format when Pixel Agents actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": pixel_xai_user_agent(),
        },
        json=payload,
        timeout=60,
        stream=True,
...
def _resolve_gemini_persona_prompt_path(gemini_config: Dict[str, Any]) -> Optional[Path]:
    ...
    if not path.is_absolute():
        try:
            ...
            path = get_pixel_agents_home() / path
        except Exception:
            path = Path.cwd() / path
    return path
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import pixel_cli as _pixel_cli

            _pixel_version = str(_pixel_cli.__version__)
        except Exception:
            _pixel_version = "0.0.0"
        # Include Pixel Agents client context following Gemini's partner
        # integration guidance:
        # https://ai.google.dev/gemini-api/docs/partner-integration
        headers["X-Goog-Api-Client"] = f"pixel-agents/{_pixel_version}"
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Pixel Agents caches Piper voice models.

    Resolves to ``~/.pixel-agents/cache/piper-voices/`` under the active
    PIXEL_AGENTS_HOME so voice downloads follow profile boundaries.
    """
    from pixel_constants import get_pixel_agents_dir
    root = Path(get_pixel_agents_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.pixel-agents/config.yaml (tts: section).
    ...
    platform = get_session_env("PIXEL_AGENTS_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `pixel-agents setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run pixel-agents setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'pixel-agents setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'pixel-agents tools' and select Piper under TTS, or install manually: "
...
def _resolve_openai_audio_client_config() -> tuple[str, str, bool]:
    ...
    ``is_managed`` is True when the config resolves to the Pixel managed audio
    ...
    if managed_gateway is None:
        ...
        if managed_pixel_tools_enabled() or prefers_gateway("tts"):
            message += (
                ". "
                + pixel_tool_gateway_unavailable_message(
    ...
    return (
        managed_gateway.pixel_user_token,
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_pixel_agents_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
