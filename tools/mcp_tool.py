...
transport, discovers their tools, and registers them into the pixel-agents
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.pixel-agents/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.pixel-agents/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Pixel Agents' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Pixel Agents explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Pixel Agents and
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            pixel_home = os.path.expanduser(
                os.getenv(
                    "PIXEL_AGENTS_HOME", os.path.join(os.path.expanduser("~"), ".pixel-agents")
                )
            )
            candidates = [
                os.path.join(pixel_home, "node", "bin", resolved_command),
                ...
                # image (which the Pixel Agents Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Pixel Agents MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Pixel Agents gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Pixel Agents document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`pixel_cli.mcp_config._unwrap_exception_group` with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Pixel Agents' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (PIXEL_AGENTS_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit PIXEL_AGENTS_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Pixel Agents' logging (agent.log via pixel_logging), tagged
                ...
                if _MCP_NOTIFICATION_TYPES and isinstance(message, ServerNotification):
                    match message.root:
                        case ToolListChangedNotification():
                            ...
                            # request. Refreshing synchropixelly inside the SDK
    ...
    async def _refresh_tools(self):
        ...
        After the initial ``await`` (list_tools), all mutations are synchropixel
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "it is not installed. Run `pixel-agents setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Pixel Agents process (kill -9, crash, force-quit)
        ...
        # ~/.pixel-agents/logs/mcp-stderr.log.
                ...
                if (
                    ...
                    post_resp = await client.post(
                        ...
                        content=(
                            ...
                            '"capabilities":{},'
                            '"clientInfo":{"name":"pixel-agents-probe",'
                            '"version":"0.1"}}}'
                        ),
                    )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Pixel Agents' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Pixel Agents is fully
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `pixel-agents mcp login {server_name}` (or delete the tokens "
        f"file under ~/.pixel-agents/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Pixel Agents processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_pixel_agents_home() / ".mcp-discovery.lock"
            )
...
# can be cleaned up asynchropixelly by _kill_orphaned_mcp_children().
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local PIXEL_AGENTS_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_pixel_agents_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_pixel_agents_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local PIXEL_AGENTS_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_pixel_agents_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Pixel Agents config file.
    ...
    ``os.environ`` (which includes ``~/.pixel-agents/.env`` loaded at startup).
    """
    try:
        from pixel_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("PIXEL_AGENTS_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        if not server.session:
            ...
            # transport swaps in a fresh session object asynchropixelly) —
        ...
        async def _call():
            ...
            # flow through Pixel Agents' MEDIA: tag convention and out to messaging
            ...
            # Pixel Agents' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Pixel Agents's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Pixel Agents uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Pixel Agents registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``pixel-agents tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Pixel Agents registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple pixel-agents
