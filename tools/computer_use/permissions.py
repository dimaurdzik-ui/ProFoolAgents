...
something different on each:
  ...
  * macOS — explicit TCC grants (Accessibility + Screen Recording). cua-driver
    ...
    the installed ``CuaDriver.app``), NOT Pixel Agents — so no Pixel Agents entitlement is
...
``pixel-agents computer-use permissions`` CLI, and ``/api/tools/computer-use/status``.
"""
