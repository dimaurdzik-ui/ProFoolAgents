...
`health_report` (surfaced by `pixel-agents computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Pixel Agents runs in).
_BLOCKED_KEY_COMBOS = {
...
# Per-Pixel Agents-session cached backends. Each backend owns its own cua-driver
...
# See PixelResearch/pixel-agents#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Pixel Agents's explicit approval bypass onto Cua's immutable mode.

    Pixel Agents has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "PIXEL_AGENTS_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown PIXEL_AGENTS_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Pixel Agents YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Pixel Agents sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Pixel Agents process that spawned it
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `pixel-agents computer-use install`. "
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (PixelResearch/pixel-agents#67052).
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    # none of those capabilities can be supplied across Pixel Agents sessions.
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of PixelResearch/pixel-agents#47072 — trycua/cua#1961
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$PIXEL_AGENTS_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_pixel_agents_dir("cache/vision", "temp_vision_images")
...
def check_computer_use_requirements() -> bool:
    ...
    `pixel-agents computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
