...
-----------------------------------------------------------------------------
    ...
    ~/.pixel-agents/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/pixel-agents/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_pixel_agents_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/pixel-agents"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Pixel Agents convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("PIXEL_AGENTS_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal Pixel Agents infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``pixel-agents checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `pixel-agents checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "pixel-agents@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Pixel Agents Checkpoint"], store, cfg_wd)
...
def _pre_v2_shadow_repos(base: Path) -> List[Dict]:
    ...
    for child in base.iterdir():
        ...
        wd_marker = child / "PIXEL_AGENTS_WORKDIR"
        if wd_marker.exists():
            try:
...
# look for ``HEAD``/``info/exclude``/``PIXEL_AGENTS_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at PIXEL_AGENTS_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "PIXEL_AGENTS_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Pixel Agents changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Pixel Agents changed", not an exact per-session ledger.
...
# ---------------------------------------------------------------------------
# Public helpers for `pixel-agents checkpoints` CLI
# ---------------------------------------------------------------------------
