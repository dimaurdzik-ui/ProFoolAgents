...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``pixel-agents tools`` repaint, tool-registration scans,
    ...
    2. ``~/.pixel-agents/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``pixel-agents auth add xai-oauth``
    ...
    try:
        ...
        auth_path = get_pixel_agents_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.pixel-agents/.env`` first, then ``os.environ``.

    Wraps :func:`pixel_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from pixel_cli.config import get_env_value as _pixel_get_env_value

        value = _pixel_get_env_value(name)
...
def pixel_xai_user_agent() -> str:
    """Return a stable Pixel Agents-specific User-Agent for xAI HTTP calls."""
    try:
        from pixel_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Pixel-Agents/{__version__}"


def pixel_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Pixel Agents,
    matching the direct HTTP integrations (search, TTS, STT, image, video).
    """
    return {"User-Agent": pixel_xai_user_agent()}


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Pixel Agents config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Pixel Agents home, then mark it seen."""
    ...
    try:
        ...
        marker_dir = get_pixel_agents_home() / "state"
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Pixel Agents-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``pixel_cli.config.get_env_value`` so keys
    stored in ``~/.pixel-agents/.env`` (the standard Pixel Agents location) are honored —
    ...
    try:
        ...
        override_base_url = str(
            get_env_value("PIXEL_AGENTS_XAI_BASE_URL")
