...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("PIXEL_AGENTS_YOLO_MODE", ""))
...
# os.environ["PIXEL_AGENTS_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export PIXEL_AGENTS_INTERACTIVE.
_pixel_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "pixel_interactive",
    default=None,
)


def set_pixel_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["PIXEL_AGENTS_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``PIXEL_AGENTS_INTERACTIVE`` env var for legacy callers.
    """
    return _pixel_interactive_ctx.set("1" if interactive else "")


def reset_pixel_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_pixel_interactive_context`."""
    _pixel_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``PIXEL_AGENTS_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _pixel_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("PIXEL_AGENTS_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("PIXEL_AGENTS_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("PIXEL_AGENTS_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("PIXEL_AGENTS_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set PIXEL_AGENTS_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind PIXEL_AGENTS_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds PIXEL_AGENTS_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("PIXEL_AGENTS_CRON_SESSION"):
        return False
    if env_var_enabled("PIXEL_AGENTS_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $PIXEL_AGENTS_HOME, or by the resolved absolute
# active profile home path such as /home/pixel-agents/.pixel-agents/config.yaml. The
# resolved-absolute form is folded into the ~/.pixel-agents/ patterns at detection
...
# go stale when PIXEL_AGENTS_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_PIXEL_AGENTS_ENV_PATH = (
    r'(?:~\/\.pixel-agents/|'
    r'(?:\$home|\$\{home\})/\.pixel-agents/|'
    r'(?:\$pixel_home|\$\{pixel_home\})/)'
    r'\.env\b'
)
# ~/.pixel-agents/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _PIXEL_AGENTS_ENV_PATH; matches the PIXEL_AGENTS_HOME override form as
# well as ~/.pixel-agents/.
_PIXEL_AGENTS_CONFIG_PATH = (
    r'(?:~\/\.pixel-agents/|'
    r'(?:\$home|\$\{home\})/\.pixel-agents/|'
    r'(?:\$pixel_home|\$\{pixel_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_PIXEL_AGENTS_ENV_PATH}|'
    rf'{_PIXEL_AGENTS_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    # `pixel-agents` and `gateway` (e.g. `pixel-agents -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bPixelAgents\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart pixel-agents gateway (kills running agents)"),
    (r'\bPixelAgents\s+update\b', "pixel-agents update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `pixel-agents gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `pixel-agents ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart pixel-agents-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart pixel-agents-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(pixel-agents|gateway|cli\.py)\b', "kill pixel-agents/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill pixel-agents` but not
    # `kill -9 $(pgrep -f pixel-agents)` because the substitution is opaque
    ...
    # the `pixel-agents gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.pixel-agents.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(pixel-agents|ai\.pixel-agents)\b', "stop/restart pixel-agents launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Pixel Agents file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.pixel-agents/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Pixel Agents-managed security file (~/.pixel-agents/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_PIXEL_AGENTS_CONFIG_PATH}|{_PIXEL_AGENTS_ENV_PATH})', "in-place edit of Pixel Agents config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_PIXEL_AGENTS_CONFIG_PATH}|{_PIXEL_AGENTS_ENV_PATH})', "in-place edit of Pixel Agents config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_PIXEL_AGENTS_CONFIG_PATH}|{_PIXEL_AGENTS_ENV_PATH})', "in-place edit of Pixel Agents config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.pixel-agents/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / PIXEL_AGENTS_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Pixel Agents home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\pixel-agents), so folding the user home
    # first would eat the prefix the Pixel Agents-home fold needs.
    command = _rewrite_resolved_pixel_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / PIXEL_AGENTS_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.pixel-agents``); the matched
...
def _rewrite_resolved_pixel_home(command: str) -> str:
    """Rewrite the resolved absolute Pixel Agents home prefix to ``~/.pixel-agents/``.

    Resolves the active ``PIXEL_AGENTS_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.pixel-agents/`` so the static ``_PIXEL_AGENTS_CONFIG_PATH`` / ``_PIXEL_AGENTS_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/pixel-agents/.pixel-agents/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$PIXEL_AGENTS_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from pixel_constants import get_pixel_agents_home
        home = get_pixel_agents_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.pixel-agents")
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Pixel Agents ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"pixel-agents-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
# =========================================================================
# Blocking gateway approval (mirrors CLI's synchropixel input() flow)
# =========================================================================
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Pixel Agents YOLO.
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["PIXEL_AGENTS_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "PIXEL_AGENTS_SPINNER_PAUSE" in os.environ:
            del os.environ["PIXEL_AGENTS_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Pixel Agents approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``PIXEL_AGENTS_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            PIXEL_AGENTS_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if env_var_enabled("PIXEL_AGENTS_CRON_SESSION"):
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set PIXEL_AGENTS_INTERACTIVE or "
                "PIXEL_AGENTS_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set PIXEL_AGENTS_INTERACTIVE or "
            "PIXEL_AGENTS_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("PIXEL_AGENTS_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``PIXEL_AGENTS_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("PIXEL_AGENTS_EXEC_ASK")
    ...
    # responds with /approve or /deny, mirroring the CLI's synchropixel
