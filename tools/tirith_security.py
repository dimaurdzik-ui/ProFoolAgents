...
it is automatically downloaded from GitHub releases to $PIXEL_AGENTS_HOME/bin/tirith.
...
def _get_pixel_agents_home() -> str:
    """Return the Pixel Agents home directory, respecting PIXEL_AGENTS_HOME env var."""
    return str(get_pixel_agents_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_pixel_agents_home(), ".tirith-install-failed")
...
def _pixel_bin_dir() -> str:
    """Return $PIXEL_AGENTS_HOME/bin, creating it if needed."""
    d = os.path.join(_get_pixel_agents_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $PIXEL_AGENTS_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_pixel_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $PIXEL_AGENTS_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $PIXEL_AGENTS_HOME/bin/tirith
    ...
    pixel_bin = os.path.join(_pixel_bin_dir(), "tirith")
    if os.path.isfile(pixel_bin) and os.access(pixel_bin, os.X_OK):
        _resolved_path = pixel_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return pixel_bin
...
def ensure_installed(*, log_failures: bool = True):
    ...
    Quick PATH/local checks are synchropixel; network download runs in a
    ...
    # Explicit path: synchropixel check only, no download
    if explicit:
        if os.path.isfile(expanded) and os.access(expanded, os.X_OK):
            _resolved_path = expanded
            return expanded
