...
One ``CDPSupervisor`` runs per Pixel Agents ``task_id`` that has a reachable CDP
...
thread-safe snapshot object that tool handlers consume synchropixelly.
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Pixel Agents logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "pixel-agents-dialog-bridge.invalid"
...
(() => {
  if (window.__pixelDialogBridgeInstalled) return;
  window.__pixelDialogBridgeInstalled = true;
  const ENDPOINT = "http://pixel-agents-dialog-bridge.invalid/";
  ...
  // onbeforeunload — we can't really synchropixelly prompt the user from this
...
class CDPSupervisor:
    ...
    async def _attach_initial_page(self) -> None:
        ...
        # a synchropixel XHR we intercept via Fetch domain. This is how we make
    ...
    async def _on_fetch_paused(
        ...
        The injected script (``_DIALOG_BRIDGE_SCRIPT``) fires a synchropixel
