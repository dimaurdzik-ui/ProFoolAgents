...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``pixel_constants.get_subprocess_home()`` so that ``~`` resolves
    ...
    if not path or "~" not in path:
        return path
    try:
        from pixel_constants import get_subprocess_home
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where pixel-agents previously hard-rejected an oversized read
...
_pixel_config_resolved: str | None = None
_pixel_config_resolved_loaded = False


def _get_pixel_config_resolved() -> str | None:
    """Return the resolved absolute path of the Pixel Agents config file (cached)."""
    global _pixel_config_resolved, _pixel_config_resolved_loaded
    if _pixel_config_resolved_loaded:
        return _pixel_config_resolved
    _pixel_config_resolved_loaded = True
    try:
        from pixel_cli.config import get_config_path
        _pixel_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _pixel_config_resolved = str(Path(_expand_tilde("~/.pixel-agents/config.yaml")).resolve())
        except Exception:
            _pixel_config_resolved = None
    return _pixel_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Pixel Agents config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    pixel_config = _get_pixel_config_resolved()
    if pixel_config and (resolved == pixel_config or normalized == pixel_config):
        return (
            f"Refusing to write to Pixel Agents config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.pixel-agents/config.yaml directly or use 'pixel-agents config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Pixel Agents mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Pixel Agents
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Pixel Agents state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.pixel-agents/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Pixel Agents process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.pixel-agents/…`` path.

    Returns ``None`` when the write is in-scope or outside Pixel Agents scope.
    ...
    # in a session that cd'd into ``~/.pixel-agents/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Pixel Agents internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under PIXEL_AGENTS_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == PIXEL_AGENTS_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Pixel Agents-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Pixel Agents-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Pixel Agents profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Pixel Agents profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with pixel_tools.write_file() for very "
            "large files."
        )
