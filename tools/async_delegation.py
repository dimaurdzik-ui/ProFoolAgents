...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    ...
    for name, sql_type in (
        ...
        ("delivery_claim", "TEXT"),
        ("delivery_claimed_at", "REAL"),
        # Raw api_server session id (X-Pixel-Agents-Session-Id) of the ORIGINATING
...
def _current_origin_session_id() -> str:
    ...
    The obvious source — ``PIXEL_AGENTS_SESSION_ID`` via ``get_session_env`` — is
    ...
    The request-scoped ``PIXEL_AGENTS_SESSION_CHAT_ID`` binding survives child
    construction: ``_bind_api_server_session`` binds ``chat_id`` to the raw
    ``X-Pixel-Agents-Session-Id``, and its only writer is ``set_session_vars`` —
    ...
    try:
        ...
        if get_session_env("PIXEL_AGENTS_SESSION_PLATFORM", "") != "api_server":
            return ""
        return get_session_env("PIXEL_AGENTS_SESSION_CHAT_ID", "") or ""
    except Exception:
        return ""
...
def dispatch_async_delegation(
    *,
        ...
        if running >= max_async_children:
            return {
                ...
                "error": (
                    ...
                    f"the chat), or run this task synchropixelly "
    ...
    try:
        ...
        # get_pixel_agents_home() under the right profile.
...
def dispatch_async_delegation_batch(
    *,
    ...
    "total_duration_seconds": N}`` dict that the synchropixel path would have
