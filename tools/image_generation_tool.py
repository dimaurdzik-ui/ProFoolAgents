...
selectable via ``pixel-agents tools`` → Image Generation; the active model is
...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users on Pixel Subscription should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps Pixel
...
# ---------------------------------------------------------------------------
# Managed FAL gateway (Pixel Subscription)
# ---------------------------------------------------------------------------
...
def _get_managed_fal_client(managed_gateway):
    ...
    with _managed_fal_client_lock:
        ...
        _managed_fal_client = _ManagedFalSyncClient(
            fal_client,
            key=managed_gateway.pixel_user_token,
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"Pixel Subscription gateway rejected model '{model}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the Pixel Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `pixel-agents tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.pixel-agents"
    ...
    # Pixel Agents cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `pixel-agents tools` → Image Generation."
            )
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions:
      - FAL_KEY signup link
      - managed-gateway status (if Pixel tools are enabled)
    ...
    if managed_pixel_tools_enabled():
        ...
        gateway_message = pixel_tool_gateway_unavailable_message(
            "managed FAL image generation",
        )
        if gateway_message:
    ...
    if managed_pixel_tools_enabled():
        lines.append(
            "  2. Sign in to a Pixel account that has the managed FAL "
            "gateway enabled (`pixel-agents setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `pixel-agents tools` "
        "→ Image Generation (run `pixel-agents plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `pixel-agents plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `pixel-agents tools` → Image Generation."
                ),
...
# model/provider via `pixel-agents tools` or `/skills`.
