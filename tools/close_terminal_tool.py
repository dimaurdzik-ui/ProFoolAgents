#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Pixel Agents desktop GUI.
...
``read_terminal`` it is gated on ``PIXEL_AGENTS_DESKTOP`` so it never appears outside
the GUI.
"""
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Pixel Agents desktop GUI (the tabs mirroring terminal(background=true) runs). "
