...
def managed_pixel_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the Pixel Tool Gateway.

    Entitlement is paid Pixel Portal service access OR a live free tool pool
...
def pixel_tool_gateway_unavailable_message(
    capability: str = "the Pixel Tool Gateway",
    *,
    force_fresh: bool = False,
) -> str:
    """Return account-aware guidance for an unavailable Pixel Tool Gateway path."""
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `pixel-agents model` to refresh your "
        "Pixel Portal login and billing status."
    )
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    if managed_enabled is None:
        managed_enabled = managed_pixel_tools_enabled()
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``pixel-agents auth add
    ...
    2. The environment / ``~/.pixel-agents/.env``. Under a multiplexed gateway turn
       ...
       ``pixel_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``pixel-agents auth
    ...
    when omitted, ``pixel_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `pixel-agents auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``pixel-agents auth add openai-api``) via
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.pixel-agents/.env`` (via
    ``pixel_cli.config.get_env_value`` when available) so tool-side
