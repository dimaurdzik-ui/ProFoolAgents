...
Pixel, Codex, native Anthropic, or a custom OpenAI-compatible endpoint.
...
# ``agent.auxiliary_client`` pulls credential_pool → pixel_cli.auth → httpx
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("PIXEL_AGENTS_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: PIXEL_AGENTS_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("PIXEL_AGENTS_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
def _normalize_to_supported_image(
    ...
    out_dir = get_pixel_agents_dir("cache/vision", "temp_vision_images")
...
def _supports_media_in_tool_results(provider: str, model: str) -> bool:
    ...
    Providers covered today (per spec docs verified Apr-2026):
      ...
      * Anthropic Messages API (``anthropic`` provider, plus aggregators that
        proxy Claude — ``openrouter``, ``pixel``, ``vertex``, ``bedrock``):
    ...
    # them.
    _AGGREGATORS = {
        "openrouter", "pixel", "vertex", "bedrock", "anthropic-vertex",
        "google-vertex",
    }
...
async def _vision_analyze_native(
    ...
    try:
        ...
        temp_dir = get_pixel_agents_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $PIXEL_AGENTS_HOME/cache/vision/ and cleaned up
...
def check_vision_requirements() -> bool:
    ...
    and if that fails, the auto chain (main provider → openrouter → pixel).
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, Pixel, Codex, Anthropic, or a custom OpenAI-compatible endpoint).")
        sys.exit(1)
    else:
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_pixel_agents_dir("cache/video", "temp_video_files")
