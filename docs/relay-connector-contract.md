...
This document is the formal interface between the **Pixel Agents gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`PixelResearch/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `pixel-agents gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`pixel-agents gateway enroll` → connector `/relay/enroll`, or
...
(`PixelResearch/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `pixel-agents` CLI.
