# RCA: SSL CA cert bundle corruption after `pixel-agents update`
...
A partial `pixel-agents update`, interrupted venv repair, or stale CA-bundle environment variable can leave Python TLS configuration pointing at a missing, empty, or unloadable CA bundle. The first outbound HTTPS client creation or request can then fail with a raw `FileNotFoundError: [Errno 2] No such file or directory` or a low-level SSL error that does not name the broken CA path.

## Root cause

Pixel Agents uses OpenAI/httpx and requests-based clients for provider calls, model metadata, gateway delivery, and web tools. Those clients inherit CA bundle settings from:

- `PIXEL_AGENTS_CA_BUNDLE`
- `SSL_CERT_FILE`
- `REQUESTS_CA_BUNDLE`
- `CURL_CA_BUNDLE`
- the bundled `certifi` package's `cacert.pem`

When the venv is partially refreshed, or when one of those env vars points at a file that no longer exists, provider client construction can fail before Pixel Agents has enough context to produce a useful message.
...
`pixel_cli doctor` exposes the same check under `SSL / CA Certificates`, so users can diagnose the problem without starting a model session.
...
For a normal corrupted Pixel Agents venv, reinstall the affected client dependencies:

```bash
python -m pip install --force-reinstall certifi openai httpx
```

For a custom/corporate CA setup, fix the env var so it points at a real PEM bundle, or unset it if Pixel Agents should use the bundled `certifi` store.

## Environment escape hatch

Set `PIXEL_AGENTS_SKIP_SSL_GUARD=1` to bypass the preflight check. This is intended only for sandboxed or managed-trust environments where the Python CA path looks unusual but downstream clients are known to work.
