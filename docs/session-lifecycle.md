...
The canonical transcript store is SQLite via `SessionDB` (from `pixel_state`). The
...
Counts consecutive restarts via a JSON file (`{PIXEL_AGENTS_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `pixel-agents update`, `pixel-agents gateway restart`,
or `/restart`.
