...
Pixel Agents supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.pixel-agents/config.yaml`:
...
Or set the env var: `PIXEL_AGENTS_KANBAN_DISPATCH_IN_GATEWAY=false`
