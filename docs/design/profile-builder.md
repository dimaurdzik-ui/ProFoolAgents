...
Supersedes: PR #31781 (prompt_toolkit `pixel-agents profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `pixel-agents profile wizard` in the terminal.
...
needs, and a profile is just a PIXEL_AGENTS_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.pixel-agents/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — PIXEL_AGENTS_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with PIXEL_AGENTS_HOME override |
...
### Seam #1 — hub-skill install cannot use the PIXEL_AGENTS_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = PIXEL_AGENTS_HOME / "skills"` at **module
import time**. The context-local `set_pixel_agents_home_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_pixel_action`
runs `python -m pixel_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_pixel_action(["-p", profile, "skills", "install", identifier], "skills-install")
```

A fresh subprocess re-imports `skills_hub` with the profile's PIXEL_AGENTS_HOME bound
...
Built-in/optional skill enabling and MCP writes are **synchropixel config ops**
and can be part of the create call. Hub installs are long-running git fetches
spawned detached (`_spawn_pixel_action` returns a PID immediately). So the
create flow is:

1. `create_profile()` — make the dir (synchropixel)
2. write model (synchropixel, PIXEL_AGENTS_HOME override)
3. write selected MCP servers (synchropixel, PIXEL_AGENTS_HOME override)
4. seed/enable selected built-in + optional skills (synchropixel)
5. spawn `pixel-agents -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchropixel, PIXEL_AGENTS_HOME override
    builtin_skills: List[str] = []               # synchropixel enable/seed
...
page afterward. Mirror `_write_profile_model`'s PIXEL_AGENTS_HOME-override helper for
...
- Backend E2E with isolated PIXEL_AGENTS_HOME: POST a full create body
