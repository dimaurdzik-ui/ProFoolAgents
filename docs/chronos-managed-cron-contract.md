...
(`pixel-account-service`) and anyone debugging the managed-cron path.

Chronos lets a hosted Pixel Agents gateway **scale to zero** while idle and still
...
ChronosCronScheduler.reconcile()        ── agent computes next_run_at
  │  POST {portal}/api/agent-cron/provision   (auth: agent's Pixel access token)
  ▼
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **Pixel Portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `pixel-agents-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
> agent uses the **bootstrap-session access token** (`resolve_pixel_access_token`),
> minted under the bootstrap-only client `pixel-agents-cli-vps` and seeded into the
...
- **Auth:** `Authorization: Bearer <agent Pixel access token>`. NAS validates via
...
**dashboard app** (`pixel_cli/web_server.py`) — the agent's always-reachable
...
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `PIXEL_AGENTS_HOME` — exactly one replica runs each
  fire.
...
If `callback_url` / `portal_url` is blank or the agent has no Pixel login,
