...
`pixel-agents config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.pixel-agents/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `PIXEL_AGENTS_HOME`, secret scope, and
