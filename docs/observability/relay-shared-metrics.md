...
Pixel Agents includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Pixel Agents and does not require `pixel-agents plugins enable
observability/nemo_relay`. Pixel Agents remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Pixel Agents execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `pixel-agents[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

Pixel Agents requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Pixel Agents installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Pixel Agents process so configured interceptors can
...
Collection remains off unless Pixel Agents policy enables it:
...
Pixel Agents core owns one Relay host and one isolated Relay session scope per Pixel Agents
session. Core lifecycle producers use
`pixel_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Pixel Agents plugin registration. Shared-metrics marks must
...
```text
Pixel Agents turn, API, and tool hooks
  -> Relay session, task, and LLM lifecycle
  -> Pixel Agents shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Pixel Agents sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
Each task run is a Relay `Function` scope named `pixel-agents.task_run`, parented to
the owning Pixel Agents session. The start counter contains only bounded execution
...
provider attempts for the same Pixel Agents API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Pixel Agents tool-call
...
cancellations. Active task ownership follows the task ID if Pixel Agents rotates its
conversation session during context compression.

Local state is written under:

```text
$PIXEL_AGENTS_HOME/telemetry/shared_metrics/metrics.sqlite3
$PIXEL_AGENTS_HOME/telemetry/shared_metrics/outbox/*.json
```
...
schema field name, its current scope is one `PIXEL_AGENTS_HOME`, so it is more
...
Deleting `$PIXEL_AGENTS_HOME/telemetry/shared_metrics` resets the identifier together
...
Run a real Pixel Agents CLI turn against the deterministic local model server:
