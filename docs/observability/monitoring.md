...
Pixel Agents gateway daemon, exported over OTLP/HTTP to an operator-configured
...
(`plugins/observability/nemo_relay/`) and its Pixel Agents-owned subscribers.

## What gets exported

| Signal | OTLP route | Content |
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `pixel-agents.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `pixel-agents.platform.up/degraded` with bounded `error_code` attributes |
...
`pixel-agents.gateway.active_agents`, `pixel-agents.gateway.background_work`, and
`pixel-agents.gateway.background_delegations` are complementary. `active_agents`
...
```bash
pixel-agents monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'pixel-agents[otlp]'`),
...
a plain `pixel-agents gateway run`: the exporter lives in the gateway process, so
...
`pixel-agents.gateway.up`, `pixel-agents.platform.up`, and `pixel-agents.platform.degraded`.
...
absent_over_time(pixel_gateway_up[5m])

# Locally owned bridge is explicitly down.
pixel_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
pixel_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
pixel_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
pixel_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(pixel_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `pixel-agents.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
pixel-agents.status = failed|unknown
pixel-agents.delivery_outcome = failed|not_configured
pixel-agents.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `pixel-agents.job_key`;
...
Pixel Agents-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/pixel_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/pixel_otel_capture.jsonl --wait 8
...
`pixel-agents.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
pixel-agents monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `pixel-agents monitoring` CLI intentionally exposes `status` only. This first
release covers only Pixel Agents-owned service-health and operational-diagnostic
signals, including Pixel Agents-owned Relay transport health. Team Gateway's
