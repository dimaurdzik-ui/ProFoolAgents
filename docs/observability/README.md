# Pixel Agents Observer Hooks

Pixel Agents observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Pixel Agents' planner, model providers, memory, tool registry,
...
Pixel Agents also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "pixel-agents.observer.v1"
```

Hook callbacks are fail-open. Pixel Agents catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Pixel Agents only builds
