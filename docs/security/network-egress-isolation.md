...
When running Pixel Agents inside Docker, the default `network_mode: host` gives the
...
The Pixel Agents [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ pixel-agents │   │ pixel-agents-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   PIXEL_AGENTS_UID=$(id -u) PIXEL_AGENTS_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=pixel-agents,pixel-agents-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://pixel-agents-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Pixel Agents trust model and vulnerability reporting
