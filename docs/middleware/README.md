# Pixel Agents Middleware

Pixel Agents middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Pixel Agents'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Pixel Agents calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Pixel Agents retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Pixel Agents guardrails,
...
- `telemetry_schema_version`: currently `pixel-agents.observer.v1`
- `middleware_schema_version`: currently `pixel-agents.middleware.v1`
...
Pixel Agents stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Pixel Agents runs
them as a nested chain in registration order. Middleware failures are fail-open:
Pixel Agents logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Pixel Agents applies middleware in this order:
...
For each tool call, Pixel Agents applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Pixel Agents pre-execution path against the effective arguments:
...
```bash
pixel-agents plugins enable <plugin-name>
```

For isolated local testing, use one `PIXEL_AGENTS_HOME` for plugin enablement and the
agent run:

```bash
export PIXEL_AGENTS_HOME=/tmp/pixel-agents-middleware-test
mkdir -p "$PIXEL_AGENTS_HOME"
pixel-agents plugins enable <plugin-name>
pixel-agents chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run pixel-agents plugins enable <plugin-name>
uv run pixel-agents chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["pixel_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/pixel-agents-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Pixel Agents expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Pixel Agents treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Pixel Agents preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Pixel Agents does not convert downstream
