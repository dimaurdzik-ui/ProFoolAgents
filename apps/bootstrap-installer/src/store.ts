...
export const $pixelAgentsHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.pixel-agents/logs/bootstrap-installer.log')
    $pixelAgentsHome.set('~/.pixel-agents')
  ...
  try {
    const [logPath, pixelAgentsHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_pixel_agents_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $pixelAgentsHome.set(pixelAgentsHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Pixel Agents" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Pixel Agents-Setup.exe --update);
...
export async function launchPixelAgentsDesktop(): Promise<void> {
  ...
  await invoke('launch_pixel_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Pixel Agents repository' },
