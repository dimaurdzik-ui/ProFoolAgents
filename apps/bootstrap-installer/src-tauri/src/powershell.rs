...
///
/// `pixel_home_override` propagates to the child as $PIXEL_AGENTS_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, pixel_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = pixel_home_override {
        cmd.env("PIXEL_AGENTS_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, pixel_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = pixel_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_pixel_home() {
