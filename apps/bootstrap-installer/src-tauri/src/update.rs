...
//!
//! Driven when the installer is launched as `Pixel Agents-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Pixel Agents desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `pixel-agents update`
//!      or repair bootstrap can race locked files),
//!   2. run `pixel-agents update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in pixel_cli/main.py),
//!   3. run `pixel-agents desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `pixel-agents update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_pixel) and
...
/// `pixel-agents update` exit code meaning "another pixel-agents process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// pixel_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `pixel-agents update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `pixel-agents update` claims
/// the same file (see `pixel_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `pixel-agents update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in pixel_cli/update_lock.py — all three read
...
/// owner that is itself and aborts ("Another Pixel Agents update is already
/// running"), then the desktop relaunches and retries forever. A foreign live
/// pid (e.g. a dashboard-spawned `pixel-agents update`) still blocks.
fn live_marker_owner(path: &Path) -> Option<MarkerOwner> {
    ...
    // Desktop `writeUpdateMarker(pixelAgentsHome, child.pid)` races ahead of us;
...
async fn run_update(app: AppHandle) -> Result<()> {
    let pixel_home = crate::paths::pixel_home();
    let install_root = pixel_home.join("pixel-agents");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_pixel`
    ...
    //
    // The same marker is the cross-process update lock (pixel_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `pixel-agents update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Pixel Agents update is already running (PID {}, started {} ago). \
    ...
    let pixel-agents = resolve_pixel(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the pixel-agents CLI under {}. Is Pixel Agents installed? \
    ...
    // async on Windows. If it still holds the venv shim, `pixel-agents update`
    ...
    // ---- stage 2: pixel-agents update -----------------------------------------
    // Pass --branch so `pixel-agents update` targets the branch this installer was
    ...
    // without --branch, `pixel-agents update` switches the checkout to `main` (a
    ...
    // --force skips `pixel-agents update`'s Windows running-exe guard (which would
    ...
    // time `pixel-agents update` runs there is no legitimate pixel-agents.exe to protect,
    // and the guard would only produce a false "Pixel Agents is still running" stop.
    //
    ...
    // probe only checks the pixel-agents.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Pixel Agents windows" message.
    ...
    // Retry-once for the update-boundary crash. `pixel-agents update` lazily imports
    ...
    // second `pixel-agents update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Pixel Agents" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Pixel Agents is still running. Close all Pixel Agents windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "pixel-agents update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::pixel_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: pixel-agents desktop --build-only ----------------------------
    // `pixel-agents update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `pixel-agents update` already does above, and `pixel-agents update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `pixel-agents desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Pixel Agents manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_pixel_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Pixel Agents to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend pixel-agents.exe (or the desktop Pixel Agents.exe
            ...
            // force-kill every Pixel Agents.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Pixel Agents still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_pixel();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_pixel(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Pixel Agents.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Pixel Agents.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `pixel-agents.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `pixel-agents.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\pixel-agents.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startPixelAgents()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// pixel-agents.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named pixel-agents.exe.)
fn force_kill_other_pixel() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `pixel-agents <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv pixel-agents shim under an install root, regardless of existence.
fn venv_pixel(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("pixel-agents.exe")
    } else {
        install_root.join("venv").join("bin").join("pixel-agents")
    }
}

/// Resolve the pixel-agents CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `pixel-agents` on PATH.
fn resolve_pixel(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_pixel(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "pixel-agents.exe" } else { "pixel-agents" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let pixel_home = crate::paths::pixel_home();
    let mut envs = vec![(
        "PIXEL_AGENTS_HOME".to_string(),
        pixel_home.as_os_str().to_os_string(),
    )];
    // `pixel-agents update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `pixel-agents update` child claims that SAME lock (pixel_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Pixel Agents is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in pixel_cli/update_lock.py.
    envs.push((
        "PIXEL_AGENTS_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        pixel_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_pixel_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Pixel Agents.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.pixel-agents-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.pixel-agents-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::pixel_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_pixel_is_under_install_root() {
        let root = Path::new("/x/pixel-agents");
        let shim = venv_pixel(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/pixel-agents"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/pixel-agents"));
        assert!(
            envs.iter().any(|(k, v)| k == "PIXEL_AGENTS_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the pixel-agents update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_pixel(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/pixel-agents");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".pixel-agents-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `pixel-agents update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(pixelAgentsHome, child.pid) races ahead
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Pixel Agents.app"]),
            Some(PathBuf::from("/Applications/Pixel Agents.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Pixel Agents.app");
        let tmp = base.join("Pixel Agents.app.pixel-agents-update-new");
        let old = base.join("Pixel Agents.app.pixel-agents-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Pixel Agents.app");
        let tmp = base.join("Pixel Agents.app.pixel-agents-update-new"); // intentionally absent
        let old = base.join("Pixel Agents.app.pixel-agents-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Pixel Agents.app");
        let tmp = base.join("Pixel Agents.app.pixel-agents-update-new"); // absent
        let old = base.join("Pixel Agents.app.pixel-agents-update-old");
        write_marker(&target, "OLD");
