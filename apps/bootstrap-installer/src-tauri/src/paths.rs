...
//!
//! Mirrors `pixel_constants.get_pixel_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\pixel-agents
//!   macOS:   ~/.pixel-agents
//!   Linux:   ~/.pixel-agents  (override via $PIXEL_AGENTS_HOME)
//!
//! NOTE (macOS): Python's get_pixel_home(), scripts/install.sh, and the
//! Electron desktop's resolvePixelAgentsHome() ALL use ~/.pixel-agents on macOS — there
...
/// Returns the canonical Pixel Agents home directory, respecting $PIXEL_AGENTS_HOME if set.
pub fn pixel_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("PIXEL_AGENTS_HOME") {
    ...
    {
        // %LOCALAPPDATA%\pixel-agents — matches scripts/install.ps1's $PixelAgentsHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("pixel-agents");
        }
    }

    // macOS + Linux + fallback: ~/.pixel-agents (matches Python get_pixel_home(),
    // install.sh, and the Electron desktop's resolvePixelAgentsHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".pixel-agents");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".pixel-agents")
}

pub fn log_dir() -> PathBuf {
    pixel_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    pixel_home().join("bootstrap-cache")
}
...
/// PIXEL_AGENTS_HOME so it survives repo checkout deletion (unlike anything under
/// pixel-agents/).
///
/// On Windows this is `%LOCALAPPDATA%\pixel-agents\pixel-agents-setup.exe`; on other
/// platforms the extension differs but the directory is the same.
pub fn installer_dest() -> PathBuf {
    let name = if cfg!(target_os = "windows") {
        "pixel-agents-setup.exe"
    } else {
        "pixel-agents-setup"
    };
    pixel_home().join(name)
}
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_pixel`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under PIXEL_AGENTS_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves PIXEL_AGENTS_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    pixel_home().join(".pixel-agents-update-in-progress")
}
...
pub fn copy_self_to_pixel_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to PIXEL_AGENTS_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_PIXEL_AGENTS_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".pixel-agents-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under PIXEL_AGENTS_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[pixel-agents-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("PIXEL_AGENTS_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_pixel_home() -> String {
    pixel_home().to_string_lossy().into_owned()
}
