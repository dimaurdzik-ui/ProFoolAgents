/**
 * Canonical Pixel Agents skin — the theme SDK's cross-surface contract.
 *
 * A skin is authored once as YAML in `$PIXEL_AGENTS_HOME/skins/<name>.yaml` (or a
 * built-in), resolved by the Python skin engine (`pixel_cli/skin_engine.py`),
 ...
 *   • CLI     → `pixel_cli/skin_engine` → prompt_toolkit / Rich styles (Python)
 *
...
export interface PixelAgentsSkin {
  name?: string
  description?: string
  colors?: SkinColors
