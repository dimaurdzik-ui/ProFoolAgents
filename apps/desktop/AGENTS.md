...
How to build Pixel Agents Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Pixel Agents surface can also
...
- A **runtime home change** (switching the underlying `PIXEL_AGENTS_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Pixel Agents — do not assume
