...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pixel-agents-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'pixel-agents-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Pixel Agents Test'], { cwd: dir })
