/**
 * windows-pixel-agents-path.ts
 *
 * Pure, dependency-injected pieces of Windows `pixel-agents` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvPixelAgentsCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `pixel-agents` shim shadowed the real
 *      pixel-agents.cmd/pixel-agents.exe; the shim then failed the --version probe and
 ...
 *      venv\Scripts\pixel-agents.exe (the console-script shim, written at the END
 ...
 *   3. resolveVenvPixelAgentsCommand() — unwrapWindowsVenvPixelAgentsCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `pixel-agents`) must
 * not shadow `pixel-agents.cmd`/`pixel-agents.exe`. The empty entry is kept LAST so
 ...
 * signals (venv python interpreter, venv pixel-agents shim, bootstrap-complete
 * marker) — gating on just the pixel-agents.exe console-script shim alone is the
...
export interface ResolveVenvPixelAgentsCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportPixelAgentsCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `pixel-agents`/`pixel-agents.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/pixel-agents(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m pixel_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportPixelAgentsCli(). A
 * venv whose update died mid-`pip install` still has python.exe + pixel-agents.exe
 ...
 *
 * Returns null when `command` is not a venv pixel-agents shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvPixelAgentsCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvPixelAgentsCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^pixel-agents(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    !canImportPixelAgentsCli(python, {
      env: {
    ...
    rememberLog?.(
      `Ignoring venv Pixel Agents at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Pixel Agents Python at ${python}`,
    command: python,
