// Backend subcommand routing for the desktop-managed Pixel Agents process.
//
// The desktop app launches its own headless backend via `pixel-agents serve` — it
...
// app hasn't updated yet, or an older `pixel-agents` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Pixel Agents profile to pin via `--profile`.
 */
 ...
 * `-m pixel_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `pixel_cli/subcommands/dashboard.py` source registers
