// Profile-delete routing logic for the `pixel-agents:api` IPC handler.
//
...
// pool backend -- spawning a fresh one would call ensure_pixel_home() and
...
/**
 * Parse a `pixel-agents:api` request into the profile name a DELETE targets, or
...
/**
 * Route the next `pixel-agents:api` request away from the primary/window backend
 ...
 * ensure_pixel_home() recreates the directory the delete just removed.
 */
