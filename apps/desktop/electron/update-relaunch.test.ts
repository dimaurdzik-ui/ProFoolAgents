 ...
 *      internals; PIXEL_AGENTS_HOME / PIXEL_AGENTS_DESKTOP_* env + cwd preserved) and is
 *      safely shell-quoted.
...
const ROOT = '/home/u/.pixel-agents/pixel-agents'
...
test('resolveUnpackedRelease returns the dir for a binary UNDER release/<plat>-unpacked', () => {
  const exec = path.join(UNPACKED, 'pixel-agents')
...
test('resolveUnpackedRelease is null for AppImage / .deb / .rpm / dev / unresolved paths', () => {
  // AppImage mount
  assert.equal(resolveUnpackedRelease('/tmp/.mount_PixelAgents12345/AppRun', ROOT, 'linux'), null)
  // .deb / .rpm system install
  assert.equal(resolveUnpackedRelease('/usr/lib/pixel-agents/pixel-agents', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease('/opt/Pixel Agents/pixel-agents', ROOT, 'linux'), null)
  // dev electron
  assert.equal(
    resolveUnpackedRelease('/home/u/.pixel-agents/pixel-agents/node_modules/electron/dist/electron', ROOT, 'linux'),
    null
  )
  // empty / missing
  assert.equal(resolveUnpackedRelease('', ROOT, 'linux'), null)
  assert.equal(resolveUnpackedRelease(path.join(UNPACKED, 'pixel-agents'), '', 'linux'), null)
})
...
test('resolveUnpackedRelease is not fooled by a sibling prefix dir', () => {
  ...
  const sneaky = path.join(ROOT, 'apps', 'desktop', 'release', 'linux-unpacked-evil', 'pixel-agents')
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--lang=en-US',
    'pixelagents://open/agent/42', // deep link — keep
    '--profile=work', // app flag — keep
    '--remote-debugging-port=9222' // internal — drop
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'pixelagents://open/agent/42', '--profile=work'])
  assert.deepEqual(collectRelaunchArgs(undefined), [])
})

test('collectRelaunchEnv preserves PIXEL_AGENTS_HOME + PIXEL_AGENTS_DESKTOP_* + sandbox opt-out only', () => {
  const env = {
    PIXEL_AGENTS_HOME: '/home/u/.pixel-agents',
    PIXEL_AGENTS_DESKTOP_REMOTE_URL: 'http://box:9119',
    PIXEL_AGENTS_DESKTOP_REMOTE_TOKEN: 'secret',
    PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS_ROOT: '/home/u/dev/pixel-agents',
    PIXEL_AGENTS_DESKTOP_APP_NAME: 'PixelAgentsSandbox',
...
test('buildRelaunchScript embeds pid/exec/args/env/cwd and is valid bash', () => {
  const script = buildRelaunchScript({
    pid: 4242,
    execPath: '/home/u/.pixel-agents/pixel-agents/apps/desktop/release/linux-unpacked/Pixel Agents',
    args: ['pixelagents://open/agent/42', "--note=it's fine"],
    env: { PIXEL_AGENTS_HOME: '/home/u/.pixel-agents', PIXEL_AGENTS_DESKTOP_REMOTE_URL: 'http://box:9119' },
    cwd: '/home/u/work dir'
  })
  ...
  assert.match(script, /export PIXEL_AGENTS_HOME='\/home\/u\/\.pixel-agents'/)
  assert.match(script, /export PIXEL_AGENTS_DESKTOP_REMOTE_URL='http:\/\/box:9119'/)
  assert.match(script, /cd '\/home\/u\/work dir'/)
  assert.match(
    script,
    /exec '.*\/linux-unpacked\/Pixel Agents' 'pixel-agents:\/\/open\/agent\/42' '--note=it'\\''s fine'/
  )

  // It must be syntactically valid bash (`bash -n`). Write to a temp file and lint.
  const tmp = path.join(os.tmpdir(), `pixel-agents-relaunch-test-${Date.now()}.sh`)
...
test('buildRelaunchScript with no args/env still lints clean', () => {
  const script = buildRelaunchScript({
    pid: 1,
    execPath: '/opt/Pixel Agents/Pixel Agents',
    args: [],
    env: {},
    cwd: ''
  })

  const tmp = path.join(os.tmpdir(), `pixel-agents-relaunch-test2-${Date.now()}.sh`)
  fs.writeFileSync(tmp, script)
  ...
  assert.match(script, /exec '\/opt\/Pixel Agents\/Pixel Agents'\n/)
})
