 ...
 * Background
 * ----------
 * After `pixel-agents update` + `pixel-agents desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything PIXEL_AGENTS_DESKTOP_* is preserved
// plus PIXEL_AGENTS_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['PIXEL_AGENTS_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['PIXEL_AGENTS_DESKTOP_']
