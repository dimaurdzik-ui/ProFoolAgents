// Unit tests for the pure Windows `pixel-agents` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvPixelAgentsCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `pixel-agents` shim shadows
//      the real pixel-agents.cmd/pixel-agents.exe.
//   2. chooseUpdaterArgs() — must gate on haveRealInstall (any real-install
//      signal), not just the pixel-agents.exe console-script shim, or healthy
//      installs get forced into a destructive --repair.
//   3. resolveVenvPixelAgentsCommand() — must probe the venv python via
//      canImportPixelAgentsCli() before trusting it, or a broken venv gets
...
function makeDeps(overrides: Partial<Parameters<typeof resolveVenvPixelAgentsCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportPixelAgentsCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    pixelAgentsHome: '/fake/pixel-agents-home',
...
test('resolveVenvPixelAgentsCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvPixelAgentsCommand('/root/venv/Scripts/pixel-agents.exe', [], deps), null)
})

test('resolveVenvPixelAgentsCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvPixelAgentsCommand('/root/venv/Scripts/pixel-agents.cmd', [], deps), null)
})

test('resolveVenvPixelAgentsCommand: returns null when the basename is not pixel-agents/pixel-agents.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvPixelAgentsCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvPixelAgentsCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvPixelAgentsCommand('/root/venv/bin/pixel-agents.exe', [], deps), null)
})

test('resolveVenvPixelAgentsCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvPixelAgentsCommand('/root/venv/Scripts/pixel-agents.exe', [], deps), null)
})

test('resolveVenvPixelAgentsCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportPixelAgentsCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvPixelAgentsCommand('/root/venv/Scripts/pixel-agents.exe', ['serve'], deps)
...
test('resolveVenvPixelAgentsCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvPixelAgentsCommand('/root/venv/Scripts/pixel-agents.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'pixel_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvPixelAgentsCommand: is case-insensitive on pixel-agents.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvPixelAgentsCommand('/root/venv/Scripts/PIXEL_AGENTS.EXE', [], deps))
  assert.ok(resolveVenvPixelAgentsCommand('/root/venv/SCRIPTS/pixel-agents.exe', [], deps))
})
