 ...
 *     `X-Pixel-Agents-Session-Token` header; WS uses `?token=`.
...
// pixel_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - pixel_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - pixel_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / pixel-agents #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-pixel_session_at', '__Secure-pixel_session_at', 'pixel_session_at']
const RT_COOKIE_VARIANTS = ['__Host-pixel_session_rt', '__Secure-pixel_session_rt', 'pixel_session_rt']

// The Pixel portal (NAS) does NOT use Pixel Agents gateway session cookies — it is a
...
// Pixel Agents Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remotePixelAgentsPath = String(entry.remotePixelAgentsPath || '').trim()

  if (remotePixelAgentsPath) {
    out.remotePixelAgentsPath = remotePixelAgentsPath
  }
 ...
 *     `--profile`, so its `PIXEL_AGENTS_HOME` scopes it.
 *
...
function pathWithGlobalRemoteProfile(path, profile, opts: ProfileRouteOptions = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://pixel-agents.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a pixel-agents session ACCESS-token cookie
...
/**
 * True if the cookie jar holds a live Pixel PORTAL (Privy) session — a non-empty
 ...
 * the Pixel Agents gateway session cookies, so cloud sign-in / discovery liveness
