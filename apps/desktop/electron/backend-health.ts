...
export interface PixelAgentsReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForPixelAgentsReady`).
   */
...
export async function waitForPixelAgentsReady(baseUrl: string, options: PixelAgentsReadyOptions): Promise<void> {
  ...
  throw new Error(`Pixel Agents backend did not become ready: ${detail}`)
}
