 ...
 *   1. PIXEL_AGENTS_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\pixel-agents\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect PIXEL_AGENTS_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.PIXEL_AGENTS_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'pixel-agents', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'pixel-agents', 'git', 'usr', 'bin', 'bash.exe'))
  }
