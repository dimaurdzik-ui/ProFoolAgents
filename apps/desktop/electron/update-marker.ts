...
export function markerPath(pixelAgentsHome) {
  return path.join(pixelAgentsHome, '.pixel-agents-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(pixelAgentsHome)
  let raw
 ...
 *
 * The Tauri-based pixel-agents-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(pixelAgentsHome, pid, { now = Date.now } = {}) {
  const file = markerPath(pixelAgentsHome)
