 ...
 * startPixelAgents() re-throw the cached error without re-attempting the connect —
...
/**
 * Whether a startPixelAgents() failure should latch into `backendStartFailure`.
 ...
 * Every subsequent `getConnection`/`api` call re-runs `startPixelAgents`, re-emits
