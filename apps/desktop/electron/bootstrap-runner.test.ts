...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'pixel-agents-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/pixel-agents-runner-test',
    sourceRepoRoot: null,
    pixelAgentsHome: '/tmp/pixel-agents-runner-test',
    logRoot: '/tmp/pixel-agents-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'pixel-agents', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'pixel-agents')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/pixel-agents',
      pixelAgentsHome: '/tmp/pixel-agents'
    }),
    [
      '--dir',
      '/tmp/pixel-agents',
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/pixel-agents', '--pixel-agents-home', '/tmp/pixel-agents', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/pixel-agents',
      pixelAgentsHome: '/tmp/home'
    }),
    ['--dir', '/tmp/pixel-agents', '--pixel-agents-home', '/tmp/home', '--branch', 'main']
  )
})
