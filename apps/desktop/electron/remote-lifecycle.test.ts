...
function ownedLock(over: any = {}) {
  return {
    ...
    port: 40000,
    profile: '',
    pixelAgentsPath: '~/.local/bin/pixel-agents',
    pixelAgentsHome: '~/.pixel-agents',
...
test('locatePixelAgents prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/pixel-agents/, 'OK']])
  assert.equal(await locatePixelAgents(ssh, '/opt/pixel-agents'), '/opt/pixel-agents')
})

test('locatePixelAgents throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong pixel-agents" bug.
  const ssh = fakeSsh([
    [/command -v pixel-agents/, '/home/u/.local/bin/pixel-agents\n'],
    [/\[ -x .*\.local\/bin\/pixel-agents/, 'OK']
  ])

  await assert.rejects(
    () => locatePixelAgents(ssh, '/bad/path/pixel-agents'),
    (err: any) => {
      assert.equal(err.kind, 'pixel-agents-not-found')
      assert.match(err.message, /\/bad\/path\/pixel-agents/)
...
test('locatePixelAgents falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v pixel-agents/, '/home/u/.local/bin/pixel-agents\n'],
    [/\[ -x .*\.local\/bin\/pixel-agents/, 'OK']
  ])

  assert.equal(await locatePixelAgents(ssh, ''), '/home/u/.local/bin/pixel-agents')
})

test('locatePixelAgents canonicalizes an installer wrapper to its executable target', async () => {
  const ssh = fakeSsh([
    [/command -v pixel-agents/, '/home/u/.local/bin/pixel-agents\n'],
    [/\[ -x .*\.local\/bin\/pixel-agents/, 'OK'],
    [/python3 -c/, '/home/u/.pixel-agents/pixel-agents/venv/bin/pixel-agents\n']
  ])

  assert.equal(await locatePixelAgents(ssh, ''), '/home/u/.pixel-agents/pixel-agents/venv/bin/pixel-agents')
})

test('locatePixelAgents falls back to ~/.local/bin/pixel-agents when the login-shell probe misses', async () => {
  ...
  assert.equal(await locatePixelAgents(ssh, ''), '~/.local/bin/pixel-agents')
})

test('locatePixelAgents tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/pixel-agents/, 'OK']])
  assert.equal(await locatePixelAgents(ssh, ''), '~/.pixel-agents/pixel-agents/venv/bin/pixel-agents')
})

test('locatePixelAgents throws a pixel-agents-not-found error with an install hint', async () => {
  const ssh = fakeSsh([]) // nothing is executable
  await assert.rejects(
    () => locatePixelAgents(ssh, ''),
    (err: any) => {
      assert.equal(err.kind, 'pixel-agents-not-found')
...
test('locatePixelAgents uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v pixel-agents/, '/x/pixel-agents'],
    [/\[ -x/, 'OK']
  ])

  await locatePixelAgents(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.pixel-agents/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.pixel-agents/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(
    spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    `~/.pixel-agents/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`
  )
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/pixel-agents'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/pixel-agents serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(
    await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/pixel-agents'),
    true
  )
  assert.equal(
...
test('cleanupStale kills ONLY a provably-ours pid, always drops the lockfile', async () => {
  ...
  await cleanupStale(notOurs, OWNERSHIP_ID, {
    pid: 5,
    spawnNonce: SPAWN_NONCE,
    pixelAgentsPath: '/x/pixel-agents',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE)
  })
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/pixel-agents', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('PIXEL_AGENTS_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('PIXEL_AGENTS_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('PIXEL_AGENTS_DASHBOARD_READY port=8765')?.[1], '8765')
})
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nPIXEL_AGENTS_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'PIXEL_AGENTS_DASHBOARD_READY port=51999\n']
  ])
...
test('connect() respawns when the lockfile pixelAgentsPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ pixelAgentsPath: '/old/stale/pixel-agents', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const ssh = fakeSsh([
    ...
    [/kill -0/, 'ALIVE'],
    [/print\("OWNED"/, 'FOREIGN\n'],
    [/--version/, 'Pixel Agents v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/cat .*\.log/, 'PIXEL_AGENTS_DASHBOARD_READY port=52050\n']
  ])

  const result = await connect(
    connectDeps(ssh, { reuseToken, remotePixelAgentsPath: '/new/pixel-agents', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'PIXEL_AGENTS_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes pixelAgentsHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/PIXEL_AGENTS_HOME/, '/home/alice/.pixel-agents\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'PIXEL_AGENTS_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"pixelAgentsHome":"\/home\/alice\/\.pixel-agents"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'PIXEL_AGENTS_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'PIXEL_AGENTS_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/pixel-agents'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.pixel-agents/pixel-agents/venv/bin/pixel-agents'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/pixel-agents'))
  assert.doesNotThrow(() => validateRemotePath('~/.pixel-agents/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/pixel-agents'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('pixel-agents'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/pixel-agents'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/pixel-agents\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/pixel-agents\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/pixel-agents\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/pixel-agents', '/usr/`id`/pixel-agents', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.pixel-agents/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/pixel-agents')
  assert.ok(result.includes('/usr/local/bin/pixel-agents'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/pixel-agents')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('PIXEL_AGENTS_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/pixel-agents', 'work', {
    tokenFilePath: `~/.pixel-agents/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /\.pixel-agents\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/pixel-agents', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('readLockfile rejects a log path outside the exact ownership and spawn path', async () => {
  const lock = ownedLock({ logPath: '~/.pixel-agents/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.pixel-agents/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/pixel-agents serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/pixel-agents serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(
    await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/pixel-agents'),
    false
  )
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/pixel-agents'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/pixel-agents'), false)
})
