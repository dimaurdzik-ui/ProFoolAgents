...
export const ZOOM_STORAGE_KEY = 'pixel-agents:desktop:zoomLevel'
 ...
 * but forgot to emit 'pixel-agents:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('pixel-agents:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
