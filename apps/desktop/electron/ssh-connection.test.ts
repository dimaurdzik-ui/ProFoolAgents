...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line =
    'setsid env PIXEL_AGENTS_DASHBOARD_SESSION_TOKEN=abc123deadbeef PIXEL_AGENTS_DESKTOP=1 pixel-agents dashboard'

  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /PIXEL_AGENTS_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /PIXEL_AGENTS_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Pixel-Agents-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Pixel-Agents-Session-Token: hdr_888'), /X-Pixel-Agents-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Pixel-Agents-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('pixel-agents', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v pixel-agents', 15000)
  assert.equal(args[args.length - 1], 'command -v pixel-agents')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `pixel-agents-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `pixel-agents-ssh-test-${process.pid}-${Date.now()}`)
