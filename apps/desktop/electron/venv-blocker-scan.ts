...
const SCAN_MODULE = 'pixel_cli._scan_venv_blockers'
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Pixel Agents process is using this installation.',
    '',
...
export function formatProbeFailedMessage(): string {
  return (
    'Update aborted: Desktop could not verify the Pixel Agents installation is free.\n' +
    '\n' +
    'Close other Pixel Agents windows and terminals, then retry.  If the problem\n' +
    'persists, run `pixel-agents update` in a terminal for detailed diagnostics.'
  )
}
