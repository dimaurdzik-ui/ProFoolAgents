...
//   PIXEL_AGENTS_WIN_SSH_HOST   ssh alias/host of the Windows box
//   PIXEL_AGENTS_WIN_SSH_USER   remote user
//   PIXEL_AGENTS_WIN_SSH_PIXEL_AGENTS absolute path to the remote pixel-agents.exe under test
const liveHost = process.env.PIXEL_AGENTS_WIN_SSH_HOST || ''
const liveUser = process.env.PIXEL_AGENTS_WIN_SSH_USER || ''
const configuredPixelAgents = process.env.PIXEL_AGENTS_WIN_SSH_PIXEL_AGENTS || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Pixel-Agents-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredPixelAgents)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredPixelAgents.replace('pixel-agents.exe', 'python.exe')}' -m pixel_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredPixelAgents.replace('pixel-agents.exe', 'python.exe')
        const terminate = `& '${python}' -m pixel_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.pixelAgentsPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m pixel_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m pixel_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
