 ...
 * the three user-facing uninstall modes to the `pixel-agents uninstall` CLI flags,
 ...
 *              `pixel-agents uninstall --gui --yes`
 *   - 'lite' → remove the GUI + agent code, KEEP user data (config / sessions
 *              / .env) for a future reinstall. `pixel-agents uninstall --yes`
 *   - 'full' → remove everything: GUI + agent + all user data.
 *              `pixel-agents uninstall --full --yes`
 *
 * Why a detached cleanup script: 'lite'/'full' delete the very venv the
 * `pixel-agents` command runs from, and every mode may need to delete the running
...
/**
 * Map an uninstall mode to the `python -m pixel_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `pixel_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'pixel_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Pixel Agents.app/Contents/MacOS/Pixel Agents  → …/Pixel Agents.app
 *   Windows: …\Pixel Agents\Pixel Agents.exe                 → …\Pixel Agents  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Pixel Agents.app/Contents/MacOS/Pixel Agents → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Pixel Agents.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Pixel Agents.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Pixel Agents$/i.test(dir) || /[\\/]pixel-agents-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/pixel-agents
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import pixel_cli`
...
function buildPosixCleanupScript({
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export PIXEL_AGENTS_HOME=${q(pixelAgentsHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import pixel_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Pixel Agents install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "PIXEL_AGENTS_HOME=${String(pixelAgentsHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
