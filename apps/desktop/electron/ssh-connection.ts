...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(PIXEL_AGENTS_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Pixel-Agents-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.pixel-agents/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'pixel-agents-desktop-ssh')
  }

  return path.join(os.homedir(), '.pixel-agents', 'desktop-ssh')
}
