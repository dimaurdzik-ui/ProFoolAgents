...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Pixel Agents doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in pixel_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `pixel-agents model` uses — instead of
...
export type PixelAgentsConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `pixel-agents model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** Pixel only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Pixel only: paid models a free-tier user cannot select (shown disabled). */
...
// folders. Mirrors pixel_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Pixel Agents, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `pixel-agents tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧ Pixel entitlement ∧
 ...
 *  not Pixel Agents); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/pixel-agents/update/check` — the backend's own update state.
...
/** One Pixel-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the Pixel Tool Gateway as a result of
   *  switching the main provider to Pixel. Empty unless provider === 'pixel'
