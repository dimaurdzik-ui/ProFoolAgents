import { JsonRpcGatewayClient } from '@pixel-agents/shared'
...
// "Timed out connecting to Pixel Agents backend" that hangs the UI (#48504).
//
...
export class PixelAgentsGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Pixel Agents gateway connection closed',
      connectErrorMessage: 'Could not connect to Pixel Agents gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Pixel Agents gateway is not connected',
...
// the call; each pooled backend already has its own PIXEL_AGENTS_HOME, so no backend
...
/** Options for a plugin REST call — mirrors the app's own `pixelAgentsDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see PixelAgentsApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.pixelAgentsDesktop?.api) {
    throw new Error('Pixel Agents desktop bridge unavailable')
  }

  const suffix = pluginPathSuffix('pluginRest', path)

  return window.pixelAgentsDesktop.api<T>({
...
export function pluginSocket(pluginId: string, path: string, onMessage: (data: unknown) => void): () => void {
  ...
  const connect = async () => {
    const connection = await window.pixelAgentsDesktop.getConnection().catch(() => null)
...
export async function listSessions(
  ...
  const result = await window.pixelAgentsDesktop.api<PaginatedSessions>({
    path:
...
// "Error invoking remote method 'pixel-agents:api': Error: 404: ..." through the
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await window.pixelAgentsDesktop.api<SidebarSessionsResponse>({
...
export function setSessionArchived(id: string, archived: boolean, profile?: string | null): Promise<{ ok: boolean }> {
  return window.pixelAgentsDesktop.api<{ ok: boolean }>({
    ...(profile ? { profile } : {}),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return window.pixelAgentsDesktop.api<SessionSearchResponse>({
...
export function getSession(id: string, profile?: string | null): Promise<SessionInfo> {
  ...
  return window.pixelAgentsDesktop.api<SessionInfo>({
    ...(profile ? { profile } : {}),
...
export function getSessionMessages(id: string, profile?: string | null): Promise<SessionMessagesResponse> {
  ...
  return window.pixelAgentsDesktop.api<SessionMessagesResponse>({
    ...(profile ? { profile } : {}),
...
export function renameSession(
  ...
  return window.pixelAgentsDesktop.api<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
...
export function getGlobalModelInfo(): Promise<ModelInfoResponse> {
  return window.pixelAgentsDesktop.api<ModelInfoResponse>({
    ...profileScoped(),
    path: '/api/model/info',
...
export function getStatus(): Promise<StatusResponse> {
  return window.pixelAgentsDesktop.api<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return window.pixelAgentsDesktop.api<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getPixelAgentsConfig(profile?: string): Promise<PixelAgentsConfig> {
  return window.pixelAgentsDesktop.api<PixelAgentsConfig>({
...
export function getPixelAgentsConfigRecord(): Promise<PixelAgentsConfigRecord> {
  return window.pixelAgentsDesktop.api<PixelAgentsConfigRecord>({
    ...profileScoped(),
    path: '/api/config'
  })
}

export function getPixelAgentsConfigDefaults(): Promise<PixelAgentsConfigRecord> {
  return window.pixelAgentsDesktop.api<PixelAgentsConfigRecord>({
    ...profileScoped(),
...
export function getPixelAgentsConfigSchema(): Promise<ConfigSchemaResponse> {
  return window.pixelAgentsDesktop.api<ConfigSchemaResponse>({
    ...profileScoped(),
    path: '/api/config/schema'
  })
}

export function savePixelAgentsConfig(config: PixelAgentsConfigRecord): Promise<{ ok: boolean }> {
  return window.pixelAgentsDesktop.api<{ ok: boolean }>({
    ...profileScoped(),
    path: '/api/config',
    method: 'PUT',
...
export function getMemoryProviderConfig(provider: string): Promise<MemoryProviderConfig> {
  return window.pixelAgentsDesktop.api<MemoryProviderConfig>({
    ...profileScoped(),
...
export function getEnvVars(): Promise<Record<string, EnvVarInfo>> {
  return window.pixelAgentsDesktop.api<Record<string, EnvVarInfo>>({
    ...profileScoped(),
    path: '/api/env'
  })
}
...
export function validateProviderCredential(
  ...
  return window.pixelAgentsDesktop.api<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return window.pixelAgentsDesktop.api<CustomEndpointsResponse>({
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return window.pixelAgentsDesktop.api<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return window.pixelAgentsDesktop.api<{ ok: boolean; provider: string; model: string }>({
...
export function revealEnvVar(key: string): Promise<{ key: string; value: string }> {
  return window.pixelAgentsDesktop.api<{ key: string; value: string }>({
    ...profileScoped(),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return window.pixelAgentsDesktop.api<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return window.pixelAgentsDesktop.api<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function startOAuthLogin(providerId: string): Promise<OAuthStartResponse> {
  return window.pixelAgentsDesktop.api<OAuthStartResponse>({
    ...profileScoped(),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return window.pixelAgentsDesktop.api<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function pollOAuthSession(providerId: string, sessionId: string): Promise<OAuthPollResponse> {
  return window.pixelAgentsDesktop.api<OAuthPollResponse>({
    ...profileScoped(),
...
export function startMemoryProviderOAuth(provider: string): Promise<MemoryProviderOAuthStatus> {
  return window.pixelAgentsDesktop.api<MemoryProviderOAuthStatus>({
    ...profileScoped(),
...
export function getSkills(): Promise<SkillInfo[]> {
  return window.pixelAgentsDesktop.api<SkillInfo[]>({
    ...profileScoped(),
    path: '/api/skills'
  })
}
...
export function getStarmapGraph(): Promise<StarmapGraph> {
  return window.pixelAgentsDesktop.api<StarmapGraph>({
    ...profileScoped(),
...
export function getLearningNode(id: string): Promise<LearningNodeDetail> {
  return window.pixelAgentsDesktop.api<LearningNodeDetail>({
    ...profileScoped(),
...
export function deleteLearningNode(id: string): Promise<{ message: string; ok: boolean }> {
  return window.pixelAgentsDesktop.api<{ message: string; ok: boolean }>({
    ...profileScoped(),
...
export function setSkillEnabled(
  ...
  return window.pixelAgentsDesktop.api<{ ok: boolean; name: string; enabled: boolean }>({
    ...profileScoped(),
...
export function testMcpServer(name: string): Promise<McpTestResult> {
  return window.pixelAgentsDesktop.api<McpTestResult>({
    ...profileScoped(),
 ...
 *  `savePixelAgentsConfig`, this REPLACES rather than deep-merges, so deletes,
...
export function authMcpServer(name: string): Promise<McpOAuthFlow> {
  return window.pixelAgentsDesktop.api<McpOAuthFlow>({
    ...profileScoped(),
...
export function getToolsets(): Promise<ToolsetInfo[]> {
  return window.pixelAgentsDesktop.api<ToolsetInfo[]>({
    ...profileScoped(),
...
export function getToolsetConfig(name: string): Promise<ToolsetConfig> {
  return window.pixelAgentsDesktop.api<ToolsetConfig>({
    ...profileScoped(),
...
export function getToolsetModels(name: string, provider?: string): Promise<ToolsetModelsResponse> {
  ...
  return window.pixelAgentsDesktop.api<ToolsetModelsResponse>({
    ...profileScoped(),
...
export function selectToolsetModel(
  ...
  return window.pixelAgentsDesktop.api<{ ok: boolean; name: string; model: string }>({
    ...profileScoped(),
...
export interface SelectToolsetProviderResponse {
  ...
  capability?: string
  /** Present (true) when a managed Pixel row was selected but the Portal
   *  entitlement is missing — the row won't activate until the user signs
   *  in to Pixel Portal. */
  needs_pixel_auth?: boolean
  /** The managed feature key (e.g. "browser") when needs_pixel_auth is set. */
  feature?: string
}
...
export function selectToolsetProvider(
  ...
  return window.pixelAgentsDesktop.api<SelectToolsetProviderResponse>({
    ...profileScoped(),
...
export function runToolsetPostSetup(name: string, key: string): Promise<ActionResponse & { key: string }> {
  return window.pixelAgentsDesktop.api<ActionResponse & { key: string }>({
    ...profileScoped(),
...
export function getTerminalBackends(): Promise<TerminalBackendsResponse> {
  return window.pixelAgentsDesktop.api<TerminalBackendsResponse>({
    ...profileScoped(),
...
export function selectTerminalBackend(backend: string): Promise<{ ok: boolean; backend: string }> {
  return window.pixelAgentsDesktop.api<{ ok: boolean; backend: string }>({
    ...profileScoped(),
...
export function getComputerUseStatus(): Promise<ComputerUseStatus> {
  return window.pixelAgentsDesktop.api<ComputerUseStatus>({
    ...profileScoped(),
...
export function grantComputerUsePermissions(): Promise<ActionResponse> {
  return window.pixelAgentsDesktop.api<ActionResponse>({
    ...profileScoped(),
...
export function getMessagingPlatforms(): Promise<MessagingPlatformsResponse> {
  return window.pixelAgentsDesktop.api<MessagingPlatformsResponse>({
...
export function updateMessagingPlatform(
  ...
  return window.pixelAgentsDesktop.api<{ ok: boolean; platform: string }>({
...
export function testMessagingPlatform(platformId: string): Promise<MessagingPlatformTestResponse> {
  return window.pixelAgentsDesktop.api<MessagingPlatformTestResponse>({
...
export function getPairing(): Promise<PairingResponse> {
  return window.pixelAgentsDesktop.api<PairingResponse>({
    ...profileScoped(),
    path: '/api/pairing'
  })
}
...
export function approvePairing(platform: string, requestId: string): Promise<{ ok: boolean; user: PairingUser }> {
  return window.pixelAgentsDesktop.api<{ ok: boolean; user: PairingUser }>({
    ...profileScoped(),
...
export function getWebhooks(): Promise<WebhooksResponse> {
  return window.pixelAgentsDesktop.api<WebhooksResponse>({
    ...profileScoped(),
    path: '/api/webhooks'
  })
}
...
export function enableWebhooks(): Promise<WebhookEnableResponse> {
  return window.pixelAgentsDesktop.api<WebhookEnableResponse>({
    ...profileScoped(),
...
export function createWebhook(body: WebhookCreatePayload): Promise<WebhookCreateResponse> {
  return window.pixelAgentsDesktop.api<WebhookCreateResponse>({
    ...profileScoped(),
    path: '/api/webhooks',
    method: 'POST',
    body
...
export function setWebhookEnabled(
  ...
  return window.pixelAgentsDesktop.api<{ enabled: boolean; name: string; ok: boolean }>({
    ...profileScoped(),
...
// Cron jobs are stored per-profile (<PIXEL_AGENTS_HOME>/cron/jobs.json), and the
...
export function getCronJobs(profile?: string): Promise<CronJob[]> {
  ...
  return window.pixelAgentsDesktop.api<CronJob[]>({
    ...profileScoped(),
...
export async function getCronJobRuns(jobId: string, limit = 20): Promise<SessionInfo[]> {
  const { runs } = await window.pixelAgentsDesktop.api<{ runs: SessionInfo[] }>({
    ...profileScoped(),
...
export async function getCronDeliveryTargets(): Promise<CronDeliveryTarget[]> {
  const { targets } = await window.pixelAgentsDesktop.api<{ targets: CronDeliveryTarget[] }>({
    ...profileScoped(),
...
export function getAutomationBlueprints(): Promise<{ blueprints: AutomationBlueprint[] }> {
  return window.pixelAgentsDesktop.api<{ blueprints: AutomationBlueprint[] }>({
    ...profileScoped(),
...
export function getProfiles(): Promise<ProfilesResponse> {
  return window.pixelAgentsDesktop.api<ProfilesResponse>({
    path: '/api/profiles',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
}
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return window.pixelAgentsDesktop.api<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string): Promise<{ ok: boolean; path: string }> {
  return window.pixelAgentsDesktop.api<{ ok: boolean; path: string }>({
    path: `/api/profiles/${encodeURIComponent(name)}`,
    method: 'DELETE'
  })
}
...
export function getProfileSoul(name: string): Promise<ProfileSoul> {
  return window.pixelAgentsDesktop.api<ProfileSoul>({
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return window.pixelAgentsDesktop.api<ProfileSetupCommand>({
    path: `/api/profiles/${encodeURIComponent(name)}/setup-command`
  })
}

export function getUsageAnalytics(days = 30): Promise<AnalyticsResponse> {
  return window.pixelAgentsDesktop.api<AnalyticsResponse>({
    ...profileScoped(),
...
export function getGlobalModelOptions(opts?: {
  ...
  return window.pixelAgentsDesktop.api<ModelOptionsResponse>({
    ...profileScoped(),
...
export interface RecommendedDefaultModel {
  provider: string
  model: string
  /** True/false for Pixel (free vs paid tier); null for other providers. */
  free_tier: boolean | null
}

// Recommended default model for a freshly-authenticated provider. Mirrors the
// curation `pixel-agents model` does — for Pixel it honors the free/paid tier so a
// free user gets a free model instead of a paid default.
export function getRecommendedDefaultModel(provider: string): Promise<RecommendedDefaultModel> {
  return window.pixelAgentsDesktop.api<RecommendedDefaultModel>({
    ...profileScoped(),
...
export function getAuxiliaryModels(): Promise<AuxiliaryModelsResponse> {
  return window.pixelAgentsDesktop.api<AuxiliaryModelsResponse>({
    ...profileScoped(),
...
export function getMoaModels(): Promise<MoaConfigResponse> {
  return window.pixelAgentsDesktop.api<MoaConfigResponse>({
    ...profileScoped(),
    path: '/api/model/moa'
  })
}
...
export function saveMoaModels(body: MoaConfigResponse): Promise<MoaConfigResponse & { ok: boolean }> {
  return window.pixelAgentsDesktop.api<MoaConfigResponse & { ok: boolean }>({
    ...profileScoped(),
    path: '/api/model/moa',
    method: 'PUT',
    body
...
export function setModelAssignment(body: ModelAssignmentRequest): Promise<ModelAssignmentResponse> {
  return window.pixelAgentsDesktop.api<ModelAssignmentResponse>({
    ...profileScoped(),
    path: '/api/model/set',
    method: 'POST',
    body
...
export function updatePixelAgents(): Promise<ActionResponse> {
  return window.pixelAgentsDesktop.api<ActionResponse>({
    ...profileScoped(),
    path: '/api/pixel-agents/update',
    method: 'POST'
  })
}
...
export function checkPixelAgentsUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return window.pixelAgentsDesktop.api<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/pixel-agents/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200): Promise<ActionStatusResponse> {
  return window.pixelAgentsDesktop.api<ActionStatusResponse>({
    ...profileScoped(),
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return window.pixelAgentsDesktop.api<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return window.pixelAgentsDesktop.api<AudioSpeakResponse>({
    ...profileScoped(),
...
export function getElevenLabsVoices(): Promise<ElevenLabsVoicesResponse> {
  return window.pixelAgentsDesktop.api<ElevenLabsVoicesResponse>({
...
// ---------------------------------------------------------------------------
// Skills hub — search / preview / scan / install (parity with `pixel-agents skills`
...
export function getSkillHubSources(): Promise<SkillHubSourcesResponse> {
  return window.pixelAgentsDesktop.api<SkillHubSourcesResponse>({
    ...profileScoped(),
...
export function searchSkillsHub(query: string, source = 'all', limit = 20): Promise<SkillHubSearchResponse> {
  ...
  return window.pixelAgentsDesktop.api<SkillHubSearchResponse>({
    ...profileScoped(),
...
export function previewSkillHub(identifier: string): Promise<SkillHubPreview> {
  return window.pixelAgentsDesktop.api<SkillHubPreview>({
    ...profileScoped(),
...
export function scanSkillHub(identifier: string): Promise<SkillHubScanResult> {
  return window.pixelAgentsDesktop.api<SkillHubScanResult>({
    ...profileScoped(),
...
// `pixel-agents mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via savePixelAgentsConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return window.pixelAgentsDesktop.api<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function getMcpCatalog(): Promise<McpCatalogResponse> {
  return window.pixelAgentsDesktop.api<McpCatalogResponse>({
    ...profileScoped(),
...
export function installMcpCatalogEntry(
  ...
  return window.pixelAgentsDesktop.api<{
    ok: boolean
    name?: string
    pid?: number
    action?: string
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `pixel-agents memory` / `pixel-agents curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return window.pixelAgentsDesktop.api<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return window.pixelAgentsDesktop.api<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return window.pixelAgentsDesktop.api<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return window.pixelAgentsDesktop.api<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `pixel-agents doctor` / `pixel-agents security
// audit` / `pixel-agents backup` / `pixel-agents debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return window.pixelAgentsDesktop.api<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return window.pixelAgentsDesktop.api<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return window.pixelAgentsDesktop.api<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return window.pixelAgentsDesktop.api<DebugShareResponse>({
    path: '/api/ops/debug-share',
    method: 'POST',
    body: {},
    // Synchropixel upload of report + logs to the paste service.
    timeoutMs: 120_000
  })
}
