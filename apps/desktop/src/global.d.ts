import type { GatewayWsUrlResult } from '@pixel-agents/shared'
...
declare global {
  interface Window {
    pixelAgentsDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<PixelAgentsConnection>
      ...
      // Pixel Agents Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new PIXEL_AGENTS_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: PixelAgentsApiRequest) => Promise<T>
      notify: (payload: PixelAgentsNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<PixelAgentsReadFileTextResult>
      selectPaths: (options?: PixelAgentsSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<PixelAgentsPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<PixelAgentsPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<PixelAgentsPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: PixelAgentsActiveWork) => void
      setTitleBarTheme?: (payload: PixelAgentsTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<PixelAgentsReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<PIXEL_AGENTS_HOME>/desktop-plugins),
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<PixelAgentsGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<PixelAgentsGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<PixelAgentsGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<PixelAgentsRepoStatus | null>
        ...
        review: {
          list: (
            repoPath: string,
            scope: PixelAgentsReviewScope,
            baseRef?: null | string
          ) => Promise<PixelAgentsReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: PixelAgentsReviewScope,
          ...
          shipInfo: (repoPath: string) => Promise<PixelAgentsReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: PixelAgentsTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<PixelAgentsTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: PixelAgentsWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: PixelAgentsPreviewFileChanged) => void) => () => void
...
export interface PixelAgentsTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface PixelAgentsTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `pixel-agents update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  pixelAgentsRoot?: string
...
export interface PixelAgentsConnection {
  baseUrl: string
  ...
  remotePixelAgentsVersion?: string
...
export interface PixelAgentsTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface PixelAgentsActiveWork {
  count: number
  titles: string[]
}

export interface PixelAgentsWindowState {
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
  nativeOverlayWidth: number
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Pixel Agents Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Pixel Agents Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Pixel Agents Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // Whether the OAuth partition holds a live Pixel portal (Privy) session — the
  // portal authenticates via Privy, so this reflects the privy-token cookie, NOT
  // the pixel-agents gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Pixel Agents Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface PixelAgentsApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface PixelAgentsNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface PixelAgentsPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface PixelAgentsReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface PixelAgentsPreviewWatch {
  id: string
  path: string
}
...
export interface PixelAgentsGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface PixelAgentsGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
// preselect it.
export interface PixelAgentsGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface PixelAgentsRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface PixelAgentsRepoStatus {
  branch: null | string
  ...
  files: PixelAgentsRepoStatusFile[]
}
...
// turn began.
export type PixelAgentsReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface PixelAgentsReviewFile {
  path: string
  added: number
  removed: number
...
export interface PixelAgentsReviewList {
  files: PixelAgentsReviewFile[]
...
export interface PixelAgentsReviewPr {
  url: string
  state: string
  number: number
}
...
export interface PixelAgentsReviewShipInfo {
  ghReady: boolean
  pr: PixelAgentsReviewPr | null
}

export interface PixelAgentsReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface PixelAgentsReadDirResult {
  entries: PixelAgentsReadDirEntry[]
  error?: string
}

export interface PixelAgentsPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface PixelAgentsSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
