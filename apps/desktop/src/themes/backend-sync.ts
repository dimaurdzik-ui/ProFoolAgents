/**
 * Live skin sync from the Pixel Agents backend.
 *
 * The backend resolves the active skin (built-in or `$PIXEL_AGENTS_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Pixel Agents
 ...
 */
export function ingestBackendSkin(skin: PixelAgentsSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  // (pixel), so we never register a converted theme under `default`. It is still a
  // valid apply TARGET though: a runtime switch back to `default` must repaint the
  // desktop to its own default (setTheme normalizes `default` → pixel). So we only
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as PixelAgentsSkin)
