...
import { BUILTIN_THEME_LIST, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, pixelTheme } from './presets'
...
const SKIN_KEY = 'pixel-agents-desktop-theme-v2'
const MODE_KEY = 'pixel-agents-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'pixel-agents-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'pixel-agents-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'pixel-agents-desktop-active-profile-v1'
const RETIRED_SKINS = new Set(['pixel-light', 'default', 'gold'])
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? pixelTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...pixelTheme.typography, ...theme.typography }
  ...
  root.dataset.pixelAgentsTheme = skinName
  root.dataset.pixelAgentsMode = rendered
  ...
  window.pixelAgentsDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('pixel-agents-boot-background', chromeBg)
    window.localStorage.setItem('pixel-agents-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.pixelAgentsThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.pixelAgentsDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Pixel Agents authoring/activating a skin from a
