 ...
 *   (`@pixel-agents/plugin-sdk` / `react*` -> live shim blobs, see sdk/runtime.ts)
 *   -> blob `import()` -> validate default PixelAgentsPlugin -> register(ctx)
 *
 ...
 * and `<pixel-agents home>/desktop-plugins/<name>/plugin.js` on disk — the door the
...
import { createPluginContext, type PixelAgentsPlugin } from './plugin'
...
/** Rewrite ONLY mapped import specifiers (@pixel-agents/plugin-sdk, react*) to their
...
export async function loadRuntimePlugin(
  ...
  try {
    ...
    if (unsupported.length > 0) {
      throw new Error(
        `unsupported import${unsupported.length > 1 ? 's' : ''}: ${unsupported.join(', ')} — ` +
          `runtime plugins may only import @pixel-agents/plugin-sdk and react`
      )
    }

    const url = URL.createObjectURL(new Blob([rewriteSpecifiers(source)], { type: 'text/javascript' }))

    let mod: { default?: PixelAgentsPlugin }
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      throw new Error(`${origin} has no valid default PixelAgentsPlugin export`)
    }
...
// ---------------------------------------------------------------------------
// The on-disk plugin door: `<pixel-agents home>/desktop-plugins/<name>/plugin.js`
...
async function scanDiskPlugins(): Promise<void> {
  ...
  try {
    ...
    // connected backend — a remote backend's pixel_home is a remote path and
...
export function watchRuntimePlugins(): void {
  ...
  const startDirWatch = async (): Promise<boolean> => {
    ...
    try {
      ...
      // pixel_home, which is a remote path in remote mode (#66899).
      const root = await desktop.desktopPluginsRoot?.()
