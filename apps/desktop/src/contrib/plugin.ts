 ...
 * `PixelAgentsPlugin`; it never touches the registry directly — it receives a
...
import { pluginRest, type PluginRestOptions, pluginSocket } from '@/pixel-agents'

import { registry } from './registry'
import type { Contribution } from './types'

export type { PluginRestOptions } from '@/pixel-agents'
 ...
 *  under `pixel-agents.plugin.<id>.` — plugins can't read or clobber each other. */
...
export interface PixelAgentsPlugin {
...
function createPluginStorage(pluginId: string): PluginStorage {
  const scoped = (key: string) => `pixel-agents.plugin.${pluginId}.${key}`
