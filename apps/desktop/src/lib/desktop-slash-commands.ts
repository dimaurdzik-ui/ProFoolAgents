...
/** Why a known Pixel Agents command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Pixel Agents profile', surface: action('profile') },
  {
    name: '/skin',
  ...
  { name: '/version', description: 'Show Pixel Agents version', surface: exec() },
...
function isKnownPixelAgentsSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Pixel Agents' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownPixelAgentsSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Pixel Agents and were never asked for. It is
