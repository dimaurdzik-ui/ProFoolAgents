...
export async function resolveMediaDisplaySrc(path: string): Promise<string> {
  ...
  if (window.pixelAgentsDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.pixelAgentsDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.pixelAgentsDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
export async function resolveMediaPlaybackSrc(path: string): Promise<string> {
  ...
  if (window.pixelAgentsDesktop && ['audio', 'video'].includes(mediaKind(path))) {
...
export function mediaStreamUrl(path: string): string {
  return `pixel-agents-media://stream/${encodeURIComponent(filePathFromMediaPath(path))}`
}
...
// (workspace, skills, ~/.pixel-agents/cache, etc.); /api/media is intentionally
