import type {
  PixelAgentsGitBaseBranch,
  PixelAgentsGitBranch,
  PixelAgentsGitWorktree,
  PixelAgentsRepoStatus,
  PixelAgentsReviewList,
  PixelAgentsReviewShipInfo
} from '@/global'
...
// (window.pixelAgentsDesktop.git); on a remote gateway that's the wrong filesystem,
...
type GitBridge = NonNullable<NonNullable<Window['pixelAgentsDesktop']>['git']>

function desktopApi<T>(path: string, body?: Record<string, unknown>): Promise<T> {
  const desktop = window.pixelAgentsDesktop

  if (!desktop) {
    throw new Error('Pixel Agents Desktop bridge is unavailable')
  }
...
const remoteGit: GitBridge = {
  worktreeList: async repoPath =>
    (await gitGet<{ worktrees: PixelAgentsGitWorktree[] }>('worktrees', { path: repoPath })).worktrees,
  ...
  branchList: async repoPath =>
    (await gitGet<{ branches: PixelAgentsGitBranch[] }>('branches', { path: repoPath })).branches,

  baseBranchList: async repoPath =>
    (await gitGet<{ branches: PixelAgentsGitBaseBranch[] }>('base-branches', { path: repoPath })).branches,

  repoStatus: repoPath => gitGet<PixelAgentsRepoStatus | null>('status', { path: repoPath }),
  ...
  review: {
    list: (repoPath, scope, baseRef) =>
      gitGet<PixelAgentsReviewList>('review/list', { base: baseRef, path: repoPath, scope }),
    ...
    shipInfo: repoPath => gitGet<PixelAgentsReviewShipInfo>('review/ship-info', { path: repoPath }),
...
export function desktopGit(): GitBridge | undefined {
  ...
  return isDesktopFsRemoteMode() ? remoteGit : window.pixelAgentsDesktop?.git
}
