...
function installDesktopBridge(partial: Partial<Window['pixelAgentsDesktop']> = {}) {
  desktopWindow.pixelAgentsDesktop = {
    fetchLinkTitle: vi.fn().mockResolvedValue(''),
    openExternal: vi.fn().mockResolvedValue(undefined),
    ...partial
  } as unknown as Window['pixelAgentsDesktop']
}
...
function installTitleBridge(title: string) {
  ...
  installDesktopBridge({ fetchLinkTitle: bridge as unknown as Window['pixelAgentsDesktop']['fetchLinkTitle'] })
...
afterEach(() => {
  ...
  if (initialPixelAgentsDesktop) {
    desktopWindow.pixelAgentsDesktop = initialPixelAgentsDesktop
  } else {
    delete desktopWindow.pixelAgentsDesktop
  }
})
...
describe('external link helpers', () => {
  ...
  it('opens links via the desktop bridge', () => {
    ...
    installDesktopBridge({ openExternal: openExternal as unknown as Window['pixelAgentsDesktop']['openExternal'] })
  ...
  it('prefixes a pretty link to a known host with its brand glyph', () => {
    ...
    const url = 'https://github.com/PixelResearch/pixel-agents/pull/123'
