import type {
  PixelAgentsConnection,
  PixelAgentsReadDirResult,
  PixelAgentsReadFileTextResult,
  PixelAgentsSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: PixelAgentsConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<PixelAgentsReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<PixelAgentsReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<PixelAgentsReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<PixelAgentsReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: PixelAgentsSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
