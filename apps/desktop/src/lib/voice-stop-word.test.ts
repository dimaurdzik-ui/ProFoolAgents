...
describe('isVoiceStopCommand', () => {
  ...
  it('matches stop commands addressed to Pixel Agents', () => {
    for (const phrase of [
      'pixel-agents stop',
      'hey pixel-agents stop',
      'hey pixel-agents, stop',
      'ok stop',
      'okay stop'
    ]) {
  ...
  it('does not match bare address words or empty input', () => {
    for (const phrase of ['', '  ', 'pixel-agents', 'hey pixel-agents', 'ok', 'okay', 'hey']) {
      expect(isVoiceStopCommand(phrase)).toBe(false)
    }
  })
...
describe('interceptsTypedVoiceStop', () => {
  it('intercepts a typed bare stop command while the conversation is active', () => {
    for (const text of ['stop', 'Stop.', 'never mind', 'hey pixel-agents, stop']) {
