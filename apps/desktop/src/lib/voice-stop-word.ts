...
//
// When someone is in a hands-free "Hey Pixel Agents" voice chat, the natural way to
...
// phrase (optionally addressed to Pixel Agents), so a real turn that merely contains
...
// Optional address prefixes so "pixel-agents stop" / "ok stop" / "hey pixel-agents, stop"
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("pixel-agents") is not a stop command on its own.
      continue
    }
 ...
 * to Pixel Agents). Returns false for anything that merely contains "stop" as part of
