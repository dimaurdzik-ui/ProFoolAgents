...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `PixelAgentsPlugin` and
...
`$PIXEL_AGENTS_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`pixel-agents-desktop-plugins` skill.
