...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'pixel-agents:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.pixelAgentsDesktop?.saveImageFromUrl) {
        if (await window.pixelAgentsDesktop.saveImageFromUrl(src)) {
