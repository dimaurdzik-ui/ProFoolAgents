...
vi.mock('@/pixel-agents', () => ({
  getPixelAgentsConfigRecord: vi.fn(async () => ({})),
  savePixelAgentsConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye pixel-agents', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye pixel-agents')
  })
