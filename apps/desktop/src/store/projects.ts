...
import type { PixelAgentsGitBaseBranch, PixelAgentsGitBranch } from '@/global'
import { translateNow } from '@/i18n'
import { desktopDefaultCwd, isDesktopFsRemoteMode, selectDesktopPaths, writeDesktopFileText } from '@/lib/desktop-fs'
import { desktopGit } from '@/lib/desktop-git'
import { isMissingRpcMethod } from '@/lib/gateway-rpc'
import { persistentAtom } from '@/lib/persisted'
import { getPixelAgentsConfig, type PixelAgentsGateway } from '@/pixel-agents'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/pixel-agents'
...
const PROJECT_SCOPE_KEY = 'pixel-agents.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Pixel Agents gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== ($activeGatewayProfile.get() || 'default')) {
    throw new Error('Active Pixel Agents profile changed while connecting')
  }
...
async function refreshProjectTreeOn(gateway: PixelAgentsGateway): Promise<void> {
...
const repoScanStates = new WeakMap<PixelAgentsGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<PixelAgentsGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getPixelAgentsConfig(context.profile))
...
// repo, returning where Pixel Agents should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<PixelAgentsGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<PixelAgentsGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.pixelAgentsDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.pixelAgentsDesktop?.writeClipboard?.(path)
  }
}
