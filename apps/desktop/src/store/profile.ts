...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/pixel-agents'
...
// /api/profiles/active `current`). "default" is the root ~/.pixel-agents. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'pixel-agents.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'pixel-agents.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.pixelAgentsDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new PIXEL_AGENTS_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.pixelAgentsDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.pixelAgentsDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'pixel-agents.desktop.showAllProfiles'
...
// Switch to the default (root ~/.pixel-agents) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.pixelAgentsDesktop?.touchBackend?.(target).catch(() => undefined)
}
