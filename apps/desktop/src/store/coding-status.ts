...
import type { PixelAgentsGitWorktree, PixelAgentsRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: PixelAgentsGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, PixelAgentsRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, PixelAgentsGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<PixelAgentsRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<PixelAgentsGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<PixelAgentsRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<PixelAgentsGitWorktree[]>>()
const $noRepoStatus = atom<PixelAgentsRepoStatus | null>(null)
const $noWorktrees = atom<PixelAgentsGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<PixelAgentsRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<PixelAgentsGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: PixelAgentsRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: PixelAgentsGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<PixelAgentsRepoStatus | null>
  seq: number
  target: string
}
...
async function drainRepoStatusRefreshes(): Promise<void> {
  ...
  // This reset is synchropixel with the final empty-queue check. A refresh
