...
test('runBillingRecovery routes Pixel to in-app Settings, never an external link', () => {
  runBillingRecovery(makeBlock({ is_pixel: true, provider: 'pixel', provider_label: 'Pixel Portal' }))
...
test('billingCtaLabel picks the right verb per route', () => {
  ...
  expect(billingCtaLabel(makeBlock({ is_pixel: true }), copy)).toBe('Open billing')
  expect(billingCtaLabel(makeBlock({ is_pixel: false }), copy)).toBe('Add credits')
})
