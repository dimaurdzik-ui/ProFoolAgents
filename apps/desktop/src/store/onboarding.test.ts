...
function provider(id: string, name = id): OAuthProvider {
  return {
    cli_command: `pixel-agents login ${id}`,
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('pixel-agents-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('pixel-agents-desktop-onboarded-v1')).toBe('1')
  })
  ...
  it('enters setup when the selected OpenRouter credential is genuinely empty', async () => {
    ...
    expect($desktopOnboarding.get().reason).toContain('No usable credentials found for openrouter.')
    expect(window.localStorage.getItem('pixel-agents-desktop-onboarded-v1')).toBeNull()
  })
...
describe('OAuth onboarding', () => {
  ...
  it('clears stale readiness errors after OAuth succeeds and model confirmation is shown', async () => {
    ...
    installApiMock(async ({ body, path }: { body?: unknown; path: string }) => {
      ...
      if (path === '/api/providers/oauth/pixel/submit') {
      ...
      if (path.startsWith('/api/model/recommended-default?')) {
        return { provider: 'pixel', model, free_tier: false }
      }

      if (path === '/api/model/set') {
        return { ok: true, provider: 'pixel', model, gateway_tools: [] }
      }
    ...
    const requestGateway: OnboardingContext['requestGateway'] = async (method, params) => {
      ...
      if (method === 'setup.runtime_check') {
        expect(params).toEqual({ provider: 'pixel' })
    ...
    $desktopOnboarding.set(
      baseState({
        flow: {
          ...
          provider: provider('pixel', 'Pixel Portal'),
          start: {
        ...
        reason:
          'No access token found for Pixel Portal login. setup.status reports configured credentials, but runtime resolution still failed.',
        requested: true
      })
    )
    ...
    if (state.flow.status === 'confirming_model') {
      expect(state.flow.label).toBe('Pixel Portal')
