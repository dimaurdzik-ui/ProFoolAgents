...
const { getProfiles } = await import('@/pixel-agents')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/pixel-agents/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<PixelAgentsConnection> = {}): PixelAgentsConnection =>
  ({
    baseUrl: 'https://pixel-agents-roy.tail.ts.net',
    mode: 'remote',
    profile: 'vps-remote',
    ...over
  }) as PixelAgentsConnection

const localConn = (over: Partial<PixelAgentsConnection> = {}): PixelAgentsConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as PixelAgentsConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<PixelAgentsConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { pixelAgentsDesktop: { getConnection } })
