import type { ConnectionState } from '@pixel-agents/shared'
import { atom, computed } from 'nanostores'

import { lastVisibleMessageIsUser } from '@/app/chat/thread-loading'
import type { ContextSuggestion } from '@/app/types'
import type { PixelAgentsConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/pixel-agents'
...
const COMPOSER_MODEL_KEY = 'pixel-agents.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'pixel-agents.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'pixel-agents.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'pixel-agents.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'pixel-agents.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'pixel-agents.desktop.lastSessionId'
...
const LAST_ROUTE_KEY = 'pixel-agents.desktop.lastRoute'
...
function workspaceCwdKey(connection: PixelAgentsConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.pixelAgentsDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.pixelAgentsDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<PixelAgentsConnection | null>(null)
...
export const setConnection = (next: Updater<PixelAgentsConnection | null>) => updateAtom($connection, next)
...
// until config loads, and re-seeded on every profile switch by usePixelAgentsConfig.
