 ...
 * app's bounds and stays visible while Pixel Agents is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'pixel-agents.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'pixel-agents.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.pixelAgentsDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.pixelAgentsDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.pixelAgentsDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.pixelAgentsDesktop?.petOverlay?.close()
}
