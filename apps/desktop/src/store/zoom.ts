...
export function setZoomPercent(percent: number): void {
  window.pixelAgentsDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.pixelAgentsDesktop?.zoom) {
  void window.pixelAgentsDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.pixelAgentsDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
