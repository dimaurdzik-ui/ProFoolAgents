...
import type { PixelAgentsReviewFile, PixelAgentsReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Pixel Agents' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'pixel-agents.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'pixel-agents.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'pixel-agents.desktop.reviewTreeMode'
const SELECTED_KEY = 'pixel-agents.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<PixelAgentsReviewFile[]>([])
...
export const $reviewShipInfo = atom<PixelAgentsReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['pixelAgentsDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: PixelAgentsReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly PixelAgentsReviewFile[], path: string): PixelAgentsReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.pixelAgentsDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.pixelAgentsDesktop?.openExternal?.(url)
    }
