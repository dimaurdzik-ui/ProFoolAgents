...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/pixel-agents'
...
const CONFIGURED_CACHE_KEY = 'pixel-agents-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'pixel-agents-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Pixel Agents is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Pixel Tool Gateway,
// mirroring pixel_cli/pixel_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
// When switching to Pixel auto-routes unconfigured tools through the Tool
...
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your Pixel subscription — no separate API keys needed.`,
...
async function fetchProviderDefaultModel(
  ...
  // `pixel-agents model` does (for Pixel it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Pixel Agents still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Pixel Agents still cannot resolve a usable provider.'
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
  ...
  if (shouldPreserveConfiguredOnFallback(runtime, state)) {
    ...
    notify({
      ...
      message:
        'Pixel Agents Desktop could not verify the running backend on startup. Some features may be unavailable until the gateway is reachable.'
    })
...
async function openSignInUrl(url: string) {
  if (window.pixelAgentsDesktop?.openExternal) {
    try {
      await window.pixelAgentsDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Pixel Agents still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Pixel Agents still cannot reach ${url}.` }
    }
