...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { pixelAgentsDesktop: { readDir: typeof readDir } }).pixelAgentsDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): PixelAgentsReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { pixelAgentsDesktop: unknown }).pixelAgentsDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: PixelAgentsReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<PixelAgentsReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { pixelAgentsDesktop: unknown }).pixelAgentsDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.pixelAgentsDesktop is missing', async () => {
    delete (window as unknown as { pixelAgentsDesktop?: unknown }).pixelAgentsDesktop
