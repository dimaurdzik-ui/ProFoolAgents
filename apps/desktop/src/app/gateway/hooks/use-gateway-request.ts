...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as PixelAgentsGateway | null
  const gatewayRef = useRef<PixelAgentsGateway | null>(null)

  const connectionRef = useRef<Awaited<
    ReturnType<NonNullable<typeof window.pixelAgentsDesktop>['getConnection']>
  > | null>(null)

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<PixelAgentsGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as PixelAgentsGateway | null
      }),
    []
  )
