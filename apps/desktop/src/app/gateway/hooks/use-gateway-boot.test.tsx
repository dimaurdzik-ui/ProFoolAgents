...
// bug that drives the REAL useGatewayBoot hook + REAL PixelAgentsGateway through a
...
function Harness({
  ...
  useGatewayBoot({
    ...
    refreshPixelAgentsConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { pixelAgentsDesktop?: unknown }).pixelAgentsDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { pixelAgentsDesktop?: unknown }).pixelAgentsDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForPixelAgents) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startPixelAgents()'s remote branch awaits waitForPixelAgents() for 45s before it
    ...
    ;(window as { pixelAgentsDesktop?: unknown }).pixelAgentsDesktop = desktop
    ...
    // After ~45s waitForPixelAgents gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Pixel Agents backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Pixel Agents couldn't start" overlay — the socket is open
