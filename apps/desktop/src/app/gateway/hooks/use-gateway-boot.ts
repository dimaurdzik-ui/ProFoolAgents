import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@pixel-agents/shared'
import { useEffect, useRef } from 'react'

import type { PixelAgentsConnection } from '@/global'
import { translateNow } from '@/i18n'
import { desktopDefaultCwd } from '@/lib/desktop-fs'
import { PixelAgentsGateway } from '@/pixel-agents'
import {
  $desktopBoot,
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.pixelAgentsDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: PixelAgentsGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: PixelAgentsConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Pixel Agents...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Pixel Agents…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Pixel Agents gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshPixelAgentsConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshPixelAgentsConfig().catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startPixelAgents re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new PixelAgentsGateway()
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          seedDefaultCwd(),
          callbacksRef.current.refreshPixelAgentsConfig(),
          ...
          // retry it), not brick boot behind the "Pixel Agents couldn't start"
