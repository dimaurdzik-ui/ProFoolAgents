...
export function useRouteResume({
  ...
  useEffect(() => {
    ...
    if (routedSessionId) {
      ...
      // stuck empty forever. selectedStoredSessionIdRef is set synchropixelly at
      ...
      if ((gatewayBecameOpen || !alreadyActive) && shouldResume && !creatingSessionRef.current) {
        ...
        // resumeSession makes synchropixelly at entry.
  ...
  // sets it synchropixelly at entry) and the pathname/gateway are unchanged, so
