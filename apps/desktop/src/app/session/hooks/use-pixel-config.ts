...
import { getPixelAgentsConfig, getPixelAgentsConfigDefaults } from '@/pixel-agents'
import {
...
interface PixelAgentsConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function usePixelAgentsConfig({ activeSessionIdRef }: PixelAgentsConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshPixelAgentsConfig = useCallback(
    async (force = false) => {
      if (force) {
      ...
      try {
        const [config, defaults] = await Promise.all([
          getPixelAgentsConfig(),
          getPixelAgentsConfigDefaults().catch(() => ({}))
        ])
        ...
        // rendering/applying Pixel Agents' built-in medium over the user's config.
  ...
  return { refreshPixelAgentsConfig, sttEnabled, voiceMaxRecordingSeconds }
}
