...
export function PetOverlayApp() {
  ...
  const setIgnore = (ignore: boolean) => {
    if (ignoreRef.current !== ignore) {
      ...
      window.pixelAgentsDesktop?.petOverlay?.setIgnoreMouse(ignore)
    }
  }
  ...
  useEffect(() => {
    const off = window.pixelAgentsDesktop?.petOverlay?.onState(payload => {
    ...
    window.pixelAgentsDesktop?.petOverlay?.control({ type: 'ready' })
  ...
  useEffect(() => {
    ...
    window.pixelAgentsDesktop?.petOverlay?.setFocusable(composerOpen)
  ...
  const onPetPointerMove = (e: React.PointerEvent) => {
    ...
    window.pixelAgentsDesktop?.petOverlay?.setBounds({
      height: drag.height,
      width: drag.width,
  ...
  const onPetPointerUp = (e: React.PointerEvent) => {
    ...
    if (drag.moved) {
      ...
      window.pixelAgentsDesktop?.petOverlay?.control({
    ...
    if (e.shiftKey) {
      window.pixelAgentsDesktop?.petOverlay?.control({ type: 'pop-in' })
    ...
    if (clickTimerRef.current) {
      clearTimeout(clickTimerRef.current)
      clickTimerRef.current = undefined
      window.pixelAgentsDesktop?.petOverlay?.control({ type: 'toggle-app' })
  ...
  const send = () => {
    ...
    if (text) {
      window.pixelAgentsDesktop?.petOverlay?.control({ text, type: 'submit' })
    }
  ...
  const openApp = () => {
    ...
    setUnread(false)
    window.pixelAgentsDesktop?.petOverlay?.control({ type: 'open-app' })
  }
  ...
  const onScale = useCallback((next: number, anchor: PetZoomAnchor) => {
    ...
    window.pixelAgentsDesktop?.petOverlay?.control({ scale: next, type: 'scale' })
  }, [])
  ...
  useEffect(() => {
    ...
    window.pixelAgentsDesktop?.petOverlay?.setBounds(bounds)
    window.pixelAgentsDesktop?.petOverlay?.control({ bounds, type: 'bounds' })
    ...
    >
      ...
      >
        ...
        <div style={{ lineHeight: 0, position: 'relative' }}>
          ...
          {unread && (
            <button
              aria-label="Open in Pixel Agents"
              onClick={openApp}
