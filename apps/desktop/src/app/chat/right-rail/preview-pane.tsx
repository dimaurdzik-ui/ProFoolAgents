        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.pixelAgentsDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Pixel Agents to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.pixelAgentsDesktop?.watchPreviewFile ||
      !window.pixelAgentsDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.pixelAgentsDesktop.onPreviewFileChanged(payload => {
    ...
    void window.pixelAgentsDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.pixelAgentsDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.pixelAgentsDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:pixel-agents-preview')
