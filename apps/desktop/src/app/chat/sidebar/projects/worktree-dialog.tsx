...
const branchActionLabel = (branch: PixelAgentsGitBranch, copy: BranchActionCopy) => {
...
export function WorktreeDialog({ repoPath, onStarted, open, onOpenChange, initialBase }: WorktreeDialogProps) {
  ...
  const [branches, setBranches] = useState<PixelAgentsGitBranch[]>([])
  ...
  const convert = async (branch: PixelAgentsGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }
