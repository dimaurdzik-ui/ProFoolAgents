...
import { extractDroppedFiles, partitionDroppedFiles, PIXEL_AGENTS_PATHS_MIME } from '../../hooks/use-composer-actions'
...
export function useComposerDrop({
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, PIXEL_AGENTS_PATHS_MIME)) {
      return
    }
