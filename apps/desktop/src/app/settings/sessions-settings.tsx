...
import type { PixelAgentsConfigRecord, SessionInfo } from '@/types/pixel-agents'
...
function AutoArchiveSetting() {
  ...
  useEffect(() => {
    ...
    void getPixelAgentsConfigRecord()
      .then(record => {
        if (!alive) {
          return
        }
  ...
  const persist = useCallback(
    async (autoArchive: boolean, archiveDays: number) => {
      ...
      try {
        await savePixelAgentsConfig(updated)
      } catch (err) {
...
// / Program Files), which buried any files Pixel Agents wrote there.
function DefaultProjectDirSetting() {
  ...
  useEffect(() => {
    ...
    // / Storybook / non-Electron context `window.pixelAgentsDesktop` is
    ...
    const settings = window.pixelAgentsDesktop?.settings
