...
import { getElevenLabsVoices, getPixelAgentsConfigSchema, savePixelAgentsConfig } from '@/pixel-agents'
import {
  $dataUrlReadMaxMb,
...
import type { ConfigFieldSchema, PixelAgentsConfigRecord } from '@/types/pixel-agents'
...
export function voiceFieldVisible(key: string, config: PixelAgentsConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<PixelAgentsConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = usePixelAgentsConfigRecord()
  ...
  const {
    ...
    queryKey: ['pixel-agents-config-schema'],
    queryFn: getPixelAgentsConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await savePixelAgentsConfig(config)
          ...
          setPixelAgentsConfigCache(config)
  ...
  const updateConfig = (next: PixelAgentsConfigRecord) => {
