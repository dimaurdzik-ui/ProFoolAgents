...
import type { ToolsetConfig } from '@/types/pixel-agents'
...
const getPixelAgentsConfigSchema = vi.fn()
...
vi.mock('@/pixel-agents', () => ({
  ...
  getPixelAgentsConfigSchema: () => getPixelAgentsConfigSchema(),
...
beforeEach(() => {
  ...
  getPixelAgentsConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getPixelAgentsConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(savePixelAgentsConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = savePixelAgentsConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
  ...
  describe('readiness pills', () => {
    it('renders the server status instead of assuming keyless rows are Ready', async () => {
      // The false-Ready bug: a logged-out Pixel Subscription row and a
  ...
  describe('managed Pixel provider activation', () => {
    const pixelBrowserConfig = () =>
      config({
        name: 'browser',
        active_provider: null,
        providers: [
    ...
    it('surfaces a sign-in notice when the PUT reports needs_pixel_auth', async () => {
      ...
      // reports needs_pixel_auth and the panel must surface a sign-in action
      // instead of the misleading "provider selected" success toast.
      const { notify } = await import('@/store/notifications')

      getToolsetConfig.mockResolvedValue(pixelBrowserConfig())
      selectToolsetProvider.mockResolvedValue({
        ok: true,
        name: 'browser',
        provider: 'Pixel Subscription (Browser Use cloud)',
        needs_pixel_auth: true,
        feature: 'browser'
      })
      ...
      // The single Pixel row auto-expands; activate via the explicit button.
      await screen.findByRole('button', { name: /Pixel Subscription/ })
      fireEvent.click(await screen.findByRole('button', { name: /Use this backend/ }))

      await waitFor(() =>
        expect(selectToolsetProvider).toHaveBeenCalledWith('browser', 'Pixel Subscription (Browser Use cloud)')
      )
      await waitFor(() =>
    ...
    it('drives the existing Pixel OAuth device-code flow from the sign-in action and refetches', async () => {
      const { notify } = await import('@/store/notifications')
      ...
      startOAuthLogin.mockResolvedValue({
        flow: 'device_code',
        session_id: 'sess-1',
        user_code: 'PIXEL-1234',
        verification_url: 'https://portal.pixelagents.com/device?user_code=PIXEL-1234',
        poll_interval: 5,
        expires_in: 600
      })
      ...
      try {
        ...
        await waitFor(() => expect(startOAuthLogin).toHaveBeenCalledWith('pixel'))
        expect(openSpy).toHaveBeenCalledWith(
          'https://portal.pixelagents.com/device?user_code=PIXEL-1234',
          '_blank',
          'noopener,noreferrer'
        )
        // Approved poll → the panel refetches the config so status flips.
        await waitFor(() => expect(pollOAuthSession).toHaveBeenCalledWith('pixel', 'sess-1'), { timeout: 8000 })
