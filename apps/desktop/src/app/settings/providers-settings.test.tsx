...
import type { EnvVarInfo, OAuthProvider } from '@/types/pixel-agents'
...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `pixel-agents auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'pixel' ? 'Pixel Portal' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
beforeEach(() => {
  ...
  disconnectOAuthProvider.mockResolvedValue({ ok: true, provider: 'pixel' })
  listOAuthProviders.mockResolvedValue({
    providers: [provider('pixel', true), provider('minimax-oauth', false)]
  })
...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove Pixel Portal' })
    await act(async () => {
      fireEvent.click(remove)
    })

    await waitFor(() => expect(disconnectOAuthProvider).toHaveBeenCalledWith('pixel'))
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    await act(async () => {
      fireEvent.click(await screen.findByText('Pixel Portal'))
    })

    expect(startManualProviderOAuth).toHaveBeenCalledWith('pixel')
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          ...
          disconnect_hint: "Use `pixel-agents auth add qwen-oauth` or that provider's CLI to remove it.",
