...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Pixel Agents gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>) {
  const original = window.pixelAgentsDesktop
  Object.defineProperty(window, 'pixelAgentsDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config }
  })

  return () => Object.defineProperty(window, 'pixelAgentsDesktop', { configurable: true, value: original })
}
