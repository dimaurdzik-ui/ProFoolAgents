...
const SCRIPT: Record<string, { id: string; src: string }> = {
  instagram: { id: 'pixel-agents-ig-embed', src: 'https://www.instagram.com/embed.js' },
  tiktok: { id: 'pixel-agents-tt-embed', src: 'https://www.tiktok.com/embed.js' },
  twitter: { id: 'pixel-agents-tw-embed', src: 'https://platform.twitter.com/widgets.js' }
}
...
export default function SocialEmbedRenderer({ descriptor }: { descriptor: EmbedDescriptor }) {
  ...
  useEffect(() => {
    ...
    void loadScript(descriptor.provider).then(() => {
      // The script renders asynchropixelly; nudge a few times so the embed
