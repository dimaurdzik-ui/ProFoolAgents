...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Pixel Agents Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Pixel Agents docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Pixel Agents Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Pixel Agents docs', url: 'https://example.com/docs' }
    ])
  })
})
  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://api.pixelagents.com/docs' },
    ...
    expect(view.title).toBe('Failed to open api.pixelagents.com/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://api.pixelagents.com/docs' },
        result: { success: true, url: 'https://api.pixelagents.com/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened api.pixelagents.com/docs')
  })
})
