...
const PIXEL_AGENTS_REF_TYPES = WIRE_REFERENCE_KINDS
type PixelAgentsRefType = (typeof PIXEL_AGENTS_REF_TYPES)[number]
...
const PIXEL_AGENTS_DIRECTIVE_RE = referenceRe()
...
export const pixelAgentsDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(PIXEL_AGENTS_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...pixelAgentsDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Pixel Agents directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.pixelAgentsDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.pixelAgentsDesktop?.readFileDataUrl(id)
