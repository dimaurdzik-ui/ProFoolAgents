...
import { getPixelAgentsConfigRecord, type PixelAgentsConfigRecord, savePixelAgentsConfig } from '@/pixel-agents'
...
export interface I18nConfigClient {
  getConfig: () => Promise<PixelAgentsConfigRecord>
  saveConfig: (config: PixelAgentsConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.pixelAgentsDesktop?.api) {
      return Promise.resolve({})
    }

    return getPixelAgentsConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.pixelAgentsDesktop?.api) {
    ...
    return savePixelAgentsConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: PixelAgentsConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: PixelAgentsConfigRecord, locale: Locale): PixelAgentsConfigRecord {
