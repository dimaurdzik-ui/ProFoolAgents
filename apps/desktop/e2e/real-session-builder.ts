 ...
 * AIAgent loop, using the E2E mock provider configured in `pixelAgentsHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(pixelAgentsHome: string) {
    this.child = spawn('uv', ['run', '--active', '--no-sync', 'python', '-m', 'tui_gateway.entry'], {
      cwd: REPO_ROOT,
      env: {
        ...process.env,
        PIXEL_AGENTS_HOME: pixelAgentsHome,
  ...
  static async start(pixelAgentsHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(pixelAgentsHome)
