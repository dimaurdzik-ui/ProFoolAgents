...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(sandbox.pixelAgentsHome, mock.url)
  fs.appendFileSync(
    path.join(sandbox.pixelAgentsHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.pixelAgentsHome)
  const builder = await RealSessionBuilder.start(sandbox.pixelAgentsHome)
  try {
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.pixelAgentsHome, mock.url)
  fs.appendFileSync(path.join(sandbox.pixelAgentsHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
  writeEnvFile(sandbox.pixelAgentsHome)
  const { app, page } = await launchDesktop(buildAppEnv(sandbox))
