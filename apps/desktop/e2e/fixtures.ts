/**
 * Shared E2E fixtures for the Pixel Agents desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → pixel-agents serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `pixel-agents desktop --source`. The
...
export interface Sandbox {
  root: string
  pixelAgentsHome: string
  userDataDir: string
  cleanup: () => void
}

export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `pixel-agents-e2e-${prefix}-${Math.random()}`))
  const pixelAgentsHome = path.join(root, 'pixel-agents-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(pixelAgentsHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(pixelAgentsHome, 'config.yaml')
 ...
 */
export function writeEnvFile(pixelAgentsHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(pixelAgentsHome, '.env')
 ...
 */
function writeEmptyConfig(pixelAgentsHome: string): void {
  const configPath = path.join(pixelAgentsHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - PIXEL_AGENTS_HOME → sandbox pixel-agents-home (isolated config/sessions)
 *  - PIXEL_AGENTS_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - PIXEL_AGENTS_DESKTOP_IGNORE_EXISTING=1 → don't pick up `pixel-agents` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS_ROOT → REPO_ROOT (dev checkout resolution)
 *  - PIXEL_AGENTS_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    PIXEL_AGENTS_HOME: sandbox.pixelAgentsHome,
    PIXEL_AGENTS_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    PIXEL_AGENTS_DESKTOP_IGNORE_EXISTING: '1',
    PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS_ROOT: REPO_ROOT,
    PIXEL_AGENTS_DESKTOP_APP_NAME: `PixelAgentsE2E-${Date.now()}`,
    ...
    PIXEL_AGENTS_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated PIXEL_AGENTS_HOME + userData
 * @param env      - the process environment (already has PIXEL_AGENTS_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeMockProviderConfig(
    sandbox.pixelAgentsHome,
    mock.url,
    options.extraDisplayConfig,
    options.extraConfig,
    options.modelContextLength,
  )
  writeEnvFile(sandbox.pixelAgentsHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.pixelAgentsHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via PIXEL_AGENTS_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`pixel-agents serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.pixelAgentsHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { PIXEL_AGENTS_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Pixel Agents backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Pixel Agents.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Pixel Agents.app', 'Contents', 'MacOS', 'Pixel Agents')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'pixel-agents')
}
 ...
 * progress without spawning a real Pixel Agents backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * PIXEL_AGENTS_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    PIXEL_AGENTS_DESKTOP_BOOT_FAKE: '1',
    PIXEL_AGENTS_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + pixel-agents-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).PIXEL_AGENTS_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS
  delete (env as Record<string, string | undefined>).PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS_ROOT
