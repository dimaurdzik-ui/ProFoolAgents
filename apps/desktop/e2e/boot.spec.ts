 ...
 * packaged binary) with a real `pixel-agents serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → pixel-agents serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Pixel Agents title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Pixel Agents')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → pixel-agents serve is spawned → WS connects → config
