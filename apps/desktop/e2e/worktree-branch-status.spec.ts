...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Pixel Agents E2E'], { cwd: repo })
...
function configureRepoCwd(pixelAgentsHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(pixelAgentsHome, mockUrl)
  fs.appendFileSync(path.join(pixelAgentsHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(pixelAgentsHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.pixelAgentsHome, mock.url, repo)
