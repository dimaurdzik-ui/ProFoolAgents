...
2. **Borderless elevation for floating panels.** Overlays float on
   `shadow-pixel` + a `--stroke-pixel` hairline, not thick framed boxes. In-panel
...
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-pixel`,
...
```
shadow-pixel           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-pixel) /* currentColor hairline, theme-adaptive */
```
...
| `--stroke-pixel` | the overlay hairline (pairs with `shadow-pixel`) |
...
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the brand glyph — the
  `pixel-girl` mark on a white tile, softly rounded, identical in light/dark.
...
- Cancellation is synchropixel in the UI even if cleanup is async: overlays,
...
- [ ] Tokens (`--ui-*`, `shadow-pixel`, `--stroke-pixel`) — zero raw colors /
      one-off shadows?
...
- [ ] Overlay uses `shadow-pixel` + `border-(--stroke-pixel)`, no hard border?
