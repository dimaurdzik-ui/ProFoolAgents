...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `pixel-agents-dev-mock-${Date.now()}`))
  ...
  return { root, pixelAgentsHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(pixelAgentsHome, mockUrl) {
  fs.writeFileSync(
    path.join(pixelAgentsHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(pixelAgentsHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.pixelAgentsHome, mock.url)
  console.log(`  PIXEL_AGENTS_HOME: ${sandbox.pixelAgentsHome}`)
  ...
  const env = {
    ...
    PIXEL_AGENTS_DESKTOP_APP_NAME: `PixelAgentsDevMock-${Date.now()}`,
  }
