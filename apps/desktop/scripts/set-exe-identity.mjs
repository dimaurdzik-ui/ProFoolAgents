#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Pixel Agents icon + version metadata onto the
// built Pixel Agents.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Pixel Agents.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `pixel-agents desktop`, the installer's
...
// path (which rebuilds via `pixel-agents desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Pixel Agents.exe>
//
...
// Stamp the Pixel Agents icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ...
      FileDescription: 'Pixel Agents',
      CompanyName: 'Pixel Agents',
      LegalCopyright: 'Copyright (c) 2026 Pixel Agents'
    }
  })

  console.log('[set-exe-identity] done — Pixel Agents icon + identity stamped')
}
