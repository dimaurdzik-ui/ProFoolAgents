...
cd apps/desktop
XCURSOR_SIZE=24 PIXEL_AGENTS_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
...
node apps/desktop/scripts/profile-typing.mjs \
  --chars=400 --cps=30 --out=/tmp/pixel-agents-typing
# → /tmp/pixel-agents-typing.cpuprofile  (open in Chrome DevTools Performance)
# → /tmp/pixel-agents-typing.before.heapsnapshot
# → /tmp/pixel-agents-typing.after.heapsnapshot
```
...
1. **Per-keystroke `scrollHeight` read removed.** The expansion useEffect
   ...
   (forces synchropixel layout). Replaced with a `draft.length > 60`
