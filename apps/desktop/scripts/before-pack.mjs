 ...
 * product name (`Pixel Agents`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Pixel Agents'
 *
 * This is a hard failure with no obvious cause for the user — `pixel-agents desktop`
 ...
 * (the mac-unpacked Pixel Agents.app bundle) and Windows (win-unpacked), so we
 ...
 * Pixel Agents.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `pixel-agents desktop --build-only` (pixel_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Pixel Agents.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Pixel Agents'}.exe`
