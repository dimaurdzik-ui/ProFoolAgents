...
// no longer bundles the Pixel Agents Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Pixel Agents.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Pixel Agents'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Pixel Agents.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Pixel Agents'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default PIXEL_AGENTS_HOME for non-sandboxed runs -- matches main.ts's
// resolvePixelAgentsHome(). On Windows it's %LOCALAPPDATA%\pixel-agents; elsewhere
// it's ~/.pixel-agents. The fresh-install sandbox launchFresh() sets its own
// PIXEL_AGENTS_HOME and never touches this.
const DEFAULT_PIXEL_AGENTS_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'pixel-agents')
  }
  return path.join(os.homedir(), '.pixel-agents')
})()
const VENV_ROOT = path.join(DEFAULT_PIXEL_AGENTS_HOME, 'pixel-agents', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'pixel-agents-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.PIXEL_AGENTS_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Pixel Agents-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Pixel Agents-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Pixel Agents-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Pixel Agents-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.PIXEL_AGENTS_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.PIXEL_AGENTS_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const pixelAgentsHome = path.join(sandbox, 'pixel-agents-home')
  ...
  env.PIXEL_AGENTS_DESKTOP_CWD = cwd
  env.PIXEL_AGENTS_DESKTOP_IGNORE_EXISTING = '1'
  env.PIXEL_AGENTS_DESKTOP_TEST_MODE = 'fresh-install'
  env.PIXEL_AGENTS_DESKTOP_USER_DATA_DIR = userDataDir
  env.PIXEL_AGENTS_HOME = pixelAgentsHome
  delete env.PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS
  delete env.PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS_ROOT
  ...
  console.log(`  PIXEL_AGENTS_HOME: ${pixelAgentsHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(pixelAgentsHome, 'pixel-agents', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Pixel Agents Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of pixel_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'pixel-agents', 'pixel_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Pixel Agents
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + PIXEL_AGENTS_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  PIXEL_AGENTS_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
