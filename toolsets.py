...
_PIXEL_AGENTS_CORE_TOOLS = [
    ...
    # pane, and react to a message with an emoji (all gated on PIXEL_AGENTS_DESKTOP
    ...
    # spawned as a kanban worker (PIXEL_AGENTS_KANBAN_TASK env set) or the current
...
_PIXEL_AGENTS_WEBHOOK_SAFE_TOOLS = [
    "web_search",
    "web_extract",
    "vision_analyze",
    "clarify",
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `pixel-agents tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "video_gen": {
        "description": (
            ...
            "``pixel-agents tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (PIXEL_AGENTS_KANBAN_TASK env "
    ...
    # Coding posture (base Pixel Agents — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in pixel_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Pixel Agents toolsets (CLI + messaging platforms)
    #
    ...
    # the `pixel-agents send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "pixel-agents-cli": {
        ...
        "tools": _PIXEL_AGENTS_CORE_TOOLS,
        "includes": []
    },

    "pixel-agents-cron": {
        # Mirrors pixel-agents-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `pixel-agents tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as pixel-agents-cli; gated by `pixel-agents tools`",
        "tools": _PIXEL_AGENTS_CORE_TOOLS,
        "includes": []
    },

    "pixel-agents-telegram": {
        "description": "Telegram bot toolset - full access for personal use (terminal has safety checks)",
        "tools": _PIXEL_AGENTS_CORE_TOOLS,
        "includes": []
    },
    
    "pixel-agents-discord": {
    ...
    "pixel-agents-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _PIXEL_AGENTS_CORE_TOOLS,
        "includes": []
    },

    "pixel-agents-homeassistant": {
    ...
    "pixel-agents-email": {
        "description": "Email bot toolset - interact with Pixel Agents via email (IMAP/SMTP)",
        "tools": _PIXEL_AGENTS_CORE_TOOLS,
        "includes": []
    },

    "pixel-agents-mattermost": {
    ...
    "pixel-agents-wecom-callback": {
        "description": "WeCom callback toolset - enterprise self-built app messaging (full access)",
        "tools": _PIXEL_AGENTS_CORE_TOOLS,
        "includes": []
    },

    "pixel-agents-yuanbao": {
    ...
    "pixel-agents-sms": {
        "description": "SMS bot toolset - interact with Pixel Agents via SMS (Twilio)",
        "tools": _PIXEL_AGENTS_CORE_TOOLS,
        "includes": []
    },
    ...
    "pixel-agents-webhook": {
        ...
        "tools": _PIXEL_AGENTS_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "pixel-agents-gateway": {
        ...
        "tools": [],
        "includes": ["pixel-agents-telegram", "pixel-agents-discord", "pixel-agents-whatsapp", "pixel-agents-slack", "pixel-agents-signal", "pixel-agents-bluebubbles", "pixel-agents-homeassistant", "pixel-agents-email", "pixel-agents-sms", "pixel-agents-mattermost", "pixel-agents-matrix", "pixel-agents-dingtalk", "pixel-agents-feishu", "pixel-agents-wecom", "pixel-agents-wecom-callback", "pixel-agents-weixin", "pixel-agents-qqbot", "pixel-agents-webhook", "pixel-agents-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``pixel-agents-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_PIXEL_AGENTS_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``pixel-agents-gateway``
    ...
    """
    core = set(_PIXEL_AGENTS_CORE_TOOLS)
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (pixel-agents-<name>).
        # Gives them _PIXEL_AGENTS_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("pixel-agents-"):
            platform_name = name[len("pixel-agents-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_PIXEL_AGENTS_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
