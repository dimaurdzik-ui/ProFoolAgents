# OpenAI-Compatible API Server for Pixel Agents
...
SSE streaming. By exposing this endpoint, pixel-agents becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  pixel-agents   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.pixel-agents/.env`
2. Run `pixel-agents gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with pixel-agents through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns pixel-agents as a model) |
...
Authorization: Bearer pixel-agents-api-key-here
...
But pixel-agents sessions have persistent state (memory, skills, tool context).
...
The agent's `run_conversation()` is synchropixel and returns the full response.
...
Frontends send `"model": "pixel-agents"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured pixel-agents model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of pixel-agents's core prompt
...
Once implemented, pixel-agents works as a drop-in backend for:
