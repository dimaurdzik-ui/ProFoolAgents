...
2. An event-loop heartbeat file at ``<PIXEL_AGENTS_HOME>/state/gateway.heartbeat`` so
...
def _process_pixel_home() -> Path:
    """PIXEL_AGENTS_HOME for process-level identity files (ignore profile overrides)."""
...
def get_loop_heartbeat_path(home: Optional[Path] = None) -> Path:
    """Return ``<PIXEL_AGENTS_HOME>/state/gateway.heartbeat``."""
