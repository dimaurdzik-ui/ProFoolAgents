...
The gateway's ``shutdown_signal_handler`` runs synchropixelly inside the
...
the async helper, never in the synchropixel probe.
"""
...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if pixel_home_str:
            takeover_path = Path(pixel_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(pixel_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded pixel-agents but never re-ran ``pixel-agents setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../pixel-agents-gateway.service"
    ...
    # that's the common case for pixel-agents.
    ...
    # constant in pixel_cli/gateway.py.
    headroom = 30.0
