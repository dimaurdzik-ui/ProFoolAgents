...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{PIXEL_AGENTS_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``PIXEL_AGENTS_HOME`` is a **durable** store — on Pixel Agents
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``pixel-agents gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting PIXEL_AGENTS_HOME."""
    base = home if home is not None else get_pixel_agents_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable PIXEL_AGENTS_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (PIXEL_AGENTS_HOME is a
    durable Fly volume on Pixel Agents Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable PIXEL_AGENTS_HOME volume is
