...
Module-level run.py helpers a handler needs (``_pixel_home``,
...
from pixel_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: pixel-agents gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Pixel Agents commands.
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        #
        # _cleanup_agent_resources is synchropixel and can block for a long time
        ...
        if multiplexed:
            ...
            try:
                ...
                with _profile_runtime_scope(profile_home):
                    display = display_pixel_agents_home()
            except Exception:
                display = display_pixel_agents_home()
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from pixel_cli.kanban import run_slash
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`pixel-agents gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _pixel_home
        ...
        try:
            ...
            atomic_json_write(
                _pixel_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _pixel_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Pixel Agents version."""
        from pixel_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _pixel_home, _load_gateway_config
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _pixel_home) / "config.yaml"
        try:
        ...
        if not model_input and not explicit_provider:
            ...
            if has_picker:
                try:
                    ...
                    # synchropixel urllib HTTP fetch on a stale cache) off the
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        # can fall through to a synchropixel models.dev HTTP fetch
                        ...
                        try:
                            from pixel_cli.context_switch_guard import (
        ...
        # through to a synchropixel models.dev HTTP fetch (requests.get, 15s
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Pixel Agents default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        config_path = _pixel_home / 'config.yaml'
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_pixel_agents_home())
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from pixel_cli.goals import draft_contract
            ...
            from pixel_cli.goals import parse_contract
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        Pixel Agents itself changed. ``--stat`` limits output to the summary.
    ...
    def _reasoning_picker_choices(self, current_effort: str) -> list:
        ...
        from pixel_constants import VALID_REASONING_EFFORTS
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _pixel_home
        from pixel_cli.write_approval_commands import handle_pending_subcommand
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.pixel-agents/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``pixel-agents skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _pixel_home
        from pixel_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `pixel-agents skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.pixel-agents/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_approvals_command(self, event: MessageEvent) -> str:
        ...
        from gateway.slash_access import policy_for_source
        from pixel_cli.approval_mode import run_approval_mode_command
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _pixel_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _pixel_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from pixel_cli.session_listing import (
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Handle /topup -- show the Pixel balance and hand off to the portal.
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # ── Pixel credits magnitudes + monthly-grant % gauge ─────────────
        # Shared with the CLI / TUI /usage block via pixel_credits_lines(): a single
        # auth-gate + portal-fetch + render path (which also honors the dev fixture).
        # Run off the event loop. The helper gates on "a Pixel account is logged in"
        # — NOT the inference provider and NOT nested under `if provider:` — so a
        # Pixel-credentialled user running inference elsewhere (or with none resident)
        ...
        try:
            from agent.account_usage import pixel_credits_lines

            credits_lines = await asyncio.to_thread(pixel_credits_lines, markdown=True)
        except Exception:
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `pixel-agents bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_approve_command(self, event: MessageEvent) -> Optional[str]:
        ...
        flow as the CLI's synchropixel input() approval.
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``pixel-agents debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Pixel Agents to the latest version.

        Spawns ``pixel-agents update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``pixel-agents update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _pixel_home, _resolve_pixel_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from pixel_cli.config import is_managed, format_managed_message
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Pixel Agents')}"
        ...
        if not pixel_cmd:
            return t("gateway.update.pixel_cmd_not_found")
        ...
        # Spawn `pixel-agents update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `pixel-agents update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # pixel_cmd is a list of argv parts we can pass directly
                ...
                pixel_cmd_str = " ".join(shlex.quote(part) for part in pixel_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {pixel_cmd_str} update --gateway"
