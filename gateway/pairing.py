...
PAIRING_DIR = get_pixel_agents_dir("platforms/pairing", "pairing")
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from pixel_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{PIXEL_AGENTS_HOME}/pairing`` while newer code/docs may
    write ``{PIXEL_AGENTS_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own PIXEL_AGENTS_HOME using the same legacy/consolidated layout rules
    as ``pixel-agents -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current PIXEL_AGENTS_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp PIXEL_AGENTS_HOME
        ...
        if profile:
            ...
            self._dir = get_pixel_agents_dir(
                "platforms/pairing",
                "pairing",
                home=profile_home,
            )
        ...
        if profile:
            ...
            # ``pixel-agents -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `pixel-agents` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> pixel-agents pairing approve ...` as root, "
                    "re-run with `docker exec -u pixel-agents <container> ...` and "
                    "chown the existing file to the pixel-agents user, or restart the "
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``pixel-agents
