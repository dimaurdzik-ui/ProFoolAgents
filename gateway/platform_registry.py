...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``pixel-agents setup`` display).
    ...
    # built-ins).  Used by ``pixel-agents gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `pixel-agents`
        # invocation -- including plain `pixel-agents chat`, which never touches any
        ...
        # `pixel-agents setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``pixel-agents setup``/``gateway status``, channel
