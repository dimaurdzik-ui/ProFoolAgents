...
Defenses 1 and 2 (the ``_PIXEL_AGENTS_GATEWAY`` guard on ``pixel-agents gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.pixel-agents.gateway")``,
...
State lives in ``<PIXEL_AGENTS_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_pixel_agents_home() / "gateway" / "restart_loop.json"
