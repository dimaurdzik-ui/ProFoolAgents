...
``<PIXEL_AGENTS_HOME>/state/gateway.lifecycle.json``:
...
def _process_pixel_home() -> Path:
    """PIXEL_AGENTS_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<PIXEL_AGENTS_HOME>/state/gateway.lifecycle.json``."""
    base = home if home is not None else _process_pixel_home()
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``pixel_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
