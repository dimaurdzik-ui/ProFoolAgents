"""Relay/connector support package for the Pixel Agents gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``pixel-agents gateway enroll`` writes them to
    ``~/.pixel-agents/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``PIXEL_AGENTS_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``pixel-agents gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from pixel_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # multi-agent scope it would prefix every reply "**Pixel Agents:**" —
...
def _resolve_relay_identity_token() -> str:
    ...
    ``pixel-agents gateway enroll`` CLI. Two modes, in precedence order:

      1. **Generic OIDC client-credentials** (air-gapped / self-hosted-IdP, NO
         Pixel Portal): when ``gateway.idp.token_url`` (or
      ...
      2. **Pixel Portal** (default): ``resolve_pixel_access_token()`` — existing
    ...
    if not token_url:
        # Mode 2 — Pixel Portal (default, unchanged behaviour).
        from pixel_cli.auth import resolve_pixel_access_token

        return resolve_pixel_access_token()
...
def self_provision_relay() -> bool:
    ...
    is already present, AND the agent can resolve its own Pixel access token. In
    that case the runtime resolves the agent's own Pixel access token (the same
    ``resolve_pixel_access_token()`` the enroll CLI / dashboard register use),
    ...
    up. The creds live ONLY in process memory — never written to ``~/.pixel-agents/.env``.
    ...
    sets neither ``PIXEL_AGENTS_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``pixel-agents gateway enroll``: has a PINNED
        ``GATEWAY_RELAY_SECRET`` -> skipped (the secret-present guard below).
      - A self-hosted box with a relay URL but no NAS identity:
        ``resolve_pixel_access_token()`` fails -> graceful no-op.
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'pixel-agents'}"
