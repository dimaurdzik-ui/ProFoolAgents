...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Pixel-Agents-Session-Id header; opt-in long-term memory scoping via X-Pixel-Agents-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Pixel-Agents-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists pixel-agents and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Pixel Agents sessions
- POST /api/sessions               — create an empty Pixel Agents session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to pixel-agents
...
def _pixel_version() -> str:
    """Return the canonical Pixel Agents version string.

    ``pixel_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``pixel_agent-*.dist-info`` after a source update until the environment is
    reinstalled. Never raises — a version probe must not be able to break the
    health endpoint.
    """
    try:
        from pixel_cli import __version__
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    from pixel_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("PIXEL_AGENTS_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``pixel-agents``)
    ...
    ``direct_model_requests`` config value here, while Pixel Agents-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Pixel Agents-aware and is
    always honored.
    """
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_pixel_agents_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted PIXEL_AGENTS_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # pixel_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same Pixel Agents session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through pixel-agents's AIAgent.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Pixel Agents-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        3. Fallback: "pixel-agents"

        Delegates the tiered fallthrough to
        :func:`pixel_cli.model_switch.resolve_effective_model` (the shared
        ...
        try:
            ...
            pass
        return resolve_effective_model(explicit, profile_name, "pixel-agents")
        ...
        if not profile:
            try:
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_pixel_agents_home())
            except Exception:
                pass
            return nullcontext()
        from gateway.run import _profile_runtime_scope
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Pixel-Agents-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Pixel-Agents-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Pixel-Agents-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Pixel-Agents-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Pixel-Agents-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``pixel-agents sessions list``
        shows API-server conversations alongside CLI and gateway ones.

        Under multiplex ``/p/<profile>/`` requests the profile runtime scope
        redirects ``get_pixel_agents_home()``, so each profile gets its own DB —
        never the default profile's file. Synchropixel: used by ``_create_agent``
        ...
        try:
            ...
            return self._open_and_cache_session_db(get_pixel_agents_home())
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "pixel_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text in allowed:
            return "pixel_browser" if text == "browser" else text
        return "api_server"
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the pixel-agents-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Pixel-Agents-Session-Key``).  Unlike ``session_id``
        ...
        # pixel_cli.model_switch.resolve_effective_model (session /model
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "pixel-agents", "version": _pixel_version()}
        )
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            ...
            "readiness": readiness,
            "platform": "pixel-agents",
            "version": _pixel_version(),
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list pixel-agents and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Pixel Agents provider/model inventory.
        ...
        Pixel Agents provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
        ...
        try:
            ...
            # Keep all synchropixel picker work off aiohttp's event loop.
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Pixel Agents version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "pixel.api_server.capabilities",
            "platform": "pixel-agents",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Pixel Agents AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Pixel-Agents-Session-Id",
                "session_key_header": "X-Pixel-Agents-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Pixel Agents sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Pixel Agents session row.
        ...
        return web.json_response({"object": "pixel-agents.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "pixel-agents.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "pixel-agents.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "pixel-agents.session", "session": self._session_response(fork)}, status=201)

    @_admit_api_agent_request
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions/{session_id}/chat — one synchropixel agent turn."""
        ...
        headers = {"X-Pixel-Agents-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Pixel-Agents-Session-Key"] = gateway_session_key
        runtime = {}
        ...
        return web.json_response(
            {
                "object": "pixel-agents.session.chat.completion",
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "pixel-agents.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Pixel-Agents-Session-Key.  This
        # is independent of X-Pixel-Agents-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Pixel-Agents-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Pixel-Agents-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Pixel-Agents-Session-Id rejected: "
            ...
            # conversation map to the same Pixel Agents session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``pixel.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Pixel-Agents-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Pixel-Agents-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["pixel-agents"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-Pixel-Agents-Completed"] = "false"
            response_headers["X-Pixel-Agents-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Pixel Agents-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Pixel-Agents-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Pixel-Agents-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Pixel-Agents-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Pixel-Agents-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: pixel.tool.progress`` SSE event so
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # response and the X-Pixel-Agents-Session-Id header.  Without this,
        ...
        response_headers = {"X-Pixel-Agents-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Pixel-Agents-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
        ...
        def _run():
            ...
            with self._profile_scope(request_profile):
                ...
                try:
                    ...
                    # (e.g. X-Pixel-Agents-Session-Id header) can track compression-
                    ...
                    if include_runtime:
                        runtime = dict(getattr(agent, "_pixel_api_runtime", {}) or {})
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Pixel-Agents-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "pixel-agents.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Pixel Agents-Relay
            ...
            # that is the exact surface the pixel-agents-0day campaign abused to write
            # ~/.pixel-agents/config.yaml and plant persistence. Sandboxing (Docker /
