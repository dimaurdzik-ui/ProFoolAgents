...
There are two ways to add a platform to the Pixel Agents gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.pixel-agents/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Pixel Agents code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `pixel-agents gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `pixel_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Pixel Agents core.
...
And add it to the `pixel-agents-gateway` composite:

```python
"pixel-agents-gateway": {
    "includes": [..., "pixel-agents-your-platform"]
}
```
...
## 12. Status Display (`pixel_cli/status.py`)
...
## 13. Gateway Setup Wizard (`pixel_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ pixel_cli/ toolsets.py \
  --include="*.py" -l | sort -u
