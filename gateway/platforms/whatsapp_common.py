...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Pixel Agents*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to PIXEL_AGENTS_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/pixel-agents), this function
    mirrors the bridge source to a writable PIXEL_AGENTS_HOME location and returns that
    ...
    # Try PIXEL_AGENTS_HOME location first
    pixel_home = get_pixel_agents_home()
    pixel_home_bridge = pixel_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to PIXEL_AGENTS_HOME if needed
    if pixel_home_bridge.exists():
        return pixel_home_bridge

    # Mirror the bridge source to PIXEL_AGENTS_HOME
    try:
        pixel_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            pixel_home_bridge,
            dirs_exist_ok=False,
