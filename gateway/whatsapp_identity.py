...
def expand_whatsapp_aliases(identifier: str) -> Set[str]:
    ...
    bridge's ``$PIXEL_AGENTS_HOME/whatsapp/session/lid-mapping-*.json`` files,
    ...
    session_dir = get_pixel_agents_dir("platforms/whatsapp/session", "whatsapp/session")
...
def canonical_whatsapp_identifier(identifier: str) -> str:
    ...
    same session-key identity Pixel Agents itself uses.

    Plugins that need per-sender behaviour (role-based routing,
    authorisation, per-contact policy) should use this so their
    bookkeeping lines up with Pixel Agents' session bookkeeping even when
