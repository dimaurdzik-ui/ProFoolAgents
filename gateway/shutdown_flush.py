...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<pixel_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active PIXEL_AGENTS_HOME."""
    from pixel_constants import get_pixel_agents_home

    flush_dir = get_pixel_agents_home() / "pending_messages"
