...
# dashboard's pre-write mutation validation (pixel_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Pixel Agents is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_pixel_agents_home() / "sessions")
    ...
    # gateway behaves exactly as before — single PIXEL_AGENTS_HOME, no profile stamping.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        # set to a recognized value. Hosted deployments (Pixel Portal / Fly) stamp
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.pixel-agents/config.yaml (primary user-facing config)
    3. ~/.pixel-agents/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through pixel_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `pixel-agents config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `pixel-agents config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``pixel-agents config set gateway.streaming.*``
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    try:
        ...
        for entry in platform_registry.plugin_entries():
            ...
            # awaits this synchropixelly) — even when the user configured none
