# nix/nixosModules.nix — NixOS module for pixel-agents
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: pixel-agents runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $PIXEL_AGENTS_HOME/.env
# and read by pixel-agents at startup — no container recreation needed for env changes.
#
# Tool resolution: the pixel-agents wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.pixel-agents;
    effectivePackage =
    ...
    pixel-agents = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-pixel-agents)
    deepConfigType = lib.types.mkOptionType {
      name = "pixel-agents-config-attrs";
      description = "Pixel Agents YAML config (attrset), merged deeply via lib.recursiveUpdate.";
      check = builtins.isAttrs;
      merge = _loc: defs: lib.foldl' lib.recursiveUpdate { } (map (d: d.value) defs);
    };

    # Generate config.yaml from Nix attrset (YAML is a superset of JSON).
    # terminal.cwd replaces the deprecated MESSAGING_CWD env var — pixel-agents
    ...
    generatedConfigFile = pkgs.writeText "pixel-agents-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # PIXEL_AGENTS_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "pixel-agents-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'PIXEL_AGENTS_DOC_EOF'\n${value}\nPIXEL_AGENTS_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "pixel-agents";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/pixel-agents";
    ...
    # pixel-agents user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "pixel-agents-container-entrypoint" ''
      set -eu

      PIXEL_AGENTS_UID="''${PIXEL_AGENTS_UID:?PIXEL_AGENTS_UID must be set}"
      PIXEL_AGENTS_GID="''${PIXEL_AGENTS_GID:?PIXEL_AGENTS_GID must be set}"

      # ── Group: ensure a group with GID=$PIXEL_AGENTS_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$PIXEL_AGENTS_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="pixel-agents"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$PIXEL_AGENTS_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$PIXEL_AGENTS_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$PIXEL_AGENTS_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$PIXEL_AGENTS_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="pixel-agents"
        TARGET_HOME="/home/pixel-agents"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$PIXEL_AGENTS_UID" -g "$PIXEL_AGENTS_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$PIXEL_AGENTS_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$PIXEL_AGENTS_UID:$PIXEL_AGENTS_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure PIXEL_AGENTS_HOME is owned by the target user.
      ...
      if [ -n "''${PIXEL_AGENTS_HOME:-}" ] && [ -d "$PIXEL_AGENTS_HOME" ]; then
        find "$PIXEL_AGENTS_HOME" \! -user "$PIXEL_AGENTS_UID" -exec chown "$PIXEL_AGENTS_UID:$PIXEL_AGENTS_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/pixel-agents-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/pixel-agents-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/pixel-agents ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/pixel-agents
        chmod 0440 /etc/sudoers.d/pixel-agents
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$PIXEL_AGENTS_UID" --regid="$PIXEL_AGENTS_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $PIXEL_AGENTS_HOME/.env.
    ...
    # Default: /var/lib/pixel-agents/workspace → /data/workspace.
  ...
  in {
    options.services.pixel-agents = with lib; {
      enable = mkEnableOption "Pixel Agents gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = pixel-agents;
        description = "The pixel-agents package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/pixel-agents";
        description = "State directory. Contains .pixel-agents/ subdir (PIXEL_AGENTS_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Pixel Agents config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $PIXEL_AGENTS_HOME/.env at activation time.
          Pixel Agents reads this file on every startup via load_pixel_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $PIXEL_AGENTS_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $PIXEL_AGENTS_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `pixel-agents gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the pixel-agents user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the pixel-agents plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Pixel Agents discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          pixel_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as pixel-agents (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in pixel-agents's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the pixel-agents CLI to environment.systemPackages and export
          PIXEL_AGENTS_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.pixel-agents symlink to the service
            stateDir. These users are automatically added to the pixel-agents group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.pixel-agents.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the pixel-agents CLI to system PATH and export PIXEL_AGENTS_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.pixel-agents/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.PIXEL_AGENTS_HOME = "${cfg.stateDir}/.pixel-agents";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.pixel-agents.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }

      # ── Warnings ──────────────────────────────────────────────────────
      # ── Per-user profile for extraPackages ───────────────────────────
      # Wire extraPackages into the pixel-agents user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.pixel-agents: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed pixel-agents
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure pixel-agents is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.pixel-agents        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.pixel-agents/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.pixel-agents/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.pixel-agents/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.pixel-agents/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.pixel-agents/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."pixel-agents-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.pixel-agents
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.pixel-agents ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.pixel-agents ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.pixel-agents -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.pixel-agents/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.pixel-agents/$_subdir"
            chmod 2770 "${cfg.stateDir}/.pixel-agents/$_subdir"
            find "${cfg.stateDir}/.pixel-agents/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # pixel-agents-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.pixel-agents/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.pixel-agents/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.pixel-agents/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.pixel-agents/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.pixel-agents/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.pixel-agents/.managed
          chmod 0644 ${cfg.stateDir}/.pixel-agents/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.pixel-agents/.container-mode <<'PIXEL_AGENTS_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    pixel_bin=${containerDataDir}/current-package/bin/pixel-agents
    PIXEL_AGENTS_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.pixel-agents/.container-mode
            chmod 0644 ${cfg.stateDir}/.pixel-agents/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.pixel-agents/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.pixel-agents";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.pixel-agents" ]; then
                  rm -f "${symlinkPath}"
                  echo "pixel-agents: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.pixel-agents -> stateDir/.pixel-agents for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.pixel-agents";
                target = "${cfg.stateDir}/.pixel-agents";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "pixel-agents: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.pixel-agents/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.pixel-agents/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.pixel-agents/auth.json
              fi
            ''}
          ''}
          ...
          # Pixel Agents reads $PIXEL_AGENTS_HOME/.env at startup via load_pixel_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.pixel-agents/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'PIXEL_AGENTS_NIX_ENV_EOF'
    ${envFileContent}
    PIXEL_AGENTS_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
                echo "" >> "$ENV_FILE"
                cat "${f}" >> "$ENV_FILE"
        ...
        find ${cfg.stateDir}/.pixel-agents/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.pixel-agents/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.pixel-agents/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.pixel-agents = {
          description = "Pixel Agents Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            PIXEL_AGENTS_HOME = "${cfg.stateDir}/.pixel-agents";
            PIXEL_AGENTS_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $PIXEL_AGENTS_HOME/.env by the activation script. load_pixel_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/pixel-agents"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the pixel-agents group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.pixel-agents = {
          description = "Pixel Agents Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              PIXEL_AGENTS_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              PIXEL_AGENTS_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env PIXEL_AGENTS_UID="$PIXEL_AGENTS_UID" \
                --env PIXEL_AGENTS_GID="$PIXEL_AGENTS_GID" \
                --env PIXEL_AGENTS_HOME=${containerDataDir}/.pixel-agents \
                --env PIXEL_AGENTS_MANAGED=true \
                --env HOME=${containerHomeDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/pixel-agents gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
