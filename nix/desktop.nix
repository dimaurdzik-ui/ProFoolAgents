# nix/desktop.nix — Pixel Agents Desktop (Electron) app build + wrapper
#
# `pixelAgentsAgent` is the fully-built `.#default` package — it ships the
# `pixel-agents` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Pixel Agents CLI").
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "pixel-agents-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "pixel-agents-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "pixel-agents-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/pixel-agents-desktop $out/bin
    cp -r ${renderer}/* $out/share/pixel-agents-desktop/
    ...
    substituteInPlace $out/share/pixel-agents-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/pixel-agents-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS to the absolute path of the nix-built `pixel-agents`
    # binary so the desktop's resolver step 4 ("existing Pixel Agents CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/pixel-agents-desktop \
      --add-flags "$out/share/pixel-agents-desktop" \
      --set PIXEL_AGENTS_DESKTOP_PIXEL_AGENTS "${lib.getExe pixelAgentsAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Pixel Agents";
    homepage = "https://github.com/PixelResearch/pixel-agents";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "pixel-agents-desktop";
  };
}
