...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Pixel Agents derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            pixel-agents = prev.pixel-agents.overrideAttrs (_old: {
              PIXEL_AGENTS_NIX_BUILD = "1";
            });
          })
        ]
      );
  ...
  # $PIXEL_AGENTS_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$PIXEL_AGENTS_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        pixel-agents = prev.pixel-agents.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "pixel-agents-env" {
    pixel-agents = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "pixel-agents-editable-env" {
    pixel-agents = dependency-groups;
  };
}
