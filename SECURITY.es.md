# Política de Seguridad de Pixel Agents

Este documento describe el modelo de confianza de Pixel Agents, identifica el
...
Reporta de forma privada a través de [GitHub Security Advisories](https://github.com/PixelResearch/pixel-agents/security/advisories/new)
o **security@pixelagents.com**. No abras issues públicos para
vulnerabilidades de seguridad. **Pixel Agents no opera un programa de
...
- Detalles del entorno (`pixel-agents version`, SHA del commit, SO, versión de Python).
...
Pixel Agents es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta Pixel Agents,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a Pixel Agents al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de Pixel Agents
...
Pixel Agents admite dos posturas de aislamiento a nivel de SO. Abordan
...
Pixel Agents admite esto de dos maneras:

- **La propia imagen Docker de Pixel Agents y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Pixel Agents
...
Pixel Agents filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Pixel Agents
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Pixel Agents
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Pixel Agents no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Pixel Agents o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Pixel Agents ha documentado una postura sobre cómo su salida debe ser
...
- **Configuraciones documentadas de emergencia.** Compensaciones seleccionadas por el operador
  ...
  la seguridad de pixel-agents-home, y similares. Los informes contra esas
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Pixel Agents
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Pixel Agents. Los errores en la ruta de
  instalación de habilidades o plugins de Pixel Agents que impidan al
...
- Pixel Agents incluye guardias de cadena de suministro para lanzamientos de servidores
