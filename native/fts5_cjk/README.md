...
Build & install to `~/.pixel-agents/lib/`:
...
storage discipline as the other indexes). On a populated database, run
    ...
    pixel-agents sessions optimize-storage

to backfill it; new messages are indexed live either way. Set
`sessions.cjk_fts: false` in `~/.pixel-agents/config.yaml` to disable. Override
the .so location with `PIXEL_AGENTS_FTS5_CJK_SO`.
