...
Thank you for contributing to Pixel Agents! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Pixel Agents to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo PixelResearch/pixel-agents "<your terms>"
  gh search prs --repo PixelResearch/pixel-agents --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/PixelResearch/pixel-agents/issues?q=) · [PRs (all states)](https://github.com/PixelResearch/pixel-agents/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Pixel Agents itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Pixel Agents install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `pixel-agents skills browse` (labeled "official") and install it with `pixel-agents skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Pixel Agents Discord](https://discord.gg/PixelResearch). Users can install it with `pixel-agents skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.pixel-agents/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(pixel_home, config)` for setup-wizard integration
...
- Integrate with `pixel-agents memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Pixel Agents ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.pixel-agents/plugins/`, project `.pixel-agents/plugins/`, or a pip entry point) — see [Build a Pixel Agents Plugin](https://api.pixelagents.com/docs/guides/build-a-pixel-agents-plugin)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [Pixel Agents Discord](https://discord.gg/PixelResearch) `#plugins-skills-and-skins` channel so users can find and install it
...
The installer creates the Pixel Agents venv, wires the `pixel-agents` command, stamps the
install method for `pixel-agents update`, and clones the full git project into
`$PIXEL_AGENTS_HOME/pixel-agents` (usually `~/.pixel-agents/pixel-agents`). That keeps your
...
Use this only if you intentionally do not want Pixel Agents' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `pixel-agents` entrypoint from this venv; running the
system `python3 -m pixel_cli.main` can pick up unrelated system Python
packages.
...
```bash
mkdir -p ~/.pixel-agents/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.pixel-agents/config.yaml
touch ~/.pixel-agents/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.pixel-agents/.env
```

### Run

```bash
# The standard installer already put `pixel-agents` on PATH.
pixel-agents doctor
pixel-agents chat -q "Hello"
```

If you used the manual clone fallback, run `./pixel-agents` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/pixel-agents" ~/.local/bin/pixel-agents
```
...
├── cli.py                    # PixelAgentsCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (pixel-agents-cli, pixel-agents-telegram, etc.)
├── pixel_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── pixel_cli/               # CLI command implementations
│   ├── main.py                   # Entry point, argument parsing, command dispatch
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, Pixel Portal
...
│
├── skills/                   # Bundled skills (copied to ~/.pixel-agents/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (api.pixelagents.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.pixel-agents/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.pixel-agents/`)

| Path | Purpose |
|------|---------|
| `~/.pixel-agents/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.pixel-agents/.env` | API keys and secrets |
| `~/.pixel-agents/auth.json` | OAuth credentials (Pixel Portal) |
| `~/.pixel-agents/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.pixel-agents/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.pixel-agents/state.db` | SQLite session database |
| `~/.pixel-agents/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.pixel-agents/cron/` | Scheduled job data |
| `~/.pixel-agents/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`pixel_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.pixel-agents/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
- **Ephemeral injection**: System prompts and prefill messages are injected at API call time, never persisted to the database or logs.
- **Provider abstraction**: The agent works with any OpenAI-compatible API. Provider resolution happens at init time (Pixel Portal OAuth, OpenRouter API key, or custom endpoint).
...
(for example `_PIXEL_AGENTS_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.pixel-agents`:
...
The user may skip setup and keep loading the skill. Pixel Agents only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `pixel-agents setup` or update `~/.pixel-agents/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Pixel Agents tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Pixel Agents" is the secondary collaborator. If the contributor's commit shows "Pixel Agents" as author because they used Pixel Agents to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Pixel Agents tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `pixel-agents --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Pixel Agents uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.pixel-agents/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `pixel_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `pixel_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Pixel Agents runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the pixel-agents wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `pixel_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `pixel_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `pixel_cli/gateway_windows.py::
...
Pixel Agents has terminal access. Security matters.
...
2. **Test manually**: Run `pixel-agents` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/PixelResearch/pixel-agents/issues)
- Include: OS, Python version, Pixel Agents version (`pixel-agents version`), full error traceback
...
- **Discord**: [discord.gg/PixelResearch](https://discord.gg/PixelResearch) — for questions, showcasing projects, and sharing skills
