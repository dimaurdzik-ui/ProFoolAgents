"""Shared constants for Pixel Agents Agent.
...
_UNSET = object()
_PIXEL_AGENTS_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_PIXEL_AGENTS_HOME_OVERRIDE", default=_UNSET
)


def set_pixel_agents_home_override(path: str | Path | None) -> Token:
    """Set a context-local Pixel Agents home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _PIXEL_AGENTS_HOME_OVERRIDE.set(value)


def reset_pixel_agents_home_override(token: Token) -> None:
    """Restore the previous context-local Pixel Agents home override."""
    _PIXEL_AGENTS_HOME_OVERRIDE.reset(token)


def get_pixel_agents_home_override() -> str | None:
    """Return the active context-local Pixel Agents home override, if any."""
    override = _PIXEL_AGENTS_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_pixel_agents_home() -> Path:
    """Return the platform-native default Pixel Agents home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "pixel-agents"
    return Path.home() / ".pixel-agents"


def _pixel_agents_home_from_env() -> Path:
    """Resolve PIXEL_AGENTS_HOME from the process environment only.

    Reads the ``PIXEL_AGENTS_HOME`` env var, falling back to the platform-native
    default.  Deliberately ignores the context-local override installed by
    :func:`set_pixel_agents_home_override`, so this reflects the process/launch
    scope rather than a per-task profile.  Shared by :func:`get_pixel_agents_home`
    and :func:`get_process_pixel_agents_home` so the two never drift.
    """
    val = os.environ.get("PIXEL_AGENTS_HOME", "").strip()
    if val:
        return Path(val)
    return _get_platform_default_pixel_agents_home()


def _warn_profile_fallback_once() -> None:
    """Warn once when falling back to the default home while a profile is active.

    Guard: if a non-default profile is sticky-active but ``PIXEL_AGENTS_HOME`` is
    ...
    if _profile_fallback_warned:
        return
    try:
        fallback_home = _get_platform_default_pixel_agents_home()
    ...
    if active and active != "default":
        ...
        msg = (
            f"[PIXEL_AGENTS_HOME fallback] PIXEL_AGENTS_HOME is unset but active "
            f"profile is {active!r}. Falling back to {fallback_home}, which "
            f"is the DEFAULT profile — not {active!r}. Any data this "
            f"process writes will land in the wrong profile. The "
            f"subprocess spawner should pass PIXEL_AGENTS_HOME explicitly "
            f"(see issue #18594)."
        )
        try:
...
def get_pixel_agents_home() -> Path:
    """Return the Pixel Agents home directory (default: platform-native path).

    Resolution order: context-local override (see
    :func:`set_pixel_agents_home_override`) → ``PIXEL_AGENTS_HOME`` env var → the
    platform-native default.  This is the single source of truth — all other
    copies should import this.

    When ``PIXEL_AGENTS_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``PIXEL_AGENTS_HOME`` explicitly (see the systemd
    template in ``pixel_agents_cli/gateway.py`` and the kanban dispatcher in
    ``pixel_agents_cli/kanban_db.py``).  See https://github.com/PixelResearch/pixel-agents/issues/18594.
    """
    override = get_pixel_agents_home_override()
    if override:
        return Path(override)

    if not os.environ.get("PIXEL_AGENTS_HOME", "").strip():
        _warn_profile_fallback_once()

    return _pixel_agents_home_from_env()


def get_process_pixel_agents_home() -> Path:
    """Return the Pixel Agents home for the running process, ignoring task overrides.

    Unlike :func:`get_pixel_agents_home`, this never follows the context-local
    override set by :func:`set_pixel_agents_home_override`.  It resolves only the
    process ``PIXEL_AGENTS_HOME`` env var (falling back to the platform default),
...
def get_default_pixel_agents_root() -> Path:
    """Return the root Pixel Agents directory for profile-level operations.

    In standard deployments this is the platform-native Pixel Agents home
    (``~/.pixel-agents`` on POSIX, ``%LOCALAPPDATA%\\pixel-agents`` on native Windows).

    In Docker or custom deployments where ``PIXEL_AGENTS_HOME`` points outside
    ``~/.pixel-agents`` (e.g. ``/opt/data``), returns ``PIXEL_AGENTS_HOME`` directly
    — that IS the root.

    In profile mode where ``PIXEL_AGENTS_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.pixel-agents/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_pixel_agents_home()
    env_home = os.environ.get("PIXEL_AGENTS_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # PIXEL_AGENTS_HOME is under ~/.pixel-agents (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — PIXEL_AGENTS_HOME itself is the root
    return env_path
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``PIXEL_AGENTS_OPTIONAL_SKILLS``.
    """
    override = os.getenv("PIXEL_AGENTS_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_pixel_agents_home() / "optional-skills"


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers.

    Mirrors :func:`get_optional_skills_dir` for the MCP catalog (Pixel-approved
    Model Context Protocol servers shipped with the repo but disabled by
    default). Packaged installs may ship ``optional-mcps`` outside the Python
    package tree and expose it via ``PIXEL_AGENTS_OPTIONAL_MCPS``.
    """
    override = os.getenv("PIXEL_AGENTS_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_pixel_agents_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``PIXEL_AGENTS_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Caller-supplied ``default`` (typically the source-checkout path)
        3. ``<PIXEL_AGENTS_HOME>/skills`` last-resort
    """
    override = os.getenv("PIXEL_AGENTS_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    if default is not None:
        return default
    return get_pixel_agents_home() / "skills"


def get_pixel_agents_dir(
    new_subpath: str,
    old_name: str,
    *,
    home: Path | None = None,
) -> Path:
    """Resolve a Pixel Agents subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to PIXEL_AGENTS_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to PIXEL_AGENTS_HOME (e.g. ``"image_cache"``).
        home: Optional explicit Pixel Agents home. Profile-aware callers that manage
            more than one home in the same process use this instead of
            temporarily mutating the process or context-local PIXEL_AGENTS_HOME.

    Returns:
        Absolute ``Path`` — legacy location if it exists with content,
        otherwise the new location.
    """
    home = home or get_pixel_agents_home()
...
def iter_pixel_agents_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Pixel Agents-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\pixel-agents\\node``. POSIX installs use
    ``$PIXEL_AGENTS_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_pixel_agents_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with pixelAgentsManagedNodePathEntries() in
...
_PIXEL_AGENTS_NODE_TARGET_MAJOR = int(os.environ.get("PIXEL_AGENTS_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Pixel Agents-managed Node trees live under ``$PIXEL_AGENTS_HOME/node`` (or a profile's
    ``PIXEL_AGENTS_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_pixel_agents_node_executable``
    used to trust file presence alone, so ``pixel-agents update`` would pick that
    ...
    try:
        from pixel_agents_cli._subprocess_compat import windows_hide_flags

        result = subprocess.run(
            [path, "--version"],
            capture_output=True,
            timeout=10,
            env=with_pixel_agents_node_path(),
...
def pixel_agents_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Pixel Agents-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_pixel_agents_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%PIXEL_AGENTS_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    index_url = f"https://nodejs.org/dist/latest-v{_PIXEL_AGENTS_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_PIXEL_AGENTS_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_pixel_agents_managed_node() -> bool:
    """Redownload Pixel Agents-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not pixel_agents_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "PIXEL_AGENTS_HOME": str(get_pixel_agents_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_pixel_agents_node_executable(command: str) -> str | None:
    """Return a Pixel Agents-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_pixel_agents_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_pixel_agents_managed_node():
        for directory in iter_pixel_agents_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Pixel Agents-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Pixel Agents-managed installs.

    This is for Pixel Agents-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_pixel_agents_node_executable(command)
    if managed:
        return managed
    if pixel_agents_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_pixel_agents_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Pixel Agents-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_pixel_agents_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``pixel-agents update`` — leaving a **dangling symlink** that ``which`` still
...
def display_pixel_agents_home() -> str:
    """Return a user-friendly display string for the current PIXEL_AGENTS_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.pixel-agents``
        profile:  ``~/.pixel-agents/profiles/coder``
        custom:   ``/opt/pixel-agents-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.pixel-agents``.  For code that needs a real ``Path``, use
    :func:`get_pixel_agents_home` instead.
    """
    home = get_pixel_agents_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``PIXEL_AGENTS_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/PixelResearch/pixel-agents/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{PIXEL_AGENTS_HOME}/home`` when the profile-home directory exists."""
    pixel_agents_home = get_pixel_agents_home_override() or (env or {}).get("PIXEL_AGENTS_HOME") or os.getenv("PIXEL_AGENTS_HOME")
    if not pixel_agents_home:
        return None
    profile_home = os.path.join(pixel_agents_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("PIXEL_AGENTS_REAL_HOME") or os.getenv("PIXEL_AGENTS_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Pixel Agents profile HOME.

    ``PIXEL_AGENTS_HOME`` scopes Pixel Agents state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={PIXEL_AGENTS_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{PIXEL_AGENTS_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{PIXEL_AGENTS_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Pixel Agents' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["PIXEL_AGENTS_REAL_HOME"] = real_home
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Normalize a cross-boundary cwd when Pixel Agents itself runs inside WSL.
...
def is_container() -> bool:
    ...
    See: PixelResearch/pixel-agents#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under PIXEL_AGENTS_HOME.

    Replaces the ``get_pixel_agents_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, pixel_agents_logging.py, pixel_agents_time.py, etc.).
    """
    return get_pixel_agents_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under PIXEL_AGENTS_HOME."""
    return get_pixel_agents_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under PIXEL_AGENTS_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_pixel_agents_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._pixel_agents_ipv4_patched = True  # type: ignore[attr-defined]
