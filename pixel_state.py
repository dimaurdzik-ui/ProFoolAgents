#!/usr/bin/env python3
"""
SQLite State Store for Pixel Agents.
...
from pixel_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, List, Optional, Tuple, TypeVar

from pixel_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``pixel-agents -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_pixel_agents_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.pixel-agents`` even when a test fixture later
    redirects ``PIXEL_AGENTS_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_pixel_agents_home()`` fresh so a runtime
       ``PIXEL_AGENTS_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_pixel_agents_home() / "state.db"
...
# pixel_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_macos_checkpoint_barrier(conn: sqlite3.Connection) -> None:
    ...
    On Darwin, ``synchropixel=FULL`` (the WAL default) issues a plain
...
def _enforce_macos_synchropixel_full(conn: sqlite3.Connection) -> None:
    """Enforce ``PRAGMA synchropixel=FULL`` on macOS to prevent btree corruption.

    On Darwin, the default ``synchropixel=NORMAL`` only calls ``fsync()``,
    ...
    macOS does not unless we explicitly set ``synchropixel=FULL``, which issues
    ...
    synchropixel mode, even if a prior connection set ``synchropixel=NORMAL``.
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``pixel_cli.kanban_db.connect`` so
    ...
    try:
        ...
        if current_mode and current_mode[0] == "wal":
            ...
            _enforce_macos_synchropixel_full(conn)
            return "wal"
    except sqlite3.OperationalError:
        pass
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    logger.warning(
        ...
        "Pixel Agents-managed installs can repair the embedded runtime with "
        "`pixel-agents update`. See `pixel-agents doctor`. This warning fires once per "
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see pixel_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    *,
    ...
    """
    try:
        # Local import avoids a circular import with pixel_cli.config.
        from pixel_cli.config import cfg_get, load_config_readonly
...
def _backup_db_file(db_path: Path) -> Optional[Path]:
    ...
    connection's POSIX advisory locks (see ``pixel_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Pixel Agents home tree (``get_pixel_agents_home()``) — the safe repair scope:
      Pixel Agents owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``pixel_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_pixel_agents_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Pixel Agents needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (pixel_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_pixel_state.py:600-632)
        ...
        probe_session_id = f"_pixel_fts_health_probe_{time.time_ns()}"
        try:
...
# (``~/.pixel-agents/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``pixel-agents sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("PIXEL_AGENTS_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_pixel_agents_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("PIXEL_AGENTS_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship pixel_state without pixel_cli). A
    ...
    try:
        ...
        logger.debug(
            "pixel_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch pixel_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``pixel_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    try:
        ...
        if not acquired:
            ...
            logger.error(
                ...
                "state-snapshots via `pixel-agents snapshot list` / "
                "`pixel-agents snapshot restore <id>`.",
                path,
            )
            return None
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple pixel-agents processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Pixel Agents
    ...
    # `pixel-agents update` leaves mixed-version processes sharing the DB until
        ...
        try:
            ...
            if (
                ...
                msg = (
                    ...
                    f"Restore from {snaps} via `pixel-agents snapshot list` / "
                    f"`pixel-agents snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`pixel-agents db optimize`),
            ...
            # ``pixel_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `pixel-agents update` to rebuild the venv with a "
            ...
            if cjk_present:
                ...
                if live:
                    ...
                    logger.warning(
                        ...
                        "run `pixel-agents sessions optimize-storage` on a host "
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Pixel Agents process held the "
    ...
    #
    # `optimize_fts_storage()` (the `pixel-agents sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`pixel-agents sessions optimize-storage`) ──
    #
    ...
    # synchropixelly — demote → new schema → chunked backfill → chunked
        ...
        def _do(conn):
            conn.execute(
                """INSERT INTO sessions (
                   ...
                   profile_name, git_repo_root, agent_id, agent_prompt_version, started_at
                )
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                   ON CONFLICT(id) DO UPDATE SET
                       ...
                       agent_prompt_version = COALESCE(sessions.agent_prompt_version, excluded.agent_prompt_version)""",
                (
                    session_id,
                    source,
                    user_id,
        ...
        *,
        ...
        def _do(conn):
            ...
            if git_repo_root is not None:
                updates.append("git_repo_root = ?")
                params.append(git_repo_root)
            ...
            if agent_prompt_version is not None:
                updates.append("agent_prompt_version = ?")
                params.append(agent_prompt_version)
            if not updates:
                return
            params.append(session_id)
    ...
    def update_session_agent(
        ...
        """Set or clear the professional-agent identity for a session.

        ``update_session_meta`` intentionally treats ``None`` as "leave the
        existing value alone".  Agent selection needs different semantics:
        selecting the general chat must write SQL NULLs, not preserve the
        previous specialist's prompt on the next resume.
        """
        self.flush_token_counts()

        def _do(conn):
            conn.execute(
                "UPDATE sessions SET agent_id = ?, agent_prompt_version = ? WHERE id = ?",
                (agent_id, agent_prompt_version, session_id),
            )
    ...
    def queue_token_counts(self, session_id: str, **kwargs) -> None:
        ...
        and applies them asynchropixelly with identical semantics.  Cheap
        (append + notify) — safe to call on the turn thread after every
        API call.  After close() has stopped the writer, falls back to the
        synchropixel path and may raise like :meth:`update_token_counts`.
        """
        with self._token_queue_cond:
            ...
            if not writer_stopped:
                ...
                if thread is None or not thread.is_alive():
                    ...
                    # until a reader's flush drained them synchropixelly.
        ...
        if writer_stopped:
            ...
            # raises at the call site, exactly like the old synchropixel
    ...
    def _stop_token_writer(self, join_timeout: float = 10.0) -> None:
        ...
        if thread is not None and thread.is_alive():
            ...
            if thread.is_alive():
                ...
                return
        # Writer exited (or never started) — apply leftovers synchropixelly.
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, Pixel) receive
        ...
        if repair_alternation and messages:
            # Lazy import: pixel_state already depends on agent.* (see
    ...
    def get_conversation_root(self, session_id: str) -> str:
        ...
        identifier — used for Pixel Portal ``conversation=`` usage tagging.
    ...
    def search_sessions(
        ...
        ``pixel-agents -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``pixel-agents sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Pixel Agents, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Pixel Agents session.

        A Pixel Agents session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Pixel Agents session is already bound to any Telegram DM topic.
    ...
    def install_agent(self, agent_id: str, template_id: str, prompt_version: int) -> None:
        """Install an agent template for the user."""
        def _do(conn):
            conn.execute(
                "INSERT OR REPLACE INTO installed_agents (id, template_id, prompt_version, created_at, is_active) VALUES (?, ?, ?, ?, 1)",
                (agent_id, template_id, prompt_version, time.time())
            )
        self._execute_write(_do)

    def uninstall_agent(self, agent_id: str) -> None:
        """Uninstall an agent from the user's catalog."""
        def _do(conn):
            conn.execute("DELETE FROM installed_agents WHERE id = ?", (agent_id,))
        self._execute_write(_do)
    ...
    def list_installed_agents(self) -> List[Dict[str, Any]]:
        ...
        conn = self._get_read_conn()
        if not conn: return []
        rows = conn.execute("SELECT id, template_id, prompt_version, created_at, is_active FROM installed_agents WHERE is_active = 1 ORDER BY created_at ASC").fetchall()
        return [dict(r) for r in rows]
