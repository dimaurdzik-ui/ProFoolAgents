...
- **Note:** The previous Open Refine reconciliation endpoint at
  ...
  the ZIP locally (default `~/.cache/pixel-agents-osint/icij/`, refreshes after
...
First call downloads the ~70 MB ZIP under `~/.cache/pixel-agents-osint/icij/`
(or `$PIXEL_AGENTS_OSINT_CACHE/icij/`). Subsequent calls reuse the cache for 30 days.
