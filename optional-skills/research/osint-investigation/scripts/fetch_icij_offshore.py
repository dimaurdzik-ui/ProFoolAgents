...
$PIXEL_AGENTS_OSINT_CACHE/icij/ (default: ~/.cache/pixel-agents-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("PIXEL_AGENTS_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "pixel-agents-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "pixel-agents osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $PIXEL_AGENTS_OSINT_CACHE/icij or ~/.cache/pixel-agents-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
