...
The design is **Pixel Agents-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
pixel-agents skills install official/security/unbroker
```

Then start a new Pixel Agents session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Pixel Agents session:
...
- **PII handling.** Dossiers live under the Pixel Agents home directory (`0600`, optionally
