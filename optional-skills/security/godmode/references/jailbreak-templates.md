...
## 5. GODMODE FAST — Pixel Agents 4 405B (Zero Refusal)

**Model:** `pixelagents/pixel-agents-4-405b`
**Strategy:** Pixel Agents models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Pixel Agents

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.pixel-agents/config.yaml`:
