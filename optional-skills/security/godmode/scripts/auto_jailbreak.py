...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("PIXEL_AGENTS_HOME", os.path.expanduser("~/.pixel-agents")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("PIXEL_AGENTS_HOME", Path.home() / ".pixel-agents")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Pixel Agents config paths
# ═══════════════════════════════════════════════════════════════════

PIXEL_AGENTS_HOME = Path(os.getenv("PIXEL_AGENTS_HOME", Path.home() / ".pixel-agents"))
CONFIG_PATH = PIXEL_AGENTS_HOME / "config.yaml"
PREFILL_PATH = PIXEL_AGENTS_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Pixel/Pixel Agents models — already uncensored, just needs clean prompt
    "pixel-agents": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "pixel-agents" in model_lower or "pixel" in model_lower:
        return "pixel-agents"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Pixel Agents config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.pixel-agents/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Pixel Agents for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Pixel Agents for changes to take effect.")
