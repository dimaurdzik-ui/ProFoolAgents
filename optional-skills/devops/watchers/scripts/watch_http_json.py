...
Usage (via cron with --no-agent):
    ...
    pixel-agents cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$PIXEL_AGENTS_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Pixel Agents-Watcher/1.0"})
