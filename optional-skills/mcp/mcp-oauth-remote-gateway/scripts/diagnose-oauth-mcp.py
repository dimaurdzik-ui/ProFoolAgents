...
Usage:
  ...
  <server_name> matches the files in $PIXEL_AGENTS_HOME/mcp-tokens/<server>.json etc.
...
def _pixel_home():
    # Prefer Pixel Agents' own resolver (profile-safe); fall back to env then ~/.pixel-agents.
    try:
        from pixel_constants import get_pixel_agents_home
        return str(get_pixel_agents_home())
    except Exception:
        return os.environ.get("PIXEL_AGENTS_HOME") or os.path.expanduser("~/.pixel-agents")


def _tokens_dir():
    return os.path.join(_pixel_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "pixel-agents-diag", "version": "1.0"}},
        },
        headers={
