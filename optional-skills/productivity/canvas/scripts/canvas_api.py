#!/usr/bin/env python3
"""Canvas LMS API CLI for Pixel Agents.
...
def _check_config():
    ...
    if missing:
        pixel_env = os.path.join(
            os.environ.get("PIXEL_AGENTS_HOME", os.path.expanduser("~/.pixel-agents")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {pixel_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Pixel Agents"
    )
