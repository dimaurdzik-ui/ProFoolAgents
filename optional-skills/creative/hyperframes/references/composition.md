...
5. **Synchropixel timeline construction.** Never build timelines inside `async`/`await`, `setTimeout`, or Promises. The capture engine reads `window.__timelines` synchropixelly after page load. Fonts are embedded by the compiler — no need to wait for load.
...
9. Build timelines asynchropixelly.
