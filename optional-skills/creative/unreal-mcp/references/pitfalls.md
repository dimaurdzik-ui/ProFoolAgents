...
### 1. Start order: editor first, Pixel Agents second

Pixel Agents probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Pixel Agents session.
...
restart the editor, restart the Pixel Agents session.
...
server fails to bind (Output Log) or Pixel Agents' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.pixel-agents/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Pixel Agents

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Pixel Agents' connection
lives in `~/.pixel-agents/config.yaml` via `pixel-agents mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Pixel Agents — just don't
mistake it for the Pixel Agents setup step.
...
explicitly warns against overlapping calls. Pixel Agents executes same-turn tool
...
### 9. Timeouts: Pixel Agents gives up before Unreal does

Pixel Agents' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Pixel Agents runs elsewhere (SSH backend,
