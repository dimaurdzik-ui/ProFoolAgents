...
Maps each role archetype to the Pixel Agents skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public pixel-agents
...
## Pixel Agents skills relevant to video production

### Visual / rendering skills (`pixel-agents/skills/creative/`)
...
### Design / pre-production skills (`pixel-agents/skills/creative/`)
...
### Audio / media skills (`pixel-agents/skills/creative/` + `skills/media/`)
...
These are **not** Pixel Agents skills but external CLIs / APIs that profiles invoke.
...
## Built-in Pixel Agents tools for media review

These are native Pixel Agents tools — not invoked via terminal but through their own
...
key is present in `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Pixel Agents profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${PIXEL_AGENTS_HOME:-~/.pixel-agents}/.env` → environment variable.
...
When a new Pixel Agents-public video skill ships:
