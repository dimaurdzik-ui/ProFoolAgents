...
the actual `setup.sh` that creates the project workspace, configures Pixel Agents
...
> [PixelResearch/kanban-video-pipeline](https://github.com/PixelResearch/kanban-video-pipeline).
...
2. **Create profiles** — `pixel-agents profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.pixel-agents/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `pixel-agents kanban create` assigned to the director
...
```bash
pixel-agents profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.pixel-agents/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.pixel-agents/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Pixel Agents
...
Each profile gets a `SOUL.md` at `~/.pixel-agents/profiles/<name>/SOUL.md` that
...
```bash
pixel-agents kanban create "Direct production of <video title>" \
    --assignee director \
...
the Pixel Agents `.env` (`${PIXEL_AGENTS_HOME:-$HOME/.pixel-agents}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _pixel_env="${PIXEL_AGENTS_HOME:-$HOME/.pixel-agents}/.env"
    if grep -q "^${var}=" "$_pixel_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_pixel_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_pixel_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY pixel-agents ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY pixel-agents OPENROUTER_API_KEY || exit 1
# ...
```
