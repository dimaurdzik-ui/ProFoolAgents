...
pixel-agents kanban watch --tenant <project-slug>

# Snapshot of the board
pixel-agents kanban list --tenant <project-slug>
pixel-agents kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
pixel-agents kanban stats --tenant <project-slug>

# Visual dashboard (browser)
pixel-agents dashboard

# Inspect a specific task (includes comments + events)
pixel-agents kanban show <task-id>

# Follow a single task's event stream
pixel-agents kanban tail <task-id>
```

Verify available subcommands with `pixel-agents kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `pixel-agents kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `pixel-agents kanban block <id>` then `pixel-agents kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `pixel-agents kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `pixel-agents kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `pixel-agents profile list` (profile exists?), `pixel-agents status` (gateway/dispatcher up?), and verify tenant. |
...
pixel-agents kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
pixel-agents kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(pixel-agents kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
pixel-agents kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
pixel-agents kanban block <task-id>
pixel-agents kanban archive <task-id>

# Diagnose what happened
pixel-agents kanban show <task-id>      # task body, comments, recent events
pixel-agents kanban tail <task-id>      # follow the live event stream
pixel-agents kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `pixel-agents kanban create` for the director
...
while true; do
    clear
    pixel-agents kanban list --tenant <slug>
    echo "---"
    pixel-agents kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `pixel-agents kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `pixel-agents profile list` and start
