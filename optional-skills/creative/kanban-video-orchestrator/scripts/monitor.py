#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `pixel-agents kanban list` and
...
def pixel_available() -> bool:
    return shutil.which("pixel-agents") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed pixel-agents CLI."""
    try:
        out = subprocess.run(
            ["pixel-agents", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `pixel-agents kanban list`
    out = subprocess.run(
        ["pixel-agents", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["pixel-agents", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not pixel_available():
        print("ERROR: 'pixel-agents' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
