# Pixel Agents Security Policy

This document describes Pixel Agents's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/PixelResearch/pixel-agents/security/advisories/new)
or **security@pixelagents.com**. Do not open public issues for
security vulnerabilities. **Pixel Agents does not operate a bug
bounty program.**
...
- Environment details (`pixel-agents version`, commit SHA, OS, Python
  version).
...
Pixel Agents is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Pixel Agents,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Pixel Agents access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Pixel Agents's documentation
...
Pixel Agents supports two OS-level isolation postures. They address
...
Pixel Agents supports this in two ways:

- **Pixel Agents's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Pixel Agents's in-process heuristics
...
Pixel Agents filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Pixel Agents
itself. Bugs in Pixel Agents's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Pixel Agents does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Pixel Agents's own documentation, or reasonable
  operator expectations would predict — including cases where
  Pixel Agents has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  pixel-agents-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Pixel Agents's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Pixel Agents. Bugs in Pixel Agents
...
- Pixel Agents includes supply-chain guards for MCP server
