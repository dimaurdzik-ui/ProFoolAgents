...
Browser-based dashboard for managing Pixel Agents configuration, API keys, and monitoring active sessions.
...
cd ../
python -m pixel_cli.main web --no-open
...
`pixel-agents dashboard` on port 9119 serves the **built** bundle from `pixel_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
This outputs to `../pixel_cli/web_dist/`, which the FastAPI server serves as a static SPA. The built assets are included in the Python package via `pyproject.toml` package-data.
...
- Use the **semantic text tokens** from `@pixel-agents/ui`'s `globals.css`:
...
- The dashboard preserves the Pixel brand uppercase aesthetic, but it is **opt-in per element, not global**.
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `pixel-agents/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Pixel Agents” — mixed case, no Mondwest, no `text-display` |
