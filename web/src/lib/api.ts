import { buildPixelAgentsWebSocketUrl } from "@pixel-agents/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/pixel-agents/). The Python backend
// injects ``window.__PIXEL_AGENTS_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__PIXEL_AGENTS_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const PIXEL_AGENTS_BASE_PATH = readBasePath();
const BASE = PIXEL_AGENTS_BASE_PATH;
...
declare global {
  interface Window {
    __PIXEL_AGENTS_SESSION_TOKEN__?: string;
    __PIXEL_AGENTS_BASE_PATH__?: string;
    ...
    __PIXEL_AGENTS_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Pixel-Agents-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__PIXEL_AGENTS_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``pixel-agents update``, ``pixel-agents gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__PIXEL_AGENTS_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__PIXEL_AGENTS_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("pixel-agents.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("pixel-agents.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__PIXEL_AGENTS_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("pixel-agents.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
 ...
 *
 * The dashboard's gated-mode WS auth (``pixel_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__PIXEL_AGENTS_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Pixel-Agents-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``pixel_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__PIXEL_AGENTS_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildPixelAgentsWebSocketUrl({
...
export const api = {
  ...
  updatePixelAgents: () =>
    fetchJSON<ActionResponse>("/api/pixel-agents/update", { method: "POST" }),
  checkPixelAgentsUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/pixel-agents/update/check${force ? "?force=true" : ""}`,
    ),
 ...
 * Pixel Portal contract V1 (the access token has no email/name claims —
...
export interface SkillHubSource {
  ...
  /** pixel-agents-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
  ...
  /** Phase 7: registered ``DashboardAuthProvider`` names (e.g. ``["pixel"]``).
   ...
   * updates are handled by the outer launcher instead of ``pixel-agents update``. */
  can_update_pixel?: boolean;
  config_path: string;
