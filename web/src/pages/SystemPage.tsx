...
import { Select, SelectOption } from "@pixel-agents/ui/ui/components/select";
...
import { PixelAgentsConsoleModal } from "@/components/PixelAgentsConsoleModal";
...
export default function SystemPage() {
  ...
  // spawned `pixel-agents import` runs non-interactively (stdin is /dev/null), so
  ...
  const loadAll = useCallback(() => {
    Promise.allSettled([
      ...
      api.checkPixelAgentsUpdate(false),
    ])
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_pixel === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkPixelAgentsUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_pixel],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_pixel === false) {
      showToast(
        "Pixel Agents updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updatePixelAgents();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "pixel-agents-update");
  ...
  const canUpdatePixelAgents = status?.can_update_pixel !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdatePixelAgents && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Pixel Agents?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'pixel-agents update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'pixel-agents update' (${updateInfo?.update_command ?? "pixel-agents update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Pixel Agents</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.pixel_version}</span>
                  {canUpdatePixelAgents &&
                    updateInfo &&
      ...
      <section className="flex flex-col gap-3">
        <H2 variant="sm" className="flex items-center gap-2 text-muted-foreground">
          <Globe className="h-4 w-4" /> Pixel Portal
        </H2>
        <Card>
          <CardContent className="flex flex-col gap-3 py-4">
            <div className="flex items-center gap-3">
              ...
              <a
                href={portal?.subscription_url || "https://portal.pixelagents.com/manage-subscription"}
                target="_blank"
                rel="noreferrer"
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">pixel-agents portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$PIXEL_AGENTS_HOME/backups/pixel-agents-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Pixel Agents backup?"
              description={`This will overwrite your current Pixel Agents configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Pixel Agents team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
