...
interface PixelAgentsConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function PixelAgentsConsoleModal({ open, onClose }: PixelAgentsConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("pixel-agents> ");
  const inputPromptRef = useRef("pixel-agents> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "pixel-agents> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Pixel Agents Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="pixel-agents-console-title"
    >
      <div
        className={cn(
          themedBody,
