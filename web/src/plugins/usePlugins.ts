...
import { api, PIXEL_AGENTS_BASE_PATH } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${PIXEL_AGENTS_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${PIXEL_AGENTS_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?pixel_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-pixel-agents-plugin", manifest.name);
