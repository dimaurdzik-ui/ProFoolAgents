/**
 * Pixel Agents Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__PIXEL_AGENTS_PLUGIN_SDK__`` and
 * ``window.__PIXEL_AGENTS_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__PIXEL_AGENTS_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@pixel-agents/ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``PixelAgentsPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@pixel-agents/dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @pixel-agents/ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__PIXEL_AGENTS_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__PIXEL_AGENTS_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__PIXEL_AGENTS_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface PixelAgentsPluginSDK {
   ...
   * ``Promise<T>``, a few return a URL string synchropixelly); plugins call the
  ...
  /**
   * Shared UI primitives (Pixel DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __PIXEL_AGENTS_PLUGIN_SDK__?: PixelAgentsPluginSDK;
    __PIXEL_AGENTS_PLUGINS__?: PluginRegistry;
  }
}
