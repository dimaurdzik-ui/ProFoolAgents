...
($INSTALL_DIR), not into the shared $PIXEL_AGENTS_HOME.

Background (shared-$PIXEL_AGENTS_HOME bug)
------------------------------------
$PIXEL_AGENTS_HOME is a data directory users frequently bind-mount into a Docker
gateway as well (``~/.pixel-agents:/opt/data``). The published image stamps 'docker'
there on boot, so if install.sh had written its 'git' marker into the same
$PIXEL_AGENTS_HOME the two installs would fight over one slot — and the container,
booting last, would win and wrongly make the host install look like 'docker'
(blocking ``pixel-agents update``).
...
``~/.pixel-agents/pixel-agents``), which is unique to this install and immune to the
shared data dir.
"""
...
def test_install_sh_stamps_code_tree_not_home() -> None:
    ...
    assert not re.search(r'>\s*"\$PIXEL_AGENTS_HOME/\.install_method"', text), (
        "install.sh must not stamp $PIXEL_AGENTS_HOME/.install_method — that data "
        "dir may be shared with a Docker gateway whose 'docker' stamp would "
        "clobber it and block host-side `pixel-agents update`"
    )
