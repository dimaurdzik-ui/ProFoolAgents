"""
Tests for timezone support (pixel_time module + integration points).
...
def _reset_pixel_time_cache():
    """Reset the pixel_time module cache (replacement for removed reset_cache)."""
    pixel_time._cached_tz = None
    pixel_time._cached_tz_name = None
    pixel_time._cache_resolved = False


# =========================================================================
# pixel_time.now() — core helper
# =========================================================================

class TestPixelAgentsTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_pixel_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["PIXEL_AGENTS_TIMEZONE"] = "Asia/Kolkata"
        result = pixel_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["PIXEL_AGENTS_TIMEZONE"] = "UTC"
        result = pixel_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["PIXEL_AGENTS_TIMEZONE"] = "Europe/London"
        tz = pixel_time.get_timezone()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When PIXEL_AGENTS_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) PIXEL_AGENTS_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("PIXEL_AGENTS_TIMEZONE=" + os.environ.get("PIXEL_AGENTS_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "PIXEL_AGENTS_TIMEZONE=NOT_SET" in result["output"], (
            "PIXEL_AGENTS_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When PIXEL_AGENTS_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("PIXEL_AGENTS_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=pixel_tz) which shifted
        absolute time when system-local tz != Pixel Agents tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Pixel Agents tz
        ...
        # Use a Pixel Agents timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _pixel_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["PIXEL_AGENTS_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_pixel_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Pixel Agents timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["PIXEL_AGENTS_TIMEZONE"] = "US/Eastern"
        _reset_pixel_time_cache()
