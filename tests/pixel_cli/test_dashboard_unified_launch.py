...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "pixel_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # PIXEL_AGENTS_HOME.  For a standard install (PIXEL_AGENTS_HOME unset) that root is
        # the platform-native default (~/.pixel-agents), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from pixel_constants import get_default_pixel_agents_root
        assert env.get("PIXEL_AGENTS_HOME") == str(get_default_pixel_agents_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (PIXEL_AGENTS_DESKTOP=1) must NOT
