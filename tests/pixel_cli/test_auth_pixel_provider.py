"""Regression tests for Pixel OAuth refresh and inference JWT interactions."""
...
from pixel_cli.auth import AuthError, get_provider_auth_state, resolve_pixel_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from pixel_cli.auth import _resolve_verify
...
def _setup_pixel_auth(
    ...
    *,
    ...
    (pixel_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_pixel_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    tmp_path,
    monkeypatch,
):
    import pixel_cli.auth as auth_mod
    ...
    _setup_pixel_auth(
        pixel_home,
        access_token=token,
        scope=auth_mod.DEFAULT_PIXEL_SCOPE,
        expires_at=_future_iso(3600),
        expires_in=3600,
    )
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))

    creds = auth_mod.resolve_pixel_runtime_credentials()

    assert creds["api_key"] == token
    assert creds["source"] == auth_mod.PIXEL_AUTH_PATH_INVOKE_JWT
    assert creds["auth_path"] == auth_mod.PIXEL_AUTH_PATH_INVOKE_JWT

    payload = json.loads((pixel_home / "auth.json").read_text())
    singleton = payload["providers"]["pixel"]
    assert singleton["agent_key"] == token
    assert datetime.fromisoformat(singleton["agent_key_expires_at"]).timestamp() > time.time() + 300

    pool_entries = payload["credential_pool"]["pixel"]
    assert len(pool_entries) == 1
    assert pool_entries[0]["agent_key"] == token
    assert pool_entries[0]["source"] == auth_mod.PIXEL_DEVICE_CODE_SOURCE


def test_resolve_pixel_runtime_credentials_invoke_jwt_is_idempotent(
    tmp_path,
    monkeypatch,
):
    import pixel_cli.auth as auth_mod
    ...
    auth_store = {
        ...
        "providers": {
            "pixel": {
                "portal_base_url": "https://portal.pixelagents.com",
                "inference_base_url": "https://inference-api.pixelagents.com/v1",
                "client_id": "pixel-agents-cli",
                "token_type": "Bearer",
                "scope": auth_mod.DEFAULT_PIXEL_SCOPE,
                "access_token": token,
    ...
    auth_path = pixel_home / "auth.json"
    ...
    monkeypatch.setattr(auth_mod, "_write_shared_pixel_state", _unexpected_shared_write)
    monkeypatch.setattr(
        auth_mod,
    ...
    assert (
        payload["providers"]["pixel"]["agent_key_obtained_at"]
        == original_obtained_at
    )


def test_resolve_pixel_runtime_credentials_reauths_when_invoke_scope_missing(
    tmp_path,
    monkeypatch,
):
    import pixel_cli.auth as auth_mod
    ...
    with pytest.raises(AuthError) as exc:
        auth_mod.resolve_pixel_runtime_credentials()

    assert exc.value.code == "missing_inference_invoke_scope"
    assert exc.value.relogin_required is True
    payload = json.loads((pixel_home / "auth.json").read_text())
    assert payload["providers"]["pixel"]["agent_key"] is None
    assert "credential_pool" not in payload or not payload["credential_pool"].get("pixel")
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))
    monkeypatch.setenv("PIXEL_AGENTS_AGENT_USE_LEGACY_SESSION_KEYS", "true")

    creds = auth_mod.resolve_pixel_runtime_credentials()

    assert creds["api_key"] == token
    payload = json.loads((pixel_home / "auth.json").read_text())
    assert payload["providers"]["pixel"]["agent_key"] == token
    ...
    result = auth_mod._pixel_device_code_login(
        portal_base_url="https://portal.example.com",
        inference_base_url="https://inference.example.com/v1",
        open_browser=False,
        timeout_seconds=1,
    ...
    assert requested_scopes == [auth_mod.DEFAULT_PIXEL_SCOPE]
    assert result["agent_key"] == login_token


def test_pixel_inference_auth_logs_do_not_include_secret_values(
    tmp_path,
    monkeypatch,
    caplog,
):
    ...
    caplog.set_level(logging.DEBUG, logger="pixel_cli.auth")
    auth_mod.resolve_pixel_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_pixel_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_pixel_auth_status() should find Pixel credentials in the pool
    even when the auth store has no Pixel provider entry — this is the
    case when login happened via the dashboard device-code flow which
    saves to the pool only.
    """
    from pixel_cli.auth import get_pixel_auth_status

    pixel_home = tmp_path / "pixel-agents"
    pixel_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no Pixel provider entry
    (pixel_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))

    # Seed the credential pool with a Pixel entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("pixel")
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    entry = PooledCredential.from_dict("pixel", {
        "access_token": token,
    ...
    status = get_pixel_auth_status()
    assert status["logged_in"] is True
    assert "example.com" in str(status.get("portal_base_url", ""))


def test_get_pixel_auth_status_empty_returns_not_logged_in(tmp_path, monkeypatch):
    """get_pixel_auth_status() returns logged_in=False when both pool
...
# =============================================================================
# _login_pixel: "Skip (keep current)" must preserve prior provider + model
# =============================================================================


class TestLoginPixelSkipKeepsCurrent:
    """When a user runs `pixel-agents model` → Pixel Portal → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "pixel" while
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        config_path = pixel_home / "config.yaml"
        ...
        return pixel_home, config_path, auth_path

    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        """Patch OAuth + model-list + prompt so _login_pixel doesn't hit network."""
        import pixel_cli.auth as auth_mod
        import pixel_cli.models as models_mod
        import pixel_cli.pixel_subscription as ns

        fake_auth_state = {
            "access_token": "fake-pixel-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.pixelagents.com",
            "portal_base_url": "https://portal.pixelagents.com",
            "refresh_token": "fake-refresh",
            "token_expires_at": 9999999999,
        }
        monkeypatch.setattr(
            auth_mod, "_pixel_device_code_login",
        ...
        def _check_pixel_free_tier(**kwargs):
            free_tier_calls.append(kwargs)
            return None

        monkeypatch.setattr(models_mod, "check_pixel_free_tier", _check_pixel_free_tier)
        monkeypatch.setattr(
            models_mod, "partition_pixel_models_by_tier",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, Pixel creds still saved."""
        import argparse
        import yaml
        from pixel_cli.auth import PROVIDER_REGISTRY, _login_pixel

        pixel_home, config_path, auth_path = self._setup_home_with_openrouter(
        ...
        _login_pixel(args, PROVIDER_REGISTRY["pixel"])
        ...
        # auth.json: active_provider restored to openrouter, but Pixel creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "pixel" in auth_after["providers"]
        assert auth_after["providers"]["pixel"]["access_token"] == "fake-pixel-token"
        # Existing openrouter creds still intact
        assert auth_after["providers"]["openrouter"]["api_key"] == "sk-or-fake"

    def test_picking_model_switches_to_pixel(self, tmp_path, monkeypatch):
        """User picks a Pixel model → provider flips to pixel with that model."""
        import argparse
        import yaml
        from pixel_cli.auth import PROVIDER_REGISTRY, _login_pixel
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        instead of leaving it as pixel."""
        import argparse
        import yaml
        from pixel_cli.auth import PROVIDER_REGISTRY, _login_pixel
        ...
        auth_path = pixel_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "pixel" after Skip
        assert auth_after.get("active_provider") in {None, ""}
        # But Pixel creds are still saved
        assert "pixel" in auth_after.get("providers", {})


# =============================================================================
# persist_pixel_credentials: shared helper for CLI + web dashboard login paths
# =============================================================================


def _full_state_fixture() -> dict:
    """Shape of the dict returned by _pixel_device_code_login /
    refresh_pixel_oauth_from_state. Used as helper input."""
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    return {
        "portal_base_url": "https://portal.example.com",
...
def test_persist_pixel_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    """Helper must populate BOTH credential_pool.pixel AND providers.pixel.

    Regression guard: before this helper existed, `pixel-agents auth add pixel`
    wrote only the pool. After the Pixel agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_pixel_runtime_credentials
    which reads providers.pixel, found it empty, raised AuthError, and the
    agent failed with "Non-retryable client error". Both stores must stay
    in sync at write time.
    """
    from pixel_cli.auth import persist_pixel_credentials, PIXEL_DEVICE_CODE_SOURCE
    ...
    entry = persist_pixel_credentials(state)

    assert entry is not None
    assert entry.provider == "pixel"
    assert entry.source == PIXEL_DEVICE_CODE_SOURCE

    payload = json.loads((pixel_home / "auth.json").read_text())

    # providers.pixel populated with the full state (new behaviour)
    singleton = payload["providers"]["pixel"]
    ...
    # credential_pool.pixel has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["pixel"]
    assert len(pool_entries) == 1, pool_entries
    pool_entry = pool_entries[0]
    assert pool_entry["source"] == PIXEL_DEVICE_CODE_SOURCE
    assert pool_entry["agent_key"] == state["agent_key"]
    assert pool_entry["inference_base_url"] == "https://inference.example.com/v1"


def test_persist_pixel_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.pixel and lets seeding
    ...
    persist_pixel_credentials(first)
    ...
    persist_pixel_credentials(second)

    payload = json.loads((pixel_home / "auth.json").read_text())

    # providers.pixel reflects the latest write (singleton semantics)
    assert payload["providers"]["pixel"]["access_token"] == second_token
    assert payload["providers"]["pixel"]["agent_key"] == second_token

    # credential_pool.pixel has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["pixel"]
    assert len(pool_entries) == 1, pool_entries
    assert pool_entries[0]["source"] == PIXEL_DEVICE_CODE_SOURCE
...
def test_persist_pixel_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    ...
    entry = persist_pixel_credentials(_full_state_fixture())
    assert entry is not None
    ...
    # No "label" key embedded in providers.pixel when the caller didn't supply one.
    payload = json.loads((pixel_home / "auth.json").read_text())
    assert "label" not in payload["providers"]["pixel"]


def test_refresh_token_reuse_detection_surfaces_actionable_message():
    """Regression for #15099.

    When the Pixel Portal server returns ``invalid_grant`` with
    ``error_description`` containing "reuse detected", Pixel Agents must surface an
    ...
    re-authenticate" string led users to report this as a Pixel Agents persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from pixel_cli.auth import _refresh_access_token
    ...
    assert "pixel-agents auth add pixel" in message.lower()
...
def test_refresh_token_exchange_sends_refresh_token_header():
    """Pixel refresh tokens must be sent in a header so sandbox proxies can
    ...
    assert client.kwargs["headers"]["x-pixel-refresh-token"] == "refresh-1"
...
# =============================================================================
# Shared Pixel token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================


@pytest.fixture
def shared_store_env(tmp_path, monkeypatch):
    """Redirect PIXEL_AGENTS_SHARED_AUTH_DIR to a tmp_path.

    Required for every test that exercises the shared Pixel store — the
    ...
    monkeypatch.setenv("PIXEL_AGENTS_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without PIXEL_AGENTS_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.pixel-agents/shared/`` across CI runs.
    """
    from pixel_cli.auth import _pixel_shared_store_path

    monkeypatch.delenv("PIXEL_AGENTS_SHARED_AUTH_DIR", raising=False)

    with pytest.raises(RuntimeError, match="shared Pixel auth store"):
        _pixel_shared_store_path()
...
def test_shared_store_write_and_read_roundtrip(shared_store_env):
    ...
    loaded = _read_shared_pixel_state()
...
def test_persist_pixel_credentials_mirrors_to_shared_store(
    tmp_path, monkeypatch, shared_store_env,
):
    """persist_pixel_credentials must populate BOTH per-profile auth.json
    AND the shared store, so a future profile's `pixel-agents auth add pixel
    --type oauth` can one-tap import instead of redoing device-code.
    """
    from pixel_cli.auth import (
        _pixel_shared_store_path,
        _read_shared_pixel_state,
        persist_pixel_credentials,
    )
    ...
    persist_pixel_credentials(_full_state_fixture())

    # Per-profile auth.json populated
    payload = json.loads((pixel_home / "auth.json").read_text())
    assert "pixel" in payload.get("providers", {})
    ...
    assert str(_pixel_shared_store_path()).startswith(str(shared_store_env))
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    every field persist_pixel_credentials() needs.
    """
    from pixel_cli import auth as auth_mod

    auth_mod._write_shared_pixel_state(_full_state_fixture())
    ...
    monkeypatch.setattr(auth_mod, "refresh_pixel_oauth_from_state", _fake_refresh)

    result = auth_mod._try_import_shared_pixel_state()
    ...
    assert result["client_id"] == "pixel-agents-cli"
...
class TestStalePortalBaseUrlMigration:
    """_migrate_stale_pixel_portal_url auto-corrects stale portal_base_url on load."""

    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from pixel_cli.auth import _load_auth_store, DEFAULT_PIXEL_PORTAL_URL
        ...
        auth_file.write_text(json.dumps({
            ...
            "providers": {
                "pixel": {
                    "portal_base_url": "https://api.pixelagents.com",
        ...
        pixel = store["providers"]["pixel"]
        assert pixel["portal_base_url"] == DEFAULT_PIXEL_PORTAL_URL
    ...
    def test_runtime_credentials_rejects_http_for_production_portal(
        ...
        auth_file = pixel_home / "auth.json"
        store = json.loads(auth_file.read_text())
        store["providers"]["pixel"]["portal_base_url"] = (
            "http://portal.pixelagents.com"
        )
        ...
        auth_mod.resolve_pixel_runtime_credentials()
        assert refresh_calls == [auth_mod.DEFAULT_PIXEL_PORTAL_URL]
