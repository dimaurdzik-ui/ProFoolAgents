...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    PIXEL_AGENTS_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_pixel_argv", lambda: ["pixel-agents"])
    ...
    assert pid == 4242
    assert captured["env"]["PIXEL_AGENTS_HOME"] == str(profile)
    assert captured["env"]["PIXEL_AGENTS_KANBAN_TASK"] == "t_spawn_tools"
