...
def pixel_home(tmp_path, monkeypatch):
    ...
    # /etc/pixel-agents on the dev/CI box can't influence the test.
    monkeypatch.setenv("PIXEL_AGENTS_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(pixel_home, monkeypatch):
    from pixel_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(pixel_home, monkeypatch):
    from pixel_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_pixel_dotenv(pixel_home=str(home))
