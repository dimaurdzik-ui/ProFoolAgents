...
from pixel_cli.dashboard_auth.cookies import SESSION_AT_COOKIE
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``pixel_session_at`` / ``pixel_session_rt``
...
class _UnreachableProvider(StubAuthProvider):
    ...
    Models the real-world bug — a self-hosted-OIDC session hits the ``pixel``
    provider first, which tries to reach Pixel Portal's JWKS; if that's
    unreachable ``pixel`` raises ProviderError. The gate must keep trying the
    ...
    def verify_session(self, *, access_token: str):
        from pixel_cli.dashboard_auth.base import ProviderError
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["pixel_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
