...
from pixel_cli.copilot_auth import _try_gh_cli_token
...
class TestProviderRegistry:
    ...
    def test_oauth_providers_unchanged(self):
        ...
        assert "pixel" in PROVIDER_REGISTRY
        assert PROVIDER_REGISTRY["pixel"].auth_type == "oauth_device_code"
...
from pixel_cli.auth import PROVIDER_REGISTRY as _REGISTRY

_EXTRA_ENV_VARS = (
    # Checked directly in resolve_provider("auto"), not via the registry.
    "OPENROUTER_API_KEY", "PIXEL_API_KEY",
    ...
    "PIXEL_AGENTS_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "PIXEL_AGENTS_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("pixel_cli.auth._load_auth_store", lambda: {})
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("pixel_cli.copilot_auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "pixel_cli.copilot_auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "pixel_cli.copilot_auth.os.access",
        ...
        monkeypatch.setattr("pixel_cli.copilot_auth.subprocess.run", _fake_run)
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("pixel_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        from pixel_cli.runtime_provider import resolve_runtime_provider
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("pixel_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "pixel_cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "pixel_cli.models.fetch_github_model_catalog",
    ...
    def test_runtime_copilot_acp_uses_process_runtime(self, monkeypatch):
        monkeypatch.setattr("pixel_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
        monkeypatch.setenv("PIXEL_AGENTS_COPILOT_ACP_ARGS", "--acp --stdio --debug")
...
class TestHasAnyProviderConfigured:
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when Pixel Agents is unconfigured."""
        from pixel_cli import config as config_module
        from pixel_cli.auth import PROVIDER_REGISTRY
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: pixel_home / ".env")
        monkeypatch.setattr(config_module, "get_pixel_agents_home", lambda: pixel_home)
        monkeypatch.setattr("pixel_cli.copilot_auth.resolve_copilot_token", lambda: ("", ""))
        ...
        monkeypatch.setattr("pixel_cli.auth.get_auth_status", lambda _pid: {})
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir()
        config_file = pixel_home / "config.yaml"
...
class TestKimiCodeCredentialAutoDetect:
    ...
    def test_non_kimi_providers_unaffected(self, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
...
class TestZaiEndpointAutoDetect:
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from pixel_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from pixel_cli.main import _PROVIDER_MODELS as main_models
        from pixel_cli.models import _PROVIDER_MODELS as models_models
...
class TestNovitaProvider:
    ...
    def test_novita_pricing_cache(self, monkeypatch):
        ...
        from pixel_cli import models as models_mod
...
class TestFetchDeepInfraModels:
    ...
    def test_returns_filtered_models_on_success(self, monkeypatch):
        ...
        import pixel_cli.models as models
        monkeypatch.setattr(
            models, "_urlopen_model_catalog_request", lambda *a, **kw: _Resp()
        )
        from pixel_cli.models import _fetch_deepinfra_models
...
class TestDeepInfraTagFiltering:
    ...
    def test_filters_by_surface_tag_and_handles_rollout_states(self, monkeypatch):
        ...
        from pixel_cli.models import _fetch_deepinfra_models_by_tag
        import pixel_cli.models as _m
...
class TestDeepInfraProviderProfile:
    ...
    def test_profile_registered_with_alias_and_aux(self):
        ...
        from pixel_cli.auth import PROVIDER_REGISTRY, resolve_provider
        from pixel_cli.config import OPTIONAL_ENV_VARS
        from pixel_cli.models import CANONICAL_PROVIDERS
