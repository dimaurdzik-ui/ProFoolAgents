"""Tests for the direct Pixel Agents-to-Relay shared-metrics runtime."""
...
from pixel_cli import lifecycle, plugins
from pixel_cli.observability import relay_runtime, relay_shared_metrics
from pixel_cli.plugins import PluginManager
...
def direct_runtime(tmp_path, monkeypatch):
    fake = _Relay()
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / "pixel-agents-home"))
    monkeypatch.setattr(relay_runtime, "_load_nemo_relay", lambda: fake)
    monkeypatch.setattr(
        "pixel_cli.config.read_raw_config_readonly",
...
def test_direct_runtime_records_without_enabling_a_plugin(direct_runtime, tmp_path):
    ...
    assert scope_starts[1][1] == "pixel-agents.task_run"
    ...
    root = tmp_path / "pixel-agents-home" / "telemetry" / "shared_metrics"
    ...
    assert metrics["pixel-agents.model_call.count"]["dimensions"]["model_family"] == "claude"
    assert metrics["pixel-agents.model_call.count"]["value"] == 1
    assert metrics["pixel-agents.task_run.started"] == {
    ...
    terminal = metrics["pixel-agents.task_run.finished"]["dimensions"]
...
def test_real_binding_drives_lifecycle_aggregation_export_and_snapshot(
    ...
    from pixel_cli.observability.shared_metrics import SharedMetricsStore

    root = tmp_path / "pixel-agents-home" / "telemetry" / "shared_metrics"
    store = SharedMetricsStore(root / "metrics.sqlite3", root / "outbox")
    tomorrow = datetime.now(timezone.utc) + timedelta(days=1)
    monkeypatch.setattr(
        "pixel_cli.observability.shared_metrics._utc_now",
        lambda: tomorrow,
    )
    ...
    assert len(by_metric["pixel-agents.task_run.started"]) == 1
    assert by_metric["pixel-agents.task_run.started"][0]["value"] == 3
    assert {
        counter["dimensions"]["outcome"]
        for counter in by_metric["pixel-agents.model_call.count"]
    } == {"success", "failed", "cancelled"}
    terminal_by_outcome = {
        counter["dimensions"]["outcome"]: counter
        for counter in by_metric["pixel-agents.task_run.finished"]
    }
    ...
    for package in package_payloads:
        assert package["schema_version"] == "pixel-agents.shared_metrics.v1"
...
def test_core_runtime_is_fail_open_without_a_published_binding(monkeypatch, caplog):
    ...
    assert not relay_runtime.emit_mark("pixel-agents.probe", session_id="s1")
    assert "Pixel Agents Relay runtime initialization failed" in caplog.text
    relay_runtime._reset_for_tests()
...
def test_managed_config_cannot_override_shared_metrics_consent(
    ...
    from pixel_cli import config, managed_scope
    ...
    monkeypatch.setenv("PIXEL_AGENTS_MANAGED_DIR", str(managed))
    config._LOAD_CONFIG_CACHE.clear()
    config._RAW_CONFIG_CACHE.clear()
    managed_scope.invalidate_managed_cache()

    token = set_pixel_agents_home_override(profile)
    try:
        assert (
...
def test_disabling_shared_metrics_stops_collection_and_shutdown_export(
    ...
    runtime.subscriber(SimpleNamespace(
        ...
        category="function",
        category_profile=None,
        name="pixel-agents.task_run",
        scope_category="start",
        metadata={
            "pixel-agents.metrics.schema_version": "pixel-agents.metrics.event.v1",
...
def test_failed_flush_keeps_daily_export_open_for_later_task(
    ...
    metrics = {metric["name"]: metric for metric in package["metrics"]}
    assert metrics["pixel-agents.task_run.started"]["value"] == 2
    assert metrics["pixel-agents.task_run.finished"]["value"] == 2
    assert flush_attempts == 2
    assert "Pixel Agents shared-metrics task flush failed" in caplog.text
