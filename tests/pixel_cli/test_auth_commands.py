...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (pixel_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from pixel_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "pixel-agents" / "auth.json").read_text())
...
def test_auth_add_pixel_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / "pixel-agents"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("pixel@example.com")
    monkeypatch.setattr(
        "pixel_cli.auth._pixel_device_code_login",
        lambda **kwargs: {
    ...
    entries = payload["credential_pool"]["pixel"]
    device_code_entries = [
    ...
    # `pixel-agents auth add pixel` must also populate providers.pixel so the
    # 401-recovery path (resolve_pixel_runtime_credentials) can refresh an
    # invoke JWT when the token expires. If this mirror is missing, recovery
    # raises "Pixel Agents is not logged into Pixel Portal" and the agent dies.
...
def test_auth_add_pixel_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`pixel-agents auth add pixel --type oauth --label <name>` must preserve the
    custom label end-to-end — it was silently dropped in the first cut of the
    persist_pixel_credentials helper because `--label` wasn't threaded through.
    """
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / "pixel-agents"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("pixel@example.com")
    ...
    pool_entry = payload["credential_pool"]["pixel"][0]
    assert pool_entry["source"] == "device_code"
    assert pool_entry["label"] == "my-pixel"

    # … and survives in providers.pixel so a subsequent load_pool() re-seeds
    # it without reverting to the auto-derived fingerprint.
    assert payload["providers"]["pixel"]["label"] == "my-pixel"


def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``pixel-agents auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``pixel-agents auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("pixel_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from pixel_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from pixel_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """pixel-agents auth add xai-oauth must set active_provider and write a pool entry.
...
def test_auth_add_xai_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``pixel-agents auth add xai-oauth`` runs must produce independent pool entries.
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`pixel-agents logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (pixel_home / "config.yaml").read_text()
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from pixel_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Pixel Agents reads from has a
...
def test_seed_from_singletons_respects_pixel_pkce_suppression(tmp_path, monkeypatch):
    """anthropic pixel_pkce must not re-seed from ~/.pixel-agents/.anthropic_oauth.json when suppressed."""
    pixel_home = tmp_path / "pixel-agents"
    pixel_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))

    import yaml
    (pixel_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (pixel_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["pixel_pkce"]},
    }))

    # Stub the readers so only pixel_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_pixel_oauth_credentials", lambda: {
    ...
    # pixel_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "pixel_pkce" not in active
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.pixel-agents/.anthropic_oauth.json",
        "auth.json providers.pixel",
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from pixel_cli.auth import is_source_suppressed
    from pixel_cli.auth_commands import auth_remove_command

    with patch(
        "pixel_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "pixel_cli.copilot_auth.get_copilot_api_token",
