"""Tests for the ``pixel-agents send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``pixel_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from pixel_cli import send_cmd
...
def test_load_pixel_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``pixel-agents send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``pixel-agents config set``) lives
    ...
    pixel_home.mkdir()
    (pixel_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_pixel_agents_home() to re-resolve under the patched env.
    from importlib import reload

    import pixel_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_pixel_env()
