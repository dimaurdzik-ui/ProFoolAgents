...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_pixel_agents_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        """
        import pixel_cli.oneshot as oneshot
        from pixel_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """pixel-agents -z bypasses PixelAgentsCLI, but recall still needs SessionDB."""
    from pixel_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "pixel_state", mod("pixel_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "pixel_cli.config",
        mod("pixel_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "pixel_cli.models",
        mod("pixel_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "pixel_cli.runtime_provider",
        mod(
            "pixel_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "pixel_cli.tools_config",
        mod("pixel_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["PIXEL_AGENTS_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="pixel/pixel-agents-test", provider="pixel", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["PIXEL_AGENTS_MODEL"] == "pixel/pixel-agents-test"
    assert env["PIXEL_AGENTS_INFERENCE_MODEL"] == "pixel/pixel-agents-test"
    assert env["PIXEL_AGENTS_TUI_PROVIDER"] == "pixel"
    assert env["PIXEL_AGENTS_INFERENCE_PROVIDER"] == "pixel"
    assert env["PIXEL_AGENTS_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["PIXEL_AGENTS_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("pixel-agents-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_pixel_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("PIXEL_AGENTS_TUI_DIR", raising=False)
