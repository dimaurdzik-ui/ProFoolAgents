"""Behavior tests for pixel_cli.inventory.
...
def _pixel_row(model: str = "openai/gpt-5.5") -> dict:
    return {
        "slug": "pixel",
        "name": "Pixel",
        "models": [model],
...
def test_list_authenticated_providers_force_fresh_is_keyword_only():
    """``force_fresh_pixel_tier`` must be keyword-only on the public listing API.
    ...
    param = sig.parameters["force_fresh_pixel_tier"]
...
def test_explicit_only_filters_ambient_credentials_but_keeps_current_and_custom_rows():
    rows = [
        ...
        {"slug": "copilot", "name": "Copilot", "models": ["gpt-5.4"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "pixel-agents"},
        {"slug": "pixel", "name": "Pixel", "models": ["anthropic/claude-sonnet-5"],
         "total_models": 1, "is_current": False, "is_user_defined": False,
         "source": "pixel-agents"},
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("pixel_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from pixel_cli import model_switch

    with patch("pixel_cli.models.clear_provider_models_cache") as clear:
