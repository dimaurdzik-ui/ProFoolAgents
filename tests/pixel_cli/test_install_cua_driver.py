...
* Be supported-platform-only — no-op silently elsewhere so ``pixel-agents update``
...
class TestInstallCuaDriverUpgrade:
    def test_upgrade_on_unsupported_platform_is_silent_noop(self):
        from pixel_cli import tools_config
...
class TestRequireConfirmedUpdate:
    """`pixel-agents update` passes require_confirmed_update=True: the full
    ...
    Explicit `pixel-agents computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `pixel-agents computer-use install --upgrade` (default flag): the old
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `pixel-agents update`.
    """
