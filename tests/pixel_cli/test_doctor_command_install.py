"""Tests for the Command Installation check in pixel-agents doctor."""
...
import pixel_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal PIXEL_AGENTS_HOME + PROJECT_ROOT for doctor tests."""
    ...
    pixel_bin = venv_bin_dir / "pixel-agents"
    pixel_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    pixel_bin.chmod(0o755)
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, pixel_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "pixel-agents"
        wrong_target = tmp_path / "wrong_pixel"
        ...
        assert cmd_link.resolve() == pixel_bin.resolve()
