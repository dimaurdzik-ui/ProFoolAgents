"""Cross-process update mutual exclusion (``pixel_cli.update_lock``).

Three surfaces can start an update of one install tree: a terminal ``pixel-agents
...
dashboard-spawned ``pixel-agents update`` was mid-``npm install``, which then failed
...
@pytest.fixture
def marker(tmp_path):
    return tmp_path / ".pixel-agents-update-in-progress"


def test_marker_path_follows_process_pixel_home(tmp_path, monkeypatch):
    """The lock must land where the Rust updater and Electron gate look.

    All three resolve the *process* PIXEL_AGENTS_HOME; a profile-scoped path would
    put the lock somewhere the other two owners never read.
    """
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path))
    assert update_marker_path() == tmp_path / ".pixel-agents-update-in-progress"
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``pixel-agents update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "Pixel Agents is still running" and retrying
