"""Tests for pixel_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_pixel_home):
    """Drop the example-dashboard fixture into the per-test PIXEL_AGENTS_HOME
    ...
    isolated ``PIXEL_AGENTS_HOME``.

    The user-plugin source is preferred over a transient
    ``PIXEL_AGENTS_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, pixel-agents-achievements, model providers
    ...
    from pixel_cli import web_server

    user_plugins_dir = get_pixel_agents_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `pixel-agents plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down PIXEL_AGENTS_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Pixel Agents vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints PIXEL_AGENTS_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import pixel_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("PIXEL_AGENTS_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("PIXEL_AGENTS_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_pixel_home):
        """Create a TestClient and isolate the state DB under the test PIXEL_AGENTS_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        import pixel_state
        from pixel_constants import get_pixel_agents_home
        from pixel_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(pixel_state, "DEFAULT_DB_PATH", get_pixel_agents_home() / "state.db")
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        PIXEL_AGENTS_HOME contextvar override (#56986), so /api/messaging/platforms
        ...
        """
        import pixel_cli.web_server as web_server
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from pixel_constants import get_pixel_agents_home
        from pixel_cli.config import load_env
        ...
        config_path = get_pixel_agents_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from pixel_constants import get_pixel_agents_home
        from pixel_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide PIXEL_AGENTS_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from pixel_constants import get_pixel_agents_home

        path = get_pixel_agents_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_pixel_agents_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["pixel-agents"]["workspace"] == "myws"
        assert cfg["hosts"]["pixel-agents"]["peerName"] == "eri"
        assert cfg["hosts"]["pixel-agents"]["environment"] == "local"
        assert cfg["hosts"]["pixel-agents"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["pixel-agents"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from pixel_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_pixel_returns_docker_guidance_without_spawning(self, monkeypatch):
        import pixel_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn pixel-agents update")
        ...
        monkeypatch.setattr(web_server, "_spawn_pixel_action", fail_spawn)
        web_server._ACTION_PROCS.pop("pixel-agents-update", None)
        web_server._ACTION_RESULTS.pop("pixel-agents-update", None)

        resp = self.client.post("/api/pixel-agents/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "pixel-agents-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull pixelagents/pixel-agents:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/pixel-agents-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull pixelagents/pixel-agents:latest" in line for line in status_data["lines"])
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/pixel_cli/test_setup_hidden_env.py.
        """
        from pixel_cli.web_server import (
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Pixel Agents provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from pixel_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from pixel_cli.config import save_env_value
        from pixel_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "pixel_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/PixelAgentsSetupBot/pixel_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/PixelAgentsSetupBot/pixel_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "pixel_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_pixel_action", fail_spawn_action)
    ...
    def test_unauthenticated_api_blocked(self):
        ...
        from starlette.testclient import TestClient
        from pixel_cli.web_server import app
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from pixel_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``pixel-agents model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from pixel_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from pixel_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from pixel_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_overrides_applied(self):
        from pixel_cli.web_server import CONFIG_SCHEMA
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from pixel_cli.web_server import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from pixel_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_pixel_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from pixel_constants import get_pixel_agents_home
        import pixel_cli.profiles as profiles_mod
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        logged-out Pixel Subscription rows, xAI TTS without Grok OAuth, and
        never-installed KittenTTS/Piper. The endpoint now reports the honest
        state so keyless ≠ ready.
        """
        import pixel_cli.tools_config as tools_config
        from pixel_cli.pixel_account import PixelPortalAccountInfo

        # Logged out of Pixel Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "pixel_cli.pixel_subscription.get_pixel_portal_account_info",
            lambda *a, **k: PixelPortalAccountInfo(
        ...
        assert by_name["Pixel Subscription"]["status"] == "needs_auth"
    ...
    def test_select_managed_pixel_provider_reports_needs_pixel_auth(self, monkeypatch):
        """Selecting a managed Pixel row while logged out flags needs_pixel_auth.
        ...
        (ensure_pixel_portal_access) — so the row never activated and nothing
        told the user to sign in. The endpoint now reports the entitlement
        gap so the client can drive the existing Pixel OAuth flow.
        """
        from pixel_cli.pixel_account import PixelPortalAccountInfo
        ...
        resp = self.client.put(
            ...
            json={"provider": "Pixel Subscription (Browser Use cloud)"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["needs_pixel_auth"] is True
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        config["terminal"]["ssh_user"] = "pixel-agents"
        save_config(config)
        ...
        assert "pixel-agents@devbox.example.com" in ssh["detail"]
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from pixel_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from pixel_cli.web_server import _denormalize_config_from_web
        from pixel_cli.config import save_config
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from pixel_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.pixel-agents/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        token = set_pixel_agents_home_override(str(other))
        try:
            results = web_server._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(pixel_home, name="ocean"):
        themes_dir = pixel_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="pixel-agents-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_pixel_home):
        ...
        monkeypatch.setattr(
            pixel_state, "DEFAULT_DB_PATH", get_pixel_agents_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``pixel_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in pixel_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_pixel_home):
        ...
        # Pin the SessionDB to the isolated PIXEL_AGENTS_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_pixel_home, _install_example_plugin):
        ...
        installs it into the per-test ``PIXEL_AGENTS_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into PIXEL_AGENTS_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, pixel-agents-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (pixel-agents-achievements is loaded by default).
        resp = self.client.get("/api/plugins/pixel-agents-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        PIXEL_AGENTS_HOME override scopes a request to another profile."""
        from pixel_constants import (
            reset_pixel_agents_home_override,
            set_pixel_agents_home_override,
        )
        ...
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(launch_home))
        from pixel_cli import web_server
        token = set_pixel_agents_home_override(str(other))
        try:
...
# or ``sh -c 'printf …'``) instead of the real ``pixel-agents --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        import pixel_cli.main as main_mod

        command = f"pixel-agents-review-python{Path(sys.executable).suffix}"
        ...
        env = {
            "PIXEL_AGENTS_CWD": str(tmp_path),
            "PIXEL_AGENTS_PYTHON": command,
            "PATH": str(bin_dir),
        }

        main_mod._apply_tui_python_env(env)

        assert env["PIXEL_AGENTS_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from pixel_cli.pty_bridge import PtyUnavailableError
        ...
        import pixel_cli.web_server as ws_mod
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from pixel_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("PIXEL_AGENTS_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_pixel_home, _install_example_plugin):
        ...
        the per-test ``PIXEL_AGENTS_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_pixel_home):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under PIXEL_AGENTS_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("PIXEL_AGENTS_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        ws.mount_spa(spa_app)
