...
Profile-aware location: ``$PIXEL_AGENTS_HOME/logs/dashboard-auth.log``.
...
from pixel_cli.dashboard_auth.audit import audit_log, AuditEvent


@pytest.fixture
def profile_home(tmp_path, monkeypatch):
    """Redirect $PIXEL_AGENTS_HOME and ~ to a tmp dir for the duration of the test."""
...
def test_audit_writes_jsonlines(profile_home):
    audit_log(AuditEvent.LOGIN_START, provider="pixel", ip="1.2.3.4")
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="pixel", user_id="u1",
...
def test_audit_redacts_token_like_fields(profile_home):
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="pixel", access_token="should-not-appear",
