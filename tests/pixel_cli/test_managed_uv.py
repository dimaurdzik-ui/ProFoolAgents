"""Tests for pixel_cli.managed_uv — one path, no guessing."""
...
def _runtime_info(
    ...
    from pixel_cli.sqlite_runtime import SQLiteRuntimeInfo
...
def _RRR(status):
    ...
    """
    from pixel_cli.managed_uv import RuntimeRepairResult
...
class TestManagedUvPath:
    def test_posix(self, tmp_path):
        with patch("pixel_cli.managed_uv.get_pixel_agents_home", return_value=tmp_path), \
             patch("pixel_cli.managed_uv.platform.system", return_value="Linux"):
            from pixel_cli.managed_uv import managed_uv_path
...
class TestEnsureUv:
    ...
    def test_installs_if_missing(self, tmp_path):
        with patch("pixel_cli.managed_uv.get_pixel_agents_home", return_value=tmp_path), \
             patch("pixel_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("pixel_cli.managed_uv._install_uv") as mock_install:
    ...
    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        from pixel_cli.managed_uv import (
            RuntimeRepairResult,
            ensure_uv,
        )
        ...
        observed = []
        with patch(
            "pixel_cli.managed_uv.get_pixel_agents_home",
            return_value=tmp_path,
        ), patch(
            "pixel_cli.managed_uv._install_uv",
            side_effect=fake_install,
        ), patch(
            "pixel_cli.managed_uv.repair_vulnerable_runtime",
            return_value=repair,
        ):
...
class TestEnsureUvUpdateBoundary:
    ...
    ``pixel-agents update`` runs the call site from the old, already-imported
    ``pixel_cli.main`` against the freshly pulled ``managed_uv``. A release
    ...
    def test_failure_unpacks_without_raising(self, tmp_path):
        with patch("pixel_cli.managed_uv.get_pixel_agents_home", return_value=tmp_path), \
             patch("pixel_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("pixel_cli.managed_uv.platform.system", return_value="Linux"), \
             patch("pixel_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from pixel_cli.managed_uv import ensure_uv
...
class TestEnsureUvWindowsSafe:
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        import subprocess
        from pixel_cli.managed_uv import _UvResult
        with pytest.raises(TypeError):
            subprocess.list2cmdline([_UvResult("C:\\pixel-agents\\uv.exe"), "pip"])
    ...
    def test_windows_returns_plain_str_safe_for_subprocess(self, tmp_path):
        ...
        with patch("pixel_cli.managed_uv.get_pixel_agents_home", return_value=tmp_path), \
             patch("pixel_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("pixel_cli.managed_uv.platform.system", return_value="Windows"):
            from pixel_cli.managed_uv import _UvResult, ensure_uv
            uv_bin = ensure_uv()
...
class TestUpdateManagedUv:
    ...
    def test_fresh_stamp_skips_network_self_update_but_not_repair(self, tmp_path, monkeypatch):
        ...
        from pixel_cli.managed_uv import RuntimeRepairResult, update_managed_uv

        uv = tmp_path / "bin" / "uv"
        _make_executable(uv)
        # Fresh stamp under the isolated PIXEL_AGENTS_HOME.
        import pixel_constants
        stamp = pixel_constants.get_pixel_agents_home() / "cache" / ".uv_self_update_stamp"
        stamp.parent.mkdir(parents=True, exist_ok=True)
        stamp.touch()

        with patch("pixel_cli.managed_uv.get_pixel_agents_home", return_value=tmp_path), \
             patch("pixel_cli.managed_uv.subprocess.run") as mock_run, \
             patch(
                 "pixel_cli.managed_uv.repair_vulnerable_runtime",
    ...
    def test_stale_stamp_runs_self_update_and_refreshes_stamp(self, tmp_path):
        ...
        from pixel_cli.managed_uv import UV_SELF_UPDATE_INTERVAL_SECONDS, update_managed_uv
...
class TestManagedPythonStore:
    def test_store_is_checkout_scoped_across_profiles(self, tmp_path, monkeypatch):
        from pixel_cli.managed_uv import managed_python_install_dir

        checkout = tmp_path / "checkout"
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / "profiles" / "alpha"))
        alpha = managed_python_install_dir(checkout)
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / "profiles" / "beta"))
        beta = managed_python_install_dir(checkout)

        expected = checkout / ".pixel-agents-runtime" / "python"
        assert alpha == expected
        assert beta == expected

    def test_environment_is_private_and_sanitized(self, tmp_path):
        from pixel_cli.managed_uv import managed_python_env
        ...
        assert env["UV_PYTHON_INSTALL_DIR"] == str(
            checkout / ".pixel-agents-runtime" / "python"
        )
        for key in (
            "CONDA_DEFAULT_ENV",
            "CONDA_PREFIX",
...
class TestRuntimeRepair:
    def test_safe_runtime_is_a_noop(self, tmp_path):
        from pixel_cli.managed_uv import repair_vulnerable_runtime

        root, live, sentinel = _make_runtime_install(tmp_path)
        current = _runtime_info(live / "bin" / "python", (3, 53, 1))
        with patch("pixel_cli.managed_uv.platform.system", return_value="Linux"), \
             patch(
                 "pixel_cli.managed_uv.probe_sqlite_runtime",
                 return_value=current,
             ), \
             patch(
                 "pixel_cli.managed_uv._install_safe_python_generation"
             ) as mock_install:
        ...
        assert not (root / ".pixel-agents-runtime").exists()
    ...
    def test_failed_candidate_preserves_live_venv(self, tmp_path):
        ...
        generation = root / ".pixel-agents-runtime" / "python" / "generation-test"
        ...
        with patch("pixel_cli.managed_uv.platform.system", return_value="Linux"), \
             ...
             patch(
                 "pixel_cli.managed_uv._stage_candidate_venv",
                 return_value=None,
             ):
            result = repair_vulnerable_runtime("uv", project_root=root)
        ...
        reacquired = _acquire_repair_lock(root / ".pixel-agents-runtime")
    ...
    def test_successful_repair_removes_parked_backup(self, tmp_path):
        ...
        candidate_python.write_text("candidate interpreter", encoding="utf-8")
        fixed = _runtime_info(candidate_python, (3, 53, 1))
        candidate_venv = root / ".pixel-agents-runtime" / "venv-candidate"
        ...
        with patch("pixel_cli.managed_uv.platform.system", return_value="Linux"), \
             ...
             patch(
                 "pixel_cli.managed_uv._smoke_candidate_venv",
...
class TestRuntimeCutover:
    def test_os_lock_blocks_concurrent_repair_and_releases(self, tmp_path):
        from pixel_cli.managed_uv import _acquire_repair_lock, _release_repair_lock

        runtime_root = tmp_path / ".pixel-agents-runtime"
    ...
    def test_post_swap_smoke_failure_rolls_back_live_venv(self, tmp_path):
        from pixel_cli.managed_uv import _cut_over_candidate

        root, live, sentinel = _make_runtime_install(tmp_path)
        runtime_root = root / ".pixel-agents-runtime"
...
class TestInstallUvInternals:
    def test_posix_sets_uv_unmanaged_install(self, tmp_path):
        ...
        with patch("pixel_cli.managed_uv._install_uv_posix") as mock_posix:
            from pixel_cli.managed_uv import _install_uv
            _install_uv(target)
...
class TestRuntimeRequestMinorLine:
    ...
    def test_requests_minor_line(self):
        from pixel_cli.managed_uv import _runtime_request
    ...
    def _run_generation(tmp_path, monkeypatch, current_version, candidate_version):
        ...
        import pixel_cli.managed_uv as managed_uv
        from pixel_cli.sqlite_runtime import SQLiteRuntimeInfo
...
class TestRefreshManagedUvCatalog:
    ...
    def test_version_change_reports_true(self, tmp_path):
        ...
        with patch("pixel_cli.managed_uv.get_pixel_agents_home", return_value=tmp_path), \
             patch("pixel_cli.managed_uv.platform.system", return_value="Linux"), \
             patch("pixel_cli.managed_uv._install_uv"), \
             patch(
                 "pixel_cli.managed_uv._uv_version_string",
...
class TestRepairRetriesAfterUvRefresh:
    def _run_repair(self, tmp_path, *, refresh_result, second_attempt):
        ...
        with patch("pixel_cli.managed_uv.platform.system", return_value="Linux"), \
             ...
             patch(
                 "pixel_cli.managed_uv._refresh_managed_uv_catalog",
    ...
    def test_retry_success_proceeds_to_staging(self, tmp_path):
        def second_attempt(root):
            generation = root / ".pixel-agents-runtime" / "python" / "generation-retry"
            candidate_python = generation / "bin" / "python"
            candidate_python.parent.mkdir(parents=True)
...
class TestDefaultLiveVenv:
    ...
    'not-applicable' on every pixel-agents update and stayed on journal_mode=DELETE
    ...
    def test_dot_venv_only_is_targeted(self, tmp_path):
        from pixel_cli.managed_uv import _default_live_venv
