"""Tests for `pixel-agents curator run` CLI behavior."""
...
def test_run_defaults_to_synchropixel(monkeypatch, capsys):
    import agent.curator as curator_state
    import pixel_cli.curator as curator_cli
    ...
    assert calls[0]["synchropixel"] is True
    assert calls[0]["dry_run"] is False
    assert "background" not in capsys.readouterr().out


def test_dry_run_default_reports_synchropixel_wording(monkeypatch, capsys):
    import agent.curator as curator_state
    import pixel_cli.curator as curator_cli
    ...
    assert "Read the report with `pixel-agents curator status`" in out
