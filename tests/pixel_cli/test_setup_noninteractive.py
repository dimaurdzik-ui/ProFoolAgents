...
import pytest
from pixel_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `pixel-agents` should not prompt for input when no provider exists and stdin is headless."""
        from pixel_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("pixel_cli.main._has_any_provider_configured", return_value=False),
            patch("pixel_cli.main.cmd_setup") as mock_setup,
        ...
        out = capsys.readouterr().out
        assert "pixel-agents config set model.provider custom" in out
