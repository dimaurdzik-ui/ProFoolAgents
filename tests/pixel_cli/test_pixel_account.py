"""Tests for normalized Pixel Portal account entitlement helpers."""
...
from pixel_cli.pixel_account import (
    ...
    reset_pixel_portal_account_info_cache,
)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    monkeypatch.setattr("pixel_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("pixel_cli.auth.resolve_pixel_access_token", lambda: "fresh-token")
    monkeypatch.setattr("pixel_cli.pixel_account._fetch_pixel_account_info", lambda *a, **kw: payload)

    info = get_pixel_portal_account_info(force_fresh=True)

    assert isinstance(info, PixelPortalAccountInfo)
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("pixel_cli.auth.get_provider_auth_state", lambda provider: {})
