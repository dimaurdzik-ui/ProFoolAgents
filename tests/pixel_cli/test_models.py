"""Tests for the pixel_cli models module."""
...
from pixel_cli.models import (
    ...
    is_pixel_free_tier, partition_pixel_models_by_tier,
    check_pixel_free_tier, _FREE_TIER_CACHE_TTL,
    union_with_portal_free_recommendations,
    union_with_portal_paid_recommendations,
)
import pixel_cli.models as _models_mod
...
class TestModelIds:
    def test_returns_non_empty_list(self):
        with patch("pixel_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
            ids = model_ids()
...
class TestFetchOpenRouterModels:
    ...
    def test_falls_back_to_static_snapshot_on_fetch_failure(self, monkeypatch):
        ...
        with patch("pixel_cli.model_catalog.get_curated_openrouter_models", return_value=None), \
             patch("pixel_cli.models._urlopen_model_catalog_request", side_effect=OSError("boom")):
    ...
    def test_filters_out_models_without_tool_support(self, monkeypatch):
        ...
        pixel-agents is tool-calling-first — surfacing a non-tool model leads to
        ...
        monkeypatch.setattr(_models_mod, "_openrouter_catalog_cache", None)
        with (
            patch("pixel_cli.model_catalog.get_curated_openrouter_models", return_value=[]),
            patch("pixel_cli.models._urlopen_model_catalog_request", return_value=_Resp()),
        ):
            models = fetch_openrouter_models(force_refresh=True)
...
class TestOpenRouterToolSupportHelper:
    ...
    def test_tools_in_supported_parameters(self):
        from pixel_cli.models import _openrouter_model_supports_tools
...
class TestFindOpenrouterSlug:
    def test_exact_match(self):
        from pixel_cli.models import _find_openrouter_slug
        with patch("pixel_cli.models.fetch_openrouter_models", return_value=LIVE_OPENROUTER_MODELS):
...
class TestIsPixelFreeTier:
    """Tests for is_pixel_free_tier — account tier detection."""

    def test_paid_service_access_allowed_true_is_not_free(self):
        assert is_pixel_free_tier({"paid_service_access": {"allowed": True}}) is False


    def test_empty_subscription_not_free(self):
        """Empty subscription dict defaults to not-free (don't block users)."""
        assert is_pixel_free_tier({"subscription": {}}) is False


    def test_empty_response_not_free(self):
        """Completely empty response defaults to not-free."""
        assert is_pixel_free_tier({}) is False


class TestPartitionPixelModelsByTier:
    """Tests for partition_pixel_models_by_tier — free vs paid tier model split."""
    ...
    def test_paid_tier_all_selectable(self):
        ...
        sel, unav = partition_pixel_models_by_tier(models, pricing, free_tier=False)
        assert sel == models
        assert unav == []
    ...
    def test_all_paid_models(self):
        ...
        sel, unav = partition_pixel_models_by_tier(models, pricing, free_tier=True)
        assert sel == []
        assert unav == models
...
class TestUnionWithPortalFreeRecommendations:
    ...
    def test_adds_portal_free_model_missing_from_curated(self):
        ...
        with patch(
            "pixel_cli.models.fetch_pixel_recommended_models",
...
class TestCheckPixelFreeTierCache:
    """Tests for the TTL cache on check_pixel_free_tier()."""
    ...
    @patch("pixel_cli.pixel_account.get_pixel_portal_account_info")
    def test_result_is_cached(self, mock_account):
        """Second call within TTL returns cached result without account lookup."""
        mock_account.return_value = PixelPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
            paid_service_access=False,
        )
        result1 = check_pixel_free_tier()
        result2 = check_pixel_free_tier()
    ...
    def test_force_fresh_bypasses_cache(self, mock_account):
        ...
        assert check_pixel_free_tier() is False
        assert check_pixel_free_tier(force_fresh=True) is False
...
class TestPixelRecommendedModels:
    """Tests for fetch_pixel_recommended_models + get_pixel_recommended_aux_model."""
    ...
    def setup_method(self):
        _models_mod._pixel_recommended_cache.clear()
    ...
    def test_fetch_caches_per_portal_url(self):
        from pixel_cli.models import fetch_pixel_recommended_models
        mock_cm = self._mock_urlopen(self._SAMPLE_PAYLOAD)
        with patch("pixel_cli.models._urlopen_model_catalog_request", return_value=mock_cm) as mock_urlopen:
            a = fetch_pixel_recommended_models("https://portal.example.com")
            b = fetch_pixel_recommended_models("https://portal.example.com")
    ...
    def test_paid_tier_prefers_paid_recommendation(self):
        ...
        with patch("pixel_cli.models.fetch_pixel_recommended_models", return_value=payload):
            text = get_pixel_recommended_aux_model(vision=False, free_tier=False)
            vision = get_pixel_recommended_aux_model(vision=True, free_tier=False)
    ...
    def test_tier_detection_error_defaults_to_paid(self):
        ...
        with (
            patch("pixel_cli.models.fetch_pixel_recommended_models", return_value=payload),
            patch("pixel_cli.models.check_pixel_free_tier", side_effect=RuntimeError("boom")),
        ):
            assert get_pixel_recommended_aux_model(vision=False) == "paid-model"
...
class TestCodexSoftAcceptPlausibilityGate:
    ...
    def test_unrelated_name_rejected_on_openai_codex(self):
        from pixel_cli.models import validate_requested_model
