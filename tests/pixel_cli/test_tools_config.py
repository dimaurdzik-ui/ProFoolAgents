"""Tests for pixel_cli.tools_config platform tool persistence."""
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'pixel-agents' instead of 'pixel-agents-cli') must warn at resolve time so an
    already-corrupted config is caught at runtime, not just during migration."""
    import pixel_cli.tools_config as _tc
    ...
    config = {"platform_toolsets": {"cli": ["pixel-agents"]}}

    with caplog.at_level(logging.WARNING, logger="pixel_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "pixel-agents" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["pixel-agents-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["pixel-agents-cli", "bogus"]}}
...
# via the platform's native composite. The composite ``pixel-agents-discord``
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["pixel-agents-telegram", "discord"]}}
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/PixelResearch/pixel-agents/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("pixel_cli.tools_config.save_config"):
...
def test_first_install_pixel_auto_configures_video_gen(monkeypatch):
    """When a Pixel subscriber checks video_gen in the toolset checklist,
    apply_pixel_managed_defaults must write video_gen.provider and
    ...
    monkeypatch.setattr("pixel_cli.pixel_subscription.managed_pixel_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "pixel"},
    ...
    monkeypatch.setattr(
        "pixel_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"video_gen"},
    )
    monkeypatch.setattr("pixel_cli.tools_config.save_config", lambda config: None)
    monkeypatch.setattr(
        "pixel_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "pixel_cli.pixel_subscription.get_pixel_portal_account_info",
        lambda *args, **kwargs: PixelPortalAccountInfo(
            logged_in=True,
            source="jwt",
            fresh=False,
    ...
    configured = []
    monkeypatch.setattr(
        "pixel_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from pixel_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """pixel-agents-gateway includes list should cover all messaging platforms."""
        from pixel_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["pixel-agents-gateway"]["includes"])
        ...
        for platform, meta in PLATFORMS.items():
            ...
            assert ts_name in gateway_includes, (
                ...
                f"pixel-agents-gateway includes"
            )
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from pixel_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from pixel_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/PixelResearch/pixel-agents/issues/6901
    """
    config = {
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from pixel_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        """Both Pixel Subscription and FAL.ai providers must carry the
        imagegen_backend tag so _configure_provider fires the picker."""
        from pixel_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from pixel_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("pixel_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("pixel_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("pixel_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `pixel-agents tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from pixel_cli.tools_config import _get_effective_configurable_toolsets
...
# ---------------------------------------------------------------------------
# Inline Pixel Portal login gate on managed-provider selection
# ---------------------------------------------------------------------------
...
#    reported as added/removed by `pixel-agents tools` ──────────────────────────




def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `pixel-agents tools` printed ``- kanban``
...
def test_vision_picker_custom_endpoint(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path))
    import pixel_cli.tools_config as tc
...
# Ready for every zero-env-var provider row, including logged-out Pixel
...
def _fake_features(*, logged_in: bool, paid: bool = True):
    account = (
        ...
        if logged_in
        else PixelPortalAccountInfo(
            logged_in=False, source="none", fresh=False, paid_service_access=None
        )
    )
    return SimpleNamespace(pixel_auth_present=logged_in, account_info=account)
...
# `pixel-agents tools post-setup <key>` child. On Windows each console child (npm,
