"""Tests for pixel_cli.agent_import — ``pixel-agents import-agent``.

Covers: source detection, Claude Code and Codex parsing, mapping into the
real Pixel Agents stores (memories/MEMORY.md, config.yaml command_allowlist /
...
Uses the profile_env fixture pattern from tests/pixel_cli/test_profiles.py:
Path.home() and PIXEL_AGENTS_HOME are redirected to tmp_path so nothing touches
the real ~/.pixel-agents.
"""
...
from pixel_cli.agent_import import (
    ENTRY_DELIMITER,
    AgentImporter,
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and PIXEL_AGENTS_HOME (profile_env pattern)
# ---------------------------------------------------------------------------

@pytest.fixture()
def profile_env(tmp_path, monkeypatch):
    """Isolated environment: Path.home() -> tmp_path, PIXEL_AGENTS_HOME -> tmp/.pixel-agents."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".pixel-agents"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(default_home))
    return tmp_path


@pytest.fixture()
def pixel_home(profile_env):
    return profile_env / ".pixel-agents"
...
def run_import(agent, source, pixel_home, execute, overwrite=False):
    return AgentImporter(
        agent=agent,
        source_root=source,
        target_root=pixel_home,
        execute=execute,
        overwrite=overwrite,
    ).run()
...
class TestDetection:
    ...
    def test_unsupported_agent_raises(self, pixel_home, tmp_path):
        with pytest.raises(ValueError):
            AgentImporter("cursor", tmp_path, pixel_home)
...
class TestDryRun:
    def test_claude_dry_run_writes_nothing(self, claude_tree, pixel_home):
        before = snapshot_tree(pixel_home)
        report = run_import("claude-code", claude_tree, pixel_home, execute=False)
        assert snapshot_tree(pixel_home) == before
        assert report["dry_run"] is True
        assert report["summary"]["imported"] > 0


    def test_dry_run_and_real_run_plan_same_items(self, claude_tree, pixel_home):
        preview = run_import("claude-code", claude_tree, pixel_home, execute=False)
        applied = run_import("claude-code", claude_tree, pixel_home, execute=True)
...
class TestClaudeCodeImport:
    @pytest.fixture()
    def report(self, claude_tree, pixel_home):
        return run_import("claude-code", claude_tree, pixel_home, execute=True)


    def test_allowlist_lands_in_config_yaml(self, report, pixel_home):
        config = yaml.safe_load((pixel_home / "config.yaml").read_text())
...
class TestCodexImport:
    @pytest.fixture()
    def report(self, codex_tree, pixel_home):
        return run_import("codex", codex_tree, pixel_home, execute=True)


    def test_mcp_servers_from_config_toml(self, report, pixel_home):
        config = yaml.safe_load((pixel_home / "config.yaml").read_text())
    ...
    def test_skill_copied(self, report, pixel_home):
        assert (pixel_home / "skills" / "codex-imports" / "db-migrate" / "SKILL.md").exists()
...
class TestSecretsNeverImported:
    def test_no_secret_values_anywhere_claude(self, claude_tree, pixel_home):
        run_import("claude-code", claude_tree, pixel_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in pixel_home.rglob("*") if p.is_file()
        )
        assert "ghp_SECRET123" not in blob
        assert "Bearer abc123" not in blob
        assert "sk-ant-SUPERSECRET" not in blob

    def test_no_secret_values_anywhere_codex(self, codex_tree, pixel_home):
        run_import("codex", codex_tree, pixel_home, execute=True)
        blob = "".join(
            p.read_text(encoding="utf-8", errors="replace")
            for p in pixel_home.rglob("*") if p.is_file()
        )
    ...
    def test_stripped_secrets_reported(self, claude_tree, pixel_home):
        report = run_import("claude-code", claude_tree, pixel_home, execute=True)
    ...
    def test_non_secret_header_kept(self, claude_tree, pixel_home):
        run_import("claude-code", claude_tree, pixel_home, execute=True)
        config = yaml.safe_load((pixel_home / "config.yaml").read_text())
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, pixel_home):
        root = profile_env / ".claude"
        root.mkdir()
        (root / "settings.json").write_text("{not json!!", encoding="utf-8")
        (root / "CLAUDE.md").write_text("- still importable\n", encoding="utf-8")
        report = run_import("claude-code", root, pixel_home, execute=True)
        ...
        assert "still importable" in (
            pixel_home / "memories" / "MEMORY.md").read_text()


    def test_empty_tree_all_skipped(self, profile_env, pixel_home):
        root = profile_env / ".codex"
        root.mkdir()
        report = run_import("codex", root, pixel_home, execute=True)
        assert report["summary"]["imported"] == 0
...
class TestMergeSemantics:
    ...
    def test_existing_skill_conflicts_without_overwrite(
            self, claude_tree, pixel_home):
        dest = pixel_home / "skills" / "claude-code-imports" / "deploy-helper"
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, pixel_home):
        run_import("claude-code", claude_tree, pixel_home, execute=True)
        first = (pixel_home / "memories" / "MEMORY.md").read_text()
        report = run_import("claude-code", claude_tree, pixel_home, execute=True)
        assert (pixel_home / "memories" / "MEMORY.md").read_text() == first
...
class TestExistingMemoryStorePreserved:
    ...
    @pytest.fixture()
    def seeded_home(self, pixel_home):
        memory = pixel_home / "memories" / "MEMORY.md"
...
class TestCliWiring:
    ...
    def test_command_dry_run_via_cli_writes_nothing(
            self, claude_tree, pixel_home, capsys):
        ...
        assert not (pixel_home / "memories" / "MEMORY.md").exists()
        assert not (pixel_home / "skills" / "claude-code-imports").exists()
        config_text = (pixel_home / "config.yaml").read_text(encoding="utf-8") \
            if (pixel_home / "config.yaml").exists() else ""
