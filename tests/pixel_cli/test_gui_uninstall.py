"""Tests for pixel_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import pixel_cli.gui_uninstall as gu


def _make_agent(pixel_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = pixel_home / "pixel-agents"
    (agent_root / "pixel_cli").mkdir(parents=True)
    (agent_root / "pixel_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(pixel_home: Path) -> None:
    """Create the source-built GUI artifacts a `pixel-agents desktop` run produces."""
    desktop = pixel_home / "pixel-agents" / "apps" / "desktop"
    ...
    (pixel_home / "pixel-agents" / "node_modules").mkdir(parents=True)
    (pixel_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(pixel_home: Path) -> None:
    (pixel_home / "config.yaml").write_text("x: 1\n")
    (pixel_home / ".env").write_text("KEY=secret\n")
    (pixel_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(pixel_home)
    _make_gui_build(pixel_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(pixel_home)
    ...
    assert summary["pixel_home"] == str(pixel_home)
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import pixel_cli.uninstall as uninstall
