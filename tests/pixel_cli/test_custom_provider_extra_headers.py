    ...
    normalized = _normalize_custom_provider_entry(
        {
            ...
            "extra_headers": {"X-Custom-Auth": "tok", "X-Client-Name": "pixel-agents"},
        }
    )
    assert normalized is not None
    assert normalized["extra_headers"] == {
        "X-Custom-Auth": "tok",
        "X-Client-Name": "pixel-agents",
    }
...
def test_fetch_api_models_sends_extra_headers_to_models_probe(monkeypatch):
    ...
    assert captured["headers"]["sleeve-harness"] == "pixel-agents"
