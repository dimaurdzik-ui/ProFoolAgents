"""Tests for Codex auth — tokens stored in Pixel Agents auth store (~/.pixel-agents/auth.json)."""
...
def _setup_pixel_auth(pixel_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Pixel Agents auth store."""
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    ...
    _setup_pixel_auth(pixel_home, access_token="")
...
def test_resolve_codex_runtime_credentials_falls_back_to_pool_when_singleton_empty(tmp_path, monkeypatch):
    ...
    (pixel_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((pixel_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``pixel-agents auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``pixel-agents auth add openai-codex``.
    ...
    (pixel_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `pixel-agents auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``pixel-agents auth add openai-codex`` for accounts B and C
    ...
    (pixel_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``pixel-agents auth add openai-codex`` (account B and account C).
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Pixel Agents auth store, not ~/.codex/."""
    pixel_home = tmp_path / "pixel-agents"
    ...
    _save_codex_tokens({"access_token": "pixel-agents-at", "refresh_token": "pixel-agents-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Pixel Agents store
    assert not (codex_home / "auth.json").exists()

    # Pixel Agents auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "pixel-agents-at"


def test_resolve_returns_pixel_auth_store_source(tmp_path, monkeypatch):
    pixel_home = tmp_path / "pixel-agents"
    _setup_pixel_auth(pixel_home)
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "pixel-agents-auth-store"
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run pixel-agents auth".
    """
    from pixel_cli.auth import (
    ...
    assert "pixel-agents auth" not in rendered
...
def test_refresh_429_without_retry_after_header(monkeypatch):
    ...
    from pixel_cli.auth import CODEX_RATE_LIMITED_CODE
...
def test_is_rate_limited_auth_error_distinguishes_credential_errors():
    ...
    from pixel_cli.auth import CODEX_RATE_LIMITED_CODE, is_rate_limited_auth_error
...
def _patch_httpx_post(monkeypatch, responses):
    """Patch pixel_cli.auth.httpx.Client so .post() returns queued responses."""
    seq = iter(responses)
    ...
    monkeypatch.setattr("pixel_cli.auth.httpx.Client", lambda *a, **k: _FakeClient())
