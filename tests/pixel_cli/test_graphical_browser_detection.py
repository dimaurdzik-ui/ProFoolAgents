"""Tests for `_can_open_graphical_browser()` in pixel_cli.auth.

Guards the fix for the May 2026 report where `pixel-agents auth add xai-oauth`
...
from pixel_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("pixel_cli.auth.sys.platform", "linux")
