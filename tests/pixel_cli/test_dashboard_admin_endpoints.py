...
def _client():
    ...
    # Keep the state DB under the isolated PIXEL_AGENTS_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_pixel_home):
    ...
    def test_http_bearer_auth_separates_secret_from_config(
        ...
        env_text = (pixel_home / ".env").read_text()
    ...
    def test_http_oauth_mode_is_persisted_for_existing_auth_flow(self):
        ...
        from pixel_cli.mcp_config import _get_mcp_servers
...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.pixel-agents/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `pixel-agents auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `pixel-agents auth add`, which clears every suppression for the
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_pixel_home):
        ...
        (get_pixel_agents_home() / "memories").mkdir(parents=True, exist_ok=True)
    ...
    def test_reset_targets(self):
        ...
        mem = get_pixel_agents_home() / "memories"
...
class TestPairingEndpoints:
    ...
    def test_pairing_is_isolated_per_profile(self):
        ...
        (get_pixel_agents_home() / "profiles" / "work").mkdir(parents=True, exist_ok=True)
...
class TestWebhookEndpoints:
    ...
    def test_enable_platform_starts_gateway_restart(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_pixel_action", fake_spawn_action)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "pixel_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("pixel-agents-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # pixel-agents-index source advertises availability + featured search.
            idx = _Src("pixel-agents-index")
        ...
        assert {"official", "github", "pixel-agents-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "pixel_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/pixel-agents/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/pixel-agents/update`` runs ``pixel-agents update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import pixel_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/pixel-agents/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_managed_runtime_dashboard_is_not_applyable(self, monkeypatch):
        ...
        body = self.client.get("/api/pixel-agents/update/check").json()
...
class TestDebugShareEndpoint:
    """POST /api/ops/debug-share returns the paste URLs synchropixelly so the
    ...
    def _setup(self, _isolate_pixel_home):
        ...
        logs = get_pixel_agents_home() / "logs"
    ...
    def test_redact_false_is_honored(self, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `pixel-agents tools` configurator."""
...
# ---------------------------------------------------------------------------
# _spawn_pixel_action env scrubbing (#52470)
# ---------------------------------------------------------------------------

def test_spawn_pixel_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    """The dashboard runs inside the gateway, so os.environ has
    _PIXEL_AGENTS_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    it, or the in-process restart-loop guard rejects the restart and it silently
    fails (#52470).
    """
    import pixel_cli.web_server as ws

    monkeypatch.setenv("_PIXEL_AGENTS_GATEWAY", "1")
    ...
    ws._spawn_pixel_action(["gateway", "restart"], "gateway-restart")

    assert "_PIXEL_AGENTS_GATEWAY" not in captured["env"]
    assert captured["env"]["PIXEL_AGENTS_NONINTERACTIVE"] == "1"
