"""Tests for pixel_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate PIXEL_AGENTS_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from pixel_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "pixel": {
                "metadata": {"display_name": "Pixel Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from pixel_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    PRIMARY = "https://api.pixelagents.com/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/PixelResearch/pixel-agents"
...
class TestCuratedAccessors:
    ...
    def test_pixel_returns_none_when_catalog_empty(self, isolated_home):
        from pixel_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_pixel_models() is None
...
class TestDefaultModelFromCache:
    ...
    def _manifest_with_default(self) -> dict:
        ...
        m["providers"]["pixel"]["models"][1]["default"] = True  # kimi-k2.6
        return m
    ...
    def test_reads_label_from_disk_cache(self, isolated_home):
        ...
        with patch.object(model_catalog, "_fetch_manifest") as fetch:
            ...
            assert (
                model_catalog.get_default_model_from_cache("pixel")
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import pixel_cli.model_catalog as model_catalog
        from pixel_cli.models import PREFERRED_SILENT_DEFAULT_MODEL

        repo_root = Path(model_catalog.__file__).resolve().parent.parent
        manifest = json.loads(
            (repo_root / "website" / "static" / "api" / "model-catalog.json").read_text()
        )
        for provider in ("openrouter", "pixel"):
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in pixel_cli.models."""



    def test_picker_pixel_row_uses_curated_list(self, tmp_path, monkeypatch):
        """The /model picker surfaces the curated ``_PROVIDER_MODELS["pixel"]``
        list in curated order — matching the ``pixel-agents model`` CLI — not the live
        ...
        # ``PIXEL_AGENTS_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` PIXEL_AGENTS_HOME directly instead.
        import importlib
        from pixel_cli import model_catalog
        from pixel_cli.models import get_curated_pixel_model_ids
        importlib.reload(model_catalog)
        try:
            from pixel_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["PIXEL_AGENTS_HOME"])
            (active_home / "auth.json").write_text(
                json.dumps(
                    {
                        "providers": {"pixel": {"access_token": "fake"}},
                        "credential_pool": {},
                    }
                )
            )
            ...
            # (``curated["pixel"] = get_curated_pixel_model_ids()``), so the test
            # stays an invariant — it can't rot as the curated/manifest list grows.
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("pixel_cli.models.check_pixel_free_tier", return_value=False), patch(
                "pixel_cli.models.union_with_portal_free_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ), patch(
                "pixel_cli.models.union_with_portal_paid_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ):
                expected = get_curated_pixel_model_ids()
                picker = list_picker_providers(
                    current_provider="pixel", max_models=99
                )
        finally:
            model_catalog.reset_cache()

        pixel_row = next((r for r in picker if r["slug"] == "pixel"), None)
        assert pixel_row is not None, "pixel row must appear when authed"
        assert pixel_row["models"] == expected
    ...
    def test_picker_max_models_cap_semantics(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            with patch.object(
                ...
                expected = get_curated_pixel_model_ids()
                full = list_picker_providers(current_provider="pixel", max_models=None)
                one = list_picker_providers(current_provider="pixel", max_models=1)
                ...
                zero = list_authenticated_providers(
                    current_provider="pixel", max_models=0
                )
        finally:
            model_catalog.reset_cache()

        def _pixel(rows):
            return next((r for r in rows if r["slug"] == "pixel"), None)

        # Only meaningful when the curated list actually exceeds 1 entry.
        assert len(expected) > 1, "test needs a multi-model curated pixel list"

        full_row = _pixel(full)
        assert full_row is not None and full_row["models"] == expected

        one_row = _pixel(one)
        assert one_row is not None and one_row["models"] == expected[:1]

        zero_row = _pixel(zero)
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["pixel"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['pixel'] / OPENROUTER_MODELS. "
