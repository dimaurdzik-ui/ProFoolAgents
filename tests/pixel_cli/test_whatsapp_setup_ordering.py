...
Before the fix, ``pixel-agents whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``pixel-agents gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    pixel-agents = home / ".pixel-agents"
    pixel-agents.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel-agents))
...
def _env_value(pixel_home: Path, key: str) -> str | None:
    env_file = pixel_home / ".env"
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from pixel_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("pixel_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``pixel-agents whatsapp`` with an existing paired session and
