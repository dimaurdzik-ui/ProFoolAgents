"""Tests for `pixel-agents chat --safe-mode` isolation."""
...
_VARS = ("PIXEL_AGENTS_SAFE_MODE", "PIXEL_AGENTS_IGNORE_USER_CONFIG", "PIXEL_AGENTS_IGNORE_RULES")
...
def test_cmd_chat_safe_mode_sets_env_before_startup(monkeypatch):
    ...
    def fake_has_provider() -> bool:
        assert os.environ["PIXEL_AGENTS_SAFE_MODE"] == "1"
        assert os.environ["PIXEL_AGENTS_IGNORE_USER_CONFIG"] == "1"
        assert os.environ["PIXEL_AGENTS_IGNORE_RULES"] == "1"
        return True
...
def test_plugin_discovery_skipped(monkeypatch):
    monkeypatch.setenv("PIXEL_AGENTS_SAFE_MODE", "1")
