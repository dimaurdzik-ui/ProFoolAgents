"""Regression test: `pixel-agents dashboard --tui` must not hard-crash.

Older Pixel Agents desktop app shells (<= 0.15.x) spawn the backend as::

    pixel-agents dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Pixel Agents couldn't
...
def _run_cli(args, timeout=60):
    """Invoke the real pixel_cli.main parser in a subprocess.
    ...
    return subprocess.run(
        [sys.executable, "-m", "pixel_cli.main", *args],
        cwd=REPO_ROOT,
        env=env,
        capture_output=True,
        text=True,
