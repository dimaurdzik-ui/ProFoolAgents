...
import pixel_cli.doctor as doctor
import pixel_cli.gateway as gateway_cli
from pixel_cli import doctor as doctor_mod
from pixel_cli.doctor import _has_provider_env_config
...
class TestDoctorPlatformHints:
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull pixelagents/pixel-agents:latest" in hint
        assert "recreate all Pixel Agents containers" in hint
        assert "pixel-agents update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor._sqlite_upgrade_hint("git")

        assert "run `pixel-agents update`" in hint
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `pixel-agents doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        env_path = pixel_home / ".env"
        env_path.write_text(
            "OPENAI_API_KEY=sk-test  # em-dash here — should not crash\n",
            encoding="utf-8",
        )

        monkeypatch.setattr(doctor_mod, "PIXEL_AGENTS_HOME", pixel_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("PIXEL_AGENTS_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_pixel_home(self, tmp_path, provider=""):
        """Create a minimal PIXEL_AGENTS_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_pixel_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "PIXEL_AGENTS_HOME", home)
        ...
        try:
            from pixel_cli import auth as _auth_mod
            monkeypatch.setattr(_auth_mod, "get_pixel_auth_status_local", lambda: {})
...
def _run_doctor_with_managed_agent_browser(monkeypatch, tmp_path, runnable):
    ...
    Pixel Agents-managed node bin (~/.pixel-agents/node/bin), not on PATH or in
    ...
    try:
        from pixel_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_pixel_auth_status", lambda: {})
        monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
        monkeypatch.setattr(_auth_mod, "get_xai_oauth_auth_status", lambda: {})
    except Exception:
        pass
...
def test_run_doctor_accepts_pixel_provider_ids_that_catalog_aliases(
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp PIXEL_AGENTS_HOME.

        ``run_doctor`` reads the module-level ``PIXEL_AGENTS_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("PIXEL_AGENTS_HOME")`` alone
        leaves doctor probing the REAL ~/.pixel-agents. On a dev machine with a
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_pixel_auth_status_local", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from pixel_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("pixel_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "pixel_cli.doctor", raising=False)
        from pixel_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the Pixel / Codex / Gemini /
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from pixel_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_pixel_auth_status_local", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """Pixel / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".pixel-agents"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "Pixel Portal auth" in out
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale PIXEL_AGENTS_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"PIXEL_AGENTS_MAX_ITERATIONS={ghost}\n")
        (pixel_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "PIXEL_AGENTS_HOME", pixel_home)
        monkeypatch.setattr(doctor_mod, "get_pixel_agents_home", lambda: pixel_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("PIXEL_AGENTS_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("PIXEL_AGENTS_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale PIXEL_AGENTS_MAX_ITERATIONS" in out
        env_after = (pixel_home / ".env").read_text(encoding="utf-8")
        assert "PIXEL_AGENTS_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_pixel_tool_progress_warning_says_unsupported_since_floor(self):
        """PIXEL_AGENTS_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_mod.collect_deprecated_env_vars({"PIXEL_AGENTS_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["PIXEL_AGENTS_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_mod.collect_deprecated_env_vars({"PIXEL_AGENTS_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["PIXEL_AGENTS_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir(parents=True)
        (pixel_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (pixel_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_mod.report_deprecated_config_and_env(
            ...
            {"PIXEL_AGENTS_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: PIXEL_AGENTS_TOOL_PROGRESS_MODE" in out
