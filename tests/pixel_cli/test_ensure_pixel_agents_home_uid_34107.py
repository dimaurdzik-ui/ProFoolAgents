"""Regression tests for #34107 — Docker UID/GID handling in ensure_pixel_home.

When Pixel Agents runs in Docker with ``PIXEL_AGENTS_UID=1000`` / ``PIXEL_AGENTS_GID=911``,
the entrypoint chowns the top-level ``PIXEL_AGENTS_HOME`` once at startup. But
...
The fix is a ``_chown_to_pixel_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_pixel_uid_gid
# ---------------------------------------------------------------------------


class TestResolvePixelAgentsUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("PIXEL_AGENTS_UID", "1000")
        monkeypatch.setenv("PIXEL_AGENTS_GID", "911")
        from pixel_cli.config import _resolve_pixel_uid_gid
...
class TestChownToPixelAgentsUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_pixel_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("PIXEL_AGENTS_UID", raising=False)
        monkeypatch.delenv("PIXEL_AGENTS_GID", raising=False)
        from pixel_cli import config as cfg
