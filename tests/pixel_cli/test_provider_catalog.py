"""Tests for the unified provider catalog (pixel_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``pixel-agents model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from pixel_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
