...
from pixel_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from pixel_cli.config import load_config, save_config, save_env_value
from pixel_cli.pixel_subscription import PixelFeatureState, PixelSubscriptionFeatures
from pixel_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside pixel-agents setup must survive the wizard's final save."""
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("pixel_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("pixel_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("pixel_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    ``get_pixel_subscription_features`` so the surface stays aligned with the
    ...
    monkeypatch.setattr("pixel_cli.pixel_subscription._has_agent_browser", lambda: True)
    monkeypatch.setattr(
