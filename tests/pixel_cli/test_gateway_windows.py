"""Tests for pixel_cli.gateway_windows."""

from pathlib import Path

import pytest

import pixel_cli.gateway as gateway
import pixel_cli.gateway_windows as gateway_windows
import pixel_cli.setup as setup
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda pixel_home: "")
    monkeypatch.setattr("pixel_cli.config.get_pixel_agents_home", lambda: str(pixel_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(venv_python), "-m", "pixel_cli.main"]
    assert cwd == str(pixel_home.resolve())
    assert env_overlay["VIRTUAL_ENV"] == str(project / "venv")
    assert str(project) in env_overlay["PYTHONPATH"].split(gateway_windows.os.pathsep)


class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_pixel_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".pixel-agents"
        home.mkdir()
        monkeypatch.setattr("pixel_cli.config.get_pixel_agents_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".pixel-agents"
        project = tmp_path / "checkout"
        monkeypatch.setattr("pixel_cli.config.get_pixel_agents_home", lambda: missing)
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "PixelAgents_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "PixelAgents_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "PixelAgents_Gateway_alice")
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Pixel Agents\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Pixel Agents\venv\Scripts\python.exe"
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("PixelAgents_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "PixelAgents_Gateway_alice")
    ...
    assert "PixelAgents_Gateway_alice.vbs" in xml_seen["text"]
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "pixel_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("PIXEL_AGENTS_HOME", "PYTHONIOENCODING", "PIXEL_AGENTS_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
# resume_pending=True) never fires when `pixel-agents gateway stop` kills the
