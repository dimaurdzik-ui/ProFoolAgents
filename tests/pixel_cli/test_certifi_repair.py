...
1. The venv-repair import probes (early recovery + `pixel-agents update`) must
...
2. ``pixel-agents doctor`` must fail the certificate check in that state, and
   ``pixel-agents doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `pixel-agents update`'s venv repair must apply
    ...
    def _run_probe_script(self, monkeypatch, tmp_path, bundle_path):
        ...
        from pixel_cli import main as main_mod
...
# =========================================================================
# 2. pixel-agents doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "pixel-agents doctor --fix" in message
