...
2. ``pixel_cli.main._refresh_windows_gateway_launchers`` — ``pixel-agents
...
def test_restart_spec_normalizes_legacy_pythonw_argv(tmp_path):
    ...
    argv = [str(pythonw), "-m", "pixel_cli.main", "gateway", "run"]
    ...
    ), mock.patch("pixel_cli.config.get_pixel_agents_home", return_value=str(tmp_path)):
...
# ---------------------------------------------------------------------------
# _refresh_windows_gateway_launchers: pixel-agents update regenerates launchers
# ---------------------------------------------------------------------------
