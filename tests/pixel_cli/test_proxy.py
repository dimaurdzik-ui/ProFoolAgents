"""Tests for the `pixel-agents proxy` subcommand and its upstream adapters."""
...
from pixel_cli.proxy.adapters import ADAPTERS, get_adapter
from pixel_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from pixel_cli.proxy.adapters.pixel_portal import PixelPortalAdapter
from pixel_cli.proxy.adapters.xai import XAIGrokAdapter
...
def _write_auth_store(pixel_home: Path, pixel_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given pixel state into a hermetic PIXEL_AGENTS_HOME."""
    auth_path = pixel_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"pixel": pixel_state},
    }))
    return auth_path
...
def test_pixel_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with patch(
        "pixel_cli.proxy.adapters.pixel_portal.resolve_pixel_runtime_credentials",
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic PIXEL_AGENTS_HOME."""
    auth_path = pixel_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("pixel_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from pixel_cli.proxy.server import create_app  # noqa: E402
