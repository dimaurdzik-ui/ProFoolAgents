...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $PIXEL_AGENTS_HOME.

    Models a host git install whose $PIXEL_AGENTS_HOME is shared with (and stamped
    'docker' by) a co-located container. The code-scoped stamp must win so the
    host install is correctly identified as 'git' and 'pixel-agents update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("pixel_cli.config.get_managed_system", return_value=None), \
         patch("pixel_cli.config.get_pixel_agents_home", return_value=home):
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $PIXEL_AGENTS_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
    with patch("pixel_cli.config.get_pixel_agents_home", return_value=home):
        from pixel_cli.config import stamp_install_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``pixel-agents update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("pixel_cli.config.get_managed_system", return_value=None), \
         patch("pixel_cli.config.get_pixel_agents_home", return_value=tmp_path), \
         patch("pixel_constants.is_container", return_value=True):
        from pixel_cli.config import detect_install_method
