...
Covers:
- Defense 1: gateway stop/restart refuse when _PIXEL_AGENTS_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "pixel-agents  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "PIXEL_AGENTS GATEWAY RESTART",           # uppercase
    ])
    def test_pixel_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "pixel-agents update",
        "pixel-agents config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `pixel-agents gateway start` is benign — starting a gateway from inside a
        ...
        "pixel-agents gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway pixel-agents
        # services must not be falsely blocked (the old `.*pixel-agents` matched any
        # pixel-agents token).
        "launchctl unload ai.pixel-agents.update-checker.plist",
        "launchctl restart ai.pixel-agents.daemon",
        "systemctl restart pixel-agents-meta.service",
        "systemctl restart pixel-agents-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_pixel_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade pixel-agents then run pixel-agents gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_pixel_gateway_once.sh). The script must live under
        # PIXEL_AGENTS_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / ".pixel-agents"))
        scripts_dir = tmp_path / ".pixel-agents" / "scripts"
        scripts_dir.mkdir(parents=True)
        (scripts_dir / "restart.sh").write_text("#!/bin/bash\nPixelAgents gateway restart\n")
        args = Namespace(
            cron_command="create",
            schedule="1h",
            prompt=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify pixel-agents gateway stop/restart refuse when _PIXEL_AGENTS_GATEWAY=1."""
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_PIXEL_AGENTS_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _PIXEL_AGENTS_GATEWAY=1.

    Issue #37453: systemctl --user restart pixel-agents-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart pixel-agents-gateway",
        "systemctl --user restart pixel-agents-gateway",
        "systemctl stop pixel-agents-gateway.service",
        "pixel-agents gateway restart",
        "launchctl kickstart gui/501/ai.pixel-agents.gateway",
        "pkill -f pixel-agents.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart pixel-agents-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-pixel-agents systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run pixel-agents gateway restart", None)
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfePixelAgents gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under PIXEL_AGENTS_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.pixel-agents.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    pixel_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run pixel-agents gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / ".pixel-agents"))
        (tmp_path / ".pixel-agents").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run pixel-agents gateway restart nightly",
        ))
