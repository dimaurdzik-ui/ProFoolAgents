"""Tests for the stale-dashboard handling run at the end of ``pixel-agents update``.

``pixel-agents update`` detects ``pixel-agents dashboard`` processes left over from the
...
def _refresh_bindings_against_live_module():
    """Rebind module-level names to the *current* ``pixel_cli.main``.

    Other tests in the suite (notably ``test_env_loader.py`` and
    ``test_skills_subparser.py``) reload or delete ``pixel_cli.main`` from
    ``sys.modules``.  When that happens on the same xdist worker before we
    run, our top-of-file ``from pixel_cli.main import ...`` bindings end
    up pointing at the *old* module object.  ``patch(\"pixel_cli.main.X\")``
    ...
    live = sys.modules.get("pixel_cli.main")
    if live is None:
        live = importlib.import_module("pixel_cli.main")
    ...
    def test_self_pid_excluded(self):
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m pixel_cli.main dashboard"),
                    _ps_line(12345, "pixel-agents dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("pixel_cli.main._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        def fake_run(args, *a, **kw):
            ...
            if args == ["systemctl", "--user", "list-unit-files", "pixel-agents-dashboard.service", "--no-legend", "--no-pager"]:
                return MagicMock(returncode=0, stdout="pixel-agents-dashboard.service enabled enabled\n", stderr="")
            if args == ["systemctl", "--user", "is-active", "pixel-agents-dashboard.service"]:
                return MagicMock(returncode=0, stdout="active\n", stderr="")
            if args == ["systemctl", "--user", "is-enabled", "pixel-agents-dashboard.service"]:
                return MagicMock(returncode=0, stdout="enabled\n", stderr="")
            if args == ["systemctl", "--user", "restart", "pixel-agents-dashboard.service"]:
                return MagicMock(returncode=0, stdout="", stderr="")
            raise AssertionError(f"unexpected subprocess.run call: {args}")

        with patch("subprocess.run", side_effect=fake_run), \
             patch("pixel_cli.main._find_stale_dashboard_pids", return_value=[12345]) as find_pids, \
             patch("os.kill") as kill:
            _kill_stale_dashboard_processes(restart_managed=True)

        assert calls == [
            ["systemctl", "--user", "list-unit-files", "pixel-agents-dashboard.service", "--no-legend", "--no-pager"],
            ["systemctl", "--user", "is-active", "pixel-agents-dashboard.service"],
            ["systemctl", "--user", "is-enabled", "pixel-agents-dashboard.service"],
            ["systemctl", "--user", "restart", "pixel-agents-dashboard.service"],
        ]
        assert all(call[:1] != ["sudo"] and call[:2] != ["systemctl"] for call in calls)
        find_pids.assert_not_called()
        kill.assert_not_called()
        assert "✓ restarted pixel-agents-dashboard.service" in capsys.readouterr().out
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys):
        with patch(
            "pixel_cli.main._kill_stale_dashboard_processes",
...
class TestWindowsWmicEncoding:
    ...
    `pixel-agents update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_invoked_with_utf8_ignore_errors(self, monkeypatch):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "CommandLine=python -m pixel_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
...
class TestSupervisedBackendRestart:
    ...
    def _live(self):
        return sys.modules["pixel_cli.main"]
    ...
    def test_supervised_pid_restarts_owning_unit(self, capsys):
        ...
        with patch.object(live, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch.object(live, "_get_pid_cgroup_path",
                          return_value="/system.slice/pixel-agents-serve.service"), \
             patch.object(live, "_get_systemd_service_for_pid",
                          return_value="pixel-agents-serve.service"), \
        ...
        restart.assert_called_once_with(
            "pixel-agents-serve.service", "/system.slice/pixel-agents-serve.service"
        )
        out = capsys.readouterr().out
        assert "✓ restarted systemd service pixel-agents-serve.service" in out
...
class TestManualBackendRespawn:
    ...
    def test_respawn_adds_no_open_to_dashboard_commands(self, tmp_path, monkeypatch):
        ...
        with patch.object(live.subprocess, "Popen", _FakePopen):
            failed = live._respawn_dashboard_processes([
                ["pixel-agents", "dashboard", "--port", "8300"],
                ["pixel-agents", "serve", "--host", "0.0.0.0"],
            ])

        assert failed == []
        assert spawned[0] == ["pixel-agents", "dashboard", "--port", "8300", "--no-open"]
        assert spawned[1] == ["pixel-agents", "serve", "--host", "0.0.0.0"]
    ...
    def test_respawn_failure_returned(self, tmp_path, monkeypatch, capsys):
        ...
        with patch.object(live.subprocess, "Popen", side_effect=OSError("no such file")):
            failed = live._respawn_dashboard_processes([["pixel-agents", "serve"]])

        assert failed == [["pixel-agents", "serve"]]
        out = capsys.readouterr().out
...
class TestCmdlineCapture:
    ...
    def test_reads_proc_cmdline_when_available(self, tmp_path, monkeypatch):
        ...
        proc_file.write_bytes(b"/usr/bin/python3\x00-m\x00pixel_cli.main\x00serve\x00")
        ...
        assert argv == ["/usr/bin/python3", "-m", "pixel_cli.main", "serve"]
    ...
    def test_falls_back_to_ps_without_proc(self, monkeypatch):
        ...
        def fake_run(args, *a, **kw):
            ...
            return MagicMock(returncode=0, stdout="pixel-agents serve --port 8300\n", stderr="")

        with patch.object(live.os.path, "exists", return_value=False), \
             patch("subprocess.run", side_effect=fake_run):
            argv = live._dashboard_cmdline_for_pid(888)

        assert argv == ["pixel-agents", "serve", "--port", "8300"]
