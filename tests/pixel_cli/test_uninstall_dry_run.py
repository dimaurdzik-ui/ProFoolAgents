...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_pixel_agents_home", lambda: pixel_home)
    monkeypatch.setattr(uninstall, "_is_default_pixel_home", lambda home: False)
    ...
    assert str(pixel_home) in output
    assert project_root.exists()
    assert pixel_home.exists()
