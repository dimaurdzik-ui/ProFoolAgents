"""Tests for `pixel-agents fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate PIXEL_AGENTS_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".pixel-agents" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from pixel_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "pixel", "model": "Pixel Agents-4-Llama-3.1-405B"},
            ]
        }
    ...
    def test_returns_copies_not_aliases(self):
        from pixel_cli.fallback_cmd import _read_chain
        cfg = {"fallback_providers": [{"provider": "pixel", "model": "foo"}]}
        result = _read_chain(cfg)
        result[0]["provider"] = "mutated"
        assert cfg["fallback_providers"][0]["provider"] == "pixel"
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from pixel_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        from pixel_cli.fallback_cmd import cmd_fallback_list
        cmd_fallback_list(types.SimpleNamespace())
        out = capsys.readouterr().out
        assert "No fallback providers configured" in out
        assert "pixel-agents fallback add" in out
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("pixel_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("pixel_cli.main._require_tty"):
            from pixel_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "pixel / Pixel Agents-4")
        with patch("pixel_cli.setup._curses_prompt_choice", return_value=1):
            from pixel_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from pixel_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `pixel-agents fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "pixel_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
