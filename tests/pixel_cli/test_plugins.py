"""Tests for the Pixel Agents plugin system (pixel_cli.plugins)."""
...
from pixel_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<pixel_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<pixel_home>/plugins/``; we derive
    ``<pixel_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <PIXEL_AGENTS_HOME>/config.yaml.
        # Config is always read from PIXEL_AGENTS_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        pixel_home_str = os.environ.get("PIXEL_AGENTS_HOME")
        if pixel_home_str:
            pixel_home = Path(pixel_home_str)
        else:
            pixel_home = base.parent
        pixel_home.mkdir(parents=True, exist_ok=True)
        cfg_path = pixel_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_plugin_can_register_and_invoke_middleware(self, tmp_path, monkeypatch):
        plugins_dir = tmp_path / "pixel_test" / "plugins"
        _make_plugin_dir(
            plugins_dir,
            "mw_plugin",
            register_body=(
        ...
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / "pixel_test"))
    ...
    def test_middleware_helpers_skip_no_listener_work(self, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.plugins.get_plugin_manager", lambda: manager)
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under pixel_plugins.<name>."""
        plugins_dir = tmp_path / "pixel_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / "pixel_test"))

        # Clean up any prior namespace module
        sys.modules.pop("pixel_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "pixel_plugins.ns_plugin" in sys.modules
    ...
    def test_user_memory_plugin_auto_coerced_to_exclusive(self, tmp_path, monkeypatch):
        ...
        pixel_home = tmp_path / "pixel_test"
...
class TestPreToolCallDirective:
    ...
    def test_approve_without_message_is_valid(self, monkeypatch):
        ...
        from pixel_cli.plugins import get_pre_tool_call_directive
        monkeypatch.setattr(
            "pixel_cli.plugins.invoke_hook",
...
class TestResolvePreToolBlock:
    ...
    def test_approve_passes_plugin_rule_key_to_gate(self, monkeypatch):
        from pixel_cli.plugins import resolve_pre_tool_block
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("pixel_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        from pixel_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from pixel_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("pixel_plugins.delayed_override_plugin")
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        """
        import pixel_cli.plugins as plugins_mod
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `pixel-agents plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_register_command_empty_name_rejected(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="pixel_cli.plugins"):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        pixel_home = tmp_path / "pixel_test"
        plugins_dir = pixel_home / "plugins"
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("pixel_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("pixel_cli.plugins.PluginContext.dispatch_tool.__module__", "pixel_cli.plugins"):
...
class TestPluginDebugLogging:
    """PIXEL_AGENTS_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("PIXEL_AGENTS_PLUGINS_DEBUG", raising=False)
        from pixel_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from PIXEL_AGENTS_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """PIXEL_AGENTS_HOME at the root resolves to 'default'."""
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """PIXEL_AGENTS_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".pixel-agents" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(prof))
