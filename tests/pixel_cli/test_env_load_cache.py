...
The cache exists to keep `pixel-agents tools` → "All Platforms" fast: every
...
def test_remove_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from pixel_cli import config as config_mod
    ...
    monkeypatch.setattr(config_mod, "ensure_pixel_home", lambda: None)
