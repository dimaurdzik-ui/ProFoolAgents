"""Tests for --ignore-user-config and --ignore-rules flags on `pixel-agents chat`.
...
files. In Pixel Agents the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.pixel-agents/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → PixelAgentsCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("PIXEL_AGENTS_IGNORE_USER_CONFIG", "PIXEL_AGENTS_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``PIXEL_AGENTS_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<pixel_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With PIXEL_AGENTS_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._pixel_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_user_config_skipped_when_flag_set(self, tmp_path, monkeypatch):
        """With PIXEL_AGENTS_IGNORE_USER_CONFIG=1, user config.yaml is ignored.

        The built-in default ``model.default`` is empty string (no user override),
        and the user's ``agent.system_prompt`` is not seen.
        """
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("PIXEL_AGENTS_IGNORE_USER_CONFIG", "1")
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("PIXEL_AGENTS_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``PixelAgentsCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting PIXEL_AGENTS_IGNORE_RULES=1 flips PixelAgentsCLI.ignore_rules True."""
        monkeypatch.setenv("PIXEL_AGENTS_IGNORE_RULES", "1")

        # Import PixelAgentsCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of PixelAgentsCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.PixelAgentsCLI)
        # Replicate the exact logic from cli.py PixelAgentsCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("PIXEL_AGENTS_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True


class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``pixel_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("PIXEL_AGENTS_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("PIXEL_AGENTS_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("PIXEL_AGENTS_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("PIXEL_AGENTS_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "PIXEL_AGENTS_IGNORE_USER_CONFIG" not in os.environ
        assert "PIXEL_AGENTS_IGNORE_RULES" not in os.environ
