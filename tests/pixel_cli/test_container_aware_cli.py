...
def container_env(tmp_path, monkeypatch):
    """Set up a fake PIXEL_AGENTS_HOME with .container-mode file."""
    pixel_home = tmp_path / ".pixel-agents"
    pixel_home.mkdir()
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))
    monkeypatch.delenv("PIXEL_AGENTS_DEV", raising=False)

    container_mode = pixel_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=pixel-agents\n"
        "exec_user=pixel-agents\n"
        "pixel_bin=/data/current-package/bin/pixel-agents\n"
    )
    return pixel_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("pixel_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "pixel-agents"
    assert info["exec_user"] == "pixel-agents"
    assert info["pixel_bin"] == "/data/current-package/bin/pixel-agents"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from pixel_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "pixel-agents"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "pixel-agents" in cmd
    assert "/data/current-package/bin/pixel-agents" in cmd
    assert "chat" in cmd
