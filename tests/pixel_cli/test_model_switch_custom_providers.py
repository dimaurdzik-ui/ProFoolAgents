...
The terminal `pixel-agents model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import pixel_cli.providers as providers_mod
import pytest
...
def test_list_authenticated_providers_includes_custom_providers(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "PIXEL_AGENTS_OVERLAYS", {})
    monkeypatch.setattr("pixel_cli.models.fetch_api_models", lambda *a, **k: [])
...
def test_picker_selection_resolves_named_custom_provider_model_id(monkeypatch):
    ...
    monkeypatch.setattr("pixel_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr(
        "pixel_cli.model_switch.get_model_capabilities",
        lambda *a, **k: None,
    )
...
def test_resolve_custom_provider_passes_key_env():
    ...
    """
    from pixel_cli.providers import resolve_custom_provider
...
def test_discovered_models_auto_saved_to_cache(monkeypatch):
    ...
    monkeypatch.setattr("pixel_cli.models.fetch_api_models", fake_fetch_api_models)
    monkeypatch.setattr(
        "pixel_cli.model_switch._save_discovered_models_to_config",
...
def test_save_discovered_models_preserves_dict_form(monkeypatch):
    ...
    from pixel_cli.model_switch import _save_discovered_models_to_config
    ...
    monkeypatch.setattr("pixel_cli.config.save_config", fake_save)
    monkeypatch.setattr(
