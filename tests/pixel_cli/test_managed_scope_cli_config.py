...
pixel_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `pixel-agents config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_pixel_home = get_pixel_agents_home()`` at import time (module
    singleton), so monkeypatching PIXEL_AGENTS_HOME after import doesn't move it.
    ...
    this only matters for tests that swap PIXEL_AGENTS_HOME.)
    """
    import cli

    cli._pixel_home = home
