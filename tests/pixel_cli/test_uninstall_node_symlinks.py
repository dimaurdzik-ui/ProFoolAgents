"""Tests for pixel_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $PIXEL_AGENTS_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Pixel Agents-managed node dir.
"""
...
def _make_pixel_node(pixel_home: Path) -> Path:
    """Create a fake $PIXEL_AGENTS_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = pixel_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    pixel_home = fake_home / ".pixel-agents"
    _make_pixel_node(pixel_home)
    ...
    removed = uninstall.remove_node_symlinks(pixel_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    pixel_home = fake_home / ".pixel-agents"
    node_bin = _make_pixel_node(pixel_home)
