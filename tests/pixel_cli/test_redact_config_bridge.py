...
`PIXEL_AGENTS_REDACT_SECRETS` at module-import time. `pixel_cli/main.py` at
...
in .env), the toggle was silently ignored in both `pixel-agents chat` and
`pixel-agents gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `PIXEL_AGENTS_REDACT_SECRETS`
env var in `pixel_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports pixel_cli.main and checks
    # _REDACT_ENABLED. Must be a subprocess — we need a clean module state.
    probe = textwrap.dedent(
        """\
        import sys, os
        # Make absolutely sure the env var is not pre-set
        os.environ.pop("PIXEL_AGENTS_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
        import pixel_cli.main  # triggers the bridge + setup_logging
        import agent.redact
        print(f"REDACT_ENABLED={agent.redact._REDACT_ENABLED}")
        print(f"ENV_VAR={os.environ.get('PIXEL_AGENTS_REDACT_SECRETS', '<unset>')}")
        """
    ) % str(REPO_ROOT)

    env = dict(os.environ)
    env["PIXEL_AGENTS_HOME"] = str(pixel_home)
    env.pop("PIXEL_AGENTS_REDACT_SECRETS", None)
...
def test_redact_secrets_default_true_when_unset(tmp_path):
    ...
    `PIXEL_AGENTS_REDACT_SECRETS=false`).
    """
    pixel_home = tmp_path / ".pixel-agents"
    pixel_home.mkdir()
    (pixel_home / "config.yaml").write_text("{}\n")  # empty config
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env PIXEL_AGENTS_REDACT_SECRETS takes precedence over config.yaml."""
    pixel_home = tmp_path / ".pixel-agents"
    pixel_home.mkdir()
    (pixel_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (pixel_home / ".env").write_text("PIXEL_AGENTS_REDACT_SECRETS=true\n")
