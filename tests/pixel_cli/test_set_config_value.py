...
def _isolated_pixel_home(tmp_path):
    """Point PIXEL_AGENTS_HOME at a temp dir so tests never touch real config."""
    env_file = tmp_path / ".env"
    env_file.touch()
    with patch.dict(os.environ, {"PIXEL_AGENTS_HOME": str(tmp_path)}):
        yield tmp_path
...
class TestExplicitAllowlist:
    ...
    def test_explicit_key_routes_to_env(self, key, _isolated_pixel_home):
        set_config_value(key, "test-value-123")
        env_content = _read_env(_isolated_pixel_home)
        assert f"{key}=test-value-123" in env_content
        # Must NOT appear in config.yaml
        assert key not in _read_config(_isolated_pixel_home)
...
class TestCatchAllPatterns:
    ...
    def test_api_key_suffix_routes_to_env(self, key, _isolated_pixel_home):
...
class TestConfigYamlRouting:
    ...
    def test_simple_key(self, _isolated_pixel_home):
        set_config_value("model", "gpt-4o")
        config = _read_config(_isolated_pixel_home)
        assert "gpt-4o" in config
        assert "model" not in _read_env(_isolated_pixel_home)


    def test_terminal_image_goes_to_config(self, _isolated_pixel_home):
    ...
    def test_terminal_docker_cwd_mount_flag_goes_to_config_and_env(self, _isolated_pixel_home):
    ...
    def test_terminal_vercel_runtime_goes_to_config_and_env(self, _isolated_pixel_home):
...
class TestFalsyValues:
    ...
    def test_config_command_accepts_empty_string(self, _isolated_pixel_home):
...
class TestConfigGetUnset:
    ...
    def test_config_get_prints_resolved_nested_value(self, _isolated_pixel_home, capsys):
    ...
    def test_config_unset_removes_yaml_key_and_synced_env(self, _isolated_pixel_home, capsys):
        set_config_value("terminal.backend", "docker")
        assert "TERMINAL_ENV=docker" in _read_env(_isolated_pixel_home)
        capsys.readouterr()
        ...
        import yaml
        reloaded = yaml.safe_load(_read_config(_isolated_pixel_home)) or {}
        assert reloaded == {}
        assert "TERMINAL_ENV=" not in _read_env(_isolated_pixel_home)
        assert "Unset terminal.backend" in capsys.readouterr().out


    def test_config_unset_removes_dotted_token_yaml_key(self, _isolated_pixel_home, capsys):
        (_isolated_pixel_home / "config.yaml").write_text(
            "platforms:\n"
            "  teams:\n"
            "    extra:\n"
        ...
        import yaml
        reloaded = yaml.safe_load(_read_config(_isolated_pixel_home))
...
class TestListNavigation:
    """pixel-agents config set must preserve YAML list fields when using numeric
    ...
    def test_indexed_set_preserves_sibling_list_entries(self, _isolated_pixel_home):
        """Setting custom_providers.0.api_key must not destroy entry 1."""
        self._write_config(_isolated_pixel_home, (
            "custom_providers:\n"
            "- name: provider-a\n"
            "  api_key: old-a\n"
    ...
    def test_indexed_set_preserves_non_targeted_fields(self, _isolated_pixel_home):
    ...
    def test_deeper_nesting_through_list(self, _isolated_pixel_home):
...
class TestStringTypedConfigValues:
    ...
    def test_string_typed_values_are_not_coerced(self, _isolated_pixel_home, value):
        """Values stay strings when DEFAULT_CONFIG declares the leaf as a string."""
        set_config_value("approvals.mode", value)

        import yaml
        saved = yaml.safe_load(_read_config(_isolated_pixel_home))
    ...
    def test_non_string_defaults_keep_existing_coercion(
        self, _isolated_pixel_home, key, value, expected
    ):
    ...
    def test_unknown_keys_keep_existing_coercion(self, _isolated_pixel_home):
...
class TestSecretRedactionInDisplay:
    ...
    def test_redact_config_value_masks_nested_api_key(self):
        from pixel_cli.config import redact_config_value
    ...
    def test_set_echo_masks_secret_value(self, _isolated_pixel_home, capsys):
    ...
    def test_set_echo_keeps_nonsecret_value(self, _isolated_pixel_home, capsys):
...
class TestSchemaValidation:
    """#34067: ``pixel-agents config set`` must not report bare success for
    ...
    a post-write notice warns that Pixel Agents may never read it and suggests the
    ...
    def test_desktop_macos_signing_identity_is_accepted(self, _isolated_pixel_home, capsys):
        """The documented TCC signing identity setting is part of the schema."""
        set_config_value("desktop.macos_signing_identity", "Pixel Agents Local Signing")
        import yaml
        saved = yaml.safe_load(_read_config(_isolated_pixel_home))
        assert saved["desktop"]["macos_signing_identity"] == "Pixel Agents Local Signing"
        assert "not a recognized config key" not in capsys.readouterr().out



    def test_force_suppresses_notice(self, _isolated_pixel_home, capsys):
        ...
        content = _read_config(_isolated_pixel_home)
...
class TestValidateConfigKey:
    ...
    def test_known_keys_pass(self, key):
        from pixel_cli.config import _validate_config_key
...
class TestDisplaySkinTouch:
    ...
    (`pixel-agents config set display.skin X` while it is already X — the recovery
    ...
    def test_reaffirming_same_skin_moves_the_watcher_signature(self, _isolated_pixel_home):
        import os as _os
        skins = _isolated_pixel_home / "skins"
        skins.mkdir()
    ...
    def test_builtin_or_missing_skin_file_is_fine(self, _isolated_pixel_home):
        """Built-ins have no user file — the set must still succeed cleanly."""
        set_config_value("display.skin", "mono")
        assert "skin: mono" in _read_config(_isolated_pixel_home)

    def test_touch_preserves_skin_file_contents(self, _isolated_pixel_home):
        skins = _isolated_pixel_home / "skins"
        skins.mkdir()
        body = "name: neon\ncolors:\n  ui_accent: '#ff33aa'\n"
