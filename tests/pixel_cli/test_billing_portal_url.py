"""Portal-URL resolution for Phase 2b billing errors (pixel_billing).
...
def _preview(monkeypatch):
    monkeypatch.setenv("PIXEL_AGENTS_PORTAL_BASE_URL", "https://nas-pr-412.pixelagents.wtf")


def test_absolutize_resolves_relative(_preview):
    assert (
        _absolutize_portal_url("/billing?topup=open")
        == "https://nas-pr-412.pixelagents.wtf/billing?topup=open"
    )
