...
When ``PIXEL_AGENTS_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.pixel-agents/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.pixel-agents            (has its own auth.json fixture)
    * Profile     -> tmp_path/.pixel-agents/profiles/coder   (active, PIXEL_AGENTS_HOME points here)
    ...
    global_root = tmp_path / ".pixel-agents"
    global_root.mkdir()
...
def test_provider_auth_state_falls_back_to_global_when_profile_has_none(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "pixel": {"access_token": "pixel-global", "refresh_token": "rt-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))

    state = get_provider_auth_state("pixel")
    assert state is not None
    assert state["access_token"] == "pixel-global"
...
def test_provider_auth_state_returns_none_when_neither_has_it(profile_env):
    ...
    assert get_provider_auth_state("pixel") is None


# ---------------------------------------------------------------------------
# _load_provider_state — internal global fallback (issue #18594 follow-up)
#
# Several runtime helpers (notably ``resolve_pixel_runtime_credentials`` and
# ``resolve_pixel_access_token``) call ``_load_provider_state`` directly with
# a profile-loaded auth store rather than going through
# ``get_provider_auth_state``. Without the fallback wired into
# ``_load_provider_state`` itself, those helpers raise ``"Pixel Agents is not
# logged into Pixel Portal"`` even though the user has a valid global Pixel
...
def test_auth_lock_reentrancy_is_scoped_after_profile_context_switch(profile_env):
    ...
    with auth._auth_store_lock():
        ...
        token = set_pixel_agents_home_override(profile_b)
        try:
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (PIXEL_AGENTS_HOME != ~/.pixel-agents, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    pixel_home = tmp_path / "classic"
    pixel_home.mkdir()
