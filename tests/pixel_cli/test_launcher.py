"""Tests for the top-level `./pixel-agents` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./pixel-agents` should use `pixel_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "pixel-agents"
    called = []

    fake_main_module = types.ModuleType("pixel_cli.main")

    def fake_main():
        called.append("pixel_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "pixel_cli.main", fake_main_module)
