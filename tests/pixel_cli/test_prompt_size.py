"""Tests for the ``pixel-agents prompt-size`` diagnostic (issue #34667)."""
...
from pixel_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(pixel_home, memory_text="", user_text=""):
    mem_dir = pixel_home / "memories"
...
def _seed_skill(pixel_home, name, description):
    skill_dir = pixel_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    ...
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "PIXEL_API_KEY",
                "ANTHROPIC_API_KEY"):
