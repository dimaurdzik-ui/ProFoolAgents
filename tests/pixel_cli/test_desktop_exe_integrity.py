...
Pixel Agents.exe on the end user's machine. Before this gate, "build succeeded" was
...
# MACHINE_ATTRIBUTES.UserEnabled — mirrors pixel_cli.main.
...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    backup_exe = desktop_dir / "release" / "win-unpacked.bak" / "Pixel Agents.exe"
    make_pe(backup_exe, PE_AMD64)  # valid old build

    with patch("pixel_cli.main._windows_native_machine", return_value="AMD64"):
    ...
    assert (desktop_dir / "release" / "win-unpacked.corrupt" / "Pixel Agents.exe").exists()
...
def test_gate_fails_clearly_without_backup(tmp_path, monkeypatch, capsys):
    ...
    with patch("pixel_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("pixel_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"):
...
# ─── end-to-end: `pixel-agents desktop --build-only` exits nonzero on corrupt exe ─
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    """The updater chain's contract: a rebuild whose Pixel Agents.exe cannot launch
    must exit nonzero (so pixel-agents-setup's retry-once kicks in) and must restore
    ...
    make_pe(desktop_dir / "release" / "win-unpacked.bak" / "Pixel Agents.exe", PE_AMD64)
    ...
    with patch("pixel_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("pixel_cli.main._stop_desktop_processes_locking_build", return_value=[]), \
         patch("pixel_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("pixel_cli.main._desktop_stamp_path", return_value=tmp_path / "stamp.json"), \
         patch("pixel_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("pixel_cli.main._windows_native_machine", return_value="AMD64"), \
         patch("pixel_cli.main.subprocess.run", return_value=pack_ok), \
