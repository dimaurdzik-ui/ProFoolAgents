...
PIXEL_AGENTS_HOME (tests/conftest.py isolation) and assert every store agrees
afterwards.
...
@pytest.fixture
def pixel_home(monkeypatch, tmp_path):
    """Fresh PIXEL_AGENTS_HOME with .env + auth.json + config.yaml fixtures."""
    home = tmp_path / "cred_home"
    home.mkdir()
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(home))
    from pixel_cli.config import invalidate_env_cache
...
def test_delete_clears_provider_models_cache(pixel_home):
    _write_env(pixel_home, ZAI_API_KEY=FAKE_ZAI_KEY)
    _write_auth(pixel_home, {"zai": [_zai_pool_fixture()["zai"][0]]})
    cache_path = pixel_home / "provider_models_cache.json"
...
def test_update_rotates_config_yaml_model_mirror(pixel_home):
    old = "sk-oe-" + "f" * 24
    new = "sk-oe-" + "g" * 24
    _write_env(pixel_home, OPENAI_API_KEY=old)
    _write_config(
        pixel_home,
        "model:\n"
        "  provider: custom\n"
    ...
    cfg_text = pixel_home.joinpath("config.yaml").read_text(encoding="utf-8")
