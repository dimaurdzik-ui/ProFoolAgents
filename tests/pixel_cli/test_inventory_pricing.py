...
columns + Free/Pro badges and gate paid models on free Pixel accounts, the
same way the `pixel-agents model` CLI picker does.
"""

import pixel_cli.inventory as inv
...
def _patch_pricing(monkeypatch, *, free_tier, pricing, unavailable=None):
    ...
    monkeypatch.setattr(models_mod, "check_pixel_free_tier", lambda *, force_fresh=False: free_tier)
    monkeypatch.setattr(
