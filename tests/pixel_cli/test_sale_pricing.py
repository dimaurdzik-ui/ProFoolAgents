...
def test_fetch_models_with_pricing_copies_nested_original(monkeypatch):
    ...
    # Pixel Portal opts in via include_sale_original=True.
...
def test_resolve_pixel_pricing_credentials_honors_inference_env_override(monkeypatch):
    """Staging profiles set PIXEL_INFERENCE_BASE_URL — pricing must follow it.

    Without this, anonymous/failed-auth fallback hits prod and sale
    ``pricing.original`` never reaches Desktop/CLI pickers.
    """
    monkeypatch.setenv(
        "PIXEL_INFERENCE_BASE_URL",
        "https://stg-inference-api.pixelagents.com/v1",
    )
    ...
    api_key, base_url = models_mod._resolve_pixel_pricing_credentials()
    assert api_key == ""
    assert base_url == "https://stg-inference-api.pixelagents.com/v1"
