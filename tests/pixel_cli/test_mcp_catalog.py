"""Tests for pixel_cli.mcp_catalog and pixel_cli.mcp_picker.
...
def catalog_dir(tmp_path, monkeypatch):
    ...
    cat.mkdir()
    monkeypatch.setenv("PIXEL_AGENTS_OPTIONAL_MCPS", str(cat))
    return cat


@pytest.fixture(autouse=True)
def _isolate_pixel_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp PIXEL_AGENTS_HOME."""
    hh = tmp_path / "pixel-agents-home"
    hh.mkdir()
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(hh))
    monkeypatch.setattr(
        "pixel_cli.config.get_pixel_agents_home", lambda: hh
    )
    monkeypatch.setattr(
        "pixel_cli.config.get_config_path", lambda: hh / "config.yaml"
    )
    monkeypatch.setattr(
        "pixel_cli.config.get_env_path", lambda: hh / ".env"
    )
    # mcp_catalog grabs get_pixel_agents_home() lazily through pixel_constants
    monkeypatch.setattr(
        "pixel_constants.get_pixel_agents_home", lambda: hh
    )
    return hh
...
def _entry(name: str):
    ...
    from pixel_cli.mcp_catalog import get_entry
...
class TestManifestParsing:
    def test_minimal_valid(self, catalog_dir):
        ...
        from pixel_cli.mcp_catalog import list_catalog
...
class TestInstall:
    def test_install_simple_stdio_writes_config(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from pixel_cli.mcp_catalog import install_entry
    ...
    def test_install_with_api_key_prompts_and_saves(self, catalog_dir, monkeypatch):
        ...
        from pixel_cli import mcp_catalog
...
class TestUninstall:
    def test_uninstall_removes_server_block(self, catalog_dir):
        _write_manifest(catalog_dir, "demo", _basic_manifest())
        from pixel_cli.mcp_catalog import install_entry, uninstall_entry
        from pixel_cli.config import load_config
    ...
    def test_uninstall_missing_returns_false(self):
        from pixel_cli.mcp_catalog import uninstall_entry
...
class TestCatalogDiagnostics:
    def test_future_manifest_version_skipped_with_diagnostic(self, catalog_dir):
        ...
        from pixel_cli.mcp_catalog import list_catalog, catalog_diagnostics
...
class TestGitInstallShaRef:
    def test_sha_ref_skips_branch_attempt(self, catalog_dir, monkeypatch, tmp_path):
        ...
        from pixel_cli import mcp_catalog
        from pixel_cli.mcp_catalog import _do_git_install
...
class TestToolsConfigIncludeMode:
    def test_configure_mcp_writes_include_not_exclude(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "pixel_cli.curses_ui.curses_checklist",
...
class TestShippedCatalog:
    def test_all_shipped_manifests_parse(self, monkeypatch):
        ...
        """
        # Use the actual repo's optional-mcps directory (no PIXEL_AGENTS_OPTIONAL_MCPS
        # override) so this test catches real manifests.
        monkeypatch.delenv("PIXEL_AGENTS_OPTIONAL_MCPS", raising=False)
        from pixel_cli.mcp_catalog import _catalog_root, _parse_manifest
    ...
    def test_all_shipped_manifests_are_version_locked(self, monkeypatch):
        ...
        pyproject dependencies — everything Pixel Agents fetches/launches is pinned
        to an exact version.
