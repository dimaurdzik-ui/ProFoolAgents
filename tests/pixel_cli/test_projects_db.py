"""Tests for the per-profile Projects store (pixel_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Pixel Agents", folders=["/tmp/pixel-agents"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "pixel-agents"
    assert proj.name == "Pixel Agents"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/pixel-agents"
    assert [f.path for f in proj.folders] == ["/tmp/pixel-agents"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "pixel-agents").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' PIXEL_AGENTS_HOME.
