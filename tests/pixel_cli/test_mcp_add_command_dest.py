"""Regression test: ``pixel-agents mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``pixel_cli/_parser.py``).  The dispatcher in ``pixel_cli/main.py``
...
result, ``pixel-agents mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the pixel-agents parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`pixel-agents mcp add foo --url ...` must keep args.command == "mcp".
