"""Tests for Pixel subscription feature detection."""

from pixel_cli.pixel_account import PixelPortalAccountInfo, PixelToolAccessInfo
from pixel_cli import pixel_subscription as ns
...
def _account(*, logged_in: bool, paid: bool | None = None) -> PixelPortalAccountInfo:
...
def _pool_account() -> PixelPortalAccountInfo:
    ...
    return PixelPortalAccountInfo(
        ...
        tool_access=PixelToolAccessInfo(enabled=True, coverage=_POOL_COVERAGE),
    )


def test_get_pixel_subscription_features_recognizes_direct_exa_backend(monkeypatch):
    env = {"EXA_API_KEY": "exa-test"}

    monkeypatch.setattr(ns, "get_env_value", lambda name: env.get(name, ""))
    monkeypatch.setattr(
        ns, "get_pixel_portal_account_info", lambda: _account(logged_in=False)
    )
    ...
    features = ns.get_pixel_subscription_features({"web": {"backend": "exa"}})

    assert features.web.available is True
    assert features.web.active is True
    assert features.web.managed_by_pixel is False
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import pixel_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "pixel_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
def test_prompt_enable_tool_gateway_pool_offers_covered_tools_only(monkeypatch):
    ...
    monkeypatch.setattr(ns, "get_pixel_portal_account_info", lambda **kw: _pool_account())
    monkeypatch.setattr(
        ns,
...
def test_apply_pixel_managed_defaults_writes_video_gen_config(monkeypatch):
    """apply_pixel_managed_defaults must write video_gen.provider and
    video_gen.use_gateway when a Pixel subscriber selects video_gen
    without a direct FAL_KEY."""
    monkeypatch.setattr(ns, "managed_pixel_tools_enabled", lambda **kw: True)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setattr(ns, "fal_key_is_configured", lambda: False)
    monkeypatch.setattr(
        ns, "get_pixel_portal_account_info",
        lambda **kw: _account(logged_in=True, paid=True),
    )

    config = {"model": {"provider": "pixel"}}
    changed = ns.apply_pixel_managed_defaults(
...
# ---------------------------------------------------------------------------
# ensure_pixel_portal_access — inline login gate for `pixel-agents tools`
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# STT — managed-by-Pixel detection (Phase 4 follow-up)
# ---------------------------------------------------------------------------
...
def _stt_features_stub(*, account_info):
    return ns.PixelSubscriptionFeatures(
        subscribed=True,
        pixel_auth_present=True,
        provider_is_pixel=True,
        account_info=account_info,
        features={
            key: ns.PixelFeatureState(
                key=key, label=key, included_by_default=True,
                available=False, active=False, managed_by_pixel=False,
...
def test_has_agent_browser_resolves_via_pixel_managed_node_path(monkeypatch, tmp_path):
    """The managed-Node rung: a runnable agent-browser under the Pixel Agents Node
    ...
    monkeypatch.setattr(
        "pixel_constants.with_pixel_node_path", lambda: {"PATH": str(managed_dir)}
    )
    monkeypatch.setattr(
        "pixel_constants.agent_browser_runnable",
