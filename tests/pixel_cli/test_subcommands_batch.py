...
def test_config_get_unset_subcommands_parse():
    """`pixel-agents config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="pixel-agents")
...
# ── deprecated `pixel-agents login` fails gracefully, not with argparse error ────
#
# `pixel-agents login` is a removed command; its handler (`login_command` in
# `pixel_cli/auth.py`) prints a deprecation notice pointing at `pixel-agents auth` /
# `pixel-agents model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `pixel-agents --help`.
