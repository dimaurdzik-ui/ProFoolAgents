...
Regression guard for the bug where `pixel-agents model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`pixel_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in PIXEL_AGENTS_OVERLAYS
        overlay = PIXEL_AGENTS_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `pixel_cli/main.py` (cmd_model): read
