"""Regression tests for the OAuth dispatcher in pixel_cli.web_server.
...
from pixel_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Pixel-Agents-Session-Token": _SESSION_TOKEN}
...
def _fake_pixel_device_data():
    return {
        "device_code": "device-code",
        "user_code": "PIXEL-1234",
        "verification_uri": "https://portal.pixelagents.com/device",
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https://portal.pixelagents.com/oauth/device/code")
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "pixel_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "pixel_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "pixel_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from pixel_cli import web_server as ws
    ...
    def fake_status():
        observed_homes.append(get_pixel_agents_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "pixel-agents auth add fake-oauth",
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "pixel-agents auth" not in detail
    finally:
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `pixel-agents model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `pixel-agents model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Pixel Agents."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``pixel-agents auth remove
    xai-oauth``.
    """
    from pixel_cli import auth as auth_mod
    from pixel_cli import web_server as ws
    ...
    # Prior `pixel-agents auth remove xai-oauth` left the source suppressed.
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to pixel_cli.auth.get_auth_status (the
    ...
    with patch("pixel_cli.auth.get_auth_status", return_value=fake_status):
