"""Tests for IRC gateway configuration via `pixel-agents setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Pixel Agents install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Pixel Agents install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#pixel-agents")
            monkeypatch.setenv("IRC_NICKNAME", "pixel-agents-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `pixel-agents setup gateway` experience with IRC present."""
