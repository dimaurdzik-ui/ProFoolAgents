"""Tests for ``pixel-agents debug`` CLI command and debug utilities."""
...
@pytest.fixture
def pixel_home(tmp_path, monkeypatch):
    """Set up an isolated PIXEL_AGENTS_HOME with minimal logs."""
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO pixel_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from pixel_cli.debug import _upload_paste_rs
        ...
        with patch("pixel_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "pixel_cli.debug.urllib.request.urlopen",
...
class TestUploadToPastebin:
    ...
    def test_falls_back_to_dpaste_com(self):
        from pixel_cli.debug import upload_to_pastebin

        with patch("pixel_cli.debug._upload_paste_rs",
                    side_effect=Exception("down")), \
             patch("pixel_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_race_truncate_after_resolve_reports_empty(self, pixel_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = pixel_home / "logs" / "agent.log"
        from pixel_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, pixel_home):
        """When truncation lands on a line boundary, keep the first full line."""
        from pixel_cli.debug import _capture_log_snapshot
        ...
        (pixel_home / "logs" / "agent.log").write_text(line * num_lines)
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of PIXEL_AGENTS_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def pixel_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated PIXEL_AGENTS_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".pixel-agents"
        home.mkdir()
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(home))
        ...
        monkeypatch.delenv("PIXEL_AGENTS_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, pixel_home_with_secret):
        from pixel_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, pixel_home_with_secret):
        from pixel_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, pixel_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when PIXEL_AGENTS_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("PIXEL_AGENTS_REDACT_SECRETS", "false")

        from pixel_cli.debug import _capture_log_snapshot

        assert os.environ.get("PIXEL_AGENTS_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, pixel_home_with_secret
    ):
        from pixel_cli.debug import _capture_log_snapshot

        log_path = pixel_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, pixel_home_with_secret):
        from pixel_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from pixel_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, pixel_home):
        from pixel_cli.debug import collect_debug_report

        with patch("pixel_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- pixel-agents dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- pixel-agents dump ---" in report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, pixel_home, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from pixel_cli.debug import run_debug_share
        ...
        with patch("pixel_cli.dump.run_dump"), \
             patch("pixel_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("pixel_cli.debug.upload_to_pastebin",
    ...
    def test_share_uploads_five_pastes(self, pixel_home, capsys):
        ...
        with patch("pixel_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- pixel-agents dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, pixel_home_with_secret, capsys
    ):
        ...
        with patch("pixel_cli.dump.run_dump"), \
             patch("pixel_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("pixel_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "pixel-agents debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, pixel_home):
        from pixel_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from pixel_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from pixel_cli.debug import delete_paste
        ...
        with patch("pixel_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.pixel-agents/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchropixelly on the next ``pixel-agents debug``
    invocation.
    """


    def test_records_pending_to_json(self, pixel_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from pixel_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_dedupes_same_url(self, pixel_home):
        """Same URL recorded twice → one entry with the later expire_at."""
        from pixel_cli.debug import _schedule_auto_delete, _load_pending
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_deletes_expired_entries(self, pixel_home):
        from pixel_cli.debug import (
        ...
        with patch("pixel_cli.debug.delete_paste", side_effect=fake_delete):
    ...
    def test_sweep_leaves_future_entries_alone(self, pixel_home):
        from pixel_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("pixel_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, pixel_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, pixel_home):
        from pixel_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("pixel_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)
...
class TestRunDebugDelete:
    ...
    def test_handles_delete_failure(self, capsys):
        from pixel_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("pixel_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_shows_privacy_notice(self, pixel_home, capsys):
        from pixel_cli.debug import run_debug_share
        ...
        with patch("pixel_cli.dump.run_dump"), \
             patch("pixel_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("pixel_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
...
class TestBuildDebugShare:
    ...
    Backs both ``pixel-agents debug share`` (CLI) and ``POST /api/ops/debug-share``
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, pixel_home):
        from pixel_cli.debug import build_debug_share

        secret = "sk-proj-SUPERSECRETtoken1234567890"
        (pixel_home / "logs" / "agent.log").write_text(
        ...
        with patch("pixel_cli.dump.run_dump"), patch(
            "pixel_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("pixel_cli.debug._schedule_auto_delete"):
    ...
    def test_optional_log_failure_is_collected_not_raised(self, pixel_home):
        from pixel_cli.debug import build_debug_share
...
# ---------------------------------------------------------------------------
# Shared bundle collection + Pixel-S3 path
# ---------------------------------------------------------------------------

class TestCollectShareBundle:

    def test_no_redact_omits_banner(self, pixel_home):
        from pixel_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, pixel_home):
        from pixel_cli.debug import collect_share_bundle
...
class TestBuildPixelBundle:
    def test_envelope_shape_and_gzip(self, pixel_home):
        import gzip
        import json as _json

        from pixel_cli.debug import build_pixel_bundle

        files = {"report": "hello", "agent.log": "log line"}
        blob = build_pixel_bundle(files, redact=True)

        # It's gzip — magic bytes.
        assert blob[:2] == b"\x1f\x8b"
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["format"] == "pixel-agents-debug-share/1"
    ...
    def test_redacted_false_recorded(self):
        ...
        blob = build_pixel_bundle({"report": "x"}, redact=False)
        envelope = _json.loads(gzip.decompress(blob).decode())
        assert envelope["redacted"] is False


class TestRunDebugSharePixel:
    def _args(self, **over):
        class _A:
            lines = 50
            expire = 7
    ...
    def test_pixel_success_prints_view_url(self, pixel_home, capsys):
        from pixel_cli.debug import run_debug_share
        ...
        with patch("pixel_cli.dump.run_dump"), patch(
            "pixel_cli.diagnostics_upload.share_to_pixel", return_value=res
        ) as share:
            run_debug_share(self._args())

        out = capsys.readouterr().out
        assert "Pixel-INTERNAL" in out
        assert "https://support.example.com/diagnostics/id-1" in out
        assert "2026-06-20T00:00:00Z" in out
        # The blob passed to share_to_pixel must be gzip bytes.
        blob = share.call_args[0][0]
        assert isinstance(blob, (bytes, bytearray)) and blob[:2] == b"\x1f\x8b"

    def test_pixel_failure_suggests_local(self, pixel_home, capsys):
        from pixel_cli.debug import run_debug_share

        with patch("pixel_cli.dump.run_dump"), patch(
            "pixel_cli.diagnostics_upload.share_to_pixel",
        ...
        assert "Pixel upload failed" in err
        assert "--local" in err

    def test_pixel_does_not_touch_pastebin(self, pixel_home):
        from pixel_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("pixel_cli.dump.run_dump"), patch(
            "pixel_cli.diagnostics_upload.share_to_pixel", return_value=res
        ), patch("pixel_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
        paste.assert_not_called()


class TestDebugSlashCommand:
    """`/debug [pixel|local]` parsing in the CLI/TUI handler.

    The classic CLI and the TUI slash worker both dispatch through
    ``PixelAgentsCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("pixel_cli.debug.run_debug_share", _fake_run):
            self._handler()(cmd_original)
        return captured

    def test_bare_debug_defaults_to_paste(self):
        c = self._captured("/debug")
        assert c["pixel"] is False and c["local"] is False
    ...
    def test_word_parsing_is_case_insensitive(self):
        c = self._captured("/debug PIXEL")
        assert c["pixel"] is True
...
class TestShareConsentGate:
    """`pixel-agents debug share` requires explicit consent before uploading.
    ...
    def _args(self, **over):
        ...
        base = dict(lines=50, expire=7, local=False, pixel=False,
    ...
    def test_non_interactive_requires_yes(self, pixel_home, capsys, monkeypatch):
        ...
        with patch("pixel_cli.dump.run_dump"), \
             patch("pixel_cli.debug.upload_to_pastebin") as mock_upload:
            with pytest.raises(SystemExit) as exc:
                run_debug_share(self._args())
    ...
    def test_local_never_prompts(self, pixel_home, capsys, monkeypatch):
