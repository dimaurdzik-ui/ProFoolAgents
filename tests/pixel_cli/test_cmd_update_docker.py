"""Tests for ``pixel-agents update`` / ``--check`` inside the Docker container.
...
fix, ``pixel-agents update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Pixel Agents, not an update to the running container, so the message
...
from pixel_cli.main import _cmd_update_check, cmd_update
...
@patch("pixel_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``pixel-agents update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull pixelagents/pixel-agents:latest" in out
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull pixelagents/pixel-agents:latest" in msg
    ...
    assert "PIXEL_AGENTS_HOME" in msg or "/opt/data" in msg, (
