...
the SAME provider universe as ``pixel-agents model`` (the CLI/TUI picker).
...
def test_every_pixel_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `pixel-agents model` universe."""
    ...
    assert not missing, (
        "providers shown in `pixel-agents model` but not configurable in the desktop "
