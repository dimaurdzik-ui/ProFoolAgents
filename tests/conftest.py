"""Shared fixtures for the pixel-agents test suite.
...
2. **Isolated PIXEL_AGENTS_HOME.** PIXEL_AGENTS_HOME points to a per-test tempdir so
   code reading ``~/.pixel-agents/*`` via ``get_pixel_agents_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".pixel-agents"`` instead of the canonical
   ``get_pixel_agents_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No PIXEL_AGENTS_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox PIXEL_AGENTS_HOME before ANY test module is imported ──────────────────
# `pixel_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_pixel_agents_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.pixel-agents/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes PIXEL_AGENTS_HOME, but fixtures run
...
# the REAL Pixel Agents root — capture it BEFORE the sandbox rewires PIXEL_AGENTS_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.pixel-agents (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("PIXEL_AGENTS_KANBAN_HOME", "").strip()
_PRE_SANDBOX_PIXEL_AGENTS_HOME = os.environ.get("PIXEL_AGENTS_HOME", "")
if not os.environ.get("PIXEL_AGENTS_HOME"):
    _SESSION_PIXEL_AGENTS_HOME = tempfile.mkdtemp(prefix="pixel-agents-test-home-")
    os.environ["PIXEL_AGENTS_HOME"] = _SESSION_PIXEL_AGENTS_HOME
    atexit.register(shutil.rmtree, _SESSION_PIXEL_AGENTS_HOME, True)

#: PIXEL_AGENTS_HOME as it stood when conftest was imported - i.e. before any test
...
PIXEL_AGENTS_HOME_AT_CONFTEST_IMPORT = os.environ.get("PIXEL_AGENTS_HOME", "")
...
# PIXEL_AGENTS_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_PIXEL_AGENTS_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``pixel_cli.voice.speak_text`` — real synthesis,
    ...
    "PIXEL_AGENTS_VOICE",
    "PIXEL_AGENTS_VOICE_TTS",
    "PIXEL_AGENTS_YOLO_MODE",
    "PIXEL_AGENTS_INTERACTIVE",
    "PIXEL_AGENTS_QUIET",
    "PIXEL_AGENTS_TOOL_PROGRESS",
    "PIXEL_AGENTS_TOOL_PROGRESS_MODE",
    "PIXEL_AGENTS_MAX_ITERATIONS",
    "PIXEL_AGENTS_SESSION_PLATFORM",
    "PIXEL_AGENTS_SESSION_CHAT_ID",
    "PIXEL_AGENTS_SESSION_CHAT_NAME",
    "PIXEL_AGENTS_SESSION_CHAT_TYPE",
    "PIXEL_AGENTS_SESSION_THREAD_ID",
    "PIXEL_AGENTS_SESSION_SOURCE",
    "PIXEL_AGENTS_SESSION_KEY",
    "PIXEL_AGENTS_GATEWAY_SESSION",
    "PIXEL_AGENTS_CRON_SESSION",
    "_PIXEL_AGENTS_GATEWAY",
    "PIXEL_AGENTS_PLATFORM",
    "PIXEL_AGENTS_MODEL",
    "PIXEL_AGENTS_INFERENCE_MODEL",
    "PIXEL_AGENTS_INFERENCE_PROVIDER",
    "PIXEL_AGENTS_TUI_PROVIDER",
    "PIXEL_AGENTS_MANAGED",
    "PIXEL_AGENTS_MANAGED_DIR",
    "PIXEL_AGENTS_DEV",
    "PIXEL_AGENTS_CONTAINER",
    "PIXEL_AGENTS_EPHEMERAL_SYSTEM_PROMPT",
    "PIXEL_AGENTS_TIMEZONE",
    "PIXEL_AGENTS_REDACT_SECRETS",
    "PIXEL_AGENTS_BACKGROUND_NOTIFICATIONS",
    "PIXEL_AGENTS_EXEC_ASK",
    "PIXEL_AGENTS_HOME_MODE",
    "PIXEL_AGENTS_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.pixel-agents/kanban.db instead of the per-test PIXEL_AGENTS_HOME.
    "PIXEL_AGENTS_KANBAN_DB",
    "PIXEL_AGENTS_KANBAN_BOARD",
    "PIXEL_AGENTS_KANBAN_HOME",
    "PIXEL_AGENTS_KANBAN_WORKSPACES_ROOT",
    "PIXEL_AGENTS_KANBAN_LOGS_ROOT",
    "PIXEL_AGENTS_KANBAN_TASK",
    "PIXEL_AGENTS_KANBAN_WORKSPACE",
    "PIXEL_AGENTS_KANBAN_RUN_ID",
    "PIXEL_AGENTS_KANBAN_CLAIM_LOCK",
    "PIXEL_AGENTS_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "PIXEL_AGENTS_DELEGATED_CHILD_CONTEXT",
    "PIXEL_AGENTS_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "pixel-agents" host.
    "PIXEL_AGENTS_HONCHO_HOST",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `pixel` plugin auto-registers itself on plugin discovery,
    ...
    "PIXEL_AGENTS_DASHBOARD_OAUTH_CLIENT_ID",
    "PIXEL_AGENTS_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and PIXEL_AGENTS_HOME to per-test tempdirs so code that
    reads ``~/.pixel-agents/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral PIXEL_AGENTS_* vars that could change test semantics.
    for name in _PIXEL_AGENTS_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("PIXEL_AGENTS_HONCHO_HOST", "pixel-agents")

    # 3. Redirect PIXEL_AGENTS_HOME to a per-test tempdir. Code that reads
    #    ``~/.pixel-agents/*`` via ``get_pixel_agents_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.pixel-agents/*`` via
    #    ``Path.home() / ".pixel-agents"`` instead of ``get_pixel_agents_home()``
    #    is a bug to fix at the callsite.
    fake_pixel_home = tmp_path / "pixel_test"
    fake_pixel_home.mkdir()
    (fake_pixel_home / "sessions").mkdir()
    (fake_pixel_home / "cron").mkdir()
    (fake_pixel_home / "memories").mkdir()
    (fake_pixel_home / "skills").mkdir()
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(fake_pixel_home))

    # 3b. pixel_state computes ``DEFAULT_DB_PATH = get_pixel_agents_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from pixel_state import ...``) that
    ...
    pixel_state_mod = sys.modules.get("pixel_state")
    if pixel_state_mod is not None and hasattr(pixel_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            pixel_state_mod, "DEFAULT_DB_PATH", fake_pixel_home / "state.db"
        )
    ...
    monkeypatch.setenv("PIXEL_AGENTS_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.pixel-agents/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import pixel_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_pixel_home(_hermetic_environment):
...
# invocation), kanban writes silently pollute the real ~/.pixel-agents. This autouse
...
# allow-list because test-level fixtures legitimately move PIXEL_AGENTS_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session PIXEL_AGENTS_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``PIXEL_AGENTS_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Pixel Agents root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_PIXEL_AGENTS_HOME:
        # PIXEL_AGENTS_HOME was genuinely set before the sandbox — honor it via the
        # normal resolver (it may be a profile dir whose root matters).
        from pixel_constants import get_default_pixel_agents_root
        return get_default_pixel_agents_root().resolve()
    # No pre-existing PIXEL_AGENTS_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".pixel-agents").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.pixel-agents`` captured at import time. Hermetic tests that legitimately
    move PIXEL_AGENTS_HOME to sibling tempdirs are unaffected.

    Only patches when ``pixel_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("pixel_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import pixel_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.pixel-agents. See #69283."
        )
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Pixel Agents home override redirects every later
    # ``get_pixel_agents_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from pixel_constants import get_pixel_agents_home_override, set_pixel_agents_home_override

        if get_pixel_agents_home_override() is not None:
            set_pixel_agents_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal pixel-agents config dict suitable for unit tests."""
    return {
...
def _ensure_current_event_loop(request):
    ...
    Python 3.11+ no longer guarantees a current loop for plain synchropixel tests.
...
# environment and finds the developer's live ``pixel-agents-gateway`` process
...
# correlated with a ``tests/pixel_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> pixel-agents-gateway``
...
def _wal_is_usable() -> bool:
    """True when Pixel Agents will actually put a database into WAL mode here.

    Pixel Agents refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Pixel Agents can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Pixel Agents managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``pixel_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_pixel_agents_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_pixel_home`` fixture
    redirects ``PIXEL_AGENTS_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.pixel-agents/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``pixel_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["PIXEL_AGENTS_VOICE_TTS"] = "1"``. The
...
#      ``pixel_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``pixel_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``pixel_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``pixel_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Pixel Agents falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Pixel Agents uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart pixel-agents-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting pixel-agents/python patterns are also blocked.
    """
    ...
    def _matches_pixel_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _PIXEL_AGENTS_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_pixel_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch pixel-agents-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m pixel_cli.main".
                if (
                    "pixel-agents" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live pixel-agents-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting pixel-agents/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `pixel-agents update` (or the
        # equivalent `python -m pixel_cli.main update`).  These commands
        ...
        if "update" in low and (
            # pixel-agents update / pixel-agents update --gateway / setsid bash -c ... pixel-agents update
            ("pixel-agents" in low and "update" in low.split())
            or
            # python -m pixel_cli.main update --gateway
            ("pixel_cli" in low and "update" in low.split())
            or
            # venv/bin/pixel-agents update  (absolute path variant used in tests)
            (".venv/bin/pixel-agents" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`pixel-agents update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_PIXEL_AGENTS_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import pixel_cli.voice as _voice
    except Exception:
