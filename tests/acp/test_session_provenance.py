...
def test_root_session_no_compression(db):
    ...
    assert prov["currentPixelAgentsSessionId"] == "root1"
    assert prov["rootPixelAgentsSessionId"] == "root1"
    assert prov["parentPixelAgentsSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_pixel_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentPixelAgentsSessionId"] == "old"
    assert prov["rootPixelAgentsSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousPixelAgentsSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootPixelAgentsSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"pixel-agents"}
    assert "sessionProvenance" in meta["pixel-agents"]
    assert meta["pixel-agents"]["sessionProvenance"]["currentPixelAgentsSessionId"] == "root1"
