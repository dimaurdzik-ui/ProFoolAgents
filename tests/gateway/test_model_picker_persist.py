...
closure the PR changed, against a real temp ``PIXEL_AGENTS_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "pixel_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_pixel_agents_home() / config.yaml`` — point it here.
    monkeypatch.setattr("pixel_constants.get_pixel_agents_home", lambda: pixel_home)
    monkeypatch.setattr("pixel_cli.config.get_pixel_agents_home", lambda: pixel_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_pixel_home", default_home)
    _stub_picker_dependencies(monkeypatch)
