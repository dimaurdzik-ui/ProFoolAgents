...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_pixel_agents_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedDiscovery:
    def test_list_approved_scopes_platform_discovery_to_profile_dir(self, tmp_path):
        ...
        # A profile's store anchors to the pixel-agents ROOT, not the current
        # PIXEL_AGENTS_HOME — the current home may itself be a profile, and nesting
        # profiles inside profiles is how a `-p work` CLI and its gateway end
        # up reading different files. Patch that seam, not get_pixel_agents_home.
        with patch("gateway.pairing.PAIRING_DIR", global_dir), patch(
            "gateway.pairing.get_default_pixel_agents_root", return_value=home
        ):
            store = PairingStore(profile="alice")
            # Scoped under the mocked root's profile dir, using the same
            # consolidated layout a standalone `pixel-agents -p alice` resolves —
...
class TestProfileScopedStorage:
    ...
    under each profile's own Pixel Agents home so a multiplexing gateway can keep
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``pixel-agents pairing`` CLI continues to work without a profile context."""
        from pixel_constants import get_pixel_agents_home
        monkeypatch.setattr("pixel_constants.get_pixel_agents_home", lambda: tmp_path)
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        """Explicit profile stores use that profile's normal Pixel Agents layout."""
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path))
    ...
    def test_profile_store_matches_profile_cli_home(self, tmp_path, monkeypatch):
        """Gateway and ``pixel-agents -p`` must resolve the same pairing store."""
