...
class TestVoiceChannelCommands:
    ...
    async def test_join_success(self, runner):
        ...
        event.source.chat_name = "Pixel Agents Server / #general"
        runner.adapters[event.source.platform] = mock_adapter
    ...
    async def test_input_reuses_bound_source_metadata(self, runner):
        ...
        bound_source = SessionSource(
            chat_id="123",
            chat_name="Pixel Agents Server / #general",
            chat_type="group",
            user_id="user1",
            user_name="user1",
        ...
        assert event.source.chat_name == "Pixel Agents Server / #general"
...
class TestVoiceChannelAwareness:
    ...
    def _make_adapter(self):
        ...
        adapter._voice_receivers = {}
        adapter._client = MagicMock()
        adapter._client.user = SimpleNamespace(id=99999, name="PixelAgentsBot")
        return adapter
    ...
    def test_returns_info_with_members(self):
        ...
        bot_member = self._make_member(99999, "PixelAgentsBot", is_bot=True)
        ...
        assert "Bob" in names
        assert "PixelAgentsBot" not in names
