...
def test_session_store_default_db_uses_runtime_pixel_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime PIXEL_AGENTS_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    pixel_state before a fixture redirected PIXEL_AGENTS_HOME used to pin every
    default SessionDB() at the developer's real ~/.pixel-agents/state.db.
    """
    config = GatewayConfig(default_reset_policy=SessionResetPolicy(mode="none"))
    fake_home = tmp_path / "alt_pixel_home"
    fake_home.mkdir()
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "pixel-agents sessions list" in raw["_README"]
