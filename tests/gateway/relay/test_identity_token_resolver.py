"""Unit tests for the generic-OIDC / Pixel-Portal caller-identity token resolver.

Covers gateway.relay._resolve_relay_identity_token() — the canonical resolver
shared by the runtime self-provision path and the `pixel-agents gateway enroll` CLI.
...
Two modes:
  ...
  2. Pixel Portal (resolve_pixel_access_token) otherwise — the default.

The HTTP POST and the Pixel resolver are monkeypatched; these prove the mode
