...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Pixel Agents"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^pixel-agents"]}
        )
        assert adapter._message_matches_mention_patterns("pixel-agents please help") is True
        assert adapter._message_matches_mention_patterns("please pixel-agents help") is False
