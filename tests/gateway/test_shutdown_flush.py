...
def test_get_flush_dir_uses_get_pixel_agents_home(tmp_path, monkeypatch):
    """Flush dir must use get_pixel_agents_home(), not hardcoded Path.home()."""
    import gateway.shutdown_flush as mod

    captured = {}

    def fake_get_pixel_agents_home():
        from pathlib import Path
        captured["called"] = True
        return tmp_path

    monkeypatch.setattr(
        "pixel_constants.get_pixel_agents_home", fake_get_pixel_agents_home
    )
