    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        cfg = PlatformConfig(
            ...
            extra={
                ...
                "port": 6697,
                "nickname": "pixel-agents",
                "channel": "#pixel-agents-dev",
                "use_tls": True,
            },
        )
        ...
        assert adapter.nickname == "pixel-agents"
        assert adapter.channel == "#pixel-agents-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_addressed_channel_message(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG #test :pixel-agents: hello there")
    ...
    async def test_ctcp_action_converted(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG pixel-agents :\x01ACTION waves\x01")
        assert len(dispatched) == 1
    ...
    async def test_unauthorized_user_blocked(self, monkeypatch):
        ...
        adapter = IRCAdapter(cfg)
        adapter._current_nick = "pixel-agents"
        ...
        await adapter._handle_line(":eve!u@host PRIVMSG #test :pixel-agents: hello")
        assert len(dispatched) == 0
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_completes_handshake_and_sends_privmsg(self, monkeypatch):
        ...
        monkeypatch.setenv("IRC_NICKNAME", "pixel-agentsbot")
        monkeypatch.setenv("IRC_USE_TLS", "false")

        # Server greets us with 001 RPL_WELCOME, then nothing for QUIT drain.
        conn = _FakeIRCConnection([b":server 001 pixel-agentsbot-cron :Welcome"])
        ...
        assert any(line.startswith("NICK pixel-agentsbot-cron") for line in sent_lines)
        assert any(line.startswith("USER pixel-agentsbot-cron 0 * :Pixel Agents (cron)")
