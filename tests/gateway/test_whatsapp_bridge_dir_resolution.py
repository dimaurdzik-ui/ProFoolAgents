...
(/opt/pixel-agents/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable PIXEL_AGENTS_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_pixel_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable PIXEL_AGENTS_HOME."""
    ...
    monkeypatch.setattr(
        "pixel_constants.get_pixel_agents_home", lambda: pixel_home
    )
    ...
    expected = pixel_home / "scripts" / "whatsapp-bridge"
