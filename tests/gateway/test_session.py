...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_pixel_agents_home(self):
        config = GatewayConfig()
        source = SessionSource(
            platform=Platform.LOCAL, chat_id="cli",
            chat_name="CLI terminal", chat_type="dm",
        ...
        with patch("pixel_constants.display_pixel_agents_home", return_value="~/.pixel-agents/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.pixel-agents/profiles/coder/cron/output/" in prompt
...
class TestSessionStoreRewriteTranscript:
    ...
    def store(self, tmp_path, monkeypatch):
        import pixel_state
        monkeypatch.setattr(pixel_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
        config = GatewayConfig()
...
class TestWhatsAppSessionKeyConsistency:
    ...
    def test_whatsapp_group_participant_aliases_share_session_key(self, tmp_path, monkeypatch):
        ...
        per-user sessions."""
        tmp_home = tmp_path / "pixel-agents-home"
        ...
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_home))
