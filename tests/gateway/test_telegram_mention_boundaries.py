...
substrings like 'foo@pixel_bot.example'.
...
def _make_adapter():
    ...
    adapter._bot = SimpleNamespace(id=999, username="pixel_bot")
    return adapter
...
class TestSubstringFalsePositivesAreRejected:
    ...
    def test_email_like_substring(self):
        """bug #12545 exact repro: 'foo@pixel_bot.example'."""
        adapter = _make_adapter()
        msg = _message(text="email me at foo@pixel_bot.example")
    ...
    def test_substring_inside_url_without_entity(self):
        ...
        adapter = _make_adapter()
        msg = _message(text="see https://example.com/@pixel_bot for details")
        assert adapter._message_mentions_bot(msg) is False


    def test_email_substring_in_caption(self):
        adapter = _make_adapter()
        msg = _message(caption="foo@pixel_bot.example")
        assert adapter._message_mentions_bot(msg) is False
...
class TestEntityEdgeCases:
    ...
    def test_malformed_entity_with_negative_offset(self):
        adapter = _make_adapter()
        msg = _message(text="@pixel_bot hi",
...
class TestCaseInsensitivity:
    ...
    def test_uppercase_mention(self):
        adapter = _make_adapter()
        text = "hi @PIXEL_AGENTS_BOT"
        msg = _message(text=text, entities=[_mention_entity(text, mention="@PIXEL_AGENTS_BOT")])
        assert adapter._message_mentions_bot(msg) is True
