...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.pixel-agents."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /pixel-agents, /btw, /stop,
        ...
        for expected in ("/pixel-agents", "/btw", "/stop", "/model", "/help"):
...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``pixel-agents gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "pixel_feedback" in created_apps[0].registered_actions
        assert "pixel_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("pixel_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("pixel_clarify_choice")
            for pattern in clarify_choice_patterns
        )
...
class TestSlashCommands:
    ...
    # instead of as /pixel-agents subcommands. This is the Discord/Telegram parity
    ...
    @pytest.mark.asyncio
    async def test_legacy_pixel_prefix_still_works(self, adapter):
        """Backward compat: /pixel-agents btw foo must still route to /btw foo.

        Old workspace manifests only declared /pixel-agents as the single slash.
...
class TestMissingCredentials:
    ...
    async def test_missing_app_token_sets_fatal_error(self):
        ...
        assert "pixel-agents gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestTrackingStructureBounds:
    ...
    async def test_slash_command_contexts_bounded(self, adapter):
        ...
        adapter.handle_pixel_command = AsyncMock(return_value=None)
        for i in range(10):
            command = {
                "command": "/pixel-agents",
                "text": "/status",
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/pixel-agents-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        assert msg_event.media_urls == ["/tmp/pixel-agents-cached.png"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_PIXEL_AGENTS_SLACK_USER_AGENT_PREFIX`` on every
    ...
    def test_pixel_slack_user_agent_prefix_format(self):
        """Module constant matches the PixelAgentsAgent/<version> convention used
        elsewhere in the codebase for platform-partner attribution."""
        assert _slack_mod._PIXEL_AGENTS_SLACK_USER_AGENT_PREFIX.startswith("PixelAgentsAgent/")
