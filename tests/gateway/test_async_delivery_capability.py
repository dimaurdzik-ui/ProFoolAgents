...
class TestAsyncDeliverySupported:
    ...
    def test_set_false_is_unsupported(self):
        ...
        try:
            ...
            assert get_session_env("PIXEL_AGENTS_SESSION_PLATFORM") == "api_server"
        finally:
...
class TestDeclareStatelessChannel:
    """``pixel-agents -z`` and cron cannot receive a completion after their turn ends.

    Cron clears the ``PIXEL_AGENTS_SESSION_*`` routing keys, so an async delegation's
