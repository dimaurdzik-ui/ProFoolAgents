...
Guards against the 60-vs-500 bug where a stale `.env PIXEL_AGENTS_MAX_ITERATIONS=60`
...
def _run_gateway_import(pixel_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
    ...
    script = textwrap.dedent(
        ...
        for k in (
            ...
            "PIXEL_AGENTS_AGENT_TIMEOUT",
            "PIXEL_AGENTS_AGENT_TIMEOUT_WARNING",
            "PIXEL_AGENTS_GATEWAY_BUSY_INPUT_MODE",
            "PIXEL_AGENTS_GATEWAY_BUSY_TEXT_MODE",
...
@pytest.fixture
def pixel_home(tmp_path: Path) -> Path:
    home = tmp_path / ".pixel-agents"
    home.mkdir()
    return home


def test_config_gateway_timeout_wins_over_stale_env(pixel_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(pixel_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
    })
    _write_env(pixel_home, {
        "PIXEL_AGENTS_AGENT_TIMEOUT": "60",
        "PIXEL_AGENTS_AGENT_TIMEOUT_WARNING": "30",
    })

    env = _run_gateway_import(pixel_home, initial_env={})

    assert env.get("PIXEL_AGENTS_AGENT_TIMEOUT") == "1800"
    assert env.get("PIXEL_AGENTS_AGENT_TIMEOUT_WARNING") == "900"


def test_config_platform_connect_timeout_supplies_env_when_unset(pixel_home: Path) -> None:
    ...
    _write_config(pixel_home, gateway_cfg={"platform_connect_timeout": 90})

    env = _run_gateway_import(pixel_home, initial_env={})

    assert env.get("PIXEL_AGENTS_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "90"


def test_env_platform_connect_timeout_wins_over_config(pixel_home: Path) -> None:
    """Unlike the agent.*/display.*/timezone bridges (config-authoritative),
    PIXEL_AGENTS_GATEWAY_PLATFORM_CONNECT_TIMEOUT is the manual-override escape hatch:
    ...
    env = _run_gateway_import(
        pixel_home,
        initial_env={"PIXEL_AGENTS_GATEWAY_PLATFORM_CONNECT_TIMEOUT": "120"},
    )

    assert env.get("PIXEL_AGENTS_GATEWAY_PLATFORM_CONNECT_TIMEOUT") == "120"
