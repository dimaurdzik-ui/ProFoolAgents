...
def test_publish_marks_evidence_upload_failure_in_pr_comment(tmp_path, monkeypatch):
    comment = {
        "id": 123,
        "body": "before\n<!-- pixel-agents-e2e-evidence:start -->\npending\n<!-- pixel-agents-e2e-evidence:end -->\nafter",
    }
    updates = []
    ...
    assert updates == [
        (
            "https://api.github.com/repos/PixelResearch/pixel-agents/issues/comments/123",
            "github-token",
            "PATCH",
            {
...
def test_find_review_comment_requires_the_evidence_marker():
    pending = "<!-- pixel-agents-ci-review-bot -->\n<!-- pixel-agents-e2e-evidence:start -->\npending\n<!-- pixel-agents-e2e-evidence:end -->"

    assert _mod._find_review_comment([{"body": "<!-- pixel-agents-ci-review-bot --> no evidence"}]) is None
