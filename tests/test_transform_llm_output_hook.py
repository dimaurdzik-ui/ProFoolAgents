...
from pixel_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(pixel_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <pixel_home>/plugins/<name> and opt it in."""
    plugin_dir = pixel_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / "pixel_empty"))
