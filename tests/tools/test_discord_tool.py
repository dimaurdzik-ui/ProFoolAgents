...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in pixel-agents-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_only_in_pixel_discord_toolset(self):
        from toolsets import TOOLSETS, _PIXEL_AGENTS_CORE_TOOLS
        assert "discord" in TOOLSETS["pixel-agents-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["pixel-agents-discord"]["tools"]
        assert "discord" not in _PIXEL_AGENTS_CORE_TOOLS
        assert "discord_admin" not in _PIXEL_AGENTS_CORE_TOOLS

    def test_discord_tools_not_in_other_toolsets(self):
        from toolsets import TOOLSETS
        for name, ts in TOOLSETS.items():
            if name in {"pixel-agents-discord", "pixel-agents-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["pixel-agents-discord"], quiet_mode=True,
