...
def test_output_path_rejects_pixel_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    pixel_home = tmp_path / "pixel-agents-home"
    pixel_home.mkdir()
    monkeypatch.setattr(file_safety, "_pixel_home_path", lambda: pixel_home)
    monkeypatch.setattr(file_safety, "_pixel_root_path", lambda: pixel_home)

    target = pixel_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    pixel_home = tmp_path / "pixel-agents-home"
    token_dir = pixel_home / "mcp-tokens"
