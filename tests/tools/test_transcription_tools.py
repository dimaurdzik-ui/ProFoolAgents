...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("PIXEL_AGENTS_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("PIXEL_AGENTS_LOCAL_STT_LANGUAGE", raising=False)
...
class TestExplicitProviderRespected:
    ...
    def test_explicit_local_uses_local_command_fallback(self, monkeypatch):
        ...
        monkeypatch.setenv(
            "PIXEL_AGENTS_LOCAL_STT_COMMAND",
...
class TestTranscribeLocalCommand:
    def test_command_provider_uses_sanitized_child_env(self, monkeypatch):
        """Salvage of #56332: command STT must not inherit Pixel Agents secrets."""
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("PIXEL_AGENTS_LOCAL_STT_LANGUAGE", "en")
...
def mock_xai_http_module():
    ...
    fake_module = MagicMock()
    fake_module.pixel_xai_user_agent = MagicMock(return_value="pixel-agents-xai/test")
...
class TestTranscribeXAI:
    ...
    def test_sends_language_and_format(self, monkeypatch, sample_ogg, mock_xai_http_module):
        ...
        monkeypatch.setenv("PIXEL_AGENTS_LOCAL_STT_LANGUAGE", "fr")
