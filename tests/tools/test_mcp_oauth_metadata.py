...
Covers:
- :class:`PixelAgentsTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.PixelAgentsMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _PIXEL_AGENTS_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# PixelAgentsTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = PixelAgentsTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path))
        storage = PixelAgentsTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (PixelAgentsMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: PixelAgentsTokenStorage, **context_attrs):
    ...
    provider = _PIXEL_AGENTS_PROVIDER_CLS.__new__(_PIXEL_AGENTS_PROVIDER_CLS)
    provider._pixel_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path))
        storage = PixelAgentsTokenStorage("mgr-srv")
        ...
        with patch.object(
            _PIXEL_AGENTS_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path))
        storage = PixelAgentsTokenStorage("flow-srv")
        ...
        with patch.object(
            _PIXEL_AGENTS_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
