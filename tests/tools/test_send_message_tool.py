...
class TestSendMessageTool:
    ...
    def test_ntfy_topic_target_bypasses_channel_directory(self):
        ...
        ntfy_cfg = SimpleNamespace(enabled=True, token=None, extra={"topic": "pixel-agents-in"})
...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        assert _call.args[2] == "hello from pixel-agents"
...
class TestParseTargetRef:
    ...
    def test_explicit_targets_round_trip_chat_and_thread(self):
        cases = [
            ...
            ("matrix", "@pixel-agents:matrix.org", "@pixel-agents:matrix.org", None),
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``pixel-agents cron``
    runs separately from ``pixel-agents gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``PIXEL_AGENTS_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``PIXEL_AGENTS_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``PIXEL_AGENTS_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
