...
class TestGptQualityPinnedToMedium:
    ...
    and cannot be overridden via config. Pinning keeps Pixel Portal billing
...
class TestManagedGatewayErrorTranslation:
    """4xx from the Pixel managed gateway should be translated to a user-actionable message."""

    def test_4xx_translates_to_value_error_with_remediation(self, image_tool, monkeypatch):
        """403 from managed gateway → ValueError mentioning FAL_KEY + pixel-agents tools."""
        ...
        managed_gateway.pixel_user_token = "test-token"
        ...
        assert "pixel-agents tools" in msg
...
class TestManagedKreaRouting:
    ...
    def test_routes_native_krea_model_to_krea_plugin_in_managed_mode(
        ...
        monkeypatch.setattr(
            "pixel_cli.plugins._ensure_plugins_discovered", lambda *a, **k: None
        )
