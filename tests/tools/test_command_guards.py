...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects PIXEL_AGENTS_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("PIXEL_AGENTS_INTERACTIVE", "PIXEL_AGENTS_GATEWAY_SESSION", "PIXEL_AGENTS_EXEC_ASK", "PIXEL_AGENTS_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["PIXEL_AGENTS_INTERACTIVE"] = "1"
        session_key = os.getenv("PIXEL_AGENTS_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No PIXEL_AGENTS_INTERACTIVE or PIXEL_AGENTS_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        def notify(data):
            ...
            # The notify fires synchropixelly before _await_gateway_decision
