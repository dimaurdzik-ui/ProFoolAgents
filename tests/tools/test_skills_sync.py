...
class TestRmtreeWritableScopeGuard:
    ...
    exception — the result is a silent ``shutil.rmtree(~/.pixel-agents/)``
    ...
    def test_refuses_anything_that_is_not_a_strict_child_of_skills(self, tmp_path):
        """``/``, ``~/.pixel-agents`` itself, a sibling dir, and the skills root
        ...
        pixel-agents = tmp_path / "home"
        pixel-agents.mkdir()
        skills = pixel-agents / "skills"
        (skills / "keep").mkdir(parents=True)
        sibling = pixel-agents / "kanban.db"  # any non-skills path
        sibling.mkdir()

        with patch("tools.skills_sync.SKILLS_DIR", skills):
            for target in (Path("/"), pixel-agents, sibling, skills):
...
class TestGetBundledDir:
    def test_env_var_override_with_default_fallback(self, tmp_path, monkeypatch):
        ...
        custom_dir.mkdir()
        monkeypatch.setenv("PIXEL_AGENTS_BUNDLED_SKILLS", str(custom_dir))
        assert _get_bundled_dir() == custom_dir

        # Empty or unset falls back to the relative path from __file__.
        monkeypatch.setenv("PIXEL_AGENTS_BUNDLED_SKILLS", "")
        assert _get_bundled_dir().name == "skills"
        monkeypatch.delenv("PIXEL_AGENTS_BUNDLED_SKILLS", raising=False)
        assert _get_bundled_dir().name == "skills"
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `pixel-agents profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.pixel-agents) rely on so bundled
    skills are never seeded at install time NOR re-injected by `pixel-agents update`.
    """
    ...
    def test_marker_skips_sync_and_removal_seeds_normally(self, tmp_path):
        ...
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        pixel_home = tmp_path / "home"
        pixel_home.mkdir()
        marker = pixel_home / ".no-bundled-skills"
        ...
        def _patches():
            ...
            stack.enter_context(patch("tools.skills_sync.SKILLS_DIR", skills_dir))
            stack.enter_context(patch("tools.skills_sync.MANIFEST_FILE", manifest_file))
            stack.enter_context(patch("tools.skills_sync.PIXEL_AGENTS_HOME", pixel_home))
            return stack
...
class TestOptOutToggleAndRemove:
    """`pixel-agents skills opt-out/opt-in` core: marker toggle + safe removal."""
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("tools.skills_sync.PIXEL_AGENTS_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync.PIXEL_AGENTS_HOME", home):
