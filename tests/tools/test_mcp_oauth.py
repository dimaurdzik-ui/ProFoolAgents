...
class TestPixelAgentsTokenStorage:
    def test_roundtrip_tokens(self, tmp_path, monkeypatch):
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path))
        storage = PixelAgentsTokenStorage("test-server")
    ...
    def test_token_file_created_with_0o600(self, tmp_path, monkeypatch):
        ...
        """
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path))
        storage = PixelAgentsTokenStorage("perm-test-server")
    ...
    def test_corrupt_tokens_returns_none(self, tmp_path, monkeypatch):
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path))
        storage = PixelAgentsTokenStorage("bad-server")
...
class TestPathTraversal:
    ...
    def test_dots_and_slashes_sanitized(self, tmp_path, monkeypatch):
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path))
        storage = PixelAgentsTokenStorage("../../../etc/passwd")
...
def test_build_oauth_auth_preserves_server_url_path():
    ...
    ``OAuthContext.get_authorization_base_url``; Pixel Agents must not pre-strip.
    """
    ...
    with patch.object(mcp_oauth, "_OAUTH_AVAILABLE", True), \
         ...
         patch.object(mcp_oauth, "PixelAgentsTokenStorage") as mock_storage_cls:
