...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        pixel_home.mkdir()
        (pixel_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"PIXEL_AGENTS_HOME": str(pixel_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(pixel_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.pixel-agents/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir()
        (pixel_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.pixel-agents/skills"


    def test_custom_container_base(self, tmp_path):
        pixel_home = tmp_path / ".pixel-agents"
        (pixel_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"PIXEL_AGENTS_HOME": str(pixel_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.pixel-agents")

        assert mounts[0]["container_path"] == "/home/user/.pixel-agents/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.pixel-agents/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.pixel-agents/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape PIXEL_AGENTS_HOME."""
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / ".pixel-agents"))
        (tmp_path / ".pixel-agents").mkdir()

        # Create a sensitive file one level above pixel_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside pixel_home
    ...
    def test_nested_subdir_inside_pixel_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of PIXEL_AGENTS_HOME must be allowed."""
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir()
        subdir = pixel_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside PIXEL_AGENTS_HOME pointing outside must be rejected."""
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir()
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))

        # Create a sensitive file outside pixel_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside pixel_home pointing outside
        symlink = pixel_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes PIXEL_AGENTS_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, pixel_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape PIXEL_AGENTS_HOME."""
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir()
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))
        ...
        sensitive.write_text("{}")
        self._write_config(pixel_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(pixel_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside PIXEL_AGENTS_HOME via config must still mount."""
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir()
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))

        (pixel_home / "oauth.json").write_text("{}")
        self._write_config(pixel_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir()
        (pixel_home / "cache" / "documents").mkdir(parents=True)
        (pixel_home / "cache" / "audio").mkdir(parents=True)
        (pixel_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.pixel-agents/cache/documents" in paths
        assert "/root/.pixel-agents/cache/audio" in paths
        assert "/root/.pixel-agents/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_pixel_agents_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir()
        # Use legacy dir name with content — get_pixel_agents_dir prefers
        # populated old over new.
        legacy_doc = pixel_home / "document_cache"
        legacy_img = pixel_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(pixel_home / "document_cache") in host_paths
        assert str(pixel_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.pixel-agents/cache/documents" in container_paths
        assert "/root/.pixel-agents/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        pixel_home = tmp_path / ".pixel-agents"
        img_dir = pixel_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.pixel-agents/cache/images/generated.png"
        )
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(pixel_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        pixel_home = tmp_path / ".pixel-agents"
        doc_dir = pixel_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — PIXEL_AGENTS_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping PIXEL_AGENTS_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"PIXEL_AGENTS_HOME": str(home)}):
    ...
    def test_per_service_token_still_mounts(self, tmp_path):
        ...
        assert [m["container_path"] for m in mounts] == [
            "/root/.pixel-agents/google_token.json"
        ]
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.pixel-agents/google_token.json" in paths
        assert "/root/.pixel-agents/.env" not in paths
