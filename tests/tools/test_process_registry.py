...
from tools.environments.local import _PIXEL_AGENTS_PROVIDER_ENV_FORCE_PREFIX
...
class TestOrphanedPipeReconciliation:
    ...
    `pixel-agents update` in Feishu spawned a background subprocess that restarted
...
class TestSpawnEnvSanitization:
    def test_spawn_local_strips_blocked_vars_from_background_env(self, registry):
        ...
        with patch.dict(os.environ, {
            ...
            registry.spawn_local(
                ...
                env_vars={
                    ...
                    f"{_PIXEL_AGENTS_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN": "forced-bot-token",
                },
            )
        ...
        assert f"{_PIXEL_AGENTS_PROVIDER_ENV_FORCE_PREFIX}TELEGRAM_BOT_TOKEN" not in env
    ...
    def test_env_poller_quotes_temp_paths_with_spaces(self, registry):
        ...
        with patch("tools.process_registry.time.sleep", return_value=None), \
            ...
            registry._env_poller_loop(
                session,
                env,
                "/path with spaces/pixel_bg.log",
                "/path with spaces/pixel_bg.pid",
                "/path with spaces/pixel_bg.exit",
            )

        assert env.commands[0][0] == "cat '/path with spaces/pixel_bg.log' 2>/dev/null"
        assert env.commands[1][0] == "kill -0 \"$(cat '/path with spaces/pixel_bg.pid' 2>/dev/null)\" 2>/dev/null; echo $?"
        assert env.commands[2][0] == "cat '/path with spaces/pixel_bg.exit' 2>/dev/null"
...
class TestSigkillEscalation:
    ...
    def test_entire_tree_is_sigkilled_not_just_parent(self, monkeypatch):
        ...
        try:
            ...
            # _terminate_host_pid SIGKILLs synchropixelly before returning, so
