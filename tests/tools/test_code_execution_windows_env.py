...
``pixel_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``PIXEL_AGENTS_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # PIXEL_AGENTS_* handling (#27303): only the operational allowlist passes;
        # every other PIXEL_AGENTS_* is dropped (the broad prefix was removed).
        "PIXEL_AGENTS_HOME": "/home/alice/.pixel-agents",        # allowlisted → kept
        "PIXEL_AGENTS_PROFILE": "default",                 # allowlisted → kept
        "PIXEL_AGENTS_INTERACTIVE": "1",                   # not allowlisted → dropped
        "PIXEL_AGENTS_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "PIXEL_AGENTS_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``pixel_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``pixel_tools.py`` and ``script.py`` writes in
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        """
        from tools.code_execution_tool import generate_pixel_tools_module
        import tempfile, ast
        stub = generate_pixel_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_pixel_tools_module(["terminal"], transport="uds")
...
# let the child import pixel_tools, a user script that printed non-ASCII
