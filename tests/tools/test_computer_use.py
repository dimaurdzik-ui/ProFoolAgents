...
def _reset_backend():
    ...
    with patch.dict(os.environ, {"PIXEL_AGENTS_COMPUTER_USE_BACKEND": "noop"}, clear=False):
        yield
    reset_backend_for_tests()
...
class TestRegistration:
    ...
    def test_cua_driver_cmd_env_override_is_resolved_dynamically(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PIXEL_AGENTS_CUA_DRIVER_CMD", str(driver))
...
class TestCuaEnvironmentScrubbing:
    ...
    def test_cua_session_sanitizes_provider_env_vars(self):
        ...
        The cua-driver MCP subprocess should not inherit Pixel Agents-managed
...
class TestClickButtonPassthrough:
    """Surface 5 (PixelResearch/pixel-agents#47072) — `middle_click` must
...
class TestImageMimeTypePropagation:
    """Surface 7 (PixelResearch/pixel-agents#47072): trycua/cua#1961 made
...
class TestMcpInvocationResolution:
    """Surface 8 (PixelResearch/pixel-agents#47072): instead of hardcoding
    ...
    the MCP subcommand doesn't require a Pixel Agents patch.
    ...
    def test_falls_back_when_manifest_missing_command(self):
        ...
        resolved driver path (so PIXEL_AGENTS_CUA_DRIVER_CMD still wins)."""
        from unittest.mock import patch
        from tools.computer_use.cua_backend import _resolve_mcp_invocation
...
class TestStructuredElementsConsumption:
    """Surface 2 (PixelResearch/pixel-agents#47072): trycua/cua#1961 made
...
class TestCapabilityDiscovery:
    """Surface 4 (PixelResearch/pixel-agents#47072): the wrapper learns
...
class TestElementTokenAttachment:
    """Surface 6 (PixelResearch/pixel-agents#47072): trycua/cua#1961 added
...
class TestSessionLifecycle:
    """Surface gap (audit June 2026): Pixel Agents never declared a cua-driver
