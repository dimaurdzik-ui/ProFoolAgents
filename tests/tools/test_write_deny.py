...
class TestWriteDenyExactPaths:
    ...
    def test_pixel_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.pixel-agents/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "pixel_root"
        ...
        # Sanity check: PIXEL_AGENTS_HOME does point to the profile dir, not the root.
        from pixel_constants import get_pixel_agents_home, get_default_pixel_agents_root
        assert get_pixel_agents_home() == profile_home
        assert get_default_pixel_agents_root() == root
...
class TestWriteAllowed:
    ...
    def test_pixel_control_files_requested_writable(self):
