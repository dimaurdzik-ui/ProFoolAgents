...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        self._managed_patchers = [
            patch("tools.web_tools.managed_pixel_tools_enabled", return_value=True),
            patch("tools.managed_tool_gateway.managed_pixel_tools_enabled", return_value=True),
        ]
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("tools.web_tools.Firecrawl"):
            with patch("tools.web_tools._read_pixel_access_token", return_value=None):
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "pixelagents.com"}):
            with patch("tools.web_tools._read_pixel_access_token", return_value="pixel-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    from tools.web_tools import _get_firecrawl_client
                    result = _get_firecrawl_client()
                    mock_fc.assert_called_once_with(
                        api_key="pixel-token",
                        api_url="https://firecrawl-gateway.pixelagents.com",
                    )
...
class TestBackendSelection:
    ...
    ``pixel-agents tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def test_managed_gateway_does_not_preempt_explicit_tavily(self):
        """Regression: a Pixel OAuth token (managed gateway "ready") must NOT
        beat an explicitly configured TAVILY_API_KEY in the fallback path.
        Free Pixel tiers don't include web search, so the user's deliberate
    ...
    def test_managed_gateway_only_falls_through_to_firecrawl(self):
        """When no explicit-credential backend is configured, a Pixel-managed
...
class TestCheckWebApiKey:
    ...
    def test_configured_firecrawl_backend_accepts_managed_gateway(self):
        with patch("tools.web_tools._load_web_config", return_value={"backend": "firecrawl"}):
            with patch("tools.web_tools._peek_pixel_access_token", return_value="pixel-token"):
...
class TestNonBuiltinProviderAvailability:
    ...
    def test_check_web_api_key_returns_true_for_custom_provider(self):
        ...
        with patch("tools.web_tools._ddgs_package_importable", return_value=False), \
             patch("tools.web_tools._peek_pixel_access_token", return_value=None):
            from tools.web_tools import check_web_api_key
            assert check_web_api_key() is True
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from pixel_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.pixel-agents/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.pixel-agents/.env), not bare os.getenv. Parametrized over the four
