...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("pixel_cli.config.load_config", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("PIXEL_AGENTS_SESSION_KEY", session_key)
        monkeypatch.setenv("PIXEL_AGENTS_EXEC_ASK", "1")
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("pixel-agents-verify-", "pixel-agents-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "pixel-agents-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/pixel-agents-verify-example.py",
            "rm -f /tmp/pixel-agents-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../pixel-agents-verify-example.py",
            "rm -f /tmp/a/../../tmp/pixel-agents-verify-example.py",
            "rm -f /var/tmp/pixel-agents-verify-example.py",
            "rm -f /tmp/pixel-agents-verify-*",
            "rm -f /tmp/pixel-agents-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/pixel-agents-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/pixel-agents-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\pixel-agents-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\pixel-agents-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\pixel-agents-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"PIXEL_AGENTS_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.pixel-agents/.env",
            "echo x | tee $PIXEL_AGENTS_HOME/.env",
            'echo x | tee "$PIXEL_AGENTS_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestPixelAgentsConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.pixel-agents/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.pixel-agents/config.yaml",
            "echo '  mode: off' >> ~/.pixel-agents/config.yaml",
            "echo x | tee ~/.pixel-agents/config.yaml",
            "echo x | tee $PIXEL_AGENTS_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.pixel-agents/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Pixel Agents absolute config.yaml is
        # handled by the project patterns, not the Pixel Agents-home rule.
        for cmd in (
            "cat ~/.pixel-agents/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.pixel-agents/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Pixel Agents-home prefixes must fold to ~/ and
    ~/.pixel-agents/ in dangerous-command detection.
    ...
    SSH, and Pixel Agents config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_pixel_agents_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/PIXEL_AGENTS_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("PIXEL_AGENTS_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.pixel-agents/pixel-agents && source venv/bin/activate && python -m pixel_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m pixel_cli.main gateway run --replace &",
            "nohup python -m pixel_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart pixel-agents-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Pixel Agents security config via IFS.
            "sed${IFS}-i ~/.pixel-agents/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep pixel-agents) bypasses the pkill/killall name-matching
    ...
    def test_kill_pgrep_expansion_detected(self):
        for cmd in (
            'kill -9 $(pgrep -f "pixel-agents.*gateway")',
            "kill -9 `pgrep pixel-agents`",
            "kill $(pgrep gateway)",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof pixel-agents)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof pixel_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof pixel-agents`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Pixel Agents service
    label achieves the same effect as `pixel-agents gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_pixel_label_detected(self):
        for cmd in (
            "launchctl stop ai.pixel-agents.gateway",
            "launchctl kickstart -k system/ai.pixel-agents.gateway",
            "launchctl bootout system/ai.pixel-agents.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.pixel-agents.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Pixel Agents labels, are
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("PIXEL_AGENTS_GATEWAY_SESSION", "PIXEL_AGENTS_CRON_SESSION",
                      "PIXEL_AGENTS_YOLO_MODE",
                      "PIXEL_AGENTS_SESSION_KEY", "PIXEL_AGENTS_INTERACTIVE")
        }
        os.environ.pop("PIXEL_AGENTS_YOLO_MODE", None)
        os.environ.pop("PIXEL_AGENTS_INTERACTIVE", None)
        # PIXEL_AGENTS_CRON_SESSION takes priority over PIXEL_AGENTS_GATEWAY_SESSION in
        ...
        os.environ.pop("PIXEL_AGENTS_CRON_SESSION", None)
        os.environ["PIXEL_AGENTS_GATEWAY_SESSION"] = "1"
        os.environ["PIXEL_AGENTS_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("pixel_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"PIXEL_AGENTS_INTERACTIVE": "1"}, clear=False):
