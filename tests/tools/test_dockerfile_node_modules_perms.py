...
Older images made /opt/pixel-agents/ui-tui and /opt/pixel-agents/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV PIXEL_AGENTS_TUI_DIR=/opt/pixel-agents/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R pixel-agents:pixel-agents /opt/pixel-agents/ui-tui" not in text
    assert "chown -R pixel-agents:pixel-agents /opt/pixel-agents/node_modules" not in text
