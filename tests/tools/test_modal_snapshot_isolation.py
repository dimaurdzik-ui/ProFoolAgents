...
def _restore_tool_modules():
    original_pixel_home = os.environ.get("PIXEL_AGENTS_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_pixel_home is None:
            os.environ.pop("PIXEL_AGENTS_HOME", None)
        else:
            os.environ["PIXEL_AGENTS_HOME"] = original_pixel_home
        _reset_modules(("tools", "pixel_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    pixel_cli = types.ModuleType("pixel_cli")
    pixel_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["pixel_cli"] = pixel_cli
    pixel_home = tmp_path / "pixel-agents-home"
    os.environ["PIXEL_AGENTS_HOME"] = str(pixel_home)
    sys.modules["pixel_cli.config"] = types.SimpleNamespace(
        get_pixel_agents_home=lambda: pixel_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="pixel-agents", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": pixel_home / "modal_snapshots.json",
