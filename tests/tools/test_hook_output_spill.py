...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("pixel_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="pixel-agents-spill-test-")
    ...
    def test_default_directory_uses_pixel_home(self):
        """When no directory override, spill under PIXEL_AGENTS_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="pixel-agents-home-")
        try:
            with patch.dict(os.environ, {"PIXEL_AGENTS_HOME": test_home}):
                # Also patch get_pixel_agents_home to the env var to mirror production.
            ...
            # ~/.pixel-agents/hook_outputs depending on get_pixel_agents_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.pixel-agents/hook_outputs/sess")),
            ]
