...
def server():
    ...
    with patch.dict(
        "sys.modules",
        {
            "pixel_constants": MagicMock(
                get_pixel_agents_home=MagicMock(return_value="/tmp/pixel_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'pixel-agents-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'pixel-agents-release' patched"
    }
