...
"No LLM provider configured. Run `pixel-agents model`..." (resume failed) or
...
def _make_agent_with_override(override, monkeypatch, config, model_cfg=None):
    ...
    # Keep credential-pool resolution off the developer's real PIXEL_AGENTS home.
...
# provider (e.g. Pixel) but who switched THIS session to a self-hosted model
...
# found" (the b200/pixel-agents-ultra-sft report). The stored MODEL NAME is the one
...
ULTRA_CONFIG = {
    ...
    "model": {"default": "some-pixel-model", "provider": "pixel"},
    "providers": {
        "pixel-agents-ultra": {
            "api": ULTRA_URL,
            "api_key": "sk-ultra",
            "models": ["pixel-agents-ultra-sft"],
        }
    },
}
...
ULTRA_LEGACY_CONFIG = {
    ...
    "custom_providers": [
        {
            "name": "pixel-agents-ultra",
            "base_url": ULTRA_URL,
            "api_key": "sk-ultra",
            "model": "pixel-agents-ultra-sft",
        }
    ],
}
...
class TestModelNameRecoversEntryIdentity:
    def test_identity_by_model_from_providers_dict_models_list(self, monkeypatch):
        ...
        assert (
            rp.find_custom_provider_identity_by_model("pixel-agents-ultra-sft")
            == "custom:pixel-agents-ultra"
        )
