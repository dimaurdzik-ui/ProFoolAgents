...
def server():
    ...
    # e.g. pixel_cli.active_sessions would bind the mocked get_pixel_agents_home
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `PIXEL_AGENTS_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("PIXEL_AGENTS_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("PIXEL_AGENTS_TUI_GATEWAY_NO_FLUSH", raising=False)
    ...
    resp = server.handle_request(
        {
            ...
            # eager_build: exercise the synchropixel build path (this test
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/pixel-agents-dev": {"name": "pixel-agents-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "pixel-agents-dev", "session_id": sid},
        })
...
def test_dispatch_runs_short_handlers_inline(server):
    """Non-long handlers return their response synchropixelly from dispatch()."""
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `pixel-agents config set` does)
    ...
    import pixel_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_pixel_agents_home", lambda: tmp_path)
    ...
    # Activate midnight, as `pixel-agents config set display.skin midnight` would.
