...
def pixel_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves PIXEL_AGENTS_HOME.
...
@pytest.fixture()
def server(pixel_home, monkeypatch):
    ...
    # Pin config resolution to the isolated PIXEL_AGENTS_HOME. Sibling test
    ...
    # module-level ``_pixel_home = get_pixel_agents_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_pixel_home", pixel_home)
...
def test_moa_bare_returns_usage(server, session, pixel_home):
    _write_moa_config(pixel_home, """
moa:
