...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"pixel-agents[termux]"' in text
    assert '"pixel-agents[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"pixel-agents[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
