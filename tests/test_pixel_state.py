"""Tests for pixel_state.py — SessionDB SQLite CRUD, FTS5 search, export."""
...
import pixel_state
from pixel_state import SCHEMA_SQL, SCHEMA_VERSION, SessionDB
...
class TestSessionLifecycle:
    ...
    def test_cjk_search_falls_back_to_like_when_trigram_unavailable(
        ...
        monkeypatch.setattr("pixel_state.sqlite3.connect", connect_without_trigram)
...
class TestSchemaInit:
    def test_wal_mode(self, db):
        ...
        from pixel_state import is_sqlite_wal_reset_vulnerable
    ...
    def test_schema_sql_is_source_of_truth(self, db):
        ...
        """
        from pixel_state import SCHEMA_SQL
...
class TestFTS5ToolCallMigration:
    ...
    def test_v10_to_v11_upgrade_backfills_tool_fields(self, tmp_path):
        ...
        try:
            ...
            # `pixel-agents db optimize` performs the v23 transition; afterwards the
            ...
            from pixel_state import SCHEMA_VERSION
...
class TestApplyWalProbe:
    ...
    def _assume_fixed_sqlite(self, monkeypatch):
        ...
        monkeypatch.setattr(
            pixel_state, "is_sqlite_wal_reset_vulnerable", lambda version_info=None: False
        )
...
def test_refresh_compression_lock_requires_holder_and_preserves_reclaimability(db, monkeypatch):
    ...
    monkeypatch.setattr(pixel_state.time, "time", lambda: 1000.0)
    ...
    monkeypatch.setattr(pixel_state.time, "time", lambda: 1005.0)
    ...
    monkeypatch.setattr(pixel_state.time, "time", lambda: 1016.0)
...
def test_refresh_cannot_resurrect_a_lock_already_reclaimed(db, monkeypatch):
    ...
    monkeypatch.setattr(pixel_state.time, "time", lambda: 1020.0)
    assert db.try_acquire_compression_lock("s1", "holder-b", ttl_seconds=10.0) is True
...
class TestGatewayRoutingPkHeal:
    ...
    def test_legacy_pk_rebuilt_to_composite(self, tmp_path):
        db_path = self._make_legacy_db(
            tmp_path, rows=[("/home/u/.pixel-agents/sessions", "agent:main:telegram:dm:1", "{}", 1.0)]
        )
        db = SessionDB(db_path=db_path)
        try:
            assert self._pk_cols(db) == ["scope", "session_key"]
            # Existing rows survive the rebuild.
            entries = db.load_gateway_routing_entries(scope="/home/u/.pixel-agents/sessions")
...
class TestApplyDatabasePragmas:
    ...
    def test_honors_wal_autocheckpoint_from_config(self, tmp_path, monkeypatch):
        import sqlite3
        from pixel_state import apply_database_pragmas
