...
leaked shell variable, so ``scripts/run_tests.sh``'s ``env -i`` was no help:
  ...
  1. A test drives the ``voice.toggle`` RPC with ``action="tts"``. The
     ...
     ``os.environ["PIXEL_AGENTS_VOICE_TTS"] = "1"``.
  ...
  3. Any later test that drives a turn to completion hits the TTS dispatch in
     ``prompt.submit``, which calls ``pixel_cli.voice.speak_text`` on a
...
Two independent defences in ``tests/conftest.py``, one test each below:
  ...
  * ``_PIXEL_AGENTS_BEHAVIORAL_VARS`` now blanks ``PIXEL_AGENTS_VOICE`` /
    ``PIXEL_AGENTS_VOICE_TTS`` at every test setup, so step 2 cannot cross a test
    boundary.
...
def test_voice_toggle_still_leaks_the_env_var_but_speech_is_stubbed(monkeypatch):
    ...
    assert os.environ.get("PIXEL_AGENTS_VOICE_TTS") == "1"
    ...
    # calls it (a late `from pixel_cli.voice import speak_text`), with the
...
def test_voice_env_does_not_leak_into_the_next_test():
    ...
    one above, in the same process, which left ``PIXEL_AGENTS_VOICE_TTS=1`` set.
    Before ``PIXEL_AGENTS_VOICE``/``PIXEL_AGENTS_VOICE_TTS`` were added to
    ``_PIXEL_AGENTS_BEHAVIORAL_VARS``, this assertion failed.
    """
    assert "PIXEL_AGENTS_VOICE_TTS" not in os.environ
    assert "PIXEL_AGENTS_VOICE" not in os.environ
...
def test_guard_can_be_opted_out_of_explicitly():
    ...
    import pixel_cli.voice as voice
