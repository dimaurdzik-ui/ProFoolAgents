...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.pixel-agents/pixel-agents/utils.py)
...
2. ``pixel-agents update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``pixel_cli/main.py`` (the ``pixel-agents update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``pixel_constants`` after the
