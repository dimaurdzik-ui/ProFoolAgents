"""`setup_path()` must also install a `pixel-agents-acp` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving a command
name against the login-shell PATH. The `pixel-agents-acp` console script generated by
the packaging metadata lives inside the install's venv, which is not on that
PATH, so those hosts report Pixel Agents as not installed. `setup_path()` therefore
writes a `pixel-agents-acp` launcher next to the `pixel-agents` one.
...
ACP_BLOCK = re.compile(
    r'(    rm -f "\$command_link_dir/pixel-agents-acp".*?'
    r'log_success "Installed pixel-agents-acp launcher[^\n]*\n)',
    re.S,
)
...
def _extract_acp_shim_block() -> str:
    ...
    assert match, (
        "could not locate the pixel-agents-acp launcher block in scripts/install.sh — "
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    pixel_bin = tmp_path / "venv" / "bin" / "python"
    pixel_bin.parent.mkdir(parents=True)
    pixel_bin.write_text("#!/bin/sh\n", encoding="utf-8")
    entrypoint = tmp_path / "pixel-agents"
    entrypoint.write_text("#!/bin/sh\n", encoding="utf-8")

    script = (
        "set -e\n"
        f"PIXEL_AGENTS_BIN={pixel_bin}\n"
        f"PIXEL_AGENTS_ENTRYPOINT={entrypoint}\n"
    ...
    return command_link_dir / "pixel-agents-acp"
...
def test_acp_launcher_does_not_follow_a_symlink_into_the_venv(tmp_path):
    ...
    An older install could leave `pixel-agents-acp` as a symlink to the venv console
    ...
    shim_path = command_link_dir / "pixel-agents-acp"
    ...
    assert console_script.read_text(encoding="utf-8") == marker, (
        "venv/bin/pixel-agents-acp was overwritten — symlink-stomp regression (#21454)"
    )
    assert not shim_path.is_symlink(), (
        "command_link_dir/pixel-agents-acp must be replaced with a regular file"
    )
