...
def session_db(tmp_path):
    ...
    os.environ["PIXEL_AGENTS_HOME"] = str(tmp_path / ".pixel-agents")
    os.makedirs(tmp_path / ".pixel-agents", exist_ok=True)
    from pixel_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".pixel-agents" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal PixelAgentsCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        PixelAgentsCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        PixelAgentsCLI._handle_branch_command(cli_instance, "/branch refactor approach")
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from pixel_cli.commands import COMMAND_REGISTRY
