...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("pixel-agents/pixel-agents-")
...
class TestWorktreeCleanup:
    ...
    def test_cleanup_nonexistent_worktree(self, git_repo):
        ...
        info = {
            ...
            "branch": "pixel-agents/nonexistent",
...
class TestStaleWorktreePruning:
    ...
    def test_prunes_old_clean_worktree(self, git_repo):
        ...
        for entry in worktrees_dir.iterdir():
            if not entry.is_dir() or not entry.name.startswith("pixel-agents-"):
                continue
            try:
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned pixel-agents/* and pr-* branches."""

    def test_prunes_orphaned_pixel_branch(self, git_repo):
        """pixel-agents/pixel-agents-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "pixel-agents/pixel-agents-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "pixel-agents/pixel-agents-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "pixel-agents/pixel-agents-deadbeef" in result.stdout
        ...
        assert "pixel-agents/pixel-agents-deadbeef" in orphaned
        ...
        assert "pixel-agents/pixel-agents-deadbeef" not in result.stdout
...
class TestWorktreeLockReaping:
    ...
    def _mk(cli, repo, name, pid=None, dirty=False, unpushed=False, age_h=100):
        ...
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", f"pixel-agents/{name}", "HEAD"],
            cwd=repo, capture_output=True,
        )
        if pid is not None:
            subprocess.run(
                ["git", "worktree", "lock", "--reason", f"pixel-agents pid={pid}", str(p)],
                cwd=repo, capture_output=True,
            )
        if unpushed:
    ...
    def test_live_locked_survives_at_any_age(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "pixel-agents-live", pid=os.getpid())
    ...
    def test_dead_locked_clean_is_reaped(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "pixel-agents-dead", pid=999999)
    ...
    def test_dead_locked_dirty_survives(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "pixel-agents-deaddirty", pid=999999, dirty=True)
        cli._prune_stale_worktrees(str(git_repo))
    ...
    def test_dead_locked_unpushed_survives(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "pixel-agents-deadunp", pid=999999, unpushed=True)
        cli._prune_stale_worktrees(str(git_repo))
    ...
    def test_unlocked_clean_stale_is_reaped(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "pixel-agents-nolock", pid=None)
        cli._prune_stale_worktrees(str(git_repo))
        assert not wt.exists(), "clean unlocked stale worktree should be reaped"

    def test_dirty_survives_over_72h(self, git_repo):
        import cli
        wt = self._mk(cli, git_repo, "pixel-agents-dirty72", pid=None, dirty=True, age_h=100)
        cli._prune_stale_worktrees(str(git_repo))
...
class TestWorktreeLockPredicate:
    ...
    def test_unlocked_returns_none(self, git_repo):
        import cli
        p = git_repo / ".worktrees" / "pixel-agents-x"
        (git_repo / ".worktrees").mkdir(exist_ok=True)
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", "pixel-agents/pixel-agents-x", "HEAD"],
    ...
    def test_foreign_lock_reason_returns_dead(self, git_repo):
        import cli
        p = self._mk_locked(git_repo, "pixel-agents-foreign", "some other tool")
...
class TestWidenedPruner:
    ...
    Previously only ``pixel-agents-*`` directories were considered, so salvage/
    ...
    # -- named (non pixel-agents-*) directories are now covered ------------------
    ...
    def test_squash_merged_tree_is_reaped(self, git_repo):
        import cli
        wt, sha = self._mk(git_repo, "pixel-agents-merged", commit=True, age_h=100)
    ...
    def test_merged_predicate_fails_safe_without_upstream(self, git_repo_no_remote):
        ...
        p = repo / ".worktrees" / "pixel-agents-noremote"
        (repo / ".worktrees").mkdir(exist_ok=True)
        subprocess.run(
...
class TestMergeVerdictCache:
    ...
    dominates ``pixel-agents -w`` startup (~0.2-1.0s per worktree, re-run on every
    ...
    def test_cache_hit_matches_uncached_verdict(self, git_repo):
        ...
        import cli
        wt, sha = self._mk(git_repo, "pixel-agents-cachehit", commit=True)
        self._merge_upstream(git_repo, sha)
    ...
    def test_new_commit_invalidates_cached_verdict(self, git_repo):
        ...
        """
        import cli
        wt, sha = self._mk(git_repo, "pixel-agents-moves", commit=True)
        self._merge_upstream(git_repo, sha)
...
class TestPruneParallelEquivalence:
    ...
    def _board(self, git_repo, tag=""):
        ...
        names = {}
        for i in range(3):
            n = f"pixel-agents-merged{tag}{i}"
            wt, sha = self._mk(git_repo, n, commit=True)
            self._merge_upstream(git_repo, sha)
            names[n] = wt
        for i in range(3):
            n = f"pixel-agents-unpushed{tag}{i}"
            wt, _ = self._mk(git_repo, n, commit=True)
            names[n] = wt
        for i in range(2):
            n = f"pixel-agents-dirty{tag}{i}"
            wt, _ = self._mk(git_repo, n, dirty=True)
            names[n] = wt
        n = f"pixel-agents-fresh{tag}"
    ...
    def _kinds(survivors):
        ...
        for n in survivors:
            for kind in ("merged", "unpushed", "dirty", "fresh"):
                if n.startswith(f"pixel-agents-{kind}"):
                    out.add(kind)
        return out
    ...
    def test_pool_failure_falls_back_to_serial(self, git_repo, monkeypatch):
        ...
        wt, sha = self._mk(git_repo, "pixel-agents-poolfail", commit=True)
        self._merge_upstream(git_repo, sha)
