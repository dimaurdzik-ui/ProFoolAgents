...
minimal ``PixelAgentsCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def pixel_home(tmp_path, monkeypatch):
    """Isolated PIXEL_AGENTS_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".pixel-agents"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves PIXEL_AGENTS_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal PixelAgentsCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, pixel_home):
        ...
        with patch("pixel_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, pixel_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, pixel_home):
        ...
        src = inspect.getsource(PixelAgentsCLI.chat)
