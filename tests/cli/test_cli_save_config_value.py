...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_pixel_agents_home(), so
        # point PIXEL_AGENTS_HOME at the temp dir (the _pixel_home import-time
        # constant is no longer consulted).
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))
        monkeypatch.setattr("cli._pixel_home", pixel_home)
        return config_path
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "pixel_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in PIXEL_AGENTS_HOME/config.yaml
    ...
    install whose PIXEL_AGENTS_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read PIXEL_AGENTS_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh PIXEL_AGENTS_HOME with NO config.yaml (managed/desktop first launch).
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir()
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))
    ...
    def test_does_not_write_repo_cli_config(self, tmp_path, monkeypatch):
        ...
        result = yaml.safe_load((pixel_home / "config.yaml").read_text())
        assert result["wake_word"]["enabled"] is True
