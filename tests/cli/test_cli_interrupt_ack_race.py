...
Root cause: ``PixelAgentsCLI.chat()`` fires ``agent.interrupt(msg)`` from its
...
def _make_cli():
    """Build a PixelAgentsCLI with prompt_toolkit stubbed (same pattern as
    ...
    with patch.dict(sys.modules, prompt_toolkit_stubs), patch.dict(
        ...
        with patch.object(_cli_mod, "get_tool_definitions", return_value=[]), patch.dict(
            ...
            return _cli_mod.PixelAgentsCLI()
