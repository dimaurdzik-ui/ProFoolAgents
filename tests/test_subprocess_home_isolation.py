...
Pixel Agents state stays profile-scoped through PIXEL_AGENTS_HOME. Host subprocesses should
...
See: https://github.com/PixelResearch/pixel-agents/issues/25114
See: https://github.com/PixelResearch/pixel-agents/issues/36144
See: https://github.com/PixelResearch/pixel-agents/issues/29015
"""
...
class TestGetSubprocessHome:
    """Unit tests for pixel_constants.get_subprocess_home()."""

    def _host_mode(self, monkeypatch):
        monkeypatch.setattr(pixel_constants, "is_container", lambda: False)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("PIXEL_AGENTS_REAL_HOME", raising=False)

    def _container_mode(self, monkeypatch):
        monkeypatch.setattr(pixel_constants, "is_container", lambda: True)
        monkeypatch.delenv("TERMINAL_HOME_MODE", raising=False)
        monkeypatch.delenv("PIXEL_AGENTS_REAL_HOME", raising=False)
    ...
    def test_host_auto_keeps_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        pixel_home = real_home / ".pixel-agents" / "profiles" / "coder"
        profile_home = pixel_home / "home"
    ...
    def test_real_mode_repairs_parent_home_already_pointing_at_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PIXEL_AGENTS_REAL_HOME", str(real_home))

        from pixel_constants import get_subprocess_home, get_real_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        self._container_mode(monkeypatch)
        base = tmp_path / ".pixel-agents" / "profiles"
        ...
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_host_auto_preserves_real_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["PIXEL_AGENTS_REAL_HOME"] == str(real_home)
    ...
    def test_profile_mode_injects_profile_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(pixel_home / "home")
        assert result["PIXEL_AGENTS_REAL_HOME"] == str(real_home)
    ...
    def test_no_injection_when_pixel_home_unset(self, monkeypatch):
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from pixel_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from pixel_cli.profiles import create_profile
