...
class TestOpenVikingSummaryUriNormalization:
    def test_normalize_summary_uri_maps_pseudo_files_to_parent_directory(self):
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/pixel-agents/.overview.md") == "viking://user/pixel-agents"
        ...
        assert OpenVikingMemoryProvider._normalize_summary_uri("viking://user/pixel-agents/memories/profile.md") == "viking://user/pixel-agents/memories/profile.md"
...
class TestOpenVikingSkillQuerySafety:
    ...
    def test_skill_markers_match_pixel_scaffolding(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PIXEL_AGENTS_BUNDLES_DIR", str(bundles_dir))
...
class TestOpenVikingRead:
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    (("uri", "viking://user/pixel-agents"),),
                ): {"result": {"content": "overview text"}},
            }
        )

        result = json.loads(provider._tool_read({"uri": "viking://user/pixel-agents/.overview.md", "level": "overview"}))

        assert result["uri"] == "viking://user/pixel-agents/.overview.md"
        assert result["resolved_uri"] == "viking://user/pixel-agents"
    ...
    def test_read_accepts_uri_batch_and_caps_batch_full_content(self):
        provider = OpenVikingMemoryProvider()
        uris = [
            "viking://user/pixel-agents/memories/a.md",
            "viking://user/pixel-agents/memories/b.md",
            "viking://user/pixel-agents/memories/c.md",
            "viking://user/pixel-agents/memories/d.md",
        ]
        provider._client = FakeVikingClient(
            {
                (
    ...
    def test_summary_uri_error_does_not_fallback_and_raises(self):
        ...
        try:
            provider._tool_read({"uri": "viking://user/pixel-agents/.overview.md", "level": "overview"})
            assert False, "Expected summary endpoint error to be raised"
        except RuntimeError:
            pass

        assert provider._client.calls == [
            ("/api/v1/content/overview", {"uri": "viking://user/pixel-agents"}),
        ]
...
class TestOpenVikingAutoRecallPrefetch:
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/peers/pixel-agents/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        monkeypatch.setenv("OPENVIKING_AGENT", "pixel-agents")
        ...
        assert records["reads"] == ["viking://user/peers/pixel-agents/memories/e2e-full.md"]
        ...
        assert all(headers.get("x-openviking-actor-peer") == "pixel-agents" for headers in normalized_headers)
...
class TestOpenVikingBrowse:
    def test_list_browse_unwraps_and_normalizes_entry_shapes(self):
        ...
        provider._client = FakeVikingClient(
            {
                (
                    ...
                    "result": {
                        "entries": [
                            {"name": "memories", "uri": "viking://user/pixel-agents/memories", "type": "dir"},
                            {"rel_path": "profile.md", "uri": "viking://user/pixel-agents/memories/profile.md", "isDir": False, "abstract": "Profile"},
                        ]
                    }
                },
            }
        ...
        result = json.loads(provider._tool_browse({"action": "list", "path": "viking://user/pixel-agents"}))

        assert result["path"] == "viking://user/pixel-agents"
        assert result["entries"] == [
            {"name": "memories", "uri": "viking://user/pixel-agents/memories", "type": "dir", "abstract": ""},
            {"name": "profile.md", "uri": "viking://user/pixel-agents/memories/profile.md", "type": "file", "abstract": "Profile"},
        ]
        assert provider._client.calls == [(
            "/api/v1/fs/ls",
            {"uri": "viking://user/pixel-agents"},
...
class TestEnsureClientReloadsEnv:
    ...
    def test_ensure_client_rebuilds_when_api_key_changes(self, monkeypatch):
        ...
        class _StubClient:
            def __init__(self, endpoint, api_key, account="", user="", agent="pixel-agents"):
    ...
    def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        ...
        assert instances[1].posts[0][1]["uri"].startswith(
            "viking://user/peers/pixel-agents/memories/"
        )
