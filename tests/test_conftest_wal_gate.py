"""The conftest WAL gate must agree with pixel_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``pixel_state``. That is deliberate: importing
``pixel_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.pixel-agents`` before the per-test ``PIXEL_AGENTS_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_pixel_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and pixel_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_pixel_state_at_collection():
    """The gate must stay import-free of pixel_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.pixel-agents, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``pixel_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "pixel_state" or name.startswith("pixel_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported pixel_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.pixel-agents during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("pixel_state", None)
