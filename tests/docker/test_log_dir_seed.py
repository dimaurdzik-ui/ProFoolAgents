"""Runtime smoke test for Docker $PIXEL_AGENTS_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the pixel-agents user after container boot.
...
registered later runs its log service as the dropped pixel-agents user and
...
def test_logs_gateways_seeded_and_pixel_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by pixel-agents after boot."""
    ...
    # Both must be owned by pixel-agents
    r = docker_exec_sh(
        container_name,
    ...
    assert "logs=pixel-agents" in r.stdout, (
        f"logs/ not owned by pixel-agents: {r.stdout}"
    )
    assert "gateways=pixel-agents" in r.stdout, (
        f"logs/gateways/ not owned by pixel-agents: {r.stdout}"
    )
