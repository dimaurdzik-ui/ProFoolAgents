...
Build the real image and verify the actual runtime behavior:
  ...
  3. --user <pixel-agents-uid> (10000) is allowed (supported non-root start)
"""
...
def test_arbitrary_user_uid_rejected(
    ...
    assert "PIXEL_AGENTS_UID" in combined or "PUID" in combined, (
...
def test_user_pinned_to_pixel_uid_works(
    built_image: str,
) -> None:
    """docker run --user 10000:10000 (the pixel-agents UID) must be allowed.
    ...
    assert r.returncode == 0, (
        f"--user 10000:10000 (pixel-agents UID) was rejected: {r.stderr[-500:]}"
    )
    assert "OK" in r.stdout