"""Harness: dashboard opt-in via PIXEL_AGENTS_DASHBOARD.

Today (tini): dashboard starts once when PIXEL_AGENTS_DASHBOARD=1; if it crashes
...
def test_dashboard_not_running_by_default(
    ...
    """Without PIXEL_AGENTS_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "pixel-agents dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without PIXEL_AGENTS_DASHBOARD"
    )
...
# whenever `PIXEL_AGENTS_DASHBOARD_HOST` was non-loopback, silently disabling
...
"""
    ...
    probe = (
        "/opt/pixel-agents/.venv/bin/python - <<'PY'\n"
        f"{py_program}"
        "PY"
    )
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    when a ``DashboardAuthProvider`` is registered (the bundled pixel
    provider activates whenever ``PIXEL_AGENTS_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    1. ``/api/auth/providers`` (publicly reachable through the gate so
       the login page can bootstrap) returns 200 with ``pixel`` in the
    ...
    start_container(
        ...
        "PIXEL_AGENTS_DASHBOARD_HOST=0.0.0.0",
        "PIXEL_AGENTS_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "pixel" in provider_names, (
        "Bundled dashboard_auth/pixel provider should register when "
        f"PIXEL_AGENTS_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and PIXEL_AGENTS_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``PIXEL_AGENTS_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
    ...
    start_container(
        ...
        "PIXEL_AGENTS_DASHBOARD=1",
        "PIXEL_AGENTS_DASHBOARD_HOST=0.0.0.0",
        "PIXEL_AGENTS_DASHBOARD_INSECURE=1",
        cmd="sleep 120",
    )
