...
Override the image with ``PIXEL_AGENTS_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("PIXEL_AGENTS_TEST_IMAGE", "pixel-agents-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``PIXEL_AGENTS_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("PIXEL_AGENTS_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged pixel-agents user
# ---------------------------------------------------------------------------
#
# Background: every Pixel Agents runtime path inside the container drops to UID
# 10000 (the ``pixel-agents`` user) via ``s6-setuidgid pixel-agents``. ``docker exec``
...
# fails (PermissionError swallowed) for pixel-agents, so a test that ran as root
...
# helpers below run every probe under ``-u pixel-agents`` unless a specific
...
def docker_exec(
    container: str,
    *args: str,
    user: str = "pixel-agents",
    timeout: int = 30,
    extra_docker_args: tuple[str, ...] = (),
) -> subprocess.CompletedProcess[str]:
    """Run a command inside ``container`` as ``user`` (default: pixel-agents).
