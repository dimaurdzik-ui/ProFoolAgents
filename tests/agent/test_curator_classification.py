...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"pixel-agents-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "pixel-agents-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "pixel-agents-dev"
...
# `pixel-agents curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `pixel-agents curator pin <umbrella>` in the rename block so
