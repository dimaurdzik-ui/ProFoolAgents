...
def test_upload_happy_path_mocked(monkeypatch):
    ...
    assert "huggingface.co/datasets/alice/pixel-agents-traces" in msg

    # Created private dataset repo
    fake_api.create_repo.assert_called_once()
    _, kwargs = fake_api.create_repo.call_args
    assert kwargs["repo_id"] == "alice/pixel-agents-traces"
    ...
    assert ukwargs["repo_id"] == "alice/pixel-agents-traces"
