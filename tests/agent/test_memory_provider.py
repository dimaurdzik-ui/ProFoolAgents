...
class TestUserInstalledProviderDiscovery:
    """Memory providers installed to $PIXEL_AGENTS_HOME/plugins/ should be found.
    ...
    def test_load_user_plugin(self, tmp_path, monkeypatch):
        """load_memory_provider() can load from $PIXEL_AGENTS_HOME/plugins/."""
        from plugins.memory import load_memory_provider
...
class TestUserInstalledProviderCli:
    ...
    ``_pixel_user_memory.<name>.cli`` without registering the parent
...
class TestSetupFieldFiltering:
    ...
    memory setup wizard in pixel_cli/memory_setup.py.
...
class TestOnMemoryWriteBridge:
    ...
    def test_memory_manager_tool_injection_deduplicates(self):
        ...
        on providers that enforce unique names (e.g. Xiaomi MiMo via Pixel Portal).

        Regression test for: duplicate mnemosyne_recall / mnemosyne_remember /
        mnemosyne_stats in tools array → 400 from Pixel Portal.
        """
        mgr = MemoryManager()
...
class TestMemoryToolToolsetGate:
    ...
    def test_composite_toolset_with_memory_injects(self):
        ...
        tools, names = self._run_memory_injection(["pixel-agents-acp"], mgr)
        assert "hindsight_recall" in names
        assert any(t["function"]["name"] == "hindsight_recall" for t in tools)

    @pytest.mark.parametrize("enabled_toolsets", [None, ["memory"], ["all"], ["pixel-agents-acp"]])
