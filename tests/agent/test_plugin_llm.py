...
""",
        ...
        from pixel_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir()
        (pixel_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))
        from pixel_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from pixel_cli.plugins import PluginContext, PluginManifest, PluginManager
...
class TestHookMode:
    ...
    def test_complete_works_from_post_tool_call_hook_when_async_caller_set(self):
        """Hooks fired synchropixelly should still work with sync
