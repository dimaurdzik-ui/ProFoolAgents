...
@pytest.fixture
def pixel_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.pixel-agents/`` with a config.yaml referencing one external dir."""
...
def test_cache_invalidates_on_mtime_change(pixel_home_with_config):
    """A config.yaml edit invalidates the cache on the next call."""
    _home, external, config = pixel_home_with_config
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different PIXEL_AGENTS_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".pixel-agents"
    ...
    home_b = tmp_path / "home_b" / ".pixel-agents"
    ...
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(home_b))
