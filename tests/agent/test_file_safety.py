...
class TestCacheFileReadBlocking:
    """Internal Pixel Agents cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        pixel_home = tmp_path / ".pixel-agents"
        cache = pixel_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._pixel_home_path", return_value=pixel_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Pixel Agents cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        pixel_home = tmp_path / ".pixel-agents"
        hub = pixel_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_pixel_home(self, tmp_path):
        """The env basename guard does not depend on PIXEL_AGENTS_HOME resolution."""
        pixel_home = tmp_path / ".pixel-agents"
        pixel_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        pixel_home = tmp_path / ".pixel-agents"
        cache = pixel_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
