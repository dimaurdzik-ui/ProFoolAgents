...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate PIXEL_AGENTS_HOME under tmp_path."""
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / ".pixel-agents"))
    (tmp_path / ".pixel-agents").mkdir()

    # Force the constants/image cache helpers to re-read PIXEL_AGENTS_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("pixel_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_pixel_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under PIXEL_AGENTS_HOME — gateway cleanup
