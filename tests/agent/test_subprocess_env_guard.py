...
``pixel_subprocess_env`` / ``_sanitize_subprocess_env``, which the factory
...
``PIXEL_AGENTS_HOME`` or secret-scrub propagation.
...
SCAN_DIRS = ("agent", "pixel_cli", "tools", "gateway", "cron", "tui_gateway")
SCAN_ROOT_FILES = ("cli.py", "pixel_constants.py")
...
ALLOWED_RAW_SPAWN_ENV_FILES = {
    # THE owner module: _sanitize_subprocess_env / pixel_subprocess_env /
