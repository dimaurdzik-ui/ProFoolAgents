...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated PIXEL_AGENTS_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="pixel_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".pixel-agents"))
    prev_home = os.environ.get("PIXEL_AGENTS_HOME")
    os.environ["PIXEL_AGENTS_HOME"] = os.path.join(test_home, ".pixel-agents")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("pixel_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["PIXEL_AGENTS_HOME"] = prev_home
