...
thinking phase.  Pixel Agents's default cloud-stream stale detector
(``PIXEL_AGENTS_STREAM_STALE_TIMEOUT`` = 180s) and non-stream detector
(``PIXEL_AGENTS_API_CALL_STALE_TIMEOUT`` = 90s) both fire before the
...
def test_non_reasoning_model_keeps_default(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("PIXEL_AGENTS_API_CALL_STALE_TIMEOUT", raising=False)
