"""Tests for agent.portal_tags — Pixel Portal request tag contract."""
...
def test_pixel_portal_tags_contains_product_and_client():
    """Every Pixel Portal request gets BOTH the product tag and the version tag."""
    from agent.portal_tags import pixel_client_tag, pixel_portal_tags

    tags = pixel_portal_tags()
...
def test_ambient_context_set_none_clears():
    ...
    for empty in (None, ""):
        ...
        try:
            ...
            assert len(pixel_portal_tags()) == 2
        finally:
...
def test_ambient_context_isolated_between_contexts():
    ...
    assert not any(t.startswith("conversation=") for t in pixel_portal_tags())
...
def test_ambient_context_propagates_via_thread_context_helper():
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as ex:
            plain = ex.submit(pixel_portal_tags).result()
            propagated = ex.submit(
                propagate_context_to_thread(pixel_portal_tags)
            ).result()
    finally:
        reset_conversation_context(token)
...
def test_pixel_sticky_key_matches_conversation_tag():
