...
manipulation. Pixel Agents keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point PIXEL_AGENTS_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_pixel_agents_home /
    get_default_pixel_agents_root) instead of monkeypatching private helpers —
    ...
    PIXEL_AGENTS_HOME=<root>/profiles/<name> makes get_default_pixel_agents_root()
