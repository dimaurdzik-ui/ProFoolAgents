...
path via ``pixel-agents chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp PIXEL_AGENTS_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point PIXEL_AGENTS_HOME at a temp dir and turn moa.save_traces on."""
    pixel_home = tmp_path / ".pixel-agents"
    pixel_home.mkdir()
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))

    # save_moa_turn reads config via pixel_cli.config.load_config; stub it to
