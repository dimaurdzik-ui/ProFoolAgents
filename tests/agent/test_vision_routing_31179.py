...
def isolated_home(monkeypatch):
    """Temp PIXEL_AGENTS_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="pixel_test_31179_")
    pixel_home = os.path.join(test_home, ".pixel-agents")
    os.makedirs(pixel_home)
...
def _fresh_modules():
    """Drop cached pixel-agents modules so each test reloads against current env."""
    _drop_reload_targets()
