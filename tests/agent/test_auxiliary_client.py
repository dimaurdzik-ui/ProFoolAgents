...
from agent.auxiliary_client import (
    ...
    _refresh_pixel_recommended_model,
...
def _clean_env(monkeypatch):
    ...
    for key in (
        ...
        "OPENAI_MODEL", "LLM_MODEL", "PIXEL_INFERENCE_BASE_URL",
...
class TestResolveTaskProviderModel:
    ...
    def test_explicit_provider_moa_unwraps_to_aggregator(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "pixel_cli.moa_config.resolve_moa_preset",
            lambda cfg, name: preset,
        )
        monkeypatch.setattr("pixel_cli.config.load_config", lambda: {"moa": {}})
        monkeypatch.setattr("pixel_cli.config.load_config_readonly", lambda: {"moa": {}})
...
class TestMoaAggregatorSharedResolution:
    ...
    Real-config tests: write an actual config.yaml under a temp PIXEL_AGENTS_HOME
                    ...
                    "moa": {
                        ...
                        "presets": {
                            ...
                            "pixel-mix": {
                                "enabled": True,
                                "reference_models": [
                                    {"provider": "pixel", "model": "pixel-agents-4-70b"}
                                ],
                                "aggregator": {
                                    "provider": "pixel",
                                    "model": "pixel-agents-4-405b",
                                },
                            },
                        },
                    }
...
class TestBuildCallKwargsMaxTokens:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("pixel", "pixel-agents-4", "https://inference-api.pixelagents.com/v1", "max_tokens"),
        ],
    )
...
class TestPixelTagsScoping:
    def test_tags_injected_when_provider_is_pixel(self, monkeypatch):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(aux, "auxiliary_is_pixel", False)
        ...
        assert kwargs["extra_body"]["tags"] == aux._pixel_portal_tags()

    def test_tags_not_injected_for_gemini_when_main_is_pixel(self, monkeypatch):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(aux, "auxiliary_is_pixel", True)
...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``pixel-agents auth status`` already reports these as logged in; compression
        ...
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))
        monkeypatch.delenv("PIXEL_AGENTS_XAI_BASE_URL", raising=False)
    ...
    def test_pool_backed_credentials_honor_base_url_env_override(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))
        monkeypatch.setenv("PIXEL_AGENTS_XAI_BASE_URL", "https://example.x.ai/v1/")
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``pixel-agents model``, no per-task override is
    ...
    fallback chain (OpenRouter / Pixel / etc.), so aux tasks billed
...
class TestGetTextAuxiliaryClient:
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("pixel_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
            from agent.auxiliary_client import _build_codex_client
    ...
    def test_returns_none_when_nothing_available(self, monkeypatch):
        ...
        with patch("agent.auxiliary_client._read_pixel_auth", return_value=None), \
    ...
    def test_custom_endpoint_uses_codex_wrapper_when_runtime_requests_responses_api(self):
        with patch("agent.auxiliary_client._resolve_custom_runtime",
             ...
             patch("agent.auxiliary_client._read_pixel_auth", return_value=None), \
             patch("agent.auxiliary_client._resolve_pixel_runtime_api", return_value=None), \
...
class TestAuxiliaryPoolAwareness:
    ...
    def test_try_pixel_refreshes_stale_pool_entry(self):
        ...
        with (
            patch("agent.auxiliary_client.load_pool", return_value=pool),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("pixel_cli.models.get_pixel_recommended_aux_model", return_value=None),
        ):
            from agent.auxiliary_client import _try_pixel

            client, model = _try_pixel()

        assert pool.refreshed is True
        assert client is not None
        assert model == _PIXEL_MODEL
    ...
    def test_call_llm_retries_pixel_after_401(self):
        class _Auth401(Exception):
            status_code = 401

        stale_client = MagicMock()
        stale_client.base_url = "https://inference-api.pixelagents.com/v1"
        stale_client.chat.completions.create.side_effect = _Auth401("stale pixel key")

        fresh_client = MagicMock()
        fresh_client.base_url = "https://inference-api.pixelagents.com/v1"
        fresh_client.chat.completions.create.return_value = {"ok": True}

        with (
            patch("agent.auxiliary_client._resolve_task_provider_model", return_value=("pixel", "pixel-model", None, None, None)),
            patch("agent.auxiliary_client._get_cached_client", return_value=(stale_client, "pixel-model")),
            ...
            patch("agent.auxiliary_client._resolve_pixel_runtime_api", return_value=("fresh-agent-key", "https://inference-api.pixelagents.com/v1")),
        ):
            result = call_llm(
                task="compression",
                messages=[{"role": "user", "content": "hi"}],
...
class TestIsModelNotFoundError:
    ...
    def test_pixel_openrouter_catalog_404(self):
        """The exact incident error: a Portal-recommended model dropped from
        the Pixel → OpenRouter catalog."""
        exc = Exception(
...
class TestRefreshPixelRecommendedModel:
    """_refresh_pixel_recommended_model picks a fresh model after a stale 404."""
    ...
    def test_falls_back_to_default_when_portal_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "pixel_cli.models.get_pixel_recommended_aux_model", _boom)
        out = _refresh_pixel_recommended_model(
            vision=False, stale_model="some/dead-model")
        assert out == _PIXEL_MODEL
    ...
    def test_returns_none_when_no_distinct_alternative(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "pixel_cli.models.get_pixel_recommended_aux_model",
            lambda **kw: _PIXEL_MODEL,
        )
        out = _refresh_pixel_recommended_model(
            vision=False, stale_model=_PIXEL_MODEL)
        assert out is None
...
class TestGetProviderChain:
    ...
    def test_returns_four_entries(self):
        ...
        assert labels == ["openrouter", "pixel", "local/custom", "api-key"]
...
class TestTryPaymentFallback:
    ...
    def test_skips_failed_provider(self):
        ...
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_pixel", return_value=(mock_client, "pixel-model")), \
             patch("agent.auxiliary_client._read_main_provider", return_value="openrouter"):
            client, model, label = _try_payment_fallback("openrouter", task="compression")
        assert client is mock_client
        assert model == "pixel-model"
        assert label == "pixel"
    ...
    def test_codex_not_in_fallback_chain(self):
        ...
        When OR/Pixel/custom/api-key all fail, payment-fallback returns None —
        Codex is never tried with a guessed model.
        """
        with patch("agent.auxiliary_client._try_openrouter", return_value=(None, None)), \
             patch("agent.auxiliary_client._try_pixel", return_value=(None, None)), \
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Pixel Agents auth / credential pool."""
...
def test_resolve_api_key_provider_skips_unconfigured_anthropic(monkeypatch):
    ...
    from pixel_cli.auth import ProviderConfig
    ...
    monkeypatch.setattr("pixel_cli.auth.PROVIDER_REGISTRY", fake_registry)
    monkeypatch.setattr(
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Pixel Agents owns the retry/timeout budget (issue #54465). The SDK
...
class TestAuxiliaryTaskExtraBody:
    def test_sync_call_merges_task_extra_body_from_config(self):
        ...
        with patch("pixel_cli.config.load_config", return_value=config), patch("pixel_cli.config.load_config_readonly", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
    ...
    def test_moa_tasks_reject_task_level_reasoning_effort(self, moa_task, caplog):
        ...
        with patch("pixel_cli.config.load_config", return_value=config), patch("pixel_cli.config.load_config_readonly", return_value=config), \
             caplog.at_level(logging.WARNING, logger="agent.auxiliary_client"):
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Pixel Agents reasoning.

    MoA slot reasoning is carried through call_llm as a Pixel Agents
...
class TestAuxiliaryClientPoisonedCacheEviction:
    ...
    See https://github.com/PixelResearch/pixel-agents/issues/23432.
    """
...
class TestBuildCallKwargsToolDedup:
    ...
    regresses.  See: https://github.com/PixelResearch/pixel-agents/issues/18478
    """
...
class TestNvidiaBillingHeaders:
    ...
    def test_resolve_provider_client_cloud_adds_billing_origin_header(self, monkeypatch):
        ...
        assert headers["X-BILLING-INVOKE-ORIGIN"] == "PixelAgentsAgent"
...
def test_pool_runtime_base_url_uses_pixel_env_override(monkeypatch):
    entry = SimpleNamespace(
        provider="pixel",
        runtime_base_url="https://inference-api.pixelagents.com/v1",
        inference_base_url="https://inference-api.pixelagents.com/v1",
        base_url="https://inference-api.pixelagents.com/v1",
    )
    monkeypatch.setenv("PIXEL_INFERENCE_BASE_URL", "https://ai.wildebeest-newton.ts.net/v1")
...
class TestAuxUnhealthyCache:
    ...
    def test_call_llm_marks_provider_unhealthy_on_402(self, monkeypatch):
        ...
        pixel_client = MagicMock()
        pixel_resp = MagicMock()
        pixel_resp.choices = [MagicMock(message=MagicMock(content="ok"))]
        pixel_client.chat.completions.create.return_value = pixel_resp
        ...
        with patch("agent.auxiliary_client._get_cached_client",
             ...
             patch("agent.auxiliary_client._try_payment_fallback",
                    return_value=(pixel_client, "n-model", "pixel")), \
...
class TestCustomEndpointApiKeyInheritance:
    ...
    def test_inherits_main_api_key_when_aux_key_empty(self, monkeypatch):
        ...
        with patch("pixel_cli.config.load_config", return_value=fake_config), patch("pixel_cli.config.load_config_readonly", return_value=fake_config), \
    ...
    def test_runtime_override_key_is_used(self, monkeypatch):
        ...
        with patch.object(ac, "_RUNTIME_MAIN_API_KEY", "sk-runtime-key"), \
             ...
             patch("pixel_cli.config.load_config", return_value={"model": {}}), patch("pixel_cli.config.load_config_readonly", return_value={"model": {}}), \
             patch.object(ac, "_create_openai_client", side_effect=_capture_create):
            client, model = resolve_provider_client(
                "custom",
                model="test-model",
