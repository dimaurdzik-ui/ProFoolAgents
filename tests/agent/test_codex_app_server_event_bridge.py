"""Regression tests for the codex_app_server → Pixel Agents UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_pixel_tools_mcp_server_emits_bare_tool_name(self):
        """The pixel-agents-tools MCP server wraps Pixel Agents' own tools for codex;
        ...
        mcp.pixel-agents-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "pixel-agents-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "pixel-agents-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "pixel-agents agent docs"
        assert calls[0].args[3] == {"query": "pixel-agents agent docs"}
