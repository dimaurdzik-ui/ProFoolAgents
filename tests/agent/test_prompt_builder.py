...
from pixel_cli.pixel_subscription import PixelFeatureState, PixelSubscriptionFeatures
...
class TestTruncateContent:
    ...
    def _reset_truncation_state(self, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.config.load_config", default_load_config)
        monkeypatch.setattr("pixel_cli.config.load_config_readonly", default_load_config)
    ...
    def test_truncation_warning_points_to_config_key(self, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.config.load_config", fake_load_config)
        monkeypatch.setattr("pixel_cli.config.load_config_readonly", fake_load_config)
...
class TestDynamicContextFileCap:
    ...
    def _no_explicit_config(self, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.config.load_config", lambda: {})
        monkeypatch.setattr("pixel_cli.config.load_config_readonly", lambda: {})
...
class TestBuildPixelSubscriptionPrompt:
    def test_includes_active_subscription_features(self, monkeypatch):
        monkeypatch.setattr("tools.tool_backend_helpers.managed_pixel_tools_enabled", lambda: True)
        monkeypatch.setattr(
            "pixel_cli.pixel_subscription.get_pixel_subscription_features",
            lambda config=None: PixelSubscriptionFeatures(
                subscribed=True,
                pixel_auth_present=True,
                provider_is_pixel=True,
                features={
                    "web": PixelFeatureState("web", "Web tools", True, True, True, True, False, True, "firecrawl"),
                    "image_gen": PixelFeatureState("image_gen", "Image generation", True, True, True, True, False, True, "Pixel Subscription"),
                    "video_gen": PixelFeatureState("video_gen", "Video generation", False, False, False, False, False, False, ""),
                    "tts": PixelFeatureState("tts", "OpenAI TTS", True, True, True, True, False, True, "OpenAI TTS"),
                    "stt": PixelFeatureState("stt", "Speech-to-text", True, True, True, True, False, True, "OpenAI Whisper"),
                    "browser": PixelFeatureState("browser", "Browser automation", True, True, True, True, False, True, "Browser Use"),
                    "modal": PixelFeatureState("modal", "Modal execution", False, True, False, False, False, True, "local"),
                },
            ),
        )

        prompt = build_pixel_subscription_prompt({"web_search", "browser_navigate"})
    ...
    def test_non_subscriber_prompt_includes_relevant_upgrade_guidance(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda config=None: PixelSubscriptionFeatures(
                subscribed=False,
                pixel_auth_present=False,
                provider_is_pixel=False,
                features={
                    "web": PixelFeatureState("web", "Web tools", True, False, False, False, False, True, ""),
                    "image_gen": PixelFeatureState("image_gen", "Image generation", True, False, False, False, False, True, ""),
                    "video_gen": PixelFeatureState("video_gen", "Video generation", False, False, False, False, False, False, ""),
                    "tts": PixelFeatureState("tts", "OpenAI TTS", True, False, False, False, False, True, ""),
                    "stt": PixelFeatureState("stt", "Speech-to-text", True, False, False, False, False, True, ""),
                    "browser": PixelFeatureState("browser", "Browser automation", True, False, False, False, False, True, ""),
                    "modal": PixelFeatureState("modal", "Modal execution", False, False, False, False, False, True, ""),
                },
            ),
        )

        prompt = build_pixel_subscription_prompt({"image_generate"})

        assert "suggest Pixel subscription as one option" in prompt
        assert "Do not mention subscription unless" in prompt

    def test_feature_flag_off_returns_empty_prompt(self, monkeypatch):
        monkeypatch.setattr("tools.tool_backend_helpers.managed_pixel_tools_enabled", lambda: False)

        prompt = build_pixel_subscription_prompt({"web_search"})
...
class TestBuildContextFilesPrompt:
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path / "pixel_home"))
        pixel_home = tmp_path / "pixel_home"
        pixel_home.mkdir()
        (pixel_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
    ...
    # --- .pixel-agents.md / PIXEL_AGENTS.md discovery ---
...
# =========================================================================
# .pixel-agents.md helper functions
# =========================================================================


class TestFindPixelAgentsMd:
    def test_finds_in_cwd(self, tmp_path):
        (tmp_path / ".pixel-agents.md").write_text("rules")
        assert _find_pixel_md(tmp_path) == tmp_path / ".pixel-agents.md"



    def test_walks_to_git_root(self, tmp_path):
        (tmp_path / ".git").mkdir()
        (tmp_path / ".pixel-agents.md").write_text("root rules")
        sub = tmp_path / "a" / "b"
        sub.mkdir(parents=True)
        assert _find_pixel_md(sub) == tmp_path / ".pixel-agents.md"
    ...
    def test_no_git_root_checks_cwd_only(self, tmp_path):
        ...
        the way to / and pick up a .pixel-agents.md planted in /tmp, /home, or /
...
class TestEnvironmentHints:
    ...
    def test_environment_hint_from_env_var_is_appended(self, monkeypatch):
        """PIXEL_AGENTS_ENVIRONMENT_HINT lets an embedder describe the runtime env."""
        import agent.prompt_builder as _pb
        monkeypatch.setattr(_pb, "is_wsl", lambda: False)
        monkeypatch.delenv("TERMINAL_ENV", raising=False)
        monkeypatch.setenv("PIXEL_AGENTS_ENVIRONMENT_HINT", "Running inside an OpenShell sandbox.")
        _pb._clear_backend_probe_cache()
        result = _pb.build_environment_hints()
