"""Tests for pixel_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Pixel Agents entry point
(pixel-agents, pixel-agents, pixel-agents-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Pixel Agents entry point imports pixel_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported pixel_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("pixel_bootstrap", None)
    import pixel_bootstrap  # noqa: WPS433
    return pixel_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Pixel Agents CLI
...
class TestEntryPointsImportBootstrap:
    """Every Pixel Agents entry point must import pixel_bootstrap as its
    ...
    # Entry points that invoke Pixel Agents as a process.  Each one must
    # import pixel_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "pixel_cli/main.py",   # pixel-agents CLI (console_script)
        "run_agent.py",          # pixel-agents (console_script)
        "acp_adapter/entry.py",  # pixel-agents-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import pixel_bootstrap' and that
        ...
        half-finished ``pixel-agents update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``pixel_bootstrap`` as a top-level module) leaves pixel-agents
        ...
        """
        # Resolve relative to the pixel-agents repo root.  Tests live
        # at tests/test_pixel_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # pixel-agents start even when pixel_bootstrap hasn't been
        ...
        assert first_import_name == "pixel_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'pixel_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import pixel_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Pixel Agents's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("PIXEL_AGENTS_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["PIXEL_AGENTS_PYTHON_SRC_ROOT"] = env
            elif "PIXEL_AGENTS_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["PIXEL_AGENTS_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/pixel-agents")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("PIXEL_AGENTS_PYTHON_SRC_ROOT", None)
            else:
                os.environ["PIXEL_AGENTS_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/pixel-agents", "/usr/lib/python"])
    ...
    def test_src_root_forced_to_front(self):
        hb = _fresh_import()
        result = self._run(hb, ["", "/opt/pixel-agents", "/usr/lib/python"])
        assert result[0] == "/opt/pixel-agents"
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Pixel Agents root.  The guard must relocate Pixel Agents to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/pixel-agents"])
        assert result[0] == "/opt/pixel-agents"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Pixel Agents root.
        assert result.index("/opt/pixel-agents") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["PIXEL_AGENTS_PYTHON_SRC_ROOT"] = "/env/pixel-agents"
            hb.harden_import_path()
            assert sys.path[0] == "/env/pixel-agents"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("PIXEL_AGENTS_PYTHON_SRC_ROOT", None)
