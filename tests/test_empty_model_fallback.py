...
class TestGetDefaultModelForProvider:
    """Unit tests for pixel_cli.models.get_default_model_for_provider."""
    ...
    def test_catalog_label_overrides_constant(self):
        ...
        with patch(
            "pixel_cli.model_catalog.get_default_model_from_cache",
            return_value="qwen/qwen3.7-max",
        ):
            assert (
                models_mod.get_preferred_silent_default_model("pixel")
                == "qwen/qwen3.7-max"
            )
            # pixel catalog carries qwen3.7-max, so the full resolver follows.
            assert (
                models_mod.get_default_model_for_provider("pixel")
                == "qwen/qwen3.7-max"
            )
