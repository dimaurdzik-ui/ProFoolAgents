...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("pixel-agents.gateway.up"), _M("pixel-agents.gateway.active_agents"),
        _M("pixel-agents.gateway.busy"), _M("pixel-agents.gateway.drainable"),
        _M("pixel-agents.gateway.restart_requested"),
        _M("pixel-agents.platform.up"), _M("pixel-agents.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("pixel-agents.cron.scheduler.heartbeat_age_seconds"),
        _M("pixel-agents.cron.scheduler.last_success_age_seconds"),
        _M("pixel-agents.cron.scheduler.catch_up_occurrences"),
        _M("pixel-agents.cron.jobs.enabled"), _M("pixel-agents.cron.jobs.running"),
        _M("pixel-agents.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Pixel Agents-owned Relay transport health" in text
