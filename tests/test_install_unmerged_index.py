...
The ``pixel-agents update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m pixel_cli.main gateway run`` — image name
    ``pythonw``, not ``pixel-agents.exe`` — so the ``taskkill /IM pixel-agents.exe`` guard
    ...
    # The pixel-agents.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM pixel-agents.exe' in text
