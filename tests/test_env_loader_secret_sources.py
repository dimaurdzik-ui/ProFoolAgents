"""Tests for the secret-source tracking in ``pixel_cli.env_loader``.

These cover the small public surface that lets `pixel-agents model` / `pixel-agents setup`
...
from pixel_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_pixel_dotenv()`` is called at module-import time from several
    hot modules (cli.py, pixel_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per PIXEL_AGENTS_HOME per process.
    """
    ...
    # cli.py, pixel_cli/main.py, run_agent.py, trajectory_compressor.py,
