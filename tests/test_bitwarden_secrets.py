...
binary download are exercised manually by `pixel-agents secrets bitwarden
...
def pixel_home(tmp_path, monkeypatch):
    """Point Pixel Agents at an isolated home directory."""
    home = tmp_path / ".pixel-agents"
    home.mkdir()
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(home))
    # Some modules cache get_pixel_agents_home; clear if needed.
    import pixel_constants
    if hasattr(pixel_constants, "_PIXEL_AGENTS_HOME_CACHE"):
        pixel_constants._PIXEL_AGENTS_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(pixel_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from pixel_cli.env_loader import _apply_external_secret_sources
