...
Uses a throwaway PIXEL_AGENTS_HOME so it never touches ~/.pixel-agents. No real names.
"""
...
def _fresh_home():
    """Point PIXEL_AGENTS_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_pixel_agents_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["PIXEL_AGENTS_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER PIXEL_AGENTS_HOME is set.
...
# a different get_pixel_agents_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"PIXEL_AGENTS_HOME (throwaway): {HOME}")
