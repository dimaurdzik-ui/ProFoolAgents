...
from acp_adapter.server import PixelAgentsACPAgent
...
class FakeAgent:
    def __init__(self):
        ...
        self.enabled_toolsets = ["pixel-agents-acp"]
...
def test_acp_real_agent_gets_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "pixel_cli.config",
        mod("pixel_cli.config", load_config=lambda: {"model": {"default": "m", "provider": "p"}}),
    )
    monkeypatch.setitem(
        sys.modules,
