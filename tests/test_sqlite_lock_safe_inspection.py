"""POSIX advisory locks must survive Pixel Agents' own database inspection.
...
Pixel Agents used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `pixel-agents sessions
...
out across Pixel Agents' inspection calls.
"""
...
from pixel_cli.sqlite_safe_read import (
...
def clean_registry():
    ...
    import pixel_cli.sqlite_safe_read as mod
...
def test_probe_and_connect_do_not_race(tmp_path, clean_registry, monkeypatch):
    ...
    """
    import pixel_cli.sqlite_safe_read as ssr
