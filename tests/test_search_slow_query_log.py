...
def test_slow_log_emitted_at_zero_threshold(db, monkeypatch, caplog):
    monkeypatch.setenv("PIXEL_AGENTS_SEARCH_SLOW_MS", "0")
    with caplog.at_level(logging.INFO, logger="pixel_state"):
...
def test_no_log_under_threshold(db, monkeypatch, caplog):
    monkeypatch.setenv("PIXEL_AGENTS_SEARCH_SLOW_MS", "60000")
    with caplog.at_level(logging.INFO, logger="pixel_state"):
