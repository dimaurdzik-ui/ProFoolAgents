...
class TestBuildApiKwargsPixelPortal:
    def test_includes_pixel_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == pixel_portal_tags(session_id=agent.session_id)
...
# ── Chat completions response handling (OpenRouter/Pixel) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_pixel_when_no_openrouter(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        from agent.auxiliary_client import _PIXEL_MODEL, get_text_auxiliary_client
        pixel_auth = {
            "access_token": _fake_invoke_jwt(),
            "scope": "inference:invoke",
        }
        with patch("agent.auxiliary_client._read_pixel_auth", return_value=pixel_auth), \
             patch("agent.auxiliary_client.OpenAI") as mock, \
             patch("pixel_cli.models.get_pixel_recommended_aux_model", return_value=None):
            client, model = get_text_auxiliary_client()
        assert model == _PIXEL_MODEL

    def test_custom_endpoint_when_no_pixel(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/Pixel keys are available.
