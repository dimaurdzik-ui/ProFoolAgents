...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    pixel_state = ModuleType("pixel_state")
    pixel_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "pixel_state", pixel_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Pixel Agents"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Pixel Agents"
