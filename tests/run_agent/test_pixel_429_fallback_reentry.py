"""Regression guard: a genuine Pixel 429 must re-enter the retry loop so the
top-of-loop Pixel rate-limit guard can activate the fallback chain.
...
``record_pixel_rate_limit()`` moments earlier and either activates a fallback
...
class TestGenuinePixel429ReentersLoop:
