...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._pixel_home / "logs" / "errors.log").resolve()
...
class TestSessionJsonSnapshotOptIn:
    ...
    def test_traversal_session_id_cannot_escape_logs_dir(self, agent, tmp_path):
        ...
        # originate from the untrusted X-Pixel-Agents-Session-Id API header) must not
...
class TestSaveSessionLogRedactsSecrets:
    ...
    def _ensure_redaction_enabled(self, monkeypatch):
        """Force redaction on regardless of host PIXEL_AGENTS_REDACT_SECRETS state.
...
class TestInit:
    ...
    def test_prompt_caching_cache_ttl_defaults_without_config(self):
        ...
        with (
            ...
            patch("run_agent.check_toolset_requirements", return_value={}),
            patch("run_agent.OpenAI"),
            patch("pixel_cli.config.load_config", return_value={}), patch("pixel_cli.config.load_config_readonly", return_value={}),
        ):
            a = AIAgent(
                api_key="test-k...7890",
                model="anthropic/claude-sonnet-4-20250514",
    ...
    def test_constructor_max_tokens_wins_over_config(self):
        ...
        with (
            ...
            patch(
                ...
                "pixel_cli.config.load_config_readonly",
                return_value={"model": {"max_tokens": 4096}},
            ),
        ):
            a = AIAgent(
...
class TestBuildSystemPrompt:
    ...
    def test_includes_pixel_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_pixel_subscription_prompt", lambda tool_names: "PIXEL SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
        assert "PIXEL SUBSCRIPTION BLOCK" in prompt
...
class TestBuildApiKwargs:
    ...
    def test_core_responses_preserves_supported_xhigh(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "pixel_cli.models.github_model_reasoning_efforts",
...
class TestExecuteToolCalls:
    ...
    def test_keyboard_interrupt_emits_cancelled_post_tool_hook(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.lifecycle.invoke_hook", _capture_hook)
        monkeypatch.setattr("pixel_cli.lifecycle.has_hook", lambda name: True)
...
class TestConcurrentToolExecution:
    ...
    def test_sequential_registry_tool_forwards_request_middleware_trace(
        ...
        from pixel_cli.middleware import RequestMiddlewareResult
        ...
        mock_msg = _mock_assistant_msg(content="", tool_calls=[tool_call])
        monkeypatch.setattr(
            "pixel_cli.middleware.apply_tool_request_middleware",
        ...
        monkeypatch.setattr(
            "pixel_cli.middleware.run_tool_execution_middleware",
            lambda _name, args, callback, **_kwargs: callback(args),
        )
        monkeypatch.setattr(
            "pixel_cli.plugins.resolve_pre_tool_block",
    ...
    def test_managed_tool_pipeline_rejects_second_dispatch(self, agent, monkeypatch):
        ...
        assert duplicate_errors == [
            "Pixel Agents tool execution callback invoked more than once"
        ]
    ...
    def test_managed_tool_pipeline_allows_one_concurrent_dispatch(
        ...
        assert outcome.result == "ok"
        assert dispatched == [{"command": "true"}]
        assert errors == ["Pixel Agents tool execution callback invoked more than once"]
        assert outcome.blocked is False
...
class TestAgentRuntimePostHookOwnershipSync:
    ...
    def test_agent_runtime_tools_emit_once_per_executor_path(
        ...
        monkeypatch.setattr(
            "pixel_cli.lifecycle.invoke_hook",
...
class TestHandleMaxIterations:
    ...
    def test_api_sanitizer_repairs_tool_call_with_empty_function_name(self, agent):
        ...
        call, because pixel-agents' dispatch loop keeps empty-name calls paired with
...
class TestRunConversation:
    ...
    def test_task_start_failure_closes_relay_turn_and_lease(self, agent):
        ...
        with (
            ...
            patch(
                "pixel_cli.observability.relay_shared_metrics.start_task_run",
                side_effect=start_error,
            ),
            patch(
                "pixel_cli.observability.relay_shared_metrics.finish_task_run"
            ) as finish_task_run,
    ...
    def test_ollama_small_runtime_context_fails_before_api_call(self, agent, caplog):
        ...
        assert "Ollama runtime context too small for Pixel Agents tool use" in caplog.text
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch(
                "pixel_cli.lifecycle.has_hook",
                side_effect=lambda name: name in {"pre_api_request", "post_api_request"},
            ),
            patch("pixel_cli.lifecycle.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_api_request_error_hook_skips_payload_work_without_listener(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.lifecycle.has_hook", lambda name: False)
        monkeypatch.setattr("pixel_cli.lifecycle.invoke_hook", _invoke_hook)
    ...
    def test_request_scoped_api_hooks_skip_payload_work_without_listeners(self, agent, monkeypatch):
        ...
        monkeypatch.setattr("pixel_cli.lifecycle.has_hook", _has_hook)
        ...
        with (
            patch("pixel_cli.lifecycle.invoke_hook", return_value=[]),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_pixel_401_refreshes_after_remint_and_retries(self, agent):
        self._setup_agent(agent)
        agent.provider = "pixel"
        ...
        with (
            ...
            patch.object(agent, "_interruptible_api_call", side_effect=_fake_api_call),
            patch.object(
                agent, "_try_refresh_pixel_client_credentials", side_effect=_fake_refresh
            ),
        ):
            result = agent.run_conversation("hello")
    ...
    def test_engine_preflight_fires_below_threshold(self, agent):
        ...
        should_compress_preflight() hook so engines like pixel-agents-lcm can
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("PIXEL_AGENTS_KANBAN_TASK", "t_test_task_123")
        ...
        with (
            ...
            patch("pixel_cli.kanban_db._record_task_failure",
                  mock_record_failure),
            patch("pixel_cli.kanban_db.connect", mock_connect),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """The exhaustion bridge must NOT fire when PIXEL_AGENTS_KANBAN_TASK
        is unset (non-kanban runs are unaffected by #29747 gap 2)."""
        self._setup_agent(agent)
        agent.max_iterations = 2

        monkeypatch.delenv("PIXEL_AGENTS_KANBAN_TASK", raising=False)
...
class TestRetryExhaustion:
    ...
    def test_content_filter_refusal_surfaced_not_retried(self, agent):
        ...
        portal (Pixel Portal fronting Anthropic) returns ``message.refusal``
...
class TestPixelCredentialRefresh:
    """Verify Pixel credential refresh rebuilds the runtime client."""

    def test_try_refresh_pixel_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "pixel_cli.auth.resolve_pixel_runtime_credentials", _fake_resolve
        )
        ...
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_pixel_client_credentials(force=True)
        ...
        assert rebuilt["kwargs"]["api_key"] == "new-pixel-key"
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.pixelagents.com/v1"
        )
        assert "default_headers" not in rebuilt["kwargs"]
        assert isinstance(agent.client, _RebuiltClient)

    def test_try_refresh_pixel_client_credentials_rebuilds_anthropic_client(
        self, agent, monkeypatch
    ):
        ...
        agent.api_key = "stale-pixel-key"
        agent.base_url = "https://inference-api.pixelagents.com/v1"
        agent._anthropic_api_key = "stale-pixel-key"
        agent._anthropic_base_url = "https://inference-api.pixelagents.com/v1"
        ...
        assert agent.base_url == "https://inference-api.pixelagents.com/v1"
        assert agent._anthropic_api_key == "fresh-portal-jwt"
        assert agent._anthropic_base_url == (
            "https://inference-api.pixelagents.com/v1"
        )
...
class TestGpt5ApiModeRouting:
    ...
    def test_pixel_gpt5_stays_on_chat_completions(self, agent):
        """Pixel serves gpt-5.x on /chat/completions — must not upgrade to codex_responses."""
        agent.provider = "pixel"
        agent.base_url = "https://inference-api.pixelagents.com/v1"
        agent.api_mode = "chat_completions"
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Pixel Agents" in agent._cached_system_prompt
