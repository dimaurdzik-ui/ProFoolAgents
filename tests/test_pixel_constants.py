"""Tests for pixel_constants module."""
...
import pixel_constants
from pixel_constants import (
    VALID_REASONING_EFFORTS,
    agent_browser_runnable,
    find_pixel_agents_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_pixel_agents_root,
    get_pixel_agents_dir,
    get_pixel_agents_home,
    get_process_pixel_agents_home,
    heal_pixel_agents_managed_node,
    pixel_agents_managed_node_tree_present,
    iter_pixel_agents_node_dirs,
    is_container,
    node_tool_runnable,
    ...
    with_pixel_node_path,
)


class TestGetDefaultPixelAgentsRoot:
    """Tests for get_default_pixel_agents_root() — Docker/custom deployment awareness."""

    def test_no_pixel_home_returns_native(self, tmp_path, monkeypatch):
        """When PIXEL_AGENTS_HOME is not set, returns ~/.pixel-agents."""
        monkeypatch.delenv("PIXEL_AGENTS_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_pixel_agents_root() == tmp_path / ".pixel-agents"
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (PIXEL_AGENTS_HOME=<root>/profiles/<name>),
        returns the Docker root, not the profile dir."""
        docker_root = tmp_path / "opt" / "data"
        profile = docker_root / "profiles" / "coder"
        profile.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(profile))
        assert get_default_pixel_agents_root() == docker_root

    def test_no_pixel_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\pixel-agents, not ~/.pixel-agents."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("PIXEL_AGENTS_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(pixel_constants.sys, "platform", "win32")

        assert get_default_pixel_agents_root() == local_appdata / "pixel-agents"



class TestGetPixelAgentsHome:
    """Tests for get_pixel_agents_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When PIXEL_AGENTS_HOME is unset on Windows, use %LOCALAPPDATA%\\pixel-agents."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("PIXEL_AGENTS_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(pixel_constants.sys, "platform", "win32")
        monkeypatch.setattr(pixel_constants, "_profile_fallback_warned", False)

        assert get_pixel_agents_home() == local_appdata / "pixel-agents"


class TestGetProcessPixelAgentsHome:
    """Tests for get_process_pixel_agents_home() — process launch scope.
    ...
    via set_pixel_agents_home_override().
    """
    ...
    def test_env_set_returns_that_path(self, tmp_path, monkeypatch):
        ...
        assert get_process_pixel_agents_home() == home




class TestPixelAgentsManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_pixel_agents_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(pixel_constants.sys, "platform", "win32")
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(home))
        monkeypatch.setattr(pixel_constants, "node_tool_runnable", lambda path: True)

        assert find_pixel_agents_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(pixel_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(pixel_constants, "heal_pixel_agents_managed_node", lambda: False)
        monkeypatch.setattr(
            pixel_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert pixel_agents_managed_node_tree_present() is True
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Pixel Agents-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(pixel_constants, "heal_pixel_agents_managed_node", _heal)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(pixel_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(pixel_constants, "_container_detected", True)
        assert is_container() is True
...
class TestResolvePerModelReasoningEffort:
    ...
    def test_exact_match(self):
        ...
        from pixel_constants import resolve_per_model_reasoning_effort
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".pixel-agents"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import pixel_cli._subprocess_compat as subprocess_compat
...
class TestGetPixelAgentsDir:
    """Tests for ``get_pixel_agents_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_pixel_agents_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_is_file_treated_as_content(self, tmp_path, monkeypatch):
        ...
        result = get_pixel_agents_dir("cache/images", "image_cache")
...
class TestWslPathTranslation:
    ...
    def test_windows_drive_to_wsl_mount(self):
        assert pixel_constants.windows_path_to_wsl(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert pixel_constants.windows_path_to_wsl("C:/Users/alex") == "/mnt/c/Users/alex"
        assert pixel_constants.windows_path_to_wsl("D:\\") == "/mnt/d/"

    def test_windows_drive_ignores_non_drive_paths(self):
        assert pixel_constants.windows_path_to_wsl("/home/alex") is None
        assert pixel_constants.windows_path_to_wsl("relative\\dir") is None




    def test_translate_maps_windows_and_unc_on_wsl(self, monkeypatch):
        monkeypatch.setattr(pixel_constants, "is_wsl", lambda: True)
        assert pixel_constants.translate_cwd_for_wsl_backend(r"C:\Users\alex") == "/mnt/c/Users/alex"
        assert pixel_constants.translate_cwd_for_wsl_backend(r"\\wsl.localhost\Ubuntu\home\alex") == "/home/alex"
        # Already-POSIX paths pass through untouched.
        assert pixel_constants.translate_cwd_for_wsl_backend("/home/alex") == "/home/alex"
