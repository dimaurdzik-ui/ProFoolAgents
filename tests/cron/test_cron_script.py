...
- Path resolution (absolute, relative to PIXEL_AGENTS_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp PIXEL_AGENTS_HOME."""
    pixel_home = tmp_path / ".pixel-agents"
    pixel_home.mkdir()
    (pixel_home / "cron").mkdir()
    (pixel_home / "cron" / "output").mkdir()
    (pixel_home / "scripts").mkdir()
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "PIXEL_AGENTS_DIR", pixel_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", pixel_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", pixel_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", pixel_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Pixel Agents provider env (SECURITY.md §2.3)."""
        from tools.environments.local import _PIXEL_AGENTS_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_PIXEL_AGENTS_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.pixel-agents/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("PIXEL_AGENTS_SESSION_PLATFORM") is None
        assert os.environ.get("PIXEL_AGENTS_SESSION_CHAT_ID") is None
        assert os.environ.get("PIXEL_AGENTS_SESSION_CHAT_NAME") is None
