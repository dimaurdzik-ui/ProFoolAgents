...
pixel/claude-fable-5), the job would silently inherit it on its next tick and
...
resolve_runtime_provider against a temp PIXEL_AGENTS_HOME) and the create_job
...
class TestProviderDriftGuard:
    ...
    def test_b_unpinned_snapshot_differs_fails_closed(self, tmp_path):
        ...
        job = _base_job(provider_snapshot="openrouter")
        success, output, final_response, error, agent_constructed = \
            _run_with_current_provider(job, "pixel", tmp_path)
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("pixel_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
                prompt="do a thing", schedule="every 1 hour", provider="pixel"
            )

        # Explicit provider → pinned → no snapshot needed, and resolution skipped.
        assert job["provider"] == "pixel"
...
def _run_with_current_provider_and_model(
    ...
    *,
    ...
    fake_db = MagicMock()
    with patch("cron.scheduler._pixel_home", tmp_path), \
         patch("cron.scheduler._get_pixel_agents_home", return_value=tmp_path), \
         patch("cron.scheduler._resolve_origin", return_value=None), \
         patch("pixel_cli.env_loader.load_pixel_dotenv"), \
         patch("pixel_cli.env_loader.reset_secret_source_cache"), \
         patch("pixel_state.SessionDB", return_value=fake_db), \
