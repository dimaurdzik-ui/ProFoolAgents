...
the shared /blueprint command handler (pixel_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated PIXEL_AGENTS_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("pixelagents://blueprint/morning-brief?")
