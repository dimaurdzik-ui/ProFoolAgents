...
profile's PIXEL_AGENTS_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.pixel-agents/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active PIXEL_AGENTS_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever PIXEL_AGENTS_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    PIXEL_AGENTS_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import pixel_constants

    monkeypatch.setattr(
        pixel_constants, "_get_platform_default_pixel_agents_home", lambda: root
    )
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile PIXEL_AGENTS_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "pixel_home"
    ...
    assert pixel_constants.get_pixel_agents_home().resolve() == profile_home.resolve()
    assert pixel_constants.get_default_pixel_agents_root().resolve() == root.resolve()

    # cron/jobs.py computes PIXEL_AGENTS_DIR from get_pixel_agents_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.PIXEL_AGENTS_DIR.resolve() == profile_home.resolve()
        assert (
