...
class TestRunJobTerminalCwd:
    ...
    def _install_stubs(monkeypatch, observed: dict):
        ...
        # Bypass the real provider resolver — it reads ~/.pixel-agents and credentials.
        from pixel_cli import runtime_provider as _rtp
        monkeypatch.setattr(
            _rtp,
        ...
        monkeypatch.setenv("PIXEL_AGENTS_CRON_TIMEOUT", "0")

        # run_job calls load_dotenv(~/.pixel-agents/.env, override=True), which will
