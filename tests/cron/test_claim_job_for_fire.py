...
These exercise the real store against a temp PIXEL_AGENTS_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated PIXEL_AGENTS_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("PIXEL_AGENTS_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_pixel_agents_home() reads the env live.
    yield tmp_path
