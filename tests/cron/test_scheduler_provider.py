...
CronScheduler plan, .pixel-agents/plans/cron-scheduler-provider-interface.md). They
...
No production code is exercised beyond the two ticker entry points:
  ...
  - pixel_cli/web_server.py::_start_desktop_cron_ticker  (desktop fallback)
...
def test_ticker_calls_tick_at_least_once_then_stops():
    ...
    # the background thread, never the synchropixel CLI path).
...
def test_desktop_ticker_calls_tick_then_stops():
    ...
    no adapters/loop."""
    from pixel_cli.web_server import _start_desktop_cron_ticker
