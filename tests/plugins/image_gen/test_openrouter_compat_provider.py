#!/usr/bin/env python3
"""Tests for the OpenRouter-compatible image gen provider (OpenRouter + Pixel)."""
...
_RUNTIME = "pixel_cli.runtime_provider.resolve_runtime_provider"
...
class TestProviderClass:
    ...
    def test_display_names(self):
        ...
        assert by_name["pixel"].display_name == "Pixel Portal"
    ...
    def test_pixel_honors_top_level_model(self):
        from plugins.image_gen.openrouter import _build_providers

        cfg = {"model": "openai/gpt-image-2"}
        pixel = {p.name: p for p in _build_providers()}["pixel"]
        with patch("plugins.image_gen.openrouter._load_image_gen_config", return_value=cfg):
            assert pixel._resolve_model_chain() == ["openai/gpt-image-2"]
...
class TestGenerate:
    ...
    def test_posts_to_resolved_base_url(self):
        """Pixel routes to its own base URL — proves the same code serves both."""
        pixel_runtime = _runtime_ok(
            provider="pixel", base_url="https://inference.pixelagents.com/v1", api_key="pixel-tok"
        )
        with patch(_RUNTIME, return_value=pixel_runtime), \
             patch("requests.post", return_value=_mock_chat_response([_PNG_DATA_URI])) as mock_post, \
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            from plugins.image_gen.openrouter import _build_providers

            pixel = {p.name: p for p in _build_providers()}["pixel"]
            result = pixel.generate(prompt="a pet")

        assert result["success"] is True
        assert result["provider"] == "pixel"
        url = mock_post.call_args[0][0]
        assert url == "https://inference.pixelagents.com/v1/chat/completions"
...
class TestRegistration:
    def test_register_both(self):
        ...
        assert set(registered) == {"openrouter", "pixel"}
