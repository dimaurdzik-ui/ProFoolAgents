...
from pixel_cli import lifecycle, plugins as plugin_api
...
""",
    ...
    monkeypatch.setenv("PIXEL_AGENTS_NEMO_RELAY_PLUGINS_TOML", str(plugins_toml))
    return plugins_toml
...
def test_shared_metrics_and_rich_plugin_share_one_core_session(
    ...
    monkeypatch.setenv("PIXEL_AGENTS_NEMO_RELAY_ATIF_ENABLED", "1")
    monkeypatch.setenv(
        "PIXEL_AGENTS_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY", str(tmp_path / "atif")
    )
    monkeypatch.setattr(
    ...
    register_metrics = next(
        ...
        and item[1].startswith("pixel-agents.nemo_relay.shared_metrics.")
    )
    register_atif = next(
    ...
    packages = list(
        (pixel_home / "telemetry" / "shared_metrics" / "outbox").glob("*.json")
    )
    assert len(packages) == 1
    package = json.loads(packages[0].read_text(encoding="utf-8"))
    assert package["metrics"][0]["name"] == "pixel-agents.model_call.count"
    assert package["metrics"][0]["value"] == 1
    assert (tmp_path / "atif" / "pixel-agents-atif-s1.json").exists()
...
def test_relay_tool_request_rewrite_precedes_pixel_authorization_boundary(
    tmp_path,
    monkeypatch,
):
