...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure PIXEL_AGENTS_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", pixel_home=str(tmp_path / ".pixel-agents"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("PIXEL_AGENTS_HOME", str(pixel_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", pixel_home=str(pixel_home))
        return p
