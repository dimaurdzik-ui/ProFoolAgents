...
Run from the PR worktree:
    ...
    cd ~/.pixel-agents/pixel-agents/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.pixel-agents/pixel-agents``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.pixel-agents/pixel-agents
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a pixel-agents checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a pixel-agents checkout"
)
...
# Isolated PIXEL_AGENTS_HOME for the config write.
