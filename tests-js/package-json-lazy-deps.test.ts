 ...
 *
 * The root ``package.json`` is installed by ``pixel-agents update`` on every user,
 ...
 *   ``pixel-agents tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``pixel-agents update`` for
 ...
 * guidance in the ``pixel-agents-dev`` skill before changing the
 * expectations.
 */
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in pixel_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`pixel-agents tools` → Browser Automation → Camofox.'
  )
})
...
test('agent-browser stays eager (default backend)', () => {
  ...
  assert.ok(
    ...
    'agent-browser is the default browser-tool backend used by every ' +
      ...
      'after `pixel-agents setup` / `pixel-agents update` without an explicit ' +
      'post_setup step.'
  )
})
