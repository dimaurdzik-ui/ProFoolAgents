...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_pixel_agents_home() under the active profile.
...
#   - cli.py, pixel_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("PIXEL_AGENTS_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if (
            ...
            # Dispatcher-spawned workers are scoped by PIXEL_AGENTS_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like pixel-agents-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("pixel-agents-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (pixel-agents-*) include _PIXEL_AGENTS_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("pixel-agents-")
    ...
    # window). Core Pixel Agent tools (toolsets._PIXEL_AGENTS_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from pixel_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to pixel-agent's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from pixel_cli.lifecycle import has_hook, invoke_hook
